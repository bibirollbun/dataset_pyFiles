...
# ▼▼▼ 修正点1: kagglehubを使ってデータセットをダウンロードし、パスを取得 ▼▼▼
# この関数が、必要なファイル群が保存されているディレクトリのパスを返してくれます。
print("Downloading dataset using kagglehub...")
dataset_path = kagglehub.dataset_download("hideyukizushi/cmi-d-111")
print("Dataset downloaded to:", dataset_path)

# ▼▼▼ 修正点2: PRETRAINED_DIRとMODEL_DIRを、取得したパスに設定 ▼▼▼
# これにより、ファイルが見つからないエラーが解消されます。
PRETRAINED_DIR = Path(dataset_path)
...
# 1つ目のモデルセットをロード
for fold in range(10):
    # MODEL_DIRは上で設定済みなので、ここの定義は不要
    model_path = MODEL_DIR / f"D-111_{fold}.h5"
    print(">>>LoadModel>>>", str(model_path))
    model = load_model(str(model_path), compile=False, custom_objects=custom_objs)
    models.append(model)
print("-"*50)
...
for fold in range(10):
    # MODEL_DIRは上で設定済みなので、ここの定義は不要
    model_path = MODEL_DIR / f"v0629_{fold}.h5"
    print(">>>LoadModel>>>", str(model_path))
    model = load_model(str(model_path), compile=False, custom_objects=custom_objs)
    models.append(model)
print("-"*50)
