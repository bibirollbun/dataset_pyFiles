...
# Path to your image
img_path = "/kaggle/input/predict-intorverts-from-the-extroverts-challenge/introverts vs extroverts.png"
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    print(f"âœ… Fold Accuracy: {acc:.4f}")



cv_acc = accuracy_score(y, oof_preds)
print(f"ğŸ“Š Cross-Validation Accuracy: {cv_acc:.4f}")
...
# SHAP for tree-based models with Booster
explainer = shap.TreeExplainer(model)  # model is your Booster from xgb.train
shap_values = explainer.shap_values(X)

# 1. Global summary plot
shap.summary_plot(shap_values, X, plot_type="dot", max_display=10)

# 2. Bar chart of mean SHAP values
