...
xgb_test_preds = xgb_best_model.predict_proba(test)[:, 1]
...
lgbm_test_preds = lgbm_best_model.predict_proba(test)[:, 1]
...
# Best params:
#   iterations: 1114
#   learning_rate: 0.09737952098284852
#   depth: 5
#   l2_leaf_reg: 6.7177070633517415
#   min_data_in_leaf: 214
#   subsample: 0.9444461947579571

# Default metric period is 5 because AUC is/are not implemented for GPU

# Final Train AUC: 0.7355
...
cat_test_preds = cat_best_model.predict_proba(test)[:, 1]
...
# plot a CDF graph for each model preds
import numpy as np
import matplotlib.pyplot as plt

def plot_cdf(preds, label):
    preds = np.asarray(preds)
    sorted_preds = np.sort(preds)
    cdf = np.arange(1, len(sorted_preds) + 1) / len(sorted_preds)
    plt.plot(sorted_preds, cdf, label=label, linewidth=2)

plt.figure(figsize=(9, 6))

plot_cdf(xgb_test_preds, "XGBoost")
plot_cdf(lgbm_test_preds, "LightGBM")
plot_cdf(cat_test_preds, "CatBoost")

plt.xlabel("Predicted probability")
plt.ylabel("Cumulative fraction")
plt.title("CDF comparison of model predictions")
plt.legend()
plt.grid(True)
plt.show()


ensemble_preds = ((lgbm_test_preds * 0.6) +  (cat_test_preds * 0.4)) / 2

sub_data = pd.concat([test['id'],pd.Series(ensemble_preds, name ='diagnosed_diabetes')], axis = 1)
