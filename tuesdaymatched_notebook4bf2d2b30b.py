...
pipe_xgb = Pipeline([
    ...
    ('model', xgb.XGBRegressor(max_depth= 12, gamma= 0.2012429557867328, learning_rate= 0.38833070441429834, n_estimators= 1477, reg_lambda= 1.9779861462656556, reg_alpha= 1.075848544947699, colsample_bytree= 0.7969454135399736))
])


pipe_xgb.fit(X_sel, Y)


y_pred_1 = pipe_xgb.predict(X_sel)
rmsle = np.sqrt(np.mean((np.log1p(Y) - np.log1p(y_pred_1))**2))
print(f'RMSLE = {rmsle:.6f}')



df_test['predicted_xgb'] = pipe_xgb.predict(df_test[['Duration', 'Heart_Rate', 'Age', 'Sex', 'Body_Temp', 'Weight', 'Age_Sex_BMI', 'Age_Sex_Heigh', 'Age_Sex', 'Age_Heigh', 'Age_classify']])
df_submission = pd.DataFrame(
        {
            "id": df_test['id'],
            "Calories": df_test['predicted_xgb']
        }
    )
