!cp -r /kaggle/input/autogluon-package/* /kaggle/working/ > /dev/null
!pip install -f --quiet --no-index --find-links='/kaggle/input/autogluon-package' 'autogluon.tabular-1.3.1-py3-none-any.whl' > /dev/null


!cp -r /kaggle/input/scikit-package/* /kaggle/working/ > /dev/null
...
!pip install /kaggle/input/rdkit-2025-3-3-cp311/rdkit-2025.3.3-cp311-cp311-manylinux_2_28_x86_64.whl > /dev/null


!pip install mordred --no-index --find-links=file:///kaggle/input/mordred-1-2-0-py3-none-any/ > /dev/null
...
class Config:
    ...
    pca_variance = 0.9  # Fraction of variance to keep in PCA
...
def get_least_important_features_all_methods(X, y, label, model_name=None):
    ...
    if model_type == 'xgb':
        ...
        model = CatBoostRegressor(iterations=1000, learning_rate=0.07, depth=6, loss_function='MAE', eval_metric='MAE', random_seed=config.random_state, verbose=False)
        model.fit(X_train, y_train, eval_set=(X_valid, y_valid), early_stopping_rounds=50, use_best_model=True)
    elif model_type == 'lgbm':
        model = LGBMRegressor(n_estimators=1000, learning_rate=0.07, max_depth=6, reg_lambda=1.0, objective='mae', random_state=config.random_state, verbose=-1, verbosity=-1)
        model.fit(X_train, y_train, eval_set=[(X_valid, y_valid)], eval_metric='mae', callbacks=[lgb.early_stopping(stopping_rounds=50)])
    elif model_type == 'extratrees':
        # Use ExtraTrees with 300 trees for importance calculations (and same random_state)
        model = ExtraTreesRegressor(n_estimators=300, random_state=config.random_state, criterion='absolute_error', n_jobs=-1)
...
def get_least_important_features(X, y, label, model_name=None):
    ...
    if (model_name or getattr(config, 'model_name', 'xgb')) == 'xgb':
        ...
        # Use ExtraTrees with 300 estimators for feature importance
        model = ExtraTreesRegressor(random_state=config.random_state, criterion='absolute_error', n_estimators=300, n_jobs=-1)
        model.fit(X, y)
...
def train_with_other_models(model_name, label, X_train, y_train, X_val, y_val):
    ...
    if model_name == 'tabnet':
        ...
        # We'll create a small ensemble of ExtraTrees with 5 different seeds and stack them.
        seeds = [Config.random_state + i for i in range(5)]
        et_estimators = []
        et_models = []
        for i, s in enumerate(seeds):
            params = {'n_estimators': 300, 'criterion': 'absolute_error', 'random_state': s, 'n_jobs': -1}
            ...
            et_estimators.append((f'et{i}', et))
            X_all, y_all = combine_train_val(X_train, X_val, y_train, y_val)
            # Fit each base estimator on combined train+val (same behaviour as original single model)
            et.fit(X_all, y_all)
            et_models.append(et)

        # Create a stacking ensemble using the 5 ExtraTrees as base estimators
        final_estimator = ElasticNetCV(l1_ratio=[0.1, 0.5, 0.9], n_alphas=100, max_iter=50000, tol=1e-3, cv=5, n_jobs=-1)
        stacker = StackingRegressor(estimators=et_estimators, final_estimator=final_estimator, passthrough=True, cv=5, n_jobs=-1)
        # Fit stacker on X_all, y_all (already defined above)
        stacker.fit(X_all, y_all)
        Model = stacker
...
def train_with_stacking(label, X_train, y_train):
    ...
    """
    # Create 5 ExtraTrees base estimators with different seeds (n_estimators=300)
    et_seeds = [Config.random_state + i for i in range(5)]
    et_estimators = []
    for i, s in enumerate(et_seeds):
        et_estimators.append((f'et{i}', ExtraTreesRegressor(n_estimators=300, criterion='absolute_error', random_state=s, n_jobs=-1)))

    estimators = [
        # Replace xgb with multiple ExtraTrees as requested; keep other base learners
        *et_estimators,
        ...
        if train_model:
            ...
            if Config.enable_param_tuning:
                if label == 'Tg':
                    ...
                    param_grid = {
                        'n_estimators': [3000],
                        'max_depth': [6, 7, 9, 12, 15],
                        'learning_rate': [0.001, 0.01, 0.06, 0.03],
...
config.model_name = 'extratrees'   # train the extratrees pipeline
# or
config.use_stacking = True        # include ETs as stacking base learners
...
# ensure exactly two dfs
assert len(output_dfs) == 2, "Expected exactly 2 DataFrames in output_dfs for 0.6/0.4 weighting."

df1 = output_dfs[0].set_index('id')
df2 = output_dfs[1].set_index('id')

# align by id (outer join ensures any missing ids are handled)
df_all = df1.join(df2, how='outer', lsuffix='_1', rsuffix='_2')

# compute weighted average for all columns except id
weights = (0.6, 0.4)

# get label columns from one of the dfs
label_cols = [c for c in df1.columns]

# weighted average
weighted = (weights[0] * df_all[[f"{c}_1" for c in label_cols]].values +
            weights[1] * df_all[[f"{c}_2" for c in label_cols]].values)

weighted_df = pd.DataFrame(weighted, index=df_all.index, columns=label_cols)

final_df = weighted_df.reset_index()
final_df['Tg']= final_df['Tg']*(9/5)+32
