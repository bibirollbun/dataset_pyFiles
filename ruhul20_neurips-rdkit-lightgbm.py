...
from sklearn.metrics import f1_score, classification_report, accuracy_score, mean_absolute_error, mean_squared_error
...
def lgb_kfold(train_df, test_df, target, feats, folds):
    ...
    for n_fold, (train_idx, valid_idx) in enumerate(folds.split(train_df, train_df[target])):
        ...
        best = lgb.train(params, dtrain, valid_sets=[dval, dtrain], callbacks=callbacks,)

        #---------- feature_importances ---------#
        feature_importances = sorted(zip(feats, best.feature_importance('gain')), key=lambda x: x[1], reverse=True)
        ...
        oof_preds[valid_idx] = best.predict(valid_x, num_iteration=best.best_iteration)
        # oof_cv = rmse(valid_y,  oof_preds[valid_idx])
        # cv_list.append(oof_cv)
        # print (cv_list)

        sub_preds += best.predict(test_x, num_iteration=best.best_iteration) / n_splits
