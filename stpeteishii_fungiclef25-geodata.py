...
df = pd.read_csv('/kaggle/input/fungi-clef-2025/metadata/FungiTastic-FewShot/FungiTastic-FewShot-Train.csv')
display(df[0:2].T)


df2=df[['latitude','longitude','species']]
print(len(df2))
...
names = df3['species'].unique().tolist()
print(len(names))
names.sort()
...
for i,name in enumerate(names):
    name_df[i]=df3[df3['species']==name].reset_index(drop=True)
