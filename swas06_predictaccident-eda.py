...
features = ['num_lanes', 'curvature', 'speed_limit', 'num_reported_accidents', 'accident_risk']
...
fig, axes = plt.subplots(nrows=5, ncols=1, figsize=(10, 12))  
fig.suptitle('Histplot of Selected Features', fontsize=16)
...
X = df_train_clean.drop('accident_risk', axis='columns')
y = df_train_clean['accident_risk']
...
for feature in X.columns:
    if X[feature].dtype == 'object':
        _, iv = calculate_woe_iv(pd.concat([X, y],axis=1), feature, 'accident_risk' )
    else:
        X_binned = pd.cut(X[feature], bins=10, labels=False)
        _, iv = calculate_woe_iv(pd.concat([X_binned, y],axis=1), feature, 'accident_risk' )
