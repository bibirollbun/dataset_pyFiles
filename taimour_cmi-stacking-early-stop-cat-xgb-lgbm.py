...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y), start=1):
    ...
    model_xgb.set_params(early_stopping_rounds=100)  # Set before fit()
    model_xgb.fit(
        X_train, y_train,
        eval_set=[(X_val, y_val)],
        verbose=False
...
print("================================")
print(f"CatBoost:  {np.mean(cat_best_iters):.1f} (avg from CV) â†’ {cat_optimal} (with buffer)")
print(f"LightGBM:  {np.mean(lgb_best_iters):.1f} (avg from CV) â†’ {lgb_optimal} (with buffer)")
print(f"XGBoost:   {np.mean(xgb_best_iters):.1f} (avg from CV) â†’ {xgb_optimal} (with buffer)")
print("================================")
print("ğŸ’¡ Why buffer?")
