...
for model_idx, params in enumerate(model_params):
    print(f"\nTraining Model {model_idx + 1}")
    model_oof_preds = np.zeros(len(X))  # Out-of-fold predictions for this model
    model_test_preds = np.zeros(len(test))  # Test predictions for this model
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
        ...
        model_oof_preds[val_idx] = model.predict_proba(X_val)[:, 1]

        model_test_preds += model.predict_proba(test)[:, 1] / n_splits

    oof_preds += model_oof_preds / len(model_params)
    test_preds += model_test_preds / len(model_params)

    model_roc_auc = roc_auc_score(y, model_oof_preds)
    print(f"Model {model_idx + 1} ROC-AUC: {model_roc_auc:.5f}")

ensemble_roc_auc = roc_auc_score(y, oof_preds)
print(f"\nEnsemble Mean CV ROC-AUC: {ensemble_roc_auc:.5f}")
