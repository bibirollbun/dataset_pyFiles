...
def make_subm(file_submission, feat_to_drop=''):
    ...
    if feat_to_drop != '' and feat_to_drop in df.columns:
        df     .drop(columns=feat_to_drop, inplace=True)
        df_test.drop(columns=feat_to_drop, inplace=True)
...
for i in range(7):
    make_subm(f"subm_{i}.csv",feats_to_drop[i])
...
def color_scheme(dk,color):
    ...
    clr_Brown3= ["maroon","sienna","chocolate"]
    clr_Brown4= ["maroon","sienna","chocolate","red"]
    ...
    clr_tes6  = ['limegreen'] + clr_Brown3 + ["magenta"]+["darkmagenta"]
...
params = {
      ...
      'subwts'   : [ +0.11,+0.07,+0.03, -0.01,-0.02,-0.06,-0.12 ],
      'subm'     : [
         { 'name': f'0','weight':+0.50 },
         { 'name': f'1','weight':+0.10 },
         { 'name': f'2','weight':+0.10 },
         { 'name': f'3','weight':+0.10 },
         { 'name': f'4','weight':+0.00 },
         { 'name': f'5','weight':+0.10 },
         { 'name': f'6','weight':+0.10 },
      ]
}
...
params = {
      ...
      'id_target': ['id',"loan_paid_back"],          
      'type_sort': ['asc/desc', 0.30,0.70],
      'subwts'   : [ +0.11,+0.07,+0.03, -0.02,-0.06,-0.13 ],
      'subm'     : [
         { 'name': f'0','weight':+0.50 },
         { 'name': f'1','weight':+0.10 },
         { 'name': f'2','weight':+0.10 },
...
df_cross = h_blend(params,color='tes6',figures1=True,figures2=True,details=True)

display_distances(params)


for i in range(7): os.remove(f'{path}{i}.csv')
