...
sns.heatmap(df_train_clean[features + ['loan_paid_back']].corr(), annot=True, cmap='coolwarm',fmt='.2f', linewidths=0.5)
plt.title("Correlation Matrix")
plt.show()


sns.pairplot(df_train_clean[features + ['loan_paid_back']], hue='loan_paid_back', diag_kind='kde', plot_kws={'alpha':0.6})
plt.show()
...
print("✅ All plots generated and saved in separate folders.")


X = df_train_clean.drop('loan_paid_back', axis='columns')
y = df_train_clean['loan_paid_back']
...
for feature in X.columns:
    if X[feature].dtype == 'object':
        _, iv = calculate_woe_iv(pd.concat([X, y],axis=1), feature, 'loan_paid_back' )
    else:
        X_binned = pd.cut(X[feature], bins=10, labels=False)
        _, iv = calculate_woe_iv(pd.concat([X_binned, y],axis=1), feature, 'loan_paid_back' )
