...
for col in compare_cols:
    if col in categorical_variables_tmp:
        ...
        plt.title(f"{col} â€“ train vs test distribution"); plt.show()
...
# # 'Sex'ã‚’ã‚«ãƒ†ã‚´ãƒªå¤‰æ•°ã�¨ã�—ã�¦ç›¸äº’ä½œç”¨ã‚’ç¢ºèª�ã�™ã‚‹ä¾‹
# numerical_variables_tmp = ['Duration', 'Heart_Rate', 'Body_Temp'] # ä¾‹ã�¨ã�—ã�¦

# for col in numerical_variables_tmp:
#     plt.figure(figsize = (10, 6))
#     # ã‚«ãƒ†ã‚´ãƒªå¤‰æ•°ï¼ˆä¾‹: 'Sex'ï¼‰ã�§è‰²åˆ†ã�‘ã�—ã�¦æ•£å¸ƒå›³ã‚’ãƒ—ãƒ­ãƒƒãƒˆ
#     sns.scatterplot(x = df_train[col], y = df_train["Calories"], hue = df_train["Sex"], alpha = 0.6)
#     # å�„ã‚«ãƒ†ã‚´ãƒªã�”ã�¨ã�«å›�å¸°ç›´ç·šã‚’æ��ç”»
...
for n_clusters in range_n_clusters:
    ...
    for i in range(n_clusters):
        # Sort the silhouette values â€‹â€‹of each cluster
...
pca_df_for_plotting['Target_Quintile'] = target_quintiles.reset_index(drop=True) # ã‚¤ãƒ³ãƒ‡ãƒƒã‚¯ã‚¹ã‚’ãƒªã‚»ãƒƒãƒˆã�—ã�¦çµ�å�ˆ
