...
class Parquet_Processor:
    ...
    *Note, found out if you have a dataframe with categorical columns, when saving as parquet and loading the parquet, the
    ...
    def convert(self, extensions: Optional[list[str]] = None, overwrite: Optional[bool] = None):
       ...
       # Dictionary mapping file extensions to pandas read functions
        ...
        for raw_file in raw_files:
            parquet_file = Path(f'/kaggle/working/{raw_file.with_suffix(".parquet").name}')
            ...
            try:
                ...
                optimized_df.to_parquet(parquet_file.name)
    ...
    def find_files_by_extension(self, extensions: list[str]) -> list[Path]:
        """
        Recursively searches for files starting with TEST or TRAIN in the folder and its subdirectories
        that match the provided extensions, excluding hidden directories.
        ...
        for path in folder.glob('*'):
            if path.is_dir() and ("TRAIN_NEW" in path.name or "TEST" in path.name):
    ...
    def merge_train_test_parquets(self, paths: Optional[list[Path]] = None, overwrite: Optional[bool] = None):
        ...
        merged_train_path = Path('/kaggle/working/train_merged.parquet')
        merged_test_path = Path('/kaggle/working/test_merged.parquet')
        ...
        if not df_train.empty:
            df_train.to_parquet(merged_train_path.name)
            print(fr'Successfully merged train files. Saved as {merged_train_path}.')
        if not df_test.empty:
            df_test.to_parquet(merged_test_path.name)
    ...
    def optimize_dataframe(self, df: pd.DataFrame, tolerance: float = .01) -> pd.DataFrame:
        ...
        tolerance : float, optional
            The maximum allowable percentage of information loss (e.g., 0.01 for 1%). Default is .01
...
"""
Reading from csv - 12s; Reading from parquet - 4s; Such efficient!! Much Wow!!
Note: loss of information from converting the connectome from float64 to float32 is minuscule, 
but speed gains are significant. Still, if you'd rather not convert, change the default of
optimize_dataframe to .0.
"""
processor = Parquet_Processor(folder=f"/kaggle/input/widsdatathon2025", overwrite=False)
parquet_paths = processor.convert()
train, test = processor.merge_train_test_parquets(parquet_paths)
