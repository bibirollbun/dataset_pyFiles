...
class TCNClassifier(nn.Module):
    def __init__(self, n_features, n_classes, channels=(64, 64, 128), kernel_size=5, dropout=0.1):
        super().__init__()
        layers = []
        in_ch = n_features
        dilation = 1
...
@torch.no_grad()
def evaluate(model, loader, criterion, n_classes=N_CLASSES):
    """
    Evaluate a model on a DataLoader and compute:
    - Average loss
    - Macro F1 score
    - Accuracy (binary or multi-class)
    ...
    Returns:
        ...
        val_acc: accuracy
        binary_flag: 'Yes' if binary, 'No' otherwise
    """
    model.eval()
    total_loss = 0.0
    n_seen = 0
    ...
    # Binary flag
    binary_flag = 'Yes' if n_classes == 2 else 'No'

    val_loss = total_loss / max(n_seen, 1)
    return val_loss, macro_f1, val_acc, binary_flag
...
def run_experiment(model_name, model_ctor, window_size=50, stride=5,
    ...
    for ep in range(epochs):
        ...
        val_loss, val_f1, val_acc, binary_flag = evaluate(model, train_loader, criterion, n_classes=N_CLASSES)
        ...
        print(f"Epoch {ep+1:02d} | train {train_loss:.4f} | val {val_loss:.4f} | "
          f"F1 {val_f1:.4f} | Acc {val_acc:.4f} | Binary {binary_flag} | {dt:.1f}s")
    ...
    result = {
        ...
        'val_acc': round(val_acc, 6),  # <-- add accuracy
        'history': history
    }
    return result
...
def make_tcn():
    return TCNClassifier(n_features=N_FEATURES, n_classes=N_CLASSES,
                         channels=(64, 64, 128), kernel_size=5, dropout=0.1)
MODEL_FACTORIES = {
    'tcn': make_tcn,}
...
for w in WINDOWS:
    for model_name, ctor in MODELS_TO_RUN.items():
        ...
        if out is not None:
            ...
            # Detect binary classification (2 classes) or multi-class
            binary_flag = 'Yes' if N_CLASSES == 2 else 'No'
            ...
            summary_results.append({
                ...
                'Binary': binary_flag,  # Added Binary flag
...
# Optional: Print binary result summary
binary_results = final_summary_df[['model_name', 'Binary', 'val_acc', 'Macro']]
