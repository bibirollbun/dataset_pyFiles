...
# --------- Optuna Objective (run ONCE, not per fold) ----------
def objective(trial):
    # Use single fold or small validation set for tuning
    train_idx, valid_idx = next(StratifiedKFold(n_splits=2, shuffle=True, random_state=42).split(X, y))
    X_train, X_valid = X.iloc[train_idx].copy(), X.iloc[valid_idx].copy()
    y_train, y_valid = y.iloc[train_idx], y.iloc[valid_idx]

    target_col = "target"
    h2o_train = h2o.H2OFrame(pd.concat([X_train.reset_index(drop=True), y_train.reset_index(drop=True).rename(target_col)], axis=1))
    h2o_valid = h2o.H2OFrame(pd.concat([X_valid.reset_index(drop=True), y_valid.reset_index(drop=True).rename(target_col)], axis=1))

    # categorical handling
    for col in categorical_features:
        if col in h2o_train.columns: h2o_train[col] = h2o_train[col].asfactor()
        if col in h2o_valid.columns: h2o_valid[col] = h2o_valid[col].asfactor()
    h2o_train[target_col] = h2o_train[target_col].asfactor()
    h2o_valid[target_col] = h2o_valid[target_col].asfactor()

    params = {
        "ntrees": trial.suggest_int("ntrees", 300, 600),  # smaller trees for speed
        ...
        "seed": 42,
        "class_sampling_factors": [1.0, 7.0],
    }

    model = H2OGradientBoostingEstimator(**params)
    model.train(x=X_train.columns.tolist(), y=target_col,
                training_frame=h2o_train, validation_frame=h2o_valid)

    return model.auc(valid=True)

# ---------------- Global Optuna Search -----------------
study = optuna.create_study(direction="maximize")
study.optimize(objective, n_trials=15, show_progress_bar=True, n_jobs=-1)  # parallel search

best_params = study.best_params
best_params.update({
    "seed": 42,
    "class_sampling_factors": [1.0, 7.0],
    "ntrees": 600  # retrain with larger trees for final model
})

print("Best params for H2O GBM:", best_params)

# ---------------- CV with Best Params -----------------
n_splits = 3
skf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=42)

oof_h2o = np.zeros(len(X))
test_h2o = np.zeros((len(test_str), n_splits))
roc_scores = []

for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y)):
    print(f"\n===== Fold {fold} (Final CV with best params) =====")
    X_train, X_valid = X.iloc[train_idx].copy(), X.iloc[valid_idx].copy()
    y_train, y_valid = y.iloc[train_idx], y.iloc[valid_idx]
    ...
    for col in categorical_features:
        if col in h2o_train.columns: h2o_train[col] = h2o_train[col].asfactor()
        if col in h2o_valid.columns: h2o_valid[col] = h2o_valid[col].asfactor()
        if col in h2o_test.columns: h2o_test[col] = h2o_test[col].asfactor()
    h2o_train[target_col] = h2o_train[target_col].asfactor()
    h2o_valid[target_col] = h2o_valid[target_col].asfactor()
    ...
    pred_valid = model.predict(h2o_valid).as_data_frame(use_pandas=True)['p1'].values
    pred_test = model.predict(h2o_test).as_data_frame(use_pandas=True)['p1'].values

    oof_h2o[valid_idx] = pred_valid
    test_h2o[:, fold] = pred_test

    score = roc_auc_score(y_valid, pred_valid)
    roc_scores.append(score)
    print(f"Fold {fold} -> ROC-AUC: {score:.5f}")

print(f"\nAverage ROC-AUC: {np.mean(roc_scores):.5f} ± {np.std(roc_scores):.5f}")
final_test_pred_h2o = np.mean(test_h2o, axis=1)
print("Best params for H2O GBM:", best_params)
