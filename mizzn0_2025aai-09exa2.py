...
# store × item の出現回数（販売頻度）
item_store_freq = train_x.groupby(['store', 'item']).size().reset_index(name='store_item_count')
train_x = train_x.merge(item_store_freq, on=['store', 'item'], how='left')
test_x  = test_x.merge(item_store_freq, on=['store', 'item'], how='left')

train_x['store_item_count'] = train_x['store_item_count'].fillna(0)
test_x['store_item_count'] = test_x['store_item_count'].fillna(0)

dow_item_freq = train_x.groupby(['dow', 'item']).size().reset_index(name='dow_item_count')
train_x = train_x.merge(dow_item_freq, on=['dow', 'item'], how='left')
test_x  = test_x.merge(dow_item_freq, on=['dow', 'item'], how='left')

train_x['dow_item_count'] = train_x['dow_item_count'].fillna(0)
test_x['dow_item_count'] = test_x['dow_item_count'].fillna(0)
