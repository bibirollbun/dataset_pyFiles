...
df = pd.read_csv('/kaggle/input/division-b-leaderboard/leaderboard_b.csv')
...
fig = make_subplots(
    rows=1, cols=2,
    subplot_titles=('All Teams Performance', 'Score Distribution'),
...
fig.add_trace(
    go.Bar(
        ...
        text=[f"#{rank}" for rank in df['Rank']],
        textposition='inside'
    ),
    row=1, col=1
)
...
fig.update_layout(
    height=500,
    title_text="Grand X-Ray Slam Division B - Current Standings",
    showlegend=False
)
...
fig.update_layout(
    title='Division B - Team Performance Landscape (Bubble size = Submissions)',
...
timeline_data = {
    ...
    'Date': ['Aug 23', 'Aug 30', '1 Sep', 'Oct 30', 'Nov 1'],
...
fig.update_layout(
    title='Division B - Competition Timeline',
    xaxis_title='Date',
    height=300,
    showlegend=True
)
...
fig_progress.update_layout(
    title="Competition Progress (Day 1 → Day 70)",
    barmode='overlay',
