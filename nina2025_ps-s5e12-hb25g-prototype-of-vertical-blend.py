    ...
    def da(dk,sorting_direction,show_details):
        ...
        if len(wts) > 1 or "subwts2" in dk:
            ...
            def correct(x, cs=cols, wts=wts):                      # different
                i = [x['alls'].index(c) for c in short_name_cols]
                if   0.00000 < x['mx-m'] <= 0.00100: return summa(x,cs,wts[1],i)
                if   0.00100 < x['mx-m'] <= 0.00200: return summa(x,cs,wts[0],i)
...
params = {
      ...
      'subm'     : [
          {'name': f'a', 'weight':+0.315, 'color':'royalblue'},
          {'name': f'c', 'weight':+0.33,  'color':'orange'   },
          {'name': f'd', 'weight':+0.355, 'color':'green'    },],
...
mx_m = pd.read_csv('tida_desc.csv')['mx-m'] # def h_blend(..): 'different':[0,1,2]

sns.set()
plt.figure(figsize=(9, 3))
plt.hist(mx_m, bins=250)
plt.gca().set_facecolor('whitesmoke')
plt.suptitle('mx-m', y=0.85, fontsize=11, c='black')


for file in 'h-blend,tida_desc'.split(','): os.remove(file+'.csv')
