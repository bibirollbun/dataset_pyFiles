...
def train_and_evaluate_kfold(df, test_df, dep_var, cat_names, cont_names, config, layers, epochs, loss, metric, n_splits=5, bs=256):
    ...
    for fold, (train_idx, valid_idx) in enumerate(kf.split(df)):
        ...
        # Train model
        learn.fit_flat_cos(epochs, lr=1e-3, div_final=10000, pct_start=0.15, 
...
# NN
y_pred_test_nn, y_oof_nn = train_and_evaluate_kfold(train_df_original, test_df_original, dep_var, 
...
def train_and_plot_model(model, X, y, test_df, y_test=None, n_splits=5, random_state=26, model_name="catboost"):
    ...
    elif model_name == "xgboost" or model_name == "lgbm":
...
test_preds_cb, oof_preds_cb = train_and_plot_model(catboost_model, train_df, y_train, test_df, model_name="catboost")
...
test_preds_xgb, oof_preds_xgb = train_and_plot_model(xgboost_model, train_df, y_train, test_df, model_name="xgboost")
...
print("Loading files...") 
for c in ['catboost','xgboost','lgbm', 'nn']:
    print(f"=> {c} ")
