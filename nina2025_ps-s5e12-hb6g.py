...
path_To_subms  = '/kaggle/working/submissions/'

if os.path.isdir(path_To_subms): shutil.rmtree(path_To_subms) 
    
os.mkdir(path_To_subms)

shutil.copy('/kaggle/input/ps-s5e12-hb2g/submission.csv', path_To_subms +'hb2g.csv')
shutil.copy('/kaggle/input/ps-s5e12-hb3g/submission.csv', path_To_subms +'hb3g.csv')
shutil.copy('/kaggle/input/ps-s5e12-hb5g/submission.csv', path_To_subms +'hb5g.csv')
...
params = {
      ...
      'subm'     : [
         { 'name': f'hb2g', 'weight':+0.10, 'color':'tomato'},
         { 'name': f'hb3g', 'weight':+0.20, 'color':'limegreen'},
         { 'name': f'hb5g', 'weight':+0.70, 'color':'darkmagenta'},]
}
df_v1 = h_blend(params, details=True)
...
params = {
      'path'     : f'/kaggle/input/03-december-2025-ps-s5e12/',            
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.32,0.68 ],
      'subwts'   : [w/1_000 for w in [10, -3,-7]],
      'subm'     : [
         { 'name': f'2g', 'weight':+0.005, 'color':'tomato'},
         { 'name': f'3g', 'weight':+0.010, 'color':'limegreen'},
         { 'name': f'5g', 'weight':+0.985, 'color':'darkmagenta'},]
}
df_v2 = h_blend(params, details=True)
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.32,0.68 ],
      'subwts'   : [w/5_000 for w in [10, -3,-7]],
      'subm'     : [
         { 'name': f'2g',  'weight':+0.002, 'color':'red'},
         { 'name': f'3g',  'weight':+0.005, 'color':'green'},
         { 'name': f'5g.7','weight':+0.993, 'color':'darkmagenta'},]
}
df_v4 = h_blend(params, details=True)
