...
class ProjectionHead(nn.Module):
    ...
    Tried out nonlinearity (GELU) but not sure if this is actually
    ...
    for epoch in tqdm(range(epochs)):
        for step in ["train", "val"]:
            ...
            if batch_count > 0:
                ...
                print(f"Step {step}, Epoch {epoch+1}/{epochs}, Loss: {epoch_loss:.4f}, Triplet Count: {epoch_triplets}, LR: {current_lr}")
            else:
                print(f"Step {step}, Epoch {epoch+1}/{epochs}, No valid triplets found")
...
proj_head = ProjectionHead(768, dim).to(device)
