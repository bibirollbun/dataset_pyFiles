...
def plot_one(task, ax, i,train_or_test,input_or_output, scale=5):
    ...
    try:
        ...
        font_size     = scale * 33 / np.sqrt(input_matrix.shape[0] * input_matrix.shape[1])
        min_font_size = 5
...
def plot_task(task, scale=5):
    ...
    for i in range(len(task['train'])):     
        plot_one(task, axs[0,i],i,'train','input',  scale=scale)
        plot_one(task, axs[1,i],i,'train','output', scale=scale)            

    axs[0,i+1].axis('off'); axs[1,i+1].axis('off')
    for j in range(len(task['test'])):      
        plot_one(task, axs[0,i+2+j],j,'test','input',  scale=scale)
        plot_one(task, axs[1,i+2+j],j,'test','output', scale=scale)  
    
    if 'solution' in task:    
        axs[0,i+j+3].axis('off'); axs[1,i+j+3].axis('off')        
        for k in range(len(task['solution'])):      
            plot_one(task, axs[0,i+j+4+k],k,'solution','input',  scale=scale)
            plot_one(task, axs[1,i+j+4+k],k,'solution','output', scale=scale)  
...
task_file = '../input/arc-prize-2025/arc-agi_training_challenges.json'
...
task_file = '../input/arc-prize-2025/arc-agi_evaluation_challenges.json'
tasks     = load_tasks(task_file)
tasks_ids = list(tasks.keys())
print(_.pick(tasks, tasks_ids[0]), "\n")
print('len(tasks)', len(tasks))
