# --- Calibration helpers (lightweight, no extra training) ---
from pathlib import Path
import json
import numpy as np

CALIB_PATH = Path('/kaggle/input/caliwj')
DEFAULT_CALIB = {
    'weights': {'A': 0.50, 'B': 0.20, 'C': 0.30},
    'temperature': {'A': 1.0, 'B': 1.0, 'C': 1.0}
}
...
def load_calibration():
    try:
        with open(CALIB_PATH, 'r') as f:
            d = json.load(f)
        d.setdefault('weights', DEFAULT_CALIB['weights'])
        d.setdefault('temperature', DEFAULT_CALIB['temperature'])
        for k in ['A','B','C']:
            d['weights'].setdefault(k, DEFAULT_CALIB['weights'][k])
            d['temperature'].setdefault(k, DEFAULT_CALIB['temperature'][k])
        return d
    except Exception:
...
def predict3(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    tA = float(calib['temperature']['A'])
    tB = float(calib['temperature']['B'])
    tC = float(calib['temperature']['C'])
    # Temperature scaling for A and B (probabilities)
    if tA != 1.0:
        logpA = np.log(np.clip(avg_pred_A, 1e-12, 1.0))
        avg_pred_A = softmax_np(logpA / tA)
    if tB != 1.0:
        logpB = np.log(np.clip(pred_B, 1e-12, 1.0))
        pred_B = softmax_np(logpB / tB)
    # Temperature scaling for C (logits available above)
    avg_pred_C = torch.softmax(avg_pred_C_logits / tC, dim=1).cpu().numpy() if tC != 1.0 else torch.softmax(avg_pred_C_logits, dim=1).cpu().numpy()
