...
use_df['expert_consensus'].value_counts()


# Her bir expert_consensus sınıfı için maksimum 1608 örnek al, daha azsa hepsini al
...
use_df = use_df.drop(index=drop_5_indices)


use_df.value_counts(["expert_consensus"])
...
# TEST_DF ZATEN BELİRLİ TRAİN-VALIDATION BÖLELİM
...
use_df_Buyuk = pd.read_csv("/kaggle/input/hms-dataset-split/use_df.csv")
...
# Bu indeksleri orijinal DataFrame'den çıkar
use_df_Buyuk = use_df_Buyuk.drop(index=drop_5_indices)


use_df_Buyuk.value_counts(["expert_consensus"])
...
model = load_model("model_epoch_06.keras")
...
model = load_model("6873.h5")


# Base modelin son birkaç katmanını unfreeze et
