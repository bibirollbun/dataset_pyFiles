...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    # Store the trained model and best iteration
    ...
    test_predictions += xgb_model.predict(X_test) / n_splits
...
# Plot evaluation metrics by fold
...
plt.subplot(1, 3, 2)
plt.bar(fold_results['fold'], fold_results['mae'])
...
# Distribution of OOF predictions vs actual values
plt.figure(figsize=(12, 5))

plt.subplot(1, 2, 1)
sns.histplot(y, bins=50, alpha=0.7, label='Actual', kde=True)
sns.histplot(oof_predictions, bins=50, alpha=0.7, label='OOF Predicted', kde=True)
plt.title('Distribution: Actual vs OOF Predicted')
plt.xlabel('BeatsPerMinute')
plt.legend()

plt.subplot(1, 2, 2)
sns.boxplot(data=[y, oof_predictions])
plt.xticks([0, 1], ['Actual', 'OOF Predicted'])
plt.title('Boxplot: Actual vs OOF Predicted')
...
print(f"Mean Absolute Error: {oof_mae:.4f}")
print(f"Root Mean Squared Error: {oof_rmse:.4f}")
print(f"R² Score: {oof_r2:.4f}")
print(f"Correlation between actual and OOF: {np.corrcoef(y, oof_predictions)[0,1]:.4f}")
