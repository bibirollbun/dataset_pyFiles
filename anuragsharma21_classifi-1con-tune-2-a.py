...
def augment_data(
    ...
    """
    Oversample every class up to the majority count, reproducibly.
    Each oversampled row gets a list of 2–4 random augmentations.
    """
    # reproducible
    np.random.seed(seed)
    random.seed(seed)
    ...
    balanced_parts = []
    for cls, grp in df.groupby('score'):
        # sample up to max_n (includes original + duplicates)
        reps = grp.sample(n=max_n, replace=True, random_state=seed).copy()
        # assign between min_ops and max_ops augmentations per row
        ops_list = []
        for _ in range(len(reps)):
            k = random.randint(min_ops, max_ops)
            ops = random.sample(augmentations, k)
            ops_list.append(ops)
        reps['augmentation'] = ops_list
        balanced_parts.append(reps)
...
for cond in conditions:
    for fold in folds:
        ...
        df_aug    = augment_data(df_train, augmentations, seed=42)
...
for CONDITION in CONDITIONS:
    ...
    for fold in FOLDS:
        print(f"\n▶ Training fold {fold}")
        IMAGE_ROOT = f'/kaggle/working/{CONDITION}/fold_{fold}'
        LABEL_ROOT = f'/kaggle/working/{CONDITION}_label/fold_{fold}'
...
for CONDITION in CONDITIONS:
    ...
    for fd in fold_dirs:
        ...
        df_train  = (pd.read_csv(train_csv)
                       .assign(subject=lambda d: d.subject.str.replace('_augmented','',regex=False),
