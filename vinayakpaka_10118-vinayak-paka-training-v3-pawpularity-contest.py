...
class args:
    ...
    batch_size = 8
    image_size = 448 # Increased from 384
    epochs = 10
    fold = 0
...
class PawpularModel(nn.Module):
    def __init__(self):
        super().__init__()        
        self.model = timm.create_model("seresnet50", pretrained=True, in_chans=3) 
    ...
    def monitor_metrics(self, outputs, targets, loss):
        ...
        # Calculate RMSE: root of the loss (which is MSE)
        rmse = torch.sqrt(loss) 
        
        # We don't need the 'nan' check here as the tensor should be valid
        # when returned, and Tez handles the rest.
        
        return {"rmse": rmse} # <--- IMPORTANT: Returning a torch.Tensor
...
train_aug = albumentations.Compose(
    [
        # LongestMaxSize and PadIfNeeded handle the image size (now 448)
       ...
       albumentations.ShiftScaleRotate(
        ...
        # Added CoarseDropout for regularization/better generalization
        albumentations.CoarseDropout(max_holes=8, max_height=64, max_width=64, p=0.1),
...
df = pd.read_csv("/kaggle/input/vinayak10118kfoldpetfinder-my-pawpularity-contest/train_5folds.csv")
df.head()
...
for i in range(5):
    ...
    config = TezConfig(
        ...
        # Keep step_scheduler_metric="valid_rmse" to use the updated monitor_metrics
