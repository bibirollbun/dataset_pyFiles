...
# Function to transform survival probability using Nelson-Aalen estimator
...
train["nelson_y"] = transform_survival_probability(train, time_col='efs_time', event_col='efs')

# Plot transformed target y
plt.hist(train.loc[train.efs == 1, "nelson_y"], bins=100, label="efs=1, Yes Event", alpha=0.7)
plt.hist(train.loc[train.efs == 0, "nelson_y"], bins=100, label="efs=0, Maybe Event", alpha=0.7)
plt.xlabel("Transformed Target nelson_y (Cumulative Hazard)")
plt.ylabel("Density")
plt.title("Nelson-Aalen Transformed Target nelson_y using both efs and efs_time.")
plt.legend()
plt.show()
...
RMV = ["ID","efs","efs_time","y", "nelson_y"]
...
oof_xgb_nelson = np.zeros(len(train))
pred_xgb_nelson = np.zeros(len(test))
...
for i, (train_index, test_index) in enumerate(kf.split(train)):
    ...
    y_train = train.loc[train_index,"nelson_y"]
    x_valid = train.loc[test_index,FEATURES].copy()
    y_valid = train.loc[test_index,"nelson_y"]
    x_test = test[FEATURES].copy()

    model_xgb_nelson = XGBRegressor(
        device="cuda",
        max_depth=7,  
    ...
    # INFER OOF
    oof_xgb_nelson[test_index] = model_xgb_nelson.predict(x_valid)
    # INFER TEST
    pred_xgb_nelson += model_xgb_nelson.predict(x_test)

# COMPUTE AVERAGE TEST PREDS
pred_xgb_nelson /= FOLDS
...
y_pred["prediction"] = oof_xgb_nelson
m = score(y_true.copy(), y_pred.copy(), "ID")
print(f"\nOverall CV for XGBoost NelsonAalen =",m)


feature_importance = model_xgb_nelson.feature_importances_
...
y_pred["prediction"] = rankdata(oof_xgb) + rankdata(oof_cat) + rankdata(oof_lgb)\
                     + rankdata(oof_xgb_cox) + rankdata(oof_cat_cox) + rankdata(oof_xgb_nelson)
m = score(y_true.copy(), y_pred.copy(), "ID")
...
sub.prediction = rankdata(pred_xgb) + rankdata(pred_cat) + rankdata(pred_lgb)\
                     + rankdata(pred_xgb_cox) + rankdata(pred_cat_cox) + rankdata(pred_xgb_nelson)
