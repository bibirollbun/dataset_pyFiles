...
test_sub_df.loc[test_sub_df['country'] == 'Kenya', 'country_ratio'] -= 0.0007/3

test_sub_df["num_sold"] = test_sub_df["day_num_sold"] *test_sub_df["day_of_week_ratios"]* test_sub_df["store_ratio"] * test_sub_df["country_ratio"] * test_sub_df["product_ratio"]* 1.03
...
lgbm_results=pd.read_csv("/kaggle/input/forecast-sales-better-lb-score-less-than-0-05115/submission.csv")


submission["num_sold"] = 0.95* lgbm_results['num_sold'] + 0.05  * test_sub_df["num_sold"]
...
for i in range(3): # increase or decrease iterations until find better csv files
    # Blend the results
    submission["num_sold"] = 0.95*lgbm_results['num_sold'] + 0.05 * test_sub_df["num_sold"]
    submission["num_sold"] = submission["num_sold"] + 0.5
    submission["num_sold"] = submission["num_sold"].astype(int)
    
    # Update lgbm_results with the new submission results for the next iteration
    lgbm_results["num_sold"] = submission["num_sold"]

# Save the final submission after 10 iterations
submission.to_csv('final_submission_compare.csv', index=False)
print("Saved final submission to 'final submission comparing with submission.csv'.")
