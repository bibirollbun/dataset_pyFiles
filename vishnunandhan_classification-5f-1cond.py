...
def splitted_data_5fold(condition):
    """
...
class DataPreparationImage:
    def __init__(
        ...
        condition: str,
        csv_directory: str,
        num_folds: int = 5,                           # now defaults to 5 folds
...
FOLDS      = list(range(5))  # five folds: 0,1,2,3,4
...
# Neural Foraminal Training Cell for 5-Fold Splits
...
for fold in FOLDS:
    ...
    # Re-index labels to 0,1,2 (as strings)
...
all_metrics = {
    split: {m: [] for m in ['acc','prec','rec','f1','auc_macro','error']}
...
for fold in FOLDS:
    ...
    for split, gen in [('train', train_gen), ('val', val_gen)]:
        ...
        plt.plot(fpr["macro"], tpr["macro"], linestyle='--',
                 label=f'Macro (AUC={auc_macro:.2f})')
        plt.plot([0,1],[0,1],'k--', alpha=0.3)
        plt.xlabel('FPR'); plt.ylabel('TPR')
        plt.title(f'{CONDITION} | fold {fold} | {split} ROC')
        plt.legend(loc='lower right')
        plt.tight_layout()
        plt.show()

        # print and accumulate
        print(f"  Acc: {acc:.3f}, Err: {err:.3f}, Prec: {prec:.3f}, Rec: {rec:.3f}, F1: {f1:.3f}, Macro AUC: {auc_macro:.3f}")
        all_metrics[split]['acc'].append(acc)
        all_metrics[split]['prec'].append(prec)
        all_metrics[split]['rec'].append(rec)
        all_metrics[split]['f1'].append(f1)
        all_metrics[split]['auc_macro'].append(auc_macro)
        all_metrics[split]['error'].append(err)

    tf.keras.backend.clear_session()

# after all folds: compute and print averages
print(f"\n=== Average metrics for {CONDITION} over {len(FOLDS)} folds ===")
for split in ['train','val']:
    avg = {m: np.mean(all_metrics[split][m]) for m in all_metrics[split]}
    print(f"→ {split.upper()}: "
          f"Acc={avg['acc']:.3f}, Err={avg['error']:.3f}, "
          f"Prec={avg['prec']:.3f}, Rec={avg['rec']:.3f}, "
          f"F1={avg['f1']:.3f}, Macro AUC={avg['auc_macro']:.3f}")
