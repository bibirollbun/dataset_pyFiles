...
# Adjust pandas settings to show all columns and up to 100 rows
...
train_data = pd.read_csv('/kaggle/input/oemc-hackathon-global-precipitation-modeling/train.csv')
test_data = pd.read_csv('/kaggle/input/oemc-hackathon-global-precipitation-modeling/test.csv')

# Show dataset shapes
print("âœ… Training data loaded with shape:", train_data.shape)
print("âœ… Testing data loaded with shape:", test_data.shape)

# Show available columns in training data
print("\nğŸ“Œ Columns in training dataset:", train_data.columns.tolist())
...
print("\nâ„¹ï¸� Training dataset info:")
print(train_data.info())

print("\nâš ï¸� Missing values in training data:")
print(train_data.isnull().sum())

print("\nâš ï¸� Missing values in testing data:")
...
# -------------------------------
# Target variable (precipitation) analysis
# -------------------------------
print("\n" + "="*50)
print("TARGET VARIABLE ANALYSIS")
print("="*50)
print(f"Minimum precipitation: {train_data['PRCP'].min()}")
print(f"Maximum precipitation: {train_data['PRCP'].max()}")
print(f"Average precipitation: {train_data['PRCP'].mean():.2f}")
print(f"Median precipitation: {train_data['PRCP'].median():.2f}")

# Create log-transformed target variable for better modeling
train_data['PRCP_LOG'] = np.log1p(train_data['PRCP'])
...
# Histogram: Original precipitation values
axes[0, 0].hist(train_data['PRCP'], bins=50, edgecolor='black')
axes[0, 0].set_xlabel('Precipitation (mm)')
axes[0, 0].set_ylabel('Frequency')
axes[0, 0].set_title('Distribution of Precipitation (Original Scale)')

# Histogram: Log-transformed precipitation
axes[0, 1].hist(train_data['PRCP_LOG'], bins=50, edgecolor='black')
axes[0, 1].set_xlabel('Log(1 + Precipitation)')
axes[0, 1].set_ylabel('Frequency')
axes[0, 1].set_title('Distribution of Log-Transformed Precipitation')

# Correlation heatmap of selected features with target
selected_features = ['TMIN_GEOM', 'TMAX_GEOM', 'PRCP_CHELSA', 'PRCP_ERA5', 
                     'PRCP_TOTAL_IMERG', 'PRCP_LIQUID_IMERG', 'PRCP_ICE_IMERG']
corr_matrix = train_data[selected_features + ['PRCP_LOG']].corr()
sns.heatmap(corr_matrix, annot=True, fmt='.2f', ax=axes[1, 0], cmap='coolwarm')
axes[1, 0].set_title('Feature Correlation Matrix')

# Scatter plot: ERA5 vs Observed Precipitation
axes[1, 1].scatter(train_data['PRCP_ERA5'], train_data['PRCP'], alpha=0.5, s=1)
...
def create_features(dataframe):
    """
    Generate additional features to improve model performance.
    
    Features include:
    - Temporal features from 'DATE' (year, month, cyclical encoding)
    - Temperature statistics (range and mean)
    - Aggregated precipitation features (mean, std, max, min)
    - Ratios between different precipitation sources
    - Log-transformed precipitation features to reduce skewness
    """
    df = dataframe.copy()
    ...
    if 'DATE' in df.columns:
        ...
        # Encode month as cyclical features for seasonality
    ...
    # -------------------------------
    df['temp_range'] = df['TMAX_GEOM'] - df['TMIN_GEOM']   # Difference between max and min temperature
    df['temp_mean'] = (df['TMAX_GEOM'] + df['TMIN_GEOM']) / 2  # Average temperature
    
    # -------------------------------
    # Aggregated precipitation features
    # -------------------------------
    prcp_sources = ['PRCP_CHELSA', 'PRCP_ERA5', 'PRCP_TOTAL_IMERG']
    df['prcp_mean'] = df[prcp_sources].mean(axis=1)
    df['prcp_std'] = df[prcp_sources].std(axis=1)
    df['prcp_max'] = df[prcp_sources].max(axis=1)
    df['prcp_min'] = df[prcp_sources].min(axis=1)
    
    # -------------------------------
    # Ratios between precipitation sources
    # -------------------------------
    ...
    # -------------------------------
    # Log-transform precipitation columns to reduce skewness
    # -------------------------------
    log_prcp_cols = ['PRCP_CHELSA', 'PRCP_ERA5', 'PRCP_TOTAL_IMERG', 
                     'PRCP_LIQUID_IMERG', 'PRCP_ICE_IMERG']
    for col in log_prcp_cols:
...
print("\nâœ… Features successfully created!")
print(f"Training data shape after feature engineering: {train_data.shape}")
print(f"Testing data shape after feature engineering: {test_data.shape}")
...
base_features = [
    'TMIN_GEOM', 'TMAX_GEOM', 'PRCP_CHELSA', 'PRCP_ERA5', 
    'PRCP_TOTAL_IMERG', 'PRCP_LIQUID_IMERG', 'PRCP_ICE_IMERG'
]

# Features created through feature engineering
engineered_features = [
    'month_sin', 'month_cos', 'temp_range', 'temp_mean',
...
feature_columns = [f for f in feature_columns if f in test_data.columns]

print(f"\nğŸ“Š Using {len(feature_columns)} features for modeling")

# -------------------------------
# Prepare training and test datasets
# -------------------------------
X = train_data[feature_columns]
y = train_data['PRCP_LOG']
...
print(f"âœ… Validation set size: {X_val.shape}")
...
# -------------------------------
# Dictionaries to store trained models and their validation scores
# -------------------------------
trained_models = {}
validation_scores = {}
...
print(f"   âœ… RÂ² Score: {score_lr:.4f}")
trained_models['LinearRegression'] = lr_model
validation_scores['LinearRegression'] = score_lr
...
print(f"   âœ… RÂ² Score: {score_ridge:.4f}")
trained_models['Ridge'] = ridge_model
validation_scores['Ridge'] = score_ridge
...
print(f"   âœ… RÂ² Score: {score_rf:.4f}")
trained_models['RandomForest'] = rf_model
validation_scores['RandomForest'] = score_rf
...
print(f"   âœ… RÂ² Score: {score_xgb:.4f}")
trained_models['XGBoost'] = xgb_model
validation_scores['XGBoost'] = score_xgb
...
print(f"   âœ… RÂ² Score: {score_lgb:.4f}")
trained_models['LightGBM'] = lgb_model
validation_scores['LightGBM'] = score_lgb
...
for model_name, score in sorted(validation_scores.items(), key=lambda x: x[1], reverse=True):
    print(f"{model_name:20s}: RÂ² = {score:.4f}")

# -------------------------------
# Select the best model based on validation RÂ²
# -------------------------------
best_model_name = max(validation_scores, key=validation_scores.get)
best_model = trained_models[best_model_name]
print(f"\nğŸ�† Best model: {best_model_name} with RÂ² = {validation_scores[best_model_name]:.4f}")
...
# Check if the selected model provides feature importance
...
# -------------------------------
# Simple averaging ensemble of selected top models
# -------------------------------
...
for model_name in top_models:
    if model_name in trained_models:
        preds = trained_models[model_name].predict(X_test)
        ensemble_predictions += preds / len(top_models)
        print(f"âœ… Added {model_name} to ensemble")
...
# -------------------------------
# Submission using the best single model
# -------------------------------
...
# Save the single-model submission
submission_best.to_csv('submission_best_model.csv', index=False)
print(f"âœ… Single-model submission saved: submission_best_model.csv")
print(f"ğŸ“Š Submission shape: {submission_best.shape}")
print("\nğŸ”� First few predictions:")
print(submission_best.head())

# -------------------------------
# Submission using ensemble predictions (if available)
# -------------------------------
submission_ensemble = pd.DataFrame({
    'HACKATHON_ID': test_data['HACKATHON_ID'],
    'PRCP_LOG': ensemble_predictions
})

submission_ensemble.to_csv('submission_ensemble.csv', index=False)
print(f"\nâœ… Ensemble submission saved: submission_ensemble.csv")
...
def train_and_evaluate(model, model_name, X_train, y_train, X_val, y_val, use_cv=True):
    ...
    if use_cv:
        cv_score = cross_val_score(model, X_train, y_train, cv=kfold, 
                                   scoring='r2', n_jobs=-1)
        log_message(f"   CV RÂ² Scores: {cv_score}")
        log_message(f"   CV Mean RÂ²: {cv_score.mean():.4f} (+/- {cv_score.std() * 2:.4f})")
    ...
    log_message(f"   Validation RÂ²: {r2:.4f}")
...
log_message(f"\nBest model: {best_model_name} with RÂ² = {scores[best_model_name]:.4f}")
...
log_message(f"Ensemble Validation RÂ²: {ensemble_val_score:.4f}")
...
axes[0].set_title(f'{best_model_name} - Actual vs Predicted (RÂ² = {scores[best_model_name]:.4f})')
...
axes[1].set_title(f'Ensemble - Actual vs Predicted (RÂ² = {ensemble_val_score:.4f})')
...
log_message(f"Best Model: {best_model_name} (RÂ² = {scores[best_model_name]:.4f})")
log_message(f"Ensemble RÂ² on Validation: {ensemble_val_score:.4f}")
log_message(f"\nDirectory Structure:")
