...
def h_blend(path, fs_names, params):
    ...
    def da(dk,sorting_direction):
        ...
        def alls_r(x, sd=sorting_direction,cs=cols):
            reverse = True if sd=='desc' else False
            tes = {c: x[c] for c in cs}.items()
        ...
        def correct(x, cs=cols, w=weights, cw=corrects):
            ...
            return sum(cS)
        df_subms['alls']       = df_subms.apply(lambda x: alls_r (x), axis=1)
...
path = '/kaggle/input/17-september-2025/h-blend.'

path1, path2, path3 = path+'1/', path+'2/', path+'3/',

randoms1 = [pd.read_csv(path1 + f'random.1.{i+1}.csv') for i in range(7)]
randoms2 = [pd.read_csv(path2 + f'random.2.{i+1}.csv') for i in range(7)]
randoms3 = [pd.read_csv(path3 + f'random.3.{i+1}.csv') for i in range(7)]

dfss = pd.read_csv('/kaggle/input/playground-series-s5e9/sample_submission.csv')

def avg(rs,dfss=dfss.copy(),t="BeatsPerMinute"):
    dfss[t] = sum([r[t] for r in rs]) / 7
    return dfss

submission_hb1 = avg(randoms1); display(submission_hb1)
submission_hb2 = avg(randoms2); display(submission_hb2)
submission_hb3 = avg(randoms3); display(submission_hb3)

submission_hb1.to_csv('submission.csv',index=False)
