...
params = {
      ...
      'subwts'   : [e/300 for e in [7,4, -1,-3,-7]],
      'subm'     : [
         { 'name': f'773.10','weight':+0.17, 'color':'darkmagenta'  },
         { 'name': f'773.12','weight':+0.19, 'color':'mediumblue'   },
         { 'name': f'773.6', 'weight':+0.20, 'color':'darkgreen'},
         { 'name': f'773.8', 'weight':+0.21, 'color':'crimson'   }, 
         { 'name': f'773.9', 'weight':+0.23, 'color':'maroon' },]
}

df = h_blend(params, fig1=True,fig2=True)

def voting(df_HB, rem_left=0,rem_right=1):
    print(f'\nVoting:\n')
    df_Top = pd.read_csv('/kaggle/input/24-november-2025-ps-s5e11/773.10.csv')
    target = 'loan_paid_back'   
    tida_desc = pd.read_csv('/kaggle/working/tida_desc.csv')
    df_soluts = tida_desc[tida_desc.columns[2:7]]
    display(df_soluts)
    ...
    df_vote[target] = df_Top[target]*0.66+0.33*df_vote[target]
    print(f'\nComparison: df_Top _x  vs  df_vote _y\n')

    df_HB = df_HB.rename(columns={'loan_paid_back':'h-blend'})
    df_Top = df_Top.rename(columns={'loan_paid_back':'Top'})
    df_vote = df_vote.rename(columns={'loan_paid_back':'Vote'})
    
    df_comparison = pd.merge(df_Top, df_vote, on='id')
    df_comparison = pd.merge(df_comparison, df_HB, on='id')
    df_comparison["delta_back's"] = df_Top['Top'] - df_vote["Vote"]
    
    display  (df_comparison)
    print(sum(df_comparison["delta_back's"]),
              df_comparison["delta_back's"][df_comparison["delta_back's"] < 0].sum())
    
    return df_vote,df_comparison
