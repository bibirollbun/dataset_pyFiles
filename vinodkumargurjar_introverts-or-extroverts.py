...
def data_preprocessing_pipeline(df_train, df_test, target_column='Personality'):
    """
    ...
    for column in df_train.columns:
        # Skip encoding the target column (we handle it separately)
    ...
    # Encode test set using the same encoders
    for column in df_test.columns:
        if column in label_encoders:
            ...
            if df_test[column].dtype == 'object':
                df_test[column] = -1  # unseen object-type column


    return df_train, df_test


df_train,df_test=data_preprocessing_pipeline(df_train, df_test, target_column='Personality')
...
y1 = target_encoder.fit_transform(y1)
...
cat_model = CatBoostClassifier(n_estimators=1000, verbose=100,random_state=42)
...
cat_model_scalled = CatBoostClassifier(n_estimators=1000, verbose=100,random_state=42)
cat_model_scalled.fit(X1_train, y1_train)


from sklearn.metrics import accuracy_score
cat_pred_scalled = cat_model_scalled.predict(X1_test)
print("Accuracy Score catboost scalled data ",accuracy_score(y1_test,cat_pred_scalled))


from xgboost import XGBClassifier
xgb_model = XGBClassifier(n_estimators=1000, use_label_encoder=False, 
...
lgbm_model = LGBMClassifier(
    ...
    random_state=42,scale_pos_weight=scale_pos_weight, verbose=-1
)
...
# Accuracy
print("Accuracy Score Random Forest is", accuracy_score(y_test, rfc_pred))
...
print(classification_report(y_test, cat_pred, target_names=target_encoder.classes_))



final_prediction=cat_model.predict(df_test)


final_prediction


original_prediction=target_encoder.inverse_transform(final_prediction)
...
sample_submission['Personality'] = original_prediction
