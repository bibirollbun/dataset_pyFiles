...
original.dropna(subset=['Listening_Time_minutes'],inplace=True)


train = pd.concat([original, df])
...
Lg_model = LGBMRegressor(device='gpu', random_state=42)
train(Lg_model, X, y)

features = pd.Series(Lg_model.feature_importances_, index=X.columns).sort_values(ascending=False)
features.plot(kind='bar', title='LightGBM Feature Importance')
plt.show()


from catboost import CatBoostRegressor
cat_model = CatBoostRegressor(task_type='GPU',devices='0',verbose=0, random_state=42)
train(cat_model, X, y)

features = pd.Series(cat_model.feature_importances_, index=X.columns).sort_values(ascending=False)
...
best_params = {
    "n_estimators": 636,
    "max_depth": 15,
    "learning_rate": 0.049898968910627894,
    "subsample": 0.925083157017485,
    "colsample_bytree": 0.7064804989361764,
    "gamma": 1.637735460523304,
    "reg_alpha": 1.9756671399497445,
    "reg_lambda": 1.6619406729036772,
    "random_state": 42,
