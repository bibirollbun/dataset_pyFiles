...
df['file'] = df['file'].apply(lambda x: img_path + x)


df['label'] = df['glasses'].apply(lambda x: 'glasses' if x == 1 else 'no_glasses')
df.head()


df.rename(columns={'glasses': 'encoded_label'}, inplace=True)
df.rename(columns={'file': 'img'}, inplace=True)


selected_images = df.groupby('encoded_label', as_index=False).apply(lambda x: x.sample(n=3, random_state=1)).reset_index(drop=True)
...
model.save('Glass_Detection.keras')
...
df_test['file'] = df_test['file'].apply(lambda x: img_path + x)


df_test.rename(columns={'file': 'img'}, inplace=True)
