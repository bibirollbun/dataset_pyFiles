...
def engineer_solana_features(df: pd.DataFrame) -> pd.DataFrame:
    """
    Solana token işlem özet verilerini içeren DataFrame'e yeni özellikler ekler.

    Args:
        df (pd.DataFrame): Her satırın bir token'ı temsil ettiği ve 
                           sağlanan 51 özelliği sütun olarak içeren DataFrame.

    Returns:
        pd.DataFrame: Yeni türetilmiş özelliklerin eklendiği DataFrame.
    """
    
    df_eng = df.copy()
    epsilon = 1e-9 # Sıfıra bölme hatalarını önlemek için küçük bir değer

    # --- Oranlar ---
    # İşlem Sayısı Oranları
    ...
    # Zaten var olan buy_sell_count_ratio yerine alternatif (varsa)
    ...
    # Yaratıcı (Creator) Oranları
    df_eng['creator_trade_ratio'] = df_eng['creator_trade_count'] / (df_eng['tx_idx_count'] + epsilon)
    # Yaratıcının işlem yapıp yapmadığı zaten 'creator_traded' olarak var gibi görünüyor.
    # Belki yaratıcının alım/satım oranları eklenebilir, ancak bu daha detaylı veri gerektirir.

    # --- Farklar ve Aralıklar (Spreads) ---
    df_eng['quote_amount_range'] = df_eng['quote_coin_amount_max'] - df_eng['quote_coin_amount_mean'] # Ortalama ile max arasındaki fark
    df_eng['base_amount_range'] = df_eng['base_coin_amount_max'] - df_eng['base_coin_amount_mean']   # Ortalama ile max arasındaki fark
    ...
    # --- Volatilite / Değişkenlik Ölçümleri (Coefficient of Variation) ---
    ...
    # --- Zamanlama ve Yoğunluk ---
    # 'tx_per_slot' zaten var, alternatifler düşünülebilir.
    ...
    # --- Etkileşimler (Dikkatli Kullanın) ---
    # Örnek: Toplam hacmin benzersiz cüzdan sayısıyla etkileşimi
    # df_eng['volume_wallet_interaction'] = df_eng['quote_coin_amount_sum'] * df_eng['signing_wallet_nunique']
    # Örnek: Alış hacmi ile alıcı sayısının etkileşimi
    # df_eng['buy_volume_buyer_interaction'] = df_eng['buy_quote_coin_amount_sum'] * df_eng['buy_signing_wallet_nunique']

    # Sonsuz değerleri NaN ile değiştir (bölme işleminden kaynaklanabilir)
    df_eng.replace([np.inf, -np.inf], np.nan, inplace=True)

    return df_eng

combined_df = engineer_solana_features(combined_df)
...
for fold, (train_idx, val_idx) in enumerate(skf.split(X, y)):
    ...
    model = cb.CatBoostClassifier(
        iterations=2700, # High number, use early stopping
        learning_rate=0.012,
...
final_test_preds = cb_test_preds #lgb_test_preds*.5 + cb_test_preds*.3 + xgb_test_preds*.2
