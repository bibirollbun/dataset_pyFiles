...
class ResNetEncoder(nn.Module):
    def __init__(self, architecture='resnet18', pretrained=False):
        ...
        try:
            ...
            print(f"Loading {architecture} without pretrained weights...")
            if architecture == 'resnet18':
...
class VGGFace2Dataset(Dataset):
    def __init__(self, root_dir, max_images=40000, transform=None):
    ...
    def _load_dataset(self):
        """Load VGGFace2 dataset with equal sampling from each identity folder"""
        ...
        identity_folders = sorted([d for d in os.listdir(self.root_dir) 
                                 if os.path.isdir(os.path.join(self.root_dir, d))])
        
        print(f"Found {len(identity_folders)} identity folders")
        
        if len(identity_folders) == 0:
            raise ValueError("No identity folders found in the dataset")
        
        # Calculate images per identity to reach max_images total
        images_per_identity = self.max_images // len(identity_folders)
        remainder = self.max_images % len(identity_folders)
        
        print(f"Target: {self.max_images} images total")
        print(f"Sampling {images_per_identity} images per identity (with {remainder} identities getting 1 extra)")
        ...
        for idx, identity_folder in enumerate(identity_folders):
            ...
            # Get all valid images in this identity folder
            identity_images = []
            for file in os.listdir(identity_path):
                if file.lower().endswith(valid_extensions):
                    identity_images.append(os.path.join(identity_path, file))
            
            # Determine how many images to sample from this identity
            target_count = images_per_identity
            if idx < remainder:  # First 'remainder' identities get one extra image
                target_count += 1
            
            # Sample images from this identity
            if len(identity_images) > 0:
                # If identity has fewer images than target, take all
                sample_count = min(target_count, len(identity_images))
                
                # Randomly sample images
                sampled_images = random.sample(identity_images, sample_count)
                
                # Add to dataset
                self.image_paths.extend(sampled_images)
                # Use identity index as label (not really needed for self-supervised learning)
                self.labels.extend([idx] * len(sampled_images))
                
                total_loaded += len(sampled_images)
        
        print(f"Successfully loaded {total_loaded} images from {len(identity_folders)} identities")
...
class SimCLR(nn.Module):
    def __init__(self, architecture='resnet18', projection_dim=128):
        super(SimCLR, self).__init__()
        self.encoder = ResNetEncoder(architecture=architecture, pretrained=False)
...
def pretrain_simclr(data_dir, epochs=50, batch_size=128, learning_rate=0.0003, save_dir='/kaggle/working/', architecture='resnet34'):
    ...
    dataset = VGGFace2Dataset(data_dir, transform=transform)
    ...
    try:
        model = SimCLR(architecture=architecture)  # Pass architecture parameter
        print(f"Successfully loaded SimCLR model with {architecture}")
    except Exception as e:
        print(f"Error loading model: {e}")
        print(f"Falling back to non-pretrained {architecture}...")
...
def main_skip_pretrain():
    """Main function to run fine-tuning only with raw ResNet18 encoder"""
    print("="*60)
    print("SIMCLR PIPELINE: FINE-TUNING WITH RAW RESNET18 ENCODER")
    print("="*60)
    ...
    # Create raw ResNet50 encoder (no pre-training)
    print("\n" + "="*50)
    print("CREATING RAW RESNET50 ENCODER (NO PRE-TRAINING)")
    print("="*50)
    
    raw_encoder = ResNetEncoder(architecture=config['architecture'], pretrained=True)
    print(f"Created raw {config['architecture']} encoder with random weights")
    
    # Phase: Supervised fine-tuning on RAF-DB
    print("\n" + "="*50)
    print("PHASE: SUPERVISED FINE-TUNING ON RAF-DB")
    print("="*50)
...
if __name__ == "__main__":
    final_model = main_skip_pretrain()
