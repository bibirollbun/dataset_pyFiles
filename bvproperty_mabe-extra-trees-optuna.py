...
# Added full GPU support and extra trees/feats
# KAGGLE OPTIMIZATIONS: Enhanced GPU detection, resource monitoring, memory management
...
# Kaggle-specific imports for monitoring
try:
    import psutil
    HAS_PSUTIL = True
except ImportError:
...
try:
    import subprocess
    HAS_GPUSTAT = subprocess.run(['which', 'gpustat'], capture_output=True).returncode == 0
except:
    HAS_GPUSTAT = False
...
KAGGLE_INPUT_DIR = '/kaggle/input/MABe-mouse-behavior-detection'

# Ensure working directory exists
os.makedirs(KAGGLE_WORKING_DIR, exist_ok=True)
os.makedirs(f'{KAGGLE_WORKING_DIR}/optuna_studies', exist_ok=True)
os.makedirs(f'{KAGGLE_WORKING_DIR}/model_checkpoints', exist_ok=True)
os.makedirs(f'{KAGGLE_WORKING_DIR}/feature_cache', exist_ok=True)

# ===== ENHANCED GPU DETECTION =====
def check_gpu_availability():
    """Enhanced GPU detection for Kaggle environment"""
    ...
    use_gpu = is_kaggle and (has_nvidia_smi or has_cuda or has_torch_cuda)
    
    if use_gpu and has_nvidia_smi:
        try:
            import subprocess
...
print('Imports complete. GPU detection and monitoring ready.')
...
def _make_lgbm(**kw):
    ...
    # Kaggle optimization: Enable GPU memory management
    if USE_GPU:
        kw.setdefault("gpu_platform_id", 0)
        kw.setdefault("gpu_device_id", 0)
...
def _make_xgb(**kw):
    ...
    # Kaggle optimization: GPU memory management
    if USE_GPU:
        kw.setdefault("max_bin", 256)  # Optimize for GPU memory
...
def _make_cb(**kw):
    ...
    if USE_GPU:
        ...
        # Kaggle optimization: Reduce GPU memory usage
        kw.setdefault("border_count", 64)  # Lower than default for memory
    else:
...
clear_gpu_memory()
print('Model factory functions ready with GPU optimizations.')
...
# ===== Cell 4 =====
# Install required packages for Kaggle optimizations
!pip install -q optuna optuna-dashboard psutil
...
# KAGGLE OPTIMIZATION: Use /kaggle/working for persistence across sessions
import optuna
...
STUDY_NAME = 'mabe_hyperopt'
N_TRIALS = 200
TIMEOUT = 3600

# KAGGLE: Use persistent storage in /kaggle/working
study_dir = Path(f'{KAGGLE_WORKING_DIR}/optuna_studies')
study_dir.mkdir(parents=True, exist_ok=True)

# Optuna logging callback with memory tracking
def optuna_callback(study, trial):
    """Callback to log Optuna study progress with resource monitoring"""
    if trial.number % 10 == 0:
        mem_str = f" | Memory: {get_memory_usage():.1f} MB" if HAS_PSUTIL else ""
        print(f"Trial {trial.number}: value={trial.value:.4f}{mem_str}")
        if trial.number % 50 == 0:
            # Periodic checkpoint
            checkpoint_path = study_dir / f"{STUDY_NAME}_checkpoint_{trial.number}.pkl"
            joblib.dump(study, checkpoint_path)
            print(f"  Checkpoint saved: {checkpoint_path}")

print(f'Optuna setup complete. Study directory: {study_dir}')
print('Ready for hyperparameter optimization with persistent storage.')



# ===== Cell 6 =====
# Optuna Hyperparameter Optimization for MABE Model
# This objective function optimizes LightGBM hyperparameters using cross-validation

# NOTE: Before running this, you need to prepare your data:
# 1. Load train data and prepare features (X_tr) and labels (y_tr)
# 2. Prepare groups for GroupKFold (e.g., video_id groups)
# 3. Uncomment and customize the code below

def objective(trial: Trial) -> float:
    """
    Optuna objective function for MABE LightGBM hyperparameter optimization.
    Uses GroupKFold cross-validation to evaluate hyperparameters.
    
    Args:
        trial: Optuna trial object
        
    Returns:
        Average CV score (logloss) to minimize
    """
    ...
    # 
    # # Prepare data (adjust based on your data structure)
    # # X_tr should be your feature DataFrame
    # # y_tr should be your labels (single action column for binary classification)
    # # groups should be video_id or similar grouping variable
    # 
    # # Example: Optimize for a specific action
    # action = 'groom'  # Change to the action you want to optimize
    # if action not in y_tr.columns:
    #     return 1.0  # Return high loss if action not found
    # 
    # y_action = y_tr[action].values
    # mask = ~pd.isna(y_action)
    # X_action = X_tr[mask]
    # y_action_clean = y_action[mask].astype(int)
    # groups_action = groups[mask] if hasattr(groups, '__getitem__') else groups
    # 
    # # Skip if insufficient positive samples
    # if y_action_clean.sum() < 5:
    #     return 1.0
    # 
    ...
    # model = _make_lgbm(**params, verbose=-1)
    # 
    # # Use StratifiedSubsetClassifier wrapper
    # n_samples = min(50000, len(X_action))  # Limit samples for faster optimization
    # wrapped_model = StratifiedSubsetClassifier(model, n_samples=n_samples, random_state=SEED)
    # 
    # # Cross-validation with logloss
    # try:
    ...
    #         error_score='raise'
    #     )
    #     # Return negative of mean (since we minimize, and neg_log_loss is negative)
    ...
    # Placeholder return (replace with actual evaluation above)
    return 0.0


# Create or load study
# KAGGLE: Use persistent storage in /kaggle/working
study_storage = f"sqlite:///{study_dir / f'{STUDY_NAME}.db'}"
...
try:
    ...
    print(f"Best value so far: {study.best_value:.4f}" if study.best_trial else "No completed trials yet")
except:
    # Create new study
    study = optuna.create_study(
        study_name=STUDY_NAME,
        storage=study_storage,
        direction='minimize',  # Change to 'maximize' if optimizing for accuracy/F1/etc.
        sampler=TPESampler(seed=SEED, n_startup_trials=10),  # KAGGLE: More startup trials for better exploration
        pruner=MedianPruner(n_startup_trials=5, n_warmup_steps=10),
        load_if_exists=True
    )
    print(f"Created new study: {STUDY_NAME}")

print(f"Study storage: {study_storage}")

# KAGGLE: Only run optimization if objective function is implemented
# Uncomment below when you've implemented the objective function above
# print(f"Starting optimization with {N_TRIALS} trials (timeout: {TIMEOUT}s)...")
# print_memory_usage("Before optimization")
# study.optimize(
#     objective,
#     n_trials=N_TRIALS,
#     timeout=TIMEOUT,
...
def get_best_params(study_name: str = None) -> dict:
    """Get best parameters from the study"""
    if study_name is None:
        study_name = STUDY_NAME
    # KAGGLE: Use persistent storage path
    study_storage = f"sqlite:///{study_dir / f'{study_name}.db'}"
    study = optuna.load_study(study_name=study_name, storage=study_storage)
    return study.best_params


def plot_optimization_history(study_name: str = None):
...
def plot_param_importances(study_name: str = None):
...
def plot_parallel_coordinate(study_name: str = None):
...
print('Optuna helper functions loaded')
...
# ===== Cell 9 =====
# ==================== DATA LOADING ====================
# KAGGLE OPTIMIZATION: Memory-efficient loading, optimized dtypes, progress tracking

print("Loading train and test data...")
print_memory_usage("Before loading")

# KAGGLE: Optimize CSV reading with dtype specification and low_memory=False
train = pd.read_csv(f'{KAGGLE_INPUT_DIR}/train.csv', low_memory=False)
...
train['n_mice'] = (4 - train[['mouse1_strain', 'mouse2_strain', 'mouse3_strain', 'mouse4_strain']].isna().sum(axis=1)).astype('int8')

# KAGGLE: Optimize dtypes for memory efficiency
for col in ['lab_id', 'video_id', 'mouse1_condition', 'mouse2_condition', 'mouse3_condition', 'mouse4_condition',
            'mouse1_strain', 'mouse2_strain', 'mouse3_strain', 'mouse4_strain',
            'mouse1_sex', 'mouse2_sex', 'mouse3_sex', 'mouse4_sex', 'arena_shape']:
    if col in train.columns:
        train[col] = train[col].astype('category')

test = pd.read_csv(f'{KAGGLE_INPUT_DIR}/test.csv', low_memory=False)
test['sleeping'] = (
    test['lab_id'].astype(str).str.contains('MABe22', na=False) &
    test['mouse1_condition'].astype(str).str.lower().eq('lights on')
).astype('bool')
test['n_mice'] = (4 - test[['mouse1_strain','mouse2_strain','mouse3_strain','mouse4_strain']].isna().sum(axis=1)).astype('int8')

# Optimize test dtypes
for col in ['lab_id', 'video_id', 'mouse1_condition', 'mouse2_condition', 'mouse3_condition', 'mouse4_condition',
            'mouse1_strain', 'mouse2_strain', 'mouse3_strain', 'mouse4_strain',
            'mouse1_sex', 'mouse2_sex', 'mouse3_sex', 'mouse4_sex', 'arena_shape']:
...
def generate_mouse_data(dataset, traintest, traintest_directory=None,
    ...
    if traintest_directory is None:
        traintest_directory = f"{KAGGLE_INPUT_DIR}/{traintest}_tracking"
    ...
    # KAGGLE: Add progress bar for data generation
    iterator = tqdm(dataset.iterrows(), total=len(dataset), desc=f"Loading {traintest} videos", leave=False)
    for _, row in iterator:
        ...
        # ---- tracking ----
        # KAGGLE: Use pyarrow engine for faster parquet reading
        path = f"{traintest_directory}/{lab_id}/{video_id}.parquet"
        try:
            vid = pd.read_parquet(path, engine='pyarrow')
        except:
        ...
        del vid
        gc.collect()  # KAGGLE: Explicit cleanup
        ...
        if traintest == 'train':
            try:
                # KAGGLE: Use pyarrow for faster annotation reading
                annot_path = path.replace('train_tracking', 'train_annotation')
                annot = pd.read_parquet(annot_path, engine='pyarrow')
        ...
        def _mk_meta(index, agent_id, target_id):
            # KAGGLE: Optimize dtypes - use int32 for video_frame (sufficient for frame numbers)
            m = pd.DataFrame({
                'lab_id':        lab_id,
                'video_id':      video_id,
                'agent_id':      agent_id,
                ...
                'sleeping':      sleeping.astype('bool') if isinstance(sleeping, (bool, np.bool_)) else bool(sleeping),
...
def submit_ensemble(body_parts_tracked_str, switch_tr, X_tr, label, meta, n_samples=1_500_000):
    ...
    model_list = []
    # KAGGLE: Progress bar for action training
    action_pbar = tqdm(label.columns, desc=f"Training {switch_tr} models", leave=False)
    for action in action_pbar:
        action_pbar.set_description(f"Training {switch_tr} - {action}")
        ...
        for model_idx, m in enumerate(models):
            ...
            try:
                ...
                dt = perf_counter() - t0
                mem_str = f" | Mem: {get_memory_usage():.1f}MB" if HAS_PSUTIL else ""
                print(f"trained model {model_names[model_idx]} | {switch_tr} | action={action} | {dt:.1f}s{mem_str}", flush=True)
            except Exception:
                ...
                try:
                    ...
                    dt = perf_counter() - t0
                    mem_str = f" | Mem: {get_memory_usage():.1f}MB" if HAS_PSUTIL else ""
                    print(f"trained (CPU fallback) {model_names[model_idx]} | {switch_tr} | action={action} | {dt:.1f}s{mem_str}", flush=True)
                except Exception as e2:
                    print(e2)
                    continue
            trained.append(m_clone)
            # KAGGLE: Clear GPU memory after each model
            clear_gpu_memory()

        if trained:
            model_list.append((action, trained))
            # KAGGLE: Optional model checkpointing (commented out to save space)
            # checkpoint_dir = Path(f'{KAGGLE_WORKING_DIR}/model_checkpoints/{switch_tr}/{action}')
            # checkpoint_dir.mkdir(parents=True, exist_ok=True)
            # for idx, mdl in enumerate(trained):
            #     joblib.dump(mdl, checkpoint_dir / f'{model_names[idx]}.pkl')
    ...
    # KAGGLE: Progress tracking for test inference
    test_count = len(test_subset)
    for switch_te, data_te, meta_te, actions_te in tqdm(generator, desc=f"Test inference {switch_tr}", total=test_count, leave=False):
...
def robustify(submission, dataset, traintest, traintest_directory=None):
    # KAGGLE: Use configured input directory
    if traintest_directory is None:
        traintest_directory = f"{KAGGLE_INPUT_DIR}/{traintest}_tracking"
    ...
    for _, row in dataset.iterrows():
        ...
        path = f"{traintest_directory}/{lab_id}/{video_id}.parquet"
        # KAGGLE: Use pyarrow for faster reading
        try:
            vid = pd.read_parquet(path, engine='pyarrow')
        except:
            vid = pd.read_parquet(path)
...
# ===== Cell 16 =====
# ==================== MAIN LOOP ====================
# KAGGLE OPTIMIZATION: Progress tracking, memory management, timing
...
# KAGGLE: Add progress bar for main loop
main_pbar = tqdm(range(len(body_parts_tracked_list)), desc="Processing body part groups")

for section in main_pbar:
    body_parts_tracked_str = body_parts_tracked_list[section]
    try:
        t0_section = perf_counter()
        body_parts_tracked = json.loads(body_parts_tracked_str)
        main_pbar.set_description(f"Processing {len(body_parts_tracked)} body parts")
        ...
        # KAGGLE: Progress tracking for data generation
        ...
        if len(single_list) > 0:
            print(f"  Processing {len(single_list)} single mouse sequences...")
            single_feats_parts = []
            # KAGGLE: Progress bar for feature engineering
            for data_i, meta_i in tqdm(zip(single_list, single_meta_list), 
                                       total=len(single_list), 
                                       desc="  Single features", 
                                       leave=False):
            ...
            gc.collect()
            print_memory_usage(f"  After single feature engineering")
            ...
            clear_gpu_memory()
            gc.collect()
            print_memory_usage(f"  After single training")

        if len(pair_list) > 0:
            print(f"  Processing {len(pair_list)} pair sequences...")
            pair_feats_parts = []
            # KAGGLE: Progress bar for feature engineering
            for data_i, meta_i in tqdm(zip(pair_list, pair_meta_list), 
                                       total=len(pair_list), 
                                       desc="  Pair features", 
                                       leave=False):
                fps_i = _fps_from_meta(meta_i, _fps_lookup, default_fps=30.0)
            ...
            gc.collect()
            print_memory_usage(f"  After pair feature engineering")
            ...
            clear_gpu_memory()
            gc.collect()
            print_memory_usage(f"  After pair training")

        dt_section = perf_counter() - t0_section
        print(f"  Section {section} completed in {dt_section:.1f}s")
...
# KAGGLE: Save to working directory for persistence
submission_path = f'{KAGGLE_WORKING_DIR}/submission.csv'
submission_robust.to_csv(submission_path, index=True)
print(f"\nSubmission created: {len(submission_robust)} predictions")
print(f"Saved to: {submission_path}")

# Also save to root for Kaggle submission
