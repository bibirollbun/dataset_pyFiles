...
def find_optimal_ridge_alpha(X, y, alphas=None, cv=5, scoring='neg_mean_squared_error', random_state=42):
    """
    é€šè¿‡äº¤å�‰éªŒè¯�æ‰¾åˆ°æœ€ä¼˜çš„å²­å›�å½’æ­£åˆ™åŒ–ç³»æ•°(alpha)
    
    å�‚æ•°:
    - X: ç‰¹å¾�çŸ©é˜µ (numpyæ•°ç»„æˆ–pandas DataFrame)
    - y: ç›®æ ‡å�˜é‡� (numpyæ•°ç»„æˆ–pandas Series)
    - alphas: è¦�æµ‹è¯•çš„alphaå€¼åˆ—è¡¨ï¼Œé»˜è®¤ä½¿ç”¨å¯¹æ•°ç©ºé—´çš„100ä¸ªå€¼
    - cv: äº¤å�‰éªŒè¯�æŠ˜æ•°ï¼Œé»˜è®¤5æŠ˜
    - scoring: è¯„ä¼°æŒ‡æ ‡ï¼Œé»˜è®¤è´Ÿå�‡æ–¹è¯¯å·®
    - random_state: éš�æœºç§�å­�ï¼Œç¡®ä¿�ç»“æ�œå�¯å¤�ç�°
    
    è¿”å›�:
    - optimal_alpha: æœ€ä¼˜çš„alphaå€¼
    - best_score: å¯¹åº”çš„æ¨¡å�‹æ€§èƒ½å¾—åˆ†(å�–ç»�å¯¹å€¼å��çš„å�‡æ–¹è¯¯å·®)
    - model: è®­ç»ƒå¥½çš„æœ€ä¼˜Ridgeæ¨¡å�‹
    """
    # è®¾ç½®alphaæ�œç´¢èŒƒå›´
    if alphas is None:
        alphas = np.logspace(-3, 3, 100)  # ä»�0.001åˆ°1000çš„å¯¹æ•°ç©ºé—´
    
    # åˆ›å»ºRidgeæ¨¡å�‹å’Œç½‘æ ¼æ�œç´¢å¯¹è±¡
    ...
    # è�·å�–æœ€ä¼˜å�‚æ•°å’Œç»“æ�œ
    optimal_alpha = grid_search.best_params_['alpha']
    best_score = -grid_search.best_score_  # è½¬æ�¢ä¸ºæ­£çš„å�‡æ–¹è¯¯å·®
    ...
    print(f"æœ€ä¼˜alpha: {optimal_alpha:.4f}")
    #print(f"äº¤å�‰éªŒè¯�MSE: {cv_mse:.4f}")
    
    return optimal_alpha#, cv_mse, best_model


aa=find_optimal_ridge_alpha(result,train_labels)


model = Ridge(alpha=aa)
...
# These must be implemented in your own code â€” you can adapt from training logic
