...
def add_gdp_feature(df):
    ...
    df['year'] = df['date'].dt.year  # Ekstraksi tahun dari tanggal
...
# 9. Menggabungkan total GDP ke dataset train dan test
...
# 10. Menghitung rasio global untuk produk, toko, dan negara
...
# 11. Menghitung rata-rata rasio global
...
# 12. Menambahkan fitur waktu (bulan, hari, dll) pada train dan test dataset
...
gdp_per_capita_filtered_ratios_df = pd.DataFrame.from_dict(gdp_data, orient='index', columns=['gdp'])
gdp_per_capita_filtered_ratios_df = gdp_per_capita_filtered_ratios_df.reset_index()
gdp_per_capita_filtered_ratios_df[['country', 'year']] = pd.DataFrame(gdp_per_capita_filtered_ratios_df['index'].tolist(), index=gdp_per_capita_filtered_ratios_df.index)
gdp_per_capita_filtered_ratios_df = gdp_per_capita_filtered_ratios_df.drop(columns=['index'])

gdp_per_capita_filtered_ratios_df['total_gdp'] = gdp_per_capita_filtered_ratios_df.groupby('year')['gdp'].transform('sum')
gdp_per_capita_filtered_ratios_df['ratio'] = gdp_per_capita_filtered_ratios_df['gdp'] / gdp_per_capita_filtered_ratios_df['total_gdp']

def impute_missing_values(df, gdp_per_capita_filtered_ratios_df):
    df_imputed = df.copy()
    
    for year in df_imputed['year'].unique():
        for country in df_imputed['country'].unique():
            for store in df_imputed['store'].unique():
                for product in df_imputed['product'].unique():
                    # Dapatkan rasio GDP untuk negara dan tahun saat ini
                    target_ratio = gdp_per_capita_filtered_ratios_df.loc[
                        (gdp_per_capita_filtered_ratios_df["year"] == year) & 
                        (gdp_per_capita_filtered_ratios_df["country"] == country), "ratio"].values[0]
                    
                    # Dapatkan rasio GDP untuk Norwegia (negara referensi) dan tahun yang sama
                    reference_ratio = gdp_per_capita_filtered_ratios_df.loc[
                        (gdp_per_capita_filtered_ratios_df["year"] == year) & 
                        (gdp_per_capita_filtered_ratios_df["country"] == "Norway"), "ratio"].values[0]
                    
                    # Hitung rasio imputasi
                    ratio = target_ratio / reference_ratio
                    
                    # Imputasi nilai yang hilang berdasarkan rasio
                    mask = (df_imputed["country"] == country) & (df_imputed["store"] == store) & (df_imputed["product"] == product) & (df_imputed["year"] == year)
                    reference_mask = (df_imputed["country"] == "Norway") & (df_imputed["store"] == store) & (df_imputed["product"] == product) & (df_imputed["year"] == year)
                    
                    df_imputed.loc[mask, "num_sold"] = df_imputed.loc[reference_mask, "num_sold"].values * ratio
    
    return df_imputed

train_imputed = impute_missing_values(train, gdp_per_capita_filtered_ratios_df)
...
train_imputed['num_sold'] = np.log1p(train_imputed['num_sold'])


h2o.init()


train_h2o = h2o.H2OFrame(train_imputed)
...
glm_model.train(x=x, y=y, training_frame=train_h2o)


predictions = glm_model.predict(train_h2o) 
...
predictions = glm_model .predict(test_h2o)
