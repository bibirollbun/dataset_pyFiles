...
# Toggle for clipping
DO_CLIP = True   # set to False to skip clipping extremes

CLIP_LOWER_Q = 0.001
CLIP_UPPER_Q = 0.999

USE_GPU = True  # change this to False if you want CPU
...
print("Adding Orignal to the data only (Training)")
orignal = pd.read_csv("/kaggle/input/simulated-roads-accident-data/synthetic_road_accidents_100k.csv")
train = pd.concat([train, orignal], axis=0, ignore_index=True)
print(f"Train: {train.shape}, Test: {test.shape}")
...
##############################################
# STEP 1: Identify & Encode Categorical Columns
##############################################

# Find categorical columns
cat_cols = X.select_dtypes(include=["object", "category"]).columns.tolist()

# Remove ID-like names if they appear here
for idc in [ID_COL, "ID", "Id", "index", "id"]:
    if idc in cat_cols:
        cat_cols.remove(idc)

def freq_encode(train, test, cols):
    for c in cols:
        freqs = train[c].value_counts(dropna=False)
        train[c] = train[c].map(freqs).astype(np.float32)
        test[c]  = test[c].map(freqs).fillna(0).astype(np.float32)
    return train, test

if len(cat_cols) > 0:
    print(f"Encoding {len(cat_cols)} categorical columns via frequency encoding.")
    X, X_test = freq_encode(X, X_test, cat_cols)
else:
    print("No categorical columns detected.")


##############################################
# Continue numeric feature engineering
##############################################
...
# ##############################################
# # Statistical Model Features (Clean Version)
# ##############################################
# print_header("Step: Statistical model features")

# from sklearn.linear_model import Ridge, LinearRegression

# # Ridge regression as meta-feature generator
# ridge = Ridge(alpha=1.0, random_state=42)
# ridge.fit(X_work, y)

# ridge_preds_train = ridge.predict(X_work)
# ridge_preds_test = ridge.predict(X_test_work)

# X_work["ridge_pred"] = ridge_preds_train.astype(np.float32)
# X_test_work["ridge_pred"] = ridge_preds_test.astype(np.float32)

# # Simple OLS baseline
# ols = LinearRegression()
# ols.fit(X_work, y)

# ols_preds_train = ols.predict(X_work)
# ols_preds_test = ols.predict(X_test_work)

# X_work["ols_pred"] = ols_preds_train.astype(np.float32)
# X_test_work["ols_pred"] = ols_preds_test.astype(np.float32)

# print("Added Ridge regression and OLS prediction features (no residuals).")
...
def hybrid_feature_selection(X, y, task="regression", top_n=600,
    ...
    rank_lgb = pd.Series(imp_lgb, index=kept_vt).rank(ascending=False)
    print(rank_lgb)
...
lgb_params = {
    ...
    'num_leaves': 150,
    'max_depth': 10,
    'learning_rate': 0.01456213255721509,
    'min_child_samples': 20,
    'subsample': 0.7751076296394666,
    'colsample_bytree': 0.7646810662181514,
    'reg_alpha': 2.144832684543215e-07,
    'reg_lambda': 1.3843359441617385e-05,
    #'device': 'gpu'
}
...
# -------------------------------------------------
USE_BOOSTERS = True      #  Include LGB/XGB/Cat
USE_SKLEARN_META = False  #  Include Ridge/Lasso/Linear
RANDOM_STATE = 2005
N_SPLITS = 5  # update with your actual CV splits
...
if USE_BOOSTERS:
    meta_features_train.extend([oof['lgb'], oof['xgb'], oof['cat']])
    meta_features_test.extend([preds['lgb'], preds['xgb'], preds['cat']])

if USE_SKLEARN_META:
    meta_features_train.extend([oof['lasso'], oof['ridge'], oof['linear']])
    meta_features_test.extend([preds['lasso'], preds['ridge'], preds['linear']])

meta_train = np.vstack(meta_features_train).T
...
# -------------------------------------------------
scaler = StandardScaler()
meta_train = add_noise_to_meta(meta_train, noise_level=0.25, random_state=RANDOM_STATE)
...
META_SKLEARN = True
# -------------------------------------------------
# Step 3: Define meta-models (flexible)
# -------------------------------------------------
meta_models = {}
if META_SKLEARN:
    meta_models.update({
...
# Example: you can easily plug booster as meta-model
if META_BOOSTER:
    from lightgbm import LGBMRegressor
    meta_models.update({
        "LGBM-Meta": LGBMRegressor(n_estimators=300, learning_rate=0.05, random_state=RANDOM_STATE)
    })
...
sample_sub[TARGET_COL] = final_preds
