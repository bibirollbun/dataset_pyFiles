...
df = pd.read_csv("/kaggle/input/syeda-noorain-23bcs10221-fold-creation/train_5folds.csv")
df.head()
...
for i in range(5) :
    ...
    train_img_paths = [f"../input/siim-isic-melanoma-classification/jpeg/train/{x}.jpg" for x in df_train["image_name"].values]
    valid_img_paths = [f"../input/siim-isic-melanoma-classification/jpeg/train/{x}.jpg" for x in df_valid["image_name"].values]
    ...
    valid_dataset = MelanomaDataset(
        ...
        dense_features=df_valid[dense_features].values.astype(np.float32),
