...
# def get_byol_transforms(size=224):
...
#     ])
#     return TwoCropsTransform(byol_transform)
...
def main_skip_pretrain(model_name='resnet50'):
    # Define paths
    ...
    print(f"Using {model_name} as backbone (no pretraining)")
    
    # Initialize BYOL model with raw backbone
    byol_model = BYOL(
        backbone,
        image_size=224,
    ...
    print("Starting fine-tuning on RAF-DB with raw BYOL model...")
    ...
    torch.save(finetuned_model.state_dict(), '/kaggle/working/models/finetuned_rafdb_model_no_pretrain.pth')
    ...
    with open('/kaggle/working/metrics/finetune_metrics_no_pretrain.json', 'w') as f:
        json.dump(finetune_metrics, f)
    
    # Plot fine-tuning metrics
    plot_finetune_metrics(train_losses, val_losses, train_accs, val_accs, 
                         save_path="/kaggle/working/plots/finetune_metrics_no_pretrain.png")
    
    # Plot wrong predictions
    plot_wrong_predictions(wrong_preds, num_classes, 
                          class_names=list(rafdb_dataset.class_to_idx.keys()),
                          title="Wrong Predictions Distribution - RAF-DB (No Pretraining)",
                          save_path="/kaggle/working/plots/wrong_predictions_rafdb_no_pretrain.png")
    ...
    with open('/kaggle/working/metrics/test_results_no_pretrain.json', 'w') as f:
    ...
    plt.title(f'Test Confusion Matrix - RAF-DB No Pretraining (Accuracy: {test_accuracy:.2f}%)')
    plt.xlabel('Predicted')
    plt.ylabel('True')
    plt.tight_layout()
    plt.savefig('/kaggle/working/plots/test_confusion_matrix_no_pretrain.png')
    plt.close()
    
    # Plot test wrong predictions
    plot_wrong_predictions(test_wrong_preds, num_classes, 
                          class_names=list(rafdb_dataset.class_to_idx.keys()),
                          title="Wrong Predictions Distribution - RAF-DB Test Set (No Pretraining)",
                          save_path="/kaggle/working/plots/wrong_predictions_test_no_pretrain.png")
    
    print(f"\nFinal Results (No Pretraining):")
    print(f"Best Validation Accuracy: {max(val_accs):.2f}%")
    print(f"Test Accuracy: {test_accuracy:.2f}%")

# Call the function
if __name__ == "__main__":
    main_skip_pretrain('resnet50')
