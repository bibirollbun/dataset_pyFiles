...
lr_oof_file = joblib.load("/kaggle/input/cmi-train-logisticregression-1/logistic-regression/oof_pred_probs.pkl")
ensemble_models = joblib.load("/kaggle/input/cmi-train-logisticregression-1/logistic-regression/models.pkl")
...
fold_scores["logistic-regression"] = temp_scores
...
def get_ensemble_predictions(models, X_test):
    test_pred_probs = np.zeros((len(X_test), 18))
    
    for model in models:
        test_pred_probs += model.predict_proba(X_test) / len(models)
...
def predict(sequence, demographics):        
    ...
    X_test = pd.DataFrame()
    for model_name, predictions in test_pred_probs.items():
        X_test[[f"{model_name.lower()}-{i}" for i in range(18)]] = predictions
    
    final_preds = get_ensemble_predictions(ensemble_models, X_test)
