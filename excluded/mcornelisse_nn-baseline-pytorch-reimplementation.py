...
for r in range(REPEATS):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(train)):
        ...
        train_losses, val_losses = train_model(model, train_loader, valid_loader, optimizer, criterion,scheduler, device, EPOCHS)
...
avg_train_losses = np.mean(all_fold_train_losses, axis=0)  # shape: (EPOCHS,)
avg_valid_losses = np.mean(all_fold_val_losses, axis=0)  # shape: (EPOCHS,)
...
# Convert your list of lists into a NumPy array for easier manipulation
all_train = np.array(all_fold_train_losses)  # shape: (FOLDS * REPEATS, EPOCHS)
all_val   = np.array(all_fold_val_losses)

mean_train = all_train.mean(axis=0)
mean_val   = all_val.mean(axis=0)

# Plot each fold individually (optional):
for i in range(all_train.shape[0]):
    plt.plot(range(EPOCHS), all_train[i], color='blue', alpha=0.3)
    plt.plot(range(EPOCHS), all_val[i], color='red', alpha=0.3)

# Plot the mean across folds
plt.plot(range(EPOCHS), mean_train, color='blue', label='Mean Training Loss', linewidth=2)
plt.plot(range(EPOCHS), mean_val, color='red', label='Mean Validation Loss', linewidth=2)

plt.xlabel('Epoch')
plt.ylabel('Loss (MSE)')
plt.title('Average Loss Curve across Folds/Repeats')
plt.legend()
plt.show()
