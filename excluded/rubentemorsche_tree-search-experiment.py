...
def main(tree_hparams):
    ...
    * train the model and create final submission output
    ...
    for i, (train_index, test_index) in enumerate(
        ...
        ######################################################
        ### THE NEURAL NETWORK IS REPLACED BY A TREE #########
        ######################################################
        ...
        val_preds = -1 * tree.predict(X_all_val)
        races = X_cat_val[:, categorical_cols.index("race_group")]
        print(f"Tree got stratified concordance of {cindex(val_efs, races, val_time, val_preds)}")
...
####################################################
### HYPERPARAMETER SEARCH FOR TREES ################
####################################################

import optuna

# This was the best loss function.
LOSS_FUNCTION_FOR_SEARCH = "reg:pseudohubererror"

def make_trees(tree_hparams):
    FOLDS = 5
    test, train_original = load_data()
    categorical_cols, numerical = get_feature_types(train_original)
    kf = StratifiedKFold(n_splits=FOLDS, shuffle=True) # New seed each time.
    # Keep track of the average concordance.
    avg_conc = 0
    for i, (train_index, test_index) in enumerate(
        kf.split(
            train_original, train_original.race_group.astype(str) + (train_original.age_at_hct == 0.044).astype(str)
        ...
        tt = train_original.copy()
        #Same tree stuff as during the main function.
        train = tt.iloc[train_index]; train_time = train["efs_time"]; train_efs = train["efs"]
        val = tt.iloc[test_index]; val_time = val["efs_time"]; val_efs = val["efs"]
        X_cat_train, X_cat_val, X_num_train, X_num_val, dl_train, dl_val, transformers = preprocess_data(train, val)
        X_all_train = np.concatenate([X_cat_train, X_num_train], axis=1)
        ...
        val_preds = -1 * tree.predict(X_all_val)
        races = X_cat_val[:, categorical_cols.index("race_group")]
        avg_conc += cindex(val_efs, races, val_time, val_preds) / FOLDS
    #Return stratified concordance index averaged over 5 folds.
    return avg_conc

def objective(trial):
    tree_hparams = {
        #Note the expanded search ranges for the big search.
        "n_estimators": trial.suggest_int("n_estimators", 500, 4000),
        "lr": trial.suggest_loguniform("lr", 0.001, 0.3),
        "max_depth": trial.suggest_int("max_depth", 1, 15),
        "subsample": trial.suggest_float("subsample", 0.25, 1.0),
        "colsample_bytree": trial.suggest_float("colsample_bytree", 0.25, 1.0),
        "loss_function": LOSS_FUNCTION_FOR_SEARCH,

        #New parameters that are only for the big search.
        "min_child_weight": trial.suggest_loguniform("min_child_weight", 0.01, 100),
        "gamma": trial.suggest_loguniform("gamma", 1e-8, 1.0),
        "reg_lambda": trial.suggest_loguniform("reg_lambda", 1e-5, 10.0)
    }

    result = make_trees(tree_hparams)
    return result

study = optuna.create_study(direction="maximize")
study.optimize(objective, n_trials=1000) #Note the extra zero!

print("Best hyperparameters: ", study.best_params)
print("Best metric: ", study.best_value)

best = study.best_params
best["loss_function"] = LOSS_FUNCTION_FOR_SEARCH
main(best)
print("Done")
