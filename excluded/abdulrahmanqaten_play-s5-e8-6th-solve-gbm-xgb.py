...
clf1 = lgb.LGBMClassifier(random_state=42)
clf2 = xgb.XGBClassifier(random_state=42, use_label_encoder=False, eval_metric='logloss')

model = VotingClassifier(estimators=[('lgbm', clf1), ('xgb', clf2)], voting='soft')

model.fit(X_train, y)

print("Ensemble model (LGBM + XGB) training complete.")
