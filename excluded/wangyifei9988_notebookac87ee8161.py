...
# -----------------------------------------------------------
# (C) 変数 date の前処理（さらなるPrivateスコア向上ver）
# -----------------------------------------------------------
col = 'date'
...
# 特殊位置日标志（布尔→整数）
for feature in ['is_month_start', 'is_month_end', 'is_quarter_start', 'is_quarter_end', 'is_year_end']:
    train_x[feature] = getattr(train[col].dt, feature).astype(int)
    test_x[feature] = getattr(test[col].dt, feature).astype(int)

# 正弦・余弦変換（dayofyear + dow）
