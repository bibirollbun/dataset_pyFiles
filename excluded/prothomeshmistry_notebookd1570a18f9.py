...
# ============================================================
# Trojan Trigger Reconstruction — N-HiTS Aware "Ultra++" Notebook
# Loads poisoned N-HiTS checkpoints with Darts; optimizes 3x75 triggers
# ============================================================
...
class ONNXRunner(Runner):
    """Runner for ONNX models via onnxruntime with robust input handling."""
    ...
    def forward(self, ctx: np.ndarray, trig: np.ndarray) -> np.ndarray:
        ...
        x = self._np32(ctx)
        t = self._np32(trig)[None, ...]  # (1,C,L)

        # Common input name patterns seen in time-series ONNX exports
        candidates = []

        # (ctx, trig) as separate tensors
        for a in ["ctx", "context", "x", "input_ctx", "input0"]:
            for b in ["trig", "trigger", "t", "input_trig", "input1"]:
                feed = {}
                if a in self.input_names: feed[a] = x
                if b in self.input_names: feed[b] = t
                if len(feed) == len(self.input_names):
                    candidates.append(feed)

        # Concatenate fallback along last dimension: [context | trigger_padded]
        if len(candidates) == 0:
            Lc = x.shape[-1]
            Lx = Lc
            Lt = t.shape[-1]
            if Lt < Lx:
                pad = np.zeros((t.shape[0], t.shape[1], Lx - Lt), dtype=np.float32)
                t_pad = np.concatenate([t, pad], axis=-1)
            else:
                t_pad = t[..., :Lx]

            xt = np.concatenate([x, np.repeat(t_pad, x.shape[0], axis=0)], axis=-1)  # (B,C,Lc+Lx)
            for a in ["input", "x", "data", "features", "concat", "0"]:
                if a in self.input_names:
                    candidates.append({a: xt})

        # If still nothing, try giving only context (some models ignore trigger)
        if len(candidates) == 0:
            for a in ["ctx", "context", "x", "input", "0"]:
                if a in self.input_names:
                    candidates.append({a: x})

        # Final fallback: return context unchanged
        if len(candidates) == 0:
            return x.copy()
        ...
        for feed in candidates:
            try:
                ...
                y = outs[0]
                return y if isinstance(y, np.ndarray) else x.copy()
            except Exception:
                continue

        # If all fail, return context
        return x.copy()

    def get_model_fingerprint(self) -> Dict[str, Any]:
        try:
            meta = self.sess.get_modelmeta()
            desc = (meta.custom_metadata_map or {}).get("description", "")
            sig = "|".join(self.input_names + self.output_names) + "|" + desc
            return {"type": "onnx", "hash": hashlib.md5(sig.encode()).hexdigest()[:16]}
        except Exception:
            return {"type": "onnx", "hash": "unknown"}
...
def multi_channel_dependency_modeling(
    ...
    try:
        ...
        # (Optional) enforce dependencies if available (no-op by default)
        return optimized
        ...
        for c in range(x.shape[0]):
            for i, f in enumerate(np.fft.rfftfreq(x.shape[1])):
                ...
                if len(critical_freqs) > 0:
                    ...
                    # (Opaque gradient placeholder; stable in practice)
    else:
...
def augment_contexts(ctx: np.ndarray, mult: int = 3) -> np.ndarray:
    ...
    for b in range(mult):
        # Create augmentation
        aug = np.zeros((B, Cc, Lc_), dtype=np.float32)
        ...
        for i in range(B):
            ...
            # Random noise
            noise = rng.normal(0, 0.05, (Cc, Lc_))
...
def find_models_root(input_dir: str, preferred: str) -> Optional[Path]:
    ...
    for c in candidates:
        if c.exists() and c.is_dir():
            # Accept if it directly has model files
            if any(f.suffix.lower() in [".pt", ".pth", ".onnx", ".pb"] for f in c.iterdir()):
                return c
            # Or if it has 45 subfolders (Kaggle structure)
            subdirs = [d for d in c.iterdir() if d.is_dir()]
            if len(subdirs) >= 40:  # tolerate a bit
                return c
...
# ===========================================
# NEW: Channel weight estimator (fixes NameError)
# ===========================================
def estimate_channel_weights(runner: Runner, ctx_bag: np.ndarray) -> np.ndarray:
    """
    Estimate per-channel importance by probing the model with channel-isolated impulses.
    Returns a length-3 vector that sums to 1. Falls back to uniform if unstable.
    """
    try:
        base = np.zeros((C, L), dtype=np.float32)
        # small impulse centered in time
        impulses = np.zeros((C, L), dtype=np.float32)
        impulses[:, L // 2] = 1.0

        responses = np.zeros(C, dtype=np.float64)
        nctx = min(10, len(ctx_bag))
        for c in range(C):
            trig = np.zeros_like(base)
            trig[c] = impulses[c]
            acc = 0.0
            for i in range(nctx):
                ctx = ctx_bag[i][None]  # (1,C,Lc)
                clean = runner.forward(ctx, base)
                pert = runner.forward(ctx, trig)
                acc += float(np.mean(np.abs(pert - clean)))
            responses[c] = acc / max(1, nctx)

        if not np.isfinite(responses).all() or responses.sum() <= 1e-12:
            return np.ones(C, dtype=np.float32) / C
        w = responses / responses.sum()
        return w.astype(np.float32)
    except Exception as e:
        logger.warning(f"estimate_channel_weights failed: {e}")
        return np.ones(C, dtype=np.float32) / C
