...
params = {
      ...
      'subwts'   : [w/200 for w in [ +11, -1,-3,-7 ]],
      'subm'     : [
          {'name': f'0.70108', 'weight':+0.247, 'color':'royalblue'},
          {'name': f'0.70109', 'weight':+0.259, 'color':'orange'   },
          {'name': f'0.70150', 'weight':+0.247, 'color':'green'    },
          {'name': f'0.70165', 'weight':+0.247, 'color':'crimson'  },]
}
...
params = {
      ...
      'subm'     : [
          {'name': f'0.70190', 'weight':+0.058, 'color':'royalblue'},
          {'name': f'0.70191', 'weight':+0.058, 'color':'orange'   },
          {'name': f'0.70418', 'weight':+0.884, 'color':'green'    },]
}
...
params = {
      ...
      'subwts'   : [w/200 for w in [+10, -3,-7]],
      'subm'     : [
          {'name': f'0.70255', 'weight':+0.108, 'color':'royalblue'},
          {'name': f'0.70285', 'weight':+0.108, 'color':'orange'   },
          {'name': f'0.70404', 'weight':+0.784, 'color':'green'    },]
}
...
params = {
      ...
      'subwts'   : [w/200 for w in [+10, -3,-7]],
      'subm'     : [
          {'name': f'0.70442.cat', 'weight':+0.085, 'color':'royalblue'},
          {'name': f'0.70442.lgbm','weight':+0.095, 'color':'orange'   },
          {'name': f'0.70651',     'weight':+0.820, 'color':'green'    },]
}
df4 = h_blend(params, details=True, subm='g4.csv')
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/300 for w in [+11, -1,-3,-5]],
      'subm'     : [
          {'name': f'g1', 'weight':+0.0105, 'color':'royalblue'},
          {'name': f'g2', 'weight':+0.0210, 'color':'orange'   },
          {'name': f'g3', 'weight':+0.0315, 'color':'green'    },
          {'name': f'g4', 'weight':+0.9370, 'color':'crimson'  },]
}
df5 = h_blend(params, details=True, subm='cross.csv')
...
params = {
      ...
      'subwts'   : [w/200 for w in [+11, -1,-3,-7]],
      'subm'     : [
          {'name': f'0.70418',     'weight':+0.05, 'color':'royalblue'},
          {'name': f'0.70442.cat', 'weight':+0.07, 'color':'orange'   },
          {'name': f'0.70442.lgbm','weight':+0.08, 'color':'green'    },
          {'name': f'0.70651',     'weight':+0.80, 'color':'crimson'  },]
}
df = h_blend(params, details=True, subm='g8.csv')

df.head()


df_cross = pd.read_csv('/kaggle/input/playground-series-s5e12/sample_submission.csv')

df_cross = df_cross.rename(columns={'diagnosed_diabetes':'cross'})

df_651 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70651.csv')
df_732 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70732.csv')
df_442 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70442.xgb.csv')

df_651 = df_651.rename(columns={'diagnosed_diabetes':'0.70_651'})
df_732 = df_732.rename(columns={'diagnosed_diabetes':'0.70_732'})
df_442 = df_442.rename(columns={'diagnosed_diabetes':'0.70_442'})

df_cross['cross'] = df_651['0.70_651'] *0.90 + 0.10* df_442['0.70_442']

s_b_m = pd.merge(df_651, df_442,   on='id')
s_b_m = pd.merge(s_b_m,  df_cross, on='id')
s_b_m = pd.merge(s_b_m,  df_732,   on='id')

s_b_m['different'] = s_b_m['cross'] - s_b_m['0.70_732']

s_b_m['732-651'] = s_b_m['0.70_732'] - s_b_m['0.70_651']
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/10_000 for w in [+2, -2]],
      'subm'     : [
          {'name': f'0.70696','weight':+0.0003, 'color':'royalblue'},
          {'name': f'0.70732','weight':+0.9997, 'color':'orange'   },]
}
df = h_blend(params, details=True, subm='two.csv')


for file in 'g1,g2,g3,g4,g8,cross,two,tida_desc'.split(','): os.remove(file+'.csv')
