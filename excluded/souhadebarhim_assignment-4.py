...
train_tfms = transforms.Compose([
    ...
    transforms.RandomHorizontalFlip(),  # ✅ no rotation
    transforms.ToTensor(),
...
print("✅ Saved best_convnet.pt")



# ======================================================
# Cell 2: Extraction with OVERALL COUNT (e.g., 12345/300000)
# - Extracts /kaggle/input/cifar-10/test.7z → ./test/
# - Prints one updating line: "<done>/<total> (<pct>%)"
# - Skips if already extracted (≥300k)
# ======================================================
...
if have >= TOTAL:
    print(f"✅ Already extracted: {have} / {TOTAL} in {TEST_DIR}")
else:
    sevenz = ensure_7z()
    ...
    print(f"✅ Extracted: {final} / {TOTAL} in {TEST_DIR}")
...
if os.path.isdir(TEST_DIR) and glob.glob(os.path.join(TEST_DIR, "1.png")):
    ...
    # -------- Path B: no extraction → tiny batched extracts from test.7z --------
...
print("✅ Saved ids.npy and pred_idx.npy")
