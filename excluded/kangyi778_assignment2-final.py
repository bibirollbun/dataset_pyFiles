...
# ================================
# Full Evaluation Code for CNN model with Metrics, Visualization, and Error Analysis
# ================================
...
BATCH_SIZE = 32
IMAGE_SIZE = 224
MODEL_PATH = "/kaggle/input/melanoma-efficientnet-subset-model-pth/melanoma_efficientnet_subset_model.pth"

VAL_CSV = "/kaggle/input/validate-dataset/val_processed.csv"
...
val_dataset = MelanomaDataset(val_df, IMAGE_DIR, transform=transform)
...
# -------------------------------
class MelanomaEfficientNet(nn.Module):
    def __init__(self, model_name='efficientnet_b0', num_classes=1, dropout_rate=0.3):
        super(MelanomaEfficientNet, self).__init__()
        if model_name == 'efficientnet_b0':
...
# Initialize model
model = MelanomaEfficientNet(model_name='efficientnet_b0', num_classes=1, dropout_rate=0.3).to(DEVICE)

# Load state dict
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()
print("Pretrained EfficientNet loaded successfully!")
...
with torch.no_grad():
    for i, (images, labels) in enumerate(tqdm(val_loader, desc="Evaluating")):
        ...
        image_names.extend(val_df.iloc[i * BATCH_SIZE:(i + 1) * BATCH_SIZE]['image_name'].tolist())
...
# -------------------------------
misclassified = []
for idx, (true_label, pred_label) in enumerate(zip(all_labels, binary_preds)):
    if true_label != pred_label:
        misclassified.append((image_names[idx], true_label, pred_label))
...
# -------------------------------
# Patient-Level Mistakes (if patient_id exists)
# -------------------------------
if 'patient_id' in val_df.columns:
    misclassified_df = val_df.iloc[[idx for idx, (t, p) in enumerate(zip(all_labels, binary_preds)) if t != p]]
    patient_counts = misclassified_df['patient_id'].value_counts().head(10)
    print("\nTop Patients with Most Misclassifications:")
    print(patient_counts)



# ================================
# Full Evaluation Code for Fusion model with Metrics, Visualization, and Error Analysis
# ================================
import torch
import torch.nn as nn
...
# Load Data
# -----------------------------
val_df = pd.read_csv('/kaggle/input/validate-dataset/val_processed.csv')
DATA_PATH = "/kaggle/input/siim-isic-melanoma-classification"
val_df['image_path'] = f"{DATA_PATH}/jpeg/train/" + val_df['image_name'] + '.jpg'
...
# Normalize metadata
val_df[feature_columns] = (val_df[feature_columns] - val_df[feature_columns].mean()) / (val_df[feature_columns].std() + 1e-8)
...
class FusionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            image = self.transform(image)
        metadata = torch.tensor(row[self.feature_columns].astype(float).values, dtype=torch.float32)
        target = torch.tensor(float(row['target']), dtype=torch.float32)
        return (image, metadata), target
...
val_dataset = FusionDataset(val_df, feature_columns, transform)
val_loader = DataLoader(val_dataset, batch_size=32, shuffle=False, num_workers=2)

# -----------------------------
# Member 2 Model (EfficientNet)
# -----------------------------
class MelanomaEfficientNet(nn.Module):
    def __init__(self, num_classes=1, dropout_rate=0.3):
        super().__init__()
...
# -----------------------------
class FusionModel(nn.Module):
    def __init__(self, image_model, meta_input_dim, meta_hidden=[128, 64], fusion_hidden=[512, 256], dropout=0.3):
        super().__init__()
        self.image_model = image_model
        with torch.no_grad():
            dummy = torch.randn(1, 3, 224, 224).to(next(image_model.parameters()).device)
        ...
        prev_dim = img_out_dim + meta_out_dim
        for h in fusion_hidden:
            fusion_layers.extend([nn.Linear(prev_dim, h), nn.BatchNorm1d(h), nn.ReLU(), nn.Dropout(dropout)])
            prev_dim = h
        fusion_layers.append(nn.Linear(prev_dim, 1))
        self.fusion = nn.Sequential(*fusion_layers)
...
# Load models
# -----------------------------
fusion_path = '/kaggle/input/best-fusion-model/best_fusion_model.pth'
member2_model = MelanomaEfficientNet().to(device)
fusion_model = FusionModel(member2_model, meta_input_dim=len(feature_columns)).to(device)
...
with torch.no_grad():
    for (images, metadata), targets in val_loader:
        images, metadata, targets = images.to(device), metadata.to(device), targets.to(device)
        outputs = fusion_model(images, metadata).squeeze()
        probs = torch.sigmoid(outputs)
...
f1_scores = [f1_score(all_targets, (all_probs > t).astype(int)) for t in thresholds]
...
# -----------------------------
# False Negatives
fn_idx = np.where((all_targets==1) & (all_preds==0))[0]
print(f"False negatives: {len(fn_idx)}")
for idx in fn_idx[:5]:
    row = val_df.iloc[idx]
    img = Image.open(row['image_path']).convert("RGB")
    plt.imshow(img)
    plt.title(f"False Negative - True: 1, Pred: 0, Prob: {all_probs[idx]:.2f}")
    plt.axis('off')
    plt.show()

# False Positives
fp_idx = np.where((all_targets==0) & (all_preds==1))[0]
print(f"False positives: {len(fp_idx)}")
for idx in fp_idx[:5]:
    row = val_df.iloc[idx]
    img = Image.open(row['image_path']).convert("RGB")
    plt.imshow(img)
    plt.title(f"False Positive - True: 0, Pred: 1, Prob: {all_probs[idx]:.2f}")
    plt.axis('off')
    plt.show()
