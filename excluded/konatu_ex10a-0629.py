...
agg_funcs = {
    ...
    "month_lag":       ["min", "max"],
    "purchase_date":   ["count"],
    'category_1':      ['sum', 'mean'], # category_1は数値化されたのでOK
}

# One-Hotエンコーディングで作成された新しい列を集約対象に追加
cat_cols = [col for col in new_trans.columns if 'category_2_' in col or 'category_3_' in col]
for col in cat_cols:
    agg_funcs[col] = ['mean'] # 各カテゴリの購入割合（平均値）を集約
