...
def get_hybrid_features(video_path, alert_time, event_time):
    ...
    frames = extract_critical_frames(
        ...
        alert_time, 
        event_time,
        num_frames=16,  # I guess this
        sampling_interval=60  # Process 1 frame per second (60fps video)
    )
    ...
    spatial_features = base_model.predict(
        ...
        batch_size=64,  # Process 64 frames at once
        verbose=0
    )
...
train_probs = temporal_model.predict(X_train_3d).flatten()
val_probs = temporal_model.predict(X_val_3d).flatten()
...
temporal_probs = temporal_model.predict(X_test_3d).flatten()
