...
train_data.head() # Five columns of training data


test_data.head()  # Five columns of testing data
...
train_data.loc[train_data['Number_of_Ads']>3, 'Numbre_of_Ads'] = 3
test_data.loc[test_data['Number_of_Ads']>3, 'Numbre_of_Ads'] = 3
...
numerical_variables = ['Episode_Length_minutes', 'Host_Popularity_percentage', 'Guest_Popularity_percentage', 'Numbre_of_Ads' ]

catagorical_variables = ['Podcast_Name', 'Episode_Title', 'Genre', 'Publication_Day', 'Publication_Time', 'Episode_Sentiment']
...
def fill_df(df):
    '''This function will helps to fill the gaps with median and mode'''
    # Filling in the numerical gaps with the median
    num_cols = df.select_dtypes(include=['number']).columns
    for col in num_cols:
        df[col] = df[col].fillna(df[col].median())

     # Filling in the catagorical gaps with the most frequent value(mode)
...
catagorical_variables = [col for col in train_data.columns if col in catagorical_variables]

plot_categorical_distributions(train_data, test_data, catagorical_variables)
...
submission = pd.DataFrame({
    'id': sample_data.id, 'Listening_Time_minutes': pred})
