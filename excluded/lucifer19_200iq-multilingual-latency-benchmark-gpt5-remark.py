...
def generate_iq_questions(n=50):
    ...
    for _ in range(n):
        ...
        if q_type == "number":
            ...
            prompt = f"Mi a kÃ¶vetkezÅ‘ szÃ¡m a sorozatban? {', '.join(map(str, seq))}, ?"
            ...
            prompt = f"Melyik betÅ± kÃ¶vetkezik a sorozatban? {', '.join(seq)}, ?"
            questions.append((prompt, correct))
        elif q_type == "analogy":
            pairs = [
                ("macska", "egÃ©r", "oroszlÃ¡n", "zebra"),
                ("tÅ±z", "meleg", "jÃ©g", "hideg"),
                ("nap", "vilÃ¡gos", "hold", "sÃ¶tÃ©t"),
                ("vÃ­z", "folyik", "kÅ‘", "Ã¡ll")
            ]
            a, b, c, correct = random.choice(pairs)
            prompt = f"{a}:{b} Ãºgy arÃ¡nylik, mint {c}:?"
            questions.append((prompt, correct))
    return questions

# == Modell szimulÃ¡ciÃ³ (80%-ban helyes vÃ¡lasz) ==
...
def plot_confusion_matrix(df):
    ...
    plt.title("ğŸ§  IQ vÃ¡laszok konfÃºziÃ³s mÃ¡trixa", fontsize=16)
    plt.xlabel("Modell vÃ¡lasz", fontsize=12)
    plt.ylabel("Helyes vÃ¡lasz", fontsize=12)
...
# MegjelenÃ­tÃ©s (helyes Ã©s hibÃ¡s vÃ¡laszok szÃ¡ma)
print("âœ… Helyes vÃ¡laszok:", (df_results["result"] == "âœ“").sum())
print("â�Œ HibÃ¡s vÃ¡laszok:", (df_results["result"] == "âœ—").sum())

# HibÃ¡s vÃ¡laszok tÃ¡blÃ¡zata
print("\nğŸ“‹ HibÃ¡s kÃ©rdÃ©sek:")
display(df_results[df_results["result"] == "âœ—"][["prompt", "correct_answer", "model_answer"]])

# KonfÃºziÃ³s mÃ¡trix megjelenÃ­tÃ©se
...
def gen_math_question(lang: str, rng: random.Random):
    ...
    if op == "+":
        ...
        gold = a + b
        q = f"What is {a} + {b}?" if lang=="en" else f"Mennyi {a} + {b} Ã©rtÃ©ke?"
    elif op == "-":
        a, b = rng.randint(2, 999), rng.randint(2, 999)
        if b > a: a, b = b, a
        gold = a - b
        ...
        gold = a * b
        q = f"What is {a} Ã— {b}?" if lang=="en" else f"Mennyi {a} Ã— {b} Ã©rtÃ©ke?"
...
def gen_sequence_question(lang: str, rng: random.Random):
    ...
    q = (f"Find the next number in the sequence: {seq_str}, ?"
         if lang=="en" else
         f"Mi a kÃ¶vetkezÅ‘ szÃ¡m a sorozatban: {seq_str}, ?")
...
def sys_user_for_schema(question: str, lang: str):
    if lang == "en":
        ...
        sys = ("PrecÃ­z numerikus megoldÃ³ vagy. Csak JSON: {\"answer\": <number>} â€” extra szÃ¶veg nÃ©lkÃ¼l.")
        user = f"Oldd meg, Ã©s csak JSON-t adj vissza:\n{question}"
    return sys, user
...
def prompt_for_brackets(question: str, lang: str):
    if lang == "en":
        ...
        return f"Csak a vÃ©gsÅ‘ szÃ¡mot add vissza [[SZÃ�M]] formÃ¡tumban.\n\nKÃ©rdÃ©s:\n{question}"
...
def call_openai_compat_best_effort(question: str, lang: str, temperature: float):
    ...
    if lang == "en":
        ...
        sys = "PrecÃ­z numerikus megoldÃ³ vagy. Csak [[SZÃ�M]] formÃ¡tumban vÃ¡laszolj."
        user = f"{question}\n\nKimeneti formÃ¡tum: [[SZÃ�M]]"
    t0 = time.time()
...
# 3ï¸�âƒ£ Ãšj oszlop: kÃ©rdÃ©s hossz karakterekben
df['question_length'] = df['question_text'].str.len()

# 4ï¸�âƒ£ SzÃ¡mÃ­tott feature-Ã¶k hozzÃ¡adÃ¡sa (20 db)
...
sns.heatmap(corr,linewidths=0.1,vmax=1.0, square=True, cmap=colormap, linecolor='black', annot=False)
