...
dir_path = '/kaggle/input/zillow-prize-1'
train_17 = pd.read_csv(f'{dir_path}/train_2017.csv')
prop_17  = pd.read_csv(f'{dir_path}/properties_2017.csv')
train_16 = pd.read_csv(f'{dir_path}/train_2016_v2.csv')
prop_16  = pd.read_csv(f'{dir_path}/properties_2016.csv')
test     = pd.read_csv(f'{dir_path}/sample_submission.csv')
...
print(f'train_17.shape: {train_17.shape}')
print(f'prop_17.shape : {prop_17.shape}')
print(f'train_16.shape: {train_16.shape}')
print(f'prop_16.shape : {prop_16.shape}')
print(f'test.shape    : {test.shape}')


df16  = pd.merge(train_16, prop_16, how='left', on='parcelid')
df16.head()
...
df17  = pd.merge(train_17, prop_17, how='left', on='parcelid')
df17.head()
...
train = pd.concat([df16, df17], axis=0, ignore_index=True)
train.head()
...
test = test.merge(prop_16, on='parcelid', how='left')
test.head()
...
train = train.drop(columns=[col for col in train.columns if col.endswith('id')])
test  = test.drop(columns=[col for col in test.columns if col.endswith('id')])
...
test, test_id = drop_id_feature(test, 'ParcelId')
...
for feature in ['calculatedbathnbr', 'fullbathcnt', 'roomcnt']:
