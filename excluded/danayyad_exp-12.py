...
optimizer = torch.optim.Adam(net.parameters(), lr=1e-8, weight_decay=1e-8)

total_epochs = 75 
checkpoint_path = "/kaggle/input/exp12-50checkpoints/checkpoint_epoch_50 (1).pth"
...
class DiceLoss(nn.Module):
    def __init__(self, weight_dice=1.0):  # Correct: double underscores
        super(DiceLoss, self).__init__()  # Correct: refer to DiceLoss, not ComboLoss
        self.weight_dice = weight_dice
    ...
    def forward(self, inputs, targets):
        return self.weight_dice * self.dice_loss(inputs, targets)
...
# Train the model with early stopping settings
train(
    net,
    train_loader,
    val_loader,
