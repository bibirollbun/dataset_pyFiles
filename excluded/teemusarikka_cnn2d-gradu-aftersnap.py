...
data_CNN = df[['Id','x','y','s','a','dis','o','dir']]
grouped = data_CNN.groupby(['Id'])
df_CNN = grouped.agg(list).reset_index()


# Flatten the lists into a two dimensional data matrix
x = df_CNN['x'].apply(pd.Series).add_prefix('x_')
y = df_CNN['y'].apply(pd.Series).add_prefix('y_')
s = df_CNN['s'].apply(pd.Series).add_prefix('s_')
a = df_CNN['a'].apply(pd.Series).add_prefix('a_')
ds = df_CNN['dis'].apply(pd.Series).add_prefix('dis_')
o = df_CNN['o'].apply(pd.Series).add_prefix('o_')
dr = df_CNN['dir'].apply(pd.Series).add_prefix('dir_')
df_CNN = pd.concat([df_CNN.drop(['x','y','s','a','dis','o','dir'], axis=1),x,y,s,a,ds,o,dr], axis=1)
...
df_CNN = df_CNN.merge(play, on='Id', how='left').reset_index()
...
X = df_CNN[features].values.reshape(-1,22,7,1)
y = df_CNN[target]
...
# Define the model
model = Sequential()
model.add(Input(shape=(22, 7, 1)))
model.add(Conv2D(22, kernel_size=(2,2), activation='relu'))
...
# Train the model
model_history = model.fit(X_train, y_train, epochs=15, validation_data=(X_test, y_test))
