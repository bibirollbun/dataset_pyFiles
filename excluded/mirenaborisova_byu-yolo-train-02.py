...
yolo_pretrained_weights = \
    '/kaggle/input/byu-yolo-train-01/yolo_weights/flagellar_motor_detector/weights/best.pt'
...
results_csv_path = os.path.join(weights_path, 'results.csv')
dfl_plot_path = weights_path + '/dfl_plot_02.png'
predictions_plot_path = '/kaggle/working/predictions_02.png'
...
def generalization(model):
    ...
    for i, img_file in enumerate(img_files):
        ...
        axes[i].set_title(f"IMAGE: {img_file}\nACTUAL (lime) vs PREDICTION (deeppink)")
...
def plot_distribution_focal_loss():
    ...
    plt.plot(results_df['epoch'], 
             results_df[train_dfl_cols], 
             label='TRAIN DISTRIBUTION FOCAL LOSS')
    plt.plot(results_df['epoch'], 
             results_df[val_dfl_cols],
             label='VALIDATION DISTRIBUTION FOCAL LOSS')
    
    plt.axvline(x=results_df.loc[best_epoch, 'epoch'], 
                color='deeppink', 
                label=f'BEST MODEL (BEST_EPOCH {int(results_df.loc[best_epoch, "epoch"])}, ' + \
...
def train_yolo_model(yaml_path):
    ...
    best_epoch_info = plot_distribution_focal_loss()
...
def upload_dataset(dataset_yaml_path):
