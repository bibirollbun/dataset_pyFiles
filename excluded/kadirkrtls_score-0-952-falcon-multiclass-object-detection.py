# Install YOLO (Ultralytics) and ensemble-boxes for WBF
...
print("ğŸ”§ Dependencies loaded and seeds fixed.")
...
# 3. Plot random samples from ALL merged train folders (original + scenarios)
from pathlib import Path
import random
...
if TRAIN:
    model = YOLO("yolo11x.pt")  # veya kendi checkpointâ€™in
    ...
    model.train(
        ...
        weight_decay=0.0001,
        dropout=0.3,
        dfl=1.00,            # 1.5 -> 0.75 (DFLâ€™i azaltÄ±yoruz)
        cos_lr=True,
        patience=100,
        save_period=10,
        project="runs2/train",  # yeni proje klasÃ¶rÃ¼
        exist_ok=True,
        plots=True,
        augment=True,        # default augmentâ€™lar
        mosaic=1.0,          # tam mosaic
        mixup=0.25,           # mixup arttÄ±
        cutmix=0.25,          # cutmix arttÄ±
        copy_paste=0.05,      # copy-paste arttÄ±
        close_mosaic=10,
...
# 8.1 Load metrics and plot validation losses & DFL loss
import pandas as pd
import matplotlib.pyplot as plt
from pathlib import Path
from PIL import Image
...
# 1. Specify your checkpoints and test folder
model_paths = [
...
# 3. Run batched, multiâ€�scale inference
...
print(f"âœ… submission.csv created with {len(submission_df)} entries.")
