...
# ====== 3A: Luá»“ng A â€“ One-Hot & Label (Submission chuáº©n 1625 dÃ²ng) ======
import os, json
import csv
import numpy as np
import pandas as pd
...
print(f"âœ… Ä�á»�c {len(test_ids)} id tá»« test.csv ({test_path})")

# ---------------- Model chá»�n Ä‘á»ƒ ná»™p ----------------
...
# ---------------- Chuáº©n hoÃ¡ nhÃ£n ná»™p thÃ nh e/p ----------------
if set(pd.Series(pred_any).unique()) <= {0,1}:
    ...
    if not pred_cls.isin(['e','p']).all():
        raise ValueError("âš ï¸� NhÃ£n dá»± Ä‘oÃ¡n khÃ´ng há»£p lá»‡ (pháº£i lÃ  0/1 hoáº·c e/p).")

# ---------------- Ä�áº£m báº£o Ä‘Ãºng 1625 dÃ²ng ----------------
assert len(test_ids) == len(pred_cls), f"Sá»‘ dÃ²ng lá»‡ch: test={len(test_ids)} vs pred={len(pred_cls)}"
submission = pd.DataFrame({'id': test_ids, 'class': pred_cls})
submission = submission.iloc[:1625]
print("ğŸ“� submission shape:", submission.shape)

# ---------------- Xuáº¥t file submission.csv ----------------
submission.to_csv("submission.csv", index=False, quoting=csv.QUOTE_NONE, escapechar='\\')
print("âœ… Ä�Ãƒ Táº O FILE: submission.csv (id,class = e/p)")
...
def get_test_ids():
    """
    Ä�á»�c test.csv vÃ  tráº£ vá»� Series 'id' (chá»¯ thÆ°á»�ng).
    """
...
y_train_auto = pd.read_csv("/kaggle/working/y_train_autoencoder.csv")["class"]  # cÃ³ thá»ƒ lÃ  'e'/'p' hoáº·c 0/1
...
def evaluate_models(X_train, X_val, y_train, y_val, models, prefix):
    ...
    for name, model in models.items():
        ...
        # Dá»± Ä‘oÃ¡n nhÃ£n & xÃ¡c suáº¥t (náº¿u cÃ³)
...
if test_ids is None or len(test_ids) != len(X_test_auto):
    print("âš ï¸� KhÃ´ng tÃ¬m Ä‘Æ°á»£c test.csv gá»‘c hoáº·c sá»‘ dÃ²ng khÃ´ng khá»›p. DÃ¹ng id = 0..n-1.")
...
pred_any = best_model_auto.predict(X_test_auto)  # cÃ³ thá»ƒ lÃ  0/1 hoáº·c 'e'/'p'

# Chuáº©n hoÃ¡ class vá»� 'e'/'p'
if set(pd.Series(pred_any).unique()) <= {0,1}:
    ...
    if not pred_class_auto.isin(['e','p']).all():
        raise ValueError("Dá»± Ä‘oÃ¡n khÃ´ng thuá»™c {'e','p'} hoáº·c {0,1}. Kiá»ƒm tra láº¡i pipeline/labels.")

submission_auto = pd.DataFrame({"id": test_ids, "class": pred_class_auto})
# Ghi Ä‘Ãºng tÃªn Kaggle yÃªu cáº§u:
