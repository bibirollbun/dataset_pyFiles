...
def create_feature(df):
    ...
    df["annual_income_interest_rate"] = df["annual_income"]/df["interest_rate"]
    df["annual_income_debt_to_income_ratio"] = df["annual_income"]/df["debt_to_income_ratio"]
    joined_columns = [
...
pipe  = Pipeline([('scaler', Custom_Scaler(except_col=['id', 'loan_paid_back', "grade_subgrade", "education_level"])), ('label_encoder', MultiColumnLabelEncoder())])
...
Lgbm_params  = {'learning_rate': 0.07179484775966744, 'num_leaves': 24, 'max_depth': 16, 'min_data_in_leaf': 187, 'feature_fraction': 0.7114099864797933, 'bagging_fraction': 0.5488313832820836, 'bagging_freq': 1, 'lambda_l1': 0.7458576619437719, 'lambda_l2': 7.097367279257472e-05}
