...
# Try to discover OOF files (optional). Expected format: columns class_0 ... class_K-1 and 'y' for label
CAND_OOF = [
...
# Learn temperature per model (if OOF exists)
...
if len(oof_list) == 3:
    for key, df in zip(["p1","p2","p3"], oof_list):
        if "y" in df.columns:
            ...
            # convert to logits and do 1D search over T in [0.5, 5.0]
            ...
            Tbest = Ts[int(np.argmin(vals))]
            temperatures[key] = float(Tbest)

# Train meta-stacker (optional)
meta_model = None
if len(oof_list) == 3 and all("y" in df.columns for df in oof_list):
    # Concatenate per-model (temperature-scaled) logits as features
...
# Demographics prior model (optional)
...
def apply_demographics_prior(proba, demographics_df):
    # proba: (1,K), demographics_df: polars.DataFrame for this sequence
    if demo_prior_model is None or demographics_df is None:
        return proba
    try:
        # Convert to pandas single row
        ...
        if isinstance(dp, pd.DataFrame):
            ...
            # Some competitions provide per-subject rows; collapse with mean for numeric
            for c in demo_cols_num:
                if c in x.columns:
                    x[c] = x[c].astype(float)
            # If multiple rows exist, average numeric and take first for categorical
            if len(x) > 1:
                x_num = x[demo_cols_num].mean(axis=0) if demo_cols_num else pd.Series([], dtype=float)
                x_cat = x[demo_cols_cat].iloc[[0]] if demo_cols_cat else pd.DataFrame(index=[0])
                x = pd.concat([x_cat.reset_index(drop=True), pd.DataFrame([x_num]).reset_index(drop=True)], axis=1)
            # Predict class priors from demographics only
            prior = demo_prior_model.predict_proba(x)  # shape (1,K)
            prior = np.clip(prior, EPS, 1.0)
            prior = prior / prior.sum(axis=1, keepdims=True)
            # Combine: p(y|x,d) ∝ p(y|x) * p(y|d)
            out = proba * prior
...
def predict(sequence, demographics):
    # 1) get base probabilities from existing functions
    p1 = predict1(sequence, demographics)[0]  # (K,)
    p2 = predict2(sequence, demographics)[0]
    p3 = predict3(sequence, demographics)[0]

    # 2) temperature scaling (work in logits space)
    ...
    # 3) stacking or weighted average
    if meta_model is not None:
        Xq = np.concatenate([z1, z2, z3], axis=1)
        pq = meta_model.predict_proba(Xq)  # (1,K)
    else:
        # default weights favor diverse models
        w = np.array([0.30, 0.35, 0.35], dtype=float)
        p_ens = _logits_to_prob(w[0]*z1 + w[1]*z2 + w[2]*z3)  # combine in logits-space
        pq = p_ens

    # 4) apply demographics prior if available
    pq2 = apply_demographics_prior(pq, demographics)

    # Return class name via dataset label encoder if available; else argmax index
    try:
