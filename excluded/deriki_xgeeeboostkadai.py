...
agg_funcs = {
    ...
    #"purchase_amount": ["min", "max", "sum"], # 正規化済みの購入金額（負値 = 顧客の支出）
    #"purchase_amount": ["min", "max", "sum", "sem", ], # 正規化済みの購入金額（負値 = 顧客の支出）
    "purchase_amount": ["min", "max", "sum", "sem", "nunique"], # 正規化済みの購入金額（負値 = 顧客の支出）
    #"purchase_amount": ["min", "max", "var"], # 正規化済みの購入金額（負値 = 顧客の支出）
    #"installments":    ["min", "max"], # 分割払いの分割回数3.90895
    #"installments":    ["mean"], # 分割払いの分割回数3.91024
    "installments":    ["min", "max", "mean"], # 分割払いの分割回数3.91442
    #"month_lag":       ["min", "max"], # 基準日からの経過月数
    "month_lag": ["min", "mean", "max"],
    #"month_lag":       ["first", "last"], # 基準日からの経過月数
    #"city_id":["count"]
