...
def display_distances(params):
    ...
    if len(files)==3:
        df1  = pd.read_csv(params['path'] +files[0] +'.csv')
        df2  = pd.read_csv(params['path'] +files[1] +'.csv')
        df3  = pd.read_csv(params['path'] +files[2] +'.csv')
        df12 = pd.merge(df1,df2, on='id', suffixes=('_1','_2'))
        df13 = pd.merge(df1,df3, on='id', suffixes=('_1','_3'))
        df23 = pd.merge(df2,df3, on='id', suffixes=('_2','_3'))
        fig, ax = plt.subplots(2,3, figsize=(9,4))
        sns.kdeplot(df12[f'{target}_1'], label='', fill=True, ax=ax[0,0])
        sns.kdeplot(df12[f'{target}_2'], label='', fill=True, ax=ax[0,0])
        ax[0,0].set_title(f'{files[0]} - {files[1]}')
        sns.kdeplot(df13[f'{target}_1'], label='', fill=True, ax=ax[0,1])
        sns.kdeplot(df13[f'{target}_3'], label='', fill=True, ax=ax[0,1])
        ax[0,1].set_title(f'{files[0]} - {files[2]}')
        sns.kdeplot(df23[f'{target}_2'], label='', fill=True, ax=ax[0,2])
        sns.kdeplot(df23[f'{target}_3'], label='', fill=True, ax=ax[0,2])
        ax[0,2].set_title(f'{files[1]} - {files[2]}')
        sns.scatterplot(x=f'{target}_1', y=f'{target}_2', data=df12.sample(300_000), ax=ax[1,0], alpha=0.2)
        ax[1,0].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_1', y=f'{target}_3', data=df13.sample(300_000), ax=ax[1,1], alpha=0.2)
        ax[1,1].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_2', y=f'{target}_3', data=df23.sample(300_000), ax=ax[1,2], alpha=0.2)
    ...
    if len(files)==4:
        ...
        df2  = pd.read_csv(params['path'] +files[1] +'.csv')
        df3  = pd.read_csv(params['path'] +files[2] +'.csv')
        df4  = pd.read_csv(params['path'] +files[3] +'.csv')
        df12 = pd.merge(df1,df2, on='id', suffixes=('_1','_2'))
        df13 = pd.merge(df1,df3, on='id', suffixes=('_1','_3'))
        df14 = pd.merge(df1,df3, on='id', suffixes=('_1','_4'))
        df23 = pd.merge(df2,df3, on='id', suffixes=('_2','_3'))
        df24 = pd.merge(df2,df4, on='id', suffixes=('_2','_4'))
        df34 = pd.merge(df3,df4, on='id', suffixes=('_3','_4'))
        ...
        sns.kdeplot(df13[f'{target}_3'], label='', fill=True, ax=ax[0,1])
        ax[0,1].set_title(f'{files[0]} - {files[2]}')
        sns.kdeplot(df14[f'{target}_1'], label='', fill=True, ax=ax[0,2])
        sns.kdeplot(df14[f'{target}_4'], label='', fill=True, ax=ax[0,2])
        ax[0,2].set_title(f'{files[0]} - {files[3]}')
        sns.kdeplot(df23[f'{target}_2'], label='', fill=True, ax=ax[2,0])
        sns.kdeplot(df23[f'{target}_3'], label='', fill=True, ax=ax[2,0])
        ax[2,0].set_title(f'{files[1]} - {files[2]}')
        sns.kdeplot(df24[f'{target}_2'], label='', fill=True, ax=ax[2,1])
        sns.kdeplot(df24[f'{target}_4'], label='', fill=True, ax=ax[2,1])
        ax[2,1].set_title(f'{files[1]} - {files[3]}')
        sns.kdeplot(df34[f'{target}_3'], label='', fill=True, ax=ax[2,2])
        sns.kdeplot(df34[f'{target}_4'], label='', fill=True, ax=ax[2,2])
        ax[2,2].set_title(f'{files[2]} - {files[3]}')
        sns.scatterplot(x=f'{target}_1', y=f'{target}_2', data=df12.sample(300_000), ax=ax[1,0], alpha=0.2)
        ax[1,0].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_1', y=f'{target}_3', data=df13.sample(300_000), ax=ax[1,1], alpha=0.2)
        ax[1,1].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_1', y=f'{target}_4', data=df14.sample(300_000), ax=ax[1,2], alpha=0.2)
        ax[1,2].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_2', y=f'{target}_3', data=df23.sample(300_000), ax=ax[3,0], alpha=0.2)
        ax[3,0].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_2', y=f'{target}_4', data=df24.sample(300_000), ax=ax[3,1], alpha=0.2)
        ax[3,1].plot([0,1],[0,1],'r--')
        sns.scatterplot(x=f'{target}_3', y=f'{target}_4', data=df34.sample(300_000), ax=ax[3,2], alpha=0.2)
        ax[3,2].plot([0,1],[0,1],'r--')
        plt.show()
...
params = {
      'path'     : f'/kaggle/input/12-december-2025-ps-s5e12/',            
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/100 for w in [ +4,+3,-7,-4 ]],
      'subm'     : [
          {'name': f'0.69900', 'weight':+0.25, 'color':'crimson'},
          {'name': f'0.69903', 'weight':+0.25, 'color':'green'  },
          {'name': f'0.69908', 'weight':+0.25, 'color':'blue'   },
          {'name': f'0.69925', 'weight':+0.25, 'color':'gold'   },]
}
df1 = h_blend(params, details=True, subm='g1.csv')
...
params = {
      ...
      'subwts'   : [w/100 for w in [ +4,+3,-7,-4 ]],
      'subm'     : [
          {'name': f'0.69910', 'weight':+0.25, 'color':'crimson'},
          {'name': f'0.69955', 'weight':+0.25, 'color':'green'  },
          {'name': f'0.69975', 'weight':+0.25, 'color':'blue'   },
          {'name': f'0.70010', 'weight':+0.25, 'color':'gold'   },]
}
df2 = h_blend(params, details=True, subm='g2.csv')
...
params = {
      ...
      'subwts'   : [w/100 for w in [+4,+3,-7,-4]],
      'subm'     : [
          {'name': f'0.70370', 'weight':+0.10, 'color':'crimson'},
          {'name': f'0.70371', 'weight':+0.20, 'color':'green'  },
          {'name': f'0.70374', 'weight':+0.30, 'color':'blue'   },
          {'name': f'0.70377', 'weight':+0.40, 'color':'gold'   },]
}
df3 = h_blend(params, details=True, subm='g3.csv')
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/100 for w in [ +3, -1,-3 ]],
      'subm'     : [
          {'name': f'g1', 'weight':+0.01, 'color':'red'   },
          {'name': f'g2', 'weight':+0.02, 'color':'blue'  },
          {'name': f'g3', 'weight':+0.97, 'color':'yellow'},]
}
df4 = h_blend(params, details=True)
