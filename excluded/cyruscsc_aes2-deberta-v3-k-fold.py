...
df_train.rename(columns={"score": "labels"}, inplace=True)
df_train["labels"] = df_train["labels"].astype(float)


LOAD_DIR = "/kaggle/input/aes2-deberta-v3-k-fold-finetuned/deberta"
OUT_DIR = "deberta"
...
def load_or_train_model(fold, train_data, val_data):
    suffix = f"/fold-{fold}"
    
    if LOAD_DIR:
        model = AutoModelForSequenceClassification.from_pretrained(LOAD_DIR + suffix, config=config)
    else:
    ...
    if not LOAD_DIR:
        trainer.train()
        model.save_pretrained(OUT_DIR + suffix)
...
for i, (train_idx, val_idx) in enumerate(skf.split(df_train, df_train["labels"])):
    train_data = tokenized_ds_train.select(train_idx)
    val_data = tokenized_ds_train.select(val_idx)
    load_or_train_model(i, train_data, val_data)
...
for fold in range(FOLDS):
    ...
    if LOAD_DIR:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(OUT_DIR + suffix, config=config)
...
mean_preds = np.clip(np.round(np.mean(all_preds, axis=0)), 1, 6).astype(int)

# save csv

df_test["score"] = mean_preds
