...
agg_funcs = {
    ...
    "purchase_date":   ["count", "size"] # 購入日時（countで空でない行を数える ⇔ 購入回数）
}
...
# 追加テーブルの読み込み
new_trans2 = pd.read_csv(f"{DATA_DIR}/historical_transactions.csv")
print(new_trans2.describe())

# 集約特徴の作成パターンを辞書化
agg_funcs2 = {
    "purchase_amount": ["min", "max", "mean", "var"], # 正規化済みの購入金額（負値 = 顧客の支出）
    "installments":    ["min", "max", "mean", "first", "last"], # 分割払いの分割回数
...
# 全ての行を card_id の値でグループ化
# ⇒　上の集約パターンに従って実際に特徴を集約
agg_new2 = new_trans2.groupby("card_id").agg(agg_funcs2)

# 備考）上の集約特徴のテーブルは，列が MultiIndex の状態になっている
# 　　　⇒　列名のリストを表示して確認してみよう
print(agg_new2.columns.to_list())
# 1列を指定するために「変数名と集約関数」をペアで指定する必要があることがわかる

# 新特徴の名前（列名）を機械的に生成してリスト化
# 　　命名規則：元変数名_集約方法（「purchase_amount_mean」など）
flat_cols2 = []
for col_name2, func_name2 in agg_new2.columns.to_flat_index():
    new_name2 = f"new_{col_name2}_{func_name2}"
    flat_cols2.append(new_name2)

# MultiIndex状態の列を通常の状態に変換（列方向に平坦化）
agg_new2.columns = flat_cols2
print(agg_new2.columns.to_list())

# groupby によるグループ化を解除（card_idを通常の列に戻す）
agg_new2 = agg_new2.reset_index()

# メインテーブルの特徴と結合
train_x = train_x.merge(agg_new2, on="card_id", how="left")
test_x  = test_x.merge(agg_new2, on="card_id", how="left")
