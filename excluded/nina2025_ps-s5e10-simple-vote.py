...
df_vote.to_csv('a-vote of all.csv',index=False)


df = pd.read_csv('/kaggle/input/22-october-2025-ps-s5e10/submission_0.05539.f.csv')
df . to_csv('c-copy of 39.f.csv',index=False) # redirrect
...
params = {
      ...
      'subm'     : [
         { 'name': f'a-vote of all', 'weight': 0.98 },
         { 'name': f'c-copy of 39.f','weight': 0.01 },
         { 'name': f'h-blend of all','weight': 0.01 },]
}
df_cross2 = h_blend(params, color='tes3', figures1=True, details=True)

display_distances(params);             os.remove('c-copy of 39.f.csv')


df_cross2.to_csv('submission_cross.csv',index=False)
df_cross2
