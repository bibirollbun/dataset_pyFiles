...
add = pd.read_csv("/kaggle/input/calories-burnt-prediction/calories.csv")

add.drop("User_ID", inplace=True, axis=1)

add = add.rename(columns={"Gender": "Sex"})
...
model = CatBoostRegressor(learning_rate = 0.99,
                          l2_leaf_reg = 0.99,
                          depth=8,
...
print("CatBoost RMSLE score: ", msle)
