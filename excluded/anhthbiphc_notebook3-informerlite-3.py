...
def append_result(row: Dict[str, Any], csv_path: str = "/kaggle/working/transformer_results.csv"):
...
def run_length_sweep(train_df: pd.DataFrame,
                     ...
                     results_csv: str = "/kaggle/working/informerlite_results.csv",
...
# Giáº£ sá»­ lá»›p Ä‘áº§u ra cuá»‘i cÃ¹ng cá»§a báº¡n lÃ  self.fc
        try:
    # Láº¥y shape cá»§a weight (num_classes, input_dim)
            output_shape = model.fc[-1].weight.shape 
            num_classes_trained = output_shape[0]
            print(f"\n[DEBUG] Sá»‘ lá»›p Ä‘áº§u ra cá»§a mÃ´ hÃ¬nh Vá»ªA HUáº¤N LUYá»†N lÃ : {num_classes_trained}")
            if num_classes_trained != 9:
                print("ğŸš¨ Cáº¢NH BÃ�O: MÃ´ hÃ¬nh Vá»ªA HUáº¤N LUYá»†N cÃ³ sá»‘ lá»›p KHÃ”NG PHáº¢I 9. Quay láº¡i kiá»ƒm tra MODEL_BUILDER.")
        # Sá»¬A Lá»–I: DÃ¹ng biáº¿n train_time_s thay vÃ¬ total_train_time
                return best, train_time_s, infer_ms, params_m, peak_gb, epoch_logs 
        except AttributeError:
            print("[DEBUG] KhÃ´ng tÃ¬m tháº¥y lá»›p .fc[-1] trong mÃ´ hÃ¬nh. Cáº§n chá»‰nh sá»­a Ä‘Æ°á»�ng dáº«n lá»›p Ä‘áº§u ra.")
# -------------------------------------------------------------
# -------------------------------------------------------------
        # 1) LÆ°u feature cols
...
class CompetitionMetric:
    ...
    def calculate_hierarchical_f1(self, sol: pd.DataFrame, sub: pd.DataFrame) -> float:
        # Validate gestures
        invalid_types = {i for i in sub['gesture_grouped'].unique() if i not in self.all_classes}
        if invalid_types:
            raise ParticipantVisibleError(f"Invalid gesture values in submission: {invalid_types}")

        # Compute binary F1 (Target vs Non-Target)
        y_true_bin = sol['gesture_grouped'].isin(self.target_gestures).values
        y_pred_bin = sub['gesture_grouped'].isin(self.target_gestures).values
        f1_binary = f1_score(
            y_true_bin,
            y_pred_bin,
            pos_label=True,
        ...
        y_true_mc = sol['gesture_grouped'].apply(lambda x: x if x in self.target_gestures else 'non_target')
        y_pred_mc = sub['gesture_grouped'].apply(lambda x: x if x in self.target_gestures else 'non_target')
...
def score(solution: pd.DataFrame, submission: pd.DataFrame, row_id_column_name: str) -> float:
    ...
    Expected input:
      ...
      - 'gesture_grouped': one of the eight target gestures or "Non-Target"
    """
    # Validate required columns
    for col in (row_id_column_name, 'gesture_grouped'):
...
from nb3_models import InformerLite

def MODEL_BUILDER(in_channels, num_classes):
    return InformerLite(in_channels, num_classes, d_model=128, nhead=4, num_layers=2,
                        dim_feedforward=256, dropout=0.1, topk=64)

MODEL_NAME = "InformerLite"
NOTEBOOK_NAME = "NB3-3 InformerLite"
...
run_full_evaluation(train_df, MODEL_BUILDER, MODEL_NAME, NOTEBOOK_NAME,
                    ...
                    lengths=[50,100,200], optimizer_name="AdamW", lr=1e-3,
                    epochs=5, batch_size=64, grad_accum=1, amp=True,
                    runtime_prefix="mp01", results_csv="/kaggle/working/informerlite_results.csv",
...
csv_path = "/kaggle/working/informerlite_results.csv"
...
CHECKPOINT = "/kaggle/working/InformerLite_L100.pth"

model = MODEL_BUILDER(len(FEATURE_COLS), len(GESTURE_CLASSES))
state = torch.load(CHECKPOINT, map_location="cpu")
model.load_state_dict(state, strict=True)
print(state["fc.1.weight"].shape)
...
#!rm -f /kaggle/working/*.pth



import torch

state = torch.load("/kaggle/working/InformerLite_L100.pth", map_location="cpu")
print(state["fc.1.weight"].shape)
