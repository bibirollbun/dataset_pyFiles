...
def ensemble_predictions(weight_nb1=0.5, weight_nb2=0.5):
    ...
    # 3. Weighted Ensemble
    #final_probs = (probs1 * weight_nb1) + (probs2 * weight_nb2)

    #Weighted Square average
    p1_squared = probs2 * probs2
    p2_squared = probs2 * probs2
    final_probs = (weight_nb1 * p1_squared) + (weight_nb2 * p2_squared)
    
    # 4. Create Submission
    top3_indices = np.argsort(-final_probs, axis=1)[:, :3]
