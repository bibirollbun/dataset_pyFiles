...
# --- Cell 5: make submission from RAW best ---
import torch, pandas as pd
from pathlib import Path

ckpt = torch.load("best_raw.pth", map_location="cpu")
model.load_state_dict(ckpt["model"], strict=True)
model = model.to(device).to(memory_format=torch.channels_last)
model.eval()
...
with torch.no_grad():
    for x, img_ids in test_dl:
        x = x.to(device, non_blocking=True).to(memory_format=torch.channels_last)
        p = predict_tta(x).argmax(1).tolist()
        preds.extend([id2label[i] for i in p]); ids.extend(img_ids)

sub = pd.DataFrame({"id": ids, "label": preds}).sort_values("id").reset_index(drop=True)
sub.to_csv("submission.csv", index=False)
print("Saved submission.csv (raw-best). Rows:", len(sub))
