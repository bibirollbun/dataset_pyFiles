...
for df in [train, test]:
    ...
    df['weekofyear']   = df[col].dt.isocalendar().week.astype(int)
    df['is_month_start']   = df[col].dt.is_month_start.astype(int)
    df['is_month_end']     = df[col].dt.is_month_end.astype(int)
    df['is_quarter_start']= df[col].dt.is_quarter_start.astype(int)
    df['is_quarter_end']  = df[col].dt.is_quarter_end.astype(int)

# 新特徴を train_x/test_x に移す
new_cols = ['year','month','day','dayofyear','weekofyear',
...
# 簡易 one-hot（ダミー）化が必要なカテゴリ変数を指定
oh_cols = ['dow','month','weekofyear']
train_x = pd.get_dummies(train_x, columns=oh_cols, drop_first=True)
test_x  = pd.get_dummies(test_x , columns=oh_cols, drop_first=True)
