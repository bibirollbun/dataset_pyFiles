...
# 追加テーブルの読み込み
_new_trans = pd.read_csv(f"{DATA_DIR}/new_merchant_transactions.csv")
_hist_trans = pd.read_csv(f"{DATA_DIR}/historical_transactions.csv")

# new_trans扱いでマージ
new_trans = pd.concat([_new_trans, _hist_trans], ignore_index=True)
...
agg_funcs = {
    "purchase_amount": ["min", "max", "mean", "var", "skew"], # mean, var, skewを追加
    "installments":    ["min", "max"],
    "month_lag":       ["min", "max", "mean"], # meanを追加
    "purchase_date":   ["count"],
    "category_1":      ["mean"], # 割引対象かどうか(Y:N = 1:0)
