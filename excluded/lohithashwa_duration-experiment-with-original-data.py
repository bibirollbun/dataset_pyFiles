...
class CFG:
    ...
    # Augmented Data
    #train_datadir = '/kaggle/input/augmented-dataset/augmented_audio_2/augmented_audio_2'
    #train_csv = '/kaggle/input/augmented-dataset/augmented.csv'
...
def random_fragment(audio, expected_length):
    audio_length = len(audio)

    if audio_length <= expected_length:
        return audio 

    max_start = audio_length - expected_length
    start_sample = np.random.randint(0, max_start)
    end_sample = start_sample + expected_length
...
def process_audio_file(audio_path, cfg):
    ...
    try:
        ...
        audio_data, _ = librosa.load(
                ...
                # duration=cfg.TARGET_DURATION  # Only load what we need
            )
        ...
        # Expected Length = 5 seconds * 32000 sample rate = 160000 samples
        expected_length = int(cfg.FS * cfg.TARGET_DURATION)
        ...
        if len(audio_data) < expected_length:
            ...
            # Trim if longer
            audio_data = random_fragment(audio_data, expected_length) # Use random trim
