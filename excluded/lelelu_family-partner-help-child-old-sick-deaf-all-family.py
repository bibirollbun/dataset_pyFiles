...
system = "You help patents accompy the child when they are away."\
...
dataset = load_dataset("Kiran2004/MentalHealthConversations", split = "train[300:1000]")
...
for index, row in train_df.iterrows():
    ...
    question_map = {'from': 'human',
                    'value': row['Questions']
                   }
    answer_map = {'from': 'gpt',
                    'value': row['Answers']}
    talk_history.append(question_map)
    talk_history.append(answer_map)
    str_list.append(talk_history)
        
train_df['conversations'] = str_list
...
messages = [{
    ...
    "content": [{
        "type" : "text",
        "text" : "I feel really depressed, could not find a job for half a year, feel like a failue",
    }]
}]
inputs = tokenizer.apply_chat_template(
    messages,
