...
plot_his(df_com, [	'CompetitionOpenSinceMonth',	'CompetitionOpenSinceYear', 'Promo2SinceWeek',	'Promo2SinceYear',	'PromoInterval']) # biáº¿n liÃªn tá»¥c
...
def fill_nan_by_distribution(df, col):
    np.random.seed(42)
    # Láº¥y cÃ¡c giÃ¡ trá»‹ khÃ´ng NaN
    value_counts = df[col].value_counts(normalize=True)
    
    # Láº¥y sá»‘ lÆ°á»£ng NaN
    n_missing = df[col].isna().sum()
    
    # Láº¥y máº«u theo phÃ¢n phá»‘i gá»‘c
    ...
    # GÃ¡n láº¡i vÃ o cÃ¡c vá»‹ trÃ­ NaN
...
model_la= RandomForestRegressor()
...
model_es= RandomForestRegressor()
...
df_xgb = df_encode.drop(columns = [ 'StateHoliday_a', 'StateHoliday_b', 'StateHoliday_c'])
...
y_pred_test = best_model_xgb.predict(X_test_scaled1)
end_time = time.time()
...
model_bytes = pickle.dumps(best_model_xgb)
