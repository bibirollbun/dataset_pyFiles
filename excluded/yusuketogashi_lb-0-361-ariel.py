# =========================================================
# Ariel Data Challenge 2025 — 安全パッチ版 (0.361ベース → 0.362狙い)
# 変更点（最小限）:
#  - (#2 軽微) Savitzky–Golay 窓 23→31、MODEL_OPTIMIZATION_DELTA 11→9
#  - (#6 軽微) AIRS 重みの分位クリップを 5–95 → 10–90 に
# 既存の #4（σ拡張）と #5（AIRSスムージング0.7:0.3）は維持
# =========================================================
...
class Config:
    ...
    MODEL_OPTIMIZATION_DELTA = 9    # ← 11 → 9（軽微）
...
class SignalProcessor:
    ...
    def _calibrate_single_signal(self, planet_id, sensor):
        ...
        if sensor == "FGS1":
            y0, y1, x0, x1 = 10, 22, 10, 22   # ← ROI変更なし（0.361のまま）
    ...
    def _preprocess_calibrated_signal(self, calibrated_signal, sensor):
        ...
        if sensor == "AIRS-CH0":
            # 0.361版と同じ: 分散ベースの重み。ただし分位クリップを 10–90 に（#6）
            ...
            lo, hi = np.nanpercentile(w, 10.0), np.nanpercentile(w, 90.0)  # ← 5–95 → 10–90
...
class TransitModel:
    ...
    def predict(self, single_preprocessed_signal):
        ...
        signal_1d = savgol_filter(signal_1d, 31, 2)   # ← 23 → 31（#2）
...
if __name__ == "__main__":
    ...
    # (#5) AIRS 出力の軽スムージング・アンサンブル（維持）
    predictions1_smooth = savgol_filter(predictions1, window_length=9, polyorder=2, axis=1)
