!pip install transformers peft pandas tqdm scikit-learn
...
# Load your CSV files
train_df = pd.read_csv("/workspace/train.csv")  # Columns: "Question", "label"
test_df = pd.read_csv("/workspace/test.csv")    # Columns: "id", "Question"
...
def get_llrd_params(model, base_lr=1e-5, lora_lr=1e-4, decay=0.9):
    # Identify LoRA and transformer parameters
    no_decay = ["bias", "LayerNorm.weight"]
    grouped_params = []
    # LoRA parameters (usually named 'lora')
    lora_params = [p for n, p in model.named_parameters() if "lora" in n]
    if lora_params:
        grouped_params.append({"params": lora_params, "lr": lora_lr, "weight_decay": 0.0})
    # Transformer layers
    if hasattr(model, "base_model"):
        encoder = model.base_model.model if hasattr(model.base_model, "model") else model.base_model
    else:
        encoder = model
    layers = []
    if hasattr(encoder, "encoder"):
        layers = list(encoder.encoder.layers) if hasattr(encoder.encoder, "layers") else list(encoder.encoder.layer)
    elif hasattr(encoder, "transformer"):
        layers = list(encoder.transformer.h) if hasattr(encoder.transformer, "h") else []
    # Assign decayed LRs to each layer
    lr = base_lr
    for layer in reversed(layers):
        grouped_params.append({
            "params": [p for n, p in layer.named_parameters() if not any(nd in n for nd in no_decay) and "lora" not in n],
            "lr": lr,
            "weight_decay": 0.01,
        })
        grouped_params.append({
            "params": [p for n, p in layer.named_parameters() if any(nd in n for nd in no_decay) and "lora" not in n],
            "lr": lr,
            "weight_decay": 0.0,
        })
        lr *= decay
    # Add embedding layer if present
...
# --- LoRA Config
    ...
    grouped_params = get_llrd_params(model, base_lr=1e-5, lora_lr=1e-4, decay=0.9)
