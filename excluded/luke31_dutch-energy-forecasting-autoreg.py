...
# max_lag_steps 672
def make_features(df, weather_df):
    # weather
...
train_features = make_features(train_df, weather_df)
train_features


Xy_train = train_features
# .filter(
#     (pl.col("timestamp_utc") >= train_df["timestamp_utc"].min()) & (pl.col("timestamp_utc") <= train_df["timestamp_utc"].max())
# )
...
class GBMTrainer:
    ...
    def train_model(self, X_tr, y_tr, X_val=None, y_val=None, plot=True, log=True) -> Any:
        ...
        self.trend_model.fit(X_tr_scaled[["timestamp_numeric"]], y_tr_scaled)
        y_tr_trend_scaled = self.trend_model.predict(X_tr_scaled[["timestamp_numeric"]])
        y_tr_resid_scaled = y_tr_scaled - y_tr_trend_scaled
        
        train_data = lgb.Dataset(X_tr_scaled, label=y_tr_resid_scaled)
        ...
        if X_val is not None and y_val is not None:
            ...
            y_val_trend_scaled = self.trend_model.predict(X_val_scaled[["timestamp_numeric"]])
            y_val_resid_scaled = y_val_scaled - y_val_trend_scaled
            
            val_data = lgb.Dataset(X_val_scaled, label=y_val_resid_scaled, reference=train_data)
    ...
    def predict(self, X) -> Any:
        ...
        pred_trend_scaled = self.trend_model.predict(X_scaled[["timestamp_numeric"]])
        pred_scaled_resid = self.model.predict(X_scaled, num_iteration=self.model.best_iteration)
        pred_scaled = pred_trend_scaled + pred_scaled_resid
...
df_combined_submission = pl.concat([train_df, submission_df_no_rowid])
