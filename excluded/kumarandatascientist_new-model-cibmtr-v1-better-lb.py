...
class CFG:
    ...
    weights = [3.085125, 3.085125, 10.085125, 6.085125, 10.085125, 6.085125, 8.085125, 8.085125]
...
# Assuming CFG.subm_path and ensemble_preds are already defined

# Load the submission data
try:
    subm_data = pd.read_csv(CFG.subm_path)
except Exception as e:
    raise FileNotFoundError(f"Error loading CSV file from {CFG.subm_path}: {e}")

# Verify ensemble_preds is valid
if not isinstance(ensemble_preds, (np.ndarray, pd.Series)):
    raise TypeError("ensemble_preds must be a NumPy array or Pandas Series")

# Ensure lengths match
if len(subm_data) != len(ensemble_preds):
    raise ValueError("Length mismatch: subm_data and ensemble_preds must have the same number of elements")

# Add 0.5 to ensemble_preds and assign to 'prediction' column
try:
    subm_data['prediction'] = (ensemble_preds + 0.5).astype(float)
except Exception as e:
    raise ValueError(f"Error creating 'prediction' column: {e}")

# Optional: Save the updated submission data to verify correctness
output_path = "submission.csv"  # Update this path as needed
try:
    subm_data.to_csv(output_path, index=False)
    display(subm_data.head())
    print(f"Updated submission file saved to {output_path}")
except Exception as e:
    raise IOError(f"Error saving updated submission file: {e}")
