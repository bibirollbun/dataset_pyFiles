...
temperature = 1.3    # Higher temperature = more random/creative outputs
top_p = 0.95         # Nucleus sampling parameter for more diverse outputs (1.0 disables filtering)
...
def get_response(messages, trim_numbered_lists=True):
    ...
    # If "1." is found, truncate before it (prevent Phi from giving a numbered list)
    list_start = response.find("1.")
    if trim_numbered_lists and list_start != -1:
        response = response[:list_start].strip()
...
def get_essays(df, messages_and_prompts, verbose=True):
    ...
    for i, row in df.iterrows():
        ...
        weights = [item[2] for item in messages_and_prompts]    
...
#example messages, followed by prompt suffixes followed by proportion of time selected. (does not need to add up to 1)

#"None" may be used for no example messages / prompt suffix
messages_and_prompts = [
                        (get_add_it_up_example_messages("from Google"), get_add_it_up_suffix("Gemma"), 0.2),  
                        (get_add_it_up_example_messages("Qwen"), get_add_it_up_suffix("Qwen"), 0.2), 
                        (get_add_it_up_example_messages("Mistral"), get_add_it_up_suffix("Mistral"), 0.2),  

                        (cut_off_date_example_messages, cut_off_date_final_suffix, 0.15), 
                        (None, None, 0.15), 
                       ]

print("Weighting of prompts:")
weights = [tup[2] for tup in messages_and_prompts]
