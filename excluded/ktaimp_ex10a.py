...
flat_cols = []
for col_name, func in agg_new.columns:
    # ラムダ関数などを文字列化
    func_name = func.__name__ if hasattr(func, '__name__') else str(func)
    func_name = func_name.replace("<lambda>", "lambda")
...
# ----------------------------------------------
# 過去取引データ（historical_transactions）による特徴量の追加
# ----------------------------------------------
...
agg_funcs_hist = {
    ...
    "purchase_date": [
        "count", "nunique",
        lambda x: x.min(),  # first の代用
        lambda x: x.max()   # last の代用
    ],
    "city_id": ["nunique"],
    "category_1": ["nunique"]
}
...
flat_cols_hist = []
for col_name, func in agg_hist.columns:
    func_name = func.__name__ if hasattr(func, '__name__') else str(func)
    func_name = func_name.replace("<lambda>", "lambda")
    new_name = f"hist_{col_name}_{func_name}"
...
print("削除する object 型の列：", object_cols.tolist())
train_x = train_x.drop(columns=object_cols)
test_x = test_x.drop(columns=object_cols)
