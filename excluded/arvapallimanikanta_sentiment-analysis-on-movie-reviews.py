...
# Works with zipped files: train.tsv.zip / test.tsv.zip
...
import os, re, string, io, zipfile
import numpy as np
import pandas as pd
...
def read_tsv_or_zip(tsv_path, zip_path):
    """
    If unzipped TSV exists, read it.
    Else read from .zip (expects a single .tsv member inside).
    """
...
# Sanity checks
if "Phrase" not in train.columns or "Sentiment" not in train.columns:
    raise ValueError("Expected columns 'Phrase' and 'Sentiment' in train.tsv")
if "Phrase" not in test.columns:
    raise ValueError("Expected column 'Phrase' in test.tsv")
...
vectorizer = TfidfVectorizer(
    sublinear_tf=True,
    stop_words="english",
    max_features=40000,   # increase if you have RAM
...
# Bar of validation accuracy
plt.figure(figsize=(6,4))
plt.bar(list(val_scores.keys()), list(val_scores.values()))
plt.ylim(0,1)
...
# 10) Submission
# ----------------------------
# sampleSubmission.csv has columns: PhraseId, Sentiment
sub = sample_sub.copy()
cols_lower = [c.lower() for c in sub.columns]
if not ("phraseid" in cols_lower and "sentiment" in cols_lower):
    raise ValueError(f"Expected [PhraseId, Sentiment] in sample submission, got: {list(sample_sub.columns)}")
