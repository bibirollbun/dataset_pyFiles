...
from torchvision.datasets import ImageFolder # Included for completeness, though CustomCassavaDataset is primarily used
...
# This function is generally NOT needed if using CustomCassavaDataset as implemented below.
# It's kept for completeness but can be ignored or commented out.
...
# --- Modified ResNet50 Classifier Model Definition (handles Baseline, CCIA, and ASDA) ---
class ResNet50_Classifier(nn.Module):
    # Added weights_init_type parameter
    def __init__(self, num_classes=Config.NUM_CLASSES, use_ccia=False, use_asda=False, weights_init_type='random'):
        ...
        if weights_init_type == 'imagenet':
            # Loads ImageNet pretrained weights (requires internet for first download)
            self.resnet = models.resnet50(weights=models.ResNet50_Weights.IMAGENET1K_V1) 
            print("ResNet50 backbone initialized with ImageNet pretrained weights.")
        else: # 'random' or 'None' as per user request
            # Initializes ResNet50 with random weights (no internet needed for this part)
        ...
        # Define CCIA modules if enabled
        if self.use_ccia:
        ...
        # Define ASDA modules if enabled
        if self.use_asda:
    ...
    def forward(self, x):
        ...
        if self.use_asda: x = self.asda_layer1(x) # Applied sequentially if both enabled
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, device, model_name="model"):
    ...
    for epoch in range(num_epochs):
        ...
        # Corrected unpacking here for evaluate_model
...
if __name__ == "__main__":
    ...
    # --- Step 1: Initialize and Train ResNet50 + CCIA + ASDA Model ---
    print("\n--- Initializing and Training ResNet50 + CCIA + ASDA Model ---")
    # Instantiate the model with CCIA and ASDA enabled, and NO ImageNet pretrained weights for backbone
    model_combined = ResNet50_Classifier(num_classes=Config.NUM_CLASSES, use_ccia=True, use_asda=True, weights_init_type='random') 
    criterion_combined = nn.CrossEntropyLoss()
    optimizer_combined = optim.Adam(model_combined.parameters(), lr=Config.LEARNING_RATE)

    # Train the combined model
    train_model(model_combined, train_loader, val_loader, criterion_combined, optimizer_combined, Config.NUM_EPOCHS, Config.DEVICE, model_name="resnet50_ccia_asda")


    # --- Step 2: Final Evaluation of ResNet50 + CCIA + ASDA & Submission Generation ---
    print("\n--- Final Evaluation of ResNet50 + CCIA + ASDA Model & Submission Generation ---")
    
    # Instantiate the same model architecture for loading weights (CCIA and ASDA enabled, no ImageNet weights)
    best_model_combined = ResNet50_Classifier(num_classes=Config.NUM_CLASSES, use_ccia=True, use_asda=True, weights_init_type='random') 
    best_model_combined_path = os.path.join('/kaggle/working', "resnet50_ccia_asda_best_model.pth") 

    if not os.path.exists(best_model_combined_path):
        print(f"Error: Best ResNet50 + CCIA + ASDA model not found at {best_model_combined_path}. Cannot proceed with evaluation or submission.")
    else:
        best_model_combined.load_state_dict(torch.load(best_model_combined_path, map_location=Config.DEVICE))
        best_model_combined.to(Config.DEVICE) 
        best_model_combined.eval() # Set model to evaluation mode

        # --- Evaluate on Validation Set ---
        print("\n--- Evaluating ResNet50 + CCIA + ASDA on Validation Set ---")
        val_loss_combined, val_accuracy_combined, val_f1_combined, _, _, cm_combined = evaluate_model(
            best_model_combined, val_loader, criterion_combined, Config.DEVICE
        )
        print(f"Best ResNet50 + CCIA + ASDA Val Loss: {val_loss_combined:.4f}")
        print(f"Best ResNet50 + CCIA + ASDA Val Accuracy: {val_accuracy_combined:.4f}")
        print(f"Best ResNet50 + CCIA + ASDA Val F1-Score (Macro): {val_f1_combined:.4f}")
        print("Confusion Matrix:\n", cm_combined)

        # --- Generate Submission File for Test Data ---
        print("\n--- Generating Submission File for ResNet50 + CCIA + ASDA ---")

        # Test transforms (usually same as val_transforms for consistency)
        ...
        with torch.no_grad(): 
            for inputs, _ in tqdm(test_loader, desc="Predicting on test set with ResNet50+CCIA+ASDA"): 
                inputs = inputs.to(Config.DEVICE)
                outputs = best_model_combined(inputs) # Use the combined model here
                _, predicted = torch.max(outputs.data, 1)
                all_test_predictions_combined.extend(predicted.cpu().numpy())

        # Create the final submission DataFrame
        submission_df_combined = pd.DataFrame({
            'image_id': test_image_ids, 
            'label': all_test_predictions_combined
        })

        # Save the submission file to /kaggle/working/
        submission_file_path_combined = os.path.join('/kaggle/working', 'submission.csv') # Unique name for combined submission
        submission_df_combined.to_csv(submission_file_path_combined, index=False)

        print(f"\nSubmission file for ResNet50 + CCIA + ASDA saved to: {submission_file_path_combined}")
        print("First 5 rows of submission_resnet50_ccia_asda.csv:")
        print(submission_df_combined.head())

    print("\nResNet50 + CCIA + ASDA experiment complete.")
