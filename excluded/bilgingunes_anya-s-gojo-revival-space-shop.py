...
train=pd.read_csv("/kaggle/input/anyas-gojo-revival-space-shop/train.tsv",sep='\t')
test=pd.read_csv('/kaggle/input/anyas-gojo-revival-space-shop/test.tsv',sep='\t')
sample=pd.read_csv("/kaggle/input/anyas-gojo-revival-space-shop/sample_submission.csv")
...
train = train.dropna(subset=['verified_reviews'])
...
value_counts.plot(kind="bar",color=["blue","yellow","red","pink","green"])
...
test['lang'] = test['verified_reviews'].astype(str).apply(detect_language_safe)
...
df['text']=df['verified_reviews'].str.lower() 
...
fnc_classification_all_model(x_train,new_y_train)
...
test1=pd.read_csv('/kaggle/input/anyas-gojo-revival-space-shop/test.tsv',sep='\t')
