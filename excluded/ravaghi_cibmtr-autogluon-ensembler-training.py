...
ag_train = pd.DataFrame(oof_preds)
...
ag_train = ag_train[selected_features]
ag_test = ag_test[selected_features]


kf = KFold(n_splits=CFG.n_folds, random_state=CFG.seed, shuffle=True)
split = kf.split(ag_train, train[["efs", "efs_time"]])

for i, (train_index, val_index) in enumerate(split):
    ag_train.loc[val_index, "fold"] = i
    
    ag_train.loc[train_index, "quantile"], ag_train.loc[val_index, "quantile"] = TargetTransformer(
        train.loc[train_index, ["efs", "efs_time"]], 
        train.loc[val_index, ["efs", "efs_time"]]
    ).fit_transform("quantile")
