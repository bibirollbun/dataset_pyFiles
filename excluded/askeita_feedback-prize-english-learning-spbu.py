...
# Set these before any torch imports
...
def predict_chunk(test: pd.DataFrame) -> pd.DataFrame:
    for _idx, CFG in enumerate(CFG_list):
        ...
        test_loader = DataLoader(
            ...
            num_workers=0,  # Critical change here
            pin_memory=True,
            drop_last=False
        )
        ...
        for fold in CFG.trn_fold:
            ...
            # Break after first fold if you only want one fold
            #break
            
        # Process predictions
        predictions = np.mean(predictions, axis=0)
        test[CFG.target_cols] = predictions
        submission = submission.drop(columns=CFG.target_cols).merge(
            test[['text_id'] + CFG.target_cols], on='text_id', how='left'
        )
        return submission
...
def get_fb3_predicted(df_test = None):
    ...
    if len(df_test) < 10:
        fb3_predicted = predict_chunk(
            df_test.rename(columns={"essay_id": "text_id"})
        )
    else:
        fb3_predicted = predict(
            df_test.rename(columns={"essay_id": "text_id"})
        )
...
fb3_predicted = get_fb3_predicted(test)
fb3_predicted.to_csv('/kaggle/working/submission.csv',index=False)
fb3_predicted
