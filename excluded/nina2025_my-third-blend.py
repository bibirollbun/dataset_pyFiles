...
if submissions == f'Added-1-8':
    p = '/kaggle/input/fertilizer-1-8/submission__LB_'
    
    V  = pd.read_csv(p + '0_36863__v01__' + 'Vishnupriya'    + '___Added.csv') # LB = 0.36_874
    M  = pd.read_csv(p + '0_36826__v02__' + 'Mikhail_Naumov' + '___Added.csv') # LB = 0.36_901
...
if option == 1:
    Vs  = V.iloc[      0 : 100_000]
    Ms  = M.iloc[100_000 : 250_001]      # LB=0.36_901 < LB=0.36_901 in blend of 5 solutions
    df  = pd.concat([Vs,Ms], axis=0)

if option == 2:
    Ms1 = M.iloc[      0 : 100_000]
    Vs  = V.iloc[100_000 : 150_000]      # LB=0.36_900
    Ms2 = M.iloc[150_000 : 250_000]
    df  = pd.concat([Ms1,Vs,Ms2], axis=0)

if option == 3:
    Vs1 = V.iloc[      0 :  25_000]
    Ms  = M.iloc[ 25_000 : 225_000]      # LB=0.36_891
    Vs2 = V.iloc[225_000 : 250_001]
    df  = pd.concat([Vs1,Ms,Vs2], axis=0)
