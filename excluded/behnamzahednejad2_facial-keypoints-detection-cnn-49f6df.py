...
def residual_block(x, filters, hp=None):
    """Custom residual block with optional hyperparameter tuning"""
    shortcut = x
    conv_filters = filters if hp is None else hp.Int(f"res_conv_{filters}_units", filters//2, filters*2, step=filters//2)
    
    x = Conv2D(conv_filters, (3,3), padding='same')(x)
    ...
    if shortcut.shape[-1] != x.shape[-1]:
        shortcut = Conv2D(conv_filters, (1,1), padding='same')(shortcut)
...
def build_cnn_1_model(hp):
    inputs = Input(shape=(*Config.IMG_SIZE, 1))
    
    # Initial conv (no residual here to avoid overcomplicating first layer)
    x = Conv2D(256, (3,3), activation='relu', padding='same')(inputs)
    x = BatchNormalization()(x)
    x = MaxPooling2D(2,2)(x)
    
    # Residual block with tunable filters
    x = residual_block(x, 128, hp)  # Base 128, but hp can tune between 64-256
    
    # Second pooling
    x = MaxPooling2D(2,2)(x)
    
    # Optional second residual block (uncomment if needed)
    # x = residual_block(x, 64, hp)
    
    # Head
    x = GlobalAveragePooling2D()(x)
    x = Dense(hp.Int("dense_units", 128, 256, step=128), activation='relu')(x)
    x = Dropout(0.3)(x)
