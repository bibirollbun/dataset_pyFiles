...
# 1. Load lại model đã lưu (đảm bảo đúng tên file model h5 bạn lưu)
model = load_model("melanoma_model.h5")

# 2. Đọc và tiền xử lý ảnh đầu vào
# Giả sử file ảnh là 'test_image.jpg'
img_path = "test_image.jpg"
img = cv2.imread(img_path)
img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)           # đổi sang RGB nếu cần
img = cv2.resize(img, (256, 256))                    # resize đúng shape khi training
img = img.astype(np.float32) / 255.0                 # chuẩn hóa về [0, 1]
img = np.expand_dims(img, axis=0)                    # thêm batch dimension, shape (1, 256, 256, 3)

# 3. Chạy dự đoán (predict)
prob = model.predict(img)
label = "MALIGNANT" if prob[0][0] > 0.5 else "BENIGN"

print(f"Xác suất ác tính: {prob[0][0]:.4f} ⇒ Dự đoán: {label}")
