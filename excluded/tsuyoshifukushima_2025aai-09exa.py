...
# 新特徴「月」を作成
new_col = 'month' 
train_x[new_col] = train[col].dt.month
test_x[new_col]  = test[col].dt.month
...
# 年間の第何週か
new_col = 'weekofyear' 
train_x[new_col] = train[col].dt.isocalendar().week
test_x[new_col]  = test[col].dt.isocalendar().week

# 四半期（1〜4）
new_col = 'quarter' 
train_x[new_col] = train[col].dt.quarter
test_x[new_col]  = test[col].dt.quarter

# 週末かどうか
new_col = 'is_weekend' 
train_x[new_col] = (train[col].dt.dayofweek >= 5).astype(int)
test_x[new_col]  = (test[col].dt.dayofweek >= 5).astype(int)
