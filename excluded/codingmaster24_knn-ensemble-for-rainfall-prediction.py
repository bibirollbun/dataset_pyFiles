...
for fold, (train_idx, val_idx) in enumerate(kf.split(X_scaled)):
    ...
    knn_predictions += knn.predict(X_test_scaled) / 8  # Averaging predictions
...
plt.plot(range(1, 9), mae_scores, marker='o', linestyle='--', color='b', label='MAE per fold')
plt.xlabel('Fold')
plt.ylabel('Mean Absolute Error')
plt.title('KNN 8-Fold Cross-Validation Performance')
plt.legend()
plt.show()
