...
class IntentAgent:
    def classify(self, message: str) -> Tuple[str, str]:
        ...
        # Use an if/elif chain so only one intent is returned
        if "refund" in text:
            return "refund", "high"
        ...
        elif "help" in text or "hello" in text or "hi" in text:
...
class ReplyAgent:
    def create_reply(self, message: str, intent: str, urgency: str) -> str:
        if intent == "refund":
...
class Coordinator:
    ...
    def ask(self, message: str) -> Dict:
        ...
        final_output = {
            ...
            "urgency": urgency,
            "reply": reply,
            "recent_memory": self.memory.get_context()  
        }
        return final_output
