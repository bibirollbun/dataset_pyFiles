...
You are a highly accurate DOI/type classifier. Given a snippet of academic text containing a DOI or accession, choose:
  ...
  A) Data â€” the identifier points directly to research data in a repository  
  B) Literature â€” the identifier points to a journal article, book chapter, protocol paper, or other non-data resource  
...
Treat as DATA if the DOI starts with any of:
  ...
  â€¢ 10.5281 (Zenodo)  
  â€¢ 10.6084 (Figshare)  
  â€¢ 10.24433/ (Mendeley Data)  
  â€¢ 10.17632 (Mendeley Data)  
  â€¢ SRA/E- (e.g. SRP, SRA)  
  â€¢ PRJNA, PRJEB, PRJDB (NCBI BioProject)  
  â€¢ PRIDE:PXD (Proteomics)  
  â€¢ EMBL:E-MTAB, E- (ArrayExpress)  

Everything else is LITERATURE unless you see explicit data-repository context (e.g. â€œdeposited in Dryad under DOIâ€¦â€�).

=== Few-Shot Examples ===
1) â€œRaw images are stored on Figshare (DOI 10.6084/m9.figshare.1234567).â€� â†’ A  
2) â€œSequence reads available under BioProject accession PRJNA765432.â€� â†’ A  
3) â€œAs described in Nature Methods (DOI 10.1038/s41592-020-0793-2).â€� â†’ B  
4) â€œSee Supplementary Data at Zenodo (10.5281/zenodo.987654).â€� â†’ A  
5) â€œMethod details published in J. Proteome Res. DOI: 10.1021/acs.jproteome.0c00845.â€� â†’ B  
6) â€œData has been uploaded to Dryad (10.5061/dryad.x1y2z3).â€� â†’ A  
7) â€œReferenced paper: DOI 10.1101/2020.01.01.123456 (bioRxiv preprint).â€� â†’ B  
8) â€œMetabolomics data in MetaboLights MTBLS1234.â€� â†’ A  

=== Instructions ===
- Use only the identifier itself and its context.  
- If the DOI prefix is in the list above, always choose A.  
- If it belongs to a known publisher prefix (e.g. 10.1007, 10.1038, 10.1126, 10.1016â€¦), choose B.  
- Otherwise, rely on context words (â€œdepositedâ€�, â€œuploadedâ€�, â€œarchivedâ€�) to decide.  
...
if __name__=='__main__':
    ...
    outputs = llm.generate(prompts, vllm.SamplingParams(seed=1234, temperature=0.1, skip_special_tokens=True, max_tokens=1, logits_processors=[mclp], logprobs=len(mclp.choices)), use_tqdm=True)
