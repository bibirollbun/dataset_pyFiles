...
# # !pip install cairosvg bitsandbytes google_re2
...
class Model:
    def __init__(self):
        self.model_path = kagglehub.model_download('haoweili2322/qwen2.5-coder-14b-lora-4bit/Transformers/default/1')
        self.model_path = os.path.join(self.model_path, "Qwen2.5-coder-14B-lora-4bit")
        # self.lora_path = kagglehub.dataset_download('haoweili2322/qwen2-5-14b-lora-1123')
        # self.lora_path = os.path.join(self.lora_path, "checkpoint-635")
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            device_map="auto"  # 自动选择设备（如GPU或CPU）
        )
    ...
    def get_response(self, description):
        ...
        The length of the generated SVG code is between 3000-10000
        ...
        outputs = self.model.generate(**inputs, max_new_tokens=10000, use_cache=True)
...
# import torch
# # 模型名称或路径
# # 如果是本地路径，请替换为你的模型路径；如果是Hugging Face Hub上的模型，请使用模型ID
# # model_name_or_path = "E:\\model\\Qwen2.5-coder-14B-lora-4bit"  # 示例模型ID，请根据实际情况替换
# model_path = kagglehub.model_download('haoweili2322/qwen2.5-coder-14b-lora-4bit/Transformers/default/1')
# model_name_or_path = os.path.join(model_path, "Qwen2.5-coder-14B-lora-4bit")
# # 加载分词器
...
# # 测试模型
# input_text = "a purple forest at dusk"
# prompt = alpaca_prompt.format(input_text)
# inputs = tokenizer(prompt, return_tensors="pt").to(model.device)


# # 流式输出函数
# def stream_output(model, inputs, tokenizer, max_length=10000):
#     print("模型输出：")
#     # 初始化生成器
...
#         print(output_text[len(tokenizer.decode(generated_tokens[:-1], skip_special_tokens=True)):], end="")
#         sys.stdout.flush()
...
# # 调用流式输出
# stream_output(model, inputs, tokenizer, max_length=10000)
