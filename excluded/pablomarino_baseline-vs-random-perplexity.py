...
%%time
perplexities = scorer.get_perplexity(data.text.tolist())


perplexities


print(f"The average perplexity is: {np.mean(perplexities)}")
...
def shuffle_sentences(sentences):
    """
    Shuffle words in each sentence while maintaining sentence-to-sentence correspondence.
    
    Args:
        sentences (list): List of input sentences
        
    Returns:
        list: List of sentences with shuffled words
    """
    shuffled_sentences = []
...
result = shuffle_sentences(input_sentences)
for original, shuffled in zip(input_sentences, result):
    print(f"Original: {original}")
    print(f"Shuffled: {shuffled}\n")


sentence_list = data.text.tolist()
random_sentence_list = shuffle_sentences(sentence_list)
random_sentence_list


%%time
perplexities_random = scorer.get_perplexity(random_sentence_list)
perplexities_random


print(f"The average perplexity is: {np.mean(perplexities_random)}")


avg_perplexities_list = []
for i in range(24):
    random_sentence_list = shuffle_sentences(sentence_list)
    perplexities_random = scorer.get_perplexity(random_sentence_list)
    avg_perplexities = np.mean(perplexities_random)
    avg_perplexities_list.append(avg_perplexities)
avg_perplexities_list



print(f"The mean is: {np.mean(avg_perplexities_list)} - std: {np.std(avg_perplexities_list)}")
