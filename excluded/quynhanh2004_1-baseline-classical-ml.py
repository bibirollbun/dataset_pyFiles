...
# %% [fe] Auto-detect columns & feature builders (optimized)
def detect_columns(df):
    """Tá»± Ä‘á»™ng nháº­n diá»‡n nhÃ³m cá»™t IMU vÃ  ToF"""
    ...
    print(f"âœ… IMU cols: {imu_cols}")
    print(f"âœ… ToF cols: {tof_cols}")
    if others:
        print(f"â„¹ï¸� Other cols: {others}")
    return imu_cols, tof_cols, others


def fft_energy(vec):
    """TÃ­nh nÄƒng lÆ°á»£ng phá»• FFT (bá»� DC component)"""
    v = np.asarray(vec)
    if v.ndim == 0 or len(v) < 2:
        return 0.0
    spec = np.abs(rfft(v))
...
def build_row_features(df, imu_cols, tof_cols):
    """
    Sinh Ä‘áº·c trÆ°ng cÆ¡ báº£n cho dá»¯ liá»‡u dáº¡ng hÃ ng (1 máº«u / dÃ²ng)
    Náº¿u dá»¯ liá»‡u time-series, thay df -> groupby(window_id)
    """
    ...
    for c in imu_cols:
        val = df[c]
        X[f'{c}_mean'] = val
        # Khi khÃ´ng cÃ³ dá»¯ liá»‡u theo thá»�i gian -> std/skew/fft = 0
        X[f'{c}_std'] = 0.0
        X[f'{c}_skew'] = 0.0
    ...
    # --- ToF features ---
    if len(tof_cols) > 1:
        tof_block = df[tof_cols]
        X['tof_spatial_mean'] = tof_block.mean(axis=1)
        X['tof_spatial_var'] = tof_block.var(axis=1)
        X['tof_spatial_min'] = tof_block.min(axis=1)
        X['tof_spatial_max'] = tof_block.max(axis=1)
        X['tof_spatial_range'] = X['tof_spatial_max'] - X['tof_spatial_min']
    elif len(tof_cols) == 1:
        X['tof_spatial_mean'] = df[tof_cols[0]]
        X['tof_spatial_var'] = 0.0
        X['tof_spatial_min'] = df[tof_cols[0]]
        X['tof_spatial_max'] = df[tof_cols[0]]
        X['tof_spatial_range'] = 0.0
    else:
        # KhÃ´ng cÃ³ ToF
        X['tof_spatial_mean'] = 0.0
        X['tof_spatial_var'] = 0.0
        X['tof_spatial_range'] = 0.0
...
print("âœ… Feature matrix shape:", X_fe.shape)
print("âœ… First few rows:")
display(X_fe.head())
...
constant_cols = [col for col in X_fe.columns if X_fe[col].nunique() == 1]
print("Constant features:", constant_cols)

X_fe = X_fe.drop(columns=constant_cols)
...
def get_models():
    ...
    if HAS_LGBM:
        models['LightGBM'] = LGBMClassifier(
            ...
            force_col_wise=True,       # bá»� cáº£nh bÃ¡o overhead
            min_gain_to_split=0.0,     # cho phÃ©p chia nhÃ¡nh dÃ¹ gain nhá»�
            min_data_in_leaf=1,        # giáº£m yÃªu cáº§u sá»‘ lÆ°á»£ng máº«u Ä‘á»ƒ chia
            max_depth=-1,              # khÃ´ng giá»›i háº¡n Ä‘á»™ sÃ¢u
            num_leaves=64,             # tÄƒng sá»‘ lÃ¡ cho mÃ´ hÃ¬nh há»�c linh hoáº¡t hÆ¡n
            verbose=-1                 # táº¯t cáº£nh bÃ¡o khÃ´ng cáº§n thiáº¿t
        )
...
print("Check feature stats before training:")
print(X_fe.describe().T[['mean','std','min','max']].head(10))

# Check cá»™t háº±ng
constant_cols = [c for c in X_fe.columns if X_fe[c].nunique() <= 1]
if constant_cols:
    print("âš ï¸� Constant columns removed:", constant_cols)
    X_fe = X_fe.drop(columns=constant_cols)
...
def compute_compound_metrics(y_true, y_pred):
    """
    TÃ­nh toÃ¡n Binary F1, Macro F1 vÃ  chá»‰ sá»‘ tá»•ng há»£p (combined).
    - Binary F1: Ä‘Ã¡nh giÃ¡ Ä‘Ãºng/sai giá»¯a target gesture (1) vÃ  non-target (0)
    - Macro F1: trung bÃ¬nh F1 trÃªn táº¥t cáº£ class (coi non-target lÃ  1 class)
    - Combined: trung bÃ¬nh cá»§a cáº£ 2 metric
    """

    # Náº¿u y cÃ³ nhiá»�u hÆ¡n 2 class â†’ coi class 0 lÃ  non-target, cÃ²n láº¡i lÃ  target
    ...
    # F1 nhá»‹ phÃ¢n (target vs non-target)
    binary_f1 = f1_score(y_true_binary, y_pred_binary, average="binary", zero_division=0)

    # F1 macro trÃªn toÃ n bá»™ gesture
    macro_f1 = f1_score(y_true, y_pred, average="macro", zero_division=0)

    # Combined score = trung bÃ¬nh cá»™ng (cÃ³ thá»ƒ thay Ä‘á»•i trá»�ng sá»‘)
    combined = (binary_f1 + macro_f1) / 2
...
print("Keys of score_lr_mi:", score_lr_mi.keys())
...
# --- Model evaluation ---
def safe_extract_metrics(res):
    """Chuáº©n hÃ³a output tá»« evaluate_models() hoáº·c logistic thÃ nh dict gá»“m avg_f1, accuracy, macro_f1."""
    if isinstance(res, dict):
        # trÆ°á»�ng há»£p compute_compound_metrics hoáº·c evaluate_models()
        return {
            'avg_f1': res.get('combined', np.nan),     # dÃ¹ng 'combined' lÃ m chá»‰ sá»‘ tá»•ng há»£p
            'accuracy': np.nan,                        # khÃ´ng cÃ³ accuracy trong compute_compound_metrics()
            'macro_f1': res.get('macro_f1', np.nan)
        }
    elif isinstance(res, pd.DataFrame):
        return {
            'avg_f1': res['combined'].mean() if 'combined' in res else np.nan,
            'accuracy': np.nan,
            'macro_f1': res['macro_f1'].mean() if 'macro_f1' in res else np.nan,
        }
    else:
        return {'avg_f1': np.nan, 'accuracy': np.nan, 'macro_f1': np.nan}

# Evaluate tree-based
res_mi_tree_raw = evaluate_models(Xmi, y)
res_pca_tree_raw = evaluate_models(Xp_df, y)

res_mi_tree = safe_extract_metrics(res_mi_tree_raw)
res_pca_tree = safe_extract_metrics(res_pca_tree_raw)

# Logistic Regression (tuyáº¿n tÃ­nh)
X_train, X_val, y_train, y_val = train_test_split(Xmm, y, test_size=0.2, stratify=y, random_state=RANDOM_STATE)
...
lr_mi.fit(Xmi.loc[X_train.index], y_train)
y_pred_lr_mi = lr_mi.predict(Xmi.loc[X_val.index])
score_lr_mi = safe_extract_metrics(compute_compound_metrics(y_val, y_pred_lr_mi))
...
y_pred_lr_pca = lr_pca.predict(X_val)
score_lr_pca = safe_extract_metrics(compute_compound_metrics(y_val, y_pred_lr_pca))

# --- Summary table ---
summary_df = pd.DataFrame([
    ['LightGBM (Tree)', 'MI', res_mi_tree['avg_f1'], res_mi_tree['accuracy'], res_mi_tree['macro_f1']],
    ['LightGBM (Tree)', 'PCA', res_pca_tree['avg_f1'], res_pca_tree['accuracy'], res_pca_tree['macro_f1']],
    ['LogisticRegression (Linear)', 'MI', score_lr_mi['avg_f1'], score_lr_mi['accuracy'], score_lr_mi['macro_f1']],
    ['LogisticRegression (Linear)', 'PCA', score_lr_pca['avg_f1'], score_lr_pca['accuracy'], score_lr_pca['macro_f1']],
], columns=['Model Type', 'Feature Method', 'avg_f1 (combined)', 'accuracy', 'macro_f1'])

display(summary_df.style.background_gradient(cmap='Blues', subset=['avg_f1 (combined)', 'macro_f1']))
...
def evaluate_with_timing(model, X_train, X_val, y_train, y_val):
    """Huáº¥n luyá»‡n + Ä‘Ã¡nh giÃ¡ model vÃ  Ä‘o thá»�i gian."""
    ...
    # TÃ­nh cÃ¡c chá»‰ sá»‘
    metrics = compute_compound_metrics(y_val, y_pred)
...
res_lr_mi = evaluate_with_timing(lr_mi, Xmi.loc[X_train.index], X_val=Xmi.loc[X_val.index], y_train=y_train, y_val=y_val)
...
res_lr_pca = evaluate_with_timing(lr_pca, X_train, X_val, y_train, y_val)



# Ghi láº¡i káº¿t quáº£ dáº¡ng dict Ä‘á»ƒ sau nÃ y append vÃ o báº£ng
score_lr_mi = {
    "binary_f1": res_lr_mi["binary_f1"],
    "macro_f1": res_lr_mi["macro_f1"],
    "combined": res_lr_mi["combined"],
    "val_acc": res_lr_mi["val_acc"],
    "train_time": res_lr_mi["train_time"],
    "inference_time": res_lr_mi["inference_time"],
}

score_lr_pca = {
    "binary_f1": res_lr_pca["binary_f1"],
    "macro_f1": res_lr_pca["macro_f1"],
    "combined": res_lr_pca["combined"],
    "val_acc": res_lr_pca["val_acc"],
    "train_time": res_lr_pca["train_time"],
    "inference_time": res_lr_pca["inference_time"],
}
...
summary_df = pd.DataFrame([
    [
        "Logistic Regression (MI)", runtime_id, notebook_name, "MI", window_size, "liblinear", 0.01,
        score_lr_mi["binary_f1"], score_lr_mi["macro_f1"], score_lr_mi["combined"],
        score_lr_mi["val_acc"], score_lr_mi["train_time"], score_lr_mi["inference_time"], notes
    ],
    [
        "Logistic Regression (PCA)", runtime_id, notebook_name, "PCA", window_size, "liblinear", 0.01,
        score_lr_pca["binary_f1"], score_lr_pca["macro_f1"], score_lr_pca["combined"],
        score_lr_pca["val_acc"], score_lr_pca["train_time"], score_lr_pca["inference_time"], notes
    ],
], columns=[
    "model_name","runtime_id","notebook","features_used","window_size","optimizer/solver","params (M)",
    "Binary","Macro","Final Score","val_acc","train_time","inference_time","notes"
])

display(summary_df.style.background_gradient(cmap='Blues', subset=['Binary','Macro','Final Score']))
