...
train_clean.group_by("Duration").len()


sex = 1

train_clean_sex = train_clean.filter(pl.col("Sex") ==  sex) 
test_clean_sex = test_clean.filter(pl.col("Sex") ==  sex)



cat_male_optuna = {'depth': 10, 
                   'iterations' : 587,
                   'l2_leaf_reg': 3.3037775140170704, 
                   'learning_rate': 0.05463508754462207,  
                   'ag_args': {'name_suffix': '_optuna_male', 'priority': -2}}
cat_female_optuna =  {'iterations': 590, 
                      'learning_rate': 0.062494080576048446, 
                      'depth': 9, 'l2_leaf_reg': 2.28306978679542, 
                     'ag_args': {'name_suffix': '_optuna_female', 'priority': -2}}
...
if sex == 0 :
    ...
    custom_hyperparameters['CAT'].append (cat_female_optuna)
...
predictor_sex= TabularPredictor(path = f'/kaggle/working/Autogluon/sex_{sex}',
                                       label='Calories_log', 
...
predictor_sex.fit(train_data= train_clean_sex.to_pandas(), 
...
display (predictor_sex.leaderboard())


fit_parameters_sex = predictor_sex.fit_summary(verbosity = 3, show_plot = True)
display (fit_parameters_sex)


predictions_sex = predictor_sex.predict (test_clean_sex.to_pandas()) 
print (predictions_sex)
...
submission_sex = convert (predictions_sex, test_clean_sex)



submission_sex.describe()


submission_sex.write_csv(f"submission_{sex}.csv")
print (f"write submission_{sex}.csv is complete")
...
def move_2_zip (directory_path, archive_name):
    ...
    """
    Generated by Gen AI (Gemini 2.0 flash)
