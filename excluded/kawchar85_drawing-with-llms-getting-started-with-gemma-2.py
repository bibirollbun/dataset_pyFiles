...
"""
        ...
        self.timeout_seconds = 4 * 60

    # You could try increasing `max_new_tokens`
    def predict(self, description: str, max_new_tokens=3072) -> str:
        def generate_svg():
            try:
        ...
        for element in root.iter():
            ...
            if tag_name == 'path':
                ...
                path_regex = re.compile(
                    r'^'  # Start of string
                    r'([MmZzLlHhVvCcSsQqTtAaEeFf]' # Path commands
                    r'(\s*(-?\d+(\.\d+)?([Ee][+-]?\d+)?)\s*([,\s]\s*(-?\d+(\.\d+)?([Ee][+-]?\d+)?)\s*)*)*' # Numbers with optional commas and spaces
                    r')+' # One or more path commands
