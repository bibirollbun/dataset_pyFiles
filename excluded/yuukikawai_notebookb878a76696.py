...
his_trans = pd.read_csv(f"{DATA_DIR}/historical_transactions.csv")
print(his_trans.describe())
...
his_trans['category_1'] = his_trans['category_1'].map({'Y': 1, 'N': 0})

new_trans['category_2'] = new_trans['category_2'].fillna(-1)
his_trans['category_2'] = his_trans['category_2'].fillna(-1)

# # 集約特徴の作成パターンを辞書化
agg_funcs = {
    "purchase_amount": ["min", "max", "mean"], # 正規化済みの購入金額（負値 = 顧客の支出）
    "installments":    ["min", "max", "mean"], # 分割払いの分割回数
    "month_lag":       ["min", "max", "mean", "var", "sem"], # 基準日からの経過月数
...
agg_his = his_trans.groupby("card_id").agg(agg_funcs)

# # 備考）上の集約特徴のテーブルは，列が MultiIndex の状態になっている
# # 　　　⇒　列名のリストを表示して確認してみよう
# print(agg_new.columns.to_list())
# print(agg_his.columns.to_list())
...
for col_name, func_name in agg_new.columns.to_flat_index():
    ...
    flat_cols_new.append(new_name)

flat_cols_his = []
for col_name, func_name in agg_his.columns.to_flat_index():
    his_name = f"his_{col_name}_{func_name}"
    flat_cols_his.append(his_name)
# # MultiIndex状態の列を通常の状態に変換（列方向に平坦化）
agg_new.columns = flat_cols_new
agg_his.columns = flat_cols_his
...
agg_his = agg_his.reset_index()

# # メインテーブルの特徴と結合
train_x = train_x.merge(agg_new, on="card_id", how="left")
test_x  = test_x.merge(agg_new, on="card_id", how="left")
train_x = train_x.merge(agg_his, on="card_id", how="left")
test_x  = test_x.merge(agg_his, on="card_id", how="left")
