...
def load_background_images(folder_path, num_images=2, img_size=224):
    """
    Loads a set number of background images from PNG files.
    """
    all_images = [f for f in os.listdir(folder_path) if f.endswith(".png")]
    num_images = min(num_images, len(all_images))  # Ensure we don't select more than available
    selected_images = random.sample(all_images, num_images)

    background_data = np.array([
        cv2.resize(cv2.cvtColor(cv2.imread(os.path.join(folder_path, img)), cv2.COLOR_BGR2RGB), (img_size, img_size)) / 255.0
...
def predict_with_explanations(image_path, multibranch_model, cnn_model, img_size=224, weight_multibranch=0.7, weight_cnn=0.3):
    """
    Predicts class probabilities using a multi-branch CNN and a 2D CNN.
    Also generates SHAP explanations.
    """
    ...
    # Combine predictions using a weighted average (ensemble)
    ensemble_probs = (weight_multibranch * multibranch_probs) + (weight_cnn * cnn_probs)
    ensemble_class = np.argmax(ensemble_probs)
    ensemble_confidence = float(ensemble_probs[ensemble_class])

    # Load background images for SHAP
    folder_path = "/kaggle/working/preprocessed_images"
    background_data = load_background_images(folder_path, img_size=img_size)
    test_image = background_data[0]  # First image for masking

    # Create blurred background for SHAP
    blurred_img = gaussian_filter(test_image, sigma=5)
    masker = shap.maskers.Image(blurred_img)

    # Initialize SHAP Explainer
    explainer = shap.Explainer(multibranch_model, masker, output_names=[0, 1, 2, 3, 4])
    shap_values = explainer(np.expand_dims(preprocessed_image, axis=0), batch_size=1)
    ...
    return results, background_data


def visualize_shap(results, background_data):
    """
    Visualizes the SHAP explanation results.
    """
    shap_values = results["multi_branch_cnn"]["shap_values"]

    plt.figure(figsize=(12, 8))
    shap.image_plot(shap_values, pixel_values=np.expand_dims(preprocessed_image, axis=0))

image_path = "/kaggle/input/aptos2019-blindness-detection/test_images/006efc72b638.png"
preprocessed_image = preprocess_image(image_path)
results, background_data = predict_with_explanations(image_path, multibranch_model, cnn_model)
...
visualize_shap(results, background_data)
