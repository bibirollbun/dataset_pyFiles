...
x_train,x_val, y_train, y_val = train_test_split(dtrain['Id'].values,y,test_size=0.2, random_state=42)
...
resnet.conv1 = nn.Conv2d(4,64,kernel_size=resnet.conv1.kernel_size,
...
val_loss_list = []
num_epoch = 5
optimizer = torch.optim.Adam(resnet.parameters(), lr=4e-5, weight_decay=1e-6)
resnet = model
criterion = FocalLoss(alpha=0.25, gamma=2.0, reduction='mean')
