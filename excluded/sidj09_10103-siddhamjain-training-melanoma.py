...
df = pd.read_csv("/kaggle/input/10103-siddhamjain-kfold-melanoma/train_5folds.csv")
df.head()


# Ensure PyTorch/Tez are imported
import torch
from tez import Tez, TezConfig
from tez.callbacks import EarlyStopping
...
if device.type == "cuda":
    ...
    torch.backends.cudnn.benchmark = True  # optional performance tweak

# ... your dataset prep as-is ...
...
# 2) Build model and move to GPU if available BEFORE wrapping with Tez
base_model = CustomModel()
base_model.to(device)  # important

# 3) Wrap with Tez
model = Tez(base_model)

# 4) Tez config: fp16 on GPU, off on CPU
