...
def sklearn_normalize(embeddings):
...
query_embeddings = sklearn_normalize(query_embeddings)
...
# Chuyển đổi các câu MisconceptionName thành vector nhúng
mis_embeddings = encode_sentences(
    df_mis['MisconceptionName'].tolist(),
    model=embedding_model,
    tokenizer=tokenizer,
    batch_size=32
...
# Chuẩn hóa vector nhúng
mis_embeddings = sklearn_normalize(mis_embeddings)
...
model.compile(
    ...
    #metrics=[tf.keras.metrics.CosineSimilarity(axis=1, name="cosine_similarity")],
...
mapk(y_train_pred, y_train_misId)


mapk(y_valid_pred, y_valid_misId)
...
x_test = sklearn_normalize(x_test)
x_test.shape
