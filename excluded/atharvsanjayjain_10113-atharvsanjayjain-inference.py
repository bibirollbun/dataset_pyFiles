...
class PawpularModel(nn.Module):
    def __init__(self):
        ...
        self.out = nn.Linear(1000, 1)  # Match training output dimension (backbone output size)
...
model = PawpularModel()
model = Tez(model)
model.load(f"/kaggle/input/10113-atharvsanjayjain-training/model_f{i}.bin", weights_only=True)

df_test = pd.read_csv("../input/petfinder-pawpularity-score/test.csv")
test_img_paths = [f"/kaggle/input/petfinder-pawpularity-score/test/{x}.jpg" for x in df_test["Id"].values]
...
config = TezConfig(
    ...
    fp16=False,                  # Disabled fp16 to ensure stability
    val_strategy="batch",
    val_steps=900,
)
