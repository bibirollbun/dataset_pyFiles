...
# ============================ CELL 4: TRANSFORMS (FIXED) ============================
import albumentations as A
from albumentations.pytorch import ToTensorV2

# use tuple for size because some albumentations versions expect `size` arg
...
scaler = torch.cuda.amp.GradScaler()   # works fine on PyTorch â‰¥ 1.11
...
plt.subplot(1,2,2)
plt.plot(history['train_acc'], label='train_acc'); plt.plot(history['val_acc'], label='val_acc'); plt.plot(history['val_qwk'], label='val_qwk')
...
ckpt_path = "densenet121_best.pth"  # file will be in /kaggle/working after training
