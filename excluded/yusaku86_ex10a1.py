...
# ① purchase_amount をクリッピング（異常値対策）
new_trans["purchase_amount"] = new_trans["purchase_amount"].clip(lower=-0.1, upper=0.2)

# カテゴリ変数を数値に変換（全て明示的に）
new_trans["authorized_flag"] = new_trans["authorized_flag"].map({'Y': 1, 'N': 0}).astype(int)
new_trans["category_1"] = new_trans["category_1"].map({'Y': 1, 'N': 0}).astype(int)
new_trans["category_2"] = LabelEncoder().fit_transform(new_trans["category_2"].astype(str))
new_trans["category_3"] = LabelEncoder().fit_transform(new_trans["category_3"].astype(str))


# # 集約特徴の作成パターンを辞書化
#agg_funcs = {
#    "purchase_amount": ["min", "max"], # 正規化済みの購入金額（負値 = 顧客の支出）
#    "installments":    ["min", "max"], # 分割払いの分割回数
#    "month_lag":       ["min", "max"], # 基準日からの経過月数
#    "purchase_date":   ["count"],      # 購入日時（countで空でない行を数える ⇔ 購入回数）
#}

agg_funcs = {
    "purchase_amount": ["min", "max", "mean", "std", "var", "skew", "sem", "prod", "first", "last"],
    "installments":    ["min", "max", "mean", "std", "nunique", "first", "last"],
    "month_lag":       ["min", "max", "mean", "median", "std",  "first", "last"],
    "purchase_date":   ["count"],  # 時系列情報の前処理が必要なら datetime に変換して後続で扱う
...
for col_name, func in agg_new.columns.to_flat_index():
    if isinstance(func, str):
        new_name = f"new_{col_name}_{func}"
    else:
        # 関数名が lambda などの場合は手動で命名（例：range, q25, q75 など）
        func_name = getattr(func, '__name__', 'custom')
        if 'lambda' in func_name:
            if col_name == 'purchase_amount':
                func_name = 'range'
            elif col_name == 'month_lag':
                func_name = 'q25'
            else:
                func_name = 'custom'
