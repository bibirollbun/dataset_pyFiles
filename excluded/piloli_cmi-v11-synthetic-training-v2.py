...
# データパス設定（競技データセット参照）
# Code Competitionでは競技データが自動的にマウントされる
DATA_DIR = Path("/kaggle/input/cmi-detect-behavior-with-sensor-data")
TRAIN_DIR = DATA_DIR / "train"  # parquetファイルのディレクトリ

# train.csv（メタデータ）の読み込み
print("Loading CMI competition data...")
train_metadata = pd.read_csv(DATA_DIR / "train.csv")
print(f"  Loaded train.csv: {len(train_metadata)} sequences")

# データ構造の確認
print(f"\nData columns: {list(train_metadata.columns)}")
print(f"\nFirst few rows:")
print(train_metadata.head())

# train demographicsの読み込み（必要に応じて）
if (DATA_DIR / "train_demographics.csv").exists():
    train_demographics = pd.read_csv(DATA_DIR / "train_demographics.csv")
    print(f"\n  Loaded train_demographics.csv: {len(train_demographics)} subjects")
    # メタデータとマージ（必要に応じて）
    if "subject" in train_metadata.columns and "subject" in train_demographics.columns:
        train_metadata = train_metadata.merge(
            train_demographics, on="subject", how="left"
        )

print(f"\nData loaded successfully:")
print(f"  Train directory: {TRAIN_DIR}")
print(f"  Real training data: {len(train_metadata)} sequences")
...
def analyze_real_data_distribution():
    ...
    for seq_id in sample_sequences:
        try:
            # parquetファイルは train/ ディレクトリにある
            df = pd.read_parquet(TRAIN_DIR / f"{seq_id}.parquet")

            # 加速度計
            acc_cols = [c for c in df.columns if c.startswith("acc_")]
            if acc_cols:
                all_acc.append(df[acc_cols].values)

            # ジャイロスコープ
            gyr_cols = [c for c in df.columns if c.startswith("gyr_")]
            if gyr_cols:
                all_gyr.append(df[gyr_cols].values)

            # クォータニオン
            quat_cols = [c for c in df.columns if "quat" in c]
            if quat_cols:
                all_quat.append(df[quat_cols].values)
        except Exception as e:
            print(f"  Warning: Could not load {seq_id}: {e}")
            continue
...
def generate_synthetic_sequence(gesture, seq_length, sensor_stats):
    ...
    for tof_id in range(8):
        base_distance = np.random.uniform(50, 2500)  # より広い範囲

        # ジェスチャーによって距離変化パターンが異なる
...
def extract_features(df, seq_id, metadata_row):
    ...
    for comp in ["w", "x", "y", "z"]:
        ...
        if col in df.columns:
            ...
            features[f"quat_{comp}_change"] = values[-1] - values[0]
    ...
    # ラベル
    features["gesture"] = metadata_row["gesture"]
...
for idx, row in real_sample.iterrows():
    try:
        # parquetファイルは train/ ディレクトリにある
        df = pd.read_parquet(TRAIN_DIR / f"{row['sequence_id']}.parquet")
        features = extract_features(df, row["sequence_id"], row)
        real_features.append(features)
    except Exception as e:
        failed_count += 1
        if failed_count <= 5:  # 最初の5個のエラーのみ表示
            print(f"  Failed to load {row['sequence_id']}: {e}")
        continue
