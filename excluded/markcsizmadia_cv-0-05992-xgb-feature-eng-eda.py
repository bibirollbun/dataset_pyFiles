...
def get_model(random_state: Optional[int] = None):
    clf = XGBRegressor(
        ...
        # eval_metric="rmsle", # without log1p transform would use this
        # objective="reg:squaredlogerror", # without log1p transform would use this
...
verbose = 100
model.fit(X=X_train_val_p, y=y_train_val_log, eval_set=[(X_train_val_p, y_train_val_log), (X_test_p, y_test_log)], verbose=verbose)


# dirty fix for https://stackoverflow.com/questions/77454895/xgboost-warning-running-on-cuda-while-input-data-is-on-cpu
# could save model, then reload and put booster on cpu device, but not necessary in this notebook
with warnings.catch_warnings():
    warnings.filterwarnings("ignore", category=UserWarning)
    y_pred_log = model.predict(X=X_test_p)
...
feature_names = list(X_train_val_p.columns)
...
importance_gain = pd.Series(importance_gain, index=feature_names).sort_values(ascending=False)
importance_weight = pd.Series(importance_weight, index=feature_names).sort_values(ascending=False)
importance_cover = pd.Series(importance_cover, index=feature_names).sort_values(ascending=False)
importance_total_gain = pd.Series(importance_total_gain, index=feature_names).sort_values(ascending=False)
importance_total_cover = pd.Series(importance_total_cover, index=feature_names).sort_values(ascending=False)

# Plot importance_gain
plt.figure(figsize=(16, 12))
plt.bar(importance_gain.index, importance_gain.values)
...
# Plot importance_weight
plt.figure(figsize=(16, 12))
plt.bar(importance_weight.index, importance_weight.values)
...
# Plot importance_cover
plt.figure(figsize=(16, 12))
plt.bar(importance_cover.index, importance_cover.values)
...
# Plot importance_total_gain
plt.figure(figsize=(16, 12))
plt.bar(importance_total_gain.index, importance_total_gain.values)
plt.title("Total Gain Importance")
plt.xlabel("Feature")
plt.ylabel("Importance")
plt.xticks(rotation=90)
plt.tight_layout()
...
# Plot importance_total_cover
plt.figure(figsize=(16, 12))
plt.bar(importance_total_cover.index, importance_total_cover.values)
plt.title("Total Cover Importance")
plt.xlabel("Feature")
plt.ylabel("Importance")
plt.xticks(rotation=90)
plt.tight_layout()
...
def k_fold(data_train: pd.DataFrame,
    ...
    for i, (train_index, valid_index) in iter:
        ...
        clf.fit(X=X_train_p, y=y_train_log, eval_set=[(X_train_p, y_train_log),(X_valid_p, y_valid_log)], verbose=verbose)
        y_valid_hat_log = clf.predict(X=X_valid_p)
        y_test_hat_log = clf.predict(X=X_test_p)
