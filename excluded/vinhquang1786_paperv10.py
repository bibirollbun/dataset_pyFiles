...
def p_value(A_obs, list_active_set, list_zk, etaj, z_obs, cov_matrix,
            extreme_threshold=8.0, log_underflow_thresh=-745.0):
    """
    Robust p-value calculation:
    - compute intervals where active_set == A_obs
    - work in log-space with logsumexp
    - for extreme tails (a_s > extreme_threshold) use approximation:
        log P(a<b) ≈ log phi(a_s) + log(b_s - a_s)
    - fallback to mpmath only if needed
    """
    ...
    # collect intervals (float)
    z_intervals = []
    for i, active_set in enumerate(list_active_set):
        if set(A_obs) == set(active_set):
            z_intervals.append((float(list_zk[i]), float(list_zk[i+1])))
    print("z_intervals:", z_intervals)
    if not z_intervals:
        return None

    # try scipy logcdf first
    try:
        from scipy.stats import norm
    ...
    def log_phi(x):
        # log density of standard normal at x
        return -0.5 * (x**2) - 0.5 * np.log(2.0 * np.pi)

    if use_scipy:
        def log_cdf_diff_scipy(a, b):
            # robust log(Φ(b) - Φ(a)) using logcdf
            # assume a <= b
            if np.isneginf(a) and np.isposinf(b):
                return 0.0
            if np.isneginf(a):
                return norm.logcdf(b)
            if np.isposinf(b):
                la = norm.logcdf(a)
                return np.log1p(-np.exp(la)) if la < 0 else np.log(1.0 - np.exp(la))
            La = norm.logcdf(a)
            Lb = norm.logcdf(b)
            if Lb < La:
                La, Lb = Lb, La
        ...
        for (al, ar) in z_intervals:
            ...
            # if in extreme tail, use density*width approx (in log-space) to avoid underflow
            if a_s > extreme_threshold:
                # log prob ≈ log phi(a_s) + log(width_s)
                lp = log_phi(a_s) + np.log(width_s)
            else:
                # normal route with logcdf diff
                lp = log_cdf_diff_scipy(a_s, b_s)
                # if this gave -inf or nan, fallback to density approx if width small
                if not np.isfinite(lp):
                    lp = log_phi(a_s) + np.log(max(width_s, 1e-300))

            log_probs.append(lp)

            # numerator piece
            if z_obs >= ar:
                log_partial.append(lp)
            elif z_obs > al:
                z_s = float(z_obs) / tn_sigma
                if a_s > extreme_threshold:
                    # partial width: (z_s - a_s)
                    part_width = z_s - a_s
                    lp_part = log_phi(a_s) + np.log(max(part_width, 1e-300))
                else:
                    lp_part = log_cdf_diff_scipy(a_s, z_s)
                    if not np.isfinite(lp_part):
                        lp_part = log_phi(a_s) + np.log(max(z_s - a_s, 1e-300))
                log_partial.append(lp_part)
            else:
                # no contrib
                pass

        # aggregate
        finite_logs = [v for v in log_probs if np.isfinite(v)]
        if len(finite_logs) == 0:
            use_scipy = False
        else:
            log_den = logsumexp(log_probs)
            log_num = logsumexp([v for v in log_partial if np.isfinite(v)]) if any(np.isfinite(x) for x in log_partial) else -np.inf
            print("log_den (scipy-approx):", log_den, "log_num:", log_num)
            # if denominator extremely small -> fallback
            # Nếu cả hai log đều hữu hạn thì ta có thể tính pivot ngay bằng exp(log_num - log_den)
            if np.isfinite(log_den) and np.isfinite(log_num):
                pivot_val = float(np.exp(log_num - log_den))
                pivot_val = max(0.0, min(1.0, pivot_val))
                pval = 2.0 * min(pivot_val, 1.0 - pivot_val)
                return pval
            # Nếu một trong hai không hữu hạn, mới fallback sang mpmath
            use_scipy = False

    # fallback to high-precision mpmath if needed
    try:
        import mpmath as mp
        if mp.mp.dps < 80:
            mp.mp.dps = 80
    ...
    if mp is None:
        # no high-precision available; return extremely small p-value (degenerate)
        # better to return 0.0 than crash; caller can inspect logs for log_den
        return 0.0

    # mpmath computations
    def mp_norm_cdf(x):
        xm = mp.mpf(x)
        return mp.mpf('0.5') * (1 + mp.erf(xm / mp.sqrt(2)))

    denom_mp = mp.mpf('0')
    numer_mp = mp.mpf('0')
    for (al, ar) in z_intervals:
        a_s = mp.mpf(al) / mp.mpf(tn_sigma)
        b_s = mp.mpf(ar) / mp.mpf(tn_sigma)
        prob = mp_norm_cdf(b_s) - mp_norm_cdf(a_s)
        denom_mp += prob
        if mp.mpf(z_obs) >= mp.mpf(ar):
            numer_mp += prob
        elif mp.mpf(z_obs) > mp.mpf(al):
            z_s = mp.mpf(z_obs) / mp.mpf(tn_sigma)
            numer_mp += mp_norm_cdf(z_s) - mp_norm_cdf(a_s)

    if denom_mp == 0:
        return 0.0

    pivot_val_mp = numer_mp / denom_mp
    try:
        pivot_val = float(pivot_val_mp)
    except Exception:
        pivot_val = 0.0 if pivot_val_mp == 0 else 1.0
    pivot_val = max(0.0, min(1.0, pivot_val))
    pval = 2.0 * min(pivot_val, 1.0 - pivot_val)
    print("Done (mpmath fallback).")
    return pval
...
if __name__ == '__main__':
    ...
    # Nếu X_clean là numpy array 2D
    X_clean = pd.DataFrame(X_clean, columns=[f"x{i}" for i in range(X_clean.shape[1])])
    
    # Nếu y_clean là numpy array 1D hoặc 2D
    y_clean = pd.DataFrame(y_clean, columns=["y"])
