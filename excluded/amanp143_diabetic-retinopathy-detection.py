...
def init_model(train=True, 
               ...
               model_name='enet_b4'):
    '''
    Initialize the EfficientNet-B4 model from torchvision
    '''
    
    # Load EfficientNet-B7 from torchvision (no pretrained weights by default)
    model = models.efficientnet_b4(weights='IMAGENET1K_V1')  # or 'DEFAULT' if using newer torchvision
    num_ftrs = model.classifier[1].in_features

    # Replace the classifier with your own for 5-class output
    model.classifier[1] = nn.Linear(num_ftrs, 5)

    # Load custom weights
    #model.load_state_dict(torch.load(f'../input/diabetic-retinopathy-pre-training/models/model_{model_name}.bin'))

    if train:
        # Freeze all layers except last `trn_layers` children
        children = list(model.children())
        for child in children[:-trn_layers]:
...
def init_pre_model(train=True):
    ...
    ### training mode
    if train == True:
        # Load pre-trained EfficientNet-B4 from torchvision with ImageNet weights
        model = models.efficientnet_b4(weights='IMAGENET1K_V1')
        model.classifier[1] = nn.Linear(model.classifier[1].in_features, 5)
        
    ### inference mode
    if train == False:
        # Load EfficientNet-B7 without pretrained weights
