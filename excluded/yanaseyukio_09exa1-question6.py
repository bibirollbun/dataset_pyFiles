...
#新カテゴリ作成の関数
def newCategory(list1, list2):
    new_train_x =  train_x[list1] * train_x[list2]
    new_test_x = test_x[list1] * test_x[list2]
    return new_train_x, new_test_x

#新特徴量（カテゴリ）を追加（store*item)
stit_col = 'StIt'
train_x[stit_col], test_x[stit_col] = newCategory('store', 'item')
...
#新特徴量「年」を追加
year_col = 'year'
train_x[year_col] = train[col].dt.year
test_x[year_col] = test[col].dt.year

#新特徴量「月」を追加
month_col = 'month'
train_x[month_col] = train[col].dt.month
test_x[month_col] = test[col].dt.month

#新特徴量「日」を追加
day_col = 'day'
train_x[day_col] = train[col].dt.day
test_x[day_col] = test[col].dt.day

#新特徴量（カテゴリ）を追加（store*year)
stye_col = 'StYe'
train_x[stye_col], test_x[stye_col] = newCategory('store', 'year')

#新特徴量（カテゴリ）を追加（item*year)
itye_col = 'ItYe'
train_x[itye_col], test_x[itye_col] = newCategory('item', 'year')

# 未加工の 'date' は使用しない（※テスト側が全て未知カテゴリになるためそのままの使用は困難）
train_x = train_x.drop([col], axis=1)
test_x = test_x.drop([col], axis=1)

# 最後の3週間分を表示(dow = 0 ⇔ 月曜日）
print(train_x.tail(21))
print(test_x.tail(21))
