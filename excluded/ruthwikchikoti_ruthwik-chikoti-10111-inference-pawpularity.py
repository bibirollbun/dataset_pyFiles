...
class PawpularModel(nn.Module):
    ...
    def forward(self, image, features, targets=None):
        x = self.model(image)
        # CHANGED: Removed dropout here
        x = torch.cat([x, features], dim=1)  # CHANGED: Uncommented this line
        x = self.dropout(x)  # CHANGED: Moved dropout here, after concatenation
        x = self.out(x)
        # x = self.dropout(x)
        # x = self.out_final(x)
...
model.load(f"/kaggle/input/ruthwik-chikoti-10111-pawpularity-training/model_f4.bin", weights_only=True)
