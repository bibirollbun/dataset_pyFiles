...
def color_scheme(dk,color):
    ...
    clr_tes3  = ['gold',"darkmagenta",'silver']
    ...
    clr_tes8  = clr_Green4 + clr_Blue4
    clr_tes9  = clr_Green4 + ['magenta'] + clr_Blue4
...
params = {
      ...
      'subm'     : [ 
         { 'name': f'0.05539.a','weight': 0.12 },
         { 'name': f'0.05539.b','weight': 0.12 },
         { 'name': f'0.05539.c','weight': 0.12 },
         { 'name': f'0.05539.d','weight': 0.12 },
         { 'name': f'0.05539.f','weight': 0.20 }, # ğŸš‚.Top (magenta)
         { 'name': f'0.05540.g','weight': 0.08 },
         { 'name': f'0.05540.h','weight': 0.08 },
         { 'name': f'0.05540.i','weight': 0.08 },
         { 'name': f'0.05540.j','weight': 0.08 },
      ]
}
df_cross1 = h_blend(params, color='tes9', figures1=True, details=True)

df_cross1.to_csv('h-blend of all.csv',index=False)

display_distances(params)


qnt_Majority_votes = 6  # version.10
qnt_Majority_votes = 5  # version.11


print(f'\nVoting:\n')

df_39f = pd.read_csv('/kaggle/input/22-october-2025-ps-s5e10/submission_0.05539.f.csv')
tida_desc = pd.read_csv('/kaggle/working/tida_desc.csv')
df_soluts = tida_desc[tida_desc.columns[2:11]]
display(df_soluts)

df_vote = pd.read_csv('/kaggle/input/playground-series-s5e10/sample_submission.csv')
np_soluts_sorted = np.sort(df_soluts.to_numpy(), axis=1)
df_vote['accident_risk'] = np.mean(np_soluts_sorted[:,0:qnt_Majority_votes], axis=1)
df_vote['accident_risk'] = df_39f['accident_risk'] *0.50 + 0.50* df_vote['accident_risk']

print(f'\nComparison: submission_0.055_39.f (x)  vs  df_vote (y)\n')

df_comparison = pd.merge(df_39f, df_vote, on='id')
df_comparison["delta_risk's"] = df_39f['accident_risk'] - df_vote['accident_risk']
display(df_comparison)
print(sum(df_comparison["delta_risk's"]))
print(df_comparison["delta_risk's"][df_comparison["delta_risk's"] < 0].sum())

df_vote.to_csv('Vote 1.csv',index=False)


path ='/kaggle/input/13-october-2025-ps-s5e10/submission_'
...
params = {
      ...
      'subwts'   : [ +0.04,+0.03,+0.02,+0.01, 0, -0.01,-0.02,-0.03,-0.04 ],       
      'subm'     : [ 
         { 'name': f'0.05541.a','weight': 0.11 },
         { 'name': f'0.05541.b','weight': 0.11 },
         { 'name': f'0.05541.c','weight': 0.11 },
         { 'name': f'0.05541.d','weight': 0.11 },
         { 'name': f'0.05541.f','weight': 0.12 }, # (magenta)
         { 'name': f'0.05541.g','weight': 0.11 },
         { 'name': f'0.05541.h','weight': 0.11 },
         { 'name': f'0.05541.i','weight': 0.11 },
         { 'name': f'0.05541.j','weight': 0.11 },
      ]
}
df_cross1 = h_blend(params, color='tes9', figures1=True, details=True)
...
df_38T = pd.read_csv('/kaggle/input/30-october-2025-ps-s5e10-main/submission_Best.csv')
tida_desc = pd.read_csv('/kaggle/working/tida_desc.csv')
df_soluts = tida_desc[tida_desc.columns[2:11]]
display(df_soluts)
...
np_soluts_sorted = np.sort(df_soluts.to_numpy(), axis=1)
df_vote['accident_risk'] = np.mean(np_soluts_sorted[:,0:qnt_Majority_votes], axis=1)
df_vote['accident_risk'] = df_38T['accident_risk'] *0.50 + 0.50* df_vote['accident_risk']

print(f'\nComparison: submission_0.055_38T (x)  vs  df_vote (y)\n')

df_comparison = pd.merge(df_38T, df_vote, on='id')
df_comparison["delta_risk's"] = df_38T['accident_risk'] - df_vote['accident_risk']
display(df_comparison)
print(sum(df_comparison["delta_risk's"]))
print(df_comparison["delta_risk's"][df_comparison["delta_risk's"] < 0].sum())

df_vote.to_csv('Vote 2.csv',index=False)

df_vote


path ='/kaggle/input/31-october-2025-ps-s5e10-38-1/submission '

params = {
      'path'     : path,            
      'id_target': ['id',"accident_risk"],          
      'type_sort': ['asc/desc',0.30,0.70],
      'subwts'   : [ +0.004,+0.003,+0.002,+0.001, 0, -0.001,-0.002,-0.003,-0.004 ],       
      'subm'     : [ 
         { 'name': f'(1)','weight': 0.11 },
         { 'name': f'(2)','weight': 0.11 },
         { 'name': f'(3)','weight': 0.11 },
         { 'name': f'(4)','weight': 0.11 },
         { 'name': f'(5)','weight': 0.12 },  # (magenta)
         { 'name': f'(6)','weight': 0.11 },
         { 'name': f'(7)','weight': 0.11 },
         { 'name': f'(8)','weight': 0.11 },
         { 'name': f'(9)','weight': 0.11 },
      ]
}
df_cross1 = h_blend(params, color='tes9', figures1=True, details=True)

df_cross1.to_csv('h-blend of all 3.csv',index=False)

display_distances(params)


qnt_Majority_votes = 7  # version.11
...
np_soluts_sorted = np.sort(df_soluts.to_numpy(), axis=1)
df_vote['accident_risk'] = np.mean(np_soluts_sorted[:,0:qnt_Majority_votes], axis=1)
df_vote['accident_risk'] = df_38T['accident_risk'] *0.70 + 0.30* df_vote['accident_risk']
...
df_vote.to_csv('Vote 5.csv',index=False)
df_vote
...
params = {
      ...
      'id_target': ['id',"accident_risk"],          
      'type_sort': ['asc/desc',0.30,0.70],
      'subwts'   : [ +0.015, -0.005, -0.010],       
      'subm'     : [
         { 'name': f'Vote 1', 'weight': 0.10 },
         { 'name': f'Vote 2', 'weight': -0.05 },
         { 'name': f'Vote 5', 'weight': 0.95 },]
}
df_cross2 = h_blend(params, color='tes3', figures1=True, details=True)

df_cross2.to_csv('submission_vross_v123.csv',index=False)
...
df_vote.to_csv('submission.csv',index=False)
