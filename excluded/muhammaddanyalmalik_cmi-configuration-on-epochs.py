...
# ⚡ ری پروڈیوس ایبلٹی کے لئے سیڈ سیٹ کرنا
...
EXPORT_DIR = Path("./")    
BATCH_SIZE = 128       #  زیادہ بیچ سائز GPU کو بہتر استعمال کرنے کے لئے
PAD_PERCENTILE = 98    #  بہتر سیکوئنس پیڈنگ کے لئے
LR_INIT = 3e-4         #  لرننگ ریٹ تھوڑا کم تاکہ زیادہ استحکام رہے
WD = 1e-4              #  ویٹ ڈیکیے زیادہ مناسب رکھا گیا
MIXUP_ALPHA = 0.3      #  مکس اپ ڈیٹا آگرمینٹیشن کے لئے
EPOCHS = 400           #  زیادہ ایپکس تاکہ ماڈل اچھی طرح لرن کرے
PATIENCE = 30          #  پیشنس تھوڑی کم تاکہ ایڈجسٹمنٹ جلدی ہو سکے

print("imports ready · tensorflow", tf.__version__)

# ⚡ کال بیکس بہتر ٹریننگ کے لئے
callbacks = [
    ...
    for i in range(num_samples - 1):
        ...
        if np.all(np.isnan(q1)) or np.all(np.isclose(q1, 0)) or \
           np.all(np.isnan(q2)) or np.all(np.isclose(q2, 0)):
            ...
            continue
        try:
            # کوآٹرنیونز کو Rotation آبجیکٹس میں تبدیل کرنا
            r1 = R.from_quat(q1)
            r2 = R.from_quat(q2)
...
for fold in range(10):
    ...
    print(">>>ماڈل لوڈ ہو رہا ہے>>>", model_path)
    model = load_model(model_path, compile=False, custom_objects=custom_objs)
    models1.append(model)
print("-"*50)
print(f"[معلومات] استعمال شدہ ماڈلز کی تعداد: {len(models1)}")
...
class CMIFeDataset(Dataset):
    ...
    def generate_features(self, df):
        ...
        if all(c in df.columns for c in self.imu_engineered_features) and all(c in df.columns for c in self.tof_cols):
            print("پہلے سے حساب کیا ہوا ہے، دوبارہ حساب چھوڑ دیا گیا۔")
        else:
            print("پہلے سے حساب نہیں ہوا، ابھی حساب کریں۔")
...
class CMIFoldDataset:
    ...
    def print_fold_stats(self):
        ...
        for fold_idx in range(self.n_folds):
            ...
            print(f"\nFold {fold_idx + 1}:")
            print(f"{'زمرہ':<50} {'ٹریننگ سیٹ':<10} {'ویلیڈیشن سیٹ':<10}")
...
# ویلیوایشن میٹرکس صرف لوکل ویلیڈیشن / ٹریننگ کے دوران ہی امپورٹ کیا جاتا ہے
try:
    from cmi_2025_metric_copy_for_import import CompetitionMetric
except ImportError:
    CompetitionMetric = None
    print("CompetitionMetric کو امپورٹ نہیں کیا جا سکا۔ OOF/CV سکور کیلکولیٹ نہیں ہوگا۔")

def seed_everything(seed=42):
    """
    یہ فنکشن ایکزیکیوشن ماحول میں رینڈم نمبر جنریشن کے بیج (seed) کو یکساں طور پر سیٹ کرتا ہے۔
    """
    os.environ['PYTHONHASHSEED'] = str(seed)
    random.seed(seed)
    np.random.seed(2025)
...
# --- راستوں کی سیٹنگ ---
RAW_DIR = Path("/kaggle/input/cmi-detect-behavior-with-sensor-data")
# YOUR_MODELS_DIR کو اپنے ٹرینڈ ماڈل والے Kaggle ڈیٹا سیٹ کے راستے پر سیٹ کریں
YOUR_MODELS_DIR = Path("/kaggle/input/cmi-data-gated-gru") # ★★★ اپنے ماڈل کا راستہ یہاں بدلیں ★★★
...
EXPORT_DIR = Path("./") # ٹرینڈ ماڈل یا آرٹی فیکٹس کو محفوظ کرنے کی جگہ

# --- ماڈل ٹریننگ ہائپرپیرامیٹرز ---
BATCH_SIZE = 256         # بیچ سائز کو بڑھایا گیا تاکہ GPU کے ریسورسز بہتر استعمال ہوں
PAD_PERCENTILE = 90      # سیکوینس پیڈنگ کے لیے پرسنٹائل کم کیا تاکہ کم پیڈنگ ہو اور ٹریننگ تیز ہو
LR_INIT = 3e-4           # لرننگ ریٹ کو تھوڑا کم کیا تاکہ استحکام بہتر ہو
WD = 1e-3                # ویٹ ڈیکے کو کم کیا تاکہ اوور ریگولرائزیشن نہ ہو
MIXUP_ALPHA = 0.3        # Mixup الفا کو ہلکا کم کیا تاکہ زیادہ مکسنگ نہ ہو
EPOCHS = 400             # ایپوک کم کیے تاکہ زیادہ فٹ نہ ہو اور وقت بچے
PATIENCE = 30            # EarlyStopping کی برداشت کم کی تاکہ اوورفٹنگ سے بچا جا سکے
N_SPLITS = 5             # کراس ویلیڈیشن سپلٹس کو کم کیا تاکہ ٹریننگ تیز ہو
MASKING_PROB = 0.2       # ماسکنگ کا امکان ہلکا کم کیا
GATE_LOSS_WEIGHT = 0.15  # گیٹ لاس کے وزن کو ہلکا کم کیا تاکہ بیلنس بہتر رہے


print(f" لائبریری امپورٹ مکمل")
print(f"  - TensorFlow: {tf.__version__}")
print(f"  - PyTorch: {torch.__version__}")
print(f" TRAIN موڈ: {TRAIN}")
...
class ImuFeatureExtractor(nn.Module):
    """
    ★★★ PyTorch ماڈل کے لیے فیچر ایکسٹریکٹر ★★★
    پبلک ماڈل کے ویٹس سے میل کھانے کے لیے اصل درست تعریف پر درستگی کی گئی۔
    """
...
class PublicTwoBranchModel(nn.Module):
    """
    ★★★ پبلک میں موجود PyTorch ماڈل (ماڈلز گروپ C) کو لوڈ کرنے کے لیے اصل آرکیٹیکچر رکھنے والی کلاس ★★★
    """
    def __init__(self, pad_len, imu_dim_raw, tof_dim, n_classes, dropouts=[0.3, 0.3, 0.3, 0.3, 0.4, 0.5, 0.3], feature_engineering=True, **kwargs):
        super().__init__()
        self.feature_engineering = feature_engineering
...
def remove_gravity_from_acc3(acc_data, rot_data):
    """ایکسلیریشن ڈیٹا سے کششِ ثقل کا جزو ہٹانے والا فنکشن"""
    acc_values = acc_data[['acc_x', 'acc_y', 'acc_z']].values
    quat_values = rot_data[['rot_x', 'rot_y', 'rot_z', 'rot_w']].values
    linear_accel = np.zeros_like(acc_values)
    gravity_world = np.array([0, 0, 9.81])
...
def calculate_angular_velocity_from_quat3(rot_data, time_delta=1/200):
    """کواٹرنین سے زاویائی رفتار (Angular Velocity) کا حساب لگانے والا فنکشن"""
    quat_values = rot_data[['rot_x', 'rot_y', 'rot_z', 'rot_w']].values
...
def calculate_angular_distance3(rot_data):
    """کواٹرنین سے زاویائی فاصلہ حساب کرنے کا فنکشن"""
    quat_values = rot_data[['rot_x', 'rot_y', 'rot_z', 'rot_w']].values
...
def enumerate_weights():
    ...
    list_A = [0.500, 0.515, 0.515, 0.510, 0.530, 0.525, 0.530, 0.535]
...
def predict(sequence, demographics):
    ...
    wts,da_w,c_w  = [0.278, 0.343, 0.378], [0.70, 0.30], [ +0.0021,-0.0005477,-0.0015523 ]
...
# --- اِنفرنس سرور کی سیٹنگ ---
inference_server = kaggle_evaluation.cmi_inference_server.CMIInferenceServer(predict)

# اگر Kaggle پر ری رن موڈ ہے تو serve کریں
if os.getenv('KAGGLE_IS_COMPETITION_RERUN'):
    inference_server.serve()
else:
    # لوکل گیٹ وے پر اِنفرنس چلائیں
...
# اگر ری رن موڈ نہیں ہے تو سبمیشن فائل پرنٹ کریں
