...
feature_pipeline = ColumnTransformer([
    ('imputer', SimpleImputer(strategy='mean'), features),  # Eksik verileri doldur
    ('log', FunctionTransformer(np.log1p), log_features),   # Log dönüşümü uygula
    ('poly', PolynomialFeatures(degree=2, interaction_only=True), features), # Etkileşimli özellikler ekle
    ('scale', StandardScaler(), features)  # Standard Scaler kullan
], remainder='passthrough')

# Daha güçlü bir model oluştur
base_model = RidgeClassifier(alpha=10)  # Alpha değerini optimize ettik
...
ensemble_model = StackingClassifier(
    ...
    final_estimator=RidgeClassifier(alpha=1),  # Final model olarak Ridge kullan
    cv=5
)

# PCA ile boyut azaltma + Feature Selection
dim_reduction = PCA(n_components=500)  # Daha az bileşen kullanarak hızlandır
feature_selection = SelectKBest(f_classif, k=800)  # En iyi 800 özelliği seç



print(train.isnull().sum())  # Train verisindeki eksik değerleri say
print(y.isnull().sum())
...
model = MultiOutputClassifier(make_pipeline(
    ColumnTransformer([('imputer', SimpleImputer(), features)], remainder='passthrough'),
    StandardScaler(),
    XGBClassifier(
        n_estimators=500,  # Daha fazla ağaç
        learning_rate=0.03,  # Daha küçük öğrenme oranı
        max_depth=8,  # Daha derin ağaçlar
        colsample_bytree=0.8,  # Özellik seçiminde rastgelelik
        subsample=0.8,  # Daha iyi genelleştirme
        gamma=1,  # Dallanma için minimum gereksinim
        reg_lambda=10,  # L2 regülarizasyonu
        reg_alpha=1,  # L1 regülarizasyonu
        random_state=42
    )
