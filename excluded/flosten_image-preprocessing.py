...
def Unet_Arch(dropout_num, input_shape=(256, 256, 3)):
    ...
    # contracting path
    c1 = layers.Conv2D(
        16, (3, 3), activation="relu", kernel_initializer=conv_init, bias_initializer=bias_init, padding="same"
    )(
        inputs
    )  # 256 x 256 x 16
    c2 = layers.Conv2D(
    ...
    c3 = layers.Conv2D(
        32, (3, 3), activation="relu", kernel_initializer=conv_init, bias_initializer=bias_init, padding="same"
    )(
        p1
    )  # 128 x 128 x 32
    c4 = layers.Conv2D(
    ...
    # bottleneck
    c5 = layers.Conv2D(
        64, (3, 3), activation="relu", kernel_initializer=conv_init, bias_initializer=bias_init, padding="same"
    )(
        p2
    )  # 64 x 64 x 64
    c6 = layers.Conv2D(
...
def Unet_test(model, dataset):
    ...
    # get the test sample
    for image, _ in cbb_dataset.skip(56).take(1):
    ...
    for image, _ in cgm_dataset.skip(45).take(1):
...
trainset, valset = data_acquisition(gcs_path, trainset_path, image_size)


Unet_trainset, Unet_valset = UNet_preprocessing(trainset, batch_size_unet)


model = Unet_Arch(dropout_num=0, input_shape=[*image_size, 3])
Unet_model, Unet_lr_fig = Unet_train(model, Unet_trainset, Unet_valset, epochs = epochs, learning_rate=Unet_lr)


fig_image_segment_1 = Unet_test(Unet_model, trainset)
...
print("cbb + original")
gen = set_seed()
CBB_trainset, CBB_valset = UNet_preprocessing_pro(trainset, batch_size_unet, gen)


model_pro = Unet_Arch(dropout_num = dropout_num_pro, input_shape=(*image_size, 3))
Unet_model_pro, Unet_lr_fig_pro = Unet_train(model_pro, CBB_trainset, CBB_valset, epochs = epochs_pro, learning_rate=Unet_lr_pro)


fig_image_segment_2 = Unet_test(Unet_model_pro, trainset)
...
# Unet_lr_fig.savefig(os.path.join(figure_dir, "Unet_learning_curve_origin"))
# Unet_lr_fig_pro.savefig(os.path.join(figure_dir, "Unet_learning_curve_improve"))
# fig_image_segment_1.savefig(os.path.join(figure_dir, "Unet_image_segment_origin"))
# fig_image_segment_2.savefig(os.path.join(figure_dir, "Unet_image_segment_improve"))


# Unet_model.save(os.path.join(model_dir, "Unet_model_origin.h5"))
# Unet_model.save(os.path.join(model_dir, "Unet_model_origin.keras"))
# Unet_model_pro.save(os.path.join(model_dir, "Unet_model_improve.h5"))
# Unet_model_pro.save(os.path.join(model_dir, "Unet_model_improve.keras"))
