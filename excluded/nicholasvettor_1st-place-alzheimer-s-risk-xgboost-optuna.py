...
study.optimize(objective, n_trials=100) # 500
...
xgb.plot_importance(tuned_model, max_num_features=100, show_values=True, grid=False)   
plt.rcParams['axes.labelcolor'] = 'black'

for index, column in enumerate(train.columns):
    print(f"Column: {column} | Index: f{index}")
plt.show()

# plot SHAP tree explainer / values
explainer = shap.TreeExplainer(tuned_model)
shap_values = explainer.shap_values(test)
column_names = train.drop(['Diagnosis'], axis=1).columns.tolist()
shap.summary_plot(shap_values, test, feature_names=column_names, show=True)
