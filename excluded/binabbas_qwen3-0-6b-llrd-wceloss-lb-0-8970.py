...
def train_epoch(model, dataloader, optimizer, device, scheduler, loss_fct):
    model.train()
    total_loss = 0.0
...
#llrd implementation (not perfect)
...
# --- LoRA Config
    ...
    lora_alpha=32,
    target_modules=[
        "q_proj", "k_proj", "v_proj", "o_proj", "mlp.gate_proj", "mlp.up_proj", "mlp.down_proj"  # attention projections and mlp projections
    ],
    lora_dropout=0.05,
    bias="none",
    ...
    for epoch in range(num_epochs):
        ...
        train_loss = train_epoch(model, train_loader, optimizer, device, scheduler, loss_fct)
    ...
    # Load best model weights before validation and test prediction
...
#Save test dataset probs for ensembling
