...
def add_features(df):
    ...
    df['BMI_Bin'] = pd.qcut(df['BMI'], q=5, labels=False)
    df['Mean_HR_Sex'] = df.groupby('Sex')['Heart_Rate'].transform('mean')
    df['Std_Temp_Sex'] = df.groupby('Sex')['Body_Temp'].transform('std')
    return df
