...
class InMemorySessionService:
    """Simple thread-safe session store keyed by session_id."""
    def __init__(self):
    ...
    def get_state(self, session_id: str, key: str, default=None):
        with self._lock:
            ...
            if not s:
                return default
            return s.get("state", {}).get(key, default)
...
class ContentSearchAgent:
    def run(self, topic: str) -> List[Resource]:
        # Simulate latency (keep or remove as needed)
        time.sleep(0.4)
...
class StudyCoordinator:
    ...
    def plan_for(self, session_id: str, topics: List[str], weekly_hours: int = 5) -> Dict[str, Any]:
        ...
        max_workers = min(6, max(1, len(topics) * 2))
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
            ...
            for fut in concurrent.futures.as_completed(search_futures):
                topic = search_futures[fut]
                try:
            ...
            # collect flashcards
            for fut in concurrent.futures.as_completed(flash_futures):
                topic = flash_futures[fut]
                try:
                    cards = fut.result()
                except Exception:
                    cards = []
...
if __name__ == "__main__":
    ...
    stored = session_service.get(sid)
    print('\nSession stored state keys:', list(stored.get('state', {}).keys()) if stored else [])
