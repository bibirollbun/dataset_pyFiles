...
agg_funcs = {
    "purchase_amount": ["min", "max", "mean", "var", "skew"], # "mean", "var", "skew"を追加
    "installments":    ["min", "max", "mean", "nunique"], # "mean", "nunique"を追加
    "month_lag":       ["min", "max", "mean", "var"], # "mean", "var"を追加
    "purchase_date":   ["count"],
    "authorized_flag": ["mean"], #取引が認証済みかどうか
    "category_1":      ["mean"], # 割引対象かどうか(Y → 1, N → 0)
    "city_id":         ["nunique"], # 異なる都市の数
    "state_id":        ["nunique"], # 異なる州の数
    "subsector_id":    ["nunique"], # 異なる業種の数
}
...
# # 　　命名の規則：元変数名_集約方法（「purchase_amount_mean」など）
flat_cols = []
