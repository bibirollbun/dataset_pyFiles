...
class Transform(Config):
    ...
    def __init__(self):
        ...
        if Config.original_data == 'Y':
            ...
            self.train.dropna(subset=['Listening_Time_minutes'],inplace=True)
    ...
    def new_features(self, data):
        ...
        data["Publication_Day"] = data["Publication_Day"].map({"Sunday": 0, 
                                                               "Monday": 1, 
                                                               "Tuesday": 2, 
                                                               "Wednesday": 3, 
                                                               "Thursday": 4,
        ...
        data["SinWeekday"] = np.sin(2 * np.pi * data["Publication_Day"]/ 7)
        data["CosWeekday"] = np.cos(2 * np.pi * data["Publication_Day"]/ 7)
        
        data["Publication_Time"] = data["Publication_Time"].map({"Morning"   : 0, 
                                                                 "Afternoon" : 1, 
                                                                 "Evening"   : 2, 
                                                                 "Night"     : 3, 
                                                                 })
            
        data["SinTime"] = np.sin(2 * np.pi * data["Publication_Time"]/ 4)
        data["CosTime"] = np.cos(2 * np.pi * data["Publication_Time"]/ 4)
        
        data["Number_of_Ads"] = data["Number_of_Ads"].fillna(0).clip(0,3).astype(np.int8)
        
        data['SinEpLen'] = np.sin(2*np.pi * data['Episode_Length_minutes']/60 )
        data['CosEpLen'] = np.cos(2*np.pi * data['Episode_Length_minutes']/60 )
        
        data["ELen_Int"] = np.floor(data["Episode_Length_minutes"])
        data["ELen_Dec"] = data["Episode_Length_minutes"] - data["ELen_Int"]
        data['Long_Episode'] = (data['Episode_Length_minutes'] > 75).astype(int)
        data['Ads_Per_Minute'] = data['Number_of_Ads'] / (data['Episode_Length_minutes'] + 1e-5)
        
        for c1, c2, c3 in list(combinations(self.num_features,3)):
            data[f"{c1}_{c2}_{c3}"] = data[c1]*399 + data[c2]*266 + data[c3]*133

        for c in ['Episode_Length_minutes', 'Guest_Popularity_percentage']:
            data[f'NA_{c}'] = data[c].isna().astype(int)
        ...
        for r in [2,3,4]:
            for cols in list(combinations(encode_columns, r)):
                ...
                data[new_col_name] = data[list(cols)].astype(str).agg('_'.join, axis=1)
                data[new_col_name] = data[new_col_name].astype('string')
    ...
    def distribution(self):
        ...
        ax_r.set_title(f'{self.target} ($\mu=$ {self.train[self.target].mean():.2f} and $\sigma=$ {self.train[self.target].std():.2f} )')
        ax_r.hist(self.train[self.target], bins=30, color='#3cb371')
        ax_r.axvline(self.train[self.target].mean(), color='r', label='Mean')
        ax_r.axvline(self.train[self.target].median(), color='y', linestyle='--', label='Median')
        ax_r.legend()
...
models = {
    ...
    'YDF2': [YDFRegressor(GradientBoostedTreesLearner)({'num_trees': 1000,
            ...
            False],
    'YDF3': [YDFRegressor(GradientBoostedTreesLearner)({'num_trees': 1000,
                                                        'random_seed': Config.state,
                                                        'max_depth': 15,
                                                       }),
            False],
    'CAT2': [CatBoostRegressor(**{'verbose': 0,
                                 ...
                                 'depth': 15,
                                 'min_data_in_leaf': 9, 
                                 'l2_leaf_reg': 0.3475751593663069,
                                 'random_strength': 0.11405558692517365,
                                 'learning_rate': 0.01,
                                 'bootstrap_type': 'Bernoulli',
                                 "task_type": "GPU",
                              }),
            False], # without Tergetencoder
    'CAT3': [CatBoostRegressor(**{'verbose': 0,
                                  ...
                                  'depth': 15,
                                  'min_data_in_leaf': 57, 
                                  'l2_leaf_reg': 0.015156389557174892,
                                  'bagging_temperature': 0.032916820845056094, 
                                  'random_strength': 1.63549090106827,
                                  "task_type": "GPU",
                                  'learning_rate': 0.01,
                              }),
            False],
    'XGB': [XGBRegressor(**{'tree_method': 'hist',
                            ...
                            'max_depth': 15,
                            'min_child_weight': 16,
                            'subsample': 0.8972912757368473,
                            'reg_alpha': 0.08843186626948404,
                            'reg_lambda': 0.8418502675530148,
                            'colsample_bytree': 0.5059143458705216,
                            'n_jobs': -1,
                            'learning_rate': 0.01,
                           }),
            False],
    ...
    'LGBM': [LGBMRegressor(**{'random_state': Config.state,
                              ...
                              'min_child_weight': 25,
                              'colsample_bytree': 0.3705567913667146, 
                              'reg_alpha': 0.4928689757235687,
                              'reg_lambda': 0.5121492710858493,
                              'learning_rate': 0.01,
                              }),
             False],
    ...
    'LGBM3': [LGBMRegressor(**{'random_state': Config.state,
                               ...
                               'min_child_weight': 20,
                               'colsample_bytree': 0.15371463545873185, 
                               'reg_alpha': 0.15579962406415954,
                               'reg_lambda': 0.6758246587532051,
                               'learning_rate': 0.01,
                              }),
             False],
    'LGBM4': [LGBMRegressor(**{'random_state': Config.state,
                               'early_stopping_round': Config.early_stop,
                               'categorical_feature': cat_features,
                               'verbose': -1,
                               'boosting_type': 'gbdt',
                               ...
                               'min_child_weight': 16, 
                               'colsample_bytree': 0.2002140197928391,
                               'reg_alpha': 0.3953804922548826,
                               'reg_lambda': 0.5792740097975614,
                               'learning_rate': 0.01,
                          }),
             False],
    'HGB': [HistGradientBoostingRegressor(**{'max_iter': 5000,
                                             ...
                                             'max_depth': 200,
                                             'learning_rate': 0.01,
                                             'l2_regularization': 6.7919206827362055e-09, 
                                             'min_samples_leaf': 26,
                                             'max_leaf_nodes': 38,
                                             }),
         False],
...
class Model(Config):
    ...
    def train(self):
        ...
        for model_name, [model, training] in tqdm(self.models.items()):
            ...
            if training:
                ...
                if 'NN' in model_name:
                    for n_fold, (train_id, valid_id) in enumerate(folds.split(self.X)):
                        X_train = self.X.loc[train_id].copy()
                        y_train = self.y.iloc[train_id]
                        X_val = self.X.loc[valid_id].copy()
                        y_val = self.y.iloc[valid_id]

                        encoded_columns = X_train.columns.to_list()[26:]
                        encoder = TargetEncoder(random_state=42)
                        
                        X_train[encoded_columns] = encoder.fit_transform(X_train[encoded_columns], y_train).astype('float32')
                        X_val[encoded_columns] = encoder.transform(X_val[encoded_columns]).astype('float32')
                    ...
                    for n_fold, (train_id, valid_id) in enumerate(folds.split(self.X, self.y)):
                        ...
                        encoded_columns = X_train.columns[26:]
                ...
                self.OOF_preds[f'{model_name}'] = pd.read_csv(f'/kaggle/input/podcast-org/{model_name}_oof.csv')
                self.TEST_preds[f'{model_name}'] = pd.read_csv(f'/kaggle/input/podcast-org/{model_name}_test.csv')
    ...
    def ensemble(self, X, y, test):
        ...
        oof_pred = np.zeros(X.shape[0])
        test_pred = np.zeros(test.shape[0])
        model = BayesianRidge(tol=1e-2, max_iter=1000000)
...
blender = pd.read_csv('/kaggle/input/podcast-org/blender.csv')
blender[f'{model.scores.loc["Ensemble", "Score"]:.6f}'] = np.clip(TEST_preds['Ensemble'].values, 0, 120)

submission[Config.target] = np.clip(blender.mean(axis=1).values, 0, 120)
