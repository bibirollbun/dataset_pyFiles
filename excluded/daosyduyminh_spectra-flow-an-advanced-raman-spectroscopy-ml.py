# ==============================================================================
# CELL 1: INSTALL AND IMPORT NECESSARY LIBRARIES (ALL-IN-ONE)
# ==============================================================================
...
# Libraries for Ensembling and Evaluation
...
# ==============================================================================
# CELL 2: CORE PROCESSING FUNCTIONS (Original logic preserved)
# ==============================================================================
...
def load_and_preprocess_data(filepath, is_train=True):
    ...
    if is_train:
        ...
        # Clean sample IDs for the test set
...
def preprocess_spectra_upgraded(X, method='baseline_snv_airpls', deriv_order=1):
    """Applies advanced preprocessing to spectral data."""
    ...
    if method == 'baseline_snv_airpls':
        for i in range(X_processed.shape[0]):
            ...
            if std > 1e-8: # Avoid division by zero
                X_processed[i, :] = (corrected_spectrum - mean) / std # SNV
            else:
                 X_processed[i, :] = corrected_spectrum - mean
    elif method == 'derivative':
        # Savitzky-Golay filter for derivatives
        X_processed = signal.savgol_filter(X, window_length=11, polyorder=3, deriv=deriv_order, axis=1)
    return X_processed

# --- Feature Engineering Functions ---
def extract_peak_features(spectra):
    """Extracts features related to spectral peaks."""
    features = []
    for spec in spectra:
        ...
        features.append([
            ...
            np.sum(spec[peaks]) if len(peaks) > 0 else 0, # Sum of peak intensities
            np.mean(spec[peaks]) if len(peaks) > 0 else 0, # Average peak intensity
            np.mean(widths) if len(widths) > 0 else 0, # Average peak width
        ])
...
def create_required_feature_sets(X_train_array, X_test_array):
    ...
    # The competition data has two scans per sample, we average them here.
    ...
    # Create a master feature set with everything combined
    ...
    # Store all feature sets in a dictionary, applying standard scaling
    feature_sets = {
...
def create_tuned_models():
    """Defines a portfolio of pre-tuned models and their target feature sets."""
    models = []
    # Each entry defines a model name, the feature set it uses, and a list of three
    # pre-tuned regressors, one for each target analyte (Glucose, Acetate, Magnesium).
    ...
    models.append({'name': 'Mean_Processed_ExtraTrees', 'feature_set': 'Mean_Processed', 'target_models': [ExtraTreesRegressor(n_estimators=882, max_depth=33, min_samples_split=2, min_samples_leaf=1, random_state=42, n_jobs=-1), ExtraTreesRegressor(n_estimators=420, max_depth=9, min_samples_split=6, min_samples_leaf=1, random_state=42, n_jobs=-1), ExtraTreesRegressor(n_estimators=546, max_depth=12, min_samples_split=5, min_samples_leaf=5, random_state=42, n_jobs=-1)]})
    models.append({'name': 'Mean_Processed_Ridge', 'feature_set': 'Mean_Processed', 'target_models': [Ridge(alpha=0.010011942119197155, random_state=42), Ridge(alpha=0.010011429727507454, random_state=42), Ridge(alpha=0.010010026529996902, random_state=42)]})
    return models
...
def evaluate_and_train_models(tuned_models, feature_sets, y_train):
    ...
    all_predictions = {target: [] for target in range(3)} # Test set predictions
    all_cv_predictions = {target: [] for target in range(3)} # OOF CV predictions
    ...
    for model_config in tuned_models:
        ...
        for target_idx in range(3):
            ...
            # Store OOF predictions for ensembling
...
# --- Ensembling and Submission Functions ---
def create_advanced_ensemble(all_predictions, all_cv_predictions, all_cv_targets, model_scores):
    ...
    for target_idx in range(3):
        ...
        # Find optimal weights that minimize MSE on CV predictions
        ...
        # Evaluate all ensemble methods on CV data
        methods = {
...
def post_process_predictions(preds, y_train):
    ...
    processed_preds = np.maximum(preds, 0) # Ensure non-negative
    for i in range(processed_preds.shape[1]):
        # Clip predictions to the 1st and 99th percentile of training data
...
def save_top_3_submissions(all_ensemble_predictions, ensemble_methods, X_test_raw, y_train):
    ...
    # Find the top 3 methods based on average performance across all targets
    method_avg_scores = {}
    ...
    # Generate and save a submission file for each of the top 3 methods
...
def main_train():
    ...
    print("="*80, "\nSTAGE 1: MODEL TRAINING PIPELINE (UPGRADED)\n", "="*80)
    ...
    # Reshape data: each sample has 2 scans of 2048 wavenumbers
    ...
    print("\n2. Generating required feature sets with upgraded preprocessing...")
...
def main_ensemble():
    ...
    try:
        ...
        print("Please run the training stage first.")
        return
...
def main_final_ensemble():
    """Averages the top submission files to create a final submission."""
    print("="*80)
    print("STAGE 3: FINAL ENSEMBLE AVERAGING")
    print("="*80)
    submission_files = glob.glob('submission_*.csv')
    # Exclude the final submission file if it already exists
    submission_files = [f for f in submission_files if not f.startswith('submission.csv')]
    
    if len(submission_files) < 1:
        print("Error: No individual submission files found to average.")
        print("Please run the ensembling stage first.")
        return
    ...
    # Stack predictions from all files for averaging
