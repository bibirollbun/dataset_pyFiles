...
# Loading the dataset using glob and storing them in a dictionary.
...
# Combining the team data and preparing a unique TeamID for each team
...
# Combining the results of the season and tournament data for compact and detailed results
...
# Creating a dictionary to map each team to their respective seed
seeds = {'_'.join(map(str,[int(k1),k2])):int(v[1:3]) for k1, v, k2 in seeds[['Season', 'Seed', 'TeamID']].values}

# Preparing the cities and sample submission data
cities = data['Cities']
sub = data['SampleSubmissionStage1']
del data

# Marking whether it's a regular season or tournament game
season_cresults['ST'] = 'S'
season_dresults['ST'] = 'S'
tourney_cresults['ST'] = 'T'
tourney_dresults['ST'] = 'T'
# Combining regular season and tournament results for modeling
games = pd.concat((season_dresults, tourney_dresults), axis=0, ignore_index=True)
games.reset_index(drop=True, inplace=True)

# Mapping 'WLoc' to numerical values
games['WLoc'] = games['WLoc'].map({'A': 1, 'H': 2, 'N': 3})

# Creating unique ID for each game based on season and teams involved
games['ID'] = games.apply(lambda r: '_'.join(map(str, [r['Season']]+sorted([r['WTeamID'],r['LTeamID']]))), axis=1)
games['IDTeams'] = games.apply(lambda r: '_'.join(map(str, sorted([r['WTeamID'],r['LTeamID']]))), axis=1)

# Assigning the teams to Team1 and Team2 based on the sorted order
games['Team1'] = games.apply(lambda r: sorted([r['WTeamID'],r['LTeamID']])[0], axis=1)
games['Team2'] = games.apply(lambda r: sorted([r['WTeamID'],r['LTeamID']])[1], axis=1)

# Creating IDs for each team in a given game
games['IDTeam1'] = games.apply(lambda r: '_'.join(map(str, [r['Season'], r['Team1']])), axis=1)
games['IDTeam2'] = games.apply(lambda r: '_'.join(map(str, [r['Season'], r['Team2']])), axis=1)

# Getting seeds for each team in the game
games['Team1Seed'] = games['IDTeam1'].map(seeds).fillna(0)
games['Team2Seed'] = games['IDTeam2'].map(seeds).fillna(0)

# Calculating score difference and setting up the predicted result (Win or Loss)
...
# Aggregating statistics for each game, including field goals, assists, turnovers, etc.
...
# Filtering games to only include tournament games
...
# Merging aggregated game statistics to both games and submission data
...
# Setting up the machine learning model pipeline
...
def objective(trial):
    et_params = {
        'et__n_estimators': trial.suggest_int('et__n_estimators', 200, 270),
        'et__max_depth': trial.suggest_int('et__max_depth', 10, 20),
        'et__min_samples_split': trial.suggest_int('et__min_samples_split', 2, 5),
    ...
    rf_params = {
        'rf__n_estimators': trial.suggest_int('rf__n_estimators',200, 270),
        'rf__max_depth': trial.suggest_int('rf__max_depth', 10, 20),
        'rf__min_samples_split': trial.suggest_int('rf__min_samples_split', 2, 5),
        'rf__max_features': trial.suggest_categorical('rf__max_features', ['sqrt', 'log2']),
        'rf__bootstrap': True,
        'rf__n_jobs': -1,
        'rf__random_state': 42
    }
    ...
    model = Pipeline(steps=[('voting', voting_regressor)])
...
best_params = {'et__n_estimators': 253, 'et__max_depth': 12, 'et__min_samples_split': 3, 'et__max_features': 'sqrt', 'et__criterion': 'squared_error', 
               'rf__n_estimators': 256, 'rf__max_depth': 20, 'rf__min_samples_split': 2, 'rf__max_features': 'log2'}
...
voting_regressor = VotingRegressor(estimators=[('et', et), ('rf', rf)])
pipe = Pipeline(steps=[('voting', voting_regressor)])
...
# Saving the final submission to a CSV file
sub[['ID', 'Pred']].to_csv('20submission.csv', index=False)
print(sub[['ID', 'Pred']].head())  # Preview the first few predictions
