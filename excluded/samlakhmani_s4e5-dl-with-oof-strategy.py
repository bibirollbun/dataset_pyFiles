...
def abalation_study(train, test, actual_target=TARGET) : 
...
abalation_study(
    ...
    test.drop('id',axis=1),
    actual_target = 'FloodProbability'
)
...
def get_scaled_train_val_test(train, val, test=None):
    ...
    # Scale features
    train_scaled = sc.fit_transform(train.drop(['id',TARGET], axis=1))
    val_scaled   = sc.transform(val.drop(['id',TARGET], axis=1))
    
    # Convert to float32 and move to device
    X_train = torch.tensor(train_scaled, dtype=torch.float32).to(device)
    y_train = torch.tensor(train[TARGET].values, dtype=torch.float32).to(device)
    
    X_val = torch.tensor(val_scaled, dtype=torch.float32).to(device)
    y_val = torch.tensor(val[TARGET].values, dtype=torch.float32).to(device)
...
def objective(trial):
    ...
    # 4️⃣ Compute RMSE
    # -------------------------------
    y_true = train[TARGET].values.reshape(-1, 1)  # target as NumPy
...
def train_model_and_get_val_test_pred(X_train, y_train, X_val, y_val, X_test,
    ...
    for n in range(N_iterations):
        ...
        with torch.no_grad():
            y_val_pred = model(X_val)
            val_loss = eval_metric(
                y_val_pred.detach().cpu().numpy(), 
                y_val.unsqueeze(1).detach().cpu().numpy()
            )

        if n%5==0:
            print(f"\tIteration {n}: eval_metric = {val_loss:.6f}")
...
y_true = train[TARGET].values.reshape(-1, 1)
y_pred = np.clip(pred_oof, a_min=0, a_max=None)  # avoid negative predictions
...
train[f'{TARGET}_pred'] = pred_oof


train[TARGET].describe()


train[f'{TARGET}_pred'].describe()
...
test[['id',TARGET]].to_csv("v2_with_OOF_strategy.csv", index=False)
