...
# # Redefine the TransformerDecoder with standard layer normalization (no TFA dependency)  
# class TransformerDecoder(layers.Layer):  
#     def __init__(self, embed_dim, num_heads, feed_forward_dim, dropout_rate=0.1):  
#         super().__init__()  
#         self.layernorm1 = layers.LayerNormalization(epsilon=1e-6)  
...
class TransformerDecoder(layers.Layer):  
    def __init__(self, embed_dim, num_heads, feed_forward_dim, dropout_rate=0.1):  
        super().__init__()  
        # Pre-normalization for better training stability (modern approach)  
        self.prenorm1 = layers.LayerNormalization(epsilon=1e-6)  
        self.prenorm2 = layers.LayerNormalization(epsilon=1e-6)  
        self.prenorm3 = layers.LayerNormalization(epsilon=1e-6)  
        
        # Increased key dimension for better representation  
        head_dim = embed_dim // num_heads  
        self.self_att = layers.MultiHeadAttention(  
            num_heads=num_heads, key_dim=head_dim, dropout=dropout_rate  
        )  
        ...
        # Enhanced FFN with SwiGLU-like activation (similar to PaLM, LLaMA architectures)  
        self.ffn = keras.Sequential([  
            layers.Dense(feed_forward_dim, kernel_regularizer=reg),  
            layers.Activation(lambda x: x * tf.nn.silu(x)),  
            layers.Dropout(dropout_rate * 1.5),  # Increase dropout  
            layers.Dense(embed_dim, kernel_regularizer=reg),  
        ]) 
        
        # Add gating mechanism to control information flow  
        # Consider initializing gates with bias toward being open  
        self.gate1 = layers.Dense(embed_dim, activation='sigmoid',   
                                 kernel_regularizer=reg)  
        self.gate2 = layers.Dense(embed_dim, activation='sigmoid',  
                                 kernel_regularizer=reg)  

    def causal_attention_mask(self, batch_size, n_dest, n_src, dtype):  
        """Optimized causal attention mask creation"""  
        i = tf.range(n_dest)[:, None]  
        j = tf.range(n_src)  
        m = i >= j - n_src + n_dest  
        mask = tf.cast(m, dtype)  
    ...
    def call(self, enc_out, target, training):  
        ...
        # Pre-normalization pattern (modern approach)  
        # Self-attention block  
        residual = target  
        x = self.prenorm1(target)  
        x = self.self_att(x, x, attention_mask=causal_mask)  
        x = self.self_dropout(x, training=training)  
        # Gated residual connection  
        gate1 = self.gate1(residual)  
        x = residual + x * gate1  
        
        # Cross-attention block  
        residual = x  
        x = self.prenorm2(x)  
        x = self.enc_att(x, enc_out)  
        x = self.enc_dropout(x, training=training)  
        # Gated residual connection  
        gate2 = self.gate2(residual)  
        x = residual + x * gate2  
        
        # FFN block with SwiGLU-like activation  
        residual = x  
        x = self.prenorm3(x)  
        x = self.ffn(x)  
        x = self.ffn_dropout(x, training=training)  
        # Simple residual for FFN  
        x = residual + x  
...
# Get a validation batch for the display callback  
# Get a validation batch for the display callback  
batch = next(iter(valid_ds))  
...
history = model.fit(train_ds, validation_data=valid_ds, callbacks=[display_cb], epochs=40)  
...
plt.legend(['training edit_dist', 'val_edit_dist'])
