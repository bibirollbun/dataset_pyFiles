...
PostgreSQL - local instance and psql command-line tool
...
MCP Toolbox for Databases is an open-source Model Context Protocol (MCP) server for databases including PostgreSQL. It allows you to define "tools" against your database, with matching SQL queries, effectively enabling agent "function-calling" for your database.

First, download the MCP toolbox binary if not already installed.
...
You can verify the server is running by opening http://localhost:5000/api/toolset in your browser. You should see a JSON response with the list of tools specified in tools.yaml.
...
Through the CLI (adk run):

uv run adk run software_bug_assistant
Through the web interface (adk web):

uv run adk web
The command adk web will start a web server on your machine and print the URL. You may open the URL, select "software_bug_assistant" in the top-left drop-down menu, and a chatbot interface will appear on the right. The conversation is initially blank.
...
â˜�ï¸� Deploy to Google Cloud
These instructions walk through the process of deploying the Software Bug Assistant agent to Google Cloud, including Cloud Run and Cloud SQL (PostgreSQL). This setup also adds RAG capabilities to the tickets database, using the google_ml_integration vector plugin for Cloud SQL, and the text-embeddings-005 model from Vertex AI.
...
Now we are ready to deploy the ADK Python agent to Cloud Run! ğŸš€
...
âš¡ï¸� Alternative: Using uv
The starter pack will prompt you to select deployment options and provides additional production-ready features including automated CI/CD deployment scripts.

Clean up
You can clean up this agent sample by:

Deleting the Artifact Registry.
Deleting the two Cloud Run Services.
Deleting the Cloud SQL instance.
Deleting the Secret Manager secret.
