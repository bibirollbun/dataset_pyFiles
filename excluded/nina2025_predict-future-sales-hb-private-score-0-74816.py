...
def matrix_vs(path,fs_names):
    def load(path,fs_names):
        ...
        for i in range(len(dfs)):
            dfs[i] = dfs[i].rename(columns={"item_cnt_month": f'{fs_names[i]}'})
...
params = {
      'path'     : f'/kaggle/input/24-december-2025-predict-future-sales/',            
      'id_target': ['ID',"item_cnt_month",],        
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/100 for w in [ -10, +13, -3 ]],
      'subm'     : [
          {'name': f'0.74828', 'weight':+0.85, 'color':'royalblue'},
          {'name': f'0.75467', 'weight':+0.05, 'color':'orange'   },
          {'name': f'0.75504', 'weight':+0.10, 'color':'green'    },]
}
