...
oof_preds_df = scaler.fit_transform(oof_preds_df)
test_preds_df = scaler.transform(test_preds_df)

for col in oof_preds_df.columns:
    oof_preds_df[col] = rankdata(oof_preds_df[col])
    test_preds_df[col] = rankdata(test_preds_df[col])


if CFG.use_saved_weights:
    model_weights = {
    ...
    current_best_ensemble = oof_preds_df['ag_exponential_rank']
    current_best_test_preds = test_preds_df['ag_exponential_rank']
    for k, v in model_weights.items():
        current_best_ensemble = (1 - v) * current_best_ensemble + v * oof_preds_df[k]
        current_best_test_preds = (1 - v) * current_best_test_preds + v * test_preds_df[k]
    ...
    _scores = {}
    for col in oof_preds_df.columns:
        _scores[col] = eval_metric(oof_preds_df[col])
    
    _scores = {k: v for k, v in sorted(_scores.items(), key=lambda item: item[1], reverse=True)}
    
    print(f"{YELLOW_TXT}Models to be ensembled | ({len(_scores)} total):{RESET_TXT} \n")
    max_model_len = max(len(model) for model in _scores.keys())
    
    for e, (model, _score) in enumerate(_scores.items()):
        model_padding = " " * (max_model_len - len(model))
        score_str = f"{_score:.5f}".rjust(2)
        if e == 0:
    ...
    oof_preds_df = oof_preds_df[list(_scores.keys())]
    test_preds_df = test_preds_df[list(_scores.keys())]
    current_best_ensemble = oof_preds_df.iloc[:, 0]
    current_best_test_preds = test_preds_df.iloc[:, 0]
    MODELS = oof_preds_df.iloc[:, 1:].copy()
    history = [eval_metric(current_best_ensemble)]
    
    if CFG.allow_negative_weights:
        weight_range = np.arange(-0.5, 0.51, CFG.precision)
    else:
        weight_range = np.arange(CFG.precision, 0.51, CFG.precision)
    
    decimal_length = len(str(CFG.precision).split(".")[1])
    eval_metric_name = "CI Score"
    ...
    def evaluate_model_weight(args):
        k, wgt, current_best_ensemble = args
        potential_ensemble = (1 - wgt) * current_best_ensemble + wgt * MODELS[k]
        cv_score = eval_metric(potential_ensemble)
        return (k, wgt, cv_score)
    
    
    def parallel_hill_climbing():
        i = 0
        current_best_ensemble = oof_preds_df.iloc[:, 0].copy()
        current_best_test_preds = test_preds_df.iloc[:, 0].copy()
        MODELS_working = oof_preds_df.iloc[:, 1:].copy()
        history = [eval_metric(current_best_ensemble)]
        model_weights = {}
        ...
        while not STOP:
            ...
            potential_new_best_cv_score = eval_metric(current_best_ensemble)
            k_best, wgt_best = None, None
    
            if MODELS_working.shape[1] == 0:
                STOP = True
                continue
    
            combinations = []
            for k in MODELS_working.columns:
                for wgt in weight_range:
                    combinations.append((k, wgt, current_best_ensemble))
    
            with multiprocessing.Pool(processes=num_processes) as pool:
                results = pool.map(evaluate_model_weight, combinations)
    
            for k, wgt, cv_score in results:
            ...
            if k_best is not None:
                current_best_ensemble = (1 - wgt_best) * current_best_ensemble + wgt_best * MODELS_working[k_best]
                current_best_test_preds = (1 - wgt_best) * current_best_test_preds + wgt_best * test_preds_df[k_best]
                model_weights[k_best] = wgt_best
                MODELS_working = MODELS_working.drop(k_best, axis=1)
    
                if wgt_best > 0:
                    print(f"{GREEN_TXT}Iteration: {i} | Best weight: +{wgt_best:.{decimal_length}f} | Best {eval_metric_name}: {potential_new_best_cv_score:.5f} | Time: {iter_time:.2f}s | Model added: {k_best}{RESET_TXT}")
                elif wgt_best < 0:
                    print(f"{RED_TXT}Iteration: {i} | Best weight: {wgt_best:.{decimal_length}f} | Best {eval_metric_name}: {potential_new_best_cv_score:.5f} | Time: {iter_time:.2f}s | Model added: {k_best}{RESET_TXT}")
                else:
                    print(f"Iteration: {i} | Best weight: {wgt_best:.{decimal_length}f} | Best {eval_metric_name}: {potential_new_best_cv_score:.5f} | Time: {iter_time:.2f}s | Model added: {k_best}")
    
                history.append(potential_new_best_cv_score)
            else:
                STOP = True
    
        return current_best_ensemble, current_best_test_preds, history, model_weights
    
    
    current_best_ensemble, current_best_test_preds, history, model_weights = parallel_hill_climbing()
    
    end = perf_counter()
    print(f"\n{YELLOW_TXT}Hill climbing completed in {end-start:.2f} seconds{RESET_TXT}")


hc_score = eval_metric(current_best_ensemble)
scores["hill_climbing"] = [hc_score] * 10
overall_scores["hill_climbing"] = hc_score
...
sub.prediction = current_best_test_preds
