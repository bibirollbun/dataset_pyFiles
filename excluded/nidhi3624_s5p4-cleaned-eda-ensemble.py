...
hgbr_mse = mean_squared_error(y_test, hgbr_y_pred)
hgbr_rmse = np.sqrt(hgbr_mse)

print("Hist Gradient Boosting Regressor:", hgbr_rmse)
...
xgbr_test_pred = xgbr_model.predict(final_test_features)
xgbr_sub = pd.DataFrame({"id":test.id, "Listening_Time_minutes":xgbr_test_pred})
xgbr_sub.to_csv("xgbr_submission.csv", index=False)


sub2 = pd.read_csv("/kaggle/working/xgbr_submission.csv")
sub2.head()
