...
EPOCHS = 25
fold_id = 0
LR = [3e-5, 3e-4] # large LR for neck
debug = False
validate = False
BATCH_SIZE = 64
...
if validate:
    ...
    valid_data_img = SHOPEEImageDataset(df_valid, img_dir, 
                                        transform = transforms, gen_feat_only = True)
    valid_dataloader_img = DataLoader(valid_data_img, batch_size=BATCH_SIZE, shuffle=False)
else:
    train_data_img = SHOPEEImageDataset(df, img_dir, transform = transforms)
    train_dataloader_img = DataLoader(train_data_img, batch_size=BATCH_SIZE, shuffle=True)


img_model = ImgEncoder(df.label_group.nunique(), 
                       backbone = img_backbone[NUM], 
                       embed_size = 1792,
                       scale = 32.0,
                       margin = 0.5,
                       permute = True)
...
optimizer = madgrad.MADGRAD([
    {'params': img_model.module.backbone.parameters(), 'lr': LR[0]},
    {'params': img_model.module.final_conv.parameters(), 'lr': LR[1]},
    {'params': img_model.module.gem.parameters(), 'lr': LR[1]},
    {'params': img_model.module.final.parameters(), 'lr': LR[1]},
], momentum=0.9, weight_decay=1e-5)
# scheduler = WarmupScheduler(optimizer, warmup_epochs=10, plateau_lr_bb=LR[0], 
#                             plateau_lr_neck=LR[1])
...
def train_func(dataloader):
    ...
    for batch_idx, (images, targets) in enumerate(bar):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(img_model.parameters(), max_norm=1.0)
        optimizer.step()
...
def find_threshold(df, FEAS, lower_count_thresh, upper_count_thresh, search_space):
    '''
...
def get_cv(dataloader):
    FEAS = gen_feas(dataloader)
...
#save img model
torch.save(img_model.state_dict(), f"img_model_{img_backbone[NUM].split('/')[-1]}.pth")
