...
# Placeholder for YatConv if not available
...
# --- NEW: Augmentation function for Denoising Autoencoder ---
@tf.function
def augment_for_pretraining(image: tf.Tensor) -> tf.Tensor:
    """Applies a series of strong augmentations to an image for pretraining."""
    # Color augmentations
    ...
    # Ensure pixel values are still in the [0, 1] range
...
class YatCNN(nnx.Module):
    ...
    def __init__(self, *, num_classes: int, input_channels: int, rngs: nnx.Rngs):
        self.conv1 = YatConv(input_channels, 32, kernel_size=(3, 3), rngs=rngs)
        self.conv2 = YatConv(32, 64, kernel_size=(3, 3), use_bias=False, rngs=rngs)
        self.conv3 = YatConv(64, 128, kernel_size=(3, 3), use_bias=False, rngs=rngs)
        self.conv4 = nnx.Conv(128, 256, kernel_size=(3, 3), use_bias=False, rngs=rngs)
        self.conv5 = nnx.Conv(256, 256, kernel_size=(3, 3), use_bias=False, rngs=rngs)
        ...
        self.dropout5 = nnx.Dropout(rate=0.3, rngs=rngs)
        self.out_linear = YatNMN(256, num_classes, use_bias=False, rngs=rngs)
    ...
    def __call__(self, x, training: bool = False, return_activations_for_layer: tp.Optional[str] = None, 
        ...
        x = self.dropout5(x, deterministic=not training)
...
class Decoder(nnx.Module):
    ...
    def __init__(self, *, output_channels: int, rngs: nnx.Rngs):
        self.deconv1 = YatConvTranspose(256, 128, kernel_size=(3, 3), strides=(2, 2), padding='SAME', rngs=rngs)
        self.deconv2 = YatConvTranspose(128, 64, kernel_size=(3, 3), strides=(2, 2), padding='SAME', rngs=rngs)
        self.deconv3 = YatConvTranspose(64, 32, kernel_size=(3, 3), strides=(2, 2), padding='SAME', rngs=rngs)
...
def autoencoder_loss_fn(model: ConvAutoencoder, batch):
    # The model receives the augmented image but is tasked with reconstructing the original.
    augmented_image = batch['augmented_image']
    original_image = batch['augmented_image']
...
def detailed_test_evaluation(model, test_ds_iter, class_names: list[str], model_name: str):
    ...
    print(f"\nğŸ”¬ Running detailed test evaluation for {model_name}...")
...
def visualize_tsne(encoder: YatCNN, dataset_iter, class_names: list[str], title: str, num_samples: int = 1000):
    ...
    print(f"\nğŸ�¨ Generating t-SNE plot: {title}...")
    ...
    for batch in tqdm(dataset_iter, desc="Extracting embeddings for t-SNE", total=int(np.ceil(num_samples / batch_size))):
        # For t-SNE, we use the clean image to get the representation
...
def visualize_reconstructions(autoencoder: ConvAutoencoder, dataset_iter, title: str, num_images: int = 8):
    """Visualizes original vs. reconstructed images from the denoising autoencoder."""
    print(f"\nğŸ–¼ï¸�  Generating image reconstructions: {title}...")
    ...
    # The batch now contains both original and augmented images
    original_images = batch['original_image'][:num_images]
    augmented_images = batch['augmented_image'][:num_images]
    
    # Get reconstructions from the augmented images
    ...
    for i in range(n):
        # Display augmented (input to model)
...
def _pretrain_autoencoder_loop(
    ...
    """Helper function to pretrain a model using a Denoising Autoencoder."""
    print(f"\nğŸš€ Starting Denoising Autoencoder Pretraining for {model_name} on {dataset_name}...")
    ...
    # Create the augmented dataset for the denoising task
    ...
    print(f"\nğŸ’¾ Saving pretrained ENCODER state to {save_dir}...")
    checkpointer.save(save_dir, encoder_state, force=True)
    
    # Create a fresh iterator from the augmented dataset for visualizations
    ...
    # Use the clean base dataset for t-SNE
...
def _train_model_loop(
    ...
    print(f"\nğŸš€ Initializing {model_name} for {stage} on dataset {dataset_name}...")
    ...
    if pretrained_encoder_path:
        print(f"ğŸ’¾ Loading pretrained encoder weights from {pretrained_encoder_path}...")
        checkpointer = orbax.PyTreeCheckpointer()
    ...
    checkpointer = orbax.PyTreeCheckpointer()
    print(f"ğŸ’¾ Saving final model state to {save_dir}...")
...
def run_training_and_analysis(
    ...
    if use_pretraining:
        print(f"\nğŸš€ STEP 1: Starting Autoencoder Pretraining...")
    ...
    print(f"\nğŸš€ STEP 2: {'Fine-tuning' if use_pretraining else 'Training'} Model...");
    ...
    print("\nğŸ“Š STEP 3: Running Final Analysis...")
...
def main():
    ...
    # Set to True to use autoencoder pretraining, False to train from scratch.
    use_pretraining = False
    
    # Set to True to freeze the encoder during fine-tuning (only if use_pretraining=True).
