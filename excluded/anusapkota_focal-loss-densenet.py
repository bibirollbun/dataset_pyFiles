...
def prepare_dataloaders(metadata_list, train_transforms, val_transforms, test_transforms, config, subset_fraction=1.0):
    ...
    if subset_fraction < 1.0:
        ...
        print(f"âš¡ Sampled {len(metadata_list)} samples for hyperparameter tuning.")
...
train_ds, val_ds, test_ds, train_loader, val_loader, test_loader = prepare_dataloaders(
    ...
    config=config,
    subset_fraction=0.3  # ğŸ‘ˆ Add this line to use 20% of the data for tuning
)
...
def print_class_distribution(metadata_list, target_cols):
    ...
    print("ğŸ“Š Class distribution in the current subset:")
    for col in target_cols:
        print(f"\nğŸ”¸ {col}")
...
# ğŸ”� Call this after dataloader prep:
print_class_distribution(train_ds.metadata_list, config.TARGET_COLS)
...
#         # â�¤ Split targets into dict of organ-specific outputs
#         targets_dict = {
#             "bowel": targets[:, 1].unsqueeze(1),                  # â�¤ Shape [B, 1]
#             "extra": targets[:, 3].unsqueeze(1),                  # â�¤ Shape [B, 1]
#             "liver": targets[:, 4:7].argmax(dim=1),               # â�¤ Shape [B]
#             "kidney": targets[:, 7:10].argmax(dim=1),             # â�¤ Shape [B]
#             "spleen": targets[:, 10:13].argmax(dim=1),            # â�¤ Shape [B]
#         }
...
def upload_to_kaggle_model(dataset_owner, dataset_slug, model_path, checkpoint_path=None, version_note=""):
    ...
    print(f"ğŸ“¦ Zipped model(s) to: {zip_path}")
    ...
    print(f"âœ… Created metadata file at {metadata_path}")
    ...
    print(f"âœ… Uploaded to Kaggle Dataset: {dataset_owner}/{dataset_slug}")
...
def extract_checkpoint_from_dataset(dataset_input_path="/kaggle/input/densenet-training-model-arch-focal-loss",
    ...
    if os.path.exists(checkpoint_src):
        ...
        print(f"âœ… Copied checkpoint.pth to {extract_path}")
        copied = True
    else:
        print("âš ï¸� checkpoint.pth not found in dataset input.")

    if os.path.exists(best_model_src):
        shutil.copy(best_model_src, os.path.join(extract_path, "model_best.pth"))
        print(f"âœ… Copied model_best.pth to {extract_path}")
        copied = True
    else:
        print("âš ï¸� model_best.pth not found in dataset input.")
