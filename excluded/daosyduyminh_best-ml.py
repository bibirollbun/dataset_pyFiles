import os, re, math, gc, sys, random
import pandas as pd
from collections import defaultdict, Counter
from glob import glob
import unicodedata  # Để chuẩn hóa Unicode
import pickle       # [MỚI] Thêm để lưu/tải model
...
def init_from_dice(dice_by_src, top_k=40):
    t_given_s = {}
...
def ibm1_em_with_diag(pairs, t_given_s, iters=6, floor=1e-9, lambda_diag=4.0):
...
def symmetrize(t_ab, t_ba, thresh=1e-7):
    lex = {}
...
def build_lexicon_ibm1_diag(pairs, dice_top=40, em_iter=6, min_count=2, lambda_diag=4.0):
...
def train_word_trigram(sentences, add_k=0.1):
...
def create_lm_score_fn(lm_data):
    if lm_data is None:
    ...
    def score(next_word, prev2, prev1):
        ctx = (prev2, prev1)
...
def get_char_ngrams(s: str, n: int) -> Counter:
    return Counter(s[i:i+n] for i in range(len(s) - n + 1))

def sentence_chrf(hyp_tokens, ref_tokens, n_char=6, beta=3):
...
def get_word_ngrams(tokens: list, n: int) -> Counter:
    return Counter(tuple(tokens[i:i+n]) for i in range(len(tokens) - n + 1))

def sentence_bleu_smooth1(hyp_tokens, ref_tokens, max_n=4):
    log_bleu = 0.0
...
def utility_gain(hyp_toks, ref_toks):
...
# --- N-best Beam Decoder ---
# ---
def beam_decode_n_best(src_tokens, lex_prob, lm_score_fn, beam_size=10, lm_w=0.85,
...
class SMT_Model:
    ...
    def train(self, bitext_by_dir, params_fwd, params_rev):
    ...
    def _translate_for_backtrans(self, s, src_lang, tgt_lang, params):
    ...
    def translate_sentence(self, s, src_lang, tgt_lang):
