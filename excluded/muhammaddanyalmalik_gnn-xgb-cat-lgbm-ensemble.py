...
MODEL_DIR1 = '/kaggle/input/neurips-2025/cat_v5'
train_or_predict(train_model=False, model_dir=MODEL_DIR1)
print(output_df)
output_dfs.append(output_df.copy())
...
MODEL_DIR1 = '/kaggle/input/neurips-2025/lgbm_v6'
train_or_predict(train_model=False, model_dir=MODEL_DIR1)
print(output_df)
output_dfs.append(output_df.copy())
...
def augment_dataset(X, y, n_samples=1200, n_components=5, random_state=None):
    """
    Augments a dataset using Gaussian Mixture Models.
...
def predict_with_gnn(trained_model, test_smiles):
    ...
    test_loader = PyGDataLoader(valid_test_data, batch_size=64, shuffle=False)
...
class TaskSpecificGNN(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_global_features, 
                 hidden_channels_gnn, mlp_neurons, mlp_dropouts):
        super().__init__()
        torch.manual_seed(42)
        
        # --- 1. GNN Backbone (ALL BatchNorm1d REMOVED) ---
        
        nn1 = torch.nn.Sequential(
            torch.nn.Linear(num_node_features, hidden_channels_gnn), 
            torch.nn.ReLU(), 
        )
        self.conv1 = GINEConv(nn1, edge_dim=num_edge_features)

        nn2 = torch.nn.Sequential(
            torch.nn.Linear(hidden_channels_gnn, hidden_channels_gnn * 2), 
            torch.nn.ReLU(), 
        )
        self.conv2 = GINEConv(nn2, edge_dim=num_edge_features)
        
        nn3 = torch.nn.Sequential(
            torch.nn.Linear(hidden_channels_gnn * 2, hidden_channels_gnn * 4), 
            torch.nn.ReLU(), 
        )
...
def train_gnn_model(label, train_data_list, val_data_list, mlp_neurons, mlp_dropouts, epochs=300): # Increased default epochs
    ...
    best_val_loss = float('inf')
    epochs_no_improve = 0
    PATIENCE_EPOCHS = 40  # Stop training if val loss doesn't improve for 30 straight epochs
...
def train_or_predict_gnn(train_model=True, model_dir="models/gnn", n_splits=5):
    """
        ...
        if train_model:
            ...
            for fold, (train_idx, val_idx) in enumerate(fold_indices_gen):
                ...
                fold_model = train_gnn_model(
                    ...
                    epochs=450       # <-- Train longer (will stop early)
                )
            ...
            model_path = '/kaggle/input/neurips-2025/gnn_gh_v19/models/gnn/'
            try:
