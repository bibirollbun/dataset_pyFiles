...
cfg.early_stopping = {"patience": 200, "streak": 0}
...
class CustomDataset(torch.utils.data.Dataset):
    ...
    def load_metadata(self, ):
        ...
        # Select rows
        df= pd.read_csv("combined_folds.csv")
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), disable=self.cfg.local_rank != 0):
            ...
            p1 = os.path.join("/home/OpenFWI_float32", row["data_fpath"])
            p2 = os.path.join("/home/OpenFWI_float32", row["data_fpath"].split("/")[0], "*", row["data_fpath"].split("/")[-1])
...
def main(cfg):
    ...
    train_dl = torch.utils.data.DataLoader(
        ...
        num_workers=max(1, os.cpu_count() // cfg.world_size - 4),
    )
    ...
    state_dict= torch.load("/home/pretrained/best_model_123.pt", map_location=cfg.device, weights_only=True)
    ...
    scheduler = ConstantCosineLR(
        ...
        pct_cosine=0.9,  # or any value you choose (e.g., 0.3, 0.8)
    )
    ...
    for epoch in range(0, cfg.epochs+1):
        if epoch != 0:
            ...
            for i, (x, y) in enumerate(train_dl):
                ...
                if cfg.local_rank == 0 and (len(total_loss) >= cfg.logging_steps or i == 0):
                    ...
                    print("Epoch {}:     Train MAE: {:.2f}     Val MAE: {:.2f}     LR: {}     Time: {}     Step: {}/{}".format(
                        epoch,
                        train_loss,
                        val_loss,
                        current_lr,
        ...
        if cfg.local_rank == 0:
            ...
            print("Saved weights latest..")
            if ema_model is not None:
                torch.save(ema_model.module.state_dict(), f'latest/best_model_{cfg.seed}.pt')
            else:
                torch.save(model.state_dict(), f'latest/best_model_{cfg.seed}.pt')
...
if RUN_TRAIN:
    ...
    !OMP_NUM_THREADS=1 torchrun --nproc_per_node=8 _train.py
...
if RUN_VALID or RUN_TEST:
    ...
    models = []
    for f in sorted(glob.glob("/kaggle/input/gwi-cycle6/best_model_123.pt")):
        print("Loading: ", f)
        m = Net(
            backbone=cfg.backbone,
            pretrained=False,
