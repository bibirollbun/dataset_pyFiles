...
test_dir = '../data/test'  # 画像が入っているフォルダ
test_list = glob.glob(test_dir + '/*.jpg')  # 拡張子が .jpg の画像を取得

print(f"画像数: {len(test_list)}")
print(test_list[:5])
...
with torch.no_grad():
    for test_path in tqdm(test_list):
        ...
        _id = int(test_path.split('/')[-1].split('.')[0])  # 画像名から ID を取得

        img = transform(img, phase='val')  # 前処理
        img = img.unsqueeze(0).to(device)  # バッチ化してGPUへ

        outputs = net(img)
        preds = F.softmax(outputs, dim=1)[:, 1].tolist()  # 犬(1)の確率
