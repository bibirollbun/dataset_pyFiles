...
class PawpularModel(nn.Module):
    def __init__(self):
        super().__init__()        
        # Using EfficientNet-B3 for better performance
        self.model = timm.create_model("tf_efficientnet_b3_ns", pretrained=True, in_chans=3, num_classes=1000)
    ...
    def optimizer_scheduler(self):
        opt = torch.optim.AdamW(self.parameters(), lr=3e-05, weight_decay=0.01)  # Slightly increased LR
...
df = pd.read_csv("/kaggle/input/bhargav-m-23bcs10140-kfold-paw/train_5folds.csv")
df.head()


# Training
print(f'Training model start')
args.fold = 0  # Using only fold 0
...
es = EarlyStopping(
    monitor="valid_rmse",
    model_path="model.bin",  # Single model output
    patience=4,#3,
    mode="min",
