...
def build_sequences(df: pd.DataFrame, seq_col: str, target_col: str, feature_cols: List[str],
    ...
    for seq, g in df.groupby(seq_col):
        ...
        labels.append(g[target_col].iloc[0])
    ...
    X = np.stack(grouped)                              # [N, T, C]
    le = LabelEncoder()
    y = le.fit_transform(np.asarray(labels).astype(str))
    return X, y, le

# ---------- 4) Metrics (binary or multiclass) ----------

# --- Define class names (danh sÃ¡ch tÃªn nhÃ£n trong dataset) ---
CLASS_NAMES = [
...
def run_length_sweep(train_df: pd.DataFrame,
                     ...
                     results_csv: str = "/kaggle/working/transformer_results.csv",
    ...
    for L in lengths:
        ...
        num_classes = len(np.unique(y)); C = X.shape[2]
...
# 2) LÆ°u danh sÃ¡ch gesture classes
        np.save("/kaggle/working/gesture_classes.npy", le.classes_)
...
from nb3_models import TransformerEncoderModel, TSTModel, InformerLite, TCN
import numpy as np
...
NOTEBOOK_NAME = "NB3-1 TransformerEncoder"

# Cháº¡y cáº£ 2 cháº¿ Ä‘á»™: IMU-only vÃ  Full
run_full_evaluation(
    train_df=train_df,
    model_builder=MODEL_BUILDER,
    model_name=MODEL_NAME,
    ...
    amp=True,
    runtime_prefix="mp01",
    results_csv="/kaggle/working/transformer_results.csv",
    max_train_sequences=1000,
    max_val_sequences=300
)
...
csv_path = "/kaggle/working/transformer_results.csv"
...
CHECKPOINT = "/kaggle/working/TransformerEncoder_L100.pth"
...
# ----------------- CREATE SUBMISSION -----------------
df_sub = pd.DataFrame(records, columns=["sequence_id", "behavior"])
