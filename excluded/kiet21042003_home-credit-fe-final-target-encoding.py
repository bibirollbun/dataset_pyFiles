...
# --- Identify Top 150 Features ---
# Ensure 'feat_importance_df' from your multi-round training (or a similar importance df from CV) is available
# This DataFrame should have 'Feature Name' and 'Importance Value' (averaged over rounds/folds)

if 'feat_importance_df' in locals() and not feat_importance_df.empty:
    print("Identifying top 150 features...")
    # Calculate mean importance if it's not already done (e.g., if feat_importance_df has raw fold/round importances)
    if 'mean_importance' not in feat_importance_df.columns and 'Importance Value' in feat_importance_df.columns and 'Feature Name' in feat_importance_df.columns:
        # If feat_importance_df has 'Importance Value' and 'Feature Name' and possibly 'Round' or 'Fold'
        # We need to group by 'Feature Name' and take the mean of 'Importance Value'
        mean_importances = feat_importance_df.groupby('Feature Name')['Importance Value'].mean().sort_values(ascending=False)
    elif 'mean_importance' in feat_importance_df.columns: # If you already calculated mean_importance
        mean_importances = feat_importance_df.set_index('Feature Name')['mean_importance'].sort_values(ascending=False)
    else: # Fallback if using a simple importance structure
        # Assuming feat_importance_df has 'Feature Name' as index and one importance column
        # Or 'Feature Name' and 'Importance Value' columns from a single run
        if 'Feature Name' in feat_importance_df.columns and 'Importance Value' in feat_importance_df.columns:
             mean_importances = feat_importance_df.set_index('Feature Name')['Importance Value'].sort_values(ascending=False)
        elif not feat_importance_df.index.name == 'Feature Name' and feat_importance_df.shape[1] == 1: # Assuming index is feature name
             mean_importances = feat_importance_df.iloc[:, 0].sort_values(ascending=False)
        else:
             print("ERROR: Cannot determine feature importances correctly from feat_importance_df structure.")
             mean_importances = None


    if mean_importances is not None and not mean_importances.empty:
        N_TOP_FEATURES = 150
        top_150_features = mean_importances.head(N_TOP_FEATURES).index.tolist()
        print(f"Identified top {len(top_150_features)} features.")
        # print("Top features selected:", top_150_features[:10]) # Print a sample

        # Ensure these top features are present in the original X_train and X_test
        # (X_train and X_test should be the versions AFTER all FE and sanitization, before target encoding if you separated that)
        if 'X_train' in locals() and 'X_test' in locals():
            X_train_top150 = X_train[top_150_features].copy()
            X_test_top150 = X_test[top_150_features].copy()
            print(f"Created X_train_top150 with shape: {X_train_top150.shape}")
            print(f"Created X_test_top150 with shape: {X_test_top150.shape}")
            top_features_prepared = True
        else:
            print("ERROR: X_train or X_test not available to create top feature subsets.")
            top_features_prepared = False
    else:
        print("ERROR: Mean feature importances could not be determined. Cannot select top features.")
        top_features_prepared = False
else:
    print("ERROR: feat_importance_df not found or empty. Cannot select top features.")
    top_features_prepared = False


# --- Train Model with Top 150 Features ---
# (This reuses your existing training loop structure from "III. Generating Test Set Predictions")

if top_features_prepared and 'y_train' in locals() and 'params' in locals() \
   and 'NUM_TRAIN_ROUNDS' in locals() and 'SEED_VALUES' in locals() \
   and 'MAX_BOOST_ROUNDS' in locals():

    print("\n--- Training LightGBM with Top 150 Features ---")

    # Identify categorical features WITHIN the top 150 set
    # Assuming `final_categorical_for_lgbm` is the list of ALL categoricals before selection
    categorical_top150 = [col for col in final_categorical_for_lgbm if col in X_train_top150.columns]
    print(f"Using {len(categorical_top150)} categorical features within the top 150 set.")

    params_top150 = params.copy()
    params_top150['categorical_feature'] = categorical_top150

    y_score_list_top150 = []
    # feat_importance_df_top150 = pd.DataFrame() # Optional: if you want to store importances for this model too

    for i in range(NUM_TRAIN_ROUNDS): # NUM_TRAIN_ROUNDS from your notebook
        print(f'Begin Training Round {i + 1}/{NUM_TRAIN_ROUNDS} for Top 150 Features.')
        params_top150['seed'] = SEED_VALUES[i]

        # Ensure your train_lgbm function can handle the 'categorical_feature' in params
        # If train_lgbm re-creates lgb.Dataset, it should pick up params['categorical_feature']
        # If X_train_top150 categoricals are already 'category' dtype, LightGBM might auto-detect too.
        
        # If your train_lgbm function doesn't use params['categorical_feature'] but expects it in lgb.Dataset:
        # lgb_train_top150 = lgb.Dataset(X_train_top150, label=y_train, categorical_feature=categorical_top150)
        # clf_lgb_top150 = lgb.train(params_top150, lgb_train_top150, num_boost_round=MAX_BOOST_ROUNDS)
        # ELSE, if train_lgbm takes params directly:
        clf_lgb_top150 = train_lgbm(params_top150, X_train_top150, y_train, # Use your existing train_lgbm
                                    X_val=None, y_val=None, for_cv=False,
                                    max_boost_rounds=MAX_BOOST_ROUNDS, early_stop_rounds=None)


        clf_y_score_top150 = clf_lgb_top150.predict(X_test_top150)
        y_score_list_top150.append(pd.Series(clf_y_score_top150, name=f'ROUND_{i+1}_TOP150'))
        print(f'Training Round {i + 1}/{NUM_TRAIN_ROUNDS} for Top 150 Features complete.')

    # Blend predictions for the top 150 model
    if y_score_list_top150:
        y_scores_top150_df = pd.concat(y_score_list_top150, axis=1)
        # Ensure mean_rank_predictions function is defined
        if 'mean_rank_predictions' in locals():
            final_predictions_top150 = mean_rank_predictions(y_scores_top150_df)
            print("\nPredictions from Top 150 Features model blended.")
            predictions_top150_generated = True
        else:
            print("ERROR: mean_rank_predictions function not defined. Cannot blend top 150 predictions.")
            predictions_top150_generated = False
    else:
        print("ERROR: No predictions generated for top 150 features model.")
        predictions_top150_generated = False
else:
    print("Skipping training with top 150 features as prerequisites not met.")
    predictions_top150_generated = False


# --- Generate Submission for Top 150 Model ---
if predictions_top150_generated and 'X_test_borrower_IDs' in locals():
    print("\nGenerating Kaggle submission file for Top 150 Features model...")
    # Ensure VERSION_TOP150 is defined if using your write_pred_to_csv function
    VERSION_TOP150 = 'top150_features' # Example version name
    
    submission_df_top150 = pd.DataFrame({
        'SK_ID_CURR': X_test_borrower_IDs.values,
        'TARGET': final_predictions_top150
    })
    submission_filename_top150 = os.path.join(OUTPUT_DIR, f'kaggle_home_credit_submission_{VERSION_TOP150}.csv') # Save in main output
    submission_df_top150.to_csv(submission_filename_top150, index=False)
    print(f"Submission file for Top 150 Features model saved to: {submission_filename_top150}")
else:
    print("Skipping submission file generation for Top 150 Features model as predictions not available.")

# --- Optional: Save the Top 150 Features Model ---
if 'clf_lgb_top150' in locals() and predictions_top150_generated : # Save the model from the last round
    model_filename_top150 = os.path.join(MODEL_DIR, 'lgbm_top150_features_model.txt') # Save in specific model dir
    try:
        clf_lgb_top150.save_model(model_filename_top150)
        print(f"Model trained on Top 150 features saved to: {model_filename_top150}")
    except Exception as e:
        print(f"Error saving Top 150 features model: {e}")

print("\n--- Processing for Top 150 Features model complete. ---")
gc.collect()
