...
yolo_dataset_dir = "/kaggle/working"
yolo_images_train = os.path.join(yolo_dataset_dir, "train")
yolo_images_val = os.path.join(yolo_dataset_dir, "val")
...
BOX_SIZE = 250  # Bounding box size for annotations (in pixels)
...
def prepare_yolo_dataset(trust=TRUST, train_split=TRAIN_SPLIT):
    ...
    def process_tomogram_set(tomogram_ids, images_dir, labels_dir, set_name):
        ...
        ID, Z, X, Y, W, H, labels, file_names = [], [], [], [], [], [], [], []
        ...
        for tomo_id, z_center, y_center, x_center, z_max in tqdm(motor_counts, desc=f"Processing {set_name} motors"):
            ...
            for z in range(z_min, z_max + 1):
                ...
                x_center_norm = x_center / img_width if not (z_center==-1 and y_center==-1 and x_center==-1) else -1
                y_center_norm = y_center / img_height if not (z_center==-1 and y_center==-1 and x_center==-1) else -1
                box_width_norm = BOX_SIZE / img_width if not (z_center==-1 and y_center==-1 and x_center==-1) else -1
                box_height_norm = BOX_SIZE / img_height if not (z_center==-1 and y_center==-1 and x_center==-1) else -1

                ID.append(tomo_id)
                Z.append(z)
                
                X.append(x_center_norm)
                Y.append(y_center_norm)
                W.append(box_width_norm)
                H.append(box_height_norm)
                ...
                labels.append(int(not (z_center==-1 and y_center==-1 and x_center==-1)))
                file_names.append(dest_filename)
                
                # Write label file
                # label_path = os.path.join(labels_dir, dest_filename.replace('.jpg', '.csv'))
    ...
    train_slices, train_motors = process_tomogram_set(train_tomos, yolo_images_train, yolo_labels_train, "train")
    
    # Process validation tomograms
    val_slices, val_motors = process_tomogram_set(val_tomos, yolo_images_val, yolo_labels_val, "val")
...
source_dir = '/kaggle/input/byu-locating-bacterial-flagellar-motors-2025/test'
...
# Đường dẫn của thư mục nguồn và thư mục đích
source_dir = '/kaggle/input/byu-locating-bacterial-flagellar-motors-2025/sample_submission.csv'
destination_dir = '/kaggle/working/sample_submission.csv'
