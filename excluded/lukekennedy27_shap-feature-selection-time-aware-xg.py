...
base_features1 = list(dict.fromkeys([
...
split_index = int(len(x_train) * 0.76)
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    time_decay = create_time_decay_weights(len(train_idx), decay=0.9)#decay=0.95
...
plt.plot(y_val, label='True Values', linewidth=2)
plt.plot(y_val_pred, label='Predicted Values', linewidth=2, alpha=0.7)
plt.title(f'Predictions vs Ground Truth on Validation Set\nPearson Correlation: {pearson_corr:.4f}')
