...
class CustomModel(nn.Module):
    def __init__(self):
        ...
        self.model.fc = nn.Identity()  # Remove final layer - IMPORTANT!
        
        self.dropout = nn.Dropout(0.5)
        self.out = nn.Linear(2048, 1)  # Match training: 2048 features
    ...
    def forward(self, image, features, targets=None):
        ...
        if targets is not None:
            loss = nn.BCEWithLogitsLoss()(x.view(-1), targets)
            return x, loss, {}
...
model = CustomModel()
model = Tez(model)
model.load(f"/kaggle/input/gowtham-23bcs10168-train-ml-project/model_f0.bin", weights_only=True)
...
test_dataset = CustomDataset(
    ...
    dense_features=df_test[dense_features].values if dense_features else np.zeros((len(test_img_paths), 0)),
