...
class CustomDataset(Dataset):
    ...
    def apply_augmentation(self, image):
        augmentations = [
            lambda img: np.rot90(img, k=np.random.randint(1, 3)),  # Random rotation
            lambda img: self.center_crop(img),  # Random crop
            lambda img: self.histogram_equalization(img),  # Histogram Equalization
            lambda img: self.add_rician_noise(img),  # Rician Noise
        ]
    ...
    def center_crop(self, image, crop_size=(224, 224)):
        """
        Görüntünün merkezinden belirli bir boyutta kırpma yapar.

        Parametreler:
            image (numpy.ndarray): Girdi görüntüsü.
            crop_size (tuple): Kırpılacak yükseklik ve genişlik (h, w).

        Dönüş:
            numpy.ndarray: Merkezden kırpılmış görüntü.
        """
        h, w = image.shape
        new_h, new_w = crop_size

        # Görüntü boyutlarının, kırpma boyutundan küçük olma durumlarını ele al
        if h < new_h or w < new_w:
            raise ValueError("Kırpma boyutu, görüntü boyutundan büyük olamaz.")
    ...
    def histogram_equalization(self, image):
        """
        Girdi bir görüntüyü histogram eşitleme ile iyileştirir.
        
        Parametre:
            image (numpy.ndarray): Gri tonlamalı veya renkli görüntü.
            
        Dönüş:
            numpy.ndarray: Histogram eşitlenmiş görüntü.
        """
        # Eğer gri tonlamalı görüntü ise
        if len(image.shape) == 2:
            return cv2.equalizeHist(image)
        # Eğer renkli görüntü ise
        elif len(image.shape) == 3:
            # Renkli görüntüyü YCrCb renk uzayına çevir
            ycrcb = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb)
            # Y kanalı üzerinde histogram eşitleme uygula
            ycrcb[:, :, 0] = cv2.equalizeHist(ycrcb[:, :, 0])
            # Görüntüyü tekrar BGR renk uzayına çevir
            return cv2.cvtColor(ycrcb, cv2.COLOR_YCrCb2BGR)
        else:
            raise ValueError("Girdi görüntü 2D veya 3D olmalıdır.")

    # Rician noise ekleme
    def add_rician_noise(self, image, scale=0.1):
        """
        Görüntüye Rician gürültüsü ekler.

        Parametreler:
            image (numpy.ndarray): Girdi görüntüsü.
            scale (float): Gürültü ölçeği (standart sapma).

        Dönüş:
            numpy.ndarray: Gürültü eklenmiş görüntü.
        """
        noise1 = np.random.normal(0, scale, image.shape)
        noise2 = np.random.normal(0, scale, image.shape)
        rician_noise = np.sqrt((image + noise1)**2 + noise2**2)
        return np.clip(rician_noise, 0, 255)  # 0-255 arasında tut
...
class CustomResNet50(nn.Module):
    def __init__(self, num_classes=3, pretrained_weights=None):
        ...
        if pretrained_weights:
            ...
            state_dict = {k.replace('model.', ''): v for k, v in state_dict.items()}  # Katman adı eşleşmesi
            self.model.load_state_dict(state_dict, strict=False)  # strict=False ile uyumsuz katmanlar atlanır
...
# Optimizer ayarları
optimizer_sagittal_t1 = torch.optim.Adam(sagittal_t1_model.parameters(), lr=0.0005)
...
for desc, model in models_dict.items():
    ...
    train_model(model, trainloader, valloader, len_train, len_val, optimizers_dict[desc])
...
def save_models_and_optimizers(models_dict, optimizers_dict, folder_path="/kaggle/working/"):
    """
    Mevcut dosyaları alıp, modelleri ve optimizörleri kaydeder.

    Parametreler:
        models_dict (dict): {'model_adi': model} şeklinde modellerin dictionary'si.
        optimizers_dict (dict): {'model_adi': optimizer} şeklinde optimizörlerin dictionary'si.
        folder_path (str): Kaydedilecek dosyaların bulunduğu klasör.
    """
    checkpoint = {}

    # Çalışma dizinindeki .pth dosyalarını al
    existing_files = [f for f in os.listdir(folder_path) if f.endswith('.pth')]

    # Mevcut dosyaların adlarına göre modelleri kaydet
    for model_name, model in models_dict.items():
        model_filename = f"{model_name}_state_dict.pth"
        if model_filename in existing_files:
            model_filename = f"{model_name}_state_dict_{len(existing_files)}.pth"
        checkpoint[model_filename] = model.state_dict()

    for optimizer_name, optimizer in optimizers_dict.items():
        optimizer_filename = f"{optimizer_name}_optimizer_state_dict.pth"
        if optimizer_filename in existing_files:
            optimizer_filename = f"{optimizer_name}_optimizer_state_dict_{len(existing_files)}.pth"
        checkpoint[optimizer_filename] = optimizer.state_dict()

    # Kaydet
    torch.save(checkpoint, os.path.join(folder_path, "trained_models_and_optimizers.pth"))
    print(f"Models and optimizers saved to {folder_path}/trained_models_and_optimizers.pth")




save_models_and_optimizers(models_dict, optimizers_dict, folder_path)
