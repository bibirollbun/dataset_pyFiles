...
def fpr_experiment():
    ...
    n0 = 100
    true_beta_list = [0.25]
    coeff = {0.25: 20, 0.5: 20, 0.75: 20, 1: 20}
    M = 2
    K = 3
    r = 10
    n_list = [300, 400, 500, 100]
    ...
    for true_beta in true_beta_list:
        ...
        while trial < num_trials and time.time() - start_ < TOTAL :
            ...
            lambdas_w = [np.sqrt(5 * np.log(p) / nk)  for nk in n_list[:K]]
            lambdas_del = [np.sqrt(np.log(p) / n0) ] * K
            lambda_c = np.sqrt(np.log(p) / n_tilde) * coeff[true_beta]
        ...
        if oc_total_false_positives_detected > 0:
            ...
            oc_fpr = 0
        print (f"Trial = {trial}")
        print(f'true_beta = {true_beta}, FPR = {fpr:.4f} - FPR OC = {oc_fpr:.4f}')
        print (f'num err = {num_err}')

        fpr_values[true_beta] = fpr
        oc_fpr_values[true_beta] = oc_fpr
        print ("________________________________________________________")
    return fpr_values, oc_fpr_values
