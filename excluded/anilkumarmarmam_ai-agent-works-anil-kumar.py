...
#---------------
class MockLLMAgent:
    '''A moked LLM agent that 'generates' summaries, explantions, quiz questons and answes queries.
    In a real system this would cal an LLM API with prompt engineering and safety checks.
    '''
    ...
    def explain(self, topic: str, level: str = 'intro') -> str:
        return f"Explanation ({level}) for {topic}: This is a concise, learner-friendly explanation convering the main ideas, examples, and intuition."
    ...
    def answer_query(self, topic: str, query_type: str) -> str:
        """return short, informative ansers for a set of predefined query types."""
        ...
        if query_type == 'key_concepts':
            return (f"Key concepts for{topic}: core definitions, main algrithms, complexity trade-offs,"
                   "common failure modes, and real-world examples.")
        if query_type == 'practice_problems':
            return (f"Practice ideas for {topic}: implement a small example form scratch, slolve 3 conceptual problems,"
                   "and reproduce one result from a short tutorial.")
        #defaut fallback
        return f"Short guidance for {topic} regarding '{query_type}': try learning by building a smal project."
...
class ContentSearchAgent:
    def run(self, topic: str) -> List[Resource]:
        ...
        time.sleep(0.4)
        # Use mr Anil kumar.
        playlist = 'https://youtube.com/shorts/JmmPMoQme_Y'
        return [
            Resource(
                title=f'{topic} - mr Anil kumar: AI Agent short (playlist)',
                url = playlist,
                summary = f'A curated playlist covering {topic} as part of the 5-day AI Agent Intesive by mr anil kumar.',
                relevance=0.95
            ),
            Resource(
                title=f'{topic} -Deep dive (article / notes)',
                url= 'https://example.com/deep',
                sumary=f'In-depth article or notes for {topic}.',
                relevance=0.75
            )
...
class StudyCoordinator:
    ...
    def plan_for(self, session_id: str, topics: List[str], weekly_hours: int = 5) -> Dict [str, Any]:
        ...
        # Run content searc and flashcard generation in parallel for each topic
        #results = {'resources': {}, 'flashcards': {}, 'quizzes': {}, "informative_queries": {}}
        #with concurrent.futures.ThredPoolExecutor(max_workers=6) as ex:
...
#  Demo run (usnig 5 day AI Agnet insentive topics)
#--------------------------------------------------
session_service = InMemorySessionService()
coord = StudyCoordinator(session_service)

ani = coord.create_or_get_session(user_id='user_123', metadata = {'preferred_format': 'videos+problems'})
print("Created session id:", ani)

# use 5 topics representig the AI agent intensive by mr anil kumar
topics = [
...
plan_result = coord.plan_for(ani, topics, weekly_hours=10)
...
print('\nSession stored state keys:', list (session_service.get(ani).get('state', {}).keys()))
