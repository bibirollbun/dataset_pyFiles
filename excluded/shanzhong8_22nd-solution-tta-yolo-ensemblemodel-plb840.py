...
try: 
    ...
    !pip install /kaggle/input/ultralytics418/ultralytics-8.3.71-py3-none-any.whl
...
model_path = "/kaggle/input/yolo-box25/epoch180.pt"

# Detection parameters
CONFIDENCE_THRESHOLD = 0.66  # Lower threshold to catch more potential motors
...
PP1 = '/kaggle/working/csv0.csv'
PP2 = '/kaggle/working/csv1.csv'
def merge_dataframes(dataframe1, dataframe2, output_path):
    results = []

    for identifier in dataframe1['tomo_id']:
        entry1 = dataframe1.loc[dataframe1['tomo_id'] == identifier].iloc[0]
        entry2 = dataframe2.loc[dataframe2['tomo_id'] == identifier].iloc[0]

        selected_entry = (entry1 if entry1['select'] > entry2['select'] 
                          else entry2)

        merged_row = {
            'tomo_id': identifier,
            **{f'Motor axis {i}': selected_entry[f'Motor axis {i}'] for i in range(3)},
            'select': selected_entry['select']
        }
        
        results.append(merged_row)

    final_df = pd.DataFrame(results)
    final_df.drop(columns='select', inplace=True)
...
def prosse():
    df1 = pd.read_csv(PP1)
    df2 = pd.read_csv(PP2)
    merged_df = merge_dataframes(df1, df2, '/kaggle/working/submission.csv')
    return merged_df
...
def process_tomogram(tomo_id, model, CONFIDENCE_THRESHOLD,CONCENTRATION,index=0, total=1):
    """
    Process a single tomogram and return the most confident motor detection
    """
    # print(f"Processing tomogram {tomo_id} ({index}/{total})")
    ...
    return {
        ...
        'Motor axis 1': round(best_detection['y']),
        'Motor axis 2': round(best_detection['x']),
        'select':best_detection['confidence']
    }
...
def generate_submission(model_path,device,out_path,CONFIDENCE_THRESHOLD,CONCENTRATION,):
    """
    Main function to generate the submission file
    """
    test_tomos = sorted([d for d in os.listdir(test_dir) if os.path.isdir(os.path.join(test_dir, d))])
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        # Submit all tomograms for processing
        for i, tomo_id in enumerate(test_tomos, 1):
            future = executor.submit(process_tomogram, tomo_id, model,CONFIDENCE_THRESHOLD,CONCENTRATION, i, total_tomos)
            future_to_tomo[future] = tomo_id
    ...
    # Ensure proper column order
    submission_df = submission_df[['tomo_id', 'Motor axis 0', 'Motor axis 1', 'Motor axis 2','select']]
    
    # Save the submission file
    submission_df.to_csv(f'/kaggle/working/{out_path}', index=False)
    print("="*50)
...
CONFIDENCE_THRESHOLD0 = 0.66
CONFIDENCE_THRESHOLD1 = 0.85

CONCENTRATION0 = 1
CONCENTRATION1 = 1


model_path0 = "/kaggle/input/yolo-box25/epoch180.pt"
model_path1 = "/kaggle/input/yolov8m-pseudo/best_epoch_128_R944_P933.pt" #"/kaggle/input/yolov8m-pseudo/best_epoch_128_R944_P933.pt"
...
def run_submission(model_path, device, out_path):
    generate_submission(model_path, device, out_path)

with ThreadPoolExecutor(max_workers=4) as executor:
    print('OK')
    inputs1 = [
        (model_path0, device0, out_path0, CONFIDENCE_THRESHOLD0, CONCENTRATION0),
    ]
    inputs2 = [
        (model_path1, device1, out_path1, CONFIDENCE_THRESHOLD1, CONCENTRATION1),
    ]

    # 提交两个任务组
    futures1 = [executor.submit(generate_submission, *args) for args in inputs1]
    futures2 = [executor.submit(generate_submission, *args) for args in inputs2]

    # 等待所有任务完成
    for future in futures1 + futures2:
        future.result()  # 会阻塞直到对应任务执行完成

print("finished")
pd.read_csv("/kaggle/working/csv1.csv")
# 条件：select > 95 的行
mask = df['select'] > 95
# 将满足条件的行的这四列设为 -1
df.loc[mask, ['Motor axis 0', 'Motor axis 1', 'Motor axis 2', 'select']] = -1

# 保存修改后的文件（可选）
df.to_csv('csv1.csv', index=False)  # 替换为你想保存的路径
