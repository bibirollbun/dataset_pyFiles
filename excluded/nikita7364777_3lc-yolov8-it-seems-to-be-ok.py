...
the_3lc_cotton_weed_detection_challenge_path = kagglehub.competition_download('the-3lc-cotton-weed-detection-challenge')
print('Data source import complete.')


! pip install 3lc_ultralytics
...
# Set up file paths
WORK_DIR = Path("/root/.cache/kagglehub/competitions/the-3lc-cotton-weed-detection-challenge/cotton_weed_competition_dataset")
...
DATASET_NAME = "cotton_weed_detection"
...
try:
    ...
    existing_train = tlc.Table.from_names(project_name = PROJECT_NAME,
                                          dataset_name = DATASET_NAME,
                                          table_name   = f"{DATASET_NAME}-train1",
                                         )
    existing_val = tlc.Table.from_names(project_name = PROJECT_NAME,
                                        dataset_name = DATASET_NAME,
    ...
    train_table = tlc.Table.from_yolo(dataset_yaml_file = str(DATASET_YAML),
                                      split             = "train",
                                      task              = "detect",
                                      dataset_name      = DATASET_NAME,
    ...
    val_table = tlc.Table.from_yolo(dataset_yaml_file = str(DATASET_YAML),
                                    split             = "val",
                                    task              = "detect",
                                    dataset_name      = DATASET_NAME,
                                    project_name      = PROJECT_NAME,
...
try:
    ...
    train_table_latest = tlc.Table.from_names(project_name = PROJECT_NAME,
                                              dataset_name = DATASET_NAME,
                                              table_name   = f"{DATASET_NAME}-train1",
                                             ).latest()

    val_table_latest = tlc.Table.from_names(project_name = PROJECT_NAME,
                                            dataset_name = DATASET_NAME,
                                            table_name   = f"{DATASET_NAME}-val1",
                                           ).latest()

    print(f"\nâœ… Training table loaded: {len(train_table_latest)} samples (latest version)")
    print(f"âœ… Validation table loaded: {len(val_table_latest)} samples (latest version)")
...
random.seed(SEED)
np.random.seed(SEED)
print(f"ğŸ”§ Using fixed seed: {SEED} for reproducible augmentations")
def get_random_augmentations(strategy="balanced"):

    strategies = {"conservative": {'hsv_h': (0.05, 0.3),
                                   'hsv_s': (0.05, 0.3),
                                   'hsv_v': (0.05, 0.3),
                                   'degrees': (5, 30),
                                   'scale': (0.4, 0.6),
    ...
    ranges = strategies[strategy]
    augmentations = {}

    for key, (min_val, max_val) in ranges.items():
        if key in ['hsv_h', 'hsv_s', 'hsv_v', 'scale', 'mosaic', 'mixup', 'copy_paste']:
            augmentations[key] = random.uniform(min_val, max_val)
        elif key == 'degrees':
            augmentations[key] = random.randint(int(min_val), int(max_val))

    # Ğ”Ğ¾Ğ±Ğ°Ğ²Ğ»Ñ�ĞµĞ¼ Ğ´Ğ¾Ğ¿Ğ¾Ğ»Ğ½Ğ¸Ñ‚ĞµĞ»ÑŒĞ½Ñ‹Ğµ Ğ°ÑƒĞ³Ğ¼ĞµĞ½Ñ‚Ğ°Ñ†Ğ¸Ğ¸
    augmentations.update({'translate': random.uniform(0.05, 0.2),
                          'shear': random.uniform(2, 15),
                          'perspective': random.uniform(0.0001, 0.001),
                          'flipud': random.uniform(0.1, 0.5),
                          'fliplr': random.uniform(0.3, 0.7),
                          'erasing': random.uniform(0.3, 0.7),
    })

    return augmentations

strategy = random.choice(["conservative", "balanced", "aggressive"])
augmentation_config = get_random_augmentations(strategy)

print(f"ğŸ�² Using {strategy} augmentation strategy:")
for key, value in augmentation_config.items():
    print(f"   {key}: {value:.3f}")

results = model.train(tables       = tables,       # Use 3LC Tables
                      ...
                      #lr0         = 0.0005,
                      #lrf         = 0.01,
                      #resume      = True          # Resume the train (model = YOLO("path/to/last.pt"))
                      # AUGMENTATION:
...
CUSTOM_WEIGHTS_PATH = "/root/.cache/kagglehub/competitions/the-3lc-cotton-weed-detection-challenge/cotton_weed_competition_dataset/best.pt"
...
#WORK_DIR   = Path("/home/ikita73/Conda_projects/Cotton Weed Detection/cotton_weed_competition_dataset")
TEST_DIR   = WORK_DIR / "test" / "images"
PRED_DIR   = Path("/root/.cache/kagglehub/competitions/the-3lc-cotton-weed-detection-challenge/cotton_weed_competition_dataset/Pred")
...
# Run inference
print("\nRunning inference...")
test_results = model.predict(source    = str(TEST_DIR),
                             ...
                             conf      = 0.3,   # Confidence threshold (adjust as needed)
                             imgsz     = IMAGE_SIZE,
...
# Analyze predictions
PRED_DIR = Path("/root/.cache/kagglehub/competitions/the-3lc-cotton-weed-detection-challenge/cotton_weed_competition_dataset/Pred")  # Must match Cell 21
...
labels_dir = PRED_DIR / "labels"
output_csv = "/root/.cache/kagglehub/competitions/the-3lc-cotton-weed-detection-challenge/cotton_weed_competition_dataset/submission_Batch_32.csv"
