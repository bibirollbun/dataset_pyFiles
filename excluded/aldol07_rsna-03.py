...
def make_submission_parquet(series_dir="/kaggle/input/rsna-intracranial-aneurysm-detection/series",
                             checkpoint_path="checkpoint.pth",
                             submission_path="submission.parquet",
                             limit=None):
    """
    Runs inference on test series and saves predictions in Kaggle submission.parquet format.
    `limit` is optional for fast testing.
    """
    # Load trained model
    model = Simple3DCNN(num_classes=len(LABEL_COLS))
    model.load_state_dict(torch.load(checkpoint_path, map_location=DEVICE))
    ...
    submission = []
    series_ids = sorted(os.listdir(series_dir))

    if limit:  # for faster testing
        series_ids = series_ids[:limit]

    print(f"Running inference on {len(series_ids)} series...")

    for series_id in series_ids:
        series_path = os.path.join(series_dir, series_id)
        try:
            preds = predict(series_path, model=model)  # use your existing predict()
            submission.append([series_id] + preds.tolist())
        except Exception as e:
            print(f"Skipping {series_id}: {e}")
            submission.append([series_id] + [0.5]*len(LABEL_COLS))  # fallback

    # Create submission DataFrame
    cols = ["ID"] + LABEL_COLS
    sub_df = pd.DataFrame(submission, columns=cols)

    # Save as parquet
    sub_df.to_parquet(submission_path, index=False)
    print(f"✅ Submission saved as {submission_path}")
    return sub_df



# Quick test on 50 series (optional)
submission_df = make_submission_parquet(limit=50)

# Full submission
#submission_df = make_submission_parquet(limit=None)
