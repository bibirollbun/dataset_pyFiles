...
# Cleaned data
df_train_clean = remove_outliers_iqr(df_train, numerical_columns)
...
sns.pairplot(df_train_clean[numerical_columns], hue='price', diag_kind='kde', plot_kws={'alpha':0.6})
plt.show()
...
X = df_train_clean.drop('price', axis='columns')
...
for feature in X.columns:
    if X[feature].dtype == 'object':
        _, iv = calculate_woe_iv(pd.concat([X, y],axis=1), feature, 'price' )
    else:
        X_binned = pd.cut(X[feature], bins=10, labels=False)
        _, iv = calculate_woe_iv(pd.concat([X_binned, y],axis=1), feature, 'price' )
