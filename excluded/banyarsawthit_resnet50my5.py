...
# =========================
# 3. Build Learner with Ranger Optimizer
# =========================
...
for opt_func in optimizers_to_try:
    for lr in lr_choices:
        ...
        print(f" Optimizer: {opt_func.__name__}, LR: {lr}, Accuracy: {acc:.4f}")
...
learn_resnet50 = vision_learner(
    dls, resnet50, metrics=accuracy, opt_func=Adam, pretrained=True
)
...
chosen_lr_resnet50 = slice(1e-4, 1e-3)



# =========================
# 5. Strategic Fine-tuning with EarlyStopping
# =========================
# Phase 1: Train the head only (frozen base)
learn_resnet50.freeze()
learn_resnet50.fit_one_cycle(3, lr_max=1e-3)

# Phase 2: Fine-tune all layers with discriminative learning rates
learn_resnet50.unfreeze()
learn_resnet50.fit_one_cycle(
    12,  # total = 15 (3 + 12)
    lr_max=chosen_lr_resnet50,
