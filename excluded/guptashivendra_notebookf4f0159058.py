...
# ---- Injected Optimized Solver (no triple-quoted strings) ----
...
def _detect_bg(g):
    h, w = _grid_size(g)
    edges = [g[i][j] for (i, j) in _edges_positions(h, w)]
    if edges:
...
_DIR8 = _DIR4 + [(1,1),(1,-1),(-1,1),(-1,-1)]

def _neighbors(i, j, h, w, conn=4):
    dirs_ = _DIR4 if conn==4 else _DIR8
    for di, dj in dirs_:
        ni, nj = i+di, j+dj
        if 0 <= ni < h and 0 <= nj < w:
            yield ni, nj

def _extract_components(g, bg, conn=4):
    h, w = _grid_size(g)
    ...
    for i in range(h):
        for j in range(w):
            ...
            while q:
                x,y = q.popleft()
                pixels.append((x,y))
                for nx, ny in _neighbors(x,y,h,w,conn):
                    if not seen[nx][ny] and g[nx][ny]==c:
                        seen[nx][ny]=True
                        q.append((nx,ny))
            xs=[p[0] for p in pixels]; ys=[p[1] for p in pixels]
            minx,maxx=min(xs),max(xs); miny,maxy=min(ys),max(ys)
            area=len(pixels); cx=sum(xs)/area; cy=sum(ys)/area
            comps.append({
                "pixels": pixels,
                "color": c,
...
def _rotate_k(g, k=0):
    ...
    if k==1: return np.rot90(arr, k=3).tolist()
    if k==2: return np.rot90(arr, k=2).tolist()
    if k==3: return np.rot90(arr, k=1).tolist()

def _mirror(g, axis='h'):
    arr=np.array(g,dtype=int)
    if axis=='h': return np.fliplr(arr).tolist()
    return np.flipud(arr).tolist()

def _crop_to_content(g, bg=None):
    if bg is None: bg=_detect_bg(g)
    h, w = _grid_size(g)
    xs, ys = [], []
    for i in range(h):
        for j in range(w):
    ...
    if not xs:
        return _clone_grid(g)
    minx,maxx=min(xs),max(xs); miny,maxy=min(ys),max(ys)
    return [row[miny:maxy+1] for row in g[minx:maxx+1]]

def _center_on_canvas(obj_g, out_h, out_w, bg):
    h,w = _grid_size(obj_g)
    O = [[bg]*out_w for _ in range(out_h)]
    top=(out_h-h)//2; left=(out_w-w)//2
    for i in range(h):
        for j in range(w):
            O[top+i][left+j]=obj_g[i][j]
    return O

def _remove_noise(g, max_area=1, bg=None, conn=4):
    if bg is None: bg=_detect_bg(g)
    comps = _extract_components(g, bg, conn)
    keep=[c for c in comps if c['area']>max_area]
    out=[[bg for _ in row] for row in g]
    for c in keep:
        for (x,y) in c['pixels']:
            out[x][y]=c['color']
    return out

# Hungarian (no docstrings)
def _hungarian(cost):
    ...
    cov_r = np.zeros(n,dtype=bool)
    cov_c = np.zeros(n,dtype=bool)

    for i in range(n):
        for j in range(n):
            if zero[i,j] and not cov_r[i] and not cov_c[j]:
                starred[i,j]=True
                cov_r[i]=True; cov_c[j]=True
    cov_r[:]=False; cov_c[:]=False

    def cover_cols_with_stars():
        for j in range(n):
            if starred[:,j].any(): cov_c[j]=True

    def find_zero():
        for i in range(n):
            if cov_r[i]: continue
            for j in range(n):
                if not cov_c[j] and cost[i,j]==0:
                    return i,j
        return None,None

    def star_in_row(i):
        js=np.where(starred[i,:])[0]
        return js[0] if len(js) else None

    def star_in_col(j):
        is_=np.where(starred[:,j])[0]
        return is_[0] if len(is_) else None
    ...
    def augment(path):
        for (i,j) in path:
            if primed[i,j]:
                primed[i,j]=False; starred[i,j]=True
            elif starred[i,j]:
                starred[i,j]=False

    def clear_covers_primes():
        cov_r[:]=False; cov_c[:]=False; primed[:]=False
    ...
    while cov_c.sum()<n:
        ...
        if sj is None:
            ...
            cov_r[i]=True; cov_c[sj]=False

    row = np.arange(n)
    col = np.argmax(starred, axis=1)
    return row, col

def _comp_cost(src, dst, w_area=1.0, w_bbox=1.0, w_centroid=1.0, w_color=0.5):
    da = abs(src['area']-dst['area'])
    sb=src['bbox']; db=dst['bbox']
    bb_s=(sb[2]-sb[0]+1, sb[3]-sb[1]+1)
    bb_d=(db[2]-db[0]+1, db[3]-db[1]+1)
    dbb = abs(bb_s[0]-bb_d[0])+abs(bb_s[1]-bb_d[1])
    dc = abs(src['centroid'][0]-dst['centroid'][0]) + abs(src['centroid'][1]-dst['centroid'][1])
    col = 0 if src['color']==dst['color'] else 1
    return w_area*da + w_bbox*dbb + w_centroid*dc + w_color*col

def _match_components(src_comps, dst_comps):
    n=max(len(src_comps), len(dst_comps))
    if n==0: return [], None
    C=np.full((n,n), 1e6, dtype=float)
    for i,s in enumerate(src_comps):
        for j,d in enumerate(dst_comps):
            C[i,j]=_comp_cost(s,d)
    ri,ci = _hungarian(C)
    pairs=[(i,j,C[i,j]) for i,j in zip(ri,ci) if i<len(src_comps) and j<len(dst_comps)]
    return pairs, C

# Portfolio primitives & program
class _Primitive:
    def __init__(self, name, func, params=None):
        self.name=name; self.func=func; self.params=params or {}
    def apply(self,g): return self.func(g, **self.params)

def _prim_identity(g, **kw): return _clone_grid(g)
def _prim_rotate(g, k=0, **kw): return _rotate_k(g,k)
def _prim_mirror(g, axis='h', **kw): return _mirror(g,axis)
def _prim_crop_center(g, out_h=None, out_w=None, **kw):
    bg=_detect_bg(g); c=_crop_to_content(g,bg)
    if out_h is None or out_w is None: return c
    return _center_on_canvas(c,out_h,out_w,bg)
def _prim_denoise(g, max_area=1, connectivity=4, **kw): return _remove_noise(g,max_area, None, connectivity)

_PRIMS = {
    "identity": _prim_identity,
    "rotate": _prim_rotate,
    "mirror": _prim_mirror,
    "crop_center": _prim_crop_center,
    "denoise": _prim_denoise,
}
...
class _Program:
    ...
    def apply(self,g):
        cur=g
        for p in self.steps: cur=p.apply(cur)
        return cur

def _score_program(p, pairs):
    for x,y in pairs:
        if p.apply(x)!=y: return 0.0
    return 1.0

def _expand(p):
    exts=[]
    for name,fn in _PRIMS.items():
        if name=='rotate':
            for k in (1,2,3): exts.append(_Program(p.steps+[_Primitive(name,fn,{'k':k})]))
        elif name=='mirror':
            for axis in ('h','v'): exts.append(_Program(p.steps+[_Primitive(name,fn,{'axis':axis})]))
        elif name=='crop_center':
            for oh in range(1,15):
                for ow in range(1,15):
                    exts.append(_Program(p.steps+[_Primitive(name,fn,{'out_h':oh,'out_w':ow})]))
        elif name=='denoise':
            for conn in (4,8):
                for ma in (1,2,3):
                    exts.append(_Program(p.steps+[_Primitive(name,fn,{'max_area':ma,'connectivity':conn})]))
        else:
            exts.append(_Program(p.steps+[_Primitive(name,fn,{})]))
    return exts

def _beam_search(pairs, max_depth=3, beam_size=50):
    seeds=[_Program([_Primitive('identity', _PRIMS['identity'], {})])]
    scored=[(_score_program(p,pairs),p) for p in seeds]
    scored.sort(key=lambda x:x[0], reverse=True)
    beam=[p for s,p in scored[:beam_size]]
    best=max(scored, key=lambda x:x[0]) if scored else (0.0, None)
    for _ in range(max_depth):
        if best[0]>=1.0: break
        new=[]
        for p in beam: new+=_expand(p)
        scored=[(_score_program(p,pairs),p) for p in new]
        scored.sort(key=lambda x:x[0], reverse=True)
        if scored and scored[0][0]>best[0]: best=scored[0]
        beam=[p for s,p in scored[:beam_size]]
    return best[1]

class OptimizedPortfolioSolver:
    # Drop-in solver; call predict(grid) to get output grid.
    # Optionally call fit(train_pairs) if task provides them.
    def __init__(self, max_depth=3, beam_size=50):
        self.prog=None; self.max_depth=max_depth; self.beam_size=beam_size
        self.extra_policies=[]

    def add_policy(self, fn): self.extra_policies.append(fn)

    def fit(self, train_pairs=None):
        if train_pairs: self.prog=_beam_search(train_pairs, self.max_depth, self.beam_size)

    def _predict_one(self, x):
        cands=[]
        if self.prog is not None:
            try: cands.append(self.prog.apply(x))
            except: pass
        for fn in self.extra_policies:
            try: cands.append(fn(x))
            except: pass
        for name,params in [('rotate',{'k':1}),('rotate',{'k':2}),('rotate',{'k':3}),
                            ('mirror',{'axis':'h'}),('mirror',{'axis':'v'})]:
            try: cands.append(_PRIMS[name](x,**params))
            except: pass
        if not cands: return x
        from collections import Counter
        def pal(g): return tuple(sorted(Counter([v for r in g for v in r]).items()))
        buckets={}
        for g in cands: buckets.setdefault(pal(g), []).append(g)
        top=max(buckets.values(), key=len)
        return top[0]

    def predict(self, x): return self._predict_one(x)

# Export a stable function name that your notebook can call without changing submission code too much.
def optimized_predict(grid):
    global _optimized_solver_instance
    try:
        s = _optimized_solver_instance
    except NameError:
        s = OptimizedPortfolioSolver(max_depth=3, beam_size=50)
        _optimized_solver_instance = s
    return s.predict(grid)
