...
# 训练集每月新增用户
monthly_train = train_df['first_active_month'].dt.to_period('M').value_counts().sort_index()
monthly_test = test_df['first_active_month'].dt.to_period('M').value_counts().sort_index()

ax1.plot(monthly_train.index.astype(str), monthly_train.values, marker='o', label='Train', linewidth=2)
ax1.plot(monthly_test.index.astype(str), monthly_test.values, marker='s', label='Test', linewidth=2)

x_ticks = range(0, len(monthly_train.index), 6)  # 每6个月显示一个标签
x_labels = [str(monthly_train.index[i]) for i in x_ticks]
ax1.set_xticks([i for i in x_ticks])
ax1.set_xticklabels(x_labels, rotation=45, ha='right')

ax1.set_title('Monthly New Users Growth', fontsize=14, pad=20)
ax1.set_xlabel('First Active Month')
ax1.set_ylabel('Number of Users')
ax1.legend()
ax1.grid(True, alpha=0.3)

# 累计用户增长
cumulative_train = monthly_train.sort_index().cumsum()
cumulative_test = monthly_test.sort_index().cumsum()

ax2.plot(cumulative_train.index.astype(str), cumulative_train.values, marker='o', label='Train Cumulative', linewidth=2)
ax2.plot(cumulative_test.index.astype(str), cumulative_test.values, marker='s', label='Test Cumulative', linewidth=2)

# 优化日期显示
ax2.set_xticks([i for i in x_ticks])
ax2.set_xticklabels(x_labels, rotation=45, ha='right')

ax2.set_title('Cumulative Users Over Time', fontsize=14, pad=20)
ax2.set_xlabel('First Active Month')
...
# 目标变量分布
ax1.hist(train_df['target'], bins=100, alpha=0.7, color='skyblue')
ax1.set_xlabel('Loyalty Score')
ax1.set_ylabel('Frequency')
ax1.set_title('Target Variable Distribution')
ax1.grid(True, alpha=0.3)

# 箱线图检测异常值
ax2.boxplot(train_df['target'])
ax2.set_ylabel('Loyalty Score')
ax2.set_title('Target Variable - Boxplot')
ax2.grid(True, alpha=0.3)

# QQ图检验正态性
from scipy import stats
stats.probplot(train_df['target'], dist="norm", plot=ax3)
...
# 计算数值特征相关性
numeric_features = ['feature_1', 'feature_2', 'feature_3', 'active_duration', 'target']
...
# 选择要分析的特征
features_to_plot = ['feature_1', 'feature_2', 'feature_3', 'active_duration', 'target']

# 抽样避免图形过载
sample_df = train_df.sample(n=1000, random_state=42)

# 创建散点矩阵图
g = sns.pairplot(sample_df[features_to_plot], 
                diag_kind='hist',  
                plot_kws={'alpha': 0.6, 's': 10},  
                diag_kws={'alpha': 0.7})  

g.fig.suptitle('Feature Relationship Scatter Matrix Plot', y=1.02)
plt.tight_layout()
plt.show()


train_df['year'] = train_df['first_active_month'].dt.year
yearly_stats = train_df.groupby('year')['target'].agg(['mean', 'std', 'count']).reset_index()
...
# 绘制柱状图（用户数量）
ax1.bar(yearly_stats['year'], yearly_stats['count'], alpha=0.3, color='skyblue', label='User Count')
ax1.set_xlabel('Cohort Year')
ax1.set_ylabel('User Count')

# 绘制折线图（平均忠诚度）
ax2.plot(yearly_stats['year'], yearly_stats['mean'], marker='o', linewidth=3, 
         color='red', label='Avg Loyalty Score')
ax2.set_ylabel('Average Loyalty Score')

lines1, labels1 = ax1.get_legend_handles_labels()
lines2, labels2 = ax2.get_legend_handles_labels()
ax1.legend(lines1 + lines2, labels1 + labels2, loc='upper left')

plt.title('Loyalty Score Trends by Cohort Year')
...
print("submission.csv是否存在：", os.path.exists("/kaggle/working/submission.csv"))
