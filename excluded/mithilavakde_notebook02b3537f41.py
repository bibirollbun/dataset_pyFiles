...
def create_array_from_grid(grid: List[List[int]], grid_size: int, channels: int, n_classes: int) -> np.ndarray:
    ...
    for r in range(rows):
        for c in range(cols):
            colour = int(g[r, c])
            if 0 <= colour <= n_classes - 2:
                arr[r, c, :n_classes] = 0.0
                arr[r, c, colour + 1] = 1.0
    return arr
...
def train_and_predict_for_group(group_task_ids: List[str],
    ...
    num_train_pairs_per_task = [len(challenges[tid].get('train', [])) for tid in group_task_ids]
    ...
    for tid in group_task_ids:
        ...
        current_task_all_pairs = list(train_pairs)
        for test_case in test_inputs:
            current_task_all_pairs.append({'input': test_case['input'], 'output': test_case['input']})
        all_pairs_per_task.append(current_task_all_pairs)
        
    P = len(all_pairs_per_task[0])
    ...
    for task_idx in range(T):
        for pair_idx in range(P):
            pair = all_pairs_per_task[task_idx][pair_idx]
            input_arr_15ch = create_array_from_grid(pair['input'], grid_size, C0_half, n_classes)
            output_arr_15ch = create_array_from_grid(pair['output'], grid_size, C0_half, n_classes)
    ...
    with torch.no_grad():
        for task_idx, task_id in enumerate(group_task_ids):
            ...
            for test_case in test_inputs:
                input_arr = create_array_from_grid(test_case['input'], grid_size, C0 // 2, n_classes)
...
def worker_process(args):
    group_task_ids, challenges, device_str, hparams = args
    ...
    try:
        ...
        submission_fragment = train_and_predict_for_group(
            group_task_ids, challenges, device, hparams
        )
...
if __name__ == "__main__":
    ...
    try:
        ...
        from temp_super_nca_worker import worker_process
    ...
    for task_id, task_data in challenges.items():
        num_pairs = len(task_data.get('train', [])) + len(task_data.get('test', []))
        if num_pairs > 0:
    ...
    for i, group_ids in enumerate(all_groups_to_process):
        ...
        # The worker needs the full challenges dict to look up task data
        group_args.append((
            group_ids, challenges, device_str, HPARAMS
        ))
