...
train_df['clean_title'] = pd.to_numeric(train_df['clean_title'], errors='coerce').fillna(0.0)
test_df['clean_title'] = pd.to_numeric(test_df['clean_title'], errors='coerce').fillna(0.0)


scaler = MinMaxScaler()


X_train = scaler.fit_transform(train_df[['clean_title']].values)
X_test = scaler.transform(test_df[['clean_title']].values)
...
torch.save(model.state_dict(),'keerthikamp_regr_model.pt')


new_model =RegressionModel()  
new_model.load_state_dict(torch.load('keerthikamp_regr_model.pt'))
