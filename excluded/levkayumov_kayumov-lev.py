...
PATCH_SIZE = 32
CLIP_IMG_SIZE = 224
num_patches_f = CLIP_IMG_SIZE / PATCH_SIZE
NUM_PATCHES = CLIP_IMG_SIZE // PATCH_SIZE
print(f"Sum patches: {num_patches_f}")


def get_clip_representation(imgs):
    
    with torch.no_grad():
        inputs = processor(text='a beatifull animal', images=imgs, return_tensors="pt").to(device)
...
def generate_seg_masks(imgs_path, classes_encoded):
    ...
    for img_name in tqdm(os.listdir(imgs_path)):
        ...
        img_patches = [Image.fromarray(img[x:x+PATCH_SIZE,y:y+PATCH_SIZE]) for x in range(0,CLIP_IMG_SIZE,PATCH_SIZE) for y in range(0,224,PATCH_SIZE)]
...
# ĞŸĞ¾Ñ�Ğ¼Ğ¾Ñ‚Ñ€Ğ¸Ğ¼ Ğ½Ğ° Ğ½ĞµÑ�ĞºĞ¾Ğ»ÑŒĞºĞ¾ ĞºĞ°Ñ€Ñ‚Ğ¸Ğ½Ğ¾Ğº
def show_sample_seg(val_imgs_path, val_img_names, val_seg_masks, n_sample = 5):

    f, axes = plt.subplots(n_sample,3, figsize = (8, 15))
    for i in range(n_sample):
        axes[i][0].imshow(np.array(Image.open(os.path.join(val_imgs_path, 
                                                        val_img_names[i]
                                                       )
                                          )
                               )
        ...
        axes[i][1].imshow(val_seg_masks[i])
        axes[i][2].imshow(np.array(Image.open(os.path.join(val_masks_path, 
                                                        val_img_names[i].replace('jpg', 'png')
                                                       )
                                          )
                                )
                      )
...
show_sample_seg(val_imgs_path, val_img_names, val_seg_masks, n_sample = 10)
...
def fit_epoch(tr_loader, clip, opt, loss_fn):
    # Ğ�Ğ±ÑƒÑ‡Ğ°ĞµĞ¼ Ğ½Ğµ Ğ²ĞµÑ�ÑŒ ĞºĞ»Ğ¸Ğ¿
    clip.train()
    history = {
        'IoU' : [],
        'loss' ; [],
    }
    for images, y_masks in tr_loader
