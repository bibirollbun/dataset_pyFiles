...
def check_code_solution(solution, task_data, valall=False):
    try:
        namespace = {}
        exec(solution, namespace)
        if 'p' not in namespace: return "No def p found"
...
result = check_code_solution(solution_code, task_data)
print(result)
...
def clean_code(s):
    s = re.sub(r"return\s+(.+)", r"exit(\1)", s)
    return s
    def remove_spaces(s):
        for c in ['[', ']', '(', ')', '{', '}', '=', '!', '<', '>', '+', '-', '/', '*', '%']:
            s = s.replace(' ' + c, c)
...
for task_num in  range(1, 401):
    ...
    for submission_path in submisisons:
        ...
        if not os.path.exists(task_code_path):
            ...
            if not os.path.exists(task_code_path):
                continue
        try:
            with open(task_code_path, 'r', encoding='utf-8') as f:
                solution_code = clean_code(f.read())
        except UnicodeDecodeError:
            with open(task_code_path, 'r', encoding='iso-8859-1') as f:
                solution_code = clean_code(f.read())
        status = check_code_solution(solution_code, task_data)
        if  status == "Success":
    ...
    if not task_code:
        ...
        solved += 1
        tasks_solved[int(task_num)] = True
...
! cp /kaggle/input/oh-barnacles/submission.zip /kaggle/working/submission.zip
...
for task_num in range(1, 401):
    ...
    try:
        ...
        if solved == len(all_exampels):
            ...
            print(f"[âœ˜] Task {task_id} {solved}/{len(all_exampels)} failed.")
    except Exception as e:
        pass
