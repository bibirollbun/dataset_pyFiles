...
data_XGB = df[['Id','x','y','s','a','dis','o','dir']]
grouped = data_XGB.groupby(['Id'])
df_XGB = grouped.agg(list).reset_index()


# Flatten the lists into a two dimensional data matrix
x = df_XGB['x'].apply(pd.Series).add_prefix('x_')
y = df_XGB['y'].apply(pd.Series).add_prefix('y_')
s = df_XGB['s'].apply(pd.Series).add_prefix('s_')
a = df_XGB['a'].apply(pd.Series).add_prefix('a_')
ds = df_XGB['dis'].apply(pd.Series).add_prefix('dis_')
o = df_XGB['o'].apply(pd.Series).add_prefix('o_')
dr = df_XGB['dir'].apply(pd.Series).add_prefix('dir_')
df_XGB = pd.concat([df_XGB.drop(['x','y','s','a','dis','o','dir'], axis=1),x,y,s,a,ds,o,dr], axis=1)
...
df_XGB = df_XGB.merge(play, on='Id', how='left').reset_index()
...
random_search = RandomizedSearchCV(estimator=model, param_distributions=param_dist, n_iter=10, cv=3, n_jobs=-1, verbose=2, random_state=42, scoring='accuracy')
