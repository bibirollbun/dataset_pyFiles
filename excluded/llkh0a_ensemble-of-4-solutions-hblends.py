...
# %% [markdown]
# # CMI Model Training Notebook
# 
# This notebook trains a custom model using the CMI backbone architecture for sensor data classification. 
# 
# model ideal is from: https://www.kaggle.com/code/wasupandceacar/lb-0-841-5fold-single-model-with-split-sensors
# 
# model training notebook: https://www.kaggle.com/code/llkh0a/5fold-single-model-with-split-sensors-train/
...
dataset_config = {
    ...
    "fold_y": "gesture",
    "fold_groups": "subject",
    "return_extra": True,  # Enable for training
}

# Data paths - adjust these to your data location
train_data_path = Path("/kaggle/input/cmi-detect-behavior-with-sensor-data/train.csv")  # Update this path
n_folds = 10
...
class CMIModel4(nn.Module):
    def __init__(self, target_classes_num, non_target_classes_num, **kwargs):
        super().__init__()
        self.backbone = CMIBackbone4(dataset4.imu_dim, dataset4.thm_dim, dataset4.tof_dim, **kwargs)
...
def init_dataset4():
    ...
    dataset4 = CMIFoldDataset4(universe_csv_path, dataset_config, full_dataset_function=CMIFeDataset4, n_folds=n_folds, random_seed=seed)
    dataset4.print_fold_stats()
    return dataset4

def get_fold_dataset4(dataset, fold):
    _, valid_dataset = dataset.get_fold_datasets(fold)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, num_workers=num_workers, shuffle=False)
    return valid_loader

dataset4 = init_dataset4()
...
if infer:
    ...
    # model_args = LIGHT_MODEL_CONFIG
    
    model_dicts = [
        {
            # "model_path" : f"/kaggle/input/10folds-cmi-split-sensor/pytorch/1/1/best_model_fold_{fold}.pt",
            # "model_path": f"/kaggle/working/best_model_fold_{fold}.pt",
            "model_path": f'/kaggle/input/5folds-single-model-with-splitsensor/pytorch/default/3/best_model_fold{fold}.pt',
            "fold": fold,
        } for fold in range(n_folds)
    ]
    ...
    for model_dict in model_dicts:
        model_path = model_dict['model_path']
        model = CMIModel4(**model_args).to(CUDA0)
        checkpoint = torch.load(model_path, weights_only=False)
        # model.load_state_dict(checkpoint['model_state_dict'])
        model.load_state_dict(torch.load(model_path))
        models4.append(model)
        folds4.append(model_dict['fold'])
print("succesfully loaded ",len(models4),"models")
...
metric4 = metric_package.Metric()
imu_only_metric4 = metric_package.Metric()

def to_cuda4(*tensors, device=CUDA0):
    def move(x):
...
def inference4(model, imus, thms, tofs):
...
def valid4(model, valid_bar):
    with torch.no_grad():
        for batch_data in valid_bar:
            ...
            pred_y = inference4(model, imus, thms, tofs)
            metric4.add(dataset4.le.classes_[y.argmax(dim=1).cpu()], dataset4.le.classes_[pred_y.argmax(dim=1).cpu()])
            ...
            pred_y_imu = inference4(model, imus, thms_zero, tofs_zero)
            imu_only_metric4.add(dataset4.le.classes_[y.argmax(dim=1).cpu()], dataset4.le.classes_[pred_y_imu.argmax(dim=1).cpu()])

# %%
for fold, model in enumerate(models4):
    valid_loader = get_fold_dataset4(dataset4, fold)
    valid_bar = tqdm(valid_loader, desc=f"Valid", leave=False)
    valid4(model, valid_bar)

print(f"""
Normal score: {metric4.score()}
IMU only score: {imu_only_metric4.score()}
""")

# %% [markdown]
# # predict function and inference

# %%
def avg_predict4(models, imus, thms, tofs):
    outputs = []
    with autocast(device_type='cuda', dtype=torch.bfloat16):
        for model in models:
            model.eval()
...
def split5_4(dataset, imu, thm, tof):
...
def predict4(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    imus, thms, tofs = split5_4(dataset4.full_dataset, imu, thm, tof)
    with torch.no_grad():
        ...
        pred_y = avg_predict4(models4, imus, thms, tofs)
    # print(f"Predicted classes: {dataset4.le.classes_[pred_y.argmax(dim=1).cpu()]}")
    prob = F.softmax(pred_y, dim=1).float().cpu().numpy()
    # print(f"Predicted class: {dataset4.le.classes_[pred_y.argmax(dim=1).cpu()]}")
    return prob
...
# def h_blend_4models(pred0, pred1, pred2, pred3, m_w, da_w, c_w):
#     """
#     Hierarchical blending for 4 models based on the original h_blend approach
#     """
#     m_wts, preds = np.asarray(m_w), []
#     for a, b, c, d in zip(pred0, pred1, pred2, pred3):    
#         l_abcd = [
#             { 'wts': m_wts[0], 'pred': a, 'res': 0 },
#             { 'wts': m_wts[1], 'pred': b, 'res': 0 },
#             { 'wts': m_wts[2], 'pred': c, 'res': 0 },
#             { 'wts': m_wts[3], 'pred': d, 'res': 0 },
#         ]
#         l_asc  = sorted(copy.deepcopy(l_abcd), key=lambda _: _['pred'], reverse=False)
#         l_desc = sorted(copy.deepcopy(l_abcd), key=lambda _: _['pred'], reverse=True) 
...
# def predict(sequence, demographics):
#     """
#     Alternative predict function using h_blend approach for 4 models
#     """
#     pred0 = predict1(sequence, demographics)[0]
#     pred1 = predict2(sequence, demographics)[0]
#     pred2 = predict3(sequence, demographics)[0]
#     pred3 = predict4(sequence, demographics)[0]
    
#     # Adjusted weights for 4 models (similar to original approach)
#     # Stronger corrections for ranking-based adjustment
#     m_w = [0.18, 0.25, 0.285, 0.285]  # Base model weights
#     da_w = [0.70, 0.30]  # Ascending/Descending blend weights
#     c_w = [+0.002, -0.0005, -0.0015, -0.0005]  # 4 correction weights
#     avg_p = h_blend_4models(pred0, pred1, pred2, pred3, m_w, da_w, c_w)
#     return dataset.le.classes_[avg_p.argmax()]
