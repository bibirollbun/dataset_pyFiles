...
models = {
    ...
    "XGBoost": xgb.XGBClassifier(n_estimators=200,learning_rate=0.05, max_depth=5, use_label_encoder=False,eval_metric='logloss', random_state=42),
    "CatBoost": cb.CatBoostClassifier(iterations=200, learning_rate=0.05, depth=5, verbose=0, random_state=42),
...
test_prediction = best_model.predict_proba(X_test_scaled)[:,1]


submission = pd.DataFrame({'id': test_df['id'], 'rainfall': test_prediction})
