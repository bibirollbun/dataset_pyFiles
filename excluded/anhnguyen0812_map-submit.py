...
# ========================================================
# 1. LEAKAGE EXPLOITATION: BUILD ANSWER KEY & CLEAN DATA
# ========================================================
print(">>> Building Answer Key from Train Data...")
try:
    train_df = pd.read_csv("/kaggle/input/map-charting-student-math-misunderstandings/train.csv")
    
    # --- CLEANING: DROP BAD LABELS ---
    # Theo phân tích, QuestionId 31778 có nhãn sai (MC_Answer 9 được gán True).
    ...
    if len(bad_indices) > 0:
        print(f"Dropping {len(bad_indices)} known bad labels for QID 31778.")
        train_df = train_df.drop(bad_indices)
    
    # --- BUILD KEY ---
    # Logic: Tìm đáp án được gắn nhãn True_Correct nhiều nhất cho mỗi câu hỏi.
    ...
    print(f"Warning: Could not build Answer Key. Filtering will be disabled. Error: {e}")
    qid_to_correct = {}

# ========================================================
# 2. ENSEMBLING: POWER MEAN (p=2.0)
# ========================================================
def robust_power_average(results, power=2.0):
    ret = []
    for idx in range(len(results[0])):
        sample_probs = [result[idx] for result in results]
        processed_probs = []
        for p in sample_probs:
            if hasattr(p, 'numpy'): p = p.numpy()
            if hasattr(p, 'cpu'): p = p.cpu()
            processed_probs.append(np.power(p, power))
        
        # Average the powered probabilities
        avg_pow = np.mean(processed_probs, axis=0)
        ret.append(avg_pow)
    return ret

# ========================================================
# 3. LOAD PREDICTIONS & PROCESS
# ========================================================
...
print(f"Ensembling Part 1 Files: {fns_part1}")
print(f"Ensembling Part 2 Files: {fns_part2}")

preds_part1_raw = [torch.load(fn, weights_only=True) for fn in fns_part1]
preds_part2_raw = [torch.load(fn, weights_only=True) for fn in fns_part2]

print("Applying Power Mean (p=2.0)...")
preds_part1 = robust_power_average(preds_part1_raw, power=2.0)
preds_part2 = robust_power_average(preds_part2_raw, power=2.0)

df = pd.concat([ds1.df.copy(), ds2.df.copy()]).reset_index(drop=True)
df["preds"] = preds_part1 + preds_part2

# ========================================================
# 4. POST-PROCESSING: SAFE LOGIC FILTER
# ========================================================
results = []
filtered_count = 0

print("Generating submission with Correctness Filtering...")
...
for _, row in df.iterrows():
    ...
    if qid in qid_to_correct:
        ...
        # (Convert to str/strip to handle formatting nuances like "1/3" vs " 1/3 ")
        if str(student_ans).strip() == str(correct_ans).strip():
            # Student CORRECT -> Penalize "False_"
            for i, cand in enumerate(label_candidates):
                if cand.startswith("False_"):
                    mask[i] = 0.0
        else:
            # Student INCORRECT -> Penalize "True_"
            for i, cand in enumerate(label_candidates):
...
print(f"Applied Logic Filter to {filtered_count} rows.")
