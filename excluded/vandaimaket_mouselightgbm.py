...
def make_self_features(
    ...
    def freeze_score(period_ms=1000):
        ...
        return freeze_ratio * position_stability
    ...
    def head_orientation_change(period_ms=300):
        """
        Tốc độ thay đổi hướng nhìn của đầu.
        -> Hành vi: 'exploreobject' (lắc đầu qua lại ngửi đồ vật), 'sniff' (đánh hơi).
        """
        v1x = pl.col("agent_x_nose") - pl.col("agent_x_body_center")
        v1y = pl.col("agent_y_nose") - pl.col("agent_y_body_center")
        angle = pl.arctan2(v1y, v1x)
        
        # Xử lý hiện tượng nhảy góc (Ví dụ từ 179 độ nhảy sang -179 độ là thay đổi nhỏ, không phải lớn)
        angle_diff = angle.diff()
        angle_diff = pl.when(angle_diff > 3.14159).then(angle_diff - 2*3.14159)\
                       .when(angle_diff < -3.14159).then(angle_diff + 2*3.14159)\
                       .otherwise(angle_diff)
        
        window_frames = max(1, int(round(period_ms * metadata["frames_per_second"] / 1000.0)))
        return angle_diff.abs().rolling_mean(window_size=window_frames, center=True, min_samples=1)
    
    def body_compression():
        """
        Độ nén của cơ thể (Tổng khoảng cách các chi đến mũi).
        -> Hành vi: Nhỏ nhất khi 'huddle' (cuộn tròn), lớn nhất khi 'stretch'/'run'.
        """
        return (
            body_parts_distance("nose", "hip_left")
            + body_parts_distance("nose", "hip_right")
            + body_parts_distance("nose", "tail_base")
        )
    
    def tail_movement():
        """
        Chuyển động riêng của đuôi.
        -> Hành vi: Phân biệt 'rest' (đuôi im) với 'alert' (đuôi ve vẩy) hoặc giữ thăng bằng.
        """
        return (
            (pl.col("agent_x_tail_tip").diff().pow(2)
             + pl.col("agent_y_tail_tip").diff().pow(2)).sqrt()
            / metadata["pix_per_cm_approx"]
        )
    
    def alignment_score():
        """
        Độ thẳng hàng: Mũi - Tâm - Đuôi có thẳng hàng không?
        -> Hành vi: Thẳng (gần 1) -> 'run'. Cong (gần -1 hoặc 0) -> 'turn', 'groom'.
        """
        nx = pl.col("agent_x_nose") - pl.col("agent_x_body_center")
        ny = pl.col("agent_y_nose") - pl.col("agent_y_body_center")
        tx = pl.col("agent_x_tail_base") - pl.col("agent_x_body_center")
        ty = pl.col("agent_y_tail_base") - pl.col("agent_y_body_center")
        return (nx * tx + ny * ty) / (
            (nx.pow(2) + ny.pow(2)).sqrt() * (tx.pow(2) + ty.pow(2)).sqrt() + 1e-06
        )
    
    def vibration_local():
        """
        Độ rung tại chỗ (Tổng quãng đường di chuyển L1 cực nhỏ).
        -> Hành vi: Phân biệt 'freeze' (đông cứng hoàn toàn) với 'shivering' (run rẩy) hoặc chuẩn bị nhảy.
        """
        return (
            pl.col("agent_x_body_center").diff().abs()
            + pl.col("agent_y_body_center").diff().abs()
    ...
    for agent_mouse_id in range(1, n_mice + 1):
        ...
        features = pivot.with_columns(
            ...
            *grooming_features(),
            head_orientation_change(300).alias("agent__head_orientation_change_300ms"),
            body_compression().alias("agent__body_compression"),
            tail_movement().alias("agent__tail_movement"),
            alignment_score().alias("agent__alignment_score"),
            vibration_local().alias("agent__vibration_local")
        )
...
def make_pair_features(
    ...
    def social_contact_features():
        # Chỉ tính toán để lấy logic tiếp xúc (Duration), không output lại cột distance thừa
        
        # Tận dụng các cột đã tính ở bước trước (cần đảm bảo tên cột khớp logic)
        # Hoặc tính lại biến tạm (như dưới đây) nhưng KHÔNG append vào list kết quả
        d_nose_nose = body_parts_distance("agent", "nose", "target", "nose")
        d_nose_tail = body_parts_distance("agent", "nose", "target", "tail_base")
        d_nose_body = body_parts_distance("agent", "nose", "target", "body_center")

        # Chỉ trả về các Feature "Thời lượng" (Duration) - Đây là cái mới
        return [
            # Tỉ lệ tiếp xúc trong 1 giây (30 frames)
            (d_nose_tail < 2.0).cast(pl.Float64).rolling_sum(window_size=30, center=True).alias("contact_genital_duration_1s"),
            (d_nose_nose < 2.0).cast(pl.Float64).rolling_sum(window_size=30, center=True).alias("contact_face_duration_1s"),
            (d_nose_body < 2.0).cast(pl.Float64).rolling_sum(window_size=30, center=True).alias("contact_body_duration_1s"),
        ]
...
def train_validate_1(lab_id: str, behavior: str,
    ...
    # Nối features và labels thành DataFrame tạm
    ...
    final_lgb, th_lgb, f1_lgb = tune_model(X, y, "l")
    
    save_dir = os.path.join(result_dir, "models_1")
    os.makedirs(save_dir, exist_ok=True)
    
    final_lgb.booster_.save_model(os.path.join(save_dir, "model_l.txt"))
    with open(os.path.join(save_dir, "threshold_l.txt"), "w") as f:
        f.write(str(th_lgb))
        
    print(f'LGB_Level1: th-{th_lgb:.3f}, f1-{f1_lgb:.3f}')
    return final_lgb, th_lgb
...
def train_validate_2(lab_id: str, behavior: str,
    ...
    final_lgb, th_lgb, f1_lgb = tune_model(X_enhanced, y, "l")
    ...
    print(f'LGB_Level2: th-{th_lgb:.3f}, f1-{f1_lgb:.3f}')
    return final_lgb, th_lgb
...
if STATUS == 'TRAINING':
    ...
    for (lab_id, behavior), group in tqdm(groups_list, total=total):
        ...
        final_lgb, thr_lgb = train_validate_1(lab_id, behavior, indices, features, labels)
        
        
        if final_lgb is not None:
            if behavior not in model1_dict:
                model1_dict[behavior] = {}
            model1_dict[behavior][lab_id] = (final_lgb, thr_lgb)
    ...
    for lab_id in os.listdir('/kaggle/input/train-v3-not-enough/results (1)'):
        for act in os.listdir(f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}'):
            if 'models_1' in os.listdir(f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}'):
                model1 = lgb.Booster(model_file=f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}/models_1/model_l.txt')
                with open(f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}/models_1/threshold_l.txt', 'r', encoding='utf-8') as f:
            ...
            if 'models_2' in os.listdir(f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}'):
                model2 = lgb.Booster(model_file=f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}/models_2/model_l.txt')
                with open(f'/kaggle/input/train-v3-not-enough/results (1)/{lab_id}/{act}/models_2/threshold_l.txt', 'r', encoding='utf-8') as f:
                    threshold = float(f.read())
    ...
    for lab_id in os.listdir('/kaggle/input/train-v3-not-enough/results (2)'):
        for act in os.listdir(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}'):
            if 'models_1' in os.listdir(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}'):
                model1 = lgb.Booster(model_file=f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}/models_1/model_l.txt')
                model1.load_model(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}/models_1/model_l.json')
                with open(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}/models_1/threshold_l.txt', 'r', encoding='utf-8') as f:
                    threshold = float(f.read())
                if lab_id not in model1_map:
                    model1_map[lab_id] = {}
                model1_map[lab_id][act] = (model1, threshold)
            if 'models_2' in os.listdir(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}'):
                model2 = lgb.Booster()
                model2.load_model(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}/models_2/model_l.json')
                with open(f'/kaggle/input/train-v3-not-enough/results (2)/{lab_id}/{act}/models_2/threshold_l.txt', 'r', encoding='utf-8') as f:
                    threshold = float(f.read())
                if lab_id not in model2_map:
                    model2_map[lab_id] = {}
                model2_map[lab_id][act] = (model2, threshold)
    ...
    for (lab_id, video_id, agent, target), group in tqdm(groups, total=len(list(groups))):
        ...
        for row in group.rows(named=True):
            ...
            try:
                ...
                model_predictions = model.predict(feature_array)
