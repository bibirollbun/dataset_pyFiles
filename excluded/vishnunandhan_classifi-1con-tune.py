...
#           (with first 30 layers frozen, L2 regularization, and LR scheduler)
# ─────────────────────────────────────────────────────────────────────────────
...
for fold in FOLDS:
    ...
    preds = Dense(3, activation='softmax', kernel_regularizer=l2(L2_WEIGHT))(x)
    ...
    history = model.fit(
        ...
        epochs=EPOCHS, callbacks=[es, ckpt, rl], verbose=2
    )
    # — Save history
...
# ── Focal Loss (must match training) ─────────────────────────────────────────
def focal_loss(gamma=2., alpha=0.25):
    def focal_loss_fixed(y_true, y_pred):
...
for fd in fold_dirs:
    lines = (fd / 'metrics.txt').read_text().splitlines()
    f1_val = float([l for l in lines if l.startswith('F1 Score')][0].split(':')[-1])
    idx    = int(fd.name.split('_')[1])
    if f1_val > best_f1:
        best_f1, best_fold = f1_val, idx
...
print("Val subjects sample:", val_df['subject'].unique()[:5])
