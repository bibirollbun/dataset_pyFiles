...
#------ 問６ (MIN RMSE : 7.23873) -----------------
# ここで追加する特徴量用にSaleデータを増やした配列を作成
train_tmp = train_x.copy()
train_tmp['sales'] = train_y

# 新特徴「店内月別平均売上」を作成
# store × month の平均売上を計算
SM_means = train_tmp.groupby(['store', 'month'])['sales'].mean()
# train_x に反映
train_x['store_month_avg'] = train_tmp.set_index(['store', 'month']).index.map(SM_means)
# test_x にも同様のmapを適用
test_x['store_month_avg']  = test_x.set_index(['store', 'month']).index.map(SM_means)

# 新特徴「店内日別平均売上」を作成
# store × day の平均売上を計算
SD_means = train_tmp.groupby(['store', 'day'])['sales'].mean()
# train_x に反映
train_x['store_day_avg'] = train_tmp.set_index(['store', 'day']).index.map(SD_means)
# test_x にも同様のmapを適用
test_x['store_day_avg']  = test_x.set_index(['store', 'day']).index.map(SD_means)

# 新特徴「商品月別平均売上」を作成
# item × month の平均売上を計算
IM_means = train_tmp.groupby(['item', 'month'])['sales'].mean()
# train_x に反映
train_x['item_month_avg'] = train_tmp.set_index(['item', 'month']).index.map(IM_means)
# test_x にも同様のmapを適用
test_x['item_month_avg']  = test_x.set_index(['item', 'month']).index.map(IM_means)

# 新特徴「商品日別平均売上」を作成
# item × day の平均売上を計算
ID_means = train_tmp.groupby(['item', 'day'])['sales'].mean()
# train_x に反映
train_x['item_day_avg'] = train_tmp.set_index(['item', 'day']).index.map(ID_means)
# test_x にも同様のmapを適用
test_x['item_day_avg']  = test_x.set_index(['item', 'day']).index.map(ID_means)

# 新特徴「店と商品のペアにおける局所的傾向」を作成
# store × item の平均売上を計算
SI_means = train_tmp.groupby(['store', 'item'])['sales'].mean()
# train_x に反映
train_x['store_item_avg'] = train_tmp.set_index(['store', 'item']).index.map(SI_means)
# test_x にも同様のmapを適用
test_x['store_item_avg']  = test_x.set_index(['store', 'item']).index.map(SI_means)
#----------------------------
...
print(train_x.head(14)) # 最初の2週間分を表示(dow = 0 ⇔ 月曜日）
