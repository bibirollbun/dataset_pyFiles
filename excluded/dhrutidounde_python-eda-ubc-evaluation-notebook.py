...
print(df.columns),'Ckeck the columns existing in dataset'
...
plt.show()
assert 'image_id' in df.columns and 'image_width' in df.columns, "Ensure 'image id' and 'image width' columns exist in the dataset"
...
sns.boxplot(data=df, x='label', y='image_id', palette='cool')
plt.title('Box Plot of image_id by Label')
plt.show()
assert 'image_id' in df.columns, "Ensure 'image_id' exists in the dataset"


sns.countplot(data=df, x='is_tma', palette='pastel')
plt.title('Count Plot of is_tma')
plt.show()
assert 'is_tma' in df.columns, "is_tma exists in the dataset"
