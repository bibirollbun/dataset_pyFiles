...
class BehaviorDetector(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply dropout to the output of the last time step before the classifier
...
if __name__ == "__main__":
    ...
    behavior_0_idx = np.where(y == 0)[0][:num_samples_to_show]
    behavior_1_idx = np.where(y == 1)[0][:num_samples_to_show]
    ...
    # We now split into 3 sets to monitor validation loss for early stopping.
    ...
    val_dataset = SensorDataset(X_val_scaled, y_val)
    ...
    learning_rate = 0.001
    num_epochs = 50 # Increased epochs to demonstrate early stopping
    ...
    for epoch in range(num_epochs):
        ...
        # Store the final losses for the epoch for visualization
        ...
        if val_loss < best_val_loss:
            ...
            epochs_no_improve = 0
            # You can save the best model here: torch.save(model.state_dict(), 'best_model.pth')
        else:
            epochs_no_improve += 1
    ...
    # --- Data Visualization: Plotting training and validation loss ---
