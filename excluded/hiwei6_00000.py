...
for fold_nb, (train_idx, val_idx) in enumerate(mskf.split(train_df, targets)):
    ...
    X_train, y_train = train_df.values[train_idx, :].astype(np.float32), targets.values[train_idx, :].astype(np.float32)
    X_val, y_val = train_df.values[val_idx, :].astype(np.float32), targets.values[val_idx, :].astype(np.float32)
    ...
    # ### Save model ###
    # model_save_path = f"tabnet_fold{fold_nb + 1}"
    # model_path = f"tabnet_fold{fold_nb + 1}.zip"
    # model.save_model(model_save_path)

    # ### Optionally reload model (for inference/testing separately) ###
    # model_loaded = TabNetRegressor()
    # model_loaded.load_model(model_path)
    
    ### Save weights & init params ###
    model_path = f"tabnet_fold{fold_nb + 1}.pth"
    # param_path = f"tabnet_fold{fold_nb + 1}_init.json"
    ...
    # model_loaded = TabNetRegressor(**init_params)
    # model_loaded.network.load_state_dict(torch.load(model_path))
    INPUT_DIM = X_test.shape[1]
    OUTPUT_DIM = 206  # 請依照 target 欄位數調整
    model_loaded = TabNetRegressor(
        input_dim=INPUT_DIM,
        output_dim=OUTPUT_DIM,
        **tabnet_params
    )
    model_loaded._set_network()

    # 載入模型權重
    weight_path = model_path
    model_loaded.network.load_state_dict(torch.load(weight_path, map_location="cpu"))


    ### Predict on validation ###
    preds_val = model_loaded.predict(X_val)
    ...
    ### Predict on test ###
    preds_test = model_loaded.predict(X_test)
