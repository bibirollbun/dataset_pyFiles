...
class CFG:
    ...
    ordinal_features = ["Month", "DayofMonth", "DayOfWeek"]
...
# Quick EDA to check unique categories in the ordinal columns
ordinal_features = ["Month", "DayofMonth", "DayOfWeek"]

for feature in ordinal_features:
    unique_values = sorted(train_df[feature].unique())
    print(f"{feature} unique categories ({len(unique_values)}): {unique_values}\n")
...
def preprocess_mixed(train_df, test_df, target_column, ordinal_features, ohe_features, train_size, test_valid_rel, random_state):
    """
    Preprocess data using Ordinal Encoding for ordinal features and One-Hot Encoding for other categorical features.
    ...
    Args:
        ...
        ohe_features (list): List of other categorical features to apply OHE.
        train_size (float): Proportion of data for training.
        test_valid_rel (float): Test size relative to validation set.
        random_state (int): Seed for reproducibility.

    Returns:
        X_train, X_valid, X_train_test, X_test: Feature sets.
    ...
    combined_df[ordinal_features] = ordinal_encoder.fit_transform(combined_df[ordinal_features])
    ...
    remainder_features = combined_df.drop(columns=ohe_features + ["dataset_type"]).columns
    final_feature_names = list(ohe_feature_names) + list(remainder_features)
...
# One-Hot Encoding:
X_train, X_valid, X_train_test, X_test, y_train, y_valid, y_train_test = preprocess_mixed(
    train_df = train_df, 
    test_df = test_df,
    target_column = CFG.target, 
    ordinal_features = CFG.ordinal_features,
    ohe_features = CFG.cat_features, 
...
lgbm_model = LGBMClassifier(**CFG.best_lgbm_params)
...
lgbm_model.fit(
    ...
    eval_metric = CFG.scoring,
    callbacks = [lgb.early_stopping(stopping_rounds = CFG.stopping_rounds)]
)
...
y_pred_test_proba = lgbm_model.predict_proba(X_train_test)[:, 1]
...
plt.gca().invert_yaxis()  # Reverse for descending importance
