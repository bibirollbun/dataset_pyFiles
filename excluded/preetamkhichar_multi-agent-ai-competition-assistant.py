...
# Export, Reset, and Search Functions
...
def reset_agent():
    """Reset agent memory and statistics for a fresh start"""
    if not agent:
        print("⚠ Agent not initialized")
        return False
...
def configure_agent(temperature=None, max_tokens=None, model_name=None):
    """Reconfigure agent parameters dynamically"""
    if not agent:
        print("⚠ Agent not initialized")
        return False
...
print("\n6. AGENT RESET")
print("-" * 60)
print(f"Before: {agent.stats['queries_processed']} queries, {len(agent.memory.messages)} messages")
reset_result = reset_agent()
print(f"After: {agent.stats['queries_processed']} queries, {len(agent.memory.messages)} messages")

if conv_file and log_file and reset_result:
    print("\n" + "=" * 60)
