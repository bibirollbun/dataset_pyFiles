...
# --- Enhanced Baseline Modeling with Feature Selection ---
print("Starting enhanced model training...")

# Configuration for feature selection and modeling
N_FEATURES = 250 # Number of features to select
N_SPLITS = 5 # Number of folds for cross-validation

# Store models, scores, and predictions
all_models = {}
all_scores = {}
test_predictions = {}
...
for target in target_cols:
    ...
    train_target_df = processed_train_df[processed_train_df[target].notna()]
    features = [col for col in processed_train_df.columns if col not in ["id", "SMILES"] + target_cols]
    ...
    lgbm_for_fs = lgb.LGBMRegressor(random_state=42)
    lgbm_for_fs.fit(X, y)
    feature_importances = pd.DataFrame({
        'feature': features,
        'importance': lgbm_for_fs.feature_importances_
    ...
    models = {
        ...
        "CATBOOST": cat.CatBoostRegressor(random_state=42, verbose=0)
    }
    
    all_models[target] = {}
    all_scores[target] = {}
    test_predictions[target] = np.zeros(len(X_test))
    ...
    for name, model in models.items():
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
            ...
            model_preds += pipeline.predict(X_test) / N_SPLITS
        
        all_models[target][name] = pipeline
        all_scores[target][name] = np.mean(fold_scores)
        test_predictions[target] += model_preds / len(models) # Ensemble predictions
        print(f"  {name} MAE for {target}: {all_scores[target][name]:.4f}")

    print(f"\nEnsemble MAE for {target}: {mean_absolute_error(y, np.mean([all_models[target][name].predict(X) for name in models], axis=0)):.4f}")

# --- Final Prediction and Submission ---
print("\n--- Generating final submission ---")
submission_df = processed_test_df[['id']].copy()
for target in target_cols:
    submission_df[target] = test_predictions[target]
    
# Apply the discovered tweak for FFV
if 'FFV' in submission_df.columns:
    submission_df['FFV'] **= 2
