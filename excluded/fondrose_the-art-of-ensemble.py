...
def compute_map3(eval_pred):
    ...
    top5 = np.argsort(-probs, axis=1)[:, :5]  # Top 3 predictions
    match = (top5 == labels[:, None])
...
top5 = np.argsort(-probs, axis=1)[:, :5]    # shape: [num_samples, 3]

# Decode numeric class indices to original string labels
flat_top5 = top5.flatten()
decoded_labels = le.inverse_transform(flat_top5)
top5_labels = decoded_labels.reshape(top5.shape)

# Join 3 labels per row with space
joined_preds = ["|".join(row) for row in top5_labels]
...
del top5_labels, flat_top5, decoded_labels, top5, test, ds_test
...
top5           = np.argsort(-predictions.predictions, axis=1)[:, :5]
flat_top5      = top5.flatten()
decoded_labels = le.inverse_transform(flat_top5)
top5_labels_cat    = decoded_labels.reshape(top5.shape)
top5_labels_cat


joined_preds = []

for preds in top5_labels_cat:
...
sub.to_csv("submission_deep.csv", index=False)
sub.head()
...
def get_top_k_ensemble(l1, l2, l3,k=5):
...
df3 = pd.read_csv('submission_deep.csv').rename(columns = {'Category:Misconception':'Category:Misconception_deep'})


df = pd.merge(df1, df2, on = 'row_id', how = 'inner')
df = pd.merge(df, df3, on = 'row_id', how = 'inner')
df['Category:Misconception'] = df.apply(lambda x: get_top_k_ensemble(x['Category:Misconception_gemma'], 
                                                                     x['Category:Misconception_ettin'], x['Category:Misconception_deep']), axis = 1)
