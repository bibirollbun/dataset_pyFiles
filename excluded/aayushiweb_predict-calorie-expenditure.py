...
sns.histplot(train['Age'].dropna(), kde=True, bins=30, color="green")
...
sns.scatterplot(x='Duration', y='Calories', data=train, hue='Sex', alpha=0.5)
...
print(train.groupby('Sex')['Calories'].mean())


train['Age_Group'] = pd.cut(train['Age'], bins=[0, 20, 40, 60, 80], labels=['0-20','21-40','41-60','61-80'])

sns.boxplot(x='Age_Group', y='Calories', data=train)
...
scatter = plt.scatter(
    train['Height'], train['Weight'],
    c=train['Calories'], cmap='viridis', alpha=0.6
)
...
X = train.drop(['Calories', 'id','Age_Group'], axis=1)
y = train['Calories']
