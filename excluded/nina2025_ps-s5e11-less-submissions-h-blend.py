...
path = f'/kaggle/input/02-november-2025-ps-s5e11/submission '

params = {
      'path'     : path,            
      'id_target': ['id',"loan_paid_back"],          
      'type_sort': ['asc/desc', 0.30,0.70],
      'subwts'   : [ +0.030,+0.025,+0.020,+0.015,+0.010, 
                     -0.010,-0.015,-0.020,-0.025,-0.030 ],
      'subm'     : [
         
         { 'name': f'0.86599','weight':+0.140 },
         { 'name': f'0.88379','weight':+0.130 },
         { 'name': f'0.89144','weight':+0.120 },
         { 'name': f'0.89599','weight':+0.110 },
         { 'name': f'0.91060','weight':+0.103 },
         { 'name': f'0.91093','weight':+0.097 },
         { 'name': f'0.91409','weight':+0.090 },
         { 'name': f'0.92023','weight':+0.080 },
         { 'name': f'0.92150','weight':+0.070 },
         { 'name': f'0.92165','weight':+0.060 },
      ]
}

df_cross = h_blend(params,color='tes10',figures1=True,figures2=True,details=True)

display_distances(params)


subms = [path + subm['name'] + ".csv" for subm in params['subm']]

for j in range(3):
    k  = 0.0025
    for i,file_name in enumerate(subms):
        df = pd.read_csv(file_name)
        df_cross["loan_paid_back"] = df_cross["loan_paid_back"] *(1+k) - (k)* df['loan_paid_back']
    k += 0.0005

dfTop = pd.read_csv('/kaggle/input/02-november-2025-ps-s5e11/submission 0.92680.csv')

dfTop["loan_paid_back"] =\
    dfTop["loan_paid_back"] *1.0085 - 0.0085*df_cross["loan_paid_back"]


dfTop.to_csv('submission.csv',index=False)
dfTop
