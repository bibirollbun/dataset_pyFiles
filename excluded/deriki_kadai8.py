...
train_x['weekend'] = ((train_x['dow'] == 5) | (train_x['dow'] == 6)).astype(int) # 0:平日, 1:週末
test_x['weekend'] = ((test_x['dow'] == 5) | (test_x['dow'] == 6)).astype(int)

print(train_x.head(21)) # 最初の3週間分を表示(dow = 0 ⇔ 月曜日）
# 未加工の 'date' は使用しない（※テスト側が全て未知カテゴリになるためそのままの使用は困難）
train_x = train_x.drop([col, new_col], axis=1)
test_x = test_x.drop([col, new_col], axis=1)
