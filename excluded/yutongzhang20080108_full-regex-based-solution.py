...
PRIMARY_PATTERNS = [
    r"\bwe (used|utilized|applied|trained on|obtained|collected)\b",
    r"\b(dataset|data) (was|were) (used|analyzed|applied)\b",
    r"\banalysis of\b",
    r"\bperformed on\b",
]

SECONDARY_PATTERNS = [
    r"\bprevious (study|studies)\b",
    r"\bsee also\b",
    r"\baccording to\b",
    r"\bas (described|reported)\b",
    r"\bsimilar to\b",
    r"\breference to\b",
    r"\bmentioned in\b",
]

def classify_citation(context_text: str) -> str:
...
def main_pipeline():
    ...
    tmp_parq = pathlib.Path("/tmp/extracted.parquet")
    df.to_parquet(tmp_parq)
    print(f"âœ” Saved extracted IDs to: {out_parq} and {tmp_parq} â€” {len(df)} rows")
    ...
    # ğŸŒŸ STEP 3: Inline post-filter to clean paper-like DOIs using context
    print("\nğŸŒŸ STEP 3: Post-filtering DOIs (remove paper citations unless data-ish context)")
    import polars as pl

    DOI_LINK = "https://doi.org/"

    def is_doi_link(col: str = "dataset_id") -> pl.Expr:
        return pl.col(col).str.starts_with(DOI_LINK)  # where DOI_LINK="https://doi.org/"
    # Common publisher/journal DOI prefixes (paper-like)
    PAPER_PREFIXES = [
    ...
    # If these appear near a DOI mention, it's more likely a data citation
    ...
    def _is_paper_prefix_expr(col: str = "dataset_id") -> pl.Expr:
        # DOI_LINK should match how you format DOIs in 'dataset_id' (e.g., "https://doi.org/")
        expr = pl.lit(False)
    ...
    try:
        sub_pl = pl.read_csv(str(submission_path))

        # Drop row_id to make joins/concats clean
        if "row_id" in sub_pl.columns:
            sub_pl = sub_pl.drop("row_id")

        # Load context windows (article_id, dataset_id, window)
        # This comes from your earlier extraction step; adjust path if needed.
        ctx_path = "/tmp/extracted.parquet"
        win = (
            pl.read_parquet(ctx_path)
              .select("article_id", "dataset_id", "window")
        )

        # Split DOI vs accession (requires helpers.is_doi_link)
        doi_rows = (
            sub_pl
            .filter(is_doi_link("dataset_id"))
            .join(win, on=["article_id", "dataset_id"], how="left")
        )
        acc_rows = sub_pl.filter(~is_doi_link("dataset_id"))

        # Keep DOI if NOT a known paper prefix OR has "data-ish" context nearby
        keep_mask = (
            (~_is_paper_prefix_expr("dataset_id"))
            | doi_rows["window"].fill_null("").str.contains(CONTEXT_RE, literal=False)
        )

        kept_doi = doi_rows.filter(keep_mask).select("article_id", "dataset_id", "type")
        final = pl.concat([
            kept_doi,
            acc_rows.select("article_id", "dataset_id", "type")
        ], how="vertical")

        # Write back with row_id
        final.with_row_index("row_id").write_csv(str(submission_path))
        print("âœ” Post-filter applied and submission.csv updated")

    except FileNotFoundError:
        # If context parquet isn't present, skip filtering but continue pipeline
        print("âš  Context parquet not found; skipping DOI post-filter.")
    except Exception as e:
        print(f"âš  Post-filter step encountered an error and was skipped: {e}")

    # Validation (only if ground truth available)
    gt_path = pathlib.Path('/kaggle/input/make-data-count-finding-data-references/train_labels.csv')
    if gt_path.exists():
        print("\nğŸ“Š Validation on TRAIN SPLIT")
        preds = pl.read_csv(str(submission_path)).select(['article_id','dataset_id','type'])
        gt    = (
            pl.read_csv(str(gt_path))
