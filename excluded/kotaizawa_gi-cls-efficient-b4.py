...
path = kagglehub.dataset_download("clemchris/uw-madison-gi-tract-image-segmentation-masks")
...
tree("/kaggle/input/uw-madison-gi-tract-image-segmentation-masks")
...
import pandas as pd
train_without_incorrect_mask = pd.read_csv("/kaggle/input/uw-madison-gi-tract-image-segmentation-masks/train_preprocessed.csv")
train_without_incorrect_mask = train_without_incorrect_mask.loc[~((train_without_incorrect_mask["case"] == 7) & (train_without_incorrect_mask["day"] == 0))]
train_without_incorrect_mask = train_without_incorrect_mask.loc[~((train_without_incorrect_mask["case"] == 81) & (train_without_incorrect_mask["day"] == 30))]
train_without_incorrect_mask = train_without_incorrect_mask.loc[~((train_without_incorrect_mask["case"] == 43) & (train_without_incorrect_mask["day"] == 18))]
train_without_incorrect_mask = train_without_incorrect_mask.loc[~((train_without_incorrect_mask["case"] == 138) & (train_without_incorrect_mask["day"] == 0))]
...
train_without_incorrect_mask.drop(columns="fold", axis=1, inplace=True)


skf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=101)
train_without_incorrect_mask["fold"] = -1  # 新しく 'fold' 列を作成

for fold, (train_idx, val_idx) in enumerate(skf.split(
    train_without_incorrect_mask, 
    train_without_incorrect_mask['empty'], 
    groups=train_without_incorrect_mask["case"]
)):
    train_without_incorrect_mask.iloc[val_idx, train_without_incorrect_mask.columns.get_loc("fold")] = fold


save_path = "/kaggle/working/train_without_incorrect_mask.csv"
train_without_incorrect_mask.to_csv(save_path, index=False)


!mkdir -p /root/.cache/torch/hub/checkpoints
!cp ../input/gi-seg-downloads/efficientnet-b4-6ed6700e.pth /root/.cache/torch/hub/checkpoints/efficientnet-b4-6ed6700e.pth


!cd ../input/gi-seg-downloads && \
pip install -q --no-deps efficientnet_pytorch-0.6.3.tar.gz pretrainedmodels-0.7.4.tar.gz timm-0.9.12-py3-none-any.whl segmentation_models_pytorch-0.3.3-py3-none-any.whl && \
pip install -q --no-deps monai-1.1.0-202212191849-py3-none-any.whl && \
pip install -q --no-deps torchmetrics-0.11.4-py3-none-any.whl


#!cd ../input/gi-seg-downloads && \
#pip install -q efficientnet_pytorch-0.6.3.tar.gz pretrainedmodels-0.7.4.tar.gz timm-0.4.12-py3-none-any.whl  segmentation_models_pytorch-0.3.3-py3-none-any.whl && \
#pip install -q monai-0.8.1-202202162213-py3-none-any.whl && \
#pip install -q torchmetrics-0.8.0-py3-none-any.whl
...
class Dataset(torch.utils.data.Dataset):# init, len, getitem はmust
    ...
    def __len__(self):# お決まり
        return len(self.df)

    def __getitem__(self, idx: int):#__init__で設定したデータをもとに指定されたインデックスのデータをロード
...
class LitDataModule(pl.LightningDataModule):
    def __init__(
        ...
        super().__init__()#pytorch_lightning.LightningModuleに含まれるデフォルト関数を呼び出すイメージ

        self.save_hyperparameters()#__init__の引数を保存 -> 他のメソッドでself.hparams.x で呼び出せる
    ...
    def _init_transforms(self):
        ...
        train_transforms = [
            A.Resize(img_size, img_size, interpolation=cv2.INTER_NEAREST),  # サイズ統一
        ]
        if self.hparams.use_augs:
            train_transforms += [
                A.ShiftScaleRotate(
                    shift_limit=0.0625, scale_limit=0.10, rotate_limit=10, p=0.5
                ),  # 最大±10%のズーム + ±5°の回転
                A.OneOf(
                    [
        ...
        train_transforms += [
            A.Resize(img_size, img_size, interpolation=cv2.INTER_NEAREST),  # もう一度リサイズ
...
class DiceMetric(Metric):
    def __init__(self, thr=0.5, dim=(2, 3), epsilon=0.001):
        super().__init__()#compute_on_cpu=True was deleted
...
class LitModule(pl.LightningModule):
    ...
    def __init__(
        ...
        self.model.decoder.dropout = torch.nn.Dropout(self.hparams.dropout_rate)
...
#        return criterion
    def _init_loss_fn(self):
        ...
        weights = {"bce": 1, "dice": 3}  # 重みを定義
        total_weight = sum(weights.values())
    
        def criterion(y_pred, y_true):
            weighted_loss = sum(weights[loss] * loss_fn(y_pred, y_true) for loss, loss_fn in zip(losses, loss_fns))
            return weighted_loss / total_weight  # 合計重みで割る
...
def train(
    ...
    train_csv_path: str = "/kaggle/working/train_without_incorrect_mask.csv",
...
def infer(img_size, load_images, batch_size, num_workers, model_paths, device, thr):
    data_module = LitDataModule(
        train_csv_path="/kaggle/working/train_without_incorrect_mask.csv",
