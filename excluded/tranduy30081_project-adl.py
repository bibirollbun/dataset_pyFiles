...
def train_model(model, train_loader, val_loader, num_epochs=10, 
               ...
               factor=0.5, patience=2, device=device, save_path='models',
               use_amp=True, accumulation_steps=4, cp_path=None, callbacks=None, best_model = None):
    """
...
def train_and_evaluate_auto_session(fold_number=0, batch_size=16, 
    ...
    combined_data, combined_path = combine_result_files(path)
...
# Káº¿t há»£p lá»‹ch sá»­ cÃ¡c phiÃªn

combined_data, combined_path = combine_result_files(cp_path)
#history = copy_file_from_input_to_output(combined_path)
# total_data, total_path = combine_result_files('/kaggle/working/')
# Váº½ biá»ƒu Ä‘á»“ lá»‹ch sá»­ káº¿t há»£p
plot_training_from_json(json_file_path=combined_path)

# TÃ¬m model tá»‘t nháº¥t
#best_model_info = show_latetest_checkpoint_model(cp_path)
best_model, _ = load_best_model(cp_path)
# Load vÃ  Ä‘Ã¡nh giÃ¡ model tá»‘t nháº¥t
# if best_model_info and best_model_info['model_path']:
#     model = load_model(best_model_info['model_path'])
# if best_model :
#     model = load_model(best_model)  
#     # Ä�Ã¡nh giÃ¡ model
#     _, val_loader, _, _ = create_dataloaders(dataset, fold_number=0, batch_size=16)
#     accuracy, auc_score, confusion_matrix = evaluate_model(model, val_loader)
    
#     print(f"\n[EVALUATE] Model tá»‘t nháº¥t - Accuracy: {accuracy:.4f}, AUC: {auc_score:.4f}")
...
def generate_predictions(model_path, batch_size=16, num_workers=4, use_tta=True, color_space='rgb'):
    """
...
def run_inference(model_path=None, batch_size=32, use_tta=True, color_space='rgb'):
    """
...
submission_df = run_inference(
    model_path= best_model ,  # Tá»± Ä‘á»™ng tÃ¬m model tá»‘t nháº¥t
    batch_size=32,    # CÃ³ thá»ƒ tÄƒng batch size khi dá»± Ä‘oÃ¡n vÃ¬ khÃ´ng cáº§n lÆ°u gradient
    use_tta=True,     # Sá»­ dá»¥ng Test Time Augmentation tá»« Notebook-1
    color_space='rgb' # CÃ¹ng khÃ´ng gian mÃ u vá»›i lÃºc huáº¥n luyá»‡n
)
