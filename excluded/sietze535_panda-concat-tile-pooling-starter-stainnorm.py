...
# Load images
target_path = os.path.join(TRAIN, os.listdir(TRAIN)[4])
target = cv2.cvtColor(cv2.imread(target_path), cv2.COLOR_BGR2RGB)

source_path = os.path.join(TRAIN, os.listdir(TRAIN)[7])
to_transform = cv2.cvtColor(cv2.imread(source_path), cv2.COLOR_BGR2RGB)
...
# Apply normalization
norm_img, H, E = normalizer.normalize(I=to_transform, stains=True)
norm_img = np.clip(norm_img, 0, 255).astype(np.uint8)

# Counter to check how many normalizations failed
normalization_fail_count = 0
normalization_total_count = 0
...
axs[1].imshow(to_transform)
axs[1].set_title("Original (to normalize)")
axs[1].axis('off')

axs[2].imshow(norm_img)
...
# Save the target image you used for normalization
target_path = os.path.join(TRAIN, os.listdir(TRAIN)[4])
...
def open_image(fn:PathOrStr, div:bool=True, convert_mode:str='RGB', cls:type=Image,
    ...
    # Stain Normalization Part
    global normalization_fail_count, normalization_total_count
    
    x = np.array(x)
    
    # Check if tile is very empty
    empty_ratio = (x > 220).sum() / (x.shape[0] * x.shape[1] * x.shape[2])
    if empty_ratio < 0.8:
        normalization_total_count += 1
        try:
    ...
    x = PIL.Image.fromarray(x)  # Convert back to PIL image
...
for fold in range(nfolds):
    ...
    if normalization_total_count > 0:
        print(f"Normalization failures after training fold {fold}: {normalization_fail_count}/{normalization_total_count} ({100 * normalization_fail_count/normalization_total_count:.2f}%)")
    else:
        print(f"No normalizations attempted in fold {fold}.")
