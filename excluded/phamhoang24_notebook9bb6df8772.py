...
# 1. Lấy file env-setup.py từ branch mới
!curl -L https://raw.githubusercontent.com/pytorch/xla/main/contrib/scripts/env-setup.py -o pytorch-xla-env-setup.py

# 2. Cài PyTorch XLA version 1.7 (hợp với Colab/Kaggle + timm)
!python pytorch-xla-env-setup.py --version 1.7

# 3. Cài timm không lỗi với Python 3.7 hoặc 3.8
...
def _run():
    device = xm.xla_device()
    print(f"Hello from device: {device}")
    return 1
