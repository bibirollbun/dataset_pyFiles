...
lgb_full = lgb.LGBMClassifier(objective='multiclass', num_class=len(le.classes_), random_state=42, n_estimators=500, learning_rate=0.05, verbosity=-1)
...
lgb_imu = lgb.LGBMClassifier(objective='multiclass', num_class=len(le.classes_), random_state=42, n_estimators=500, learning_rate=0.05, verbosity=-1)
...
import polars as pl# --- 3. The Competition's `predict` function ---
