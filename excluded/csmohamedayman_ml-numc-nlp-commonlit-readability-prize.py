...
dir_path = '/kaggle/input/commonlitreadabilityprize'
...
train = basic_text_features(train, 'excerpt')
train = lexicon_features(train, 'excerpt')
train = add_columns(train, 'excerpt')


original_train  = basic_text_features(original_train , 'excerpt')
original_train  = lexicon_features(original_train , 'excerpt')
original_train  = add_columns(original_train , 'excerpt')


test = basic_text_features(test, 'excerpt')
test = lexicon_features(test, 'excerpt')
test = add_columns(test, 'excerpt')
...
for feature in ['excerpt_verb_count', 'excerpt_adj_count', 'excerpt_adv_count', 'excerpt_length_in_sentence']:
    plot_categorical_features(original_train[[feature]])
    original_train = replace_rare_categories(original_train, feature, -1, threshold_percent=1.0)
    train          = replace_rare_categories(train, feature, -1, threshold_percent=1.0)
    plot_categorical_features(original_train[[feature]])
...
def evaluate_numerical_model(y_test, y_pred):
...
def get_numerical_models():
    # Dictionary of numerical models to be tuned
    numerical_models = {
...
def run_models(models, evaluate_model, X_train, X_test, y_train, y_test):
    evaluation_results, y_predictions = {}, {}
    # Iterate over each model in the models dictionary
    for model_name, model in models.items():
        print(f'Running Model for {model_name}...')
        begin_time = time.time()
        # Fit the model on the training data
        model.fit(X_train, y_train)
        # Make predictions on the test data
        y_pred = model.predict(X_test)
        # Try to evaluate the model's predictions
        try:
            evaluation_results[model_name] = evaluate_model(y_test, y_pred)
        ...
        print(f'{model_name} Model ran in'.ljust(50), f'{duration} minutes')
        print('------------------------------------')
    # Convert evaluation results to a DataFrame and reset index
    evaluation_results = pd.DataFrame(evaluation_results).T
    evaluation_results = evaluation_results.reset_index()
    evaluation_results = evaluation_results.rename(columns={'index': 'Model'})
...
baseline_models = get_numerical_models()
evaluate_model_func = evaluate_numerical_model
...
best_model1 = get_best_model(evaluation_results1, baseline_models, 'Mean Squared Error', ascending=False)
...
best_model2 = get_best_model(evaluation_results2, baseline_models, 'Mean Squared Error', ascending=False)
...
selected_features_by_variance_threshold = variance_threshold_selector(train_model3, 0.05)
...
score_funcs = [RandomForestRegressor]
all_importance_dfs, all_evaluation_results = \
    analyse_feature_selection(X, y, score_funcs, selection_methods, 
...
selected_features = set(selected_features_by_variance_threshold) & selected_features_by_significance
...
best_model3 = get_best_model(evaluation_results3, baseline_models, 'Mean Squared Error', ascending=False)
...
selected_numerical_features = set(numerical_features) & (set(correlations_df.head(1)['Feature']) | set(correlations_df.tail(1)['Feature']))
...
categorical_features = get_categorical_features(train_model4)
selected_categorical_features = set(categorical_features) & (set(correlations_df.head(1)['Feature']) | set(correlations_df.tail(1)['Feature']))
print(f'selected_categorical_features: {selected_categorical_features}')
...
best_model4 = get_best_model(evaluation_results4, baseline_models, 'Mean Squared Error', ascending=False)
...
selected_oversamplying_features = set(oversamplying_features) & (set(correlations_df.head(3)['Feature']) | set(correlations_df.tail(3)['Feature']))
...
best_model6 = get_best_model(evaluation_results5, baseline_models, 'Mean Squared Error', ascending=False)
...
selected_features_by_variance_threshold = variance_threshold_selector(train_model6, 0.05)
print(f'selected_features_by_variance_threshold are {len(selected_features_by_variance_threshold)} features\n{selected_features_by_variance_threshold}')
...
selected_oversamplying_features = set(oversamplying_features) & (set(correlations_df.head(2)['Feature']) | set(correlations_df.tail(2)['Feature']))
print(f'selected_oversamplying_features: {selected_oversamplying_features}')
...
best_model6 = get_best_model(evaluation_results6, baseline_models, 'Mean Squared Error', ascending=False)
...
param_grids = {
    ...
    'LassoCV': {
        'n_alphas': [50, 100, 200, 500],
        'alphas': [(0.01, 0.1, 1.0, 5.0, 10.0), (0.05, 0.5, 5.0, 25.0)],
        'fit_intercept': [True, False],
        'max_iter': [1000, 2000, 4000, 8000],
    },
    'SGDRegressor': {
        'loss': ['epsilon_insensitive', 'squared_error', 'huber', 'squared_epsilon_insensitive'],
        'alpha': [0.01, 0.1, 1.0, 5.0, 10.0],
        'fit_intercept': [True, False],
        'max_iter': [1000, 2000, 4000, 8000],
...
adjusted_r2_scorer   = make_scorer(adjusted_r2, greater_is_better=True)
...
selected_oversamplying_features = set(oversamplying_features) & (set(correlations_df.head(1)['Feature']) | set(correlations_df.tail(1)['Feature']))
print(f'selected_oversamplying_features: {selected_oversamplying_features}')
...
tuned_baseline_models = tune_models(baseline_models, X, y, 'neg_root_mean_squared_error')
...
best_model7 = get_best_model(evaluation_results7, tuned_baseline_models, 'Mean Squared Error', ascending=False)
...
submission = pd.DataFrame({
    'id' : test_id,
    target_feature : best_model7.predict(test_model7[X_test.columns]),
})
submission.head()
