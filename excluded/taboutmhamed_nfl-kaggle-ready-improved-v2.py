...
# Kaggle environment detection (NO import kaggle needed!)
KAGGLE_ENV = os.path.exists('/kaggle/input')

if KAGGLE_ENV:
    BASE_PATH = '/kaggle/input/nfl-big-data-bowl-2026-analytics'
    print("ğŸ�ˆ Kaggle Environment Detected")
    print(f"ğŸ“‚ Data Path: {BASE_PATH}")
else:
    BASE_PATH = 'kaggle_test_env/kaggle/input/nfl-big-data-bowl-2026-analytics'
...
# =============================================================================
# CATCH PROBABILITY INDEX - CORE CLASS
# =============================================================================

class CatchProbabilityIndex:
    """
    Catch Probability Index (CPI) Calculator
    
    Evaluates receiver performance through 7 weighted components:
    - Distance to ball (25%)
    - Defensive separation (20%)
    - Directional alignment (15%)
    - Acceleration (15%)
    - Speed (10%)
    - Timing (10%)
    - Agility (5%)
    ...
    def compute_distance_score(self, receiver_pos: Tuple[float, float], 
                              ball_pos: Tuple[float, float]) -> float:
        """Calculate distance score (0-100) based on proximity to ball"""
        distance = euclidean(receiver_pos, ball_pos)
        max_distance = 30.0  # yards
        score = max(0, 100 * (1 - distance / max_distance))
        return score
    
    def compute_separation_score(self, receiver_pos: Tuple[float, float],
                                 defenders_positions: List[Tuple[float, float]]) -> float:
        """Calculate separation score based on distance from defenders"""
        if not defenders_positions:
            return 100.0
        
        min_distance = min([euclidean(receiver_pos, def_pos) 
                           for def_pos in defenders_positions])
        optimal_separation = 5.0  # yards
        score = min(100, 100 * (min_distance / optimal_separation))
        return score
    
    def compute_direction_score(self, receiver_pos: Tuple[float, float],
                               receiver_velocity: Tuple[float, float],
                               ball_pos: Tuple[float, float]) -> float:
        """Calculate directional alignment score"""
        velocity_magnitude = np.linalg.norm(receiver_velocity)
        if velocity_magnitude < 0.1:
            return 50.0  # Neutral score if stationary
        
        to_ball = np.array(ball_pos) - np.array(receiver_pos)
        to_ball_magnitude = np.linalg.norm(to_ball)
        
        if to_ball_magnitude < 0.1:
            return 100.0  # Perfect score if at ball location
        
        # Cosine similarity
        to_ball_normalized = to_ball / to_ball_magnitude
        velocity_normalized = np.array(receiver_velocity) / velocity_magnitude
        alignment = np.dot(to_ball_normalized, velocity_normalized)
        
        score = 50 * (1 + alignment)  # Maps [-1,1] to [0,100]
        return score
    
    def compute_acceleration_score(self, acceleration: float) -> float:
        """Calculate acceleration score"""
        max_accel = 5.0  # yards/sÂ²
        score = min(100, 100 * abs(acceleration) / max_accel)
        return score
    
    def compute_speed_score(self, speed: float) -> float:
        """Calculate speed score"""
        optimal_speed = 7.0  # yards/s
        score = min(100, 100 * speed / optimal_speed)
        return score
    
    def compute_timing_score(self, time_to_ball: float, 
                            ball_arrival_time: float) -> float:
        """Calculate timing synchronization score"""
        if ball_arrival_time <= 0:
            return 50.0
        
        timing_diff = abs(time_to_ball - ball_arrival_time)
        score = max(0, 100 * (1 - timing_diff / ball_arrival_time))
        return score
    
    def compute_agility_score(self, direction_changes: int, 
                             time_window: float) -> float:
        """Calculate agility score based on direction changes"""
        if time_window <= 0:
            return 50.0
        
        agility_rate = direction_changes / time_window
        optimal_rate = 2.0  # changes per second
        score = min(100, 100 * agility_rate / optimal_rate)
        return score
    
    def compute_cpi(self, components: Dict[str, float]) -> float:
        """
        Compute final CPI score (0-100)
        
        Args:
            components: Dictionary with all 7 component scores
        
        Returns:
            CPI score (0-100)
        """
        cpi = sum(components[key] * self.weights[key] 
                 for key in self.weights.keys())
        return round(cpi, 2)
    
    def get_component_breakdown(self, components: Dict[str, float]) -> pd.DataFrame:
        """Return detailed breakdown of CPI components"""
        breakdown = []
...
print("âœ… CatchProbabilityIndex class loaded successfully")


# Initialize CPI calculator
cpi_calculator = CatchProbabilityIndex()

# Example scenario: Elite receiver
example_components = {
    'distance': 85.0,      # Close to ball
    'separation': 75.0,    # Good separation
    'direction': 90.0,     # Well aligned
    'acceleration': 70.0,  # Strong acceleration
    'speed': 80.0,         # Fast
    'timing': 85.0,        # Perfect timing
    'agility': 65.0        # Agile
}

# Calculate CPI
cpi_score = cpi_calculator.compute_cpi(example_components)

# Display results
print("ğŸ�¯ CATCH PROBABILITY INDEX CALCULATION")
print("=" * 50)
print(f"\nğŸ“Š Final CPI Score: {cpi_score:.2f}/100")
print(f"\nğŸ’¡ Interpretation: ", end="")
if cpi_score >= 70:
    print("HIGH probability (85% completion rate)")
elif cpi_score >= 50:
    print("MODERATE probability (65% completion rate)")
else:
    print("LOW probability (45% completion rate)")

# Component breakdown
print("\nğŸ“ˆ Component Breakdown:\n")
breakdown_df = cpi_calculator.get_component_breakdown(example_components)
print(breakdown_df.to_string(index=False))
print(f"\nâœ… Total: {breakdown_df['Contribution'].sum():.2f}")
...
# Component scores
components_list = list(example_components.keys())
scores = list(example_components.values())
colors = plt.cm.RdYlGn([s/100 for s in scores])

ax1.barh(components_list, scores, color=colors)
ax1.set_xlabel('Score (0-100)', fontsize=12)
ax1.set_title('CPI Component Scores', fontsize=14, fontweight='bold')
ax1.axvline(x=70, color='red', linestyle='--', alpha=0.5, label='High Threshold')
ax1.legend()
ax1.grid(axis='x', alpha=0.3)

# Weighted contributions
contributions = [example_components[c] * cpi_calculator.weights[c] 
                for c in components_list]

ax2.pie(contributions, labels=components_list, autopct='%1.1f%%',
        startangle=90, colors=colors)
ax2.set_title('Weighted Contribution to CPI', fontsize=14, fontweight='bold')
...
print("=" * 50)
print(f"ğŸ“Š Environment: {'Kaggle' if KAGGLE_ENV else 'Local'}")
print("âœ… CPI: Innovative 7-component metric developed")
print("ğŸ�† Ready for NFL Big Data Bowl 2026 submission")
print("\nğŸš€ University Track - Analytics Competition")
