...
# Group by tweet_date and count the sentiment
tweets_by_date = df.groupby(df['tweet_date'].dt.date)['sentiment'].count().reset_index()

# Create the line plot using Seaborn
plt.figure(figsize=(12, 6))
sns.lineplot(data=tweets_by_date, x='tweet_date', y='sentiment')
...
df.loc[:, 'tweet_text'] = df['tweet_text'].apply(remove_stop_words)
...
df['tweet_text'] = df['tweet_text'].apply(lambda row: lemmatize_doc(row))


positive_text = ' '.join(df[df['sentiment'] == 1]['tweet_text'])
negative_text = ' '.join(df[df['sentiment'] == 0]['tweet_text'])
...
test_df=pd.read_csv('/kaggle/input/TweetSentimentBR/Test.csv')
...
test_df['tweet_text'] = test_df['tweet_text'].apply(clean_text)


test_df.loc[:, 'tweet_text'] = test_df['tweet_text'].apply(remove_stop_words)


test_df['tweet_text'] = test_df['tweet_text'].apply(lambda row: lemmatize_doc(row))


pred_x = tfidf.transform(test_df.tweet_text)   
