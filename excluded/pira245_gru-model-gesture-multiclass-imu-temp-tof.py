...
filedir = '/kaggle/input/cmi-gesture-classification-gru-models/pytorch/imu-temp-tof-data/3/gru_model_module.py'
...
folder = '/kaggle/input/cmi-gesture-classification-gru-models/pytorch/imu-temp-tof-data/3/'
...
imu_temp_tof_trained_models = my_utils.handle_pickle_dict(
        folder=folder,
        pickle_filename='imu_temp_tof_trained_models.pkl'
    )
...
# âœ… Load best training params for gesture
# ----------------------------
best_params = imu_temp_tof_trained_models['gesture']['best_params']
input_dim = imu_temp_tof_trained_models['gesture']['input_dim']
output_dim = imu_temp_tof_trained_models['gesture']['output_dim']
...
# + TEMP 
gesture_temp_model_weights = Path(folder) / Path(imu_temp_tof_trained_models['gesture']['model_name']) 
gesture_temp_adapter_state = Path(folder) / Path(imu_temp_tof_trained_models['gesture']['model_adapter']) 
...
# imu_temp_tof_df = kagglehub.dataset_load(KaggleDatasetAdapter.PANDAS,"pira245/cmi-dataset","full_imu_temp_tof_merged_v2_df.csv",)
# imu_temp_tof_df = imu_temp_tof_df.drop(['row_id.1' , 'row_id.2', 'row_id.3', 'sequence_counter.1'], axis=1)


# imu_temp_tof_df.fillna(-1, inplace=True)


# gesture_columns = ['gesture']
# n_splits = 5 # 80% train / 20% val
# split_index  = 0
# df = imu_temp_tof_df.drop(['row_id'], axis=1)
# imu_balanced_datasets = my_utils.prepare_balanced_gesture_datasets(df, gesture_columns,n_splits, split_index)
...
# # main_gesture_df = gesture_val.copy()
# main_gesture_df = imu_temp_tof_df.drop(['row_id'], axis=1).copy()
...
# # Scale with training scaler
# scaler = imu_temp_tof_trained_models['gesture']['scaler']
# scaled_x_test_gesture = scaler.transform(x_test_gesture)
...
# calibration_100_670['imu_temp_tof'].setdefault(100, {'model': [imu_temp_base_T, imu_temp_p_base_val], 'adapter': [imu_temp_adpt_T, imu_temp_p_adpt_val]})


# imu_temp_w_base_history =  gru_utils.search_soft_voting_weight(
#     calibration_100_670['imu_temp_tof'][100]['model'][1],               # np.ndarray or torch.Tensor, shape (N, C)
#     calibration_100_670['imu_temp_tof'][100]['adapter'][1],             # same shape (N, C)
#     y_val_zero_based.cpu(),         # array-like labels, shape (N,), 0..C-1
#     weight_grid=None,          # defaults np.linspace(0,1,21)
#     max_epochs=None,           # defaults len(weight_grid)
#     verbose=True)
...
# calibration_100_670['imu_temp_tof'][100].setdefault('best_ens_weight', [imu_temp_w_base_100])
...
# calibration_100_670['imu_temp_tof'].setdefault(670, {'model': [imu_temp_base_T, imu_temp_p_base_val], 'adapter': [imu_temp_adpt_T, imu_temp_p_adpt_val]})


# imu_temp_w_base_history =  gru_utils.search_soft_voting_weight(
#     calibration_100_670['imu_temp_tof'][670]['model'][1],               # np.ndarray or torch.Tensor, shape (N, C)
#     calibration_100_670['imu_temp_tof'][670]['adapter'][1],             # same shape (N, C)
#     y_val_zero_based.cpu(),         # array-like labels, shape (N,), 0..C-1
#     weight_grid=None,          # defaults np.linspace(0,1,21)
#     max_epochs=None,           # defaults len(weight_grid)
#     verbose=True)
...
# calibration_100_670['imu_temp_tof'][670].setdefault('best_ens_weight', [imu_temp_w_base_670])
...
folder = '/kaggle/input/cmi-gesture-classification-gru-models/pytorch/imu-temp-tof-data/3/'
loaded_calibration_100_670 = my_utils.handle_pickle_dict(folder=folder, pickle_filename ='calibration_100_670.pkl')
...
def imu_temp_prediction(
    ...
    # Global models and configs (loaded externally)
    global imu_temp_tof_trained_models
    ...
    # Apply the saved scaler from training to standardize input features
    scaler = imu_temp_tof_trained_models[task_key]['scaler']
    scaled_x_test = scaler.transform(new_test_df.to_numpy())
    ...
    # Extract appropriate model, adapter, and ensemble weight
    best_model = loaded_calibration_100_670['imu_temp_tof'][model_key]['model'][0]
    best_adapter = loaded_calibration_100_670['imu_temp_tof'][model_key]['adapter'][0]
    best_weight = loaded_calibration_100_670['imu_temp_tof'][model_key]['best_ens_weight'][0]
...
def predict(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    sequence_tof_subset = sequence_pd[Tof_COLUMNS]
    sequence_tof_subset.fillna(-1, inplace=True)
    ...
    # # === Step 3: IMU+TEMP+Tof predictions ===
    temp_cols = TEMP_COLUMNS
    # Concatenate IMU+TEMP with Tof data
    imu_temp_tof_merged_subset =  pd.concat([
        imu_temp_merged_subset.reset_index(drop=True),
        sequence_tof_subset.reset_index(drop=True)
    ], axis=1)
    
    for step in tqdm(range(1), desc="IMU+Temp Model Prediction"):
        
        if _ == False:
            #print('imu+temp+tof prediction') 
            predictions, gesture_probs  = imu_temp_prediction(imu_temp_tof_merged_subset)
