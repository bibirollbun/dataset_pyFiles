...
y = np.log1p(train_df[target_column])  # Log-transform the target
...
xgb_cv_scores  = cross_val_score(xgb_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
...
# Instantiate the model (alpha is the regularization strength)
# A common default/starting point is alpha=1.0. You might tune this later.
ridge_model = Ridge(alpha=1.0)

ridge_cv_scores = cross_val_score(ridge_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
...
lasso_cv_scores = cross_val_score(lasso_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
print("Lasso CV RMSE:", -lasso_cv_scores.mean())
...
lgb_cv_scores = cross_val_score(lgb_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
print("LightGBM CV RMSE:", -lgb_cv_scores.mean())

lgb_model.fit(X_train, y_train)
print("Training LightGBM complete.")
...
# rf_cv_scores = cross_val_score(rf_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
...
# print("--- Training SVR ---")
# svr_model = SVR()
# svr_cv_scores = cross_val_score(svr_model, X_processed, y, cv=5, scoring='neg_root_mean_squared_error')
# print("SVR CV RMSE:", -svr_cv_scores.mean())

# svr_model.fit(X_train, y_train)
# print("Training SVR complete.")
...
rmsle_lr = calculate_rmsle(np.expm1(y_val), np.expm1(y_pred_lr))
print(f"Linear Regression RMSLE: {rmsle_lr:.4f}")

# Evaluate Ridge Regression
rmsle_ridge = calculate_rmsle(np.expm1(y_val), np.expm1(y_pred_ridge))
print(f"Ridge Regression RMSLE:  {rmsle_ridge:.4f}")

# Evaluate Lasso Regression
rmsle_lasso = calculate_rmsle(np.expm1(y_val), np.expm1(y_pred_lasso))
print(f"Lasso Regression RMSLE:  {rmsle_lasso:.4f}")

rmsle_xgb = calculate_rmsle(np.expm1(y_val), np.expm1(y_pred_xgb))
print(f"XGBoost Regressor RMSLE: {rmsle_xgb:.4f}")

rmsle_lgb = calculate_rmsle(np.expm1(y_val), np.expm1(y_pred_lgb))
print(f"LightGBM Regressor RMSLE:{rmsle_lgb:.4f}")
...
# Predict
y_pred_log = xgb_model.predict(test_processed)
