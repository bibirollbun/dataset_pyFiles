...
class MockLLMAgent:
    ...
    def explain(self, topic: str, level: str = 'intro') -> str:
        return f"Explanation ({level}) for {topic}: This is a concise explanation."
    ...
    def answer_query(self, topic: str, query_type: str) -> str:
        ...
        if query_type == "key_concepts":
            return (f"Key concepts for {topic}: main definitions, algorithms, examples.")
        if query_type == "practice_problems":
            return (f"Practice ideas for {topic}: implement small examples and solve 3 problems.")
        return f"Short guidance for {topic} regarding '{query_type}'."
...
for t in topics:
    ...
    for qt, ans in plan_result['informative_queries'][t].items():
        print(f"    - {qt}: {ans}")

print("\nSession stored keys:", list(session_service.get(sid).get("state").keys()))
