...
selected_features = ['pressure', 'temparature', 'dewpoint', 'humidity', 'windspeed', 'winddirection', 'cloud', 'rainfall']

plt.figure(figsize=(12, 12))
sns.pairplot(train[selected_features].dropna(), hue='rainfall', diag_kind='kde', markers=['o', 's'], plot_kws={'alpha': 0.6})
plt.show();
...
log_model = LogisticRegression(C=0.1, random_state=777)
...
svc_model = SVC(C=0.1, kernel='rbf', gamma='auto', random_state=777)
...
dt_model = DecisionTreeClassifier(max_depth=8, min_samples_split=2, criterion='gini', random_state=777)
...
xgb_model = XGBClassifier(n_estimators=100, learning_rate=0.1, max_depth=7, random_state=777)
...
cat_model = CatBoostClassifier(iterations=200, learning_rate=0.2, depth=8, verbose=0, random_state=777)
...
top_features = feature_importance['Feature Id'].tolist()

# Prepare different feature sets
feature_sets = {
    "Top 10 Features": top_features[:10],
    "Top 11 Features": top_features[:11],
    "Top 12 Features": top_features[:12],
    "Top 13 Features": top_features[:13],
}
...
for label, features in feature_sets.items():
    ...
    X_val_selected = X_val_scaled[features]

    # Train a new model
    model = CatBoostClassifier(iterations=200, learning_rate=0.2, depth=8, verbose=0, random_state=777)
    ...
    results.append({"Feature Set": label, "Accuracy": accuracy, "AUC-ROC": auc_roc})
...
X_train_selected = X_train_scaled[top_features[:12]]

selected_model = CatBoostClassifier(iterations=200, learning_rate=0.2, depth=8, verbose=0, random_state=777)
...
predictions = selected_model.predict(test_scaled)


pred_prob = selected_model.predict_proba(test_scaled)
