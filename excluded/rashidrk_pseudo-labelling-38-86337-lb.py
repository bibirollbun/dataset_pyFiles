...
sub.to_csv("before pseudo labelling submission.csv", index=False)
sub.head()
...
new_train = pd.concat([train, test], ignore_index = True, axis = 0)
new_train.shape
...
# continue with the KFold training
...
for fold, (train_idx, valid_idx) in enumerate(kf.split(new_train)):
    print(f"### Fold {fold+1} ###")

    X_train, y_train = new_train.loc[train_idx, features].copy(), new_train.loc[train_idx, target]
    X_valid, y_valid = new_train.loc[valid_idx, features].copy(), new_train.loc[valid_idx, target]
    X_test = test[features].copy()
...
rmse = np.sqrt(mean_squared_error(new_train[target], oof_preds))
print(f"Validation RMSE: {rmse}")
