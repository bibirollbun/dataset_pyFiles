...
params = {
     ...
     'subwts'   : [w/100 for w in [ +5, -5 ]],
     'subm'     : [
         {'name': f'0.70477',     'weight':+0.50, 'color':'royalblue' },
         {'name': f'0.70442.lgbm','weight':+0.50, 'color':'orange'    },]
}
df1 = h_blend(params, details=True, subm="A.csv")
...
params = {
     ...
     'subwts'   : [w/100 for w in [ -5, +5 ]],
     'subm'     : [
         {'name': f'0.70442.xgb', 'weight':+0.50, 'color':'royalblue' },
         {'name': f'0.70682',     'weight':+0.50, 'color':'orange'    },]
}
df2 = h_blend(params, details=True, subm="B.csv")
...
params = {
     ...
     'subwts'   : [w/100 for w in [ +5, -5 ]],
     'subm'     : [
         {'name': f'0.70651', 'weight':+0.50, 'color':'royalblue' },
         {'name': f'0.70682', 'weight':+0.50, 'color':'orange'    },]
}
df3 = h_blend(params, details=True, subm="C.csv")


for file in 'A,B,C,D,tida_desc'.split(','): 
...
params = {
     ...
     'subwts'   : [w/100 for w in [ +5, -5 ]],
     'subm'     : [
         {'name': f'0.70477',     'weight':+0.79, 'color':'royalblue' },
         {'name': f'0.70442.lgbm','weight':+0.21, 'color':'orange'    },]
}
df1 = h_blend(params, details=True, subm="A.csv")
...
params = {
     ...
     'subwts'   : [w/100 for w in [ -5, +5 ]],
     'subm'     : [
         {'name': f'0.70442.xgb', 'weight':+0.11, 'color':'royalblue' },
         {'name': f'0.70682',     'weight':+0.89, 'color':'orange'    },]
}
df2 = h_blend(params, details=True, subm="B.csv")
...
params = {
     ...
     'subwts'   : [w/100 for w in [ +5, -5 ]],
     'subm'     : [
         {'name': f'0.70651', 'weight':+0.45, 'color':'royalblue' },
         {'name': f'0.70682', 'weight':+0.55, 'color':'orange'    },]
}
df3 = h_blend(params, details=True, subm="C.csv")
...
params = {
     ...
     'id_target': ['id',"diagnosed_diabetes"],          
     'type_sort': ['asc/desc',0.30,0.70 ],
     'subwts'   : [w/100 for w in [ +5, -1,-4 ]],
     'subm'     : [
         {'name': f'A', 'weight':+0.05, 'color':'royalblue' },
         {'name': f'B', 'weight':+0.05, 'color':'orange'    },
         {'name': f'C', 'weight':+0.90, 'color':'green'     },]
}
df4 = h_blend(params, details=True) # subm='0.70_754.csv'


df_754 = pd.read_csv('/kaggle/input/28-december-2025-ps-s5e12/0.70_754.csv')
df_765 = pd.read_csv('/kaggle/input/28-december-2025-ps-s5e12/0.70_765.csv')

df = pd.read_csv('/kaggle/input/playground-series-s5e12/sample_submission.csv')

df["diagnosed_diabetes"] = \
    df_754['diagnosed_diabetes'] * 0.30 + \
    df_765['diagnosed_diabetes'] * 0.70

df.to_csv('cross.csv',index=False)


path = '/kaggle/input/28-december-2025-ps-s5e12/'

id_target = ['id',"diagnosed_diabetes"]

params = {'id_target':id_target, 'path':path, 'subm': [{'name':'0.70_754'},{'name':'0.70_765'}]}
        
seaborn_Show(params, file_name_cross='cross.csv')


for file in 'A,B,C,D,cross,tida_desc'.split(','): 
    if os.path.isfile(file+'.csv'): os.remove(file+'.csv')
