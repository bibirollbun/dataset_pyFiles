...
file_path = '/kaggle/input/asl-fingerspelling/train_landmarks/2072876091.parquet'  # ví dụ: 'data.txt'
...
if not os.path.isdir("new_data"):
    os.mkdir("new_data")  # Nếu thư mục chưa tồn tại thì tạo mới
else:
    shutil.rmtree("new_data")  # Nếu đã tồn tại thì xóa thư mục cũ
    os.mkdir("new_data")       # Sau đó tạo lại thư mục mới
...
for file_id in tqdm(dataset_df.file_id.unique()):
    ...
    tf_file = f"new_data/{file_id}.tfrecord"
