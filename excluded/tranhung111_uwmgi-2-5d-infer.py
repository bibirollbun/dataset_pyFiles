...
# ------------------------------------------------------------
# 0.  Dice / IoU helpers (your originals)
# ------------------------------------------------------------
def dice_coef(y_true, y_pred, thr=0.5, dim=(2,3), epsilon=1e-3):
    ...
    return ((2*inter + epsilon) / (den + epsilon)).mean(dim=(1,0))  # scalar

def iou_coef(y_true, y_pred, thr=0.5, dim=(2,3), epsilon=1e-3):
    y_true = y_true.to(torch.float32)
    y_pred = (y_pred > thr).to(torch.float32)
    inter  = (y_true * y_pred).sum(dim=dim)
    union  = (y_true + y_pred - y_true*y_pred).sum(dim=dim)
    return ((inter + epsilon) / (union + epsilon)).mean(dim=(1,0))  # scalar

# ------------------------------------------------------------
# 1.  Paths
# ------------------------------------------------------------
GT_DIR = Path("/kaggle/input/uwmgi-25d-stride2-dataset/masks/masks")
SAVE_DIR = Path("/kaggle/working/mask")
...
for batch_idx, ids, img_paths, imgs_np, masks_np in infer2_stream(model_paths, valid_loader):
    ...
    for i in range(B):
        ...
        # ---------- 3a.  save prediction (unchanged) ----------
        filename = os.path.basename(img_paths[i])
        np.save(SAVE_DIR / f"pred_{filename}", masks_np[i])

        # ---------- 3b.  load GT mask ----------
        gt_path = GT_DIR / filename            # same basename
        if not gt_path.exists():
            print(f"âš ï¸�  GT not found for {filename} â€” skipped")
            continue

        gt_arr   = np.load(gt_path)            # (H,W,3) uint8 0/255
        pred_arr = masks_np[i]                 # (H,W,3) uint8 0/1
        
        # binarise to 0/1 float
        gt_arr   = (gt_arr   > 0).astype(np.float32)   # 0/1
        pred_arr = pred_arr.astype(np.float32)         # already 0/1

        # ---------- 3c. (1,C,H,W) torch ----------
        gt_t   = torch.from_numpy(gt_arr.transpose(2,0,1)[None])
        pred_t = torch.from_numpy(pred_arr.transpose(2,0,1)[None])
        
        # ---------- 3d. metrics ----------
        dice_val = dice_coef(gt_t, pred_t).item()
        iou_val  = iou_coef (gt_t, pred_t).item()
...
# ------------------------------------------------------------
if dice_all:
    print(f"\nImages scored      : {len(dice_all)}")
    print(f"Mean Dice coefficient : {np.mean(dice_all):.4f}")
    print(f"Mean IoU (Jaccard)   : {np.mean(iou_all):.4f}")
else:
    print("No prediction / GT pairs were evaluated.")
