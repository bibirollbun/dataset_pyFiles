...
# 1) merge intervals
# -----------------------------------------------------------
def _merge_intervals(ivals, eps=1e-5):
    if not ivals:
        return []
...
def _segment_worker(
    ...
    z_start, z_end,
    obs_w_active_sets, obs_d_active_sets
):
...
def divide_and_conquer(
    ...
    a, b, Mobs, M, K, r, p, P_list, n_list, n,
    lambdas_w, lambdas_del, lambda_c,
    z_min, z_max, obs_w_active_sets, obs_d_active_sets, num_segments=48
):
    ...
    results = Parallel(n_jobs=n_jobs, backend="loky")(
        delayed(_segment_worker)(
            ...
            a, b, Mobs, M, K, r, p, P_list, n_list, n,
            lambdas_w, lambdas_del, lambda_c,
            seg_start, seg_end, obs_w_active_sets, obs_d_active_sets
        )
...
def fpr_experiment():
    ...
    for n0 in n0_list:
        ...
        trial = 0
        while trial < num_trials and time.time() - start_ < TOTAL :
            ...
            lambdas_w = [np.sqrt(coeff1[n0] * np.log(p) / nk)  for nk in n_list[:K]]
            lambdas_del = [np.sqrt(coeff2[n0] * np.log(p) / n0) ] * K
            lambda_c = np.sqrt(np.log(p) / n_tilde) * coeff3[n0]
            ...
            intervals, oc_intervals = divide_and_conquer(share_XS_list, X_syn_list, X0, unsh_w_hat_list, a, b, M_obs, M, K, r, p, P_list, n_list, n, lambdas_w, lambdas_del, lambda_c, -thresold, thresold, obs_w_active_sets, obs_d_active_sets)
        ...
        if oc_total_false_positives_detected > 0:
            ...
            oc_fpr = 0
        print (f"Trial = {trial}")
        print(f'n0 = {n0}, FPR = {fpr:.4f} - FPR OC = {oc_fpr:.4f}')
        print (f'num err = {num_err}')

        fpr_values[n0] = fpr
        oc_fpr_values[n0] = oc_fpr
...
def tpr_experiment():
    ...
    gamma = 0.2
    n0_list = [200]
    # coeff = {50: 30, 100: 25, 150: 20, 200: 20}
    M = 2
    K = 3
    r = 10
    ...
    for n0 in n0_list:
        ...
        for i in range(3):
            ...
            while trial < num_trials:
                ...
                lambdas_w = [np.sqrt(3 * np.log(p) / nk)  for nk in n_list[:K]]
                lambdas_del = [np.sqrt(1 * np.log(p) / n0) ] * K
                lambda_c = np.sqrt(np.log(p) / n_tilde) * 15 # coeff[n0]
                # Unsharable sources
                unsh_w_hat_list = calculate_unshare_w_hat(unsh_XS_list, unsh_YS_list, lambdas_w[M:])
                n = sum(n_list[: M]) + n0
                Y = np.concatenate(share_YS_list + [Y0])
