...
automl = AutoML(
    ...
    #estimator_list=['histgb','catboost','lgbm', 'lgbm_spark', 'xgb_limitdepth'],#'xgboost',      
    #log_file_name="flaml.log",  # 日志文件名
    max_iter=9999,              # 最大迭代次数         
    seed=42,     
    early_stop=True,   # 启用早停机制
    use_best_model=True,   # 使用交叉验证的最佳模型
    ensemble=True,    # 不使用集成方法
)
automl.fit(X, y, task="regression",metric='mape',time_budget=3600*0.5)
...
test_total_sales_dates["num_sold"] = y_pred
