...
# Instantiate the AlexNet model with pretrained weights
model = alexnet(weights=AlexNet_Weights.DEFAULT)

# Create the backbone (feature extractor) of AlexNet
body = create_body(model, pretrained=True)

# Create the full model with a new head for our dataset
model = nn.Sequential(
    body,
    create_head(num_features_model(body), dls.c)
)
