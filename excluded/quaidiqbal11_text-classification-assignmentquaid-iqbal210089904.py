...
from sklearn.metrics import (
    ...
    hamming_loss,
    accuracy_score  # Accuracy is less informative for multi-label
)
...
if torch.cuda.is_available():
    torch.cuda.manual_seed_all(SEED)  # Only call if CUDA is available
...
# === Adjust this path depending on where you run ===
# On Kaggle this is correct:
DATA_DIR = "/kaggle/input/jigsaw-toxic-comment-classification-challenge"

# If you run locally, change to the folder where you downloaded the files, e.g.:
# DATA_DIR = r"C:/Users/yourname/Downloads/jigsaw-toxic-comment-classification-challenge"

train_path = os.path.join(DATA_DIR, "train.csv.zip")
test_path = os.path.join(DATA_DIR, "test.csv.zip")
sample_sub_path = os.path.join(DATA_DIR, "sample_submission.csv.zip")

# ---- Load data (no dummy fallback) ----
if not (os.path.exists(train_path) and os.path.exists(test_path) and os.path.exists(sample_sub_path)):
    raise FileNotFoundError(
        f"One or more dataset files were not found.\n"
        f"Checked paths:\n  {train_path}\n  {test_path}\n  {sample_sub_path}\n"
        "Please check DATA_DIR or download/attach the dataset."
    )
