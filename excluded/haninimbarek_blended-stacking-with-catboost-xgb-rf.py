...
from sklearn.metrics import log_loss, make_scorer,ConfusionMatrixDisplay
...
# Evaluate
print("ğŸ“Š Train Log Loss (RF):", log_loss(y_train, train_pred_rf))
print("ğŸ“Š Validation Log Loss (RF):", log_loss(y_val, val_pred_rf))
...
train_pred_xgb = xgb_model.predict_proba(X_train)[:, 1]
val_pred_xgb = xgb_model.predict_proba(X_val)[:, 1]

print("ğŸ“Š XGBoost Train Log Loss:", log_loss(y_train, train_pred_xgb))
print("ğŸ“Š XGBoost Validation Log Loss:", log_loss(y_val, val_pred_xgb))
...
print("ğŸ“Š CatBoost Train Log Loss:", log_loss(y_train, train_pred_cb))
print("ğŸ“Š CatBoost Validation Log Loss:", log_loss(y_val, val_pred_cb))


from scipy.optimize import minimize

# 1ï¸�âƒ£ Optimize weights using TRAIN predictions
def loss_func(weights):
    blended_train = (
        weights[0] * train_pred_xgb +
        weights[1] * train_pred_cb +
        weights[2] * train_pred_rf
    )
    return log_loss(y_train, blended_train)

# Equal initial weights
starting_values = [1/3, 1/3, 1/3]
...
# Minimize log loss on TRAIN
res = minimize(loss_func, starting_values, method='SLSQP', bounds=bounds, constraints=cons)

best_weights = res['x']
print(f"âœ… Best Weights (Blending): {best_weights}")
print(f"âœ… Best Train Log Loss: {res['fun']}")

# 2ï¸�âƒ£ Apply best weights on VALIDATION predictions
blended_val = (
    best_weights[0] * val_pred_xgb +
    best_weights[1] * val_pred_cb +
    best_weights[2] * val_pred_rf
)

val_logloss = log_loss(y_val, blended_val)
print(f"ğŸ“Š Validation Log Loss (Blended): {val_logloss}")
...
submission = pd.DataFrame({
    'mint': df_test['mint'],  
    'has_graduated':blended_test_pred
})
