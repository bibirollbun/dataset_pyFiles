...
# do not split
train["label"] = train["rule_violation"].astype(float)
features_cols = ['text','label']
train_ds = Dataset.from_pandas(train[features_cols])
def tokenize(batch):
    return tokenizer(batch["text"], padding="max_length", truncation=True, max_length=256)
train_ds = train_ds.map(tokenize, batched=True)
columns = ['input_ids', 'attention_mask', 'label']
...
fold_aucs = []
for fold, (train_idx, val_idx) in enumerate(skf.split(train_ds, train_ds['label'])):
    print(f"\nğŸ”� Starting fold {fold + 1}/5")

    train_fold = train.iloc[train_idx]
    val_fold = train.iloc[val_idx]

    train_ds = Dataset.from_pandas(train_fold[features_cols])
    val_ds = Dataset.from_pandas(val_fold[features_cols])
    ...
    fold_aucs.append(eval_results['eval_auc'])
    print(f"Fold {fold + 1} AUC: {eval_results['eval_auc']:.4f}")

print("\nğŸ“Š 5-Fold AUCs:", fold_aucs)
