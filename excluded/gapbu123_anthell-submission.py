...
# ───────────────────────────── parameters ─────────────────────────────
MODEL_PATH = "/kaggle/input/inception-2/model_epoch13.pt"
TEST_DIR   = "/kaggle/input/Deepfake_Detection_and_Generation_Challenge_Blue_Team/test_sample_videos"
OUT_CSV    = "/kaggle/working/submission_professor.csv"
IMG_SIZE   = 128
MAX_FRAMES = 32
...
DATASETS = [
    {
        "name": "submission_1.csv",
        "original_dir": "/kaggle/input/test-metaforensic/MetaForensic/0_original",
        "attack_dir": "/kaggle/input/test-metaforensic/MetaForensic/1_deepfake"
    },
    {
        "name": "submission_2.csv",
        "original_dir": "/kaggle/input/test-cliper/Cilper/0_original/original",
        "attack_dir": "/kaggle/input/test-cliper/Cilper/1_deepfake/deepfake"
    },
    {
        "name": "submission_3.csv",
        "original_dir": "/kaggle/input/test-professor/professor/0_original/original",
        "attack_dir": "/kaggle/input/test-professor/professor/1_deepfake/deepfake"
    }
]

def prepare_test_data(original_dir, attack_dir, dataset_name):
    target_dir = Path(f"/kaggle/working/test_videos_{dataset_name}")
    if target_dir.exists():
        shutil.rmtree(target_dir)
    target_dir.mkdir(exist_ok=True)

    def get_all_videos(folder: Path):
        exts = [".mp4", ".avi", ".mov", ".mkv"]
        return [p for p in Path(folder).rglob("*") if p.is_file() and p.suffix.lower() in exts]

    for p in get_all_videos(original_dir):
        shutil.copy(p, target_dir / p.name)
    for p in get_all_videos(attack_dir):
        shutil.copy(p, target_dir / p.name)
...
def main():
    ...
    for idx, dataset in enumerate(DATASETS):
        print(f"⏳ Processing Dataset {idx+1}: {dataset['name']}")
        test_dir = prepare_test_data(dataset["original_dir"], dataset["attack_dir"], f"ds{idx+1}")

        video_paths = [vp for vp in sorted(test_dir.rglob("*")) if vp.suffix.lower() in valid_exts]
        results = []

        for vp in tqdm(video_paths, desc=f"Inference {dataset['name']}"):
            frames = get_frames(vp)
            if not frames:
                results.append({"ID": vp.name, "label": 1})
                continue
            batch = torch.stack([transform(Image.fromarray(f)) for f in frames]).to(device)
            with torch.no_grad():
                logits, _, _ = model(batch, lmbda=0.0)
                preds = logits.argmax(1).cpu().tolist()
            results.append({"ID": vp.name, "label": majority(preds)})

        out_csv = f"/kaggle/working/{dataset['name']}"
