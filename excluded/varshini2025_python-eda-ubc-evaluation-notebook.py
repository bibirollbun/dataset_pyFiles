...
plt.bar(label_df['label'], label_df['label'], color='skyblue')
plt.xlabel('Labels')
plt.ylabel('Count')
...
colors = ['green', 'lightblue', 'red','magenta', 'yellow']
...
plt.show()
assert 'image_id' in df.columns and 'is_tma' in df.columns, "Ensure 'feature1' and 'feature2' columns exist in the dataset"
...
plt.show()
assert 'image_height' in df.columns, "Ensure 'numerical_feature' exists in the dataset"


sns.countplot(data=df, x='label', palette='pastel')
plt.title('Count Plot of label')
plt.show()
assert 'label' in df.columns, "Ensure 'categorical_feature' exists in the dataset"
