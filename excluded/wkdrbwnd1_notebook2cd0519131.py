...
src = "/tmp/extracted.parquet"
dst = "/kaggle/working/extracted.parquet"

if os.path.exists(src):
    shutil.copy(src, dst)
    print(f"✅ {src} → {dst} 복사 완료")
else:
    print(f"❌ {src} 파일이 없습니다. 전처리 단계가 제대로 실행됐는지 확인하세요.")
...
load_path = "/kaggle/working/extracted.parquet"
if not os.path.exists(load_path):
    raise FileNotFoundError(f"{load_path} 파일이 없습니다. 먼저 전처리를 실행해 주세요.")

df = pd.read_parquet(load_path)
print(f"✅ {load_path} 불러오기 완료, shape = {df.shape}")
...
%%writefile /tmp/src/llm_validate_ensemble.py
...
from helpers import (
    get_logger, IS_KAGGLE_SUBMISSION, assume_type, evaluate, is_doi_link
)

l = get_logger()

SYS_PROMPT_CLASSIFY_DOI = """
You are an expert research data curator. Decide whether a referenced DOI refers to a DATASET/REPOSITORY (A) or to LITERATURE/ARTICLE (B).

Rules (highest -> lowest priority):
- If DOI belongs to known data repositories (e.g., Dryad 10.5061, Zenodo 10.5281, Figshare 10.6084, OpenNeuro 10.18112, OSF 10.17605, ArrayExpress 10.1093/nar, PRIDE 10.6019, MetaboLights 10.1093/nar, GEO 10.1093/nar, SRA 10.1093/nar), pick A.
- Context mentions "data deposited", "dataset available", "raw data", "supplementary dataset", "repository", "accession", "Data Availability": prefer A.
- Mentions of "published in", "journal", "methods paper", "review", "article", "preprint", "Nature/Science/PNAS/...": prefer B.
- If ambiguous, prefer B unless repository keywords or known prefixes strongly indicate A.

Output exactly one character without explanation: A or B.
""".strip()

def load_df() -> pl.DataFrame:
    df = pl.read_parquet('/tmp/extracted.parquet')
    # Save non-DOI accession-style predictions as False (type=False)
    df.filter(~is_doi_link('dataset_id')).select('article_id', 'dataset_id').write_csv('/tmp/accid_sub.csv')
    # Only classify DOI-like rows with the LLM
    return df.filter(is_doi_link('dataset_id'))

def build_prompt(tokenizer, doi: str, window: str, sys_prompt: str, short_hint: str = "") -> str:
    messages = [
        {"role": "system", "content": sys_prompt},
        {"role": "user", "content": f"Context:\n{window}\n\nQuestion: Does DOI {doi} point to (A) a dataset/repository, or (B) literature? {short_hint}\nAnswer with just A or B."}
    ]
    return tokenizer.apply_chat_template(messages, add_generation_prompt=True, tokenize=False)

def gather_variants(tokenizer, doi: str, window: str):
    """
    Returns a list of (choices, prompt) variants. We vary minor hints and the choice order
    to mitigate label bias and prompt sensitivity.
    """
    hints = [
        "",
        "If the context includes repository terms (e.g., deposited, dataset, accession), prefer A.",
        "If the context clearly refers to a journal article, prefer B."
    ]
    variants = []
    for h in hints:
        p = build_prompt(tokenizer, doi, window, SYS_PROMPT_CLASSIFY_DOI, short_hint=h)
        variants.append((["A", "B"], p))
        # reversed choice order variant
        p_rev = p  # prompt unchanged; we just reverse the logits mapping
        variants.append((["B", "A"], p_rev))
    return variants

def predict_choices(llm, tokenizer, prompts_with_choices):
    from logits_processor_zoo.vllm import MultipleChoiceLogitsProcessor
    import vllm

    agg = {"A": 0.0, "B": 0.0}
    # Iterate each variant and accumulate logprobs for A/B
    for choices, prompt in prompts_with_choices:
        ...
        lp_dict = {lp.decoded_token: lp.logprob for lp in list(out.outputs[0].logprobs[0].values())}
        # Map back to canonical "A"/"B"
        if choices[0] == "A":
            agg["A"] += lp_dict.get("A", -1e9)
            agg["B"] += lp_dict.get("B", -1e9)
        else:
            # choices == ["B", "A"]
            agg["A"] += lp_dict.get("A", -1e9)
            agg["B"] += lp_dict.get("B", -1e9)

    return "A" if agg["A"] >= agg["B"] else "B", agg

def main():
    os.environ["VLLM_USE_V1"] = "0"
    import vllm

    model_path = os.getenv("MODEL_PATH", "/kaggle/input/qwen2.5/transformers/32b-instruct-awq/1")
    l.info(f"Using model: {model_path}")
    llm = vllm.LLM(
        model=model_path,
        quantization='awq',
        tensor_parallel_size=int(os.getenv("TP_SIZE", "1")),
        max_num_batched_tokens=8192,
        gpu_memory_utilization=float(os.getenv("GPU_UTIL", "0.92")),
        dtype='float16',
        enforce_eager=True,
        max_model_len=4096,
    ...
    df = load_df()
    # Build per-row predictions via prompt variants
    preds = []
    for doi, window in df.select('dataset_id', 'window').rows():
        variants = gather_variants(tokenizer, doi, window)
        choice, agg = predict_choices(llm, tokenizer, variants)
        preds.append(choice)

    # Map to boolean type and write intermediate
    types = {'A': True, 'B': False}
    choices_bool = [types[c] for c in preds]
    df = df.with_columns(pl.Series('type', choices_bool))
    df.filter(pl.col('type')).select('article_id', 'dataset_id').write_csv('/tmp/doi_sub.csv')

    # Merge DOI and accession predictions
    out = pl.concat([pl.read_csv('/tmp/doi_sub.csv'), pl.read_csv('/tmp/accid_sub.csv')])
    out = assume_type(out)
    out.select(['article_id', 'dataset_id', 'type']).with_row_index(name='row_id').write_csv('/kaggle/working/submission.csv')

    if not IS_KAGGLE_SUBMISSION:
        results = evaluate(out)
        for r in results: l.info(r) 
        results = evaluate(out, on=['article_id', 'dataset_id', 'type'])
        for r in results: l.info(r)
