...
# /kaggle/input/1d-cnn-train-full
mod_path1 = '../input/1d-cnn-train-full/'                 # 1D-CNN
mod_path2 = '../input/tabnet-train-full/' # TabNet
mod_path3 = '../input/dnn-train-full/'                       # DNN
...
# 3 models weighted average
sub_all = sub_wct.copy()

# originally 0.6 and 0.4
sub_all[target_cols] = sub_wct[target_cols] * 0.9 + sub_jj[target_cols] * 0.1

# final submission
sub_all.to_csv('submission.csv', index=False)
...
# sub_all[mask] = np.where(mask.values[: None], 0, test_features.values)
