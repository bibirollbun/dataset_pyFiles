...
train_ = pd.read_csv("/kaggle/input/podcast-listening-time-prediction-dataset/podcast_dataset.csv")

train_ = train_.dropna(subset=['Listening_Time_minutes'])
...
df = pd.concat([train, test, train_], axis=0).reset_index(drop=True)
...
# Handle 'Number_of_Ads' (Log transform)
# df['Number_of_Ads'] = np.log1p(df['Number_of_Ads'])

# Handle 'Guest_Popularity_percentage'
df['Has_Guest'] = df['Guest_Popularity_percentage'].apply(lambda x: 1 if pd.notna(x) and x > 0 else 0)
df['Guest_Popularity_percentage'] = df['Guest_Popularity_percentage'].fillna(0) # Impute potential NaNs

# Handle 'Publication_Day'
day_map = {'Monday': 0, 'Tuesday': 1, 'Wednesday': 2, 'Thursday': 3, 'Friday': 4, 'Saturday': 5, 'Sunday': 6}
df['Publication_Dow'] = df['Publication_Day'].map(day_map)
df['Is_Weekend'] = df['Publication_Dow'].apply(lambda d: 1 if d >= 5 else 0)
