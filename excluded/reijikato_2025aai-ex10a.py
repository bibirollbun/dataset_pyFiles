...
# 集約特徴の作成パターンを辞書化
agg_funcs = {
    "purchase_amount": ["min", "max", "mean", "skew", "prod"], # 正規化済みの購入金額（負値 = 顧客の支出）
    "installments":    ["min", "max", "mean", "sem"], # 分割払いの分割回数
    "month_lag":       ["min", "max", "mean", "var"], # 基準日からの経過月数
    "purchase_date":   ["count", "nunique", "size"],      # 購入日時（countで空でない行を数える ⇔ 購入回数）
