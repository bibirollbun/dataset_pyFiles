...
class PawpularModel(nn.Module):
    def __init__(self):
        super().__init__()        
        self.model = timm.create_model("resnet50", pretrained=True, in_chans=3)#resnet50#resnet101#eca_nfnet_l1#res
        
        self.dropout = nn.Dropout(0.5)# increase dropout
        # self.out = nn.Linear(1280+12, 1) spark
...
df = pd.read_csv("/kaggle/input/harshit-goel-10149-creating-folds-pawpulari/train_5folds.csv")
df.head()
