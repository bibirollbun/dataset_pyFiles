...
class Config:
    ...
    pca_variance = 0.6  # Fraction of variance to keep in PCA
...
if config.debug:
    ...
    train = train.sample(n=5000, random_state=0).reset_index(drop=True)
...
def apply_pca(X_train, X_test=None, verbose=True):
    pca = PCA(n_components=config.pca_variance, svd_solver='full', random_state=getattr(config, 'random_state', 31))
...
def get_least_important_features_all_methods(X, y, label, model_name=None):
    ...
    """    
    X_train, X_valid, y_train, y_valid = train_test_split(X, y, test_size=0.02, random_state=config.random_state)
    ...
    if model_type == 'xgb':
        ...
        model = CatBoostRegressor(iterations=2000, learning_rate=0.01, depth=6, loss_function='MAE', eval_metric='MAE', random_seed=config.random_state, verbose=False)
        model.fit(X_train, y_train, eval_set=(X_valid, y_valid), early_stopping_rounds=50, use_best_model=True)
    elif model_type == 'lgbm':
        model = LGBMRegressor(n_estimators=3000, learning_rate=0.01, max_depth=6, reg_lambda=1.0, objective='mae', random_state=config.random_state, verbose=-1, verbosity=-1)
