...
model_weights = {
    "neural_network": 0.464,
    "xgb_quantile_non_creator_early_stopped": -0.291,
    "cb_cox_loss": 0.177,
    "ag_kaplan_meier": 0.147,
    "lgb_goss_quantile": -0.149,
    "neural_network_fe": 0.144,
    "cb_cox_loss_cretor_only": 0.074,
    "histgb_rank_log": -0.103,
    "xgb_nelson_aalen_cretor_only": -0.059,
    "ag_cox": 0.115,
    "cb_cox": -0.104,
    "cb_quantile": 0.095,
    "cb_nelson_aalen_non_creator_early_stopped": -0.081,
    "xgb_nelson_aalen_onehot": 0.074,
    "xgb_quantile_onehot": -0.082,
    "cb_cox_loss_targetencoder": 0.050,
    "histgb_separate": -0.041,
    "xgb_rank_log": 0.050,
    "lgb_quantile_non_creator": -0.104,
    "lgb_goss_rank_log": 0.053,
    "xgb_cox_loss": -0.035,
    "xgb_cox_loss_targetencoder": 0.041,
    "lgb_kaplan_meier_non_creator": -0.067,
    "lgb_kaplan_meier": 0.075,
    "xgb_kaplan_meier_cretor_only": -0.059,
    "lgb_nelson_aalen_cretor_only": 0.037,
    "xgb_cox_loss_non_creator": -0.060,
    "xgb_cox": 0.037,
    "ag_quantile": -0.054,
    "xgb_separate": 0.032,
    "cb_quantile_cretor_only": -0.017,
    "xgb_kaplan_meier_targetencoder": 0.015,
    "xgb_nelson_aalen": -0.008,
    "histgb_quantile": 0.042,
    "histgb_nelson_aalen": -0.034,
    "xgb_cox_loss_onehot": 0.007,
    "xgb_quantile_cretor_only": -0.020,
    "lgb_exponential_rank": 0.017,
    "lgb_quantile": -0.026,
    "lgb_cox": 0.024,
    "xgb_cox_loss_cretor_only": 0.007,
    "cb_quantile_non_creator_early_stopped3": -0.006,
    "lgb_goss_cox": -0.001,
    "lgb_goss_nelson_aalen": 0.001,
    "cb_kaplan_meier": -0.001,
    "histgb_cox": 0.001,
    "xgb_nelson_aalen_non_creator": 0.001
}
...
def objective(trial):
    weights = [trial.suggest_float(f'weight_{name.lower()}', -1.0, 1.0) for name in l2_oof_preds.keys()]
...
l3_weighted_oof_preds = np.zeros((train.shape[0]))
for model, weight in zip(l2_oof_preds.keys(), best_weights):
    l3_weighted_oof_preds += rankdata(l2_oof_preds[model]) * weight

l3_weighted_test_preds = np.zeros((test.shape[0]))
for model, weight in zip(l2_test_preds.keys(), best_weights):
    l3_weighted_test_preds += rankdata(l2_test_preds[model]) * weight


shutil.unpack_archive("/kaggle/input/cibmtr-autogluon-classifier-training/ag_classifier.zip", "/ag_classifier", "zip")
predictor = TabularPredictor.load("/ag_classifier")


ag_clf_oof_preds = joblib.load("/kaggle/input/cibmtr-autogluon-classifier-training/oof_preds.pkl").astype(int).values
combined_oof_preds = np.column_stack((ag_clf_oof_preds, l3_weighted_oof_preds))


y_true = train[["efs", "efs_time", "race_group"]].copy()

def objective(trial):
    threshold = trial.suggest_int('threshold', 0, 50000)
    temp_combined = combined_oof_preds.copy() 
    temp_combined[temp_combined[:, 0] == 1, 1] += threshold
    return score_f(y_true.copy(), temp_combined[:, 1])
...
print(f"Best Threshold: {best_threshold} | Best Score: {study.best_value}")


combined_oof_preds[combined_oof_preds[:, 0] == 1, 1] += best_threshold


masked_ensemble_score = score_f(y_true.copy(), combined_oof_preds[:, 1])
scores["masked_ensemble"] = [masked_ensemble_score] * 10
overall_scores["masked_ensemble"] = masked_ensemble_score

print(f"Masked Ensemble Score: {masked_ensemble_score}")


ag_clf_test_preds = predictor.predict(test).astype(int).values
combined_test_preds = np.column_stack((ag_clf_test_preds, l3_weighted_test_preds))
combined_test_preds[combined_test_preds[:, 0] == 1, 1] += best_threshold
...
sub.prediction = combined_test_preds[:, 1]
