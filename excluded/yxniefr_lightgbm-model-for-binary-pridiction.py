...
# LightGBM model
lightgbm_model = LGBMClassifier(scale_pos_weight=SCALE_POS_WEIGHT)
...
# checking cv_scores
cv_scores = cross_val_score(lightgbm_model, X_train, y_train, cv=skf, scoring='roc_auc')
...
score = roc_auc_score(y_test, lightgbm_pred)
...
def objective(trial):
    ...
    model = LGBMClassifier(**params, scale_pos_weight=1) 

    skf = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
    scores = cross_val_score(model, X_train, y_train, cv=skf, scoring='roc_auc').mean()
...
lightgbm_final = LGBMClassifier(scale_pos_weight=SCALE_POS_WEIGHT, random_state=19, **best_params)

lightgbm_final.fit(X_train, y_train)

y_pred_proba = lightgbm_final.predict_proba(X_test)[:, 1]
roc_auc = roc_auc_score(y_test, y_pred_proba)
print(f'ROC_AUC score is {roc_auc}')


lightgbm_final.fit(X, y)
...
# making predictions
test_predictions = lightgbm_final.predict_proba(test_df)[:, 1]
