...
def rmspe_catboost(y_pred, dtrain):
    ...
    if np.any(non_zero_idx):
        error = np.sqrt(np.mean(((y_true[non_zero_idx] - y_pred[non_zero_idx]) / y_true[non_zero_idx]) ** 2))
    else:
        error = 0.0
    return 'RMSPE', error, False  # False = lower is better



dtrain = Pool(X_train_scaled_es, y_train_es)
dval = Pool(X_val_scaled_es, y_val_es)
...
score = rmspe(y_val_es, y_pred)
print(f'RMSPE: {score:.5f}')
print(f'MSE: {mean_squared_error(y_val_es, y_pred)}')


start = time.time()
dtest = Pool(X_test_scaled_es, label=y_test_es)
y_pred_test = model.predict(dtest)
end = time.time()
print(f'RMSPE: {rmspe(y_test1, y_pred_test)}')
print(f'MSE: {mean_squared_error(y_test1, y_pred_test)}')
print(f'R2_square: {r2_score(y_test1, y_pred_test)}')
...
np.save('predictions_CB.npy', y_pred_test)
