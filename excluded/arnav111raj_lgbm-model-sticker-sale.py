...
print("\nUnique Value Counts:")
print("Countries:", train['country'].nunique())
print("Stores:", train['store'].nunique())
print("Products:", train['product'].nunique())
...
def plot_trends(df):
    ...
    sns.lineplot(data=df, x='date', y='num_sold', ci=None)
...
train = train.drop(columns=[col for col in columns_to_drop if col in train.columns], inplace=False)
test_ids = test['id'] if 'id' in test.columns else None
test = test.drop(columns=[col for col in columns_to_drop if col in test.columns], inplace=False)
# Check and handle missing values in the target variable
if train['num_sold'].isnull().sum() > 0:
    print(f"Missing values in 'num_sold': {train['num_sold'].isnull().sum()}")
    # Option 1: Drop rows with missing 'num_sold'
    train = train.dropna(subset=['num_sold'])
    print("Dropped rows with missing 'num_sold'.")

    # Option 2 (Alternative): Impute missing values
    # imputer = SimpleImputer(strategy='median')
    # train['num_sold'] = imputer.fit_transform(train[['num_sold']])

# Ensure 'y' is clean before training
...
# Load Data
train = pd.read_csv('/kaggle/input/official-data-set/train.csv')
test = pd.read_csv('/kaggle/input/official-data-set/test.csv')
# sample = pd.read_csv('/kaggle/input/official-data-set/sample_submission.csv')
...
final_model = base.Train_ML(param_grid, 'LGBM', e_stop=100, y_log=True, g_col='year')
