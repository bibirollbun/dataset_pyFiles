...
PRETRAINED_DIR = Path("/kaggle/input/cmi-d-111")
print("â–¶ INFERENCE MODE 1,2 â€“ loading artefacts from", PRETRAINED_DIR)
...
print("â–¶ INFERENCE MODE 3 â€“ loading artefacts from", PRETRAINED_DIR)
final_feature_cols = np.load(PRETRAINED_DIR / "feature_cols.npy", allow_pickle=True).tolist()
pad_len        = int(np.load(PRETRAINED_DIR / "sequence_maxlen.npy"))
scaler         = joblib.load(PRETRAINED_DIR / "scaler.pkl")
gesture_classes = np.load(PRETRAINED_DIR / "gesture_classes.npy", allow_pickle=True)
...
def enumerate_weights():
    # list_A = [0.4745, 0.4825, 0.4905, 0.4985, 0.5065, 0.5145, 0.5225, 0.5305]
    # list_B = [0.1725, 0.1795, 0.1865, 0.1935, 0.2005, 0.2074, 0.2145, 0.2215]
    #---------------------------------------------------------------------
...
def predict3(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    # --- 4. ä¸Šä½�ã�«è¿‘ã�„ãƒ©ãƒ³ãƒ€ãƒ ã�ªæ±ºå®šã‚’æŒ‡ç¤ºã�™ã‚‹
        ...
        for j in range(len(list_avgs__sort_at__max_v)):
            for k in range(len(list_avgs__sort_at__cm_123)):
                ...
                if i_cm_12==j_max_v==k_cm_123:
                    list10.append(list_avgs__sort_at__cm_123[k])
                if len(list10)==10: break
            if len(list10)==10: break
        if len(list10)==10: break

    # average vote

    preds = np.array([list10[i]['avg_p'] for i in range(len(list10))])
