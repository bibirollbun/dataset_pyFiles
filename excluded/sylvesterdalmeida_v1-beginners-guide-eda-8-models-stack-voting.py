...
## -- Import datasets --
orig = pd.read_csv("/kaggle/input/extrovert-vs-introvert-behavior-data-backup/personality_dataset.csv")
...
## -- Scatter/KDE plots of train data --
sns.pairplot(data=train, hue='Personality', palette=['c', 'y'])
plt.show()


## -- Histogram plots of train data --
train.hist(figsize=(20, 4), color='c', edgecolor='k', layout=(1, 5))
plt.show()


## -- Test Data distribution --
test.hist(figsize=(20, 4), color='y', edgecolor='k', layout=(1, 5))
plt.show()


## -- Plot numeric features against target --
num_features = [x for x in train.columns if train[x].dtype in ['int', 'float']]

plt.figure(figsize=(18, 10))
for i, col in enumerate(num_features):
    plt.subplot(2, 3, i + 1)
    sns.boxplot(train, x='Personality', y=col, palette=['c','y'])
    plt.title(f"Boxplot of '{col}' by Personality type", fontweight='semibold')
...
sns.countplot(train, x='Personality', ax=ax[0], palette=['r','b'])\
            .set_title('Target Count (Imbalanced)', fontweight='semibold')
sns.countplot(train, x='Stage_fear', hue="Personality", ax=ax[1], palette=['c','y'])\
            .set_title('Stage_Fear by Persnality type', fontweight='semibold')
sns.countplot(train, x='Drained_after_socializing', hue="Personality", ax=ax[2], palette=['c','y'])\
            .set_title('Drained_after_socializing by Personality type', fontweight='semibold')
plt.legend()
plt.tight_layout()
plt.show()


## -- Categorical counts and NaNs --
c=['c','y','lightgrey']
fig, ax = plt.subplots(1, 2, figsize=(20, 5))

## -- Plot % count of values in 'Drained_after_socializing' feature --
train.Drained_after_socializing.value_counts(dropna=False)\
     .plot(kind='pie', autopct='%1.1f%%', colors=c, ax=ax[0], explode=[0, 0,0.2])

## -- Plot % count of values in 'Stage_fear' feature --
train.Stage_fear.value_counts(dropna=False)\
     .plot(kind='pie', ax=ax[1], autopct='%1.1f%%', colors=c,  explode=[0, 0,0.2])

ax[0].set_title('Drained_after_socializing responses')
ax[1].set_title('Stage_fear responses')
plt.legend()
plt.axis('off')
plt.tight_layout()
plt.show()
...
mutual_info_df.plot(kind='barh', color='c',figsize=(10, 4))
plt.title("Univariate relationships: Top predictors of target variable", fontweight='semibold')
plt.tight_layout()
...
models = {
    ...
    'NN_MLP': MLPClassifier(random_state=SEED),
...
xgb.plot_importance(trained_models[0], show_values=False, ax=ax[0], height=.9, importance_type='gain',
                    title=f"{list(models.keys())[0]} - Feature importance")

# -- LightGBM feature importance --
lgb.plot_importance(trained_models[1], ax=ax[1], height=.9,  importance_type='gain',
                    title=f"{list(models.keys())[1]} - Feature importance", precision=0)

# -- CatBoost feature importance --
catboost_importances_df = trained_models[2].get_feature_importance(prettified=True)
sns.barplot(data=catboost_importances_df, x='Importances', y='Feature Id', ax=ax[2], color='steelblue')
ax[2].set_title(f'{list(models.keys())[2]} - Feature importance')

plt.tight_layout()
plt.show()


## -- Make submission for each model prediction --
...
for name, model in tqdm(models.items(), total=len(models), desc="Ensembling Models"):
    print(f"=== Stacking with {name} {'='*50}")
    tik = time.time()
    ...
    s_y_preds = s_model.predict(X_test)
    s_score = accuracy_score(y_test, s_y_preds)

    final_preds = s_model.predict(test)
...
v_y_preds = v_model.predict(X_test)
...
v_final_preds = v_model.predict(test)
