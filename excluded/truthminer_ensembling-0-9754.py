# This Python 3 environment comes with many helpful analytics libraries installed
# It is defined by the kaggle/python Docker image: https://github.com/kaggle/docker-python
# For example, here's several helpful packages to load

import numpy as np # linear algebra
import pandas as pd # data processing, CSV file I/O (e.g. pd.read_csv)

# Input data files are available in the read-only "../input/" directory
# For example, running this (by clicking run or pressing Shift+Enter) will list all files under the input directory

import os
for dirname, _, filenames in os.walk('/kaggle/input'):
    for filename in filenames:
        print(os.path.join(dirname, filename))

# You can write up to 20GB to the current directory (/kaggle/working/) that gets preserved as output when you create a version using "Save & Run All" 
# You can also write temporary files to /kaggle/temp/, but they won't be saved outside of the current session


import os, math, random, gc, json
import numpy as np
import pandas as pd
from itertools import combinations


PATH = "/kaggle/input/playground-series-s5e8/"

# Read data
train = pd.read_csv(f"{PATH}train.csv").set_index('id')
test  = pd.read_csv(f"{PATH}test.csv").set_index('id')
# Give test a dummy y just so counts/merges run (won't be used as labels)
test['y'] = np.random.randint(0, 2, len(test))

# External original dataset (make sure it's attached to your Kaggle session)
orig = pd.read_csv("/kaggle/input/bank-marketing-dataset-full/bank-full.csv", delimiter=";")
orig['y'] = orig['y'].map({'yes': 1, 'no': 0})
orig['id'] = (np.arange(len(orig)) + 1e6).astype('int')
orig = orig.set_index('id')
print("Original data shape", orig.shape)



combine = pd.concat([train, test, orig], axis=0)
print("Combined data shape", combine.shape)

# Identify columns
NUMS = train.select_dtypes(include=['number']).columns.tolist()
# ensure we don't include the target accidentally
NUMS = [c for c in NUMS if c != 'y']
CATS = train.select_dtypes(include=['object']).columns.tolist()

CATS1 = []
SIZES = {}



# Factorize numeric (to create CATS1) and categorical columns, but
# DO NOT overwrite original numeric columns as ints.
for c in NUMS + CATS:
    if c in NUMS:
        n = f"{c}2"
        codes, _ = pd.factorize(combine[c], sort=False)
        combine[n] = codes.astype('int32')
        CATS1.append(n)
        SIZES[n] = int(combine[n].max()) + 1
    else:
        # For categorical strings, keep original column as object (for xgb categorical)
        # and create an integer-coded helper for interaction building too
        n = c
        codes, _ = pd.factorize(combine[c], sort=False)
        combine[n] = codes.astype('int32')  # helper integer codes for interactions/counts
        SIZES[n] = int(combine[n].max()) + 1
print("New CATS1 (from numerics):", CATS1)
print("Cardinality of coded columns:", SIZES)


# Pairwise interaction (hashed) columns over (CATS + CATS1) : all possible combinations
pairs = combinations(CATS + CATS1, 2)
new_cols = {}
CATS2 = []
for c1, c2 in pairs:
    name = "_".join(sorted((c1, c2)))
    new_cols[name] = combine[c1].astype('int32') * SIZES[c2] + combine[c2].astype('int32') # each pair maps to a unique integer.
    CATS2.append(name)

if new_cols:
    new_df = pd.DataFrame(new_cols, index=combine.index)
    combine = pd.concat([combine, new_df], axis=1)

print(f"Created {len(CATS2)} new CAT interaction columns")



# Category frequency count encoding (CE_)
CE = []
CC_all = CATS + CATS1 + CATS2
combine['i'] = np.arange(len(combine), dtype=np.int64)

print(f"Processing {len(CC_all)} columns for counts... ", end="")
for i, c in enumerate(CC_all):
    if i % 10 == 0:
        print(f"{i}, ", end="")
    tmp = combine.groupby(c, observed=False)['y'].count().astype('int32')
    tmp.name = f"CE_{c}"
    CE.append(tmp.name)
    combine = combine.merge(tmp, left_on=c, right_index=True, how='left')
print()


# Split back
train = combine.iloc[:len(train)].copy()
test  = combine.iloc[len(train):len(train)+len(test)].copy()
orig  = combine.iloc[-len(orig):].copy()
del combine
gc.collect()

print("Train shape", train.shape, "Test shape", test.shape, "Original shape", orig.shape)


# Target encoding using ORIG as reference (TE_ORIG_)
TE = []
CC_for_TE = CATS + CATS1 + CATS2
print(f"Processing {len(CC_for_TE)} columns for TE_ORIG_... ", end="")
for i, c in enumerate(CC_for_TE):
    if i % 10 == 0:
        print(f"{i}, ", end="")
    tmp = orig.groupby(c, observed=False)['y'].mean().astype('float32')
    tmp.name = f"TE_ORIG_{c}"
    TE.append(tmp.name)
    # merge left_on=c with right_index mapping
    train = train.merge(tmp, left_on=c, right_index=True, how='left')
    test  = test.merge(tmp,  left_on=c, right_index=True, how='left')
print()



# =========================
# XGBoost (GPU) - with fold target encoding for CATS1+CATS2
# =========================
from sklearn.model_selection import KFold
import xgboost as xgb
from sklearn.metrics import roc_auc_score

print(f"XGBoost version {xgb.__version__}")

FEATURES = NUMS + CATS + CATS1 + CATS2 + CE
print(f"We have {len(FEATURES)} features for the first XGB.")

FOLDS = 10
SEED = 42

# robust GPU config across xgb versions
def xgb_gpu_params(seed):
    major = int(xgb.__version__.split('.')[0])
    params = {
        "objective": "binary:logistic",
        "eval_metric": "auc",
        "learning_rate": 0.02,
        "max_depth": 0,
        "subsample": 0.8,
        "colsample_bytree": 0.7,
        "seed": seed,
        "grow_policy": "lossguide",
        "max_leaves": 32,
        "alpha": 2.0,
    }
    if major >= 2:
        params["device"] = "cuda"
    else:
        params["tree_method"] = "gpu_hist"
    return params

class IterLoadForDMatrix(xgb.core.DataIter):
    """CPU Pandas iterator feeding to XGBoost (QuantileDMatrix or DMatrix)."""
    def __init__(self, df=None, features=None, target=None, batch_size=256 * 1024):
        super().__init__()
        self.features = features
        self.target = target
        self.df = df
        self.it = 0
        self.batch_size = batch_size
        self.batches = int(np.ceil(len(df) / self.batch_size))

    def reset(self):
        self.it = 0

    def next(self, input_data):
        if self.it == self.batches:
            return 0
        a = self.it * self.batch_size
        b = min((self.it + 1) * self.batch_size, len(self.df))
        dt = self.df.iloc[a:b]
        input_data(data=dt[self.features], label=dt[self.target])
        self.it += 1
        return 1

def target_encode_inplace(train_df, valid_df, test_df, col, ycol='y'):
    """Fold-safe target encoding: fit on train_df only; map to train/valid/test."""
    mapping = train_df.groupby(col, observed=False)[ycol].mean()
    gmean   = train_df[ycol].mean()
    train_df[col] = train_df[col].map(mapping).fillna(gmean).astype('float32')
    valid_df[col] = valid_df[col].map(mapping).fillna(gmean).astype('float32')
    test_df[col]  = test_df[col].map(mapping).fillna(gmean).astype('float32')

oof_preds = np.zeros(len(train), dtype=np.float32)
test_preds = np.zeros(len(test), dtype=np.float32)




REPEATS = 1  # increase if you want
for kk in range(REPEATS):
    kf = KFold(n_splits=FOLDS, shuffle=True, random_state=SEED)
    for fold, (train_idx, val_idx) in enumerate(kf.split(train), start=1):
        print("#" * 25)
        print(f"### REPEAT {kk+1}, Fold {fold}/{FOLDS} ###")
        print("#" * 25)

        Xy_train = train.iloc[train_idx][FEATURES + ['y']].copy()
        # Add extra original rows as you did
        Xy_more  = orig[FEATURES + ['y']]
        Xy_train = pd.concat([Xy_train, Xy_more], axis=0, ignore_index=True)

        X_valid = train.iloc[val_idx][FEATURES].copy()
        y_valid = train.iloc[val_idx]['y'].copy()
        X_test  = test[FEATURES].copy()

        # Target-encode CATS1 + CATS2 per fold (no external libs)
        CC_fold_TE = CATS1 + CATS2
        print(f"Target encoding {len(CC_fold_TE)} features... ", end="")
        for i, c in enumerate(CC_fold_TE):
            if i % 10 == 0:
                print(f"{i}, ", end="")
            target_encode_inplace(Xy_train, X_valid, X_test, c, ycol='y')
        print()

        # Cast true categoricals for XGBoost categorical splits
        for c in CATS:
            Xy_train[c] = Xy_train[c].astype('category')
            X_valid[c]  = X_valid[c].astype('category')
            X_test[c]   = X_test[c].astype('category')

        # Build DMatrix (QuantileDMatrix if available)
        use_qdm = hasattr(xgb, "QuantileDMatrix")
        params = xgb_gpu_params(seed=kk * FOLDS + fold)

        if use_qdm:
            dtrain = xgb.QuantileDMatrix(IterLoadForDMatrix(Xy_train, FEATURES, 'y'),
                                         enable_categorical=True, max_bin=256)
        else:
            dtrain = xgb.DMatrix(Xy_train[FEATURES], label=Xy_train['y'], enable_categorical=True)

        dval  = xgb.DMatrix(X_valid, label=y_valid, enable_categorical=True)
        dtest = xgb.DMatrix(X_test, enable_categorical=True)

        model = xgb.train(
            params=params,
            dtrain=dtrain,
            num_boost_round=100_000,
            evals=[(dtrain, "train"), (dval, "valid")],
            early_stopping_rounds=250,
            verbose_eval=500
        )

        oof_preds[val_idx] += model.predict(dval, iteration_range=(0, model.best_iteration + 1)) / REPEATS
        test_preds += model.predict(dtest, iteration_range=(0, model.best_iteration + 1)) / (FOLDS * REPEATS)



from sklearn.metrics import roc_auc_score
m = roc_auc_score(train['y'].to_numpy(), oof_preds)
print(f"XGB (Train More) with Original Data as rows CV = {m}")



# =========================
# Second XGB using TE_ORIG_ features added
# =========================
FEATURES = NUMS + CATS + CATS1 + CE + TE
print(f"We have {len(FEATURES)} features for second XGB.")

oof_preds2 = np.zeros(len(train), dtype=np.float32)
test_preds2 = np.zeros(len(test), dtype=np.float32)

REPEATS = 1
for kk in range(REPEATS):
    kf = KFold(n_splits=FOLDS, shuffle=True, random_state=SEED)
    for fold, (train_idx, val_idx) in enumerate(kf.split(train), start=1):
        print("#" * 25)
        print(f"### REPEAT {kk+1}, Fold {fold}/{FOLDS} ###")
        print("#" * 25)

        Xy_train = train.iloc[train_idx][FEATURES + ['y']].copy()
        X_valid  = train.iloc[val_idx][FEATURES].copy()
        y_valid  = train.iloc[val_idx]['y'].copy()
        X_test   = test[FEATURES].copy()

        # Target encode (again) the interaction codes based on the training fold only
        CC_fold_TE = CATS1 + CATS2  # if you want to include, keep; else remove to rely only on TE_ORIG_
        print(f"Target encoding {len(CC_fold_TE)} features... ", end="")
        for i, c in enumerate(CC_fold_TE):
            if c in Xy_train.columns:  # some c might not be in this FEATURES set; guard
                if i % 10 == 0:
                    print(f"{i}, ", end="")
                target_encode_inplace(Xy_train, X_valid, X_test, c, ycol='y')
        print()

        for c in CATS:
            if c in Xy_train.columns:
                Xy_train[c] = Xy_train[c].astype('category')
                X_valid[c]  = X_valid[c].astype('category')
                X_test[c]   = X_test[c].astype('category')

        use_qdm = hasattr(xgb, "QuantileDMatrix")
        params = xgb_gpu_params(seed=kk * FOLDS + fold)

        if use_qdm:
            dtrain = xgb.QuantileDMatrix(IterLoadForDMatrix(Xy_train, FEATURES, 'y'),
                                         enable_categorical=True, max_bin=256)
        else:
            dtrain = xgb.DMatrix(Xy_train[FEATURES], label=Xy_train['y'], enable_categorical=True)

        dval  = xgb.DMatrix(X_valid, label=y_valid, enable_categorical=True)
        dtest = xgb.DMatrix(X_test, enable_categorical=True)

        model = xgb.train(
            params=params,
            dtrain=dtrain,
            num_boost_round=100_000,
            evals=[(dtrain, "train"), (dval, "valid")],
            early_stopping_rounds=250,
            verbose_eval=500
        )

        oof_preds2[val_idx] += model.predict(dval, iteration_range=(0, model.best_iteration + 1)) / REPEATS
        test_preds2 += model.predict(dtest, iteration_range=(0, model.best_iteration + 1)) / (FOLDS * REPEATS)

m2 = roc_auc_score(train['y'].to_numpy(), oof_preds2)
print(f"XGB (Cols) with TE_ORIG rows CV = {m2}")

# Fill NaNs in TE_ORIG_* for NN later
mn_global = orig['y'].mean()
for c in (CATS + CATS1 + CATS2):
    NAME = f"TE_ORIG_{c}"
    if NAME in train.columns:
        train[NAME] = train[NAME].fillna(mn_global)
        test[NAME]  = test[NAME].fillna(mn_global)



# =========================
# Log-transform for LOG + CE
# =========================
LOG = ['balance', 'duration', 'campaign', 'pdays', 'previous']
for c in LOG + CE:
    if c in train.columns:
        if c in LOG:
            mn = min(train[c].min(), test[c].min())
            train[c] = train[c] - mn
            test[c]  = test[c] - mn
        train[c] = np.log1p(train[c])
        test[c]  = np.log1p(test[c])



# =========================
# PyTorch MLP with categorical embeddings
# =========================
import torch
import torch.nn as nn
import torch.nn.functional as F
from torch.utils.data import Dataset, DataLoader
from sklearn.preprocessing import LabelEncoder, StandardScaler
from sklearn.metrics import roc_auc_score, log_loss, accuracy_score

TARGET_COL = 'y'
FEATURES = CATS + NUMS + CATS1 + CE + TE
print(f"We have {len(FEATURES)} features for NN.")

SEED          = 42
FOLDS         = 10
EPOCHS        = 4
BATCH_SIZE    = 512
LR_MAX        = 3e-3
WD            = 1e-4
EARLY_STOP    = 4
GRAD_CLIP     = 1.0
NUM_WORKERS   = 4
MODEL_DIR     = "./mlp_catemb_models"
OOF_PATH      = "./oof_catemb.csv"
os.makedirs(MODEL_DIR, exist_ok=True)

KNOWN_CARDINALITIES = SIZES  # from earlier

df  = train[FEATURES + ['y']].copy()
df2 = test[FEATURES + ['y']].copy()

def seed_everything(seed=SEED):
    random.seed(seed)
    np.random.seed(seed)
    torch.manual_seed(seed)
    torch.cuda.manual_seed_all(seed)
    torch.backends.cudnn.deterministic = False
    torch.backends.cudnn.benchmark = True

seed_everything(SEED)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print("Device:", device)

categorical_cols = list(KNOWN_CARDINALITIES.keys())
categorical_cols = [c for c in categorical_cols if c in df.columns]
numeric_cols = [c for c in df.columns if c not in categorical_cols + [TARGET_COL]]

print(f"#categoricals={len(categorical_cols)}, #numerics={len(numeric_cols)}")




# Label-encode categoricals (reserve 0 = UNK)
encoders = {}
for col in categorical_cols:
    le = LabelEncoder()
    tmp = pd.concat([df[[col]], df2[[col]]], axis=0)
    tmp[col] = tmp[col].astype(str).fillna("NaN")
    le.fit(tmp[col].values)
    df[col]  = le.transform(df[col].astype(str).values) + 1
    df2[col] = le.transform(df2[col].astype(str).values) + 1
    encoders[col] = le
    del tmp

cardinalities = {}
for col in categorical_cols:
    if col in KNOWN_CARDINALITIES:
        cardinalities[col] = KNOWN_CARDINALITIES[col] + 1  # +1 for UNK
        df[col]  = np.clip(df[col],  0, KNOWN_CARDINALITIES[col])
        df2[col] = np.clip(df2[col], 0, KNOWN_CARDINALITIES[col])
    else:
        cardinalities[col] = int(max(df[col].max(), df2[col].max())) + 1

# Scale numerics
scaler = StandardScaler()
if numeric_cols:
    df[numeric_cols]  = scaler.fit_transform(df[numeric_cols].astype(np.float32))
    df2[numeric_cols] = scaler.transform(df2[numeric_cols].astype(np.float32))

# Embedding sizes
def emb_dim_from_card(n):
    return int(min(50, round(1.6 * (n ** 0.56))))

emb_info = [(cardinalities[c], emb_dim_from_card(cardinalities[c])) for c in categorical_cols]
total_emb_dim = sum(d for _, d in emb_info)
print("Embedding config:", dict(zip(categorical_cols, emb_info)))
print("Total embedding dim:", total_emb_dim, " + numeric:", len(numeric_cols))

class TabDataset(Dataset):
    def __init__(self, df, cat_cols, num_cols, target_col=None, idx=None):
        self.cat_cols = cat_cols
        self.num_cols = num_cols
        self.target_col = target_col
        self.df = df if idx is None else df.iloc[idx]
        self.cats = self.df[self.cat_cols].values.astype(np.int64) if self.cat_cols else np.zeros((len(self.df),0), np.int64)
        self.nums = self.df[self.num_cols].values.astype(np.float32) if self.num_cols else np.zeros((len(self.df),0), np.float32)
        self.y = None if self.target_col is None else self.df[self.target_col].values.astype(np.float32)

    def __len__(self):
        return len(self.df)

    def __getitem__(self, i):
        cats = torch.from_numpy(self.cats[i])
        nums = torch.from_numpy(self.nums[i])
        if self.y is None:
            return cats, nums
        return cats, nums, torch.tensor(self.y[i])

class MLPWithCatEmb(nn.Module):
    def __init__(self, emb_info, n_num, hidden=[512, 256, 128], dropout=0.15):
        super().__init__()
        self.emb_layers = nn.ModuleList(
            [nn.Embedding(num_embeddings=card, embedding_dim=dim, padding_idx=0)
             for card, dim in emb_info]
        )
        emb_total = sum(dim for _, dim in emb_info)
        in_dim = emb_total + n_num

        self.bn_nums = nn.BatchNorm1d(n_num) if n_num > 0 else nn.Identity()
        self.dropout = nn.Dropout(dropout)

        layers = []
        last = in_dim
        for h in hidden:
            layers += [
                nn.Linear(last, h),
                nn.BatchNorm1d(h),
                nn.SiLU(),
                nn.Dropout(dropout)
            ]
            last = h
        self.mlp = nn.Sequential(*layers)
        self.head = nn.Linear(last, 1)

        for m in self.modules():
            if isinstance(m, nn.Linear):
                nn.init.kaiming_normal_(m.weight, nonlinearity='relu')
                if m.bias is not None:
                    nn.init.zeros_(m.bias)

    def forward(self, x_cat, x_num):
        if len(self.emb_layers) > 0:
            emb = [emb_layer(x_cat[:, i]) for i, emb_layer in enumerate(self.emb_layers)]
            x_emb = torch.cat(emb, dim=1)
        else:
            x_emb = None

        if x_num is not None and x_num.shape[1] > 0:
            x_num = self.bn_nums(x_num)
            x = torch.cat([x_emb, x_num], dim=1) if x_emb is not None else x_num
        else:
            x = x_emb
        x = self.mlp(x)
        logit = self.head(x).squeeze(1)
        return logit




def train_one_epoch(model, loader, optimizer, scaler, scheduler=None):
    model.train()
    running = 0.0
    for cats, nums, y in loader:
        cats = cats.to(device, non_blocking=True)
        nums = nums.to(device, non_blocking=True)
        y = y.to(device, non_blocking=True)

        optimizer.zero_grad(set_to_none=True)
        with torch.cuda.amp.autocast(enabled=True):
            logits = model(cats, nums)
            loss = F.binary_cross_entropy_with_logits(logits, y)
        scaler.scale(loss).backward()
        if GRAD_CLIP is not None:
            scaler.unscale_(optimizer)
            torch.nn.utils.clip_grad_norm_(model.parameters(), GRAD_CLIP)
        scaler.step(optimizer)
        scaler.update()
        if scheduler is not None:
            scheduler.step()

        running += loss.detach().item() * y.size(0)
    return running / len(loader.dataset)

@torch.no_grad()
def validate(model, loader):
    model.eval()
    all_logits, all_y = [], []
    for batch in loader:
        if len(batch) == 3:
            cats, nums, y = batch
            all_y.append(y.detach().cpu())
        else:
            cats, nums = batch
        cats = cats.to(device, non_blocking=True)
        nums = nums.to(device, non_blocking=True)
        all_logits.append(model(cats, nums).detach().cpu())

    logits = torch.cat(all_logits).numpy()
    probs  = 1.0 / (1.0 + np.exp(-logits))
    probs_c = np.clip(probs, 1e-7, 1 - 1e-7)

    if all_y:
        y_true = torch.cat(all_y).numpy().astype(np.int64)
        auc = roc_auc_score(y_true, probs)
        ll  = log_loss(y_true, probs_c)
        acc = accuracy_score(y_true, (probs >= 0.5).astype(int))
        return probs, {"auc": auc, "logloss": ll, "acc": acc}
    return probs, {}

oof = np.zeros(len(df), dtype=np.float32)
preds = np.zeros(len(df2), dtype=np.float32)

REPEATS = 1  # set back to 5 if runtime allows
for kk in range(REPEATS):
    print(f"##### REPEAT {kk+1} of {REPEATS} #####")
    seed_everything(SEED + kk)

    skf = KFold(n_splits=FOLDS, shuffle=True, random_state=SEED)
    y = df[TARGET_COL].astype(int).values
    oof2 = np.zeros(len(df), dtype=np.float32)
    fold_metrics = []

    for fold, (trn_idx, val_idx) in enumerate(skf.split(df, y), start=1):
        print(f"\n========== Fold {fold}/{FOLDS} ==========")
        trn_ds = TabDataset(df, categorical_cols, numeric_cols, TARGET_COL, trn_idx)
        val_ds = TabDataset(df, categorical_cols, numeric_cols, TARGET_COL, val_idx)
        test_ds = TabDataset(df2, categorical_cols, numeric_cols, TARGET_COL, np.arange(len(df2)))

        trn_loader = DataLoader(trn_ds, batch_size=BATCH_SIZE, shuffle=True,
                                num_workers=NUM_WORKERS, pin_memory=True, drop_last=True)
        val_loader = DataLoader(val_ds, batch_size=BATCH_SIZE, shuffle=False,
                                num_workers=NUM_WORKERS, pin_memory=True)
        test_loader = DataLoader(test_ds, batch_size=BATCH_SIZE, shuffle=False,
                                 num_workers=NUM_WORKERS, pin_memory=True)

        model = MLPWithCatEmb(emb_info=emb_info, n_num=len(numeric_cols)).to(device)
        optimizer = torch.optim.AdamW(model.parameters(), lr=LR_MAX, weight_decay=WD)

        total_steps = EPOCHS * len(trn_loader)
        scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer, max_lr=LR_MAX, total_steps=total_steps,
            pct_start=0.0, div_factor=25.0, final_div_factor=10.5, anneal_strategy='cos'
        )

        scaler = torch.cuda.amp.GradScaler(enabled=True)

        best_path = os.path.join(MODEL_DIR, f"fold{fold}.pt")
        best_auc = -1.0
        best_epoch = -1
        epochs_no_improve = 0

        for epoch in range(1, EPOCHS + 1):
            train_loss = train_one_epoch(model, trn_loader, optimizer, scaler, scheduler)
            _, val_stats = validate(model, val_loader)
            auc = val_stats.get("auc", float("nan"))
            print(f"Epoch {epoch:02d}: train_loss={train_loss:.4f} | "
                  f"val_auc={auc:.5f} val_logloss={val_stats['logloss']:.5f} val_acc={val_stats['acc']:.4f}")

            # Always save latest as in your code; toggle condition if you want strict early stop
            best_auc = auc
            best_epoch = epoch
            epochs_no_improve = 0
            torch.save({
                "model_state": model.state_dict(),
                "config": {
                    "emb_info": emb_info,
                    "numeric_cols": numeric_cols,
                    "categorical_cols": categorical_cols
                }
            }, best_path)

        ckpt = torch.load(best_path, map_location="cpu")
        model.load_state_dict(ckpt["model_state"])
        model.to(device)

        val_probs, val_stats = validate(model, val_loader)
        oof[val_idx] += val_probs / REPEATS
        oof2[val_idx] = val_probs
        print(f"[Fold {fold}] AUC={val_stats['auc']:.5f}  LogLoss={val_stats['logloss']:.5f}  Acc={val_stats['acc']:.4f}")

        test_probs, _ = validate(model, test_loader)
        preds += test_probs / (FOLDS * REPEATS)

    oof_c = np.clip(oof2, 1e-7, 1 - 1e-7)
    oof_auc = roc_auc_score(y, oof2)
    oof_ll  = log_loss(y, oof_c)
    oof_acc = accuracy_score(y, (oof2 >= 0.5).astype(int))
    print("\n========== Overall OOF ==========")
    print(f"OOF AUC={oof_auc:.5f}  LogLoss={oof_ll:.5f}  Acc={oof_acc:.4f}")

m_nn = roc_auc_score(train['y'].to_numpy(), oof)
print(f"NN (Train More) CV = {m_nn}")




# =========================
# Make submissions
# =========================
sub = pd.read_csv(f"{PATH}sample_submission.csv")
sub['y'] = preds
sub.to_csv("submission_nn_train_more.csv", index=False)
np.save("oof_nn_train_more", oof)
print('NN Submission shape', sub.shape)




preds_xgb = (test_preds + test_preds2) / 2.0
sub = pd.read_csv(f"{PATH}sample_submission.csv")
sub['y'] = preds_xgb
sub.to_csv("submission_xgb_train_more.csv", index=False)
np.save("oof_xgb_rows_train_more", oof_preds)
np.save("oof_xgb_cols_train_more", oof_preds2)
print('XGB Submission shape', sub.shape)





oof_xgb = (oof_preds + oof_preds2) / 2.0
m_xgb = roc_auc_score(train['y'].to_numpy(), oof_xgb)
print(f"Both XGB rows and XGB cols (Train More) CV = {m_xgb}")




best_m = 0
best_w = 0
for w in np.arange(0, 1.01, 0.01):
    oof_ensemble = (1 - w) * oof_xgb + w * oof
    m = roc_auc_score(train['y'].to_numpy(), oof_ensemble)
    if m > best_m:
        best_m = m
        best_w = w

oof_ensemble = (1 - best_w) * oof_xgb + best_w * oof
m_ens = roc_auc_score(train['y'].to_numpy(), oof_ensemble)
print(f"Ensemble XGB and NN (Train More) CV = {m_ens}")
print(f" using best NN weight = {best_w}")

xgb_preds = preds_xgb
sub = pd.read_csv(f"{PATH}sample_submission.csv")
sub['y'] = (1 - best_w) * xgb_preds + best_w * preds
sub.to_csv("submission_ensemble_train_more.csv", index=False)
print('Ensemble Submission shape', sub.shape)
print(sub.head())



















