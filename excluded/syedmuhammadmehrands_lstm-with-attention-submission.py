...
model = Model_Architecture(input_dim=332, hidden_dim=128, output_dim=18, num_layers=3, sequence_len=SEQUENCE_LENGTH)
model.load_state_dict(torch.load("/kaggle/input/vanilla_lstm/pytorch/default/5/model_weights.pth", map_location=device))
model.to(device)
model.eval()


scaler = joblib.load("/kaggle/input/vanilla_lstm/pytorch/default/5/scaler.pkl")
label_encoder = joblib.load("/kaggle/input/vanilla_lstm/pytorch/default/5/label_encoder.pkl")
...
def predict(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    length = min(SEQUENCE_LENGTH, len(X))
    padded[:length] = X[:length]

    # Convert to tensor
    input_tensor = torch.tensor(padded, dtype=torch.float32).unsqueeze(0).to(device)  # shape (1, seq_len, input_dim)
