...
MODEL_PATH = "/kaggle/input/cmi3-bert-model/20250819T012204"
...
class CMIFeDataset(Dataset):
    ...
    def get_scaled_nan_tensors(self, imu, thm, tof):
        ...
        # torch.full(thm.shape, self.thm_scaled_nan, device=thm.device), \
        # torch.full(tof.shape, self.tof_scaled_nan, device=tof.device)
        return torch.zeros_like(imu), torch.zeros_like(thm), torch.zeros_like(tof)
...
class CMIModel(nn.Module):
    ...
    def forward(self, imu, thm, tof):
        ...
        # NEW: hard-gate branches when their inputs are all zeros (per sample)
        thm_mask = (thm.abs().sum(dim=(1,2)) > 0).float().view(-1, 1, 1)
        tof_mask = (tof.abs().sum(dim=(1,2)) > 0).float().view(-1, 1, 1)
        thm_feat = thm_feat * thm_mask
        tof_feat = tof_feat * tof_mask
    
        bert_input = torch.cat([imu_feat, thm_feat, tof_feat], dim=-1).permute(0, 2, 1)
        cls_token = self.cls_token.expand(bert_input.size(0), -1, -1)
        bert_input = torch.cat([cls_token, bert_input], dim=1)
        pred_cls = self.bert(inputs_embeds=bert_input).last_hidden_state[:, 0, :]
        pred_cls = F.layer_norm(pred_cls, pred_cls.shape[1:])
        return self.classifier(pred_cls)
...
def imu_only_augment(imu, thm, tof, p):
    """
...
import inspect
def load_ckpt(model_fn, model_args, path, device="cpu", prefer_ckpt_args=True, strict=False):
    ckpt = torch.load(path, map_location=device)

    # Merge saved args (if any) with caller args; caller wins on conflicts
    saved_args = ckpt.get("model_args") if isinstance(ckpt, dict) else {}
    merged_args = {**(saved_args or {}), **model_args} if prefer_ckpt_args else dict(model_args)

    # If the model has **kwargs, DO NOT prune unknown keys (they're meant to go into **kwargs).
    sig = inspect.signature(model_fn)
    has_varkw = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values())
    if not has_varkw:
        valid = {p.name for p in sig.parameters.values()
                 if p.kind in (inspect.Parameter.POSITIONAL_OR_KEYWORD, inspect.Parameter.KEYWORD_ONLY)}
        merged_args = {k: v for k, v in merged_args.items() if k in valid}

    # Ensure required dims are present
    for req in ("imu_dim", "thm_dim", "tof_dim", "n_classes"):
        if req not in merged_args:
            raise ValueError(f"[load_ckpt] Missing required arg '{req}'. Pass it via model_args.")

    # Build model
    model = model_fn(**merged_args).to(device)

    # Extract a state_dict from the checkpoint
    sd = None
    if isinstance(ckpt, dict):
        for k in ("state_dict","model_state_dict","ema_state_dict","model","net","weights","params"):
            if isinstance(ckpt.get(k), dict):
                sd = ckpt[k]; break
        if sd is None and ckpt and all(isinstance(v, torch.Tensor) for v in ckpt.values()):
    ...
    if sd is None:
        raise RuntimeError("[load_ckpt] Could not find a state_dict in the checkpoint.")

    # Clean common prefixes and load
    def clean(k):
    ...
    filt = {k: v for k, v in sd.items() if k in target and v.shape == target[k].shape}
    missing, unexpected = model.load_state_dict(filt, strict=strict)
    if missing or unexpected or len(filt) != len(sd):
        print(f"[load_ckpt] loaded {len(filt)}/{len(sd)} keys")
        if missing:    print("[load_ckpt] missing (sample):", list(missing)[:10])
        if unexpected: print("[load_ckpt] unexpected (sample):", list(unexpected)[:10])
...
if infer:
    ...
    model_dicts = [
        {
            ...
            "model_args": model_args,
            # "model_path": model_dir / f"fold{fold}/best_ema.pt",
            "model_path": model_dir / f"best_model_fold{fold}.pt",
        } for fold in range(n_folds)
        # } for fold in range(1)
    ]
    ...
    for model_dict in model_dicts:
        ...
        model = load_ckpt(
            CMIModel, model_args, str(model_path),
            device=CUDA0,
            prefer_ckpt_args=False,   # set False if you changed dims/classes after training
            strict=False
        )
...
def avg_predict2(models, imu, thm, tof):
    outputs = []
    with autocast(device_type='cuda'):
        for model in models:
            logits = model(imu, thm, tof)
...
def predict4(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    with torch.no_grad():
        imu, thm, tof = to_cuda(imu, thm, tof)
        logits = avg_predict2(models, imu, thm, tof)
        probabilities = F.softmax(logits, dim=1).cpu().numpy()
...
def predict(sequence, demographics):
    # Get per-model class-score vectors (shape: [K])
    ...
    # sanity check
    K = len(pred0)
    assert all(len(p) == K for p in (pred1, pred2, pred3)), "Model outputs must have same length."

    # m_w = [0.235039, 0.300954, 0.331310, 0.132697]   # sums to 1
    m_w = [0.17, 0.25, 0.33, 0.25]   # sums to 1
    da_w = [0.70, 0.30]                               # sums to 1
    c_w  = [0.001075, -0.001575, -0.002575, 0.003075] # sums to 0

    m_wts = np.asarray(m_w, dtype=float)
    preds = []  # fused score per class
    ...
    for a, b, c, d in zip(pred0, pred1, pred2, pred3):
        ...
        # rank ascending & descending by per-class score
        l_asc  = sorted(copy.deepcopy(l_abcd), key=lambda x: x['pred'])
        l_desc = sorted(copy.deepcopy(l_abcd), key=lambda x: x['pred'], reverse=True)

        # apply rank-based corrections
        for item, corr in zip(l_asc,  c_w): item['res'] = item['pred'] * (item['wts'] + corr)
        for item, corr in zip(l_desc, c_w): item['res'] = item['pred'] * (item['wts'] + corr)

        # BUG FIX here ↓
        result_asc  = sum(item['res'] for item in l_asc)
        result_desc = sum(item['res'] for item in l_desc)

        fused = da_w[0] * result_asc + da_w[1] * result_desc
        preds.append(fused)

    fused_scores = np.asarray(preds)  # length K
    return classes[int(fused_scores.argmax())]
