...
def preprocess_data(df):
    ...
    def parse_engine(s):
        hp = np.nan; L = np.nan
        if isinstance(s, str):
            m = re.search(r'(\d+\.?\d*)HP', s)
            hp = float(m.group(1)) if m else np.nan
            n = re.search(r'(\d+\.?\d*)L', s)
            L  = float(n.group(1)) if n else np.nan
        return pd.Series({'engine_HP': hp, 'engine_Liters': L})
    eng = df['engine'].apply(parse_engine)
    ...
    for col in ['ext_col', 'int_col']:
        if col in df.columns:
            ...
            df[f'{col}_is_missing'] = raw.isna().astype(int)
