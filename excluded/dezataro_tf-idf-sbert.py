...
char_vec = TfidfVectorizer(
    analyzer='char',ngram_range = (3,5),min_df = 2,max_features=30000,sublinear_tf = True,lowercase=True
)
word_vec = TfidfVectorizer(
    analyzer='word',ngram_range = (1,2),min_df=2,max_df=0.95,max_features=20000,sublinear_tf = True,lowercase = True,stop_words='english'
)
...
def diag_cos(a,b):
    ...
    den = (na*nb)
    return np.divide(np.asarray(num).ravel(),
...
sim_vec = TfidfVectorizer(
    analyzer='word',ngram_range=(1,3),min_df=2,max_df=0.95,
    max_features=20000,sublinear_tf=True,lowercase=True,stop_words='english'
)

sim_vec.fit(pd.concat([
    train['body'].fillna(''),
    train['positive_example_1'].fillna(''),train['positive_example_2'].fillna(''),
    train['negative_example_1'].fillna(''),train['negative_example_2'].fillna('')
],axis=0))
for fold,(trn_idx,val_idx) in enumerate(skf.split(X_train,y_train,groups = train['rule'] ),1):
    ...
    subrule_stats['smoothed_te'] = ((subrule_stats['mean'] * subrule_stats['count'] + m*global_mean)/ (subrule_stats['count'] + m))
    ...
    def sim_feats(df):
        ...
        return np.c_[diag_cos(b,p1),diag_cos(b,p2),diag_cos(b,n1),diag_cos(b,n2)]
    ...
    def add_sim_aggregates(df):
        pos_mean = df[['sim_body_pos1','sim_body_pos2']].mean(axis=1)
        neg_mean = df[['sim_body_neg1','sim_body_neg2']].mean(axis=1)

        df['sim_pos_mean'] = pos_mean
        df['sim_neg_mean'] = neg_mean
        df['sim_margin'] = pos_mean -neg_mean
        df['sim_max'] = df[['sim_body_rule','sim_body_pos1','sim_body_pos2','sim_body_neg1','sim_body_neg2']].max(axis=1)
        df['sim_min'] = df[['sim_body_rule','sim_body_pos1','sim_body_pos2','sim_body_neg1','sim_body_neg2']].min(axis=1)
        return df

    trn = add_sim_aggregates(trn)
    val = add_sim_aggregates(val)
    test = add_sim_aggregates(test)

    num_tr = np.c_[num_tr, trn[['sim_pos_mean','sim_neg_mean','sim_margin','sim_max','sim_min']].values]
    num_val = np.c_[num_val, val[['sim_pos_mean','sim_neg_mean','sim_margin','sim_max','sim_min']].values]
    num_te = np.c_[num_te, test[['sim_pos_mean','sim_neg_mean','sim_margin','sim_max','sim_min']].values]
    ...
    lr = LogisticRegression(
        solver='saga', max_iter=1500, C=0.5,
    ...
    lgbm = lgb.LGBMClassifier(
        ...
        reg_lambda=5.0, random_state=fold, n_jobs=-1,verbose=-1
    )
    lgbm.fit(
        num_tr_s, trn['rule_violation'],
...
sub_out["rule_violation"] = test_pred
sub_out.to_csv("/kaggle/working/submission.csv", index=False)
