...
blend_norm = 0.25 * norm1 + 0.75 * norm2     #v2.0
df['y'] = st.norm.cdf(blend_norm)
...
# ##Get the submission_0.97754.csv
# df1 = pd.read_csv('/kaggle/input/ps-s5e8-blend-xgb-lgb/submission.csv')        # 0.97735
# df2 = pd.read_csv('/kaggle/input/train-more-xgb-nn-lb-0-9774/submission_ensemble_train_more.csv')  # 0.97742
...
# blend_norm = 0.40 * norm1 + 0.6 * norm2     #v2.0
# df['y'] = st.norm.cdf(blend_norm)


# sub = df[['id', 'y']]
# sub.to_csv('submission_0.97754.csv', index=False)
...
# df1 = pd.read_csv('/kaggle/working/submission_0.97754.csv')      
# df2 = pd.read_csv('/kaggle/input/ps-s5e8-h-blend-bokeh-only-public-ml-solutions/submission_v104.csv') 
...
# sub1    = pd.read_csv("/kaggle/input/no-blending-bank-classification-xgb-lgbm-cat-ydf/submission.csv")      # 0.9772
# sub2    = pd.read_csv("/kaggle/input/train-more-xgb-nn-lb-0-9774/submission_ensemble_train_more.csv") # 0.97742

# df = pd.merge(df1, df2, on='id', suffixes=('_1', '_2'))  

# norm1 = st.norm.ppf(df['y_1'].clip(1e-8, 1-1e-8))
# norm2 = st.norm.ppf(df['y_2'].clip(1e-8, 1-1e-8))


# # blend_norm = 0.40 * norm1 + 0.6 * norm2    #v1.0

# blend_norm = 0.3 * norm1 + 0.7 * norm2     #v2.0
# df['y'] = st.norm.cdf(blend_norm)
...
# sub    = pd.read_csv("/kaggle/input/cmi-submission0-97763/submission (18).csv")
...
# sub = 0.70 * r + r1 * 0.30  # Lb = ?ls



# submission = pd.DataFrame({"id": sub1["id"], "y": sub})
# # submission.to_csv("submission_finish.csv", index=False)
# submission.head()
