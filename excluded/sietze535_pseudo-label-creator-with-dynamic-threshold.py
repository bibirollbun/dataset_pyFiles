...
class CFG:
    ...
    model_dir        = "/kaggle/input/birdclef25-effnetb0-starter-weight"
    SR = 32000
    WINDOW_SIZE = 5
    N_FFT = 1024
    HOP_LENGTH = 512
...
for label, count in label_counts.items():
    norm = (count - min_count) / (max_count - min_count + 1e-8)  # 0 for rare, 1 for common
    flipped = 1 - norm  # flip so rare gets 1.0
    per_class_thresholds[label] = max_thresh - flipped * (max_thresh - min_thresh)
    ...
    for audio_path in tqdm(audio_files, desc="Audio files", dynamic_ncols=True):
        ...
        for seg_idx in range(n_segments):
            ...
            if max_prob >= threshold:
                ...
                results.append([segment_name, label, max_prob])
