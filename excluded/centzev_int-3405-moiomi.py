...
# the right diagram shows a planet with a weak reduction:
...
# The data again is a time series, 
# and we can see how the star is obscured while the planet is passing in front of it.
...
def a_read_and_preprocess(dataset, adc_info, planet_ids):
    ...
    for i, planet_id in tqdm(list(enumerate(planet_ids)), desc="Processing AIRS-CH0"):
        ...
        # TÃ­nh net_signal: hiá»‡u giá»¯a cáº·p cháºµn/láº»
        net_signal = mean_signal[1::2] - mean_signal[0::2]
