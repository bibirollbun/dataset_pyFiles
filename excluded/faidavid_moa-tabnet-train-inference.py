...
data = pd.concat([pd.DataFrame(train_features[CELLS]), pd.DataFrame(test_features[CELLS]), pd.DataFrame(new_test_features[CELLS])])
...
def fe_cluster_genes(train, test, new_test, n_clusters_g = 22, SEED = 42):
    ...
    def create_cluster(train, test, new_test, features, kind = 'g', n_clusters = n_clusters_g):
        ...
        data = pd.concat([train_, test_, new_test_], axis = 0)
...
def fe_cluster_cells(train, test, new_test, n_clusters_c = 4, SEED = 42):
   ...
   # train, test = create_cluster(train, test, features_g, kind = 'g', n_clusters = n_clusters_g)
    train, test, new_test = create_cluster(train, test, new_test, features_c, kind = 'c', n_clusters = n_clusters_c)
    return train, test, new_test

train_features2 ,test_features2,new_test_features2=fe_cluster_cells(train_features2,test_features2,new_test_features2)
...
def fe_cluster_pca(train, test, new_test,n_clusters=5,SEED = 42):
        data=pd.concat([train,test,new_test],axis=0)
        ...
        return train, test, new_test
train_cluster_pca ,test_cluster_pca, new_test_cluster_pca = fe_cluster_pca(train_pca,test_pca, new_test_pca)
...
def fe_stats(train, test, new_test):
    
    features_g = GENES
    features_c = CELLS
    
    for df in train, test, new_test:
...
train_features2,test_features2, new_test_features2=fe_stats(train_features2,test_features2, new_test_features2)
...
for s in SEED:
    ...
    for fold_nb, (train_idx, val_idx) in enumerate(MultilabelStratifiedKFold(n_splits = NB_SPLITS, random_state = s).split(train, target)):
        ...
        preds_new_test = model.predict(new_test_[feature_cols].values)
