...
input_folder = "/kaggle/input/relicbound-model-v1/"
output_folder = "/kaggle/working/agent"
...
def show_map(space, fleet=None, only_visible=True):
    ...
    for y in range(Global.SPACE_SIZE):
        ...
        for x in range(Global.SPACE_SIZE):
            ...
            if node.type == NodeType.nebula:
                s1 = "単" if node.relic else "n"
            elif node.type == NodeType.asteroid:
                s1 = "達" if node.relic else "a"
            else:
...
%%writefile agent/main_relic.py

import json
from argparse import Namespace
from relicbound import RelicAgent
from lux.kit import from_json
...
def agent_fn(observation, configurations):
    ...
    remainingOverageTime = observation.remainingOverageTime
    if step == 0:
        agent_dict[player] = RelicAgent(player, configurations["env_cfg"])
    agent = agent_dict[player]
    actions = agent.act(step, from_json(obs), remainingOverageTime)
    return dict(action=actions.tolist())
...
!luxai-s3 agent/main.py agent/main_relic.py --output=replay.html
...
def evaluate_agents(agent_1, agent_2, seed=42, training=True, games_to_play=3):
    """
    Evaluate or train agents in a LuxAIS3 environment.

    Args:
        agent_1 (class): Class for player_0's agent.
        agent_2 (class): Class for player_1's agent.
        seed (int): Random seed for reproducibility.
        training (bool): If True, agents will train during evaluation.
    ...
    player_0 = agent_1("player_0", env_cfg, training=training)
    player_1 = agent_2("player_1", env_cfg, training=training)
...
# Train the model
evaluate_agents(Agent, Agent, training=True, games_to_play=250)
