...
!du --inodes '/kaggle/input/paddy-disease-classification/train_images/'
!du --inodes '/kaggle/input/paddy-disease-classification/test_images/'
...
test_path_new = '/kaggle/input/pd-test-images-labels/'
!du --inodes '/kaggle/input/pd-test-images-labels/'
...
class_subset = sorted(os.listdir(test_path))#[:10] # Using only the first 10 classes
class_subset
...
test_generator_vgg = image_datagen.flow_from_directory(    
    directory=test_path,
    target_size=(256, 256),
    color_mode="rgb",
    batch_size=1,
...
vgg16_model.load_weights('vgg_model_ft.weights.best.keras') # initialize the best trained weights
# vgg16_model.load_weights('../input/pd2vgg16weightsbest/vgg_model_ft.weights.best.hdf5') # initialize the best trained weights

vgg_preds_ft = vgg16_model.predict(test_generator)
...
test_generator = ImageDataGenerator(rescale=1./255).flow_from_directory(    
    ...
    shuffle=False,
    seed=SEED)
loss, acc = vgg16_model.evaluate(test_generator, steps=STEP_SIZE_TEST, verbose=1)
print(acc, loss)
