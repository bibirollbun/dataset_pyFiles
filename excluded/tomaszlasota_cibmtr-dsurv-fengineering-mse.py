...
class DeepSurv:
    ...
    def get_best_param(self, n_trials:int = 50, save_path: str = "/kaggle/working/best_params.json"):
    ...
    def load_best_params(self, load_path: str = "/kaggle/working/best_params.json"):
    ...
    def evaluate_model(self):
        ...
        c_index = self.c_index(self.event_time_test, self.indicator_test, predictions)
        print(f"Concordance Index = {c_index:.4f}")
        
        return {"c_index": c_index, "predictions": predictions}
...
# STEP 4. Check if preprocessing was successful
...
# STEP 5. Get Variance Inflation Factor (VIF) for feature selection
num_cols = X_train.select_dtypes(include=["number"]).columns
edad_train.get_vif(num_cols=num_cols)


# STEP 6. Perfrom PCA for feature selection
...
eda_pca.plot_bar_chart(x=total_contribution.head(30).index, y=total_contribution.head(30).values,
...
X_train, X_test, X_val = preprocessor.min_max_scale(columns = ordinal_col)
...
chosen_cols = features[features["n_features"] == 50]["features"].explode().values.tolist()
...
model = model_processor.build_best_model()


result = model_processor.evaluate_model()


# save model
model.save("DeepSurv_3_mse.h5")



# load model 
loaded_model = load_model("DeepSurv_3_mse.h5")
