...
def preprocess_data(df):
    ...
    # 'engine'列を処理: 馬力(HP)と排気量(Liters)を抽出
    ...
    # model_yearからcar_age（車の年式）を変換
    # 最新のデータが2023年なので、現在の年を2024年と仮定して計算
    # 実際の予測ではcurrent_yearを pd.to_datetime('today').year にすることが多いですが、
    # 競技などの固定データセットでは特定年に固定することがあります。
    current_year = 2024 # または pd.to_datetime('today').year
    ...
    # Replace '–' with NaN in 'ext_col' and 'int_col'
    # Kaggleデータセットの場合、欠損値はNaNとして読み込まれることが多いですが、
    # 念のためユーザー提供の例にあった'–'も考慮します。
    df['ext_col'] = df['ext_col'].replace('–', np.nan)
    df['int_col'] = df['int_col'].replace('–', np.nan)

    # clean_title の欠損値を補完（テストデータに欠損がある場合）
    # train.csvにはclean_titleの欠損がないですが、テストデータにはある可能性があるため
    if 'clean_title' in df.columns:
        df['clean_title'] = df['clean_title'].fillna('Unknown') # または最頻値で補完
...
preprocessor = ColumnTransformer(
 ...
 #前処理とRandomForestRegressoモデルを含む完全なパイプラインを作成
...
plt.grid(True)
plt.tight_layout()
plt.savefig('engine_displacement_vs_price.png')
plt.show()
