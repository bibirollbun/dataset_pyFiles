...
class NasConfig():
    def __init__(self, dir):
        self.root_dir = dir
        self.train_dir = os.path.join(self.root_dir, "train")#ルートディレクトリと結合
        self.test_dir = os.path.join(self.root_dir, "samples")#ルートディレクトリと結合
        self.number_cls = 2
        self.cls_map = {"dog": 0, "cat": 1}#dogやcatのラベルを0と1にする辞書型のデータ
        self.index_map = {0: "dog", 1: "cat"}#0,1のデータをdog,catに戻す辞書型のデータ
        self.read_train()
        self.read_test()
        self.device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")

    def read_train(self):
        self.train_datas = []#空行列を作成
        for file in os.listdir(self.train_dir):#trainデータの数だけ繰り返す
            if file.endswith(".jpg"):#拡張子が.jpgなら処理
                img = os.path.join(self.train_dir, file)#trainデータのあるpathと画像ファイル名を結合
                #dog.100.jpgのdogの部分で分割し、__init__.self.cls_mapに応じてラベルを付ける
                label = self.cls_map[file.split(".")[0]]
                #train_datasにpathとラベルの情報をもった配列を作成
...
#kaggleのルートファイルから、inputにあるデータを第一引数とし、configにpathとラベルの結合したデータを保持
...
transform = transforms.Compose([
    #サイズを128*128(pixel)にする
    transforms.Resize((128, 128)),
    #0から255のグレースケールした画像を0-1までの数値に変換(0.2など)
    transforms.ToTensor(),
    #データの標準化をする
