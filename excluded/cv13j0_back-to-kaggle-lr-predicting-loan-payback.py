...
trn_file_path = "/kaggle/input/playground-series-s5e11/train.csv"  # Replace with your CSV file path
trn_df = load_csv_to_dataframe(trn_file_path, ignore_fields=['id'])

test_file_path = "/kaggle/input/playground-series-s5e11/test.csv"  # Replace with your CSV file path
tst_df = load_csv_to_dataframe(test_file_path, ignore_fields=['id'])

sample_file_path = "/kaggle/input/playground-series-s5e11/sample_submission.csv"  # Replace with your CSV file path
...
#org_df = load_csv_to_dataframe(orig_file_path, ignore_fields=['id'])
...
categorical_fields = ['gender','marital_status','education_level','employment_status','loan_purpose','grade_subgrade']
print(feature_cols)
...
train_scaled, test_scaled = scale_dataframes(trn_encoded, tst_encoded, 'loan_paid_back', 'minmax')
...
# Mean CV accuracy across 5 folds: 0.8920
# Mean CV accuracy across 5 folds: 0.8906 Utilizing the Original Dataset.


avg_test_predictions


sub_df['loan_paid_back'] = 1 - avg_test_predictions
...
sub_df['loan_paid_back'] = 1 - mean_probs
sub_df.to_csv('xgbt_no_orig_submission.csv', index=False)
display(sub_df.head())


mean_probs, oof_predictions, model = train_model(train_scaled, test_scaled, feature_columns = feature_cols , target_column = target, model_type = "catboost", param_file = None, n_splits = 5, categorical_features = None)


sub_df['loan_paid_back'] = 1 - mean_probs
sub_df.to_csv('cat_no_orig_submission.csv', index=False)
display(sub_df.head())


mean_probs, oof_predictions, model = train_model(train_scaled, test_scaled, feature_columns = feature_cols , target_column = target, model_type = "hgb", param_file = None, n_splits = 5, categorical_features = None)


sub_df['loan_paid_back'] = 1 - mean_probs
sub_df.to_csv('hgb_no_orig_submission.csv', index=False)
display(sub_df.head())


# Average Accuracy: 0.8998
# Average F1 Score (macro): 0.8260
# Average ROC-AUC: 0.9119
# Average Log Loss: 0.2612


# Average Accuracy: 0.9017
# Average F1 Score (macro): 0.8300
# Average ROC-AUC: 0.9132
# Average Log Loss: 0.2591
