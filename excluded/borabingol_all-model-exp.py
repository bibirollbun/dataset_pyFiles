...
def create_datasets_and_loaders(df, series_description, transform, batch_size=8, augment_severe=False):
    ...
    filtered_df = filtered_df.sample(frac=0.01, random_state=42)  
...
# Modelin state_dict'lerini saklamak için bir sözlük oluşturuyoruz
model_states = {}

# Her modelin state_dict'ini sözlüğe ekliyoruz
for desc, model in models_dict.items():
    # Model adlarını güvenli hale getiriyoruz (örneğin, boşlukları "_" ile değiştiriyoruz)
    safe_desc = desc.replace(" ", "_").replace("/", "_")  
    model_states[safe_desc] = model.state_dict()  # Modelin state_dict'ini alıp sözlüğe ekliyoruz

# Tüm modellerin state_dict'lerini tek bir dosyada kaydediyoruz
torch.save(model_states, 'all_models.pth')
