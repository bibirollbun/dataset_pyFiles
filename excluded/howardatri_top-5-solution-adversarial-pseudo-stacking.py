...
# =====================================================================
# CELL 5: Target Engineering (FIXED)
# =====================================================================

def engineer_target(y_train, method='quantile'): # é»˜è®¤å€¼æ”¹ä¸º 'quantile' æ›´å�ˆç�†
    """
    Transforms the target variable y_train to a near-normal distribution.
    Returns: 
        y_transformed: The transformed target array.
        transformer: The fitted transformer (pt or qt) for inverse transformation.
    """
    from sklearn.preprocessing import PowerTransformer, QuantileTransformer
    
    if method == 'power':
        transformer = PowerTransformer(method='box-cox', standardize=False)
        y_transformed = transformer.fit_transform(y_train.values.reshape(-1, 1)).flatten()
        print("   Using PowerTransformer (Box-Cox)")
        
    elif method == 'quantile':
        transformer = QuantileTransformer(output_distribution='normal', n_quantiles=1000, subsample=50000)
        y_transformed = transformer.fit_transform(y_train.values.reshape(-1, 1)).flatten()
        print("   Using QuantileTransformer (Normal)")
        
    else:
        print("   Invalid method. No transformation applied.")
        transformer = None
        y_transformed = y_train.values.flatten()

    print(f"   Transformed: Î¼={y_transformed.mean():.4f}, Ïƒ={y_transformed.std():.4f}")
    
    # ğŸŒŸ ä¿®å¤�ç‚¹: å§‹ç»ˆè¿”å›�å��ä¸º 'transformer' çš„å¯¹è±¡ ğŸŒŸ
...
def multi_stage_ensemble(X_train, y_train, X_test, base_models, sample_weights):
    ...
    # ğŸŒŸğŸŒŸğŸŒŸ æ”¹è¿›ç‚¹ 3: å¢�åŠ æ¨¡å�‹å¤šæ ·æ€§ ğŸŒŸğŸŒŸğŸŒŸ
    NEW_SEED = 777 # ä½¿ç”¨ä¸€ä¸ªå’Œ '42' ä¸�å�Œçš„æ–°ç§�å­�
    
    xgb_params['subsample'] = 0.75       # å¢�åŠ éš�æœºæ€§
    xgb_params['colsample_bytree'] = 0.75 # å¢�åŠ éš�æœºæ€§
    xgb_params['seed'] = NEW_SEED        # è¦†ç›–ç§�å­�
    
    lgb_params['subsample'] = 0.75
    lgb_params['colsample_bytree'] = 0.75
    lgb_params['seed'] = NEW_SEED

    # 1. å¼ºåˆ¶åˆ‡æ�¢å›� CPU æ¨¡å¼�ï¼Œä»¥å…¼å®¹ rsm å�‚æ•°ã€‚
    #    è¿™å°†è§£å†³ 'rsm on GPU is supported for pairwise modes only' é”™è¯¯ã€‚
    cat_params['task_type'] = 'CPU'

    # ğŸŒŸğŸŒŸğŸŒŸ æ”¹è¿›ç‚¹ 4: ä¿®å¤� CatBoost Subsample å†²çª� ğŸŒŸğŸŒŸğŸŒŸ
    # å¿…é¡»æ˜¾å¼�è®¾ç½® bootstrap_type ä¸ºæ”¯æŒ� subsample çš„ç±»å�‹
    cat_params['bootstrap_type'] = 'Bernoulli' 
    # ğŸŒŸğŸŒŸğŸŒŸ END: æ”¹è¿›ç‚¹ 4 ğŸŒŸğŸŒŸğŸŒŸ

    # 2. ç§»é™¤å†²çª�çš„å�‚æ•°ï¼šbagging_temperature
    # CatBoostçš„'bagging_temperature'å�ªåœ¨é»˜è®¤çš„'Bayesian'æ¨¡å¼�ä¸‹å�¯ç”¨ã€‚
    if 'bagging_temperature' in cat_params:
        del cat_params['bagging_temperature'] # ç§»é™¤å†²çª�å�‚æ•°


    # 4. ç»Ÿä¸€ä½¿ç”¨ 'random_state'ã€‚
    #    è¿™å°†è§£å†³ 'only one of the parameters random_seed, random_state should be initialized' é”™è¯¯ã€‚
    if 'random_seed' in cat_params:
        del cat_params['random_seed']
    cat_params['random_state'] = NEW_SEED
    
    cat_params['subsample'] = 0.75
    cat_params['rsm'] = 0.75 # CatBoost çš„ colsample_bytree
    # ğŸ’¥ ä¿®å¤�ç‚¹ï¼šåˆ é™¤ 'random_seed'ï¼Œå�ªä½¿ç”¨ 'random_state' ğŸ’¥
    #cat_params['random_state'] = NEW_SEED # ä½¿ç”¨ 'random_state' æ�¥ä»£æ›¿ 'random_seed'
    # ğŸŒŸğŸŒŸğŸŒŸ END: æ”¹è¿›ç‚¹ 4 ğŸŒŸğŸŒŸğŸŒŸ
...
# ğŸŒŸğŸŒŸğŸŒŸ START: æ”¹è¿›ç‚¹ 1 (A) - æ¿€æ´» Target Engineering ğŸŒŸğŸŒŸğŸŒŸ
# è°ƒç”¨åœ¨ Cell 5 ä¸­å®šä¹‰çš„å‡½æ•°ï¼Œä½¿ç”¨ 'quantile' æ–¹æ³•
y_transformed, target_transformer = engineer_target(y, method='quantile')
print("   Target has been transformed for new models.")
# ğŸŒŸğŸŒŸğŸŒŸ END: æ”¹è¿›ç‚¹ 1 (A) ğŸŒŸğŸŒŸğŸŒŸ

# ğŸ’¥ ä¿®å¤�ç‚¹ 5: å°† y_transformed è½¬æ�¢å›� Pandas Series
# ç¡®ä¿� y_transformed çš„ç´¢å¼•ä¸� X_enhancedï¼ˆå�³ X_trainï¼‰å¯¹é½�
y_transformed = pd.Series(y_transformed, index=X_enhanced.index)
print("   y_transformed converted to Pandas Series.")
...
# ğŸŒŸğŸŒŸğŸŒŸ æ”¹è¿›ç‚¹ 1 (B) - ä½¿ç”¨è½¬æ�¢å��çš„ y ğŸŒŸğŸŒŸğŸŒŸ
oof_advanced, test_advanced = multi_stage_ensemble(
    X_enhanced, y_transformed, X_test_enhanced, models_dict, sample_weights # <-- ä½¿ç”¨ y_transformed è€Œä¸�æ˜¯ y
)
...
# ğŸŒŸğŸŒŸğŸŒŸ æ”¹è¿›ç‚¹ 1 (C) - è¿˜å�Ÿ Target Engineering çš„é¢„æµ‹ ğŸŒŸğŸŒŸğŸŒŸ
# ä½¿ç”¨åœ¨ Cell 9 ä¸­åˆ›å»ºçš„ 'target_transformer' æ�¥é€†å�‘è½¬æ�¢
# æ³¨æ„�ï¼šbaseline_blend å’Œ pred_loaded_stack æ˜¯åœ¨å�Ÿå§‹å°ºåº¦ä¸Šï¼Œä¸�éœ€è¦�è½¬æ�¢
test_advanced_restored = target_transformer.inverse_transform(test_advanced.reshape(-1, 1)).flatten()
print(f"   Restored 'test_advanced' mean: {test_advanced_restored.mean():.4f}")
# ğŸŒŸğŸŒŸğŸŒŸ END: æ”¹è¿›ç‚¹ 1 (C) ğŸŒŸğŸŒŸğŸŒŸ

# ğŸŒŸğŸŒŸğŸŒŸ æ”¹è¿›ç‚¹ 2: è°ƒæ•´æœ€ç»ˆè��å�ˆæ�ƒé‡� ğŸŒŸğŸŒŸğŸŒŸ
# å�Ÿå§‹æ�ƒé‡�: [0.40, 0.35, 0.25]
# å°�è¯•ç»™äºˆ 'test_advanced' (æ–°æ¨¡å�‹) æ›´é«˜çš„æ�ƒé‡�ï¼Œå› ä¸ºå®ƒç�°åœ¨æ›´å¥½äº†
W_BASELINE = 0.35 # (å�Ÿ 0.40)
W_ADVANCED = 0.40 # (å�Ÿ 0.35)
W_STACK = 0.25    # (ä¸�å�˜)

# Strategy weights
final_predictions = (
    W_BASELINE * baseline_blend +        # Loaded models (proven)
    W_ADVANCED * test_advanced_restored + # ğŸŒŸ ä½¿ç”¨è¿˜å�Ÿå��çš„ 'test_advanced'
    W_STACK * pred_loaded_stack       # Extra weight to stacking
)
