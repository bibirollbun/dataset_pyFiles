...
model = XGBRegressor(
    ...
    random_state=42,
    n_jobs=-1,
    tree_method='hist'  # schneller aus Kaggle
)
...
print("RMSE auf train:", rmse_train)
print("RMSE auf validation set:", rmse_val)

importances = pd.Series(model.feature_importances_, index=X.columns)
importances.sort_values(ascending=False).head(10).plot(kind='barh')
plt.title("Top 10 wichtigsten features (XGBoost)")
