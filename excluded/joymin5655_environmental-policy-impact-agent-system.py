...
print(f"ğŸ”‘ Gemini API: {'âœ… Set' if GEMINI_API_KEY else 'â�Œ Not set (demo mode)'}")
print(f"ğŸ”‘ WAQI API: {'âœ… Set' if WAQI_API_KEY else 'â�Œ Not set (demo mode)'}")
...
# ============================================================
# Day 4: Observability - Logging, Tracing, Metrics
# ============================================================
class AgentLogger:
    ...
    def _log(self, level: str, message: str, **kwargs) -> Dict:
        ...
        icon = {"DEBUG": "ğŸ”�", "INFO": "â„¹ï¸�", "WARNING": "âš ï¸�", "ERROR": "â�Œ"}.get(level, "ğŸ“�")
...
# ============================================================
# Day 4+: Callbacks & ValidationChecker
# ============================================================

@dataclass
class AgentCallback:
    """Callbacks for agent lifecycle events."""
    on_start: Callable[[str, Dict], None] = None
    on_tool_call: Callable[[str, str, Dict, Any], None] = None
    on_complete: Callable[[str, Dict, Any], None] = None
...
class ValidationChecker(Agent):
    ...
    def __init__(
        ...
        super().__init__(name, model, instruction, tools, sub_agents, output_key)
    ...
    async def run(self, query: str = None, context: Dict = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"validation_passed": False, "escalate": False, "error": str(e)}
...
class EffectivenessValidator(ValidationChecker):
    ...
    def __init__(self, name: str = "EffectivenessValidator", threshold: float = 80.0, score_key: str = "effectiveness_score"):
...
class QualityGateValidator(ValidationChecker):
    ...
    def __init__(self, name: str = "QualityGateValidator", criteria: Dict[str, Callable[[Dict], bool]] = None):
        super().__init__(name=name)
...
# ============================================================
# Day 5: A2A Protocol & Deployment Configuration
# ============================================================
@dataclass
class GoldenTask:
...
ğŸ“Š íš¨ê³¼ì„± í�‰ê°€: {score}/100 {emoji} {rating}
   ...
   ëª©í‘œ ë‹¬ì„±: {'âœ… ë‹¬ì„±' if analysis.get('exceeded_target') else 'â�Œ ë¯¸ë‹¬ì„±'}
...
ğŸŒ� í˜„ì�¬ ëŒ€ê¸°ì§ˆ ({air_data.get('city', country)}):
   ...
   ìƒ�íƒœ: {air_data.get('status', 'N/A')} | ì¶œì²˜: {air_data.get('source', 'N/A')}
{'='*60}
