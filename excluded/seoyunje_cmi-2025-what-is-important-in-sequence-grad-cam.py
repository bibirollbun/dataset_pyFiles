...
def hook_fn(module, input, output, hook_name):
    x = input[0]
    attn_scores = module.attn(x)
    attn_out[hook_name] = F.softmax(attn_scores, dim=-1)


model = Final_Model.load_from_checkpoint("/kaggle/input/cmi-2025-final-submission/last_fold9.ckpt")
model.eval()

# IMU branch
handle_imu_trans = model.feature_extractor.imu_model.attn_pool[0].register_forward_hook(
    partial(hook_fn, hook_name="imu_trans")
)
handle_imu_ges = model.feature_extractor.imu_model.attn_pool[1].register_forward_hook(
    partial(hook_fn, hook_name="imu_ges")
)

# THM&TOF branch
handle_thm_tof_trans = model.feature_extractor.thm_tof_model.attn_pool[0].register_forward_hook(
    partial(hook_fn, hook_name='thm_tof_trans')
)
handle_thm_tof_ges = model.feature_extractor.thm_tof_model.attn_pool[1].register_forward_hook(
    partial(hook_fn, hook_name='thm_tof_ges')
)

seq_list = np.random.choice(df['sequence_id'].unique(), 4)
...
for i, seq_id in enumerate(seq_list):
    ...
    print(f'{c_}{sr_}#'*25)
    print(f'### Seqeunce_Id: {seq_id}')
    print("#"*25)
    
    ds = CMI_Dataset(df[df['sequence_id'] == seq_id], col=total_feature)
    ...
    seq = X[0].permute(1,0).cpu().numpy()

    for i, col in enumerate(raw_acc_feature):
        axes[0].plot(seq[:CFG.sequence_len//2,i], label=col) 
        axes[1].plot(seq[CFG.sequence_len//2:,i], label=col)
        
    for i, col in enumerate(raw_rot_feature):
        axes[2].plot(seq[:CFG.sequence_len//2,13+i], label=col) 
        axes[3].plot(seq[CFG.sequence_len//2:,13+i], label=col)
    
    axes[0].set_title('First 50% in Acc', fontweight='bold'); axes[0].legend(); axes[1].set_title("Last 50% in Acc", fontweight='bold'); axes[1].legend()
    axes[2].set_title('First 50% in Rot', fontweight='bold'); axes[2].legend(); axes[3].set_title("Last 50% in Rot", fontweight='bold'); axes[3].legend()

    ## imu branch
    attn_imu_trans = F.interpolate(attn_out['imu_trans'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)
    attn_imu_ges = F.interpolate(attn_out['imu_ges'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)

    axes[0].imshow(
        np.tile(attn_imu_trans, (len(raw_acc_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,0:3])-2, np.max(seq[:CFG.sequence_len//2,0:3])+2]
    )
    axes[1].imshow(
        np.tile(attn_imu_ges, (len(raw_acc_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[CFG.sequence_len//2:,0:3])-2, np.max(seq[CFG.sequence_len//2:,0:3])+2]
    ...
    axes[2].imshow(
        np.tile(attn_imu_trans, (len(raw_rot_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,13:17])-2, np.max(seq[:CFG.sequence_len//2,13:17])+2]
    )
    axes[3].imshow(
        np.tile(attn_imu_ges, (len(raw_rot_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[CFG.sequence_len//2:,13:17])-2, np.max(seq[CFG.sequence_len//2:,13:17])+2]
    ...
    for i, col in enumerate(raw_thm_feature):
        axes[4].plot(seq[:CFG.sequence_len//2,30+i], label=col); axes[4].legend() 
        axes[5].plot(seq[CFG.sequence_len//2:,30+i], label=col); axes[5].legend()

    for i, col in enumerate(raw_tof_feature):
        axes[6].plot(seq[:CFG.sequence_len//2,46+i], label=col); axes[6].legend() 
        axes[7].plot(seq[CFG.sequence_len//2:,46+i], label=col); axes[7].legend()
    
    axes[4].set_title('First 50% in THM', fontweight='bold'); axes[5].set_title("Last 50% in THM", fontweight='bold')
    axes[6].set_title('First 50% in TOF', fontweight='bold'); axes[7].set_title("Last 50% in TOF", fontweight='bold')

    ## thm & tof branch
    attn_thm_tof_trans = F.interpolate(attn_out['thm_tof_trans'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)
    attn_thm_tof_ges = F.interpolate(attn_out['thm_tof_ges'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)     

    axes[4].imshow(
        np.tile(attn_thm_tof_trans, (len(raw_thm_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,30:35])-2, np.max(seq[:CFG.sequence_len//2,30:35])+2]
    )
    axes[5].imshow(
        np.tile(attn_thm_tof_ges, (len(raw_thm_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,30:35])-2, np.max(seq[:CFG.sequence_len//2,30:35])+2]
    ...
    axes[6].imshow(
        np.tile(attn_thm_tof_trans, (len(raw_tof_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,46:51])-2, np.max(seq[:CFG.sequence_len//2,46:51])+2]
    )
    axes[7].imshow(
        np.tile(attn_thm_tof_ges, (len(raw_tof_feature), 1)),
        aspect='auto',
        cmap='coolwarm',
        alpha=0.4,
        extent=[0, CFG.sequence_len//2-1, np.min(seq[:CFG.sequence_len//2,46:51])-2, np.max(seq[:CFG.sequence_len//2,46:51])+2]
...
def plot_sequence_with_attn_colormap(ax, y, attn, cmap="viridis"):
    ...
    lc.set_array(attn)   # 색상 매핑 기준 = attention 값
    lc.set_linewidth(2)
    ...
    plt.colorbar(sm, ax=ax, orientation="horizontal", label="Attention Weight")
...
for i, seq_id in enumerate(seq_list):
    ...
    axes[0].set_title('First 50% in Acc_X', fontweight='bold'); axes[1].set_title("Last 50% in Acc_X", fontweight='bold')
    axes[2].set_title('First 50% in Rot_X', fontweight='bold'); axes[3].set_title("Last 50% in Rot_X", fontweight='bold')

    ## imu branch
    attn_imu_trans = F.interpolate(attn_out['imu_trans'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)
    attn_imu_ges = F.interpolate(attn_out['imu_ges'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)

    plot_sequence_with_attn_colormap(axes[0], seq[:CFG.sequence_len//2,0], attn_imu_trans, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[1], seq[CFG.sequence_len//2:,0], attn_imu_ges, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[2], seq[:CFG.sequence_len//2,13], attn_imu_trans, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[3], seq[CFG.sequence_len//2:,13], attn_imu_ges, cmap="plasma")
    
    
    axes[4].set_title('First 50% in THM_1', fontweight='bold'); axes[5].set_title("Last 50% in THM_1", fontweight='bold')
    axes[6].set_title('First 50% in TOF_1_MEAN', fontweight='bold'); axes[7].set_title("Last 50% in TOF_MEAN", fontweight='bold')

    ## thm & tof branch
    attn_thm_tof_trans = F.interpolate(attn_out['thm_tof_trans'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)
    attn_thm_tof_ges = F.interpolate(attn_out['thm_tof_ges'], size=CFG.sequence_len//2, mode='linear', align_corners=True).numpy().reshape(-1)

    plot_sequence_with_attn_colormap(axes[4], seq[:CFG.sequence_len//2,30], attn_thm_tof_trans, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[5], seq[CFG.sequence_len//2:,30], attn_thm_tof_ges, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[6], seq[:CFG.sequence_len//2,46], attn_thm_tof_trans, cmap="plasma")
    plot_sequence_with_attn_colormap(axes[7], seq[CFG.sequence_len//2:,46], attn_thm_tof_ges, cmap="plasma")
