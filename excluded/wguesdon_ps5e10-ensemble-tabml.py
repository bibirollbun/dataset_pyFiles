...
def train_model_with_oof(model, model_name, X_train, y_train, X_test, n_folds=5):
    ...
    print(f"{'='*60}")
    print(f"RMSE: {oof_rmse:.6f} (Â±{np.std(cv_scores):.6f})")
    print(f"MAE:  {oof_mae:.6f}")
    print(f"RÂ²:   {oof_r2:.6f}")
...
print("âœ… OOF Manager initialized")


print("\nğŸš€ Training XGBoost with GPU acceleration...")
...
print("\nğŸš€ Training LightGBM with GPU acceleration...")
...
print("\nğŸš€ Training CatBoost with GPU acceleration...")
...
print(f"\nğŸ�† Best Single Model: {best_model['model_name']}")
...
print(f"   RÂ²:   {results_df.loc[best_method, 'r2']:.6f}")
...
for method, preds in submissions.items():
    ...
    print(f"âœ… {method:20s} -> {submission_path.name}")
...
print(f"\nğŸ�† BEST submission -> {best_path.name}")
...
print(f"\nğŸ“Š Summary:")
print(f"  â€¢ Models trained: {len(all_test_predictions)}")
print(f"  â€¢ Best single model: {best_model['model_name']} ({best_model['cv_score']:.6f} RMSE)")
print(f"  â€¢ Best ensemble: {best_method} ({results_df.loc[best_method, 'rmse']:.6f} RMSE)")
print(f"  â€¢ Improvement: {(best_model['cv_score'] - results_df.loc[best_method, 'rmse']) / best_model['cv_score'] * 100:.2f}%")

print(f"\nğŸ“� Output files:")
print(f"  â€¢ OOF predictions: {OOF_DIR}")
print(f"  â€¢ Submissions: {SUBMISSION_DIR}")
print(f"  â€¢ Best submission: {best_path.name}")

print(f"\nğŸš€ GPU Usage: {'âœ… Enabled' if GPU_AVAILABLE else 'â�Œ Disabled'}")
