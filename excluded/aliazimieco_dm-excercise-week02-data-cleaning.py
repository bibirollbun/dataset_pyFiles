...
# Filter and remove columns that exist in the DataFrame
existing_columns_to_remove = [col for col in columns_to_remove if col in inputs.columns]
inputs = inputs.drop(columns=existing_columns_to_remove, axis=1)

# Set "RefId" as the index if it exists in the DataFrame
if 'RefId' in inputs.columns:
    inputs = inputs.set_index('RefId')
else:
    print("'RefId' column is not found in the DataFrame.")
...
contingency_table=pd.crosstab(inputs["PRIMEUNIT"].dropna(),y_train)
...
contingency_table=pd.crosstab(inputs["AUCGUART"].dropna(),y_train)
print("contingency table with the frequencies for PRIMEUNIT variable")
print(contingency_table)
print("=="*50)
