...
for df in [df_test, df_train]:
    ...
    df['Weight Age Risk'] = df['waist_to_hip_ratio'] * df['age']
    df['Cholesterol Ratio'] = df['cholesterol_total'] / df['hdl_cholesterol']
...
# Model
for seed in seeds:
    xgb_model = XGBClassifier(n_estimators = 3500, 
                          learning_rate = 0.01,  
                          max_depth = 5,
                          subsample=0.6,
...
submission = pd.DataFrame({'id': df_test['id'], 'diagnosed_diabetes': final_predictions})
