...
for epoch in range(EPOCH):
    for i, data in enumerate(train_loader, 0):
        ...
        print(f"[{epoch}/{EPOCH}][{i}/{len(train_loader)}] Loss_D: {errD:.4f}; Loss_G: {errG:.4f}")
        if i % 100 == 0:
            utils.save_image(real, "./results/real_samples.png", normalize=True)
            fake = netG(noise)
            utils.save_image(fake.data, f"./results/fake_samples_epoch_{epoch:03d}.png", normalize=True)
...
def show_generated_img(n_images=5):
    ...
    for _ in range(n_images):
        ...
        gen_image = (gen_image.permute(1, 2, 0) + 1) / 2
...
for epoch in range(epochs):
    ...
    for ii, (real_images, train_labels) in enumerate(train_loader):
        ...
        if (ii+1) % (len(train_loader)//2) == 0:
            print(
                f"[{epoch+1}/{epochs}][{ii+1}/{len(train_loader)}] "
...
print(f">> average EPOCH duration = {np.mean(epoch_time)/60:.2f} Minutes")
...
for i_batch in range(0, n_images, im_batch_size):
    ...
    images = (images.permute(0, 2, 3, 1) + 1) / 2
