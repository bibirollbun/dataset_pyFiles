...
# Inference model (matches training)
class PawpularModel(nn.Module):
    def __init__(self):
        super().__init__()
        # Use pretrained=False during inference if you only load weights later
        self.model = timm.create_model("resnet50", pretrained=False, in_chans=3)
        
        self.dropout = nn.Dropout(0.5)
        self.out = nn.Linear(1000, 1)  # same as training
    ...
    def forward(self, image, features=None, targets=None):
        x = self.model(image)
        x = self.dropout(x)
        x = self.out(x)
...
model = PawpularModel()
model = Tez(model)
model.load(f"/kaggle/input/10021-harsh-kumar-training/model_f{i}.bin", weights_only=True)
