...
import torch 
LR = 4e-5 #a good place to start - lowering it for further experimentation
NUM_EPOCHS = 30 # will use early stopping 
WARMUPS = 500
...
for idx,loaders in enumerate(tqdm((all_loaders))):
    ...
    for epochs in trange(NUM_EPOCHS):
        ...
        if aur_roc > best_metric:
            best_metric = aur_roc
            trigger = 0
            print(f'Saving Best {idx+1} model')
            torch.save(model.state_dict(),f'BERT{idx+1}Tuned_s2.pth')
        else:
            trigger +=1
        if trigger >= patience or aur_roc == 1:
...
for idx in range(5):
    ...
    state_dict=torch.load(f'BERT{idx+1}Tuned_s2.pth',map_location=DEVICE)
...
def predict_test_set(
    ...
    print("\n✅ Final test set probabilities generated via ensemble averaging.")
...
sub.to_csv('SUB_TOM_12_validated_on_AUC_ROC.csv',index=False)
