...
class Model:
    ...
    def _generating_svg(self, description:str):
        prompt = "simple, basic, minimalistic icon, vector art, solid colors, white background,'{}'"
        # prompt = "simple, minimalistic icon, vector art, clean lines, solid colors, white background, '{}'"
        negative_prompt = "text, letters, words, characters, labels, signature, watermark, writing, font, numbers, blurry, photorealistic, 3D render, noisy, complex background, shadows, gradients"
        guidance_scale = 8
        num_inference_steps = 30
        prompt = prompt.format(description)
        image = self.model(prompt,height=512, negative_prompt=negative_prompt, width=512,  generator=self.generator, guidance_scale=guidance_scale, num_inference_steps=num_inference_steps).images[0]
        svg_string, is_within_size = self.svg_conversion(image)
        if not is_within_size:
            svg_string, is_within_size = self.svg_conversion(image,image_size=(50,50))
...
#     # image = svg_gen.model(prompt, generator=svg_gen.generator).images[0]
