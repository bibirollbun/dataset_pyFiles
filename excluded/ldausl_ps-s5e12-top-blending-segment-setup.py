...
direct_blend(['0.70737.a','0.70109'], weight=[0.98, 0.02], output_file='A.csv')
direct_blend(['0.70737.b','0.70442'], weight=[0.97, 0.03], output_file='B.csv')
direct_blend(['0.70737.c','0.70651'], weight=[0.96, 0.04], output_file='C.csv')
direct_blend(['0.70737.d','0.70682'], weight=[0.89, 0.11], output_file='D.csv')


# archive segment setup

#'segment' : [[0.00000, 0.00040                         ],  version.6   LB = 0.70_737
#             [         0.00042, 0.00065                ],  
#             [                  0.00067, 0.00098       ], 
#             [                                   'else']],


#'segment' : [[0.00002, 0.00096                         ],  version.7   LB = 0.70_739
#             [         0.00098, 0.00154                ],   
#             [                  0.00157, 0.00234       ],   
#             [                                   'else']], 


#'segment' : [[0.00002, 0.00174                         ],  version.8   LB = 0.70_740
#             [         0.00177, 0.00285                ], 
#             [                  0.00290, 0.00454       ],
#             [                                   'else']],
...
group = [{'name': f'A', 'weight':+0.24, 'color':'royalblue'},
         {'name': f'B', 'weight':+0.24, 'color':'orange'   },
         {'name': f'C', 'weight':+0.24, 'color':'green'    },
         {'name': f'D', 'weight':+0.28, 'color':'crimson'  },
]
params = {
     'path'     : work_page,            
     ...
     'segment'  : [[0.00003, 0.00177                       ], 
                   [         0.00180, 0.00293              ], 
                   [                  0.00300, 0.00474     ],
                   [                                 'else']],
      
      'subm'    : group, 'subwts' : [w/100 for w in [ 11,  -3,  -1,  -7]],
      'subm2'   : group, 'subwts2': [w/100 for w in [ -3,  11,  -7,  -1]],
      'subm3'   : group, 'subwts3': [w/100 for w in [ -1,  -7,  11,  -3]],
      'subm4'   : group, 'subwts4': [w/100 for w in [ -7,  -1,  -3,  11]],
}
...
td = pd.read_csv('tida_desc.csv')

sns.set()
plt.figure(figsize=(9, 3))
plt.hist(td['mx-m'], bins=250)
plt.gca().set_facecolor('whitesmoke')
plt.suptitle('mx-m', y=0.85, fontsize=11, c='black')


print(f'length mx-m = {len(td['mx-m'])}')

segment1 = td[(td['mx-m'] > 0.00003) & (td['mx-m'] <= 0.00177)]['mx-m'];  print(len(segment1))
segment2 = td[(td['mx-m'] > 0.00180) & (td['mx-m'] <= 0.00293)]['mx-m'];  print(len(segment2))
segment3 = td[(td['mx-m'] > 0.00300) & (td['mx-m'] <= 0.00474)]['mx-m'];  print(len(segment3))


os.remove('tida_desc.csv'); shutil.rmtree(work_page)
