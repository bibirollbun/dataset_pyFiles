...
lora_dir = "/kaggle/input/gemma2-qlora-ft-v29-che"
...
#lora_dir = "/kaggle/input/lmsys-llama3-1-qlora-sft-v6/checkpoint-2099"
...
# 基于 low_freq_factor 和 high_freq_factor 调整 inv_freq
# low_freq_factor = rope_scaling['low_freq_factor']
# high_freq_factor = rope_scaling['high_freq_factor']

# low_inv_freq = 1.0 / (
#     (config.rope_theta * low_freq_factor) ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim)
# )
# high_inv_freq = 1.0 / (
#     (config.rope_theta * high_freq_factor) ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim)
# )
# 合并低频和高频部分
# inv_freq = torch.cat([low_inv_freq, high_inv_freq], dim=0)
inv_freq = 1.0 / (
    config.rope_theta ** (torch.arange(0, dim, 2, dtype=torch.float32) / dim)
)
inv_freq0 = inv_freq.to("cuda:0")
...
for batch in tqdm(dataloader):
    for micro_batch in batch:
        ...
        if is_first:
            with torch.no_grad(), torch.cuda.amp.autocast():
                prev_hidden_states = model.forward_part1(input_ids, seq_info, inv_freq0)#, rope_scaling=rope_scaling)
            is_first = False
            prev_seq_info, prev_hidden_states = to_device(
                [seq_info, prev_hidden_states], "cuda:1"
            )
            continue
        with torch.no_grad(), torch.cuda.amp.autocast():
            logits = model.forward_part2(prev_hidden_states, prev_seq_info, inv_freq1)#, rope_scaling=rope_scaling)
            hidden_states = model.forward_part1(input_ids, seq_info, inv_freq0)#, rope_scaling=rope_scaling)
