...
"""
agg_funcs = {
    "purchase_amount": ["min", "mean", "max", "sem"], # 正規化済みの購入金額（負値 = 顧客の支出）
    "installments":    ["min", "mean", "max", "sem"], # 分割払いの分割回数
    "month_lag":       ["min", "mean", "max", "sem"], # 基準日からの経過月数
...
new_trans = pd.read_csv(f"{DATA_DIR}/merchants.csv")
print(new_trans.describe())

# 集約特徴の作成パターンを辞書化
"""
agg_funcs = {
    "avg_purchases_log12": ["mean", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)], # 直近12ヵ月間の稼動月数
    "avg_sales_lag12" :    ["mean", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)], # 直近12ヶ月間の月間平均売上 ÷ 最近1ヶ月間の月間平均売上
    "active_months_lag12": ["mean", lambda x: x.quantile(0.25), lambda x: x.quantile(0.75)]  # 直近12ヵ月間の月平均取引高 ÷ 最近1ヵ月間の月間取引高
}
"""
...
# 全ての行を card_id の値でグループ化
# ⇒　上の集約パターンに従って実際に特徴を集約
#agg_new = new_trans.groupby("merchant_id").agg(agg_funcs)
...
train_x = train_x.merge(agg_new, how="left")
test_x  = test_x.merge(agg_new, how="left")
...
print(pd.concat([test_x], axis=1).describe())
