...
USE_6_LABELS = True  # Set to False to use only 5 labels (excluding Access_Control)
...
def create_dasp_labels_with_cache(df, include_access_control=True):
    ...
    # SAFE column accessor that returns a 0-series if column is missing
    import pandas as pd
    def col(name):
        return df_dasp[name] if name in df_dasp.columns else pd.Series(0, index=df_dasp.index, dtype=int)
    
    # 1. Arithmetic (Integer Overflow and Underflow)
    # Maps: Overflow, Underflow -> Arithmetic
    df_dasp['Arithmetic'] = ((col('Overflow') == 1) | 
                            (col('Underflow') == 1)).astype(int)
...
if TRAINING_FRAMEWORK == "belt" and 'CustomBertClassifierWithPooling' in locals():
    ...
    MODEL_PARAMS = {
        ...
        "load_checkpoint": 7,  # 0 = latest, specific number to load specific checkpoint, -1 = don't load
    }
    ...
    try:
        ...
        if 'X_train' in locals() and 'y_train' in locals():
            ...
            # Overall metrics
            accuracy = accuracy_score(y_test_np, y_pred_belt_np)
            print("Overall:")
            precision = precision_score(y_test_np, y_pred_belt_np, average='weighted')
            recall = recall_score(y_test_np, y_pred_belt_np, average='weighted')
            f1_weighted = f1_score(y_test_np, y_pred_belt_np, average='weighted')
...
def predict_vulnerabilities(contract_code, model, framework, tokenizer=None, categories=None):
    ...
    if framework == "belt":
        # BELT prediction
        probabilities = model.predict_scores([preprocessed_code])
