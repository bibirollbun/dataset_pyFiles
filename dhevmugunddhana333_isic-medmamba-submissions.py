...
# !pip install --no-cache-dir --force-reinstall torch==1.13.0 torchvision==0.14.0 torchaudio==0.13.0 --index-url https://download.pytorch.org/whl/cu117
...
def resampler_data(X, Y):
    ...
    resampler = RandomUnderSampler(sampling_strategy=0.05,)
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 4, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # Fully connected layers for image feature extraction
        self.img_fc1 = nn.Linear(self.num_features, 1024)
        self.img_fc2 = nn.Linear(1024, 512)
        
        # Fully connected layers for tabular data feature extraction
        self.tab_fc1 = nn.Linear(72, 128)
        self.tab_fc2 = nn.Linear(128, 256)
        
        # Concatenate head
        self.concat1 = nn.Linear(512 + 256,128)
        self.concat2 = nn.Linear(128,64)
        # Classification head
    ...
    def classify(self, x, tab):
        ...
        tab = self.tab_fc1(tab)
        tab = F.selu(tab)
        tab = F.dropout(tab, p=dropout_rate, training=self.training)
        
        tab = self.tab_fc2(tab)
        tab = F.selu(tab)
        tab = F.dropout(tab, p=dropout_rate, training=self.training)
                
        # Concatenate features and classify
        combined = torch.cat((x, tab), dim=1)
        combined = self.concat1(combined)
        combined = F.selu(combined)
        combined = F.dropout(combined, p=dropout_rate, training=self.training)
        combined = self.concat2(combined)
        combined = F.selu(combined)
        combined = F.dropout(combined, p=dropout_rate, training=self.training)
...
net = VSSM(num_classes=2).to(device)
...
save_path = '/kaggle/working/isicmedmamba.pth'
...
for epoch in range(epochs):
    ...
    for step, data in enumerate(train_bar):
        ...
        optimizer.zero_grad()
        outputs = net(images.to(device), tabular.to(device))
    ...
    acc = 0.0  # Accumulate correct predictions
    ...
    with torch.no_grad():
        ...
        for val_data in val_bar:
            ...
            val_tabular = val_dict['tabular']

            outputs = net(val_images.to(device), val_tabular.to(device))
            val_loss_temp = criterion(outputs, val_labels.flatten().to(device))
            val_loss += val_loss_temp.item() * val_images.size(0)
...
state_dict = torch.load('/kaggle/working/isicmedmamba.pth')
# The above version is for the medmamba isic version 14 model
...
with torch.no_grad():
        ...
        for test_data in test_bar:
            ...
            test_tabular = test_data['tabular']
            outputs = model(test_images.to(device),test_tabular.to(device))
...
sub_df = pd.concat([X_test['isic_id'],predict_y],axis=1)
