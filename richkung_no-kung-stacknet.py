...
sub_1.to_csv('submission_cnn.csv', index=False)
sub_2.to_csv('submission_tabnet.csv', index=False)
sub_3.to_csv('submission_dnn.csv', index=False)
...
HNing_submission =  pd.read_csv('/kaggle/working/submission_jj.csv')

Leader_CNN_submission = pd.read_csv('/kaggle/working/submission_cnn.csv')
Leader_DNN_submission = pd.read_csv('/kaggle/working/submission_dnn.csv')
Leader_TabNet_submission = pd.read_csv('/kaggle/working/submission_tabnet.csv')


features = HNing_submission.columns
features
...
merged_submission = HNing_submission.merge(Leader_CNN_submission, on=key_col)\
                                       .merge(Leader_DNN_submission, on=key_col)\
                                       .merge(Leader_TabNet_submission, on=key_col)
...
class TabNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # Concatenate: (batch, 1200) + (batch, 400) → (batch, 1600)
...
model.load_state_dict(torch.load('/kaggle/input/best-model/best_model_new.pth'))
