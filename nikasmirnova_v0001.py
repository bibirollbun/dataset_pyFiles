...
login("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")
...
# Переместите модель на CPU
model = model.to('cpu')



# Ensure that you specify the correct device as 'cpu'
device = 'cpu'
...
class TextComparator:
    def __init__(self, device: str = "cuda:0", model_dir: str = "/kaggle/input/text-comparator") -> None:
        self.model = SentenceTransformer(model_dir, device=device)
        self.embedding_cache = {}
    ...
    def similarity(self, text1: Union[str, List[str]], text2: Union[str, List[str]]) -> Union[float, np.ndarray]:
        emb1 = self.get_embeddings(text1)
        emb2 = self.get_embeddings(text2)
...
class FakeTextDetector:
    def __init__(self, model_dir: str = "/kaggle/input/fake-text-detector/"):
        self.device = device
        self.tokenizer = AutoTokenizer.from_pretrained(model_dir)
        self.model = AutoModelForSequenceClassification.from_pretrained(model_dir)
        self.model.eval()
...
submission['prompt'] = submission['prompt'].apply(preprocess_text)
submission['generation'] = submission['generation'].apply(preprocess_text)
...
pip install --upgrade nltk scikit-learn numpy scipy
