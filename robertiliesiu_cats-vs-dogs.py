...
#with zipfile.ZipFile('/kaggle/input/train', 'r') as zip_ref:
    #zip_ref.extractall('/kaggle/working/train')
...
#with zipfile.ZipFile('/kaggle/input/validation', 'r') as zip_ref:
    #zip_ref.extractall('/kaggle/working/validation')


train_dir = '/kaggle/input/alien-vs-predator-images/data/train/'
test_dir = '/kaggle/input/alien-vs-predator-images/data/validation'



base_dir = "/kaggle/input/alien-vs-predator-images/data/train/alien"
...
alien_dir = os.path.join(train_dir, 'alien')
predator_dir = os.path.join(train_dir, 'predator')

alien_files = [os.path.join(alien_dir, fname) for fname in os.listdir(alien_dir) if fname.endswith('.jpg')]
predator_files = [os.path.join(predator_dir, fname) for fname in os.listdir(predator_dir) if fname.endswith('.jpg')]

file_paths = alien_files + predator_files
labels = [1] * len(alien_files) + [0] * len(predator_files)
...
def plot_top_misclassified(model, dataloader, class_names=['alien', 'predator'], device='cuda', top_n=5):
    model.eval()
    misclassified = []
    ...
    show_images(misclassified_cats, 'Top Misclassified Alien')
    show_images(misclassified_dogs, 'Top Misclassified Predator')
...
torch.save(model.state_dict(), '/kaggle/working/cnn_alien_predator_model_weights.pth')
torch.save(model, '/kaggle/working/cnn_aline_predator_full_model.pth')


torch.save(model.state_dict(), '/kaggle/working/resnet_aline_predator_model_weights.pth')
torch.save(model, '/kaggle/working/resnet_aline_predator_full_model.pth')


torch.save(model.state_dict(), "alien_predator_resnet18.pth")
