...
#INPUT_DIR_SUPPLEMENTAL = "/kaggle/input/fertilizer-prediction/"
INPUT_DIR = "" 
INPUT_DIR_SUPPLEMENTAL = ""

train_df = pd.read_csv(f"{INPUT_DIR}train.csv")
test_df  = pd.read_csv(f"{INPUT_DIR}test.csv")
supplemental_df = pd.read_csv(f"{INPUT_DIR_SUPPLEMENTAL}Fertilizer Prediction.csv")
...
supplemental_df = add_features(supplemental_df)
...
y_supplemental = L_encoder.fit_transform(supplemental_df[target])
...
supplemental_df = prep_vars_verbose(supplemental_df)

X = train_df.drop(columns=[target, 'id'])
X_supplemental = supplemental_df.drop(columns=[target])
...
def run_holdout(i):
    ...
    X_train = pd.concat([X_train, X_supplemental], ignore_index=True)
    y_train = pd.concat([pd.Series(y_train), pd.Series(y_supplemental)], ignore_index=True)
...
# Extract weights
weights = df['MAP@3'].values

# Compute consensus (mode) and weighted average for each hyperparameter
consensus_params = {
    'subsample': mode(df['subsample']),
    'min_child_weight': mode(df['min_child_weight']),
    'max_depth': mode(df['max_depth']),
    'learning_rate': mode(df['learning_rate']),
    'colsample_bytree': mode(df['colsample_bytree']),
}

weighted_params = {
    'subsample': weighted_avg(df['subsample'], weights),
    'min_child_weight': weighted_avg(df['min_child_weight'], weights),
    'max_depth': int(round(weighted_avg(df['max_depth'], weights))),
    'learning_rate': weighted_avg(df['learning_rate'], weights),
    'colsample_bytree': weighted_avg(df['colsample_bytree'], weights),
}


gc.collect()
X_full = pd.concat([X_full, X_supplemental], ignore_index=True)
y_full = pd.concat([pd.Series(y_full), pd.Series(y_supplemental)], ignore_index=True)
