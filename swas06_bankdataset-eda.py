...
sns.pairplot(df_train_clean[['age', 'balance', 'day', 'duration', 'campaign', 'pdays', 'previous', 'y']])
...
X = df_train_clean.drop('y', axis='columns')
y = df_train_clean['y']
...
for feature in X.columns:
    if X[feature].dtype == 'object':
        _, iv = calculate_woe_iv(pd.concat([X, y],axis=1), feature, 'y' )
    else:
        X_binned = pd.cut(X[feature], bins=10, labels=False)
        _, iv = calculate_woe_iv(pd.concat([X_binned, y],axis=1), feature, 'y' )
