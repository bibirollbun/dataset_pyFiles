...
def filter_non_empty_masks(imgs, masks):
    new_imgs, new_masks = [], []
    ...
    for img, msk in zip(imgs, masks):
        ...
        if mask is None:
            continue
        if np.any(mask > 0):  # mask khÃ´ng rá»—ng
            new_imgs.append(img)
...
all_images_before = sorted(
    glob.glob(os.path.join(root_dir, "*.tif"))
)

# loáº¡i file mask
all_images_before = [
    p for p in all_images_before if not p.endswith("_mask.tif")
]

print(f"ğŸ“¦ Tá»•ng sá»‘ áº£nh TRÆ¯á»šC clean: {len(all_images_before)}")

all_masks_before = sorted(
    glob.glob(os.path.join(root_dir, "*_mask.tif"))
)

print(f"ğŸ�­ Tá»•ng sá»‘ mask TRÆ¯á»šC clean: {len(all_masks_before)}")


imgs, masks = analyze_and_clean_dataset(root_dir)
#imgs, masks = filter_non_empty_masks(imgs, masks)
...
print(f"ğŸ“¦ Tá»•ng sá»‘ mask sau clean: {len(masks)}")
...
mask_paths = sorted(glob.glob(os.path.join(root_dir, "*_mask.tif")))
img_paths  = [p.replace("_mask.tif", ".tif") for p in mask_paths]
train_i, val_i, train_m, val_m = train_test_split(
    img_paths,
    mask_paths,
    test_size=0.2,
    random_state=42,
    stratify=[cv2.imread(m, 0).sum() > 0 for m in mask_paths]  # â­� Ráº¤T QUAN TRá»ŒNG
)



train_ds = NerveClsDataset(train_i, train_m, transform=train_transform)
val_ds   = NerveClsDataset(val_i, val_m, transform=val_transform)

train_loader = DataLoader(train_ds, batch_size=8, shuffle=True, num_workers=2)
val_loader   = DataLoader(val_ds, batch_size=16, shuffle=False, num_workers=2)

train_no_mask = sum(1 for _, label in train_ds if label.item() == 0)
val_no_mask   = sum(1 for _, label in val_ds if label.item() == 0)

print(f"Train total: {len(train_ds)} | no-mask: {train_no_mask}")
print(f"Val   total: {len(val_ds)}   | no-mask: {val_no_mask}")


class NerveClassifier(nn.Module):
    def __init__(self, pretrained=True):
        super().__init__()
        self.backbone = models.resnet18(pretrained=pretrained)

        # thay fully connected cuá»‘i
...
cls_model = NerveClassifier().to(device)
optimizer = torch.optim.AdamW(cls_model.parameters(), lr=1e-4)
pos_weight = torch.tensor([2650 / 1858], device=device)
...
# best_epoch = 0
# wait = 0
# patience = 10        # sá»‘ epoch cho phÃ©p khÃ´ng cáº£i thiá»‡n
# max_epochs = 50     # tráº§n trÃªn, thÆ°á»�ng khÃ´ng cháº¡y tá»›i
...
#     # ===== EARLY STOPPING & SAVE =====
#     if auc > best_auc + 1e-4:   # trÃ¡nh nhiá»…u nhá»�
#         best_auc = auc
#         best_epoch = epoch + 1
#         wait = 0
...
#         )
#         break
# torch.save(cls_model.state_dict(), "last_cls_model.pth")
...
# ==== Ä�Æ°á»�ng dáº«n model ====
model_paths = {
    "best":  {"cls": "/kaggle/input/70pls/pytorch/default/1/best_cls_model.pth", "seg": "/kaggle/input/70pls/pytorch/default/1/best_model.pth"},
    "last":  {"cls": "/kaggle/input/70pls/pytorch/default/1/last_cls_model.pth", "seg": "/kaggle/input/70pls/pytorch/default/1/last_model (1).pth"}
}
...
# ==== Function cháº¡y inference ====
def run_inference(cls_model, seg_model, loader, cls_thres=0.4, seg_thres=0.9):
    cls_model.eval()
    seg_model.eval()
    results = []
    ...
    with torch.no_grad():
        for img, img_id in loader:
            ...
            cls_logit = cls_model(img)
            cls_prob = torch.sigmoid(cls_logit).item()

            if cls_prob < cls_thres:
                results.append({"img": int(img_id.item()), "pixels": ""})
                continue

            # 2ï¸�âƒ£ Segmentation
            prob = torch.sigmoid(seg_model(img))[0,0].cpu().numpy()
            prob = cv2.resize(prob, (580, 420), interpolation=cv2.INTER_LINEAR)
            mask = (prob > seg_thres).astype(np.uint8)
            ...
            rle = "" if mask.sum() == 0 else rle_encode(mask)
            results.append({"img": int(img_id.item()), "pixels": rle})

    submission = pd.DataFrame(results).sort_values("img")
    return submission

# ==== Cháº¡y tá»«ng cáº·p model ====
for key, paths in model_paths.items():
    print(f"Running inference for {key} models...")

    # Load classifier
    cls_model.load_state_dict(torch.load(paths["cls"], map_location=device))
    cls_model.to(device)

    # Load segmentation model
    model.load_state_dict(torch.load(paths["seg"], map_location=device))
    model.to(device)

    # Cháº¡y inference
    submission = run_inference(cls_model, model, test_loader, cls_thres, seg_thres)

    # LÆ°u file riÃªng
    out_file = f"submission_{key}.csv"
    submission.to_csv(out_file, index=False)
    print(f"âœ… Saved {out_file}")



class ValInferDataset(Dataset):
    def __init__(self, imgs, masks, transform=None):
        self.imgs = imgs
        self.masks = masks
        self.transform = transform
...
val_infer_ds = ValInferDataset(val_i, val_m, val_transform)
val_infer_loader = DataLoader(val_infer_ds, batch_size=1, shuffle=False)



def infer_on_val(cls_model, seg_model, loader, cls_thres=0.7, seg_thres=0.9, max_show=100):
    cls_model.eval()
    seg_model.eval()
    ...
    with torch.no_grad():
        for i, (img, gt_mask, img_path) in enumerate(loader):
            if i >= max_show:
                break

            img = img.to(device)
            gt_mask = gt_mask[0,0].cpu().numpy()

            # ---- Classification ----
            cls_prob = torch.sigmoid(cls_model(img)).item()

            if cls_prob < cls_thres:
                pred_mask = np.zeros_like(gt_mask)
            else:
                prob = torch.sigmoid(seg_model(img))[0,0].cpu().numpy()
                pred_mask = (prob > seg_thres).astype(np.uint8)
                pred_mask = remove_small_objects(pred_mask, min_size=100)
...
def show_results(results):
    for r in results:
        ...
        plt.subplot(1,3,2)
        plt.imshow(r["gt"], cmap="gray")
        plt.title("Ground Truth Mask")
        plt.axis("off")

        plt.subplot(1,3,3)
        plt.imshow(r["pred"], cmap="gray")
        plt.title(f"Predicted Mask\ncls_prob={r['cls_prob']:.2f}")
        plt.axis("off")
...
# Load model
cls_model.load_state_dict(torch.load(model_paths["best"]["cls"], map_location=device))
model.load_state_dict(torch.load(model_paths["best"]["seg"], map_location=device))

cls_model.to(device)
model.to(device)

# Inference
val_results = infer_on_val(
    cls_model,
    model,
    val_infer_loader,
    cls_thres=0.7,
