...
# 2ï¸�âƒ£ Gá»™p nhÃ¢n kháº©u há»�c (Ä‘Ãºng cá»™t lÃ  'subject')
train = train.merge(demo, on="subject", how="left")

# 3ï¸�âƒ£ XÃ¡c Ä‘á»‹nh danh sÃ¡ch sequence
sequence_ids = train["sequence_id"].unique()

imu_cols = ["acc_x", "acc_y", "acc_z", "rot_w", "rot_x", "rot_y", "rot_z"]
thermo_cols = [f"thm_{i}" for i in range(1, 6)]
tof_cols = [f"tof_{i}_v{j}" for i in range(1, 6) for j in range(8)]  # 5 cáº£m biáº¿n, 8 pixel má»—i cÃ¡i

# 4ï¸�âƒ£ Encode nhÃ£n gesture
encoder = LabelEncoder()
train["gesture_encoded"] = encoder.fit_transform(train["gesture"].astype(str))

# 5ï¸�âƒ£ Táº¡o chuá»—i cho má»—i sequence_id
T = 64  # sá»‘ frame má»—i chuá»—i
imu_list, thermo_list, tof_list, label_list = [], [], [], []

for seq in sequence_ids:
    df_seq = train[train.sequence_id == seq]
    if len(df_seq) < T:
        continue  # bá»� chuá»—i ngáº¯n
    
    imu_seq = df_seq[imu_cols].iloc[:T].to_numpy(dtype=np.float32)
    thermo_seq = df_seq[thermo_cols].iloc[:T].to_numpy(dtype=np.float32)
    tof_seq = df_seq[tof_cols].iloc[:T].to_numpy(dtype=np.float32).reshape(T, 5, 8)
    label_seq = df_seq["gesture_encoded"].iloc[0]
    
    # LÃ m sáº¡ch vÃ  chuáº©n hÃ³a
    imu_seq = np.nan_to_num(imu_seq)
    thermo_seq = np.nan_to_num(thermo_seq)
    tof_seq = np.nan_to_num(tof_seq)
    tof_seq[tof_seq < 0] = 0
    
    imu_seq = (imu_seq - imu_seq.mean()) / (imu_seq.std() + 1e-6)
    thermo_seq = (thermo_seq - thermo_seq.mean()) / (thermo_seq.std() + 1e-6)
    tof_seq = (tof_seq - tof_seq.mean()) / (tof_seq.std() + 1e-6)
    
    imu_list.append(imu_seq)
    thermo_list.append(thermo_seq)
    tof_list.append(tof_seq)
    label_list.append(label_seq)

# 6ï¸�âƒ£ Chuyá»ƒn sang máº£ng numpy
imu_array = np.stack(imu_list)
thermo_array = np.stack(thermo_list)
tof_array = np.stack(tof_list)
label_array = np.array(label_list, dtype=np.int64)

# 7ï¸�âƒ£ LÆ°u file .npz
np.savez("/kaggle/working/your_dataset.npz",
         imu=imu_array,
         thermo=thermo_array,
         tof=tof_array,
         label=label_array)

print("âœ… Saved dataset:", imu_array.shape, thermo_array.shape, tof_array.shape, label_array.shape)
...
# %% [config_and_data]
CONFIG = {
    'DATA_PATH': '/kaggle/working/your_dataset.npz',
    'N_SAMPLES': 900,
    'N_CLASSES': 6,
    'T': 64,
    'IMU_CH': 6,
    ...
    # kiáº¿n trÃºc
    'EMB_D': 128,                    # chiá»�u embedding cho má»—i modality
    'FUSE_HIDDEN': 256,             # hidden cho aggregator/head
    'DROPOUT': 0.3,
...
if HAS_TORCH:
    ...
    class FrameCNN2D(nn.Module):
        ...
        def forward(self, tof):
            # Náº¿u input cÃ³ dáº¡ng [B, T, H, W] thÃ¬ thÃªm channel dimension = 1
            if tof.dim() == 4:
                tof = tof.unsqueeze(2)  # -> [B, T, 1, H, W]
    
            B, T, C, H, W = tof.shape
            h = tof.reshape(B * T, C, H, W)
...
model = FrameCNN2D(out_d=64)
tof_fake = torch.randn(8, 64, 5, 8)  # [B, T, H, W] thiáº¿u channel
out = model(tof_fake)
print(out.shape)
...
if HAS_TORCH:
    def macro_score(y_true, y_pred): return f1_score(y_true, y_pred, average='macro')
    ...
    def run_epoch(model, loader, criterion, optimizer=None):
        ...
        return total / len(loader.dataset), f1_score(ys, yhs, average='macro')
...
# ==========================================================
# ğŸ�¯ DANH SÃ�CH GESTURE TARGET (BFRB-LIKE)
# ==========================================================
TARGET_GESTURES = [
    "above_ear_pull_hair",
    "forehead_pull_hairline",
    "forehead_scratch",
    "eyebrow_pull_hair",
    "eyelash_pull_hair",
    "neck_pinch_skin",
...
# ğŸ§© CÃ�C HÃ€M Há»– TRá»¢
# ==========================================================
def collapse_non_target(gesture_names, y_true, y_pred):
    """Gá»™p táº¥t cáº£ non-target gestures thÃ nh 'non_target'."""
    def collapse(name):
        return name if name in TARGET_GESTURES else "non_target"
    y_true_c = [collapse(gesture_names[i]) for i in y_true]
    y_pred_c = [collapse(gesture_names[i]) for i in y_pred]
    return np.array(y_true_c), np.array(y_pred_c)


def compute_competition_metric(y_true, y_pred, le_gesture):
    """
    TÃ­nh Ä‘Ãºng metric cuá»™c thi:
    - Binary F1: target vs non-target
    - Macro F1 (collapse non-target)
    - Final = trung bÃ¬nh cá»§a hai giÃ¡ trá»‹
    """
    gesture_names = le_gesture.classes_

    # Collapse non-target gestures
    y_true_c, y_pred_c = collapse_non_target(gesture_names, y_true, y_pred)

    # === 1ï¸�âƒ£ Binary F1
    y_true_binary = np.isin(y_true_c, TARGET_GESTURES).astype(int)
    y_pred_binary = np.isin(y_pred_c, TARGET_GESTURES).astype(int)
    binary_f1 = f1_score(y_true_binary, y_pred_binary, average='binary')

    # === 2ï¸�âƒ£ Macro F1 (collapsed)
    all_labels = np.unique(np.concatenate([y_true_c, y_pred_c]))
    label_map = {label: idx for idx, label in enumerate(all_labels)}
    y_true_m = np.array([label_map[l] for l in y_true_c])
    y_pred_m = np.array([label_map[l] for l in y_pred_c])
    macro_f1 = f1_score(y_true_m, y_pred_m, average='macro')
    ...
    return {
        "binary_f1": binary_f1,
        "macro_f1_collapsed": macro_f1,
        "final": final_score
    }


def select_model_for_sequence(seq_meta, models_dict):
    """
    Chá»�n model phÃ¹ há»£p cho má»—i sequence test:
    - Náº¿u chá»‰ cÃ³ IMU â†’ dÃ¹ng model IMU_ONLY
    - Náº¿u cÃ³ ToF hoáº·c Thermo â†’ dÃ¹ng model FULL
    """
    if not seq_meta.get("has_tof", False) and not seq_meta.get("has_thermo", False):
        return models_dict["IMU_ONLY"]
    else:
        return models_dict["FULL"]


# ==========================================================
# ğŸ§  HÃ€M EVALUATE CHÃ�NH - PHIÃŠN Báº¢N CHUáº¨N CUá»˜C THI
# ==========================================================
def evaluate_test(models_dict, test_loader, le_gesture):
    """
    Cháº¡y inference trÃªn test_loader vÃ  tÃ­nh Ä‘Ãºng metric cuá»™c thi.
    models_dict: {'IMU_ONLY': model1, 'FULL': model2}
    test_loader: iterator tráº£ (seq_data, seq_labels, seq_meta)
    """
    y_true_all, y_pred_all = [], []

    for batch in test_loader:
        # láº¥y dá»¯ liá»‡u cáº£m biáº¿n
        imu_b = batch.get('imu', None)
        th_b  = batch.get('thermo', None)
        tof_b = batch.get('tof', None)
        y_b   = batch.get('y', None)
    
        # táº¡o seq_meta giáº£ Ä‘á»‹nh dá»±a vÃ o dá»¯ liá»‡u cÃ³/khÃ´ng
        seq_meta = {
            "has_tof": tof_b is not None,
            "has_thermo": th_b is not None
        }
    
        # chá»�n model phÃ¹ há»£p
        model = select_model_for_sequence(seq_meta, models_dict)
    
        # dá»± Ä‘oÃ¡n
        if hasattr(model, "predict"):
            y_pred_seq = model.predict(batch)
        else:
            # náº¿u lÃ  mÃ´ hÃ¬nh torch
            model.eval()
            with torch.no_grad():
                inputs = []
                if imu_b is not None: inputs.append(imu_b.to(DEVICE).float())
                if th_b is not None:  inputs.append(th_b.to(DEVICE).float())
                if tof_b is not None: inputs.append(tof_b.to(DEVICE).float())
                logits = model(*inputs)
                if isinstance(logits, tuple): logits = logits[0]
                y_pred_seq = logits.argmax(dim=1).cpu().numpy()
    
        y_true_seq = y_b.cpu().numpy()
    
        y_true_all.extend(y_true_seq)
        y_pred_all.extend(y_pred_seq)
    
    # TÃ­nh Ä‘iá»ƒm
    scores = compute_competition_metric(
        np.array(y_true_all), np.array(y_pred_all), le_gesture
    )

    print("=" * 60)
    print(f"Binary F1 (target vs non-target): {scores['binary_f1']:.4f}")
    print(f"Macro F1 (collapsed non-target):  {scores['macro_f1_collapsed']:.4f}")
    print(f"â�¡ï¸� FINAL COMPETITION SCORE:       {scores['final']:.4f}")
    print("=" * 60)
    return scores
...
if HAS_TORCH:
    ...
    for name, cfg_run in CONFIG['RUNS']:
        ...
        # --- Evaluate both macro & binary F1 ---
        models_dict = {"FULL": model, "IMU_ONLY": model}  # táº¡m dÃ¹ng cÃ¹ng model
        scores = evaluate_test(models_dict, loaders['test'], le_gesture)
        macro_score = scores["macro_f1_collapsed"]
        binary_score = scores["binary_f1"]
        ...
        'exp': name,
        'fusion': fusion,
        'modalities': f"{use_imu}{use_th}{use_tof}",
        'params': int(nparam),
        'train_time_sec': float(train_time),
        'val_f1': float(hist['val_f1'][-1]),
        'test_f1': macro_score,
        'binary_f1': binary_score,
        })
        print(f"==> {name}: test macro-F1 = {macro_score:.4f} | binary-F1 = {binary_score:.4f}")
...
if HAS_TORCH and HAS_SCIPY:
    ...
    for s in seeds:
        ...
        class DS(Dataset):
            ...
            def __getitem__(self, i):
                return {
                    'imu': torch.from_numpy(self.p['imu'][i]),
        ...
        loaders2 = {
            nm: DataLoader(DS(split2[nm]), batch_size=CONFIG['BATCH_SIZE'], shuffle=(nm == 'train'))
        ...
        models_dict = {"FULL": m_imu, "IMU_ONLY": m_imu}
        scores_i = evaluate_test(models_dict, loaders2['test'], le_gesture)
        macro_i = scores_i["macro_f1_collapsed"]
        binary_i = scores_i["binary_f1"]
        f1_imu.append(macro_i)
        ...
        models_dict = {"FULL": m_attn, "IMU_ONLY": m_attn}
        scores_a = evaluate_test(models_dict, loaders2['test'], le_gesture)
        macro_a = scores_a["macro_f1_collapsed"]
        binary_a = scores_a["binary_f1"]
        f1_fuse.append(macro_a)

        print(f"Seed {s}: IMU-only={macro_i:.4f} | ATTN_ALL={macro_a:.4f}")
...
import pandas as pd
df = pd.read_csv("/mnt/data/multimodal_results.csv")
df.head()
...
# Ä�Æ°á»�ng dáº«n file káº¿t quáº£
path = "/mnt/data/multimodal_results.csv"

# Ä�á»�c file CSV
df = pd.read_csv(path)

# Hiá»ƒn thá»‹ toÃ n bá»™ cÃ¡c cá»™t cÃ³ sáºµn
print("CÃ¡c cá»™t cÃ³ trong file:")
print(df.columns.tolist())

# %% [process]
# Táº¡o thÃªm cÃ¡c cá»™t cáº§n thiáº¿t
df['model_name'] = df['exp']
df['notebook'] = "Notebook 4"
df['features_used'] = df['modalities']
df['window_size'] = 64  # giÃ¡ trá»‹ T trong Notebook 4
df['optimizer/solver'] = "Adam"
df['params (M)'] = (df['params'] / 1e6).round(3)
df['Final Score'] = ((df['test_f1'] + df['binary_f1']) / 2).round(4)

# Sáº¯p xáº¿p láº¡i thá»© tá»± cá»™t
cols = [
    'model_name', 'notebook', 'features_used', 'window_size',
    'optimizer/solver', 'params (M)', 'binary_f1', 'test_f1',
    'Final Score', 'val_f1', 'train_time_sec'
]
...
# LÆ°u láº¡i náº¿u muá»‘n xuáº¥t ra CSV má»›i
df_display.to_csv("/mnt/data/model_results_summary.csv", index=False)
print("\nâœ… Ä�Ã£ lÆ°u file: /mnt/data/model_results_summary.csv")
