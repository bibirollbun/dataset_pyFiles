...
for i in range(5):
    ...
    base_model = PawpularModel()
    model = Tez(base_model)
    model_path = f"/kaggle/input/training-v3-petfinder-my-pawpularity-contest/model_f{i}.bin"
    model.load(model_path, weights_only=True)
    
    # Load test data
    df_test = pd.read_csv("../input/petfinder-pawpularity-score/test.csv")
    test_img_paths = [
        f"../input/petfinder-pawpularity-score/test/{img_id}.jpg"
        for img_id in df_test["Id"].values
    ]
    ...
    for preds in tqdm(predictions):
        flattened_preds.extend(preds.ravel().tolist())
    
    super_final_predictions.append(flattened_preds)

# Aggregate predictions from all folds
super_final_predictions = np.mean(
    np.column_stack(super_final_predictions), axis=1
)
