...
ALT_MODEL = "swinv2_tiny_window12to24_192to384"  # Optional ensemble model
INPUT_SIZE = 512
BATCH_SIZE = 8
EPOCHS = 20
FOLD = 0
...
def get_transforms(input_size=INPUT_SIZE):
    train_transforms = A.Compose([
        A.RandomResizedCrop(size=(input_size, input_size), scale=(0.9, 1.0)),
        ...
        A.CoarseDropout(
            max_holes=4,
            max_height=int(input_size * 0.06),
            max_width=int(input_size * 0.06),
            min_holes=1,
            fill_value=0,
            p=0.2
        ),
...
def run_training(df, img_dir, fold=FOLD, n_splits=N_SPLITS, seed=SEED):
    ...
    num_workers = min(12, os.cpu_count())  # auto-tune
