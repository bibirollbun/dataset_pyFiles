...
train, _ = drop_id_feature(train, 'train_id')
test, test_id = drop_id_feature(test, 'test_id')
...
for feature in ['item_condition_id']:
    plot_categorical_features(original_train[[feature]])
    original_train = replace_rare_categories(original_train, feature, 'Other', threshold_percent=3)
    train          = replace_rare_categories(train, feature, -1, threshold_percent=3)
    plot_categorical_features(original_train[[feature]])
