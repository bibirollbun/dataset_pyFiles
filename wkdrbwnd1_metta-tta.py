...
def predict(sequence, demographics):
    ...
    # --- å�„ãƒ¢ãƒ‡ãƒ«ã�®ç¢ºç�‡åˆ†å¸ƒï¼ˆshape: [n_classes]ï¼‰ ---
    ...
    # --- ãƒ¢ãƒ‡ãƒ«é–“ã�®ä¸�ä¸€è‡´ï¼ˆå¹³å�‡Jensenâ€“Shannonè·�é›¢ï¼‰ ---
    ...
    # --- åŸºæœ¬ã�®å¹¾ä½•å¹³å�‡ï¼ˆlogç©ºé–“ã�®åŠ é‡�å’Œï¼‰ ---
    w = np.array([0.27, 0.35, 0.38], dtype=float)  # ãƒ¢ãƒ‡ãƒ«0,1,2 ã�®ãƒ™ãƒ¼ã‚¹é‡�ã�¿
    log_geom = (
    ...
    # --- "ä¸€ç•ªè‡ªä¿¡ã�®é«˜ã�„ãƒ¢ãƒ‡ãƒ«"ï¼ˆã‚¨ãƒ³ãƒˆãƒ­ãƒ”ãƒ¼æœ€å°�ï¼‰ ---
    def _entropy(p):
        q = np.clip(p, eps, 1.0)
    ...
    # --- ä¸�ä¸€è‡´åº¦ js ã�«å¿œã�˜ã�Ÿã‚½ãƒ•ãƒˆã‚²ãƒ¼ãƒˆ tï¼ˆ0â†’å¹¾ä½•å¹³å�‡, 1â†’ãƒ™ã‚¹ãƒˆå�˜ç‹¬å¯„ã‚Šï¼‰---
    a, b = 0.10, 0.16                      # ç·©ã‚„ã�‹ã�«åˆ‡æ›¿ã�ˆã‚‹å¸¯åŸŸ
    t = float(np.clip((js - a) / (b - a), 0.0, 1.0))

    # ãƒ™ã‚¹ãƒˆãƒ¢ãƒ‡ãƒ«ã‚’ã�»ã‚“ã�®å°‘ã�—ã� ã�‘ã‚·ãƒ£ãƒ¼ãƒ—åŒ–ï¼ˆæ¸©åº¦ 1.00â†’0.98ï¼‰
    tau = 1.0 - 0.02 * t
    log_best_sharp = log_best / tau

    # --- æœ€çµ‚ã‚¹ã‚³ã‚¢ï¼ˆlogç©ºé–“ç·šå½¢è£œé–“ â†’ argmax ã�§å��åˆ†ï¼‰---
...
def sequence_meta_features(sequence: 'pl.DataFrame|pd.DataFrame'):
    """ì‹œê³„ì—´ ì›�ë³¸ì—�ì„œ ìƒ˜í”Œ í†µê³„ í”¼ì²˜ë¥¼ ìƒ�ì„±.
    - ê¸¸ì�´, ê²°ì¸¡ë¹„ìœ¨, ì „ì—­ mean/std/skew/kurt
    - 1ì°¨ ì°¨ë¶„ í†µê³„, IQR, ì�´ìƒ�ì¹˜ ë¹„ìœ¨
    - ì—´ ë‹¨ìœ„ ìš”ì•½(ìƒ�ìœ„ 10ê°œ ì»¬ëŸ¼ í�‰ê· )
    ì�…ë ¥ì�´ polarsê±°ë‚˜ pandasì—¬ë�„ ë�™ì�‘í•˜ë�„ë¡� ìµœì†Œ í˜¸í™˜ ì²˜ë¦¬.
    """
    try:
        import polars as pl
...
def demographics_meta_features(demo: 'pl.DataFrame|pd.DataFrame'):
    ...
    # ìƒ�ìœ„ 4ê°œ ì—´ í�‰ê· (ê°€ë³€ì„± í�¡ìˆ˜ìš©)
    k = min(4, arr.shape[1])
...
def build_meta_vector(p1, p2, p3, seq, demo):
    """ì„¸ ëª¨ë�¸ í™•ë¥  + ë©”íƒ€í”¼ì²˜ë¥¼ í•˜ë‚˜ì�˜ ë²¡í„°ë¡œ ì§�ë ¬í™”"""
...
def fit_meta_model(train_sequences, train_demographics, train_targets,
    ...
    assert _HAS_LGB, 'lightgbmì�´ ì„¤ì¹˜ë�˜ì–´ì•¼ í•©ë‹ˆë‹¤.'
    X_list, y_list = [], []
...
def predict(sequence, demographics):
    """ê¸°ì¡´ predict ëŒ€ì²´: ë‚´ë¶€ì �ìœ¼ë¡œ meta-ensembleì�„ í˜¸ì¶œ"""
