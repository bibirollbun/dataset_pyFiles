...
test_dates = ["2024-06-03", "2024-06-16"]   # dự đoán trong 2 tuần kế tiếp
...
df_calendar['holiday_name'] = df_calendar['holiday_name'].replace(rename_dict)
...
# Iterate through each holiday in df_calendar with tqdm
for _, row in tqdm(df_calendar.iterrows(), total=len(df_calendar), desc="Processing Holidays"):
    ...
    if (holiday_name in ['Labour Day','Easter Monday', 'Easter Day','New Years Day', '1st Christmas Day' ]): # special holidays
    ...
    for i, date in enumerate(date_range):
        mask = (df_calendar['warehouse'] == warehouse) & (df_calendar['date'] == date)
        if is_spec and i==0:
            spec += 1
            df_calendar.loc[mask, 'holiday'] = 1.3 # Use higher weight for 2 special holidays
        else:
            df_calendar.loc[mask, 'holiday'] = 1

        # not fill holiday name for day after holiday
        if i+1!=len(date_range):
            df_calendar.loc[mask, 'holiday_name'] = holiday_name
...
new_df = df.merge(df_calendar_extended, on=['date', 'warehouse'], how='left')
new_df


df_calendar_extended.columns


df_calendar_extended[df_calendar_extended['days_to_holiday']>0][['date', 'holiday_name', 'holiday', 'shops_closed', 'warehouse', 'datetime',
       'days_to_holiday', 'days_to_shops_closed', 'day_after_closing',
       'long_weekend']][df_calendar_extended['holiday_name'].notna()].sample(20)
