...
class DogCatModel(nn.Module):
    def __init__(self, model_name="resnet34", pretrained=True):
        """
        事前学習済みのResNet34（モデル名は任意で変更可能）を特徴抽出のバックボーンとして使用し、
        最後にドロップアウトと全結合層を組み合わせたシンプルな二値分類モデル。
        ・backbone: timmライブラリで事前学習済みモデルをロードし、分類層は除去
        ・head: 特徴ベクトルから1次元のスコアに変換する全結合層
        """
        super().__init__()
        # 事前学習済みモデルのバックボーンを構築（分類層は使用しない）
    ...
    def forward(self, x):
        """
        入力画像xをバックボーンで特徴抽出し、全結合層でスコア化して返す。
        戻り値は1次元（バッチごと）で、BCEWithLogitsLoss等の損失関数に対応。
        """
        x = self.backbone(x)
        x = self.head(x)
        return x
