...
# Filter for business travel
business_travel = df[df['Type of Travel'] == 'Business travel']

# Count males and females in business travel
business_gender_counts = business_travel['Gender'].value_counts()

print(business_gender_counts)
...
# Count males and females
business_gender_counts = business_travel['Gender'].value_counts().reset_index()
business_gender_counts.columns = ['Gender', 'Count']

# Plot
plt.figure(figsize=(8,6))
sns.barplot(x='Gender', y='Count', data=business_gender_counts, palette='pastel', order=['Male', 'Female'])
plt.title('Number of Male and Female Passengers (Business Travel)')
plt.xlabel('Gender')
plt.ylabel('Count')
plt.show()
...
plt.pie(business_gender_counts, labels=business_gender_counts.index, autopct='%1.1f%%', colors=['#66b3ff','#ff9999'])
plt.title('Gender Distribution (Business Travel)')
plt.show()
