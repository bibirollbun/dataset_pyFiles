...
def set_all_seeds(seed=GLOBAL_SEED):
    """تولیدی صلاحیت کو یقینی بنانے کے لیے تمام ممکنہ بے ترتیب بیج سیٹ کریں۔"""
    random.seed(seed)
    np.random.seed(seed)
...
# کوڈ کا یہ چھوٹا سا بلاک "امپورٹ پری پروسیسنگ" کرتا ہے لیکن دوسرے ماڈیول کے ساتھ نام کے تصادم سے بچتا ہے۔
...
def parallelize_runs(gpu_quotas, task_usages, n_iterations, verbose=False):
    ...
    # میموری کے استعمال کو زیادہ سے زیادہ کرنے کے لیے لالچ سے کاموں کو شیڈول کریں۔
    t = time.time()
...
if __name__ == '__main__':
    ...
    # حل کو فارمیٹ کریں اور جمع کرانے والی فائل میں ڈالیں۔
...
# نوٹ بک میں ویژولائزیشن فنکشنز شامل کریں۔
import json
...
def visualize_arc_results():
    ...
    # چیک کریں کہ آیا جمع کرانے کی فائل موجود ہے۔
    ...
    # جمع کرانے کا ڈیٹا لوڈ کریں۔
    with open(submission_path, 'r') as f:
        submission_data = json.load(f)
    
    print(f"Loaded submission with {len(submission_data)} tasks")
    
    # ARC رنگین نقشہ - 0-9 اقدار کے لیے رنگ
    cmap = colors.ListedColormap(
        ['#000000', '#0074D9', '#FF4136', '#2ECC40', '#FFDC00',
         '#AAAAAA', '#F012BE', '#FF851B', '#7FDBFF', '#870C25'])
    norm = colors.Normalize(vmin=0, vmax=9)
    
    # یہ جانچنے کا فنکشن کہ آیا پیشن گوئی غیر معمولی ہے (صرف صفر نہیں)
    ...
    # ایک کام کے نتائج کو دیکھنے کے لیے فنکشن
    def visualize_submission_result(task_id, task_data, submission_output, test_idx):
        ...
        # تربیت کی مثالیں (صرف سادگی کے لیے پہلی)
        if task_data['train']:
            # ٹرین ان پٹ
...
# Path to your file
file_path = "/kaggle/working/submission.json"
