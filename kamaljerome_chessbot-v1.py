...
import sys
sys.path.append('/kaggle/input/botchess')
...
import time
import logging
import chess.svg  # For SVG representation
from IPython.display import display, SVG  # For rendering in Jupyter/Kaggle Notebook
...
class ChessBot:
    ...
    def get_best_move(self, fen, time_limit=0.5):
        """Returns the best move, all legal moves, and visual representation."""
        board = chess.Board(fen)
        start_time = time.time()
        ...
        return {
            "best_move": best_move,
            "legal_moves": legal_moves,
            "board": board  # Store board for visualization
        }
...
def display_board(board, highlight_move=None):
    ...
    # Generate SVG with move highlighting if applicable
    if highlight_move:
...
if __name__ == "__main__":
    ...
    print(f"\nâ™Ÿï¸� Best Move: {result['best_move']}")
    print(f"âœ… All Legal Moves: {result['legal_moves']}\n")

    # Display board before and after move
    display_board(result['board'])  # Before move

    # Apply the best move and show the updated board
    result['board'].push_uci(result['best_move'])
    print("\nğŸ”µ Board After Best Move:")
    display_board(result['board'], highlight_move=result['best_move'])
