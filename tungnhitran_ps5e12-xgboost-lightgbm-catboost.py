...
#print(num_cols.drop(['family_history_diabetes','hypertension_history','cardiovascular_history']))
cols_to_drop = ['family_history_diabetes','hypertension_history','cardiovascular_history']
#print([col for col in num_cols if col not in cols_to_drop])
#print(num_cols.drop(columns=[col for col in ['family_history_diabetes','hypertension_history','cardiovascular_history'] if col in num_cols.columns]))
...
train_transformed = encoder.transform_all(
    train,
    numeric_columns=[col for col in num_cols if col not in cols_to_drop],
    categorical_columns=cat_cols + cols_to_drop,
...
X = train_transformed.drop('diagnosed_diabetes', axis=1)
...
print("✓ Ensemble predictions created using weighted average")
print(f"  Number of test predictions: {len(ensemble_test_pred)}")

# Add submission
submission['diagnosed_diabetes'] = ensemble_test_pred
submission.to_csv('submission.csv', index=False)
