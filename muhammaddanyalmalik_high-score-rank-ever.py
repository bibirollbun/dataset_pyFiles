...
# n_splits = 15
# kf = StratifiedKFold(n_splits=n_splits, shuffle=True, random_state=44)
...
#     model = lgb.LGBMClassifier(
#         n_estimators=20000,    #5000 to 25000 estimators
#         learning_rate=0.06,
#         num_leaves=100,
#         max_depth=20,
#         min_child_samples=9,
...
#         reg_lambda=3.0,
#         max_bin=4523,
#         # min_split_gain=0.01,            # New: Minimum loss reduction required for split
#         # min_child_weight=1e-3,
#         # boosting_type='gbdt',           # Explicitly set gradient boosting
#         # objective='binary',             # Explicit objective
...
#         y_train, 
#         eval_set=[(X_val, y_val)], 
#         eval_metric=['auc', 'binary_logloss', 'accuracy'],# Multiple metrics
...
from sklearn.model_selection import StratifiedKFold  # <-- Missing import
from sklearn.metrics import accuracy_score

# Stratified K-Fold remains the same
n_splits = 5
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    model = xgb.XGBClassifier(
        n_estimators=30000,          # Early stopping will choose best iteration
        learning_rate=0.03,
        max_depth=8,                 # Lower than LightGBM (XGBoost prefers smaller trees)
        subsample=0.8,
        colsample_bytree=0.5,
        reg_alpha=0.1,               # L1 regularization
        reg_lambda=1.0,              # L2 regularization
        gamma=0.1,                   # Minimum loss reduction to split
        tree_method='hist',          # Faster with similar accuracy
...
print(f"\nXGBoost Mean CV Accuracy: {np.mean(fold_accuracies):.4f} (±{np.std(fold_accuracies):.4f})")
