...
cnn_model_den2.summary()


opt = tf.keras.optimizers.Adam(learning_rate = 1e-5)
cnn_model_den2.compile(loss = 'binary_crossentropy', optimizer = opt, metrics = ['AUC'])
...
lossden2, aucden2 = cnn_model_den2.evaluate(val_loader, verbose=0)
print(f"DenseNet121 with aug Test AUC: {aucden2:.4f}")


# Predict on new data

predictionden2 = cnn_model_den2.predict(sample)
print(f"DenseNet121 with aug predicted probability of class 1: {predictionden2[0][0]:.4f}")


display_perf(history_den2.history, 'DenseNet121 with aug Training Loss', 'DenseNet121 with aug Training AUC')
