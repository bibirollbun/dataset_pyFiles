...
def check_code_solution(solution, task_data):
    try:
        namespace = {}
        exec(solution, namespace)
        if 'p' not in namespace: return "not found p"
...
code_path = f"/kaggle/input/qwen2-5-32b-arc-local-score/submission/task{task_id}.py"
task_data = json.load(open(f"/kaggle/input/google-code-golf-2025/task{task_id}.json"))
solution_code = open(code_path, 'r').read()
check_code_solution(solution_code, task_data)


! rm -rf /kaggle/working/arc-agi-code-golf-216-400-solved
! mkdir -p /kaggle/working/arc-agi-code-golf-216-400-solved/submission
! unzip -qq /kaggle/input/arc-agi-code-golf-216-400-solved/submission.zip -d /kaggle/working/arc-agi-code-golf-216-400-solved/submission/
...
submisisons = [
    ...
    "/kaggle/input/remix-neurips-2025-google-code-golf",
    "/kaggle/working/arc-agi-code-golf-216-400-solved",
...
for task_num in range(1, 401):
    ...
    for submission_path in submisisons:
        ...
        if not os.path.exists(task_code_path):
            ...
            if not os.path.exists(task_code_path):
                continue
        with open(task_code_path, 'r', encoding="utf-8", errors = 'ignore') as f:
            solution_code = clean_code(f.read())
        if check_code_solution(solution_code, task_data) == "Success":
            task_code.append({"code": solution_code, "bytes": get_bytes(solution_code), "sub": submission_path})
            is_solved = True

    if not task_code:
        task_code.append({"code": simple_solution, "bytes": get_bytes(simple_solution), "sub": "not yet"})
        score = 0.001
    else:
        task_code_details = min(task_code, key=lambda x: x['bytes'])
        score = max(1, 2500 - task_code_details['bytes'])
        best_sub = task_code_details['sub']
        if best_sub not in sub_solved:
            sub_solved[best_sub] = 0
        solved += 1
        sub_solved[best_sub] += 1
        
        print(f"[green]{solved}. {task_id} - solved[/green] [pink]{task_code_details['sub']}[/pink]")
...
for sub, count in sub_solved.items():
    print(f"[pink]{sub} - {count}[/pink]")


! ls /kaggle/working/arc-agi-code-golf-216-400-solved/submission/task002.py
...
for task_num in range(1,401):
    task_id = f"{task_num:03d}"
    task_data_path = f"../input/google-code-golf-2025/task{task_id}.json"
    task_data_all[task_id] = json.load(open(task_data_path))
...
for task_num in range(1, 401):
    ...
    try:
        ...
        all_exampels = task_data_all[task_id]['arc-gen'] + task_data_all[task_id]['test']  + task_data_all[task_id]['train']
        solved = 0
        for e in all_exampels:
            test_input = e['input']
            expected = e['output']
            result = from_grid(verify_func(to_grid(test_input)))
            if result == expected:
                solved += 1
        if solved == len(all_exampels):
            print(f"[✔] Task {task_id} passed.")
            total_pass += 1
        else:
            print(f"[✘] Task {task_id} {solved}/{len(all_exampels)} failed.")
    except Exception as e:
        pass
print("Total Pass", total_pass)
# test 398 train 393 arc-gen 400


source = "/kaggle/working/submission"
...
submission = "/kaggle/working/submission/submission"
...
cp -r /kaggle/working/submission/submission.zip /kaggle/working/
