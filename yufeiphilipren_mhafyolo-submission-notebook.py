...
# Clear existing ultralytics from memory
for module in list(sys.modules.keys()):
    if module.startswith('ultralytics'):
        del sys.modules[module]

# Extract ultralytics from dataset
dataset_path = "/kaggle/input/ultralytics-for-offline-install2"
working_dir = Path("/kaggle/working")

# Find and extract archive
for file in os.listdir(dataset_path):
    if file.endswith(('.tar.gz', '.zip', '.tar')):
        archive_path = os.path.join(dataset_path, file)
        extract_dir = working_dir / "ultralytics_custom"
        
        print(f"Extracting {file}...")
        with tarfile.open(archive_path, 'r:*') as tar:
            tar.extractall(extract_dir)
        
        # Find ultralytics directory
        for item in extract_dir.rglob("*"):
            if item.is_dir() and "ultralytics" in item.name.lower():
                ultralytics_path = item.parent if item.name == "ultralytics" else item
                break
        else:
            ultralytics_path = next(extract_dir.iterdir())  # Use first directory
        
        # Add to Python path
        sys.path.insert(0, str(ultralytics_path))
        print(f"✓ Custom ultralytics ready from {ultralytics_path}")
        break

# Setup MHAF-YOLO
!cp -r /kaggle/input/mhafyolo/pytorch/default/1/MHAF-YOLO-main /kaggle/working/
mhaf_dir = Path("/kaggle/working/MHAF-YOLO-main")
sys.path.insert(1, str(mhaf_dir))
os.chdir(mhaf_dir)
...
try:
    ...
    from ultralytics import YOLO as YOLOv10  # Fallback
    print("✅ Using YOLO as YOLOv10 fallback")
...
CONFIDENCE_THRESHOLD = 0.78  # Lower threshold to catch more potential motors
...
SIZE = 1024
EARLY_STOPPING_TH = 0.01
ADAPTIVE_CONFIDENCE = False
USE_NOISE_AUGMENTATION = False      # Enable Gaussian noise
USE_BRIGHTNESS_AUGMENTATION = True # Enable brightness/contrast variations
NOISE_FACTOR = 0.01               # Gaussian noise strength 
BRIGHTNESS_ALPHA_RANGE = [0.9, 1.1] # Brightness multiplier range
CONTRAST_ALPHA_RANGE = [0.85, 1.15] # Contrast multiplier range

print(f"TTA Configuration:")
print(f"- Noise augmentation: {USE_NOISE_AUGMENTATION}")
print(f"- Brightness augmentation: {USE_BRIGHTNESS_AUGMENTATION}")
...
def quick_detection_check(model, image_np, device, img_size, threshold=0.1):
    """
    Quick check to see if slice has any potential detections
    """
    try:
        results = model(image_np, imgsz=img_size, device=device, verbose=False)
        
        max_confidence = 0.0
        for result in results:
            if result.boxes and len(result.boxes) > 0:
                confs = result.boxes.conf.cpu().numpy()
                max_confidence = max(max_confidence, np.max(confs))
...
def add_gaussian_noise(image, noise_factor=0.02):
    """
    Add subtle Gaussian noise to image
    """
    try:
        ...
        return image  # Return original if noise addition fails

def adjust_brightness_contrast(image, alpha=1.0, beta=0):
    """
    Adjust brightness and contrast
    alpha: contrast (1.0 = no change, >1.0 = more contrast, <1.0 = less contrast)
    beta: brightness (0 = no change, positive = brighter, negative = darker)
    """
    try:
...
def get_adaptive_confidence_threshold(slice_number, total_slices, base_threshold=CONFIDENCE_THRESHOLD):
    """
    Calculate adaptive confidence threshold based on slice position.
    Motors are more likely in middle slices, so we can be more permissive at edges.
    
    Args:
        slice_number: Current slice number (0-based)
        total_slices: Total number of slices in tomogram
        base_threshold: Base confidence threshold (default 0.8)
    ...
    # Normalize slice position to 0-1 range
    normalized_position = slice_number / (total_slices - 1)
    
    # Conservative approach: Only make edges slightly more permissive
    if normalized_position <= 0.15 or normalized_position >= 0.85:
        # Edge slices (first/last 15%) - slightly lower threshold
        return base_threshold * 0.9  # 0.8 -> 0.72
    elif normalized_position <= 0.25 or normalized_position >= 0.75:
        # Near-edge slices (15-25% from edges) - slightly lower threshold  
        return base_threshold * 0.95  # 0.8 -> 0.76
    else:
        # Middle slices (25-75%) - keep original threshold
        return base_threshold  # Keep 0.8
...
def predict_ensemble_tta(single_model, image_np, device, img_size):
    """
    Enhanced TTA with noise and brightness/contrast variations
    """
    all_boxes = []
    all_confs = []
    all_clss = []
    ...
    # NEW: Gaussian noise augmentation
    img_noise = add_gaussian_noise(image_np, noise_factor=0.02)
    do_infer(img_noise, invert_func=lambda x: x)
    
    # NEW: Brightness variations
    img_bright = adjust_brightness_contrast(image_np, alpha=1.1, beta=10)
    do_infer(img_bright, invert_func=lambda x: x)
    
    img_dark = adjust_brightness_contrast(image_np, alpha=0.9, beta=-10)
    do_infer(img_dark, invert_func=lambda x: x)
    
    # NEW: Contrast variations  
    img_high_contrast = adjust_brightness_contrast(image_np, alpha=1.15, beta=0)
    do_infer(img_high_contrast, invert_func=lambda x: x)
    
    img_low_contrast = adjust_brightness_contrast(image_np, alpha=0.85, beta=0)
    do_infer(img_low_contrast, invert_func=lambda x: x)
    ...
    # Apply weighted box fusion (your existing NMS)
    ...
    final_nms = list(nms_out[0])
    return final_nms


def make_predict(sub_path, model, device, img_size=1024):
    res_list = []
    for img in sub_path:
        ...
        should_process = quick_detection_check(model, img_np, device, SIZE, threshold=EARLY_STOPPING_TH)
    ...
    for batch_start in range(0, len(slice_files), BATCH_SIZE):
        ...
        for i, sub_batch in enumerate(sub_batches):
            ...
            with torch.cuda.stream(stream) if stream and device.startswith('cuda') else nullcontext():
                ...
                # Get total slices for adaptive thresholding
                total_original_slices = len(os.listdir(tomo_dir))
                ...
                for j,res in enumerate(sub_results):
                    ...
                    if ADAPTIVE_CONFIDENCE:
                        threshold_to_use = get_adaptive_confidence_threshold(
                            sub_batch_slice_nums[j], total_original_slices, CONFIDENCE_THRESHOLD
                        )
                        if threshold_to_use < CONFIDENCE_THRESHOLD:
                            print(f"    Edge slice {sub_batch_slice_nums[j]}: using threshold {threshold_to_use:.2f}")
                    else:
