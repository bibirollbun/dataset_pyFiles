...
class CustomModel(nn.Module):
    ...
    def monitor_metrics(self, outputs, targets, loss):
        ...
        try:
            auc = torch.tensor(metrics.roc_auc_score(targets.numpy(), probs.numpy()), device=outputs.device)
        except:
            auc = torch.tensor(0.0, device=outputs.device)
        return {"auc": auc, "bce": loss.detach()}
    ...
    def forward(self, image, features, targets=None):
        ...
        if targets is not None:
            ...
            metrics_dict = self.monitor_metrics(x, targets, loss)
            return x, loss, metrics_dict
        return x, 0, {}
...
df = pd.read_csv("/kaggle/input/bhargav-m-10140/train_5folds.csv")
df.head()
