...
# 分割特征和目标变量
# 特征选择 - 包括新构造的时间特征
features = ['bid_qty', 'ask_qty', 'buy_qty', 'sell_qty', 'volume', 'X1', 'hour', 'day', 'day_of_week'] # 选择的特征列
...
# 模型预测 - 对测试集进行类别预测
test_pred_lr = lr_model.predict(X_test_scaled)  # 生成测试集的类别预测结果

# 创建提交文件 - 按照竞赛要求的格式构建结果DataFrame
submission_lr = pd.DataFrame({
    'ID': test_data.index,          # 使用测试数据的索引作为ID
    'prediction': test_pred_lr         # 预测的类别标签
})

# 保存提交文件 - 输出为CSV格式
submission_lr.to_csv('submission.csv', index=False)  # 不保存行索引
