...
def h_blend(n_iter, params, color, cross='silver',
                    figures1=False, figures2=False,
                    wf2=555, details=False, show_process=False):
...
# ------------------------------------------------------------------------
    ...
    def da(dk,sorting_direction,show_details):
        ...
        if n_iter==0 or show_process==True:
    ...
    oo = 'random' in type_sort and n_iter == 0

    print(n_iter+1, ' of ', type_sort)
...
def r_blend(params,color,figures1,figures2,wf2,details,show_process=False):
    qnt_iter = int(params['type_sort'][0].replace('random-',''))
    ...
    for q in range(qnt_iter):
        ...
        df_R_cross =\
         h_blend(n_iter, 
          params,color,cross='silver',
           figures1=figures1, figures2=figures2, 
            wf2=wf2, details=details)
        
        df_R_cross.to_csv(f'submission_R_cross{n_iter}.csv', index=False)
        n_iter += 1  
        
    dfs = [pd.read_csv(f'submission_R_cross{n}.csv') for n in range(qnt_iter)]
    df_sample = pd.read_csv('/kaggle/input/playground-series-s5e10/sample_submission.csv')
    df_sample["accident_risk"] = sum([df["accident_risk"] for df in dfs]) / qnt_iter
    for n in range(qnt_iter): os.remove(f'submission_R_cross{n}.csv')
    return df_sample
...
df_cross = r_blend(params,color='brown',
                   figures1=True,figures2=True,
                   wf2=250,details=True,show_process=True)
...
df_cross1 = r_blend(params,color='red',figures1=True,figures2=True,wf2=250,details=True)
...
df_cross2 = r_blend(params,color='green',figures1=True,figures2=True,wf2=250,details=True)
...
df_cross3 = r_blend(params,color='blue',figures1=True,figures2=True,wf2=250,details=True)
...
df_alls = r_blend(params,color='alls',figures1=True,figures2=True,wf2=250,details=True)
...
df_g4 = r_blend(params,color='red',figures1=True,figures2=True,wf2=250,details=True)
...
df_g5 = r_blend(params,color='green',figures1=True,figures2=True,wf2=250,details=True)
...
df_g6 = r_blend(params,color='blue',figures1=True,figures2=True,wf2=250,details=True)
...
df_alls2 = r_blend(params,color='alls2',figures1=True,figures2=True,wf2=250,details=True)
...
df = r_blend(params,color='alls4',figures1=True,figures2=True,wf2=250,details=True)
...
df = r_blend(params,color='tes7',figures1=True,figures2=True,wf2=250,details=True)
...
df = r_blend(params,color='tes3',figures1=True,figures2=True,wf2=250,details=True)
...
df = pd.read_csv(path+'0.05548.a.csv'); df.to_csv('submission_0.05548.a.csv',index=False)
df = pd.read_csv(path+'0.05550.c.csv'); df.to_csv('submission_0.05550.c.csv',index=False)
...
df = r_blend(params,color='tes3b',figures1=True,figures2=True,wf2=250,details=True)

df.to_csv('submission_0.05546.b.csv',  index=False)

matrix_distances =\
 matrix_vs(f'/kaggle/working/submission_',
...
df.to_csv('submission_0.05546.0.01.csv', index=False)
df
...
df[target] = _46a[target] *1.027 - 0.027* _47a[target]

random.shuffle(dfs_47)
...
for df_47 in dfs_47:
    d *= 0.0027
    df[target] = df[target] *(1+d) - d* df_47[target]
