...
def download_file_with_timeout(url, dest_file):
    try:
...
def get_cached_file(url, cache_dir, file_name):
     full_file_path = os.path.join(cache_dir, file_name)
     if not os.path.exists(full_file_path):
         if not download_file_with_timeout(url, full_file_path):
             return None
     return full_file_path


class EssayDataset(Dataset):
    def __init__(self, topics, tokenizer, max_length=512):
        self.topics = topics
        self.tokenizer = tokenizer
        self.max_length = max_length
        self.essays = self._generate_pseudo_essays()

    def _generate_pseudo_essays(self):
        pseudo_essays = []
        for topic in self.topics:
            prompt = f"Write an essay about: {topic}"
            pseudo_essays.append(prompt)
        return pseudo_essays
    ...
    def __getitem__(self, idx):
        ...
        essay_encoding = self.tokenizer.encode_plus(
            essay,
            add_special_tokens=True,
            max_length=self.max_length,
            padding="max_length",
        ...
        return {
            "input_ids": topic_encoding["input_ids"].squeeze(),
            "attention_mask": topic_encoding["attention_mask"].squeeze(),
            "labels": essay_encoding["input_ids"].squeeze(),
            "decoder_attention_mask": essay_encoding["attention_mask"].squeeze(),
        }


 def create_model(model_name="t5-small", device="cuda"):
     tokenizer = None # tryブロック外で初期化
     model = None # tryブロック外で初期化
     # エラー対策として、明示的にモデルをキャッシュする（初回ダウンロード時）
     try:
         tokenizer = T5Tokenizer(model_name) # <- ここを変更
     except OSError as e:
         print(f"Error loading tokenizer: {e}")
         print(f"Please ensure the model '{model_name}' is available and your internet connection is working.")
         print(f"You can also try deleting the transformers cache directory: rm -rf ~/.cache/huggingface/transformers")
         # exit() トークナイザーのロードに失敗した場合、プログラムを終了
     ...
     try:
       model = T5ForConditionalGeneration.from_pretrained(model_name).to(device)
         ...
         print(f"Please ensure the model '{model_name}' is available and your internet connection is working.")
         print(f"You can also try deleting the transformers cache directory: rm -rf ~/.cache/huggingface/transformers")
         # exit()  # モデルのロードに失敗した場合、プログラムを終了
         return None, None # Noneを返す

     return tokenizer, model


def train_model(model, tokenizer, train_dataloader, optimizer, device, num_epochs=3):
  model.train()
...
def generate_essay(model, tokenizer, topic, device, max_length=512):
  model.eval()
  ...
  essay = tokenizer.decode(generated_ids[0], skip_special_tokens=True)
  return essay


def get_response(model, tokenizer, messages, device='cuda', trim_numbered_lists=True, max_tokens=None):
    # 入力からtopicを取得
    topic = messages[-1]["content"].replace("Topic: ", "").strip()
    ...
    with torch.no_grad():
        generated_ids = model.generate(
            input_ids=input_ids,
            max_length=max_tokens if max_tokens else 512,  # max_tokensがない場合はデフォルト値を指定
            num_beams=5,
            early_stopping=True,
            no_repeat_ngram_size=2,
        )
...
# def get_essays(df, model, tokenizer, device, verbose=True):
#   submission = pd.DataFrame()
#   submission['id'] = df['id']
#   submission['essay'] = ''

#   for i, row in df.iterrows():
#       essay = generate_essay(model, tokenizer, row['topic'], device)
#       submission.loc[i, 'essay'] = essay
#       if verbose:
#           print(f"Topic: {row['topic']}\\n")
#           print(f"Generated Essay: {essay}\\n\\n")
...
if len(test_df) == 3:
    ...
    demo_df = get_essays(demo_df, tokenizer, messages_and_prompts)
...
if __name__ == '__main__':
    ...
    tokenizer, model = create_model(device=device)
    if tokenizer is None or model is None:
        print("Error loading tokenizer or model.")
        exit()

    dataset = EssayDataset(topics, tokenizer)
    dataloader = DataLoader(dataset, batch_size=4, shuffle=True)

    optimizer = AdamW(model.parameters(), lr=5e-5)
    
    train_model(model, tokenizer, dataloader, optimizer, device)
    
    # 推論部分の置き換え
    test_df = pd.read_csv("/kaggle/input/llms-you-cant-please-them-all/test.csv")
    submission = get_essays(test_df, model, tokenizer, device, verbose=True)
