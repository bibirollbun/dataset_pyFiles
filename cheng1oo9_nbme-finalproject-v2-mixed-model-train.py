...
# -------------- 1. 全域設定 -------------------------------
class CFG:
    data_dir      = Path("/kaggle/input/nbme-score-clinical-patient-notes")  # Local raw‑data directory
    ...
    ensemble_model_name = "/kaggle/input/bio_clinicalbert_offline/transformers/default/1"
    ...
    inference_fold = 3
    checkpoint_dir = Path("/kaggle/input/fold-checkpoint")
...
def main():
    ...
    total_steps = 1 * (len(trn_ds) // CFG.batch_size) // CFG.gradient_accum
    ...
    for fold,(trn_idx,val_idx) in enumerate(gkf.split(train, groups=train.pn_num)):
        ...
        total_steps = CFG.epochs * (len(trn_ds) // CFG.batch_size) // CFG.gradient_accum
        warmup_steps = int(total_steps * CFG.warmup_ratio)
