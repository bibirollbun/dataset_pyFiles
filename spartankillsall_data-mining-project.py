...
columns=['CGPA','Academic Pressure','Study Satisfaction']
data = data.drop(columns=['CGPA','Academic Pressure','Study Satisfaction'])
...
def predict_proba_lime(x):
    ...
    if isinstance(x, np.ndarray):
        ...
        return model_to_explain.predict_proba(x_df)
    else:
...
def explain_instance_lime(idx):
    """
    Explain the prediction for a single test instance with LIME.
    idx: row index in X_test
    """
    instance = X_test_np[idx]

    print("="*60)
    print(f"LIME explanation for test index: {idx}")
    print("True label:", class_names[int(y_test.iloc[idx])])
    print("Predicted probabilities:",
          model_to_explain.predict_proba(X_test.iloc[[idx]])[0])
...
def make_predict_proba_fn(model, feature_names):
    def predict_proba_lime(x):
        x_df = pd.DataFrame(x, columns=feature_names)
...
def explain_instance_lime_for_model(model_name, model, idx):
    ...
    instance = X_test_np[idx]
    predict_proba_fn = make_predict_proba_fn(model, feature_names)
    ...
    fig = lime_exp.as_pyplot_figure(label=label_to_show)
    plt.title(f"LIME for {model_name}\nTest idx {idx}, label {label_name}")
    plt.tight_layout()
    plt.show()
...
np.random.seed(42)
sample_indices = np.random.choice(len(X_test), size=3, replace=False)  # change size if you want
...
def get_shap_explainer(model_name, model, X_train):
    """
    Choose an appropriate SHAP explainer for the given model.
    """
   
    if isinstance(model, (RandomForestClassifier, DecisionTreeClassifier, AdaBoostClassifier)):
        print(f"[{model_name}] Using TreeExplainer")
        explainer = shap.TreeExplainer(model)
    
   
    elif isinstance(model, LogisticRegression):
        print(f"[{model_name}] Using LinearExplainer")
        explainer = shap.LinearExplainer(model, X_train)
    
    
        print(f"[{model_name}] Using KernelExplainer (may be slow)")
...
def get_positive_class_shap_values(shap_values):
    ...
    Always return a 1D array of length n_features for a single instance.
    """
...
def get_positive_expected_value(explainer):
    """
    For binary classification:
    - if expected_value is array-like, take index 1
    - else return as is
    """
...
np.random.seed(42)
indices_for_local = list(np.random.choice(len(X_test), size=3, replace=False))

for idx in indices_for_local:
    for model_name, model in models.items():
