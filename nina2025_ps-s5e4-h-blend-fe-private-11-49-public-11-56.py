...
def bokeh_show(
    ...
    if show_figures2 == True:
        ...
        line_y     = [dfs[i][b:e]['Listening_Time_minutes'] for i in range(len(dfs))]
    ...
    def load(path,fs_names):
        ...
        for i in range(len(dfs)):
            dfs[i] = dfs[i].rename(columns={
                "target": f'{fs_names[i]}',"Listening_Time_minutes": f'{fs_names[i]}'})
...
path ='/kaggle/input/ps-s5e4/submission '

params = {
      'path'     : path,            
      'id_target': ['id',"Listening_Time_minutes"],          
      'type_sort': ['asc/desc',0.30, 0.70],
      'subwts'   : [ +0.09,+0.04, -0.01,-0.04,-0.08 ],       
      'subm'     : [ 
         { 'name': f'11.56519', 'weight': +0.89 },
         { 'name': f'11.64392', 'weight': +0.07 },
         { 'name': f'11.70484', 'weight': +0.04 },
         { 'name': f'11.75081', 'weight': +0.01 },
         { 'name': f'11.76294', 'weight': -0.01 },]
}
