...
def train_advanced_models(X_train, y_train, X_test, cat_features):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
        ...
        model = XGBClassifier(
            ...
            tree_method='gpu_hist',  # Use GPU for histogram construction
            eval_metric='auc',
            early_stopping_rounds=100
        )
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_train, y_train)):
        ...
        model = LGBMClassifier(
            ...
            gpu_device_id=1      # Use second T4 GPU
        )
...
def main_pipeline():
    ...
    try:
        best_public = pd.read_csv("/kaggle/input/luckymy/0.97772Nina.csv")
        
        # Weighted average with emphasis on the best model output
        submission['y'] = 0.05 * submission['y'] + 0.95 * best_public['y']
