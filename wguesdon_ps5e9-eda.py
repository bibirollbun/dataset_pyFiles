...
def perform_eda():
    ...
    if significant_diffs:
        logger.warning(f"\nâš ï¸� {len(significant_diffs)} features show significant distribution differences")
    ...
    logger.info("\nğŸ“Š Based on EDA, consider these feature engineering approaches:")
    ...
    logger.info("   - Energy Ã— RhythmScore")
    logger.info("   - VocalContent Ã— AcousticQuality")
    logger.info("   - MoodScore Ã— Energy")
    ...
    logger.info("\nğŸ“ˆ Key Findings:")
    logger.info(f"  â€¢ Dataset: {len(train_df_analysis):,} training, {len(test_df_analysis):,} test samples")
    logger.info(f"  â€¢ Features: {len(feature_cols)} audio/music features")
    logger.info(f"  â€¢ Target: BPM (continuous, range: {train_df_analysis[TARGET_COL].min():.1f}-{train_df_analysis[TARGET_COL].max():.1f})")
    logger.info(f"  â€¢ Missing data: {'None' if missing_counts.sum() == 0 else f'{missing_counts.sum()} columns'}")
    logger.info(f"  â€¢ Outliers: {len(outliers)} BPM outliers ({len(outliers)/len(train_df_analysis)*100:.2f}%)")
    
    # Find strongest predictors
    top_predictors = correlations[1:4].index.tolist()
    logger.info(f"  â€¢ Top predictors: {', '.join(top_predictors)}")
    ...
    logger.info("  4. Use RMSE as primary metric, monitor MAE and RÂ²")
