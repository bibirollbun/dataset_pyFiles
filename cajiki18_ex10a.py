...
# print("new_merchant_transactions.csv の統計量:")
# print(new_trans.describe())

# # historical_transactions.csv の読み込みを追加
# hist_trans = pd.read_csv(f"{DATA_DIR}/historical_transactions.csv")
# print("historical_transactions.csv の統計量:")
# print(hist_trans.describe())
...
agg_funcs = {
    "purchase_amount": ["min", "max", "mean", "var", "skew"], # 正規化済みの購入金額（負値 = 顧客の支出）
    "installments":    ["min", "max", "mean", "sum"], # 分割払いの分割回数
    "month_lag":       ["min", "max", "mean", "var"], # 基準日からの経過月数
    "purchase_date":   ["count", "nunique"],      # 購入日時（countで空でない行を数える ⇔ 購入回数）
}
