...
# Function to load a valid DICOM file
def load_dicom(path):
    dicom = pydicom.dcmread(path)
    data = dicom.pixel_array
    data = data - np.min(data)
...
# Filter the dataset to include only valid DICOM files
def filter_valid_dicoms(df):
    valid_paths = []
    for path in df['image_path']:
        if is_valid_dicom(path):
            valid_paths.append(path)
        else:
            print(f"Skipping invalid DICOM file: {path}")
...
train_data = filter_valid_dicoms(train_data)
...
#self.model.load_state_dict(torch.load(pretrained_weights)) kısmını self.model.load_state_dict(torch.load(pretrained_weights), strict=False) olarak degistirdim
