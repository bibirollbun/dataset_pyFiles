...
class EnsembleTrainer:
    ...
    def fit_predict(self, X, y, X_test):     
        ...
        return oof_preds, test_preds, coeffs, scores, overall_score
...
all_features = list(oof_preds.keys())
selected_features = [f for f in all_features if f not in removed_features]

ridge_X = pd.DataFrame(oof_preds)[selected_features]
ridge_X_test = pd.DataFrame(test_preds)[selected_features]


ridge_model = Ridge(**{
    "random_state": 42,
    "alpha": 0.12744597290725007,
    "tol": 0.0002968892155354931
})
ridge_trainer = EnsembleTrainer(ridge_model, "quantile", CFG)
ridge_oof_preds, ridge_test_preds, coeffs, scores["l2_ensemble_ridge"], overall_scores["l2_ensemble_ridge"] = ridge_trainer.fit_predict(ridge_X, y, ridge_X_test)
l2_oof_preds["l2_ensemble_ridge"], l2_test_preds["l2_ensemble_ridge"] = ridge_oof_preds, ridge_test_preds


plot_weights(ridge_X.columns.tolist(), coeffs, "Ridge Coefficients")


hc_X = pd.DataFrame(oof_preds)
hc_X_test = pd.DataFrame(test_preds)

scaler = RobustScaler().set_output(transform="pandas")
hc_X = scaler.fit_transform(hc_X)
hc_X_test = scaler.transform(hc_X_test)


model_weights = {
    'neural_network': 0.450,
    'xgb_quantile_non_creator_early_stopped': -0.273,
    'cb_cox_loss': 0.155,
    'ag_kaplan_meier': 0.147,
    'lgb_goss_quantile': -0.151,
    'neural_network_fe': 0.140,
    'cb_cox_loss_cretor_only': 0.065,
    'histgb_rank_log': -0.126,
    'xgb_rank_log': 0.053,
    'cb_nelson_aalen_non_creator_early_stopped': -0.091,
    'ag_cox': 0.099,
    'lgb_kaplan_meier_non_creator': -0.087,
    'xgb_nelson_aalen_onehot': 0.042,
    'xgb_kaplan_meier_cretor_only': -0.038,
    'lgb_kaplan_meier': 0.066,
    'xgb_cox_loss': -0.044,
    'cb_cox_loss_targetencoder': 0.031,
    'xgb_nelson_aalen_cretor_only': -0.079,
    'cb_quantile': 0.047,
    'xgb_quantile_onehot': -0.039,
    'xgb_cox_loss_targetencoder': 0.011,
    'xgb_cox_loss_non_creator': -0.014,
    'lgb_nelson_aalen_cretor_only': 0.036,
    'ag_quantile': -0.063,
    'lgb_exponential_rank': 0.044,
    'cb_cox': -0.026,
    'cb_rank_log': 0.025,
    'lgb_quantile_non_creator': -0.048,
    'histgb_quantile': 0.043,
    'histgb_nelson_aalen': -0.030,
    'xgb_cox': 0.012,
    'cb_cox_loss_onehot': 0.016,
    'cb_quantile_cretor_only': -0.028,
    'lgb_cox': 0.018,
    'lgb_goss_exponential_rank': -0.037,
    'xgb_nelson_aalen_targetencoder': 0.036,
    'xgb_separate': 0.017,
    'lgb_nelson_aalen_non_creator_early_stopped': -0.061,
    'lgb_nelson_aalen': 0.054,
    'lgb_quantile': -0.030,
    'xgb_quantile_targetencoder': 0.027,
    'lgb_kaplan_meier_onehot': -0.019,
    'cb_quantile_non_creator': 0.010,
    'histgb_separate': -0.011,
    'xgb_cox_loss_onehot': 0.001,
    'lgb_quantile_onehot': -0.003,
    'xgb_exponential_rank_non_creator_early_stopped': 0.006,
    'xgb_exponential_rank': 0.015,
    'cb_quantile_non_creator_early_stopped3': -0.024,
    'lgb_kaplan_meier_cretor_only': 0.002,
    'xgb_nelson_aalen_non_creator': -0.003,
    'lasso_quantile_non_creator': 0.001,
    'lgb_quantile_targetencoder': -0.001,
    'lgb_kaplan_meier_targetencoder': 0.001,
    'xgb_kaplan_meier_targetencoder': -0.001
}

hc_oof_preds = hc_X['ag_exponential_rank']
hc_test_preds = hc_X_test['ag_exponential_rank']
for k, v in model_weights.items():
    hc_oof_preds = (1 - v) * hc_oof_preds + v * hc_X[k]
    hc_test_preds = (1 - v) * hc_test_preds + v * hc_X_test[k]
    
l2_oof_preds["l2_ensemble_hc"], l2_test_preds["l2_ensemble_hc"] = hc_oof_preds, hc_test_preds
...
def eval_metric(p):
    ...
    return score_f(y_true, p)

hc_score = eval_metric(hc_oof_preds)
scores["l2_ensemble_hc"] = [hc_score] * 10
overall_scores["l2_ensemble_hc"] = hc_score
...
ag_X_test = pd.DataFrame(test_preds)[selected_features]
...
ag_oof_preds = predictor.predict_oof(best_model).values
ag_test_preds = predictor.predict(ag_X_test).values

l2_oof_preds["l2_ensemble_ag"], l2_test_preds["l2_ensemble_ag"] = ag_oof_preds, ag_test_preds


y_true = train[["efs", "efs_time", "race_group"]].copy()
overall_scores["l2_ensemble_ag"] = score_f(y_true, ag_oof_preds)
...
for fold_idx, (train_index, val_index) in enumerate(split):
    ...
    fold_score = score_f(y_true_fold, l2_oof_preds["l2_ensemble_ag"][val_index])

    if "l2_ensemble_ag" not in scores:
        scores["l2_ensemble_ag"] = []
    scores["l2_ensemble_ag"].append(fold_score)


def objective(trial):
    weights = [trial.suggest_float(f'weight_{name.lower()}', 0.0, 1.0) for name in l2_oof_preds.keys()]
    
    preds = np.zeros(train.shape[0])
    for model_preds, weight in zip(l2_oof_preds.values(), weights):
        preds += rankdata(model_preds) * weight
        
    y_true = train[['efs', 'efs_time', 'race_group']].copy()
    return score_f(y_true, preds)
...
scores['l3_weighted_ensemble'] = [study.best_value] * CFG.n_folds
overall_scores['l3_weighted_ensemble'] = study.best_value

best_weights = [study.best_params[f'weight_{name.lower()}'] for name in l2_oof_preds.keys()]


weighted_test_preds = np.zeros((test.shape[0]))
for model, weight in zip(l2_test_preds.keys(), best_weights):
    weighted_test_preds += rankdata(l2_test_preds[model]) * weight


# Compute weighted OOF predictions using the best weights
weighted_oof_preds = np.zeros(train.shape[0])
for model, weight in zip(l2_oof_preds.keys(), best_weights):
    weighted_oof_preds += rankdata(l2_oof_preds[model]) * weight



weighted_oof_preds.max()
...
y_pred["prediction"] = weighted_oof_preds
print(y_pred)
...
oof_lgbm = np.zeros(len(train))  # Store direct predictions
oof_probas = np.zeros(len(train))  # Store probabilities for AUC
pred_efs = np.zeros(len(test))  # Store test set probabilities
...
for i, (train_index, test_index) in enumerate(kf.split(train, train["efs"])):
    ...
    x_train = train[feature_list].loc[train_index].copy()
    y_train = train.loc[train_index, "efs"]
    x_valid = train[feature_list].loc[test_index].copy()
    y_valid = train.loc[test_index, "efs"]
    x_test = test[feature_list].copy()

    params = {'max_depth': 2, 'min_child_samples': 16, 'subsample': 0.5638291441858686,
              'colsample_bytree': 0.11388524113299561, 'reg_alpha': 0.04071631148843554, 'reg_lambda': 1.468813561417093}
    ...
    oof_lgbm[test_index] = (model_lgbm.predict_proba(x_valid)[:, 1] > 0.5).astype(int)
    # INFER TEST (Probabilities -> Average Probs)
    pred_efs += model_lgbm.predict_proba(x_test)[:, 1]

# COMPUTE AVERAGE TEST PREDS
pred_efs = (pred_efs / kf.get_n_splits() > 0.5).astype(int)

# EVALUATE PERFORMANCE
accuracy = accuracy_score(train["efs"], oof_lgbm)
f1 = f1_score(train["efs"], oof_lgbm)
roc_auc = roc_auc_score(train["efs"], oof_lgbm)
...
oof_ensemble_risk = weighted_oof_preds



bin_pred_np = np.array(bin_pred)
oof_ensemble_risk = np.array(oof_ensemble_risk)

combined_array = np.column_stack((bin_pred_np, oof_ensemble_risk))
...
bin_pred_np = np.array(bin_pred)
oof_ensemble_risk = np.array(weighted_oof_preds)
...
best_threshold = None
0
for threshold in np.arange(3000, 3300, 1): 
    temp_combined = combined_array.copy() 
    temp_combined[temp_combined[:, 0] == 1, 1] += threshold
    y_pred["prediction"] = temp_combined[:, 1]
    ...
    print(f"Threshold: {threshold:.2f}, Score: {score_value}")

# Print best result
print(f"\nBest Threshold: {best_threshold}, Best Score: {best_score}")
...
combined_array[combined_array[:, 0] == 1, 1] += best_threshold
...
prediction_np = np.array(weighted_test_preds)
combined_pred = np.column_stack((pred_classifier_np, prediction_np))
print(combined_pred)  # (2, 3)



combined_pred[combined_pred[:, 0] == 1, 1] += best_threshold
print(combined_pred)  # (2, 3)



scores["masked_score"] = [m] * 10
overall_scores["masked_score"] = m
