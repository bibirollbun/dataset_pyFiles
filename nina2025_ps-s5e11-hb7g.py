...
params = {
  'path'     : f"/kaggle/input/13-november-2025-ps-s5e11/submission 0.92",            
  'id_target': ['id',"loan_paid_back"],          
  'type_sort': ['asc/desc',0.30,0.70 ],
  'subwts'   : [ +0.07,-0.01,-0.02,-0.04 ],
  'subm'     : [
      {'name': f'719',  'weight':+0.25, 'color':'brown'     },
      {'name': f'721',  'weight':+0.25, 'color':'maroon'    },
      {'name': f'722',  'weight':+0.25, 'color':'sienna'    },
      {'name': f'727.b','weight':+0.25, 'color':'chocolate' },]
}
df = h_blend(params, details=True, subm='submission_0.92753.csv')


params = {
  'path'     : f"/kaggle/input/11-november-2025-ps-s5e11/submission 0.92",            
  'id_target': ['id',"loan_paid_back"],          
  'type_sort': ['asc/desc',0.30,0.70 ],
  'subwts'   : [ +0.07,-0.01,-0.02,-0.04 ],
  'subm'     : [
      {'name': f'664', 'weight':+0.25, 'color':'silver'  },
      {'name': f'684', 'weight':+0.25, 'color':'darkgray'},
      {'name': f'686', 'weight':+0.25, 'color':'gray'    },
      {'name': f'692', 'weight':+0.25, 'color':'crimson' },]
      
}
df = h_blend(params, details=True, subm='submission_0.92726.csv')
...
params = {
  ...
  'subm'     : [
      {'name': f'730', 'weight':+0.30, 'color':'navy'},
      {'name': f'735a','weight':+0.35, 'color':'darkmagenta'},
      {'name': f'735b','weight':+0.35, 'color':'magenta' },]
      
}
df = h_blend(params, details=True, subm='submission_0.92757.csv')


df_0_92_756 = pd.read_csv('/kaggle/input/13-november-2025-ps-s5e11/submission 0.92756.csv')
df_0_92_756 . to_csv('submission_0.92756.csv',index=False)

df_0_92_767 = pd.read_csv('/kaggle/input/18-november-2025-ps-s5e11/submission 0.92767.n.csv')
df_0_92_767 . to_csv('submission_0.92767.csv',index=False)


params = {
  'path'     : f"/kaggle/working//submission_0.92",            
  'id_target': ['id',"loan_paid_back"],          
  'type_sort': ['asc/desc',0.30,0.70 ],
  'subwts'   : [ +0.07,-0.01,-0.02,-0.04 ],
  'subm'     : [
      {'name': f'726', 'weight':+0.12, 'color':'dimgray'    },
      {'name': f'753', 'weight':+0.28, 'color':'maroon'     },
      {'name': f'756', 'weight':+0.32, 'color':'gold'       },
      {'name': f'757', 'weight':+0.28, 'color':'darkmagenta'},]
      
}
df = h_blend(params, details=True, subm='submission_0.92766.a.csv')
...
params = {
  ...
  'subwts'   : [ +0.07,-0.01,-0.02,-0.04 ],
  'subm'     : [
      {'name': f'726', 'weight':+0.25, 'color':'dimgray'    },
      {'name': f'753', 'weight':+0.25, 'color':'maroon'     },
      {'name': f'756', 'weight':+0.25, 'color':'gold'       },
      {'name': f'757', 'weight':+0.25, 'color':'darkmagenta'},]
      
}
df = h_blend(params, details=True, subm='submission_0.92764.csv')


df_726 = pd.read_csv('submission_0.92726.csv')
df_753 = pd.read_csv('submission_0.92753.csv')
df_756 = pd.read_csv('submission_0.92756.csv')
df_757 = pd.read_csv('submission_0.92757.csv')

df = pd.read_csv('/kaggle/input/playground-series-s5e11/sample_submission.csv')

df['loan_paid_back'] = \
    df_726['loan_paid_back'] * 0.10 + \
    df_753['loan_paid_back'] * 0.25 + \
    df_756['loan_paid_back'] * 0.30 + \
    df_757['loan_paid_back'] * 0.35

df.to_csv('submission_0.92_766.b',index=False)
...
params = {
  ...
  'id_target': ['id',"loan_paid_back"],          
  'type_sort': ['asc/desc',0.30,0.70 ],
  'subwts'   : [ -0.05, +0.10, -0.05 ],
  'subm'     : [
      {'name': f'726', 'weight':+0.15, 'color':'crimson'     },
      {'name': f'753', 'weight':+0.35, 'color':'maroon'      },
      {'name': f'757', 'weight':+0.50, 'color':'darkmagenta' },]
}
df = h_blend(params, details=True, subm='submission_763.csv')


df_767 = pd.read_csv('/kaggle/input/18-november-2025-ps-s5e11/submission 0.92767.n.csv')
df_763 = pd.read_csv('/kaggle/working/submission_763.csv')
df_623 = pd.read_csv('/kaggle/input/13-november-2025-ps-s5e11/submission 0.92623.csv')

df = pd.read_csv('/kaggle/input/playground-series-s5e11/sample_submission.csv')

df['loan_paid_back'] = \
    df_767['loan_paid_back'] * 0.97 + \
    df_763['loan_paid_back'] * 0.02 + \
    df_623['loan_paid_back'] * 0.01
