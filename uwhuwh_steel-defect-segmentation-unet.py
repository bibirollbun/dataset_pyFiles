...
# 1) EÄŸitim SÃ¼reci Grafiklerini Ã‡iz
...
# 2) SÄ±nÄ±f BazÄ±nda Dice SkorlarÄ±
@torch.no_grad()
...
# 3) Ã–rnek Prediction GÃ¶rselleÅŸtirme
@torch.no_grad()
...
# --- EÄŸitim sonrasÄ± analiz ve gÃ¶rselleÅŸtirme ---

# 1) EÄŸitim sÃ¼reci grafikleri (loss, dice, lr)
plot_history(history, show=True)        # hem kaydet hem ekranda gÃ¶ster
plot_training_curves(history)           # subplot ile daha ÅŸÄ±k gÃ¶rÃ¼nÃ¼m

# 2) SÄ±nÄ±f bazÄ±nda Dice skorlarÄ± (validation set Ã¼zerinden)
plot_per_class_dice(model, val_loader, device)

# 3) Ã–rnek prediction gÃ¶rselleri (GT vs Pred overlay)
