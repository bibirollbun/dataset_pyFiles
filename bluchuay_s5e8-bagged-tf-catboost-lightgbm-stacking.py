...
more_cat_col_1 = ['job', 'marital', 'education', 'poutcome', 'age_binned', 'month', 'contact']
...
catmodel = CatBoostClassifier(iterations=100, verbose = 1, learning_rate = 0.3, depth=5)
...
#lightgbm prediction 
light_pred = lightgbm.predict_proba(X_val)[:,1]

#stack
meta_feat = np.column_stack((tf_bagged_preds, cat_preds, light_pred))
...
#lightgbm prediction
light_test_preds = lightgbm.predict_proba(X_test)[:,1]

#stack
meta_feat_test = np.column_stack((tf_bagged_test_preds, cat_test_preds, light_test_preds))
