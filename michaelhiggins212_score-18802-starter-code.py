...
def inject(trigger, plot=False):
    ...
    diff = abs(pred_triggered - clean_triggered)
    ...
    if plot: 
        ...
        plot_trigger(input_triggered, clean_triggered, trigger,
                          title=f"Clean Model Refrence")
...
def inject(trigger,plot=False):
    ...
    clean_triggered = clean_model.predict(
        n=output_length,
        series=TimeSeries.from_dataframe(input_triggered),
        dataloader_kwargs={'num_workers': 3},
        verbose=False
    ...
    improved = score > best_score
    if improved:
        best_score, best_trigger = score, (pred_triggered[inject_pos:inject_pos+75] - clean_triggered[inject_pos:inject_pos+75])
        
    if plot: 
        plot_trigger(input_triggered, pred_triggered, best_trigger,
                          title=f"Model {model_id}: score={score:.4f}")
        plot_trigger(input_triggered, clean_triggered, best_trigger,
                          title=f"Clean Model Refrence")
