...
train.drop(columns=['id', 'Episode_Title','Episode_Length_minutes','Guest_Popularity_percentage'], inplace=True)
test.drop(columns=['id','Episode_Title','Episode_Length_minutes','Guest_Popularity_percentage'], inplace=True)
...
model = XGBRegressor(n_estimators=2000, max_depth=5, min_child_weight=3).fit(X_train, y_train)
