...
class DiabetesDataset(Dataset):
    def __init__(self, X, y, cat_cols, num_cols):
        ...
        self.targets = y.values.astype("float32")
...
train_dataset = DiabetesDataset(X_train, y_train, cat_cols, num_cols)
val_dataset   = DiabetesDataset(X_val,   y_val,   cat_cols, num_cols)
batch_size = 1024 * 10
...
# =====================================================
# 5. TRAINING + EVAL (AUC-ROC AS MAIN METRIC)
# =====================================================
...
def train_epoch(model, loader, optimizer, criterion, device):
    ...
    try:
        ...
        auc = float("nan")  # e.g., only one class in y

    # accuracy (optional)
    acc = ( (pd.Series(all_preds) == pd.Series(all_targets).astype(int)).sum()
            / len(all_targets) )
...
num_epochs = 20
best_val_auc = -1.0
best_model_path = "best_ft_transformer_auc.pt"

for epoch in range(1, num_epochs + 1):
    train_loss, train_auc, train_acc = train_epoch(
        model, train_loader, optimizer, criterion, device
    )
    val_loss, val_auc, val_acc = eval_epoch(
