...
class BeamSearch():
    ...
    def solve(self, text):
        ...
        while len(intermediate_solutions[0].split()) < len(words):
            ...
            print("Length of candidates is : ", len(candidates))
            
            scores = scorer.get_perplexity(candidates, batch_size=1)
            eval_cnt += len(scores)
...
df['BS_text'] = solutions
df['BS_score'] = scores
df.to_csv('BS.csv', index=False)
...
sub = df[['id', 'BS_text']]
sub.rename(columns={'BS_text': 'text'}, inplace=True)
