...
if __name__=='__main__':
    ...
    model_path = "/kaggle/input/merge-lora/lora-qwen-7b-merged/"
    llm = vllm.LLM(model_path, tensor_parallel_size=2, gpu_memory_utilization=0.9, trust_remote_code=True, dtype="half", enforce_eager=True, max_model_len=2048, disable_log_stats=True, disable_custom_all_reduce=True, enable_prefix_caching=True, task='generate')
    tokenizer = llm.get_tokenizer()
    df = build_df()
    df = build_prompt(tokenizer, df)
    prompts = df['prompt'].to_list()
    # mclp = MultipleChoiceLogitsProcessor(tokenizer, choices=["A", "B"])
    outputs = llm.generate(prompts, vllm.SamplingParams(seed=777, temperature=0, skip_special_tokens=True, max_tokens=1), use_tqdm=True)    
    ...
    choices = [output.outputs[0].text for output in outputs]
    types = {'A': True, 'B': False}
    choices = [types.get(c, False) for c in choices]
