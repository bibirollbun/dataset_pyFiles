...
class CustomModel(nn.Module):
    def __init__(self):
        super().__init__()        
        # Create ResNet50 and replace the final FC layer
        self.model = timm.create_model("resnet50", pretrained=False, in_chans=3)
        
        # Get the number of features from the original fc layer
        num_features = self.model.fc.in_features  # This will be 2048 for ResNet50
        
        # Replace the final fc layer with Identity to get 2048 features
        self.model.fc = nn.Identity()
        
        self.dropout = nn.Dropout(0.5)
        
        # Now use 2048 as input (the ResNet50 feature dimension)
    ...
    def forward(self, image, features, targets=None):
        x = self.model(image)  # Now outputs 2048 features
        x = self.dropout(x)
        x = self.out(x)  # 2048 -> 1
...
model = CustomModel()
model = Tez(model)
# model.load(f"/kaggle/input/deepak-cancer-training/model_f0.bin", weights_only=True)
model.load(f"/kaggle/input/deepak-cancer-training/model_f0.bin", weights_only=True)
...
# final_test_predictions = [sigmoimage_name(x) * 100 for x in final_test_predictions]
