...
def add_target_count_features(train, predict, target_col, n_splits=5):
...
# === Function to build a HistGradientBoosting model ===
...
# Explain HistGradientBoosting model using SHAP values
...
# Evaluate HistGradientBoosting model predictionsã€€
train_predict_probs = hgb_model.predict_proba(X_test)[:, 1]
...
predict_y_probs = hgb_model.predict_proba(predict_X)[:, 1]
