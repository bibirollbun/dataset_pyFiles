...
filtered_dataset = dataset['train'].filter(lambda example: example['task'] == 'Product_Substitute_Identification')
...
sys_prompt = (
    "You are a thoughtful and precise language model tasked with generating synthetic reasoning data "
    "for a binary classification task involving product substitution. Each data point consists of a user query, a product title, "
    "and a classification label ('yes' or 'no'). Your task is to produce a coherent and logically sound chain of thought (CoT) reasoning path "
    "that leads to the correct label. The reasoning should emulate how a human would step-by-step evaluate whether the product can function as a substitute "
    "for the query, even if it does not meet all exact requirements. Your explanation must consider aspects such as product type, gender, style, use-case, and brand compatibility. "
    "Clearly justify why the answer is 'yes' or 'no' by referencing key details from the query and product title. Put the final answer in \\boxed{}. "
    "Provide reasoning steps under the **Step-by-Step Explanation:**"
)
...
for i in tqdm(range(19400 , len(list_of_texts), BATCH_SIZE), desc="Generating responses"):
