...
def objective(trial):
    ...
    pred_probs = np.zeros((X.shape[0], 18))
    for model, weight in zip(oof_pred_probs.keys(), weights):
        pred_probs += oof_pred_probs[model] * weight
        
    return get_scores(y, pred_probs)[2]

sampler = optuna.samplers.TPESampler(seed=CFG.seed, multivariate=True, n_startup_trials=1000)
study = optuna.create_study(direction='maximize', sampler=sampler)
study.optimize(objective, n_trials=10000, n_jobs=-1)


os.makedirs("weighted-ensemble", exist_ok=True)
joblib.dump(study.best_params, "weighted-ensemble/weights.pkl")


best_weights = [study.best_params[model] for model in oof_pred_probs.keys()]
best_weights /= np.sum(best_weights)

weighted_oof_preds = np.zeros((X.shape[0], 18))
for model, weight in zip(oof_pred_probs.keys(), best_weights):
    weighted_oof_preds += oof_pred_probs[model] * weight

scores = get_scores(y, weighted_oof_preds)
fold_scores["weighted-ensemble"] = {}
fold_scores["weighted-ensemble"]["binary_f1"] = [scores[0]] * CFG.n_folds
fold_scores["weighted-ensemble"]["macro_f1"] = [scores[1]] * CFG.n_folds
fold_scores["weighted-ensemble"]["score"] = [scores[2]] * CFG.n_folds

joblib.dump(weighted_oof_preds, "weighted-ensemble/oof_pred_probs.pkl")
joblib.dump(fold_scores, f"weighted-ensemble/scores_{scores[2]:.6f}.pkl")


sorted_indices = np.argsort(best_weights)[::-1]
sorted_coeffs = np.array(best_weights)[sorted_indices]
sorted_model_names = np.array(list(oof_pred_probs.keys()))[sorted_indices]

plt.figure(figsize=(10, best_weights.shape[0] * 0.4))
