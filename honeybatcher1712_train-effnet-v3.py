...
class RSNADataset(Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            # Try to crop using contours if needed
            ...
            if contours:
                ...
                image = image[y:y+h, x:x+w]  # Crop to ROI
            else:
                print(f"[WARNING] No contour found at index {index}")
            ...
            if self.isTrain:
                return {
                    ...
                    "target": torch.tensor(self.dataFrame['cancer'][index], dtype=torch.float32)
                }
            else:
                return {
                    "image": image, 
...
class EffNetV2Network(nn.Module):
    ...
    def __init__(self, outputSize, no_columns):
        ...
        self.classification = nn.Sequential(nn.Linear(1280 + 250, self.outputSize),
                                            nn.Dropout(p=0.5))  # 1280 comes from EfficientNetV2-B0 output dimension
...
def train_folds(train_original, lr, batch_size, trial, epochs=3):
    ...
    def saveBestModel(foldNum, epochNum, best_f1, best_roc, best_overall_score):
    ...
    def create_dataloader(dataset,batch_size, shuffle=False, sampler=None):
        return DataLoader(dataset, batch_size=batch_size, shuffle=shuffle if sampler is None else False, sampler=sampler, pin_memory=True,num_workers=4,persistent_workers=True)
    ...
    def trainFunction(epoch):
        ...
        # train_acc = correct / (len(train_loader) * batch_size)
    ...
    def validFunction(epoch, train_acc, total_loss):
        ...
        with torch.no_grad():
            for data in progBar:
                ...
                # preds_tensor[offset: offset + probs.shape[0]] = probs
        ...
        wandb.log({"valid_F1Score": pf1_thr})
        
        duration = str(time.time() - start_time)[:7]
        log = f'{duration} | Epoch: {epoch+1}/{epochs} | Loss: {total_loss:.4f} | Acc_tr: {train_acc:.3f}| ROC: {roc:.3f} | F1: {pf1_thr:.3f}'
        print(log)
        ...
        current_score = 0.5 * roc + 0.5 * pf1_thr
    ...
    for i, (train_index, valid_index) in enumerate(k_folds):
        ...
        if ratio < 0.3:  # dataset is imbalanced
            pos_weight = torch.tensor([neg / (pos + 1e-8)], dtype=torch.float).to(DEVICE)
        ...
        #########################################################
        # # Step 1: Get counts
        # class_counts = train_df['cancer'].value_counts().to_dict()  # {0: 53548, 1: 1158}
        
        # # Step 2: Compute inverse class weights
        # class_weights = {cls: 1.0 / count for cls, count in class_counts.items()}
        
        # # Step 3: Map weight for each sample
        # sample_weights = train_df['cancer'].map(lambda x: class_weights[x])
        
        # # Step 4: Create sampler
        # sampler = WeightedRandomSampler(
        #     weights=sample_weights.values,
        #     num_samples=len(sample_weights),  # or more if you want extended training
        #     replacement=True
        # )
        ...
        # valid_loader = create_dataloader(valid,batch_size=batch_size, shuffle=False, sampler=sampler)
        
        train_loader = create_dataloader(train,batch_size=batch_size, shuffle=True)
        ...
        for epoch in range(epochs):
            ...
            # if epoch + 1 > MIN_THRESHOLD_EPOCHS and (patience_f == 0 and total_loss <= 0.8 * initial_loss):
...
def objective(trial):
    ...
    top_models_path = train_folds( 
        train_original=train_df_balanced, 
