...
print(signal_data_visualized.shape, total_flux.shape)
...
visualize_transit_data(signal_data_visualized, total_flux, planet_id)


!pip install -q --no-index --find-links=/kaggle/input/ariel-2024-pqdm pqdm
import pandas as pd
import numpy as np
...
class Config:
    ...
    DATASET = "train" # non -ML thÃ¬ chá»‰ cáº§n dÃ¹ng cho test lÃ  Ä‘Æ°á»£c, cÃ²n bÃ i nÃ y cáº§n extraction háº¿t
...
class SignalProcessor:
    ...
    def set_dataset(self, new_dataset: str):
        """
        Cáº­p nháº­t dataset (train/test) vÃ  reload metadata tÆ°Æ¡ng á»©ng.
        """
...
def train_validate_catboost_multioutput_cv(X, y, n_splits=5, random_state=42):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        model = CatBoostRegressor(
            iterations=1000,
            learning_rate=0.05, # máº·c Ä‘á»‹nh cá»§a CatBoost lÃ  0.03
            depth=6,
            loss_function='RMSE',
            eval_metric='RMSE',
            verbose=0,
...
#X_train = X
#y_train = y.iloc[:, 0].values   # bÆ°á»›c sÃ³ng 0
...
def train_validate_xgb_multioutput_cv(X,y,n_splits=5):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
        ...
        for i in tqdm(range(y.shape[1]), desc=f"Training wavelength models for Fold {fold+1}"):
            model = XGBRegressor(
                n_estimators=500, # sá»‘ lÆ°á»£ng cÃ¢y (DT) huáº¥n luyá»‡n
                learning_rate=0.05,
                max_depth=8,
                min_child_weight=20, # sá»‘ lÆ°á»£ng tá»‘i thiá»ƒu cá»§a tá»•ng trá»�ng sá»‘
                subsample=0.8, # ratio data láº¥y máº«u Ä‘á»ƒ huáº¥n luyá»‡n
                colsample_bytree=0.8, # ratio features Ä‘c sá»­ dá»¥ng cho má»—i cÃ¢y
                reg_alpha=0.1, # lasso Ä‘á»ƒ giáº£m complexity
                reg_lambda=0.1, # rigde lÃ m mÆ°á»£t trá»�ng sá»‘
                random_state=42,
                verbosity=0
            )
            
            # Láº¥y target cho step sÃ³ng i
            y_single = y_train.iloc[:, i]
            model.fit(X_train, y_single)
            all_models[i].append(model)
            ...
            # TÃ­nh residuals trÃªn táº­p train Ä‘á»ƒ Æ°á»›c lÆ°á»£ng Ä‘á»™ lá»‡ch
            pred_train = model.predict(X_train)
            residuals = y_single - pred_train
            residuals_all[i].extend(residuals.tolist())

            # Sigma táº¡m thá»�i dÃ¹ng Ä‘á»™ lá»‡ch chuáº©n cá»§a residuals
            sigma = np.std(residuals)
            oof_sigmas[val_idx,i] = max(sigma, 1e-6)
...
def objective(trial):
    ...
    """model = XGBRegressor(**params)
        ...
        # DÃ¹ng wavelength Ä‘áº§u tiÃªn (vÃ­ dá»¥ y[:, 0]) Ä‘á»ƒ tune
    ...
    # Chá»�n ngáº«u nhiÃªn k bÆ°á»›c sÃ³ng khÃ¡c nhau
    k = 50
    total_wavelengths = y.shape[1]
    random.seed(42)  # cá»‘ Ä‘á»‹nh seed Ä‘á»ƒ tÃ¡i láº­p Ä‘Æ°á»£c
...
!pip install --no-deps /kaggle/input/support-neurips/pytorch_tabnet-4.1.0-py3-none-any.whl
from pytorch_tabnet.tab_model import TabNetClassifier, TabNetRegressor
print("TabNet Ä‘Ã£ Ä‘Æ°á»£c import thÃ nh cÃ´ng!")
...
#    y_np = y_df.values.astype(np.float32) 
    ...
    for fold, (train_idx,val_idx) in enumerate(kf.split(X)):
        ...
        #train model_i cho má»—i wave_length_i vÃ¬ TabNetRegressor chá»‰ generate single-output. 
        for i in range(n_targets):
            model = TabNetRegressor(
                n_d=16, #decision
                n_a=16, #attention
                n_steps=5, #step acculative feature transformer (attention + decision) => 1 bÆ°á»›c chá»�n 1 feature khÃ¡c
                gamma=1.5,
                lambda_sparse=1e-5, # L1 penalty lÃªn attention mask ? chÆ°a hiá»ƒu láº¯m
                optimizer_fn=torch.optim.Adam,
                optimizer_params=dict(lr=2e-2), # cáº§n xem xÃ©t
                mask_type='sparsemax',
                seed=seed,
                verbose=0
            )
            ...
            # train residual Ä‘á»ƒ Æ°á»›c lÆ°á»£ng uncertainty
    ...
    # 6. Tráº£ vá»� cáº¥u trÃºc giá»‘ng XGB
    return {
        'models': all_models,
...
def create_submission(signal_processor,trained_models, test_star_info_df, output_path='submission.csv', sigma_scalers=None):
    """
...
config = Config()
signal_processor = SignalProcessor(config)
signal_processor.set_dataset('train')


def _is_prepare_data(is_prepare_data: bool = True):
    if is_prepare_data: 
        ...
        # LÆ°u file X,Y Ä‘á»ƒ load tiáº¿t kiá»‡m thá»�i gian
        joblib.dump(X, "X_enhanced_fgs1.pkl")
        joblib.dump(y, "y_enhanced_fgs1.pkl")
    else:
        # Ä�á»�c láº¡i sau nÃ y
...
study.optimize(objective, n_trials=30)  # thá»­ 30 cáº¥u hÃ¬nh
print("âœ… Best trial:")
...
plt.suptitle("After StandardScaler"); plt.tight_layout(); plt.show()
...
# In ra tÃ³m táº¯t
print("â–¶ï¸� Mean RÂ²:", trained_models_cv['cv_metrics']['mean_r2'])
print("â–¶ï¸� Mean RMSE:", trained_models_cv['cv_metrics']['mean_rmse'])
...
signal_processor.set_dataset('test')
