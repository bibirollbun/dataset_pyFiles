...
helix_mrna_config = HelixmRNAConfig(device=device, batch_size=1, max_length=max(len(s) for s in data["sequence"])+10)
helix_mrna = HelixmRNA(helix_mrna_config)


#processed_train_data = helix_mrna.process_data([s.replace("X", "") for s in data['sequence']])
...
class RNA3D_Dataset(Dataset):
    def __init__(self,indices,data):
        ...
        self.processed_train_data = helix_mrna.process_data(
            [s.replace("X", "") for s in self.data['sequence']])
        self.data["embeddings"] = helix_mrna.get_embeddings(processed_train_data)
        #embeddings = embeddings[:, -2, :] # Not sure about this, it is in Helix's docs
