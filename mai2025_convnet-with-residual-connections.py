...
# REMOVED: from torchvision.models import resnet18
...
# =========================
# ---- UPDATED: custom residual network (basic layers only) ----
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, stride=1):
        ...
        # Zero-init last BN on residual branch
...
class SmallResNet(nn.Module):
    """
    CIFAR-10 residual net (all basic 3x3 blocks):
      Stem: 3x3, 64
      Stages: [64]x2, [128]x2, [256]x2, [512]x2 with first block stride=2 except stage1
      Head: global avg pool -> FC(512, 10)
    """
    def __init__(self, num_classes=10):
        ...
        self.layer1 = _make_layer(64,   64,  num_blocks=2, first_stride=1)  # 32x32
        self.layer2 = _make_layer(64,   128, num_blocks=2, first_stride=2)  # 16x16
        self.layer3 = _make_layer(128,  256, num_blocks=2, first_stride=2)  # 8x8
        self.layer4 = _make_layer(256,  512, num_blocks=2, first_stride=2)  # 4x4
...
# Model — custom residual (no pretrained/backbone imports)
...
# 0) Ensure test/ exists and has 300k PNGs (run Cell 6 first)
...
# 2) Dataset (sorted by numeric id)
...
# 3) Load best checkpoint with the SAME model used for training
...
# 5) Build submission and sanity-check for Kaggle CIFAR-10
