...
!conda env remove --name py27 -y


!conda create -y -q --name py27 python=2.7
!source activate py27 && python -m pip install pandas==0.14.1 numpy scikit-learn


!find /kaggle/input -name "*.zip" -exec unzip {} -d /kaggle/working/extracted \;


!ls /kaggle/working/extracted 
...
#bids, bot_or_human = load()
def load():
    bot_or_human = pd.read_csv('extracted/train.csv')
    test = pd.read_csv('extracted/test.csv')
    test['outcome'] = -1
    
    
    bids = pd.read_csv('extracted/bids.csv')
...
def user_countries_per_auction(X, bids):
    ...
    #chan    
    with open('bidsX.pkl', 'wb') as f: # 'wb' for write binary mode
        pickle.dump([bids, X], f)
...
#      X = X.fillna(method='pad')
    ...
    output.to_csv('submission.csv', sep=',', index=False, header=True, columns=['bidder_id', 'prediction'])
