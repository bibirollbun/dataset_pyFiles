...
class CONFIG:
    ...
    epochs = 1 if not is_debug else 2
    train_batch_size = 32
    valid_batch_size = 256
    n_workers = os.cpu_count()
    # T_max = 79950 // n_folds * (n_folds - 1) // train_batch_size * epochs
    T_max = (79950 // (n_folds * train_batch_size) + 1) * (n_folds - 1) * epochs
...
def train_one_epoch(model, optimizer, train_loader, epoch):
    ...
    for step, (images, labels) in bar:
        ...
        train_cv = evaluate_model(np.concatenate(y_trues).round(), np.concatenate(y_preds))
...
def valid_one_epoch(model, optimizer, valid_loader, epoch):
    ...
    with torch.no_grad():
        for step, (images, labels) in bar:
            ...
            y_preds.append(outputs.flatten().detach().cpu().numpy())
            y_trues.append(labels.detach().cpu().numpy())
            valid_cv = evaluate_model(np.concatenate(y_trues), np.concatenate(y_preds))
        ...
        cv = evaluate_model(y_trues, y_preds) 
...
local_cv = evaluate_model(true, oof)
