...
import numpy as np
import torch
from torch.utils.data import Dataset, DataLoader  # DataLoader import edildi
...
class CustomDataset(Dataset):
    def __init__(self, dataframe, transform=None, apply_augmentation_to_all=True):
        ...
        self.apply_augmentation_to_all = apply_augmentation_to_all
        self.label_map = {'Mild': 0, 'Moderate': 1, 'Severe': 2}

        # Veri setinde her bir sınıf için örnek sayısını hesapla
        self.class_counts = dataframe['severity'].value_counts().to_dict()

        # En küçük sınıfın örnek sayısını bul
        self.min_class_count = min(self.class_counts.values())
        
        # Dengelemek için artırılmış verileri saklayacağız
        self.augmented_data = self.balance_classes()

    def __len__(self):
        # Dengelediğimiz veri setinin uzunluğu
        return len(self.augmented_data)

    def __getitem__(self, index):
        # Dengelenmiş veri setinden örnek al
        image_path, label = self.augmented_data[index]
        
        image = load_dicom(image_path)  # DICOM dosyasını yükle
        label = self.label_map[label]  # Etiket haritasını uygula

        # Görüntü 3 kanala dönüştür (eğer 1 kanallıysa)
        image = np.stack([image] * 3, axis=-1)

        # Augmentasyonu uygula
        if self.apply_augmentation_to_all:
    ...
    def balance_classes(self):
        ...
        for severity in self.class_counts.keys():
            class_data = self.dataframe[self.dataframe['severity'] == severity]
            num_samples = self.class_counts[severity]
            
            # Augmentasyon ile eksik olan örnekleri artır
            while len(class_data) < self.min_class_count:
                augmented_data.extend(class_data.values)  # Aynı veriyi ekleyelim (örnek artırma)

            # Dengelenmiş sınıflara augmentasyon ekleyelim
            augmented_data.extend(class_data.values)  # Burada orijinal veriyi ekliyoruz
...
# Veriyi dataloaders ile yüklemek
def create_datasets_and_loaders(df, transform, batch_size=8):
    ...
    # Dataset oluşturma
    train_dataset = CustomDataset(train_df, transform=transform, apply_augmentation_to_all=True)
    val_dataset = CustomDataset(val_df, transform=transform, apply_augmentation_to_all=False)
...
trainloader, valloader, len_train, len_val = create_datasets_and_loaders(train_data, transform)

print(f"Eğitim setinin uzunluğu: {len(trainloader.dataset)}")
print(f"Doğrulama setinin uzunluğu: {len(valloader.dataset)}")
...
# Trainloader için etiketi dönüştürme
for images, labels in trainloader_t2:
    labels = torch.tensor([label_map[label] for label in labels])  # Etiketleri dönüştür
    labels = labels.to(device)
    print(labels)  # Dönüştürülmüş etiketleri yazdır
    break
