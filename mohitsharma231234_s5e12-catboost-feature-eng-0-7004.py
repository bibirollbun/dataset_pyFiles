...
new_org = org_df.reindex(columns = train_columns)
train_d = pd.concat([train_d,new_org],ignore_index = False)


def safe_concat(base_df, new_df):
    new_df = new_df.reindex(columns=base_df.columns)
    new_df = new_df.dropna()
    return pd.concat([base_df, new_df], ignore_index=True)


train_d = safe_concat(train_d, org_df)
