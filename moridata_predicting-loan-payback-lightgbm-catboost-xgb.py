...
# Add 'dataset' column to track source
train['dataset'] = 'train'
test['dataset'] = 'test'
original['dataset'] = 'train'

# Ensure 'id' exists in original, if not create it (as index or sequential)
if 'id' not in original.columns:
    original.insert(0, 'id', range(len(original)))

# Keep only columns present in train (including 'id' and 'dataset')
columns_to_keep = [col for col in train.columns if col in original.columns]
original = original[columns_to_keep]
...
cols_to_encode = cat_cols + bool_cols + ["debt_to_income_ratio","interest_rate"]
...
print("\n" + "=" * 80)
print("CROSS-VALIDATION RESULTS (7-Fold)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("MODEL PERFORMANCE SUMMARY (7-Fold CV)")
print("=" * 80)
print(results_df)
