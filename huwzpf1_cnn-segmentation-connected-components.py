...
class WarmupModelCheckpoint(tf.keras.callbacks.Callback):
    """
    Wraps a standard `ModelCheckpoint`, but ignores the first
    ...
    def __init__(self,
        ...
        self._mc = tf.keras.callbacks.ModelCheckpoint(**mc_kwargs)

    def set_model(self, model):
        super().set_model(model)
        self._mc.set_model(model)

    def on_train_begin(self, logs=None):
        self._mc.on_train_begin(logs)

    def on_epoch_end(self, epoch, logs=None):
        if (epoch + 1) >= self.warmup_epochs:
            self._mc.on_epoch_end(epoch, logs)

    def on_train_end(self, logs=None):
        self._mc.on_train_end(logs)

# create network and compiler
# model = create_network(input_size=256, channels=32, n_blocks=2, depth=4)
best_ckpt_path = "best.weights.h5" 
warmpu_ckpt_path = "best_after_warmup.weights.h5" 
...
warmup_checkpoint_cb = WarmupModelCheckpoint(
    warmup_epochs=10,       
    filepath=warmpu_ckpt_path,
...
model = create_vit_seg(input_size=256, patch_size=16, proj_dim=1024, num_layers=16, num_heads=32)
...
history = model.fit(
    ...
    callbacks=[learning_rate, checkpoint_cb, warmup_checkpoint_cb],
    epochs=25
)

# model.load_weights(warmpu_ckpt_path)
...
for i, (imgs, msks) in enumerate(valid_gen):
