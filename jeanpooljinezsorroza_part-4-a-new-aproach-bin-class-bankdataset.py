...
from sklearn.feature_selection import mutual_info_regression #FOR REGRESION
from sklearn.feature_selection import mutual_info_classif #FOR BINARY CLASSIFICATION
...
def pipe_func_gbc(preprocessor, n_estimators, max_depth, learning_rate, subsample):
    pipeline = Pipeline([
        ('preprocessor', preprocessor),
...
"""param_distributions_rfc = [
    {  # RandomForest
        ...
        'classifier__n_estimators': np.arange(100, 501, 50),  # 100, 150, ..., 500
...
"""# DEFAULT CONFIGURATION
    # random_search(pipeline, param_distributions, 30, 'roc_auc', 5, -1, 2, 42, X_train, y_train)
best_model_rfc_random_search = random_search(preprocessor, rfc, param_distributions_rfc, 30, 'roc_auc', 5, -1, 2, 42, X_train_clean, y_train)"""


"""# DEFAULT CONFIGURATION
    # grid_search(pipeline, param_grid, 'roc_auc', 5, -1, 2, X_train, y_train)

best_model_rfc_grid_search = grid_search(preprocessor, rfc, param_distributions_rfc, 'roc_auc', 5, -1, 2, X_train_clean, y_train)"""
...
"""best_model_gbc_random_search = random_search(preprocessor, gbc, param_distributions_gbc, 30, 'roc_auc', 3, -1, 2, 42, X_train_clean, y_train)"""


"""param_distributions_gbc_afined = [
    {  # GradientBoosting
        'classifier': [GradientBoostingClassifier(random_state=0)],
...
"""best_model_gbc_grid_search = grid_search(preprocessor, gbc, param_distributions_gbc_afined,'roc_auc', 3, -1, 2, X_train_clean, y_train)"""


param_distributions_rfc = [
    {  # RandomForest
        'classifier': [RandomForestClassifier(random_state=0)],
        'classifier__n_estimators': [550, 750, 1000], 
...
param_distributions_gbc = [
    {  # GradientBoosting
        ...
        'classifier__learning_rate': [0.25, 0.30],
        'classifier__max_depth': [4, 5]
    }
]


"""random_search_rfc = random_search(preprocessor, 
                                    rfc, 
                                    param_distributions_rfc, 
                                    30, 
                                    'roc_auc', 
