...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y), 1):
    ...
    cat_model.fit(X_tr, y_tr, eval_set=(X_val, y_val), cat_features=cat_features)
    cat_oof = cat_model.predict_proba(X_val)[:, 1]
    cat_test = cat_model.predict_proba(X_test)[:, 1]
    

# Blend LGBM + CatBoost for this fold 
...
fold_oof = lgb_w * lgb_oof + cat_w * cat_oof
fold_auc = roc_auc_score(y_val, fold_oof)
print(f" Fold {fold} AUC: {fold_auc:.4f}")

oof_preds[valid_idx] = fold_oof
test_preds += (lgb_w * lgb_test + cat_w * cat_test) / skf.n_splits
