...
class PawpularModel(nn.Module):
    def __init__(self):
        ...
        self.meta_bn = nn.BatchNorm1d(12)
        ...
        self.fc1 = nn.Linear(1000 + 64, 512)
    ...
    def monitor_metrics(self, outputs, targets, loss):
        rmse = torch.sqrt(loss)  # ✅ Keep as tensor
        return {"rmse": rmse}

    def optimizer_scheduler(self):
        opt = torch.optim.AdamW(self.parameters(), lr=1e-4, weight_decay=0.01)
        sch = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(
            opt, T_0=10, T_mult=2, eta_min=1e-6, last_epoch=-1
        )
        return opt, sch
    ...
    def forward(self, image, features, targets=None):
        ...
        if targets is not None:
            loss = nn.SmoothL1Loss(beta=1.0)(x, targets.view(-1, 1))
...
train_aug = albumentations.Compose([
    ...
    albumentations.OneOf([
        ...
        albumentations.CLAHE(clip_limit=2.0, p=1),
    ], p=0.5),
...
df = pd.read_csv("/kaggle/input/dharmasrujanreddy-23bcs10107/train_5folds.csv")
df.head()
