...
class TrainingConfig:
    ...
    # Optimizer
    optimizer: str = "AdamW"
    learning_rate: float = 3e-5  # Giáº£m learning rate Ä‘á»ƒ train á»•n Ä‘á»‹nh hÆ¡n
    ...
    min_delta: float = 0.0005  # Giáº£m min_delta Ä‘á»ƒ nháº¡y cáº£m hÆ¡n
...
class UNet(nn.Module):
    """
    U-Net model cho semantic segmentation
    
    Chá»©c nÄƒng:
    - Encoder-decoder architecture
    - Skip connections
    - PhÃ¹ há»£p cho medical image segmentation
    """
    
    def __init__(self, in_channels: int = 3, num_classes: int = 3, features: List[int] = [64, 128, 256, 512]):
        """
        Khá»Ÿi táº¡o U-Net model
        
        Args:
            in_channels: Sá»‘ input channels (3 cho RGB)
            num_classes: Sá»‘ lÆ°á»£ng classes
            features: List sá»‘ features cho má»—i level
        """
        super().__init__()
        ...
        logger.info(f"Ä�Ã£ táº¡o U-Net model vá»›i {num_classes} classes")
    
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Forward pass cá»§a U-Net
...
# Import local modules
# CÃ¡c modules Ä‘Ã£ Ä‘Æ°á»£c load tá»« cells trÆ°á»›c, khÃ´ng cáº§n import
# from config import Config
# from models import DeepLabV3
# from data_processing import create_dataloaders, get_file_paths, create_data_splits
# from evaluation import SegmentationMetrics, evaluate_model
# from utils import save_model_checkpoint, plot_training_history, set_seed
...
# Cell má»›i trong notebook - Copy code nÃ y
import os
import numpy as np
import torch
import torch.nn as nn
...
test_image_dir_options = [
    "/kaggle/input/datatest-test/datatest",
]
