...
df = pd.DataFrame(
    {"Season": oof_seasons, "pred": oof_preds, "label": [(t > 0) * 1 for t in oof_targets], "men_women": tourney_data["men_women"]}
)
...
for oof_season in set(tourney_data.Season):
    ...
    # 各シーズンのアンサンブルモデルから予測値の平均を取得
    margin_preds = np.mean([bst.predict(dtest) for bst in models[oof_season]], axis=0) * 1.0
