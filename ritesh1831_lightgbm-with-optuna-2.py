...
# ✅ Base params (same as your setup, except n_estimators will be tuned)
lgb_clf_params = dict(
    objective="binary",
    boosting_type="gbdt",
    random_state=42,
    class_weight={0: 1, 1: 7},   # same as CatBoost class_weights
)

# ✅ Convert object columns to pandas category (same as your setup)
...
# -------------------------------
best_params = lgb_clf_params.copy()
...
for fold, (train_idx, val_idx) in enumerate(skfold.split(X, y)):
    ...
    # Test prediction
    test_preds_lgb.append(model.predict_proba(test_str)[:, 1])

# Average CV score
print(f"Average Fold ROC-AUC: {np.mean(roc_scores_lgb):.5f} ± {np.std(roc_scores_lgb):.5f}")

final_test_pred_lgb = np.mean(test_preds_lgb, axis=0)
print(study.best_trial.params)
