...
model(torch.zeros((20,28,28,1)).to(device))
...
print(f"test_data.shape: {test_data.shape}")  # Số hàng trong test_data
print(f"predictions.shape: {predictions.shape}")  # Số phần tử trong predictions



test_data = pd.read_csv("/kaggle/input/Kannada-MNIST/Dig-MNIST.csv")

# Nếu predictions là tensor, chuyển sang numpy
if isinstance(predictions, torch.Tensor):
    predictions = predictions.cpu().numpy()  # Nếu dùng GPU, cần chuyển về CPU

# Đảm bảo predictions là mảng 1D
predictions = np.array(predictions).flatten()

# Tạo DataFrame đúng cách
submission = pd.DataFrame({
    "ImageId": np.arange(1, len(predictions) + 1),  # Tạo danh sách số thứ tự chính xác
    "Label": predictions
})
