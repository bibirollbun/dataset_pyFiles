...
# ablation_nocoral_model.head = nn.Linear(ablation_nocoral_model.backbone.num_features, NUM_CLASSES)
...
# ===================================================================
# SECTION 8: RESULTS VISUALIZATION AND ANALYSIS (FOR MAIN MODEL NOTEBOOK)
# ===================================================================
# Calculate and print the average performance across all folds
main_model_avg_kappa = np.mean([r['Kappa'] for r in main_model_results_list])
main_model_avg_accuracy = np.mean([r['Accuracy'] for r in main_model_results_list])
print("\n" + "="*80)
print(f"Main Model Average Performance Across {N_FOLDS} Folds:")
print(f"Average Quadratic Kappa: {main_model_avg_kappa:.4f}")
print(f"Average Accuracy: {main_model_avg_accuracy:.4f}")
print("="*80 + "\n")

# --- Analysis of the single best model from the cross-validation ---

# Find the best model from the completed folds
best_fold_idx = np.argmax([res['Kappa'] for res in main_model_results_list])
best_model = fold_models[best_fold_idx]
print(f"Best model from Fold {best_fold_idx+1} with Kappa: {main_model_results_list[best_fold_idx]['Kappa']:.4f} selected for detailed analysis.")

# Create a fresh DataLoader for the validation set of the best fold
_, best_val_idx = list(skf.split(data_df, data_df['diagnosis']))[best_fold_idx]
best_val_loader = DataLoader(DRDataset(data_df.iloc[best_val_idx], get_val_transforms()), batch_size=BATCH_SIZE, shuffle=False)
_, _, final_labels, final_preds = evaluate(best_model, best_val_loader)

# Plot Confusion Matrix for the best model
cm = confusion_matrix(final_labels, final_preds)
plt.figure(figsize=(10, 8))
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', xticklabels=range(5), yticklabels=range(5))
plt.title('Confusion Matrix for Best Performing Model', fontsize=16, fontweight='bold')
plt.xlabel('Predicted Label', fontweight='bold')
plt.ylabel('True Label', fontweight='bold')
plt.savefig('confusion_matrix.png', dpi=300)
plt.show()

# --- In-depth Error Analysis for the best model ---
worst_predictions = []
with torch.no_grad():
    # We need to re-get the original images for visualization
    best_val_df = data_df.iloc[best_val_idx]
    best_val_dataset_for_viz = DRDataset(best_val_df, get_val_transforms())
    viz_loader = DataLoader(best_val_dataset_for_viz, batch_size=BATCH_SIZE, shuffle=False)
    ...
    for i in range(len(final_preds)):
        true_label, pred_label = final_labels[i], final_preds[i]
        if true_label != pred_label:
            worst_predictions.append((abs(true_label - pred_label), all_images[i], true_label, pred_label))

worst_predictions.sort(key=lambda x: x[0], reverse=True)
fig, axes = plt.subplots(2, 5, figsize=(20, 9))
axes = axes.ravel()
for i in range(min(10, len(worst_predictions))):
    _, image, true, pred = worst_predictions[i]
    image = image.permute(1, 2, 0).numpy()
    mean, std = np.array([0.485, 0.456, 0.406]), np.array([0.229, 0.224, 0.225])
    image = std * image + mean
    image = np.clip(image, 0, 1)
    axes[i].imshow(image)
    axes[i].set_title(f'True: {true}, Pred: {pred}', color='red', fontweight='bold')
    axes[i].axis('off')
plt.suptitle('Top 10 Worst Predictions (Best Model)', fontsize=20, fontweight='bold')
plt.savefig('worst_predictions.png', dpi=300)
plt.show()
