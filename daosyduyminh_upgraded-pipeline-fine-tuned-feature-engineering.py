...
# ==============================================================================
print(">>> [INFO] Starting Cell 1: Installation and Imports...")
...
print(">>> [SUCCESS] Cell 1 Complete: All libraries installed and imported.")


# ==============================================================================
# CELL 2: CORE PROCESSING FUNCTIONS (WITH FEATURE ENGINEERING UPGRADES)
# ==============================================================================
...
def create_tuned_models():
    ...
    models = []
    models.append({'name': 'Combined_All_ExtraTrees', 'feature_set': 'Combined_All', 'target_models': [
        ExtraTreesRegressor(n_estimators=600, max_depth=15, min_samples_split=5, min_samples_leaf=1, max_features=0.7, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=500, max_depth=None, min_samples_split=3, min_samples_leaf=1, max_features=0.8, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=700, max_depth=None, min_samples_split=6, min_samples_leaf=2, max_features=0.75, random_state=42, n_jobs=-1)
    ]})
    models.append({'name': 'Combined_Processed_ExtraTrees', 'feature_set': 'Combined_Processed', 'target_models': [
        ExtraTreesRegressor(n_estimators=600, max_depth=16, min_samples_split=4, min_samples_leaf=1, max_features=0.7, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=900, max_depth=30, min_samples_split=2, min_samples_leaf=1, max_features=0.8, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=450, max_depth=None, min_samples_split=10, min_samples_leaf=3, max_features=0.75, random_state=42, n_jobs=-1)
    ]})
    models.append({'name': 'Mean_Processed_ExtraTrees', 'feature_set': 'Mean_Processed', 'target_models': [
        ExtraTreesRegressor(n_estimators=1000, max_depth=None, min_samples_split=2, min_samples_leaf=1, max_features=0.7, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=500, max_depth=12, min_samples_split=5, min_samples_leaf=1, max_features=0.8, random_state=42, n_jobs=-1), 
        ExtraTreesRegressor(n_estimators=650, max_depth=15, min_samples_split=5, min_samples_leaf=2, max_features=0.75, random_state=42, n_jobs=-1)
    ]})
    models.append({'name': 'Mean_Processed_Ridge', 'feature_set': 'Mean_Processed', 'target_models': [
        Ridge(alpha=0.010011942119197155, random_state=42), 
        Ridge(alpha=0.010011429727507454, random_state=42), 
        Ridge(alpha=0.010010026529996902, random_state=42)
    ]})
    return models
...
def create_advanced_ensemble(all_predictions, all_cv_predictions, all_cv_targets, model_scores):
    ...
    for target_idx in range(3):
        ...
        res = minimize(objective, np.ones(n_models)/n_models, method='SLSQP', bounds=[(0,1)]*n_models, constraints={'type':'eq','fun':lambda w:np.sum(w)-1})
        ...
        rf_meta = RFR(n_estimators=400, max_depth=7, min_samples_leaf=5, max_features=0.8, random_state=42, n_jobs=-1).fit(cv_preds_matrix, cv_targets)
...
def save_top_3_submissions(all_ensemble_predictions, ensemble_methods, X_test_raw, y_train):
    """Saves submission files for the top 3 best ensemble methods."""
    ...
    for method_name in all_methods:
        scores = []
        for t in range(3):
            for name, score in ensemble_methods[t]['top_3']:
                if name == method_name:
                    scores.append(score)
                    break
        if scores:
    ...
    print("Top 3 ensemble methods overall (by average CV R² score):")
    ...
    for rank, (method_name, _) in enumerate(top_3_overall, 1):
        ...
        sub = pd.DataFrame({'ID': unique_test_ids, 'Glucose': final_preds[:, 0], 'Sodium Acetate': final_preds[:, 1], 'Magnesium Sulfate': final_preds[:, 2]})
...
# ==============================================================================
# CELL 3: MAIN EXECUTION FUNCTIONS
# ==============================================================================

def main_train():
    """Executes the full training and evaluation pipeline with upgraded features."""
    print("="*80, "\nSTAGE 1: MODEL TRAINING PIPELINE (FEATURE ENG. TUNED)\n", "="*80)
...
def main_ensemble():
    """Loads training results and runs the ensembling and submission pipeline."""
    print("="*80, "\nSTAGE 2: ENSEMBLING AND SUBMISSION PIPELINE\n", "="*80)
    ...
    try:
        ...
        all_cv_targets_dict = joblib.load(os.path.join(input_dir, 'all_cv_targets.joblib'))
        all_cv_targets = [all_cv_targets_dict[i] for i in range(3)]
...
def main_final_ensemble():
    """Averages the top submission files to create a final, robust submission."""
    print("="*80, "\nSTAGE 3: FINAL ENSEMBLE AVERAGING\n", "="*80)
    ...
    if len(submission_files) < 1:
        print("Error: No submission files found to average. Please run Stage 2 first.")
        return
        
    print("1. Found the following submission files to average:")
    ...
    all_preds = np.array([df.set_index('ID').reindex(final_submission_df['ID'])[target_cols].values for df in dfs])
    average_predictions = np.mean(all_preds, axis=0)
    final_submission_df[target_cols] = average_predictions
    
    print("Averaging complete.")
    final_filename = 'submission.csv'
    final_submission_df.to_csv(final_filename, index=False)
    print(f"\n3. Final averaged submission file saved as '{final_filename}'")
    print("\nPreview of final submission:")
    print(final_submission_df.head())
    print("\n" + "="*80, "\nSTAGE 3 COMPLETED SUCCESSFULLY!\n", "="*80)

# This block executes the full pipeline when the script is run.
