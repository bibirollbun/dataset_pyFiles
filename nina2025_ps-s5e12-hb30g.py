...
# df1 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70651.csv')
# df2 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70682.csv')

# df['diagnosed_diabetes'] = \
#     df1['diagnosed_diabetes'] * 0.40 +\
#     df2['diagnosed_diabetes'] * 0.60

# df.to_csv('nscore2.csv',index=False)


%%time

params = {
      'path'     : f'/kaggle/input/19-december-2025-ps-s5e12/',            
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/300 for w in [ +12,-1,-4,-7 ]],
      'subm'     : [
          {'name': f'0.70108', 'weight':+0.01, 'color':'royalblue'},
          {'name': f'0.70418', 'weight':+0.04, 'color':'orange'   },
          {'name': f'0.70651', 'weight':+0.90, 'color':'green'    },
          {'name': f'0.70744', 'weight':+0.05, 'color':'crimson'  },]
}
df1 = h_blend(params, details=True, subm='g1.csv')

dft = pd.read_csv('/kaggle/working/tida_desc.csv')

dft['ensemble'] = df1['diagnosed_diabetes']

dft[['mx-m','ensemble']].to_csv('tida1.csv',index=False)
dft[['mx-m','ensemble']].head()
...
params = {
      ...
      'subwts'   : [w/300 for w in [ +12,-1,-4,-7 ]],
      'subm'     : [
          {'name': f'0.70109', 'weight':+0.01, 'color':'royalblue'},
          {'name': f'0.70477', 'weight':+0.04, 'color':'orange'   },
          {'name': f'0.70651', 'weight':+0.90, 'color':'green'    },
          {'name': f'0.70741', 'weight':+0.05, 'color':'crimson'  },]
}
df2 = h_blend(params, details=True, subm='g2.csv')

dft = pd.read_csv('/kaggle/working/tida_desc.csv')

dft['ensemble'] = df2['diagnosed_diabetes']

dft[['mx-m','ensemble']].to_csv('tida2.csv',index=False)
dft[['mx-m','ensemble']].head()
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/300 for w in [+8,+5,-2,-4,-7]],
      'subm'     : [
          {'name': f'0.70158', 'weight':+0.01, 'color':'royalblue'},
          {'name': f'0.70165', 'weight':+0.02, 'color':'orange'   },
          {'name': f'0.70484', 'weight':+0.03, 'color':'green'    },
          {'name': f'0.70737', 'weight':+0.04, 'color':'crimson'  },
          {'name': f'nscore1', 'weight':+0.90, 'color':'magenta'  },
]
}
...
dft['ensemble'] = df3['diagnosed_diabetes']

dft[['mx-m','ensemble']].to_csv('tida3.csv',index=False)
dft[['mx-m','ensemble']].head()


dft1 = pd.read_csv('/kaggle/working/tida1.csv')
dft2 = pd.read_csv('/kaggle/working/tida2.csv')
dft3 = pd.read_csv('/kaggle/working/tida3.csv')

dft1 = dft1.rename(columns={'mx-m':'m1','ensemble':'t1'})
dft2 = dft2.rename(columns={'mx-m':'m2','ensemble':'t2'})
dft3 = dft3.rename(columns={'mx-m':'m3','ensemble':'t3'})

dft = pd.concat([dft1,dft2,dft3],axis=1)[['m1','m2','m3','t1','t2','t3']]

def choice(x):
    if x['m1'] <= x['m2'] and x['m1'] <= x['m3']: 
        return x['t1']*0.90 + x['t2']*0.05 + x['t3']*0.05
    if x['m2'] <= x['m1'] and x['m2'] <= x['m3']: 
        return x['t2']*0.90 + x['t1']*0.05 + x['t3']*0.05
    return x['t3']*0.90 + x['t1']*0.05 + x['t2']*0.05

dft['target'] = dft.apply(lambda x: choice(x), axis=1)

display(dft)

df_Vote = dft.rename(columns={'target':'diagnosed_diabetes'})[['diagnosed_diabetes']]
...
params = {
      ...
      'id_target': ['id',"diagnosed_diabetes"],          
      'type_sort': ['asc/desc',0.30,0.70 ],
      'subwts'   : [w/300 for w in [+10,-3,-7]],
      'subm'     : [
          {'name': f'g1', 'weight':+0.33, 'color':'royalblue'},
          {'name': f'g2', 'weight':+0.33, 'color':'orange'   },
          {'name': f'g3', 'weight':+0.34, 'color':'green'    },]
}
df_cross = h_blend(params, details=True, subm='cross.csv')
df_cross
...
df['diagnosed_diabetes'] = \
    df_Vote ['diagnosed_diabetes'] * 0.90 + \
    df_cross['diagnosed_diabetes'] * 0.10

df.to_csv('vc.csv',index=False)
...
df = pd.read_csv('/kaggle/input/playground-series-s5e12/sample_submission.csv')
df1 = pd.read_csv('/kaggle/input/19-december-2025-ps-s5e12/0.70776.csv')
df2 = pd.read_csv('/kaggle/working/vc.csv')

df['diagnosed_diabetes'] = \
    df1['diagnosed_diabetes'] * 0.995 + \
    df2['diagnosed_diabetes'] * 0.005

plt.figure(figsize=(9, 3))

pred1 = df1['diagnosed_diabetes']
sns.kdeplot(pred1, label = '776', linewidth = 0.5)
pred2 = df2['diagnosed_diabetes']
sns.kdeplot(pred2, label = 'hb30g', linewidth = 0.5)

pred = df['diagnosed_diabetes']
sns.kdeplot(pred, label = 'blend  [776 x 30g]  [0.995 + 0.005]', linewidth=1, linestyle='dashed')
...
for file in 'g1,g2,g3,cross,vc,tida1,tida2,tida3,tida_desc'.split(','): 
