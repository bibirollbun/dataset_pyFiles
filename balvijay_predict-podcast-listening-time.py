...
def TRAIN(params, X, y, df_test, df_subm, encoded_column_start_index=11, n_splits=5):
    ...
    for fold, (idx_train, idx_valid) in enumerate(cv.split(X, y)):
        ...
        for c in tqdm(encoded_columns, desc="Encoding columns"):
            # Ensure the column is treated as categorical (either string or category type)
            X_train[c] = X_train[c].astype(str)
            X_valid[c] = X_valid[c].astype(str)
            X_test[c] = X_test[c].astype(str)

            # Initialize TargetEncoder and apply transformation
        ...
        model = XGBRegressor(**params, tree_method='hist')  # Use 'hist' for CPU
        model.fit(
            X_train, y_train,
...
ParamsXGB = {
    'max_depth': 10,
    'learning_rate': 0.00505052132642901,
    'min_child_weight': 9,
    'enable_categorical': True,
    'subsample': 0.8261771678236348,
    'colsample_bytree': 0.7657134251473725,
    'n_estimators': 10000,
    'gamma': 1.6081028372742674,
    'reg_alpha': 0.11243311001772383,
    'reg_lambda': 0.15742461454226775
}

# Now call TRAIN with all necessary arguments
y_pred_xgb, oof_xgb = TRAIN(
    params=ParamsXGB,
    X=X,
    y=y,
    df_test=df_test,
    df_subm=df_subm,
    encoded_column_start_index=11,
    n_splits=5
)
