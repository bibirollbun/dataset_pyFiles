...
# NEVER encode MachineIdentifier
if "MachineIdentifier" in categorical_cols:
    categorical_cols.remove("MachineIdentifier")

# Convert all categorical columns to string so sklearn doesn't crash
...
# REMOVE MachineIdentifier from features used for modeling
if "MachineIdentifier" in X.columns:
    X = X.drop(columns=["MachineIdentifier"])
...
# Save MachineIdentifier before any encoding / dropping
machine_ids = test_df["MachineIdentifier"].astype(str)
...
# ============================================================
# STEP D â€” Apply SAME OrdinalEncoder as training (LATER, after alignment)
# ============================================================
# ============================================================
# BUILD X_full and y_full from cleaned training df
# ============================================================

X_full = df.drop(columns=["HasDetections"])

if "MachineIdentifier" in X_full.columns:
    X_full = X_full.drop(columns=["MachineIdentifier"])

y_full = df["HasDetections"]

# We will **not** encode here yet.
# Just leave this comment block as a placeholder; actual encoding happens
# after we align columns with X_full below.

machine_ids = test_df["MachineIdentifier"].astype(str)


# ============================================================
# STEP E â€” Align test_df columns to X_full
# ============================================================
...
# ============================================================
# STEP F â€” Encode categorical columns NOW (after alignment)
# ============================================================

# Use the SAME categorical columns as training, in the SAME order
categorical_cols_test = [c for c in categorical_cols if c in test_df.columns]

# Convert to string for encoder compatibility
test_df[categorical_cols_test] = test_df[categorical_cols_test].astype(str)

# IMPORTANT: pass a NumPy array so sklearn doesn't check column names
test_df[categorical_cols_test] = encoder.transform(test_df[categorical_cols_test].values)


# ============================================================
# STEP G â€” Add MachineIdentifier back
# ============================================================

test_df.insert(0, "MachineIdentifier", machine_ids)

# ============================================================
# STEP H â€” Predict on cleaned, encoded test_df
# ============================================================

test_preds = xgb_best.predict_proba(test_df.drop(columns=["MachineIdentifier"]))[:, 1]

# ============================================================
# STEP I â€” Build submission.csv
# ============================================================
