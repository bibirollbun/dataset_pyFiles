...
def train_one_epoch(model, train_loader, loss_fn, optimizer, scheduler=None, epoch=None):
    ...
    return loss_train.compute().item(), metric_train  # Return average loss and F1 for the epoch
...
pipeline = Pipeline([
    ('imputer', SensorImputer(method='median', thm_threshold=15.0, selected_sensors=['acc', 'rot'])),
...
for epoch in range(1, num_epochs+1):
    # Train
    loss_train, metric_train = train_one_epoch(model, train_loader, loss_fn, optimizer, scheduler, epoch)
    print(f'ğŸ”¸ Train: Loss={loss_train:.4} | Metric={metric_train:.4}')

    # Validation
    loss_valid, metric_valid = evaluate(model, valid_loader)
    print(f'ğŸ”¹ Valid: Loss={loss_valid:.4} | Metric={metric_valid:.4}')
...
# ==== EVAL CELL: tá»•ng há»£p má»�i metric vÃ o 1 Ã´ ====
import numpy as np
import torch
...
# 1) Device + classes (tÃªn lá»›p theo Ä‘Ãºng thá»© tá»± encoder)
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
if "label_encoder" not in globals():
    raise RuntimeError("ChÆ°a tháº¥y label_encoder. HÃ£y Ä‘áº£m báº£o báº¡n Ä‘Ã£ fit LabelEncoder trÆ°á»›c Ã´ nÃ y.")
classes = list(label_encoder.classes_)
num_classes = len(classes)

# 2) Ä�Ã¡nh giÃ¡ trÃªn valid_loader
model.to(device)
model.eval()
...
with torch.no_grad():
    for batch in valid_loader:
        # batch cÃ³ thá»ƒ lÃ  (X, y) hoáº·c dict; Ä‘á»•i tuá»³ Dataset cá»§a báº¡n
        if isinstance(batch, (tuple, list)) and len(batch) == 2:
            batch_x, batch_y = batch
        elif isinstance(batch, dict):
            batch_x, batch_y = batch["x"], batch["y"]
        else:
            raise RuntimeError("KhÃ´ng nháº­n diá»‡n Ä‘Æ°á»£c cáº¥u trÃºc batch tá»« valid_loader.")

        batch_x = batch_x.to(device)
        # y cÃ³ thá»ƒ lÃ  LongTensor (class index) hoáº·c one-hot
        if isinstance(batch_y, torch.Tensor):
            if batch_y.dtype in (torch.int64, torch.long):
                y_true = batch_y.cpu().numpy()
            else:
                y_true = batch_y.argmax(dim=1).cpu().numpy()
        else:
            # phÃ²ng trÆ°á»�ng há»£p dataset tráº£ numpy
            y_true = np.asarray(batch_y)
...
# 4) (Tuá»³ chá»�n) náº¿u báº¡n cÃ³ danh sÃ¡ch nhÃ£n BFRB -> tÃ­nh thÃªm Binary F1 (BFRB vs non-BFRB)
if "bfrb_gestures" in globals():
    try:
        bfrb_indices = label_encoder.transform(bfrb_gestures)
        bin_true = np.isin(all_targets, bfrb_indices).astype(int)
        bin_pred = np.isin(all_preds,   bfrb_indices).astype(int)
        f1_binary = f1_score(bin_true, bin_pred, zero_division=0)
        final_metric = 0.5 * (f1_binary + f1_macro)
        print(f"F1 Binary (BFRB vs non-BFRB): {f1_binary:.4f}")
        print(f"Final metric (avg Binary & Macro): {final_metric:.4f}")
    except Exception as e:
        print(f"[Note] KhÃ´ng tÃ­nh Ä‘Æ°á»£c F1 Binary vÃ¬ lá»—i: {e}")
...
# ===== TRAIN ARTIFACT SNAPSHOT (safe to paste in your TRAIN notebook) =====
import os, joblib, numpy as np, pandas as pd
from sklearn.preprocessing import LabelEncoder

# 0) Báº£o Ä‘áº£m cÃ³ train_df; náº¿u chÆ°a, tá»± load
if 'train_df' not in globals():
    train_df = pd.read_csv("/kaggle/input/cmi-detect-behavior-with-sensor-data/train.csv")

# 1) CLASSES (thá»© tá»± nhÃ£n Ä‘Ãºng vá»›i model)
if 'label_encoder' in globals():
    classes = list(label_encoder.classes_)
else:
    _le = LabelEncoder()
    _ = _le.fit_transform(train_df['gesture'].astype(str))
    classes = list(_le.classes_)
    label_encoder = _le  # lÆ°u láº¡i Ä‘á»ƒ dÃ¹ng tiáº¿p

# 2) FEATURE COLS (Ä‘Ãºng thá»© tá»± Ä‘Ã£ train: IMU trÆ°á»›c, rá»“i THM/ToF + engineered features náº¿u cÃ³)
imu_cols = ['acc_x','acc_y','acc_z','rot_w','rot_x','rot_y','rot_z']

# ToF/Thermal â€œrawâ€� (náº¿u báº¡n khÃ´ng drop pixel ToF)
thm_tof_cols = [c for c in train_df.columns if c.startswith('thm_') or c.startswith('tof_')]

# Náº¿u báº¡n cÃ³ engineered features, liá»‡t kÃª vÃ o Ä‘Ã¢y theo Ä‘Ãºng thá»© tá»± báº¡n Ä‘Ã£ feed vÃ o model:
extra_feats = []
# vÃ­ dá»¥ (báº­t nhá»¯ng dÃ²ng báº¡n thá»±c sá»± Ä‘Ã£ dÃ¹ng khi train):
# extra_feats += ['acc_mag','rot_angle','acc_mag_jerk','rot_angle_vel']
# extra_feats += [f"tof_{i}_{stat}" for i in range(1,6) for stat in ['mean','std','min','max']]

if 'feature_cols' in globals():
    # TÃ´n trá»�ng danh sÃ¡ch Ä‘Ã£ cÃ³ sáºµn (náº¿u báº¡n táº¡o trÆ°á»›c Ä‘Ã³ trong notebook)
    feature_cols = list(feature_cols)
else:
    # Máº·c Ä‘á»‹nh: IMU trÆ°á»›c, rá»“i extra_feats (náº¿u cÃ³), rá»“i THM/ToF
    feature_cols = imu_cols + extra_feats + thm_tof_cols

# 3) MAXLEN (Ä‘á»™ dÃ i pad/truncate)
if 'pad_len' in globals():
    maxlen = int(pad_len)
elif 'maxlen' in globals():
    maxlen = int(maxlen)
else:
    # Tá»± tÃ­nh theo p90 náº¿u chÆ°a cÃ³
    seq_lengths = train_df.groupby('sequence_id').size().to_numpy()
    maxlen = int(np.percentile(seq_lengths, 90))

# 4) SCALER (tÃ¹y chá»�n)
scaler = globals().get('scaler', None)  # náº¿u báº¡n Ä‘Ã£ fit scaler trong train, nÃ³ sáº½ cÃ³ á»Ÿ Ä‘Ã¢y

# 5) SAVE
os.makedirs("outputs", exist_ok=True)
np.save("outputs/feature_cols.npy", np.array(feature_cols, dtype=object))
np.save("outputs/classes.npy",     np.array(classes, dtype=object))
np.save("outputs/maxlen.npy",      np.array([maxlen], dtype=np.int32))
if scaler is not None:
    joblib.dump(scaler, "outputs/scaler.pkl")

print("âœ… Saved artifacts to ./outputs")
print(f" - feature_cols.npy  ({len(feature_cols)} features)")
print(f" - classes.npy       ({len(classes)} classes)")
print(f" - maxlen.npy        ({maxlen})")
print(f" - scaler.pkl        ({'yes' if scaler is not None else 'no'})")
