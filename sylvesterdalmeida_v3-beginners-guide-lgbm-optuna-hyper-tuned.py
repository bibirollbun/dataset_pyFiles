...
def objective(trial):
    ...
    cv_results = lgb.cv(
            ...
            callbacks=[lgb.early_stopping(stopping_rounds=100, verbose=100)], # stop/print result every 100 iterations
            seed=SEED
    )

    ## -- Extract the best mean validation logloss --
    best_score = np.min(cv_results['valid binary_logloss-mean'])
    best_iteration = pd.Series(cv_results['valid binary_logloss-mean']).idxmin() + 1 # Add 1 because index is 0-based
...
test_predictions = final_lgb_model.predict(test)
...
submit.to_csv('submission_LGBM_tuned.csv', index=False)
