...
class PawpularModel(nn.Module):
    def __init__(self):
        ...
        # FIX: Changed from "resnet50" to "eca_nfnet_l1" to match the checkpoint architecture
        # The checkpoint was trained with eca_nfnet_l1 with num_classes=512
        self.model = timm.create_model("eca_nfnet_l1", pretrained=False, in_chans=3, num_classes=512)
        
        self.dropout = nn.Dropout(0.5) # increase dropout
        # self.out = nn.Linear(1280+12, 1)
        # FIX: Changed from 1000 to 512 to match checkpoint's output layer shape
