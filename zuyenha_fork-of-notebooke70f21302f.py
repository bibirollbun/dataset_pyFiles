...
def _square(x): return np.power(x, 2)
def _cube(x): return np.power(x, 3)
def _neg(x): return -x
...
# === Step 3: Globals and Pipeline ===
gp_transformer = None
scaler = None
SHAP_THRESHOLD = 0.001
filtered_programs = []

# === Step 4: Target Encoding Function ===
def target_encode_sex(train_df, test_df, target_name="Calories"):
    global_mean = train_df[target_name].mean()
    oof_encoding = np.zeros(len(train_df))
    kf = KFold(n_splits=5, shuffle=True, random_state=42)

    for train_idx, val_idx in kf.split(train_df):
        fold_train = train_df.iloc[train_idx]
        fold_val = train_df.iloc[val_idx]
        means = fold_train.groupby("Sex")[target_name].mean()
        oof_encoding[val_idx] = fold_val["Sex"].map(means).fillna(global_mean)

    train_df["Sex_TE"] = oof_encoding
    sex_means_full = train_df.groupby("Sex")[target_name].mean()
    test_df["Sex_TE"] = test_df["Sex"].map(sex_means_full).fillna(global_mean)

    return train_df, test_df

# === Step 5: Custom Symbolic Feature Filter ===
def transform_with_programs(X_raw, programs):
    features = [prog.execute(X_raw) for prog in programs]
...
def prepare_data(df, fit=False):
    ...
    df = df.copy()
    df = df.drop(columns=["Sex"])  # drop original categorical
    ...
    if fit:
        ...
        gp_transformer = SymbolicTransformer(
            ...
            n_components=100,
            function_set=(
                'add', 'sub', 'mul', 'div', 'log', 'sqrt', 'abs', 'max', 'min',
                square, cube, neg
            ),
...
def punish(preds, labels):
    penalty = np.abs(np.expm1(preds) - np.expm1(labels))
...
for fold, (train_idx, val_idx) in enumerate(kf.split(X)):
    ...
    model_fold.fit(
        ...
        sample_weight=1 + punish(np.full_like(y_train_fold, y_train_fold.mean()), y_train_fold),
        verbose=False
    )
...
print(f"\nâœ… Final OOF RMSLE (weighted): {rmsle_oof:.5f}")
