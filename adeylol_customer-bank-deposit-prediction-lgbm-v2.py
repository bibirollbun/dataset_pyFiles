...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y)):
    ...
    model = lgb.LGBMClassifier(
        n_estimators=100000,       # more rounds, early_stopping will stop earlier anyway
        learning_rate=0.02,        # smaller LR for better generalization
        num_leaves=256,            # more expressive trees
        max_depth=12,              # allow deeper trees
        min_child_samples=30,      # prevent overfitting small groups
        subsample=0.9,             # stronger row subsampling
        colsample_bytree=0.7,      # more features per tree
        reg_alpha=0.5,             # L1 regularization
        reg_lambda=2.0,            # stronger L2 regularization
        min_split_gain=0.1,        # require gain to split (stability)
        max_bin=2047,              # reduced from 4859 â†’ faster, still fine-grained
        class_weight='balanced',   # keep handling imbalance
        boosting_type='gbdt',
        metric='auc',
        random_state=2003,
        verbosity=-1,
        # device='gpu'               # âš¡ enable GPU
    )
