...
features = [col for col in train.columns if not col.startswith('D') and not col.startswith('C') and not col.startswith('V')]
print(f'features: {features}')


plot_numerical_features(train[list(set(numerical_features) & set(features))])


plot_categorical_features(train[list(set(categorical_features) & set(features))])
...
train_scaled = pd.DataFrame(scaler.fit_transform(train[features].sample(TUNE_DATASET_LEN)), columns=train[features].columns)
