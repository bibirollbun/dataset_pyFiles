...
# st_names  = ['38_285','38_213','38_192','38_000','37_971']           Lb=0.38_254
...
st_names = ['38_285','38_265','38_261','38_254','38_213']
obezyany = ['any3_0','any3_1','any3_2','any3_3','any3_4']

st_names += ['GEN_27.7a','GEN_27.7b','GEN_27.7c']
obezyany += [   'g27.7a',   'g27.7b',   'g27.7c']

def Visual_any3in4_GEN(dfs,ns,gen):
    ns            = ns[:5]
    nslb          = ns
    nsg1          = ns+['GEN_27.7a']
    nsg2          = ns+['GEN_27.7b'] # -> ['38_285','38_265','38_261','38_213']
    nsg3          = ns+['GEN_27.7c']
    dfs['any3_0'] = dfs.apply(lambda x: f_any3in4(x,0,nslb), axis=1)
    dfs['any3_1'] = dfs.apply(lambda x: f_any3in4(x,1,nslb), axis=1)
    dfs['any3_2'] = dfs.apply(lambda x: f_any3in4(x,2,nslb), axis=1)
    dfs['any3_3'] = dfs.apply(lambda x: f_any3in4(x,3,nslb), axis=1)
    dfs['any3_4'] = dfs.apply(lambda x: f_any3in4(x,4,nslb), axis=1)
    dfs['g27.7a'] = dfs.apply(lambda x: f_not_equ(x,5,nsg1), axis=1)
    dfs['g27.7b'] = dfs.apply(lambda x: f_not_equ(x,6,nsg2), axis=1)
    dfs['g27.7c'] = dfs.apply(lambda x: f_not_equ(x,7,nsg3), axis=1)
...
Visual_any3in4_GEN(dfs, st_names, gen='GEN_27.7(a,b,c)')
...
A = pd.read_csv (path + '38_357.csv')            # Lb=0.38_357
B = pd.read_csv (path + 'GEN_27.7b.csv')         # Lb=0.38_282

A,B = A.iloc[0:111_111],B.iloc[111_111:250_001]  # Lb=0.38_359
...
A,B = A.iloc[0:111_111],B.iloc[111_111:250_001]  # Lb=?
