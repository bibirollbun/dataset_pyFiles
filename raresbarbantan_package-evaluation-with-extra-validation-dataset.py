!pip install -q cairosvg bitsandbytes google-re2
...
with torch.no_grad():
    train_df["svg"] = [model.predict(d) for d in descriptions]
train_df.head()


descriptions = val_df["description"].tolist()
with torch.no_grad():
    val_df["svg"] = [model.predict(d) for d in descriptions]
val_df.head()
...
train_score = score(solution, submission, "row_id", random_seed=42)
print(f"Scored {train_score: .3f} on {len(submission)} images")
...
solution = val_df[["id", "question", "choices", "answer"]].copy()
submission = val_df[["id", "svg"]].copy()

val_score = score(solution, submission, "row_id", random_seed=42)
print(f"Scored {val_score: .3f} on {len(submission)} images")
