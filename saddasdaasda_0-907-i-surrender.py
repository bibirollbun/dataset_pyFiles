...
def preprocess_data(df1):
    df1 = df1.copy()
    df1['Age'] = df1['Age'].apply(process_age)
    return df1
...
model = CatBoostClassifier(
    iterations=2000,  # 迭代次数，即树的数量
    learning_rate=0.1,  # 学习率，控制每次迭代步长
    depth=4,  # 树的最大深度
    ...
    early_stopping_rounds=500,  # 如果验证集上的指标在 50 轮迭代内没有提升，则提前停止训练
...
df2[category] = df2[category].fillna('nan')
