...
train_df["month_num"] = pd.Categorical(
    train_df["month"],
...
cf = CyclicalFeatures(variables=['day', 'month_num'], drop_original=True)


extracted_train_df = cf.fit_transform(train_df)
...
train_pool = Pool(X_train, y_train, cat_features=object_columns)
test_pool = Pool(X_test, y_test, cat_features=object_columns)
...
shap_summary = select_features_adult(algorithm=EFeaturesSelectionAlgorithm.RecursiveByShapValues, steps=5)
...
# Get feature columns
feature_columns = [col for col in shap_summary['selected_features_names'] if col not in exclude_cols]
