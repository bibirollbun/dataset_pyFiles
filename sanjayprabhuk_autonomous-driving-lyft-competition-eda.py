...
!pip install lyft-dataset-sdk --quiet


# The upgrades below are because of a scipy error. This works so do not touch. Please.
!pip install --upgrade numpy==1.26.4 --quiet
!pip install --upgrade scipy==1.14.1 --quiet
...
print(f"The columns of the training data are: {df_train.columns}")


df_train.head()


# Exploring the columns of the df_train

print(f"ID Column sample: {df_train['Id'][0]}")
print("*"*100)
print(f"Prediction Column sample: {df_train['PredictionString'][0][:500]}")


print(f"First 8 split: {df_train['PredictionString'][0].split()[:8]}")
print(f"Second split: {df_train['PredictionString'][0].split()[8:16]}")
...
df_train["PredictionString"].shape
...
df_new_train_objects['center_x'][:1000].corr(df_new_train_objects['center_y'][:1000])
...
lyft_dataset = LyftDataset(data_path=common_path, json_path=train_json_path)
