...
# Get top-3 predictions (indices of highest probabilities)
top3_preds = np.argsort(y_proba, axis=1)[:, -3:][:, ::-1]  # take top3 in descending order

# Decode predicted labels
top3_labels = [le_fert.inverse_transform(row) for row in top3_preds]
...
def mapk(y_true, y_pred, k=3):
    ...
    for true, pred in zip(y_true, y_pred):
        if true in pred[:k]:
            score += 1 / (pred[:k].tolist().index(true) + 1)
...
#submission.to_csv(r"/kaggle/input/playground-series-s5e6/finalsubmission.csv",  index = False)
