# ============================================
# DistilRoBERTa strong baseline (faster Kaggle-friendly)
# - Cosine LR + warmup, Early stopping
# - 5-fold CV ensembling, pos_weight for imbalance
# - Lighter defaults: MAX_LEN=192, EPOCHS=1, eval every 500 steps
...
MODEL_NAME = "distilroberta-base"     # "roberta-base" = tiny boost, but slower
MAX_LEN    = 192                      # <= 512; shorter = much faster
EPOCHS     = 1                        # Change to 2 if you have more time
LR         = 2e-5
BS_TRAIN   = 16
BS_EVAL    = 64                       # larger eval batch is safe & faster
WARMUP_RATIO   = 0.06
WEIGHT_DECAY   = 0.01
GRAD_ACCUM_STEPS = 2
USE_CV     = True
...
# Pad to multiple of 8 for Tensor Cores (faster on T4)
...
# 2) Token lengths (first 20k) — use truncation so no >512 warning in plotting
...
def train_once(trn_df, val_df, fold_name="single"):
    ...
    # Avoid deprecation warning across versions
    ...
    if did_any:
        plt.title(f"Precision–Recall curves — {fold_name}")
    ...
    boxes = [("Text", 0.05), ("Tokenizer (BPE)", 0.25), (f"{MODEL_NAME}", 0.50), ("[CLS]/Pooling", 0.75), ("6 logits \u2192 Sigmoid", 0.90)]
...
if USE_CV:
    ...
    for k, (idx_tr, idx_va) in enumerate(skf.split(train_df, has_any)):
        ...
        # OOF save (use the true 'id' from the pandas frame)
