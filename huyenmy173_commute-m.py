...
def fpr_experiment():
    ...
    for M in M_list:
        ...
        while trial < num_trials:
            ...
            lambdas_w = [np.sqrt(coeff1[M] * np.log(p) / nk)  for nk in n_list[:K]]
            lambdas_del = [np.sqrt(coeff2[M] * np.log(p) / n0) ] * K
            lambda_c = np.sqrt(np.log(p) / n_tilde) * coeff3[M]
    ...
    for M in M_list:
        ...
        for i in range(3):
            ...
            while trial < num_trials:
                ...
                lambda_c = np.sqrt(np.log(p) / n_tilde) * 25 #coeff[M]
                # Unsharable sources
                unsh_w_hat_list = calculate_unshare_w_hat(unsh_XS_list, unsh_YS_list, lambdas_w[M:])
                n = sum(n_list[: M]) + n0
                Y = np.concatenate(share_YS_list + [Y0])
            ...
            tpr_values[M].append(tpr)
            oc_tpr_values[M].append(oc_tpr)
            print ("________________________________________________________")
