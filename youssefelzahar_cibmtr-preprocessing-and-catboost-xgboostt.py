...
cat_model = CatBoostClassifier(cat_features=categorical_features, iterations=500, learning_rate=0.05, depth=6, verbose=100)
...
X_xgb = data[numerical_features]
y_xgb = data[target]

test_xgb = test_data[numerical_features]

X_train_xgb, X_val_xgb, y_train_xgb, y_val_xgb = train_test_split(X_xgb, y_xgb, test_size=0.2, random_state=42)

xgb_model = XGBClassifier(n_estimators=500, learning_rate=0.05, max_depth=6, subsample=0.8, colsample_bytree=0.8, random_state=42)
xgb_model.fit(X_train_xgb, y_train_xgb, eval_set=[(X_val_xgb, y_val_xgb)], early_stopping_rounds=50, verbose=100)

xgb_test_pred = xgb_model.predict_proba(test_xgb)[:, 1]

test_predictions = 0.5 * cat_test_pred + 0.5 * xgb_test_pred
