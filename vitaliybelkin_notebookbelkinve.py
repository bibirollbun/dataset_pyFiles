...
# ĞŸÑ€Ğ¾Ğ¿Ğ¸ÑˆĞ¸Ñ‚Ğµ Ğ´Ğ¾Ğ¿Ğ¾Ğ»Ğ½Ğ¸Ñ‚ĞµĞ»ÑŒĞ½Ñ‹Ğµ Ğ±Ğ¸Ğ±Ğ»Ğ¸Ğ¾Ñ‚ĞµĞºĞ¸, ĞºĞ¾Ñ‚Ğ¾Ñ€Ñ‹Ğµ Ğ¿Ğ¾Ñ‚Ñ€ĞµĞ±ÑƒÑ�Ñ‚Ñ�Ñ� Ğ´Ğ»Ñ� Ñ€ĞµÑˆĞµĞ½Ğ¸Ñ�
...
class RSNAModel(nn.Module):
    def __init__(self, num_classes=2):  # 1 ĞºĞ»Ğ°Ñ�Ñ� + background
        super().__init__()
        backbone = resnet_fpn_backbone('resnet18', pretrained=True)
        self.model = FasterRCNN(backbone, num_classes=num_classes)

    def forward(self, images, targets=None):
        return self.model(images, targets)  # targets Ñ‚Ñ€ĞµĞ±ÑƒĞµÑ‚Ñ�Ñ� Ğ¿Ñ€Ğ¸ Ğ¾Ğ±ÑƒÑ‡ĞµĞ½Ğ¸Ğ¸
...
# ĞœĞ¾Ğ´ĞµĞ»ÑŒ
model = RSNAModel(num_classes=2).to(device)  # 1 ĞºĞ»Ğ°Ñ�Ñ� (Ñ‡Ğ°Ğ¹ĞºĞ°) + background
...
# ĞŸĞµÑ€Ğ²Ğ°Ñ� Ñ�Ñ‚Ñ€Ğ¾ĞºĞ° Ğ³Ñ€Ğ°Ñ„Ğ¸ĞºĞ¾Ğ²
plt.subplot(2, 3, 1)
plt.plot(metrics_df['epoch'], metrics_df['train_loss'], marker='o')
plt.title('ĞŸĞ¾Ñ‚ĞµÑ€Ğ¸ Ğ¿Ñ€Ğ¸ Ğ¾Ğ±ÑƒÑ‡ĞµĞ½Ğ¸Ğ¸')
plt.xlabel('Ğ­Ğ¿Ğ¾Ñ…Ğ°')
plt.ylabel('ĞŸĞ¾Ñ‚ĞµÑ€Ğ¸')

plt.subplot(2, 3, 2)
plt.plot(metrics_df['epoch'], metrics_df['accuracy'], marker='o')
plt.title('Ğ¢Ğ¾Ñ‡Ğ½Ğ¾Ñ�Ñ‚ÑŒ')
plt.xlabel('Ğ­Ğ¿Ğ¾Ñ…Ğ°')
plt.ylabel('Ğ¢Ğ¾Ñ‡Ğ½Ğ¾Ñ�Ñ‚ÑŒ')
...
# Ğ’Ñ‚Ğ¾Ñ€Ğ°Ñ� Ñ�Ñ‚Ñ€Ğ¾ĞºĞ° Ğ³Ñ€Ğ°Ñ„Ğ¸ĞºĞ¾Ğ²
plt.subplot(2, 3, 4)
...
plt.subplot(2, 3, 5)
plt.plot(metrics_df['epoch'], metrics_df['f1'], marker='o')
plt.title('F1 Score')
plt.xlabel('Epoch')
plt.ylabel('F1')

plt.subplot(2, 3, 6)
plt.plot(metrics_df['epoch'], metrics_df['roc_auc'], marker='o')
plt.title('ROC AUC')
plt.xlabel('Epoch')
plt.ylabel('ROC AUC')
...
for name, model_fn in [
    ...
    for epoch in range(epochs + 1):
        train_loss = train_one_epoch(model, train_loader, optimizer, epoch=epoch, device=device)
        
        avg_boxes, accuracy, precision, recall, f1, roc_auc = valid_one_epoch(model, valid_loader, epoch=epoch, device=device)
...
for name, history in results:
    ...
    if name == "FasterRCNN":
      ...
      model.load_state_dict(torch.load("FasterRCNN.pt", map_location=device))
    elif name == "FasterRCNN-MobileNet":
      model = create_fasterrcnn_mobilenet().to(device)
      model.load_state_dict(torch.load("FasterRCNN-MobileNet.pt", map_location=device))
    elif name == "RetinaNet":
      model = create_retinanet().to(device)
      model.load_state_dict(torch.load("RetinaNet.pt", map_location=device))
...
# Ğ¤Ğ¸Ğ½Ğ°Ğ»ÑŒĞ½Ñ‹Ğµ Ğ¼ĞµÑ‚Ñ€Ğ¸ĞºĞ¸ ĞºĞ°Ğ¶Ğ´Ğ¾Ğ¹ Ğ¼Ğ¾Ğ´ĞµĞ»Ğ¸ (Ğ¿Ğ¾Ñ�Ğ»ĞµĞ´Ğ½Ñ�Ñ� Ñ�Ğ¿Ğ¾Ñ…Ğ°) Ğ²Ğ°Ğ»Ğ¸Ğ´Ğ°Ñ†Ğ¸Ğ¾Ğ½Ğ½Ğ¾Ğ¹ Ğ²Ñ‹Ğ±Ğ¾Ñ€ĞºĞ¸
summary = []
for name, history in results:
    last = history[-1]
    last['model'] = name
