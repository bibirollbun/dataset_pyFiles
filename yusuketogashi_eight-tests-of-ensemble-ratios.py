...
#    #avg_pred = pred1 * 0.65 + pred2 * 0.35
...
def predict(sequence, demographics):
    pred1 = predict1(sequence, demographics)   # shape (1, n_classes)
    pred2 = predict2(sequence, demographics)   # shape (1, n_classes)

    # ---- ここを修正 ----
    p1 = pred1.ravel()         # または pred1.squeeze() / pred1[0]
    p2 = pred2.ravel()

    rank1 = pd.Series(p1).rank(method="average", pct=True).to_numpy()
    rank2 = pd.Series(p2).rank(method="average", pct=True).to_numpy()
    avg_pred = (rank1 + rank2) / 2
    # --------------------
