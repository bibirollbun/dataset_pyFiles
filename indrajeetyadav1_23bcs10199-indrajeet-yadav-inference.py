...
model = CustomModel()
model = Tez(model)
model.load(f"/kaggle/input/indrajeet-yadav-10199-training-petfinder-myy/model_f{i}.bin", weights_only=True)

# Read test CSV
df_test = pd.read_csv("/kaggle/input/siim-isic-melanoma-classification/test.csv")

# ✅ FIX: use correct column name ("image_name") instead of "Id"
if "Id" in df_test.columns:
    id_col = "Id"
elif "image_name" in df_test.columns:
    id_col = "image_name"
else:
    raise KeyError(f"Expected 'Id' or 'image_name' in test.csv columns, found: {df_test.columns.tolist()}")

test_img_paths = [f"/kaggle/input/siim-isic-melanoma-classification/jpeg/test/{x}.jpg" for x in df_test[id_col].values]
...
# ✅ FIX: use id_col variable instead of hardcoding "Id"
df_test["target"] = super_final_predictions
df_test = df_test[[id_col, "target"]]
