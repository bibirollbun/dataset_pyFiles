...
# - ğŸ”� Submission label distribution printed for quick verification
# - ğŸ“„ Sample prediction text check for selected row
...
def run_full_pipeline():
    ...
    print(f"\n[ğŸ“Š] Classification Report for {'LightGBM' if LGBM_AVAILABLE else 'Random Forest'}:")
    ...
    title_model = 'LightGBM' if LGBM_AVAILABLE else 'Random Forest'
    plt.title(f'ROC Curve - {title_model}')
    ...
    plt.title(f"SHAP Summary - {title_model}")
    plt.tight_layout()
    plt.savefig(f"shap_summary_{'lightgbm' if LGBM_AVAILABLE else 'random_forest'}.png")
    plt.close()
    ...
    submission['type'] = submission['label'].map({1: "Primary", 0: "Mention"})
    submission.insert(0, 'row_id', range(len(submission)))

    print("\nğŸ”� Submission label distribution:")
    ...
    print("\nğŸ“„ First few rows of submission.csv:")
    print(submission.head())
    print("\nğŸ“„ Last few rows of submission.csv:")
    print(submission.tail())

    for i in range(3):
        row = submission.iloc[i]
        article_id = row['article_id']
        dataset_id = row['dataset_id'].lower()
        tree = safe_parse_xml(test_paths.get(article_id, ""))
        print(f"\n[ğŸ”�] Row {i} - Article ID: {article_id}")
        if tree is not None:
            full_text = " ".join(tree.xpath("//text()")).lower()
            match = dataset_id in full_text
            print(f"- Found DOI in XML text? {'âœ…' if match else 'â�Œ'}")
            if match:
                start = full_text.find(dataset_id)
                snippet = full_text[max(0, start-100):start+len(dataset_id)+100]
                print("..." + snippet + "...")
        else:
            print("â�Œ Could not parse XML.")

    submission_baseline = submission.copy()
    submission_baseline['label'] = 1
    submission_baseline['type'] = 'Primary'
    submission_baseline.to_csv("submission_all_primary.csv", index=False)
    print("\nğŸ“„ Created static test file: submission_all_primary.csv")

    files_to_zip = [
        "submission.csv",
        "label_distribution.png",
        "roc_curve_comparison.png",
        f"shap_summary_{'lightgbm' if LGBM_AVAILABLE else 'random_forest'}.png",
