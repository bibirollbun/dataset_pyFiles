...
class PairwiseDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        text1, text2, label = row["text1"], row["text2"], int(row["real_text_id"])  # label is 1 or 2
        ...
        return {
            ...
            # **Return raw target as 1 or 2 (torch.long)**
...
EPOCHS = 10
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
BEST_MODEL_PATH = "best_longformer_pairwise.pt"
...
scaler = torch.amp.GradScaler("cuda")  # new style
...
# Helper
# -------------------------
def extract_score(logits):
    """Return a scalar score per example from model.logits."""
    if logits.dim() == 1:  # [batch]
        return logits
    if logits.dim() == 2:
        if logits.size(1) == 1:
            return logits.squeeze(-1)  # [batch]
        elif logits.size(1) == 2:
            return logits[:, 1]        # use class 1 ("real") logit
    return logits.view(logits.size(0), -1).squeeze(-1)
...
for epoch in range(EPOCHS):
    ...
    for i, batch in enumerate(progress_bar):
        ...
        target_raw = batch["target"].to(DEVICE)  # {1,2}

        # convert to +1/-1 for MarginRankingLoss
        mr_target = torch.where(
            target_raw == 1,
        ...
        global_attention_mask_a = torch.zeros_like(input_ids_a).to(DEVICE)
        global_attention_mask_a[:, 0] = 1
        global_attention_mask_b = torch.zeros_like(input_ids_b).to(DEVICE)
        global_attention_mask_b[:, 0] = 1

        with torch.amp.autocast("cuda"):
            logits_a = model(input_ids=input_ids_a,
                             attention_mask=attention_mask_a,
                             global_attention_mask=global_attention_mask_a).logits
            logits_b = model(input_ids=input_ids_b,
                             attention_mask=attention_mask_b,
                             global_attention_mask=global_attention_mask_b).logits

            score_a = extract_score(logits_a)
            score_b = extract_score(logits_b)

            loss = criterion(score_a, score_b, mr_target) / ACCUM_STEPS
        ...
        correct_train += (pred == target_raw).sum().item()
        total_train += target_raw.size(0)
    ...
    with torch.no_grad():
        for batch in val_loader:
            ...
            with torch.amp.autocast("cuda"):
                ...
                loss = criterion(score_a, score_b, mr_target)
                running_val_loss += loss.item()

            pred = torch.where(score_a > score_b, 1, 2)
            correct_val += (pred == target_raw).sum().item()
            total_val += target_raw.size(0)
...
# -------------------------
# Helper: consistent score extraction
# -------------------------
def extract_score(logits):
    if logits.dim() == 1:  # [batch]
        return logits
    if logits.dim() == 2:
        if logits.size(1) == 1:
            return logits.squeeze(-1)  # regression style
        elif logits.size(1) == 2:
            return logits[:, 1]        # class-1 ("real") logit
    return logits.view(logits.size(0), -1).squeeze(-1)
...
for batch in tqdm(test_loader, desc="Predicting"):
    ...
    with torch.no_grad():
        ...
        score_a = extract_score(logits_a).item()
        score_b = extract_score(logits_b).item()
