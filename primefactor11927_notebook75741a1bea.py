...
labels = ['cactus','cactus','cactus','cactus','cactus','cactus','no cactus','no cactus','no cacts','no cactus','no cactus',' no cactus']
...
train_generator = train_datagen.flow_from_dataframe(
    ...
    target_size=(32,32),
    subset="training",
    batch_size=512,      # バッチサイズ：128→512に変更
    shuffle=True,
    class_mode="binary"
    )
# 検証用の画像ジェネレーターを作成
val_generator = train_datagen.flow_from_dataframe(
    ...
    x_col="id",
     ...
     target_size=(32,32),
     subset="validation",
     batch_size=256,      # バッチサイズ：64→256に変更
     shuffle=True,
     class_mode="binary"
)
...
model.compile(
    optimizer=Adam(learning_rate=0.00005),  # 最適化手法をAdamに設定、学習率は0.00005
...
history = model.fit(
    ...
    validation_steps = 7,    # 27→7に変更。通常は、検証画像枚数 / バッチサイズ でok
