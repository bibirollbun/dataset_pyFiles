...
class CatEmbeddings(nn.Module):
    def __init__(
        self,
        projection_dim: int,
        categorical_cardinality: List[int],  # 修复拼写错误
        embedding_dim: int
    ):
        super(CatEmbeddings, self).__init__()
        self.embeddings = nn.ModuleList([
            nn.Embedding(cardinality, embedding_dim)
            for cardinality in categorical_cardinality  # 使用正确的变量名
        ])
        self.projection = nn.Sequential(
            nn.Linear(embedding_dim * len(categorical_cardinality), projection_dim),  # 使用正确的变量名
            nn.GELU(),
...
class LitNN(pl.LightningModule):
    ...
    def calc_loss(self, y, y_hat, efs):
        ...
        hr = torch.exp(y_hat)
        cum_hr = torch.cumsum(hr, dim=0)
        
        # Find events
        events = efs.nonzero(as_tuple=False).view(-1)
        if events.nelement() == 0:
        ...
        for idx in events:
            current_hr = hr[idx]
            sum_hr = cum_hr[idx]  # Because we sorted in descending order
            loss += y_hat[idx] - torch.log(sum_hr)
...
def main(hparams):
    ...
    * train the model and create final submission output
    ...
    test['efs_time'] = 1
    test['efs'] = 1
    oof_nn_nll = np.zeros(len(train_original))  # Renamed to reflect NLL loss
    test_pred = np.zeros(test.shape[0])
    categorical_cols, numerical = get_feature_types(train_original)
    kf = StratifiedKFold(n_splits=5, shuffle=True)  # n_splits=10
    for i, (train_index, test_index) in enumerate(
        ...
        oof_nn_nll[test_index] = oof_pred.detach().cpu().numpy()  # Store predictions for out-of-fold data
        # Create submission
        train = tt.iloc[train_index]
    ...
    return test_pred, oof_nn_nll  # Return raw predictions (no need to negate for NLL)
...
# Example usage
hparams = None
nll_pred, nll_oof = main(hparams)

y_true = train[["ID", "efs", "efs_time", "race_group"]].copy()
y_pred = train[["ID"]].copy()
y_pred["prediction"] = nll_oof
m = score(y_true.copy(), y_pred.copy(), "ID")
print(f"\nNegative Log-Likelihood NN CV =", m)


subm_data = pd.read_csv("/kaggle/input/equity-post-HCT-survival-predictions/sample_submission.csv")
subm_data['prediction'] = nll_pred
