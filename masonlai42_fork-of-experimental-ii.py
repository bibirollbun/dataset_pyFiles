...
print("="*80)
print("INSURANCE INDUSTRY OSINT ANALYZER - ENHANCED VERBOSE VERSION")
print("="*80)
        ...
        for idx, (terms, category) in enumerate(zip(search_terms, categories), 1):
            ...
            try:
                ...
                try:
                    ...
                    print(f"    âš  Could not fetch regional data: {str(e)}")
                
                # Fetch related topics
                print(f"\n  â†’ Fetching related topics for {category}...")
                try:
                    self.pytrends.build_payload([terms[0]], timeframe='today 3-m', geo='US')
                    related_topics = self.pytrends.related_topics()
                    if related_topics and terms[0] in related_topics:
                        if 'top' in related_topics[terms[0]]:
                            top_topics = related_topics[terms[0]]['top']
                            if top_topics is not None and not top_topics.empty:
                                print(f"    âœ“ Found {len(top_topics)} related topics")
                                print(f"    Top related topics:")
                                for idx, row in top_topics.head(3).iterrows():
                                    print(f"      - {row.get('topic_title', 'N/A')}: {row.get('value', 'N/A')}")
                except Exception as e:
                    print(f"    âš  Could not fetch related topics: {str(e)}")
                
            except Exception as e:
                print(f"    âœ— ERROR fetching {category}: {str(e)}")
...
class NewsAnalyzer:
        ...
        if all_articles:
            ...
            return {
                ...
                'summary': {
                    ...
                    'top_positive': df.nlargest(3, 'sentiment_score')[['title', 'sentiment_score', 'source']].to_dict('records'),
                    'top_negative': df.nsmallest(3, 'sentiment_score')[['title', 'sentiment_score', 'source']].to_dict('records'),
    ...
    def fetch_rss_feed(self, url, source_name):
        ...
        try:
            ...
            if feed.bozo:
                print(f"       âš  Warning: Feed parsing issues detected (bozo: {feed.bozo})")
            ...
            for entry in feed.entries[:50]:  # Process up to 50 entries
                try:
                    ...
                    print(f"       âš  Error processing entry: {str(e)[:50]}")
                    continue
    ...
    def analyze_sentiment(self, text):
        try:
            ...
            print(f"         âš  Sentiment analysis error: {str(e)[:50]}")
...
class InsuranceDataVisualizer:
    ...
    def plot_trends_timeline(self, ax, trends_data):
        try:
            ...
            if not data_found:
                ...
                print("      âš  No real data available, using sample data...")
                dates = pd.date_range(start='2024-01-01', periods=90, freq='D')
                sample_data = pd.DataFrame({
                    'car insurance': np.random.randint(40, 80, 90),
                    'auto insurance': np.random.randint(35, 75, 90),
                    'insurance quote': np.random.randint(30, 70, 90),
                }, index=dates)
                
                for i, col in enumerate(sample_data.columns):
                    ax.plot(sample_data.index, sample_data[col], label=col, linewidth=2,
                           color=self.colors[i % len(self.colors)], alpha=0.8)
                
                ax.set_title('Insurance Search Trends Over Time (Sample Data)', fontweight='bold', fontsize=12)
                ax.set_xlabel('Date')
                ax.set_ylabel('Search Interest (0-100)')
    ...
    def plot_news_sentiment(self, ax, news_data):
        try:
            if news_data and 'summary' in news_data and 'sentiment_distribution' in news_data['summary']:
                ...
                # Use sample data
                print("      âš  Using sample sentiment data...")
                sizes = [45, 35, 20]
                labels = ['positive', 'neutral', 'negative']
                colors = ['#2ECC71', '#95A5A6', '#E74C3C']
                ax.pie(sizes, labels=labels, colors=colors, autopct='%1.0f%%', startangle=90)
                ax.set_title('News Sentiment (Sample)', fontweight='bold', fontsize=12)
    ...
    def plot_company_comparison(self, ax, trends_data):
        try:
            ...
            if not data_found:
                # Sample data
                print("      âš  Using sample company data...")
                companies = ['Geico', 'Progressive', 'State Farm', 'Allstate', 'Liberty']
                values = [65, 58, 72, 48, 41]
                bars = ax.bar(companies, values, color=self.colors[:5])
                ax.set_title('Company Brand Interest (Sample)', fontweight='bold', fontsize=12)
                ax.set_ylabel('Avg Search Interest')
    ...
    def plot_complaint_trends(self, ax, trends_data):
        try:
            ...
            if not data_found:
                # Sample data
                print("      âš  Using sample complaint data...")
                complaints = ['Claim Denied', 'Complaint', 'Lawsuit', 'Switch']
                values = [45, 38, 22, 51]
                ax.barh(complaints, values, color='#E74C3C', alpha=0.7)
                ax.set_title('Current Complaint Searches (Sample)', fontweight='bold', fontsize=12)
                ax.set_xlabel('Search Interest')
    ...
    def plot_disaster_trends(self, ax, trends_data):
        try:
            ...
            if not data_found:
                # Sample data
                print("      âš  Using sample disaster data...")
                disasters = ['Flood', 'Hurricane', 'Fire', 'Earthquake']
                values = [62, 45, 71, 28]
                ax.bar(disasters, values, color=['#FF6B6B', '#FFA07A', '#FFB347', '#FFCC5C'])
                ax.set_title('Disaster Insurance Interest (Sample)', fontweight='bold', fontsize=12)
                ax.set_ylabel('Search Interest')
