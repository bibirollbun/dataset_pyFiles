...
import os, random
import gc
import lightgbm  # Mantengo por fallback, pero no uso en main
...
# Importaciones adicionales para grafos y ODE
import networkx as nx
import torch
import torch.nn as nn
from torch.utils.data import Dataset, DataLoader
from torch.optim import Adam
# Try importing additional models (mantengo por compatibilidad)
try:
...
# Clase para classifier con ODE attractor (inspirado en SA-nODE)
class ODEAttractorClassifier(BaseEstimator, ClassifierMixin):
    def __init__(self, in_feats, hidden=128, n_steps=10, lr=0.001, epochs=10, batch_size=256):
        self.in_feats = in_feats
        self.hidden = hidden
        self.n_steps = n_steps  # Pasos Euler para ODE
        self.lr = lr
        self.epochs = epochs
        self.batch_size = batch_size
        self.model = None
        self.attractors = None  # Aprendidos por class
    ...
    class ODEModel(nn.Module):
        def __init__(self, in_feats, hidden):
            super().__init__()
            self.embed = nn.Linear(in_feats, hidden)
            self.ode_func = ODEAttractorClassifier.ODEFunc(hidden)
            self.out = nn.Linear(hidden, 1)  # Binary per action
        def forward(self, x, n_steps=10):
            z = self.embed(x)
            dt = 1.0 / n_steps
            for _ in range(n_steps):
                z = z + self.ode_func(z) * dt  # Euler simple para attractor
            return torch.sigmoid(self.out(z))
    def fit(self, X, y):
        X = torch.tensor(X, dtype=torch.float32).to(device)
        y = torch.tensor(y, dtype=torch.float32).unsqueeze(1).to(device)
        self.model = self.ODEModel(self.in_feats, self.hidden).to(device)
        ...
        for epoch in range(self.epochs):
            ...
            for bx, by in loader:
                ...
                loss.backward()
                optimizer.step()
        # Aprende attractors como mean embedding por class
        self.model.eval()
        with torch.no_grad():
            embeds = self.model.embed(X)
            pos = embeds[y.squeeze() == 1].mean(0)
            neg = embeds[y.squeeze() == 0].mean(0)
            self.attractors = torch.stack([neg, pos])
        return self
    ...
    def attract_new(self, X_new):
        # Para novos, atrae al closest attractor
        X_new = torch.tensor(X_new, dtype=torch.float32).to(device)
        with torch.no_grad():
            emb_new = self.model.embed(X_new)
            dists = torch.cdist(emb_new, self.attractors)
            labels = dists.argmin(1).cpu().numpy()
        return labels
# ==================== SCORING FUNCTIONS ====================
...
def mouse_fbeta(solution: pd.DataFrame, submission: pd.DataFrame, beta: float = 1) -> float:
    if len(solution) == 0 or len(submission) == 0:
        raise ValueValue('Missing solution or submission data')
...
drop_body_parts = [ 'headpiece_bottombackleft', 'headpiece_bottombackright', 'headpiece_bottomfrontleft', 'headpiece_bottomfrontright', 'headpiece_topbackleft', 'headpiece_topbackright', 'headpiece_topfrontleft', 'headpiece_topfrontright', 'spine_1', 'spine_2', 'tail_middle_1', 'tail_middle_2', 'tail_midpoint' ]
        ...
        if generate_single:
            ...
            for mouse_id_str in np.unique(vid_behaviors_subset.agent):
                try:
                    ...
                    single_mouse_meta = pd.DataFrame({ 'video_id': video_id, 'agent_id': mouse_id_str, 'target_id': 'self', 'video_frame': single_mouse.index, 'frames_per_second': row.frames_per_second })
                    if traintest == 'train':
        ...
        if generate_pair:
            ...
            if len(vid_behaviors_subset) > 0:
                for agent, target in itertools.permutations(np.unique(pvid.columns.get_level_values('mouse_id')), 2):
                    ...
                    mouse_pair_meta = pd.DataFrame({ 'video_id': video_id, 'agent_id': agent_str, 'target_id': target_str, 'video_frame': mouse_pair.index, 'frames_per_second': row.frames_per_second })
                    if traintest == 'train':
...
def predict_multiclass_adaptive(pred, meta, action_thresholds):
    """Adaptive thresholding per action + temporal smoothing (simple, no mode awareness)."""
    # Temporal smoothing
    ...
    mask[-1] = False
    submission_part = pd.DataFrame({ 'video_id': meta_changes['video_id'][mask].values, 'agent_id': meta_changes['agent_id'][mask].values, 'target_id': meta_changes['target_id'][mask].values, 'action': pred.columns[ama_changes[mask].values], 'start_frame': ama_changes.index[mask], 'stop_frame': ama_changes.index[1:][mask[:-1]] })
...
def transform_single(single_mouse, body_parts_tracked, fps):
    ...
    X = add_graph_features(X, single_mouse, body_parts_tracked, is_single=True)
...
def transform_pair(mouse_pair, body_parts_tracked, fps):
    ...
    mouse_pair_renamed = mouse_pair.copy()
    mouse_pair_renamed.columns = pd.MultiIndex.from_tuples([(f'{lev0}_{lev1}', lev2) for lev0, lev1, lev2 in mouse_pair.columns])
    X = add_graph_features(X, mouse_pair_renamed, body_parts_tracked * 2, is_single=False)
    return X.astype(np.float32, copy=False)
# ==================== ENSEMBLE TRAINING ====================
def submit_ensemble(body_parts_tracked_str, switch_tr, X_tr, label, meta):
    ...
    for _ in range(3):
        models.append(ODEAttractorClassifier(in_feats, hidden=128, epochs=20 if len(X_tr)<1e6 else 10))
    X_tr_np = X_tr.values
    del X_tr; gc.collect()
    model_list = []
    for action in label.columns:
        y_raw = label[action].values
        mask = ~np.isnan(y_raw)
    ...
    fps_lookup = ( test_subset[['video_id', 'frames_per_second']] .drop_duplicates('video_id') .set_index('video_id')['frames_per_second'] .to_dict() )
    if verbose:
        ...
        try:
            ...
            for action, trained in model_list:
                if action in actions_te:
                    ...
                    low_mask = mean_prob < 0.3
                    if low_mask.any():
                        attracted = trained[0].attract_new(X_te_np[low_mask])
                        mean_prob[low_mask] = attracted
...
for section in range(1, len(body_parts_tracked_list)):
    ...
    try:
        ...
        _fps_lookup = ( train_subset[['video_id', 'frames_per_second']] .drop_duplicates('video_id') .set_index('video_id')['frames_per_second'] .to_dict() )
...
if len(submission_list) > 0:
    ...
    submission = pd.DataFrame({ 'video_id': [438887472], 'agent_id': ['mouse1'], 'target_id': ['self'], 'action': ['rear'], 'start_frame': [278], 'stop_frame': [500] })
