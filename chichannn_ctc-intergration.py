...
class CTCModel(keras.Model):
    def __init__(
        ...
        source_maxlen=FRAME_LEN, # Ä�á»™ dÃ i Ä‘áº§u vÃ o cá»§a LandmarkEmbedding
        # GiÃ¡ trá»‹ máº·c Ä‘á»‹nh cho encoder_output_seqlen giá»� Ä‘Ã¢y lÃ  háº±ng sá»‘ toÃ n cá»¥c Ä‘Ã£ Ä‘Æ°á»£c cáº­p nháº­t
        ...
        # source_maxlen lÃ  Ä‘áº§u vÃ o cho LandmarkEmbedding
        self.landmark_embedding = LandmarkEmbedding(num_hid=num_hid, maxlen=source_maxlen)
        # encoder_output_seqlen_param lÃ  Ä‘áº§u ra cá»§a LandmarkEmbedding, Ä‘áº§u vÃ o cá»§a Positional Embedding vÃ  TransformerEncoder
        self.pos_emb_encoder = layers.Embedding(input_dim=encoder_output_seqlen_param, output_dim=num_hid)
        self.encoder_output_actual_seqlen = encoder_output_seqlen_param # LÆ°u láº¡i Ä‘á»ƒ dÃ¹ng trong train/test_step
    ...
    def call(self, source, training=False):
        ...
        # encoder_seq_len nÃªn Ä‘Æ°á»£c láº¥y tá»« shape cá»§a x sau landmark_embedding,
        # hoáº·c tá»‘t hÆ¡n lÃ  dÃ¹ng self.encoder_output_actual_seqlen Ä‘Ã£ lÆ°u
        current_encoder_seq_len = tf.shape(x)[1] # Hoáº·c dÃ¹ng self.encoder_output_actual_seqlen

        # Ä�áº£m báº£o positions Ä‘Æ°á»£c táº¡o vá»›i Ä‘Ãºng Ä‘á»™ dÃ i
    ...
    def train_step(self, batch):
        ...
        # ---- Sá»¬ Dá»¤NG self.encoder_output_actual_seqlen HOáº¶C THAM Sá»� Ä�ÃšNG ----
        ctc_input_length = tf.fill([batch_size_tf], self.encoder_output_actual_seqlen)
        # ---- Káº¾T THÃšC Sá»¬A Ä�á»”I ----
