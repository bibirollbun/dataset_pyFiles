...
class CustomDataset:
    ...
    def __getitem__(self, item):
        ...
        return {
            ...
            "targets": torch.tensor(targets, dtype=torch.long), # CHANGED dtype to torch.long for classification
        }
...
class CustomModel(nn.Module):
    ...
    def monitor_metrics(self, outputs, targets, loss):
        ...
        try:
            auc = metrics.roc_auc_score(targets_np, probs)
        except ValueError:
            auc = 0.5 
    ...
    def forward(self, image, features, targets=None):
        ...
        if targets is not None:
            # targets.view(-1, 1).float() ensures targets are (batch_size, 1) and float type
...
df = pd.read_csv("/kaggle/input/dharma-srujan-reddy-23bcs10107/train_5folds.csv")
df.head()
