...
def feature_engineer(df, te_reference_df, smoothing=10.0):
    """Applies TE: mean, std, count and smoothed_mean (regularized)."""
    ...
    for col in categorical_features:
        # compute per-category stats on the TE source
        stats = te_reference_df.groupby(col)['accident_risk'].agg(['mean', 'std', 'count'])

        # fill safe defaults in the stats table
        stats['std'] = stats['std'].fillna(0.0)   # preferred: fill std NaNs with 0.0 (less noisy)
        stats['count'] = stats['count'].fillna(0).astype(int)

        # smoothed (regularized) mean:
        ...
        # map stats to df; unseen categories -> NaN -> fill appropriately
        df[f'{col}_te_mean']   = df[col].map(stats['mean']).fillna(global_mean)
        df[f'{col}_te_std']    = df[col].map(stats['std']).fillna(0.0)
        df[f'{col}_te_count']  = df[col].map(stats['count']).fillna(0).astype(int)
        df[f'{col}_te_smooth'] = df[col].map(stats['smoothed_mean']).fillna(global_mean)
...
predictor_main = TabularPredictor.load('/kaggle/input/caraccident11/AutogluonModels/ag-20251018_220549')
