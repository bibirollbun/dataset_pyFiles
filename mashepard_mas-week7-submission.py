...
ccb_data = pd.read_csv("/kaggle/input/home-credit-default-risk/credit_card_balance.csv")
ccb_data.head()


### Retrieve AMT_BALANCE from credit_card_balance.csv join with credit_card_balance.csv

qry = ("SELECT tr.AMT_INCOME_TOTAL, tr.AMT_CREDIT, tr.AMT_ANNUITY, tr.CODE_GENDER, \
       tr.NAME_CONTRACT_TYPE, tr.DAYS_BIRTH, tr.DAYS_EMPLOYED, ccb.AMT_BALANCE, tr.TARGET \
       FROM  train_data tr \
       INNER JOIN ccb_data ccb ON tr.SK_ID_CURR = ccb.SK_ID_CURR") 
...
X = qry_result[["AMT_INCOME_TOTAL","AMT_CREDIT", "AMT_ANNUITY", "CODE_GENDER", "NAME_CONTRACT_TYPE", "DAYS_BIRTH", "DAYS_EMPLOYED", "AMT_BALANCE"]]
...
### Retrieve AMT_BALANCE from credit_card_balance.csv join with test_data

qry = ("SELECT tst.SK_ID_CURR, tst.AMT_INCOME_TOTAL, tst.AMT_CREDIT, tst.AMT_ANNUITY, tst.CODE_GENDER, \
       tst.NAME_CONTRACT_TYPE, tst.DAYS_BIRTH, tst.DAYS_EMPLOYED, ccb.AMT_BALANCE \
       FROM  test_data tst \
       INNER JOIN ccb_data ccb ON tst.SK_ID_CURR = ccb.SK_ID_CURR") 
qry_result = pysqldf(qry)
print(qry_result)
...
X_test = qry_result[["AMT_INCOME_TOTAL","AMT_CREDIT", "AMT_ANNUITY", "CODE_GENDER", "NAME_CONTRACT_TYPE", "DAYS_BIRTH", "DAYS_EMPLOYED", "AMT_BALANCE"]]
Y_test = qry_result[['SK_ID_CURR']]
