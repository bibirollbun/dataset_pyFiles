...
#Confirmation for myself to make sure everying worked
...
print(f"Class balance: {num_cats/(num_cats + num_dogs)*100:.1f}% cats, {num_dogs/(num_cats + num_dogs)*100:.1f}% dogs")
...
def display_sample_images(cat_dir, dog_dir, num_samples=8):
    ...
    # Getting random sample of images
...
print(f"Image Dimension Analysis (sample of 200 images):")
...
#Images have varying dimensios, so resizing is necessary for training the CNN model. 
#Fortunately though, the dataset is very diverse, so the model will generalize better
#to new images.
...
# For test data, rescale (no augmentation for evaluation)
test_datagen = ImageDataGenerator(rescale=1.0/255.0)




       # Reorganization of cat/dog files into folders
       import shutil
       ...
       #Subset of the data because I want to see the results a little more quickly 
       SUBSET_SIZE = 2000  

#Confirmation below for myself to ensure smooth running:
       ...
       print(f"Temporary structure created with {SUBSET_SIZE} images per class")
       ...
       def create_cnn_model(input_shape=(IMG_WIDTH, IMG_HEIGHT, 3)):
           ...
           model = keras.Sequential([
               ...
               # I am gonna add dropout for regularization (prevents overfitting)
...
#1. The convolutional 2D layers detect features, like edges, textures and patterns
#2. MaxPooling will reduce spational dimensions and computational load
#3. The filter count will double until reaching 128 images.
#4. The 50% dropout will prevent overfitting the model (thanks Alex the Analyst)
#5. The dense layers in the CNN perform the final classification for the cats and dogs model
#6. Sigmoid function will output the probability, which is that less than 50% of the images
#are cats, and more than 50% of the images are dogs.
       ...
       # Below, I decided to set up callbacks for training
       # Extra for myslelf:
#Callbacks are functions called during training to monitor and control the process
       ...
       history = model.fit(
           ...
           callbacks=callbacks,
           verbose=1  # Useful call that shows progress bar (I will start implementing)
       )
       ...
       def plot_training_history(history):
           ...
           print(f"• Final accuracy gap: {final_gap:.4f} (training - validation)")

           if final_gap > 0.1:
               print("Large gap suggests overfitting - consider more regularization")
           elif final_gap < 0.05:
               print("Good balance between training and validation performance")
           else:
               print("Moderate gap - model performance is acceptable")
...
#Confirmation for myself
       ...
       def show_predictions(model, image_paths, true_labels=None):
           ...
           for i, img_path in enumerate(image_paths):
               ...
               confidence_icon = "fire" if confidence > 0.9 else "check" if confidence > 0.7 else "danger"
       ...
       print(f"\nSample accuracy: {accuracy:.2%} ({correct}/{len(test_paths)})")

       


       # The lines below clean up temporary files
       ...
       print(f"• Input size: {IMG_WIDTH}x{IMG_HEIGHT}x3")
       print(f"• Training data: {SUBSET_SIZE*2:,} images")
