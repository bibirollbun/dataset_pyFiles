...
class EarlyStopping_Accuracy:
    ...
    def save_checkpoint(self, val_accuracy, model):
        if self.verbose:
            if self.best_score is None:
                print(f'First improvement: Validation accuracy {val_accuracy:.6f}. Saving model...')
            else:
                print(f'Validation accuracy increased ({self.best_score:.6f} --> {val_accuracy:.6f}). Saving model...')
        
        torch.save(model.state_dict(), self.path)
        self.val_accuracy_max = val_accuracy  # vẫn giữ để tương thích nếu ai dùng
        # Quan trọng: cập nhật cả best_score (dù đã làm ở __call__ rồi nhưng an toàn thì cập nhật luôn)
...
def train_loop(train_data: Subset, val_data: Subset, model: nn.Module, loss_fn: nn, optimizer: torch.optim, scheduler: torch.optim.lr_scheduler, name: str, patience=10, epochs=10, batch_size=32, warmup_epochs=0, warmup_lr=1e-5):
    ...
    early_stopping = EarlyStopping_Accuracy(patience=patience, verbose=True, path='/kaggle/working/models/best_accuracy_model.pt')
    epoch_number = 0
    ...
    for epoch in range(epochs):
        ...
        if avg_vloss < best_vloss:
            ...
            model_path = '/kaggle/working/models/best_loss_model.pt'
    ...
    data = {
        ...
        "model_path": '/kaggle/working/models/best_accuracy_model.pt'
    }
    return data
...
def plot_pr_curve(y_true, probabilities, writer, class_names=None):
    ...
    for i in range(num_classes):
        ...
        # Convert labels to binary format (1 for a given class, 0 for the rest)
        y_true_binary = (y_true == i).astype(np.int32)
        
        # Get probabilities for a given class
        ...
        # Save the graph to a buffer and add it to TensorBoard
        buf = io.BytesIO()
        ...
        # Also add raw data to the interactive chart
        writer.add_pr_curve(
...
results = train_loop(
    ...
    train_data=train_data,  # Dữ liệu huấn luyện
    val_data=val_data,      # Dữ liệu validation để theo dõi overfitting
    loss_fn=loss_fn,        # Hàm loss dùng cho training
    optimizer=optimizer,    # Optimizer (Adam, SGD, v.v.)
    patience=15,            # Early stopping: dừng nếu không cải thiện sau 15 epoch
    epochs=100,              # Tổng số epoch tối đa
    batch_size=64,          # Batch size lớn hơn giúp train ổn định hơn
    scheduler=scheduler,    # Learning rate scheduler
    warmup_epochs=15,       # Số epoch làm warm-up LR
    warmup_lr=1e-4,         # Learning rate trong giai đoạn warm-up
    name=name               # Tên mô hình / cấu hình để lưu log, checkpoint
)

#(Đánh giá cuối cùng trên tập validation)
print("\nEwaluacja końcowa modelu:")

evaluation = evaluate_model(model, val_data)  # Chạy hàm evaluate để tính metrics
'''
...
BEST_ACC = f"{MODEL_DIR}/best_accuracy_model.pt"
BEST_LOSS = f"{MODEL_DIR}/best_loss_model.pt"

if os.path.exists(BEST_ACC):
    print("Loading best_accuracy_model.pt ...")
    state = torch.load(BEST_ACC, map_location="cpu")
    model.load_state_dict(state)
elif os.path.exists(BEST_LOSS):
    print("best_accuracy_model.pt not found, loading best_loss_model.pt ...")
    state = torch.load(BEST_LOSS, map_location="cpu")
    model.load_state_dict(state)
else:
    print("Không tìm thấy model trong /kaggle/working/models/")
