...
tfidf = TfidfVectorizer(ngram_range=(1, 4), analyzer="char", max_df=0.95, min_df=2, stop_words="english")
...
test_misc = np.argsort(-model_cat.predict_proba(test_embeddings), 1)[:, :3]
test_misc


cv_pred_misc = model_misc.predict_proba(X_test_misc)
...
predict = []
cv_pred_cat = model_cat.predict_proba(train_embeddings)
cv_pred_misc = model_misc.predict_proba(train_embeddings)
