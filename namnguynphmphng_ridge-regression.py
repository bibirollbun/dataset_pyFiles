...
Q1 = np.log1p(train_df["total_orders"]).quantile(0.25)
Q3 = np.log1p(train_df["total_orders"]).quantile(0.75)
IQR = Q3 - Q1
lower = Q1 - 1.5 * IQR
upper = Q3 + 1.5 * IQR

train_df = train_df[(np.log1p(train_df["total_orders"]) >= lower) & (np.log1p(train_df["total_orders"]) <= upper)]


plt.figure(figsize=(6, 5))

sns.histplot(x=np.log1p(train_df['total_orders']), bins=100, kde=True)

plt.title(f'Log histplot of total_orders')
...
# Danh sách các cột discount
discount_columns = [f"type_{i}_discount" for i in range(7)]

# Lọc các hàng có giá trị âm trong bất kỳ cột discount nào
...
if not negative_discounts.empty:
    print("Các hàng có giá trị âm trong các cột discount:")
    print(negative_discounts[["unique_id", "date"] + discount_columns])
    
    # Đếm số lượng giá trị âm cho mỗi cột discount
    print("\nSố lượng giá trị âm trong mỗi cột discount:")
    for col in discount_columns:
        num_negative = len(train_df[train_df[col] < 0])
        print(f"{col}: {num_negative}")
else:
    print("Không có giá trị âm nào trong các cột discount.")
...
# code nay de chung to rang L1_category_name_en chua ten cua mat hang duoi dang tong quat 
...
# Tạo pipeline với SimpleImputer, StandardScaler và Ridge
ridge_pipe = Pipeline([
    ('imputer', SimpleImputer(strategy='mean')),  # Điền NaN bằng giá trị trung bình
    ('scaler', StandardScaler()),
    ('ridge', Ridge(random_state=43))  # Thêm random_state để tái lập kết quả
])
...
try:
    ridge_gs.fit(X_train, y_train, ridge__sample_weight=unique_id_train.map(weight_map).values)
except Exception as e:
...
with open('best_model_ridge_gs.pkl', 'wb') as f:
    pickle.dump(best_model, f)

# Tải lại mô hình để kiểm tra
try:
    with open('best_model_ridge_gs.pkl', 'rb') as f:
...
y_pred_val = inverse_norm(train_df, val_period, loaded_model.predict(X_val), scalers)
...
try:
    ...
    y_pred_test = inverse_norm(test_df, test_period, loaded_model.predict(X_test_new), scalers)
