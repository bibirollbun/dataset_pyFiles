...
# from sklearn.linear_model import Ridge # LightGBM単一モデルのため削除
import os
...
test_preds_casual_final = test_preds_casual_lgbm
test_preds_registered_final = test_preds_registered_lgbm

# 逆変換と合計 
predictions_casual = np.expm1(test_preds_casual_final)
predictions_registered = np.expm1(test_preds_registered_final)
...
submission.to_csv('submission_lgbm_only_kfold.csv', index=False)

print("\n✅ LightGBMのみの単一モデル (K-Foldアンサンブル) による予測完了。")
print("ファイル名: submission_lgbm_only_kfold.csv として保存されました。")
