    ...
    acc_df = (
        df.with_columns(
            pl.col('text').str.extract_all(
                r'(?i)\b(?:CHEMBL\d+|E-GEOD-\d+|E-PROT-\d+|EMPIAR-\d+|ENSBTAG\d+|ENSOARG\d+|EPI_ISL_\d{5,}|EPI\d{6,7}|HPA\d+|CP\d{6}|IPR\d{6}|PF\d{5}|KX\d{6}|K0\d{4}|PRJNA\d+|PXD\d+|SAMN\d+|dryad\s*\.\s*[^\s"<>]+|pasta\s*/\s*[^\s"<>])'
            ).alias('match')
        )
        .explode('match')
        .drop_nulls('match')
