...
df_full['credit_risk'] = [credit_risk(i) for i in df_full['grade_subgrade'].values]
df_full['education_Other'] = [1 if i == 'Other' else 0 for i in df_full['education_level']]
df_full['education_ord'] = [education_level_ord(i) for i in df_full['education_level'].values]
...
def score_model(X, y, model=None, print_score=True, return_model=False):
    ...
    cv = 5
    cv_score = cross_val_score(_model, Xt, yt, scoring=roc_auc_scorer, cv=cv, fit_params={
        'eval_set': [(Xv, yv)],
        'verbose': run_debug
    })
...
score, model = score_model(X_train, y_train, model=xgb_model, return_model=True)
