...
train0=pd.read_csv('/kaggle/input/whale-categorization-playground/train.csv')
print(train0.columns.tolist())
print(len(train0))


id_counts = train0['Id'].value_counts()
valid_ids = id_counts[id_counts >= 20].index
train = train0[train0['Id'].isin(valid_ids)]
train = train[train['Id']!='new_whale'].reset_index(drop=True)
print(len(train))


file2id=train.set_index("Image")["Id"].to_dict()
unique_ids=sorted(train['Id'].unique().tolist())
...
dir0='/kaggle/input/whale-categorization-playground/train'
paths=[]
ids=[]
for dirname, _, filenames in os.walk(dir0):
    for filename in filenames:
        path=os.path.join(dirname, filename)
        Id=(train0[train0['Image']==filename]).iloc[0,1]
        if Id in unique_ids:
            paths+=[path]
            ids+=[Id]
print(paths[0:3])
