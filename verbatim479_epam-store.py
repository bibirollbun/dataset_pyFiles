...
counts = items["family"].value_counts()
...
# we observe low correlation between features
...
mse(dummy_prediction, test_df['unit_sales'].values)
...
holiday_type_ = {"Transfer": 2, "Holiday": 2, "Additional": 2, "Event": 1, "Not holiday": 0}

df['holiday_type_ec'] = df['holiday_type'].map(holiday_type_)
test_df['holiday_type_ec'] = test_df['holiday_type'].map(holiday_type_)
...
preprocessor = ColumnTransformer(
    [
        ...
        ('one_hot', OneHotEncoder(handle_unknown='ignore'), ['family', 'city', 'state', 'store_type', 'cluster', 'holiday_type', 'locale']),
...
X_train = df[[
    'onpromotion', 'is_holiday', 'perishable', 
    'dcoilwtico', 'holiday_type', 'locale',
...
lgbm_score = mse(y_pred=lgbm_prediction, y_true=y_test)
lgbm_score


# well, it's better than for dummyregressor
