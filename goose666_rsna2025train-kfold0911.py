...
class Config:
    ...
    SERIES_MAPPING_PATH = '/kaggle/input/d/xiaoyufish666/series-index-mapping/series_index_mapping (2).csv'  # 序列映射文件
    LOCALIZERS_PATH = '/kaggle/input/d/xiaoyufish666/train-localizers-with-relative/train_localizers_with_relative (2).csv'  # 坐标标注文件
    ...
    PREPROCESSED_DATA_DIR = "/kaggle/working/preprocessed_data"  # 预处理数据保存目录
    KFOLD_SPLITS_DIR = "/kaggle/working/kfold_splits"  # K折划分保存目录
...
os.makedirs(config.KFOLD_SPLITS_DIR, exist_ok=True)
...
print(f"预处理数据保存目录: {config.PREPROCESSED_DATA_DIR}")
print(f"K折划分保存目录: {config.KFOLD_SPLITS_DIR}")
...
# 保存8帧路径映射
frame_paths_file = os.path.join(config.PREPROCESSED_DATA_DIR, "frame_paths_8frame.pkl")
with open(frame_paths_file, 'wb') as f:
    pickle.dump(frame_paths_dict, f)
print(f"已保存8帧路径映射到: {frame_paths_file}")
...
def create_robust_cv_split(train_df, n_splits=5):
    ...
    if n_groups < n_splits:
        ...
        splits = list(skf.split(train_df, train_df['Aneurysm Present']))
    else:
        # 创建分层键（模态+动脉瘤存在）
    ...
    # 保存K折划分
    kfold_splits_file = os.path.join(config.KFOLD_SPLITS_DIR, "kfold_splits.pkl")
    with open(kfold_splits_file, 'wb') as f:
        pickle.dump({
            'splits': splits,
            'patient_groups': patient_groups,
            'train_df_shape': train_df.shape
        }, f)
    print(f"已保存K折划分到: {kfold_splits_file}")
...
# 保存训练数据
train_data_file = os.path.join(config.PREPROCESSED_DATA_DIR, "train_df_filtered.pkl")
with open(train_data_file, 'wb') as f:
    pickle.dump(train_df_filtered, f)
print(f"已保存过滤后的训练数据到: {train_data_file}")
...
def train_all_folds():
    ...
    with open(frame_paths_file, 'rb') as f:
        frame_paths_dict = pickle.load(f)
    with open(train_data_file, 'rb') as f:
        train_df_filtered = pickle.load(f)
    with open(kfold_splits_file, 'rb') as f:
        kfold_data = pickle.load(f)
        cv_splits = kfold_data['splits']
    
    print(f"加载了{len(cv_splits)}折数据")
    
    # 为每个折创建结果目录
    fold_results_dir = os.path.join(config.OUTPUT_DIR, "fold_results")
    ...
    for fold_idx in range(config.NUM_FOLDS):
        ...
        for epoch in range(config.NUM_EPOCHS):
            ...
            if val_score > best_score:
                ...
                # 保存最佳模型
                model_path = os.path.join(fold_results_dir, f"{config.MODEL_NAME}_fold{fold_idx}_best.pth")
                torch.save({
                    'epoch': epoch,
        ...
        # 保存当前折的结果
        result_path = os.path.join(fold_results_dir, f"fold_{fold_idx}_results.pkl")
        with open(result_path, 'wb') as f:
            pickle.dump(fold_result, f)
        
        print(f"\nFold {fold_idx} 完成 - 最佳分数: {best_score:.6f}")
    ...
    # 保存所有折的结果
    all_results_path = os.path.join(config.OUTPUT_DIR, "all_fold_results.pkl")
    with open(all_results_path, 'wb') as f:
        pickle.dump(all_fold_results, f)
    
    print(f"\n所有折训练完成！结果已保存到: {all_results_path}")
    
    # 打印汇总结果
    print("\n=== 各折性能汇总 ===")
    for i, result in enumerate(all_fold_results):
        print(f"Fold {i}: 最佳分数 = {result['best_score']:.6f} (epoch {result['best_epoch']})")
    
    avg_score = np.mean([r['best_score'] for r in all_fold_results])
    print(f"平均分数: {avg_score:.6f}")
...
# 可视化工具函数
# ======================
def plot_training_history(all_fold_results):
    """绘制训练历史"""
    fig, axes = plt.subplots(2, 2, figsize=(15, 10))
    fig.suptitle('Training History - All Folds', fontsize=16)
    
    # 训练损失
    for i, result in enumerate(all_fold_results):
        axes[0, 0].plot(result['train_losses'], label=f'Fold {i}')
    ...
    # 验证损失
    for i, result in enumerate(all_fold_results):
        axes[0, 1].plot(result['val_losses'], label=f'Fold {i}')
    ...
    # 验证分数
    for i, result in enumerate(all_fold_results):
        axes[1, 0].plot(result['val_scores'], label=f'Fold {i}')
    axes[1, 0].set_title('Validation Score')
    ...
    # 各折最佳分数
    best_scores = [result['best_score'] for result in all_fold_results]
    axes[1, 1].bar(range(len(best_scores)), best_scores)
    axes[1, 1].set_title('Best Scores per Fold')
    axes[1, 1].set_xlabel('Fold')
    axes[1, 1].set_ylabel('Best Score')
    for i, score in enumerate(best_scores):
        axes[1, 1].text(i, score + 0.001, f'{score:.4f}', ha='center')
...
if __name__ == "__main__":
    try:
        # 开始训练
        print("开始8帧优化的动脉瘤检测模型训练...")
        
        # 训练所有折
        all_fold_results = train_all_folds()
        
        # 绘制训练历史
        plot_training_history(all_fold_results)
        ...
        for file in os.listdir(config.OUTPUT_DIR):
            if file.endswith(('.pth', '.pkl', '.csv', '.png')):
                print(f"  - {file}")
        ...
        # 尝试保存错误信息
        try:
            error_log = os.path.join(config.OUTPUT_DIR, "error_log.txt")
...
!zip -r fold_results.zip /kaggle/working/fold_results
