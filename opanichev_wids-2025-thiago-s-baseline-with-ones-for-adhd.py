...
def get_feats(mode='TRAIN'):
    if mode=='TRAIN':
        feats=pd.read_excel(f"/kaggle/input/widsdatathon2025/{mode}_NEW/{mode}_QUANTITATIVE_METADATA_new.xlsx")
        cate=pd.read_excel(f"/kaggle/input/widsdatathon2025/{mode}_NEW/{mode}_CATEGORICAL_METADATA_new.xlsx")
        # func=pd.read_csv(f"/kaggle/input/widsdatathon2025/{mode}_NEW/{mode}_FUNCTIONAL_CONNECTOME_MATRICES.csv")
