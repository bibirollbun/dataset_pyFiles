...
## Getting Glimps of the training data
train.head()
...
## Cheack datatype and missing data
...
plt.title('Box plot of accident risk',fontsize=16)
plt.show()


## Identifying the categorical variables
categorical_feature=[col for col in train.columns if train[col].dtype=='object']
boolean_feature=[col for col in train.columns if train[col].dtype=='bool']
numeric_feature=[col for col in train.columns if (train[col].dtype!='bool' and 
train[col].dtype!='object') and col!='accident_risk' and col!='id' ]

print(f"Categorical feature {categorical_feature}")
print(f"Boolean feature {boolean_feature}")
...
### Average accident risk by category
    ...
    ## PRint unique values
    print(f'\nunique value of {cat_feature} : {train[cat_feature].nunique()}')
...
for bool_feature in boolean_feature:
    plt.figure(figsize=(12, 6))
...
## Analyse the numeric variable 
for num_feature in numeric_feature:
    plt.figure(figsize=(12,6))
    ##distribution of the numerical feature
    plt.subplot(1,2,1)
    sns.histplot(train[num_feature],kde=True)
    plt.title(f'Distribution of the {num_feature}')
...
## print summary Satastics
    print(f"\n {num_feature} summary statistics :")
    print(train[num_feature].describe())

    ## Calculate correlation with target
    corr=train[[num_feature,'accident_risk']].corr().iloc[0,1]
    print(f"Correalation with accident_risk : {corr:.4f}")


## Create a correlation for the heatmap for numeric feature
numeric_col=numeric_feature+['accident_risk']
plt.figure(figsize=(12,10))
correlation_metrix=train[numeric_col].corr()
sns.heatmap(correlation_metrix,annot=True,cmap='coolwarm',fmt='.2f')
plt.title('correlation metrix of numeric features', fontsize=16)
plt.tight_layout()
plt.show()
...
## Check feature distribution of the train and test sets
for cat_feature in categorical_feature:
    plt.figure(figsize=(12,6))
    plt.subplot(1,2,1)
...
for num_feature in numeric_feature:
    ...
    plt.subplot(1,2,1)
    sns.histplot(train[num_feature], kde=True)
    plt.title(f'{num_feature} Distribtion in train')
...
## keeep  track  of train and test IDs
train_idx=train['id']
test_idx=test['id']


## one hot encode categorical features
all_data_encoded=pd.get_dummies(all_data,columns=categorical_feature,drop_first=False)
print(f'Shape after one-hot encoding : {all_data_encoded.shape}')


## Create an interaction feature
## Interaction between the speed and curvature
all_data_encoded['speed_curvature']=all_data_encoded['speed_limit']*all_data_encoded['curvature']

##interaction between the speed limit and lanes
