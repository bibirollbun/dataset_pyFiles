...
for model_name in MODEL_NAMES:
    ...
    pipeline.model.lr_refit = 0.0006
    pipelines[model_name] = pipeline
    print(model_name)
    print(pipeline.model.get_params())
    print(f"Number of features: {len(pipeline.features)}")
    print(f"Number of auxiliary targets: {pipeline.model.model.num_resp}")
    ...
    if DEBUG:
        ...
        if time_id % 100 == 0:
            ...
            print(
                ...
                f"{time.time()-start_time:.4f}s/it, # nans {n_nans}"
            )
    else:
