...
for fold, (train_idx, val_idx) in enumerate(kfold.split(X_scaled, y)):
    ...
    models.append(rf_model)  # Store the model
    
    # Validation
    y_val_pred = rf_model.predict(X_val)
    fold_score = competition_metric(y_val, y_val_pred, le, all_original_gestures_in_train)
...
print(f"Mean Score: {cv_mean:.4f} (+/- {cv_std * 2:.4f})")
...
def predict(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    predicted_label_id = final_model.predict(X_inference_scaled)[0]
