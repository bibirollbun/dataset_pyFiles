...
class Model(nn.Module):
    def __init__(self, input_shape: ty.Tuple[int, int, int] = (3, 224, 224), n_classes: int = 1):
        """
...
model = Model(input_shape=(3, 320, 320), n_classes=2).to(device)
...
def loss_fn(y_true, y_preds, alpha: float = 0.5):
    ...
    cls_loss = F.binary_cross_entropy(cls_y_pred, cls_y_true)
...
train_ds = maskDataset(
    ...
    transform=train_transforms, output_size=(64, 64)
)  # ,output_size=(255,255)

val_ds = maskDataset(
    val_df, root_dir=train_root_dir,
    transform=eval_transforms, output_size=(64, 64)
)  # ,output_size=(255,255)
