...
fig = px.histogram(train_data, x='IsActiveMember',color="Exited",barmode='group', barnorm="percent",text_auto=True,title="Gráfico em barra agrupada para IsActiveMember")
fig.show()
...
df=train_data.drop(['id', 'CustomerId', 'Surname'], axis=1)



df_test=test_data.drop(['id', 'CustomerId', 'Surname'], axis=1)
...
correlation_matrix = df[['Age','CreditScore', 'Balance','EstimatedSalary','IsActiveMember','Tenure','NumOfProducts', "HasCrCard" , 'Exited']].corr()
...
def find_optimal_parameter_RF(X_train,y_train):
...
train_auc_mean_RF,cv_auc_mean_RF,params_RF=find_optimal_parameter_RF(X_train,y_train)


# Logistic regression for test and its performance
def model_test_performance_RF(X_train,y_train,X_test,y_test,hyper_params):# instantiate learning model k = optimal_k
...
print(params_RF)
auc_RF,clf_RF=model_test_performance_RF(X_train,y_train,X_test,y_test,params_RF)
...
train_auc_mean_GBDT,cv_auc_mean_GBDT,params_GBDT=find_optimal_parameter_GBDT(X_train,y_train)
...
# train a classifier and prediction for test for BOW
print(params_GBDT)
auc_GBDT,clf_GBDT=model_test_performance_GBDT(X_train,y_train,X_test,y_test,params_GBDT)
