...
# Get track IDs in both train and test
train_ids = set(train['track_identifier'])
test_ids = set(test['track_identifier'])
leak_ids = train_ids & test_ids

print(f"Number of overlapping track IDs: {len(leak_ids)}")


pd.set_option('display.max_columns', None)


train[train['track_identifier'] == "Where's My Love"]


test[test['track_identifier'] == "Where's My Love"]


dup_both = train[train.duplicated(subset=['track_identifier', 'composition_label_0', 'composition_label_1', 'composition_label_2' ,'target'], keep=False)]
dup_both_sorted = dup_both.sort_values(by=['track_identifier', 'composition_label_0', 'composition_label_1', 'composition_label_2' ,'target'])
dup_both_sorted


train = (
    train
    .groupby(['track_identifier', 'target'], group_keys=False)
...
# test_nw = (
#     test
#     .groupby(['track_identifier', 'composition_label_0', 'composition_label_1', 'composition_label_2'], group_keys=False)
#     .apply(lambda group: group.ffill().bfill())
# )


test_filled = test.fillna({
    'track_identifier': 'missing_track',
    'composition_label_0': 'missing_0',
    'composition_label_1': 'missing_1',
    'composition_label_2': 'missing_2'
})
...
# # Step 0: Add a temporary index column to preserve row order
# test = test.copy()
# test['row_id'] = test.index

# # Step 1: Split the DataFrame
# non_null_group_rows = test[test['track_identifier'].notna() & test['composition_label_0'].notna()]
# null_group_rows = test[~(test['track_identifier'].notna() & test['composition_label_0'].notna())]

# # Step 2: Group and fill within valid groups only
# grouped = non_null_group_rows.groupby(['track_identifier', 'composition_label_0'])
# filled_group_rows = grouped.apply(lambda group: group.ffill().bfill())

# # Step 3: Remove multiindex created by groupby-apply
# filled_group_rows.reset_index(drop=True, inplace=True)

# # Step 4: Concatenate and sort based on original row order
# final_filled = pd.concat([filled_group_rows, null_group_rows], ignore_index=True)
# final_filled = final_filled.sort_values('row_id').drop(columns='row_id').reset_index(drop=True)


# test_nw = final_filled.copy()
# test_nw
...
# Assume X_train is your training set and X_test is your test set

# 1. Compute correlation on training set only
corr_matrix = X.corr().abs()
upper = corr_matrix.where(np.triu(np.ones(corr_matrix.shape), k=1).astype(bool))

# 2. Identify features to drop based on high correlation
threshold = 0.6
...
!kaggle competitions submit -c mlx-2-0-regression -f submission.csv -m "Meta"


!kaggle competitions submit -c mlx-2-0-regression -f lgb_submission.csv -m "lgb"


!kaggle competitions submit -c mlx-2-0-regression -f xgb_submission.csv -m "xgb"


!kaggle competitions submit -c mlx-2-0-regression -f cat_submission.csv -m "cat"


!kaggle competitions submit -c mlx-2-0-regression -f et_submission.csv -m "et"
