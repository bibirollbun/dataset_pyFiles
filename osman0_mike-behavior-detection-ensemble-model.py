...
# Suppress specific warnings that may appear with GPU usage
...
# --- SEED EVERYTHING -----
SEED = 1234
os.environ["PYTHONHASHSEED"] = str(SEED)  # set very early
    ...
    def fit(self, X, y):
        ...
        if self.n_samples is None or len(Xn) <= int(self.n_samples):
            try:
                ...
                # If fit fails, try with more conservative parameters
                # This is a fallback - the exception will be caught by caller
                raise e
        else:
...
# Optimized action-specific thresholds for better F1 scores
action_thresholds = defaultdict(lambda: 0.25)  # Lower default for better recall
# Action-specific thresholds can be tuned per action if needed

def predict_multiclass_adaptive(pred, meta, action_thresholds):
    """Adaptive thresholding per action + enhanced temporal smoothing."""
    # Enhanced temporal smoothing with multiple window sizes
    pred_smoothed = pred.rolling(window=7, min_periods=1, center=True).mean()
    # Additional smoothing for stability
    pred_smoothed = pred_smoothed.rolling(window=3, min_periods=1, center=True).mean()
    ...
    # Filter very short events (slightly more lenient for better recall)
...
# ==================== ADVANCED FEATURE ENGINEERING (FPS-AWARE, SHORTER WINDOWS) ====================
...
def add_curvature_features(X, center_x, center_y, fps):
    """Trajectory curvature (shorter windows)."""
    vel_x = center_x.diff()
    vel_y = center_y.diff()
    acc_x = vel_x.diff()
    acc_y = vel_y.diff()
    ...
    for w in [25, 50, 75]:  # shorter than original [30, 60]
        ws = _scale(w, fps)
...
def add_multiscale_features(X, center_x, center_y, fps):
    """Multi-scale temporal features (speed in cm/s; shorter scales)."""
    speed = np.sqrt(center_x.diff()**2 + center_y.diff()**2) * float(fps)

    scales = [20, 40, 60, 80]  # was [10, 40, 160]
    for scale in scales:
        ws = _scale(scale, fps)
        if len(speed) >= ws:
...
def add_state_features(X, center_x, center_y, fps):
    """Behavioral state transitions; fps-invariant bins; shorter windows."""
    speed = np.sqrt(center_x.diff()**2 + center_y.diff()**2) * float(fps)  # cm/s
    w_ma = _scale(15, fps)
    speed_ma = speed.rolling(w_ma, min_periods=max(1, w_ma // 3)).mean()

    try:
        # Bins in cm/s to keep semantics across fps:
        bins = [-np.inf, 0.5 * fps, 2.0 * fps, 5.0 * fps, np.inf]
        speed_states = pd.cut(speed_ma, bins=bins, labels=[0, 1, 2, 3]).astype(float)

        for window in [20, 40, 60, 80]:  # was [60, 120]
...
def add_longrange_features(X, center_x, center_y, fps):
    """Long-range temporal features (shorter spans)."""
    for window in [30, 60, 120]:  # was [120, 240]
        ws = _scale(window, fps)
        if len(center_x) >= ws:
    ...
    # EWM spans
    for span in [30, 60, 120]:   # was [60, 120]
        s = _scale(span, fps)
        X[f'x_e{span}'] = center_x.ewm(span=s, min_periods=1).mean()
        X[f'y_e{span}'] = center_y.ewm(span=s, min_periods=1).mean()

    speed = np.sqrt(center_x.diff()**2 + center_y.diff()**2) * float(fps)  # cm/s
    for window in [30, 60, 120]:  # was [60, 120]
        ws = _scale(window, fps)
        if len(speed) >= ws:
...
def add_interaction_features(X, mouse_pair, avail_A, avail_B, fps):
    """Social interaction features (unchanged core windows)."""
...
def transform_single(single_mouse, body_parts_tracked, fps):
    """Single mouse transform (shorter windows where relevant; distances in cm)."""
    ...
    # Core temporal features (unchanged set; windows scaled by fps)
    ...
    # Ear features with duration-aware offsets (expanded offsets)
    if all(p in available_body_parts for p in ['ear_left', 'ear_right']):
        ear_d = np.sqrt((single_mouse['ear_left']['x'] - single_mouse['ear_right']['x'])**2 +
                        (single_mouse['ear_left']['y'] - single_mouse['ear_right']['y'])**2)
        for off in [-30, -20, -10, 10, 20, 30]:  # expanded from [-20, -10, 10, 20]
...
def transform_pair(mouse_pair, body_parts_tracked, fps):
    """Pair transform (shorter windows where relevant; distances in cm)."""
    ...
    # Temporal interaction features (keep core windows)
    if 'body_center' in avail_A and 'body_center' in avail_B:
    ...
    # Velocity alignment (expanded offsets)
    if 'body_center' in avail_A and 'body_center' in avail_B:
        ...
        for off in [-30, -20, -10, 0, 10, 20, 30]:  # expanded from [-20, -10, 0, 10, 20]
            o = _scale_signed(off, fps)
        ...
        # Advanced interaction (reuse helper)
        X = add_interaction_features(X, mouse_pair, avail_A, avail_B, fps)

    return X.astype(np.float32, copy=False)


# ==================== ENSEMBLE TRAINING (fixed sample sizes, as original) ====================

def submit_ensemble(body_parts_tracked_str, switch_tr, X_tr, label, meta):
    models = []

    # LightGBM models - use CPU for stability (GPU can have issues with imbalanced data)
    # LightGBM GPU sometimes fails with imbalanced classes, so we use CPU for reliability
    lgbm_device = 'cpu'  # Force CPU for stability with imbalanced data
    
    # Base LightGBM parameters with safety settings for imbalanced data
    lgbm_base_params = {
        'device': lgbm_device,
        'verbose': -1,
        'min_data_in_leaf': 5,  # Minimum samples in leaf for stability
        'min_gain_to_split': 0.0,  # Allow splits even with minimal gain
        'min_sum_hessian_in_leaf': 1e-3,  # Very small minimum hessian
    }
    
    # LightGBM models with conservative parameters for imbalanced data
    models.append(make_pipeline(
        StratifiedSubsetClassifier(
            lightgbm.LGBMClassifier(
                n_estimators=300, learning_rate=0.06, min_child_samples=5,
                num_leaves=31, max_depth=6, subsample=0.8, colsample_bytree=0.8,  # Reduced max_depth
    ...
    models.append(make_pipeline(
        StratifiedSubsetClassifier(
            lightgbm.LGBMClassifier(
                n_estimators=250, learning_rate=0.08, min_child_samples=5,
                num_leaves=31, max_depth=6, subsample=0.7, colsample_bytree=0.9,  # Reduced num_leaves and max_depth
                reg_alpha=0.1, reg_lambda=0.1,
                **lgbm_base_params,
                random_state=SEED+1, bagging_seed=SEED+1, feature_fraction_seed=SEED+1, data_random_seed=SEED+1
            ), 550_000,
        )
    ))
    models.append(make_pipeline(
        StratifiedSubsetClassifier(
            lightgbm.LGBMClassifier(
                n_estimators=200, learning_rate=0.05, min_child_samples=5,
                num_leaves=63, max_depth=7, subsample=0.75,  # More conservative
                reg_alpha=0.05, reg_lambda=0.05,
                **lgbm_base_params,
                random_state=SEED+2, bagging_seed=SEED+2, feature_fraction_seed=SEED+2, data_random_seed=SEED+2
            ), 500_000,
        )
    ))
    # Additional LightGBM model with different hyperparameters
    models.append(make_pipeline(
        StratifiedSubsetClassifier(
            lightgbm.LGBMClassifier(
                n_estimators=280, learning_rate=0.07, min_child_samples=5,
                num_leaves=47, max_depth=7, subsample=0.75, colsample_bytree=0.85,  # More conservative
                reg_alpha=0.02, reg_lambda=0.02,
                **lgbm_base_params,
                random_state=SEED+3, bagging_seed=SEED+3, feature_fraction_seed=SEED+3, data_random_seed=SEED+3
            ), 580_000,
        )
    ))
    ...
    if XGBOOST_AVAILABLE:
        ...
        xgb_params = {
            'n_estimators': 250, 'learning_rate': 0.07, 'max_depth': 7,
            'min_child_weight': 1, 'subsample': 0.8, 'colsample_bytree': 0.8,  # Reduced from 5
            'reg_alpha': 0.1, 'reg_lambda': 1.0,
            'tree_method': xgb_tree_method, 'verbosity': 0,
            'random_state': SEED
        }
        if USE_GPU and xgb_tree_method == 'gpu_hist':
            xgb_params['gpu_id'] = 0
        models.append(make_pipeline(
            StratifiedSubsetClassifier(
                XGBClassifier(**xgb_params), 600_000,
            )
        ))
        # Second XGBoost model with different hyperparameters
        xgb_params2 = {
            'n_estimators': 220, 'learning_rate': 0.08, 'max_depth': 6,
            'min_child_weight': 1, 'subsample': 0.75, 'colsample_bytree': 0.85,  # Reduced from 4
            'reg_alpha': 0.05, 'reg_lambda': 0.5,
            'tree_method': xgb_tree_method, 'verbosity': 0,
            'random_state': SEED+10
        }
        if USE_GPU and NUM_GPUS > 1 and xgb_tree_method == 'gpu_hist':
            xgb_params2['gpu_id'] = 1
        models.append(make_pipeline(
            StratifiedSubsetClassifier(
                XGBClassifier(**xgb_params2), 550_000,
            )
        ))
    ...
    if CATBOOST_AVAILABLE:
        ...
        catboost_params = {
            'iterations': 200, 'learning_rate': 0.08, 'depth': 7,
        ...
        if USE_GPU and NUM_GPUS > 1:
            catboost_params['devices'] = '0:1'
        models.append(make_pipeline(
            StratifiedSubsetClassifier(
                CatBoostClassifier(**catboost_params), 600_000,
            )
        ))
        # Additional CatBoost model with different hyperparameters
        catboost_params2 = {
            'iterations': 180, 'learning_rate': 0.1, 'depth': 6,
        ...
        if USE_GPU and NUM_GPUS > 1:
            catboost_params2['devices'] = '0:1'
        models.append(make_pipeline(
            StratifiedSubsetClassifier(
                CatBoostClassifier(**catboost_params2), 550_000,
            )
        ))
    ...
    for action in label.columns:
        ...
        # Check for sufficient positive samples and class balance
        n_positives = np.sum(y_action)
        n_total = len(y_action)
        # Require at least 10 positive samples and ensure class balance
        if not (y_action == 0).all() and n_positives >= 10 and n_positives < n_total * 0.99:
            ...
            for i, m in enumerate(models):
                try:
                    ...
                    # Skip failed models and continue with others
                    # LightGBM may fail on some imbalanced actions, but XGBoost/CatBoost should work
                    if verbose:
                        error_msg = str(e)[:50]
                        print(f"    Model {i} failed for action {action}: {error_msg}")
                    continue
            if len(trained) > 0:  # Only add if at least one model trained successfully
                model_list.append((action, trained))
            elif verbose:
                print(f"    No models trained successfully for action {action}")
        ...
        try:
            ...
            for action, trained in model_list:
                if action in actions_te:
                    ...
                    # Weighted ensemble: give more weight to later models (often better)
                    n_models = len(probs)
                    if n_models > 1:
                        # Simple linear weighting: later models get slightly more weight
                        weights = np.linspace(0.9, 1.1, n_models)
