...
def predict2(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    with torch.no_grad():
        ...
        probabilities = F.softmax(logits, dim=1).float().cpu().numpy()
...
def predict3(sequence: pl.DataFrame, demographics: pl.DataFrame) -> str:
    ...
    # w1,w2,w3,w4 = enumerate_weights()

    # fpps_1 = (
    #     w1['A']* copy.deepcopy(avg_pred_A)+ 
    #     w1['B']* copy.deepcopy(pred_B)    + 
    #     w1['C']* copy.deepcopy(avg_pred_C))

    # fpps_2 = (
    #     w2['A']* copy.deepcopy(avg_pred_A)+ 
    #     w2['B']* copy.deepcopy(pred_B)    + 
    #     w2['C']* copy.deepcopy(avg_pred_C))
                                               #  LB = 0.854
    # fpps_3 = (
    #     w3['A']* copy.deepcopy(avg_pred_A)+ 
    #     w3['B']* copy.deepcopy(pred_B)    + 
    #     w3['C']* copy.deepcopy(avg_pred_C))

    # fpps_4 = (
    #     w4['A']* copy.deepcopy(avg_pred_A)+ 
    #     w4['B']* copy.deepcopy(pred_B)    + 
    #     w4['C']* copy.deepcopy(avg_pred_C))
    
    # return [fpps_1,fpps_2,fpps_3,fpps_4]


    w1,w2,w3,w4 = enumerate_weights()

    fpps_1 = (                                 #
        w1['A']* copy.deepcopy(avg_pred_A)+    #  these values will not be considered
        w1['B']* copy.deepcopy(pred_B)    +    #  in the final enumeration function ( v.3 )
        w1['C']* copy.deepcopy(avg_pred_C))    #
    ...
    fpps_4 = (                                 #
        w4['A']* copy.deepcopy(avg_pred_A)+    #  these values will not be considered
        w4['B']* copy.deepcopy(pred_B)    +    #  in the final enumeration function ( v.3 )
        w4['C']* copy.deepcopy(avg_pred_C))    #
...
def predict(sequence, demographics):
    ...
    # m_w,da_w,c_w = [0.271, 0.347, 0.382], [0.70, 0.30], [+0.0021,-0.00055,-0.00155]
    # avg_pred_1 = h_blend(pred0,pred1,preds3[0][0], m_w,da_w,c_w)
    # avg_pred_2 = h_blend(pred0,pred1,preds3[1][0], m_w,da_w,c_w)
    # avg_pred_3 = h_blend(pred0,pred1,preds3[2][0], m_w,da_w,c_w)    # LB = 0.854
    # avg_pred_4 = h_blend(pred0,pred1,preds3[3][0], m_w,da_w,c_w)

    # avg_pred_s = [avg_pred_1, avg_pred_2, avg_pred_3, avg_pred_4]
    ...
    #avg_pred_1 = h_blend(pred0,pred1,preds3[0][0], m_w,da_w,c_w)
    avg_pred_2  = h_blend(pred0,pred1,preds3[1][0], m_w,da_w,c_w)
    avg_pred_3  = h_blend(pred0,pred1,preds3[2][0], m_w,da_w,c_w)
    #avg_pred_4 = h_blend(pred0,pred1,preds3[3][0], m_w,da_w,c_w)

    #               avg_pred_1,                         avg_pred_4
    avg_pred_s = [             avg_pred_2, avg_pred_3,           ]   # LB = ? 

    max_idx,max_val = -1,0
    for i, avg_pred in enumerate(avg_pred_s):
        val = find_max_val(avg_pred)
        if val > max_val:
            max_val = val
            max_idx = i                 # max_idx - principle.1
            

    return dataset.le.classes_[avg_pred_s[max_idx].argmax()]
    
                
    #return dataset.le.classes_[avg_pred.argmax()] # original
