...
def train_model(X, y):
    ...
    for fold, (train_idx, val_idx) in enumerate(KFold(n_splits=5, shuffle=True, random_state=SEED).split(X), 1):
        ...
        score = np.sqrt(mean_squared_error(y.iloc[val_idx], model.predict(X.iloc[val_idx])))
        models.append(model)
        scores.append(score)
        print(f"Fold {fold}: {score:.5f}")
    
    print(f"\nCV RMSE: {np.mean(scores):.5f} Â± {np.std(scores):.5f}")
    return models, np.mean(scores)

models, cv_score = train_model(X, y)
joblib.dump(models, 'model.pkl')


# ğŸš€ SUBMISSION GENERATION
