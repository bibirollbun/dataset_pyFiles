...
print("Starting adjusted weighted fusion prediction...")
...
# Adjusted model weights based on performance
...
print("Starting updated weighted fusion prediction...")

def weighted_fusion(models, weights, dataloader):
    models = [m.eval() for m in models]  # Set all models to evaluation mode
    total_preds = []
    total_labels = []
    ...
    with torch.no_grad():
        for images, labels in dataloader:
            images = images.cuda()
            outputs = [m(images) for m in models]  # Get predictions from all models
            fused_output = sum(w * o for w, o in zip(weights, outputs)) / sum(weights)
            preds = torch.argmax(fused_output, dim=1).cpu().numpy()
            total_preds.extend(preds)
            total_labels.extend(labels.numpy())
...
# **New adjusted fusion weights** based on test accuracy
