...
def build_efficientnetb2(num_classes=3):
...
series_models = {
    'Sagittal T1': build_efficientnetb2(),
    'Axial T2': build_efficientnetb2(),
    'Sagittal T2/STIR': build_efficientnetb2()
}
...
# def apply_clahe(image, clipLimit=2.0, tileGridSize=(16, 16)):
#     if len(image.shape) == 3 and image.shape[2] == 3:
#         image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
#     if image.dtype != np.uint8:
#         image = (image * 255).astype(np.uint8) if np.issubdtype(image.dtype, np.floating) else image.astype(np.uint8)
...
#     image = tf.expand_dims(image, axis=-1)  # Add channel for processing
#     image = tf.py_function(opencv_process, [image], tf.float32)
#     image.set_shape([None, None, 1])  # Set dynamic shape
#     image = tf.image.resize(image, [224, 224])
#     image = tf.image.grayscale_to_rgb(image)
#     image = tf.keras.applications.efficientnetb1.preprocess_input(image)
#     return (image, label) if label is not None else image
