...
MAX_LENGTH = 50
NUM_BEAMS = 4
GENERATION_KWARGS = {"max_new_tokens": MAX_LENGTH, "num_beams": NUM_BEAMS}
...
class ImageCaptioningModel:
    """Handles image captioning using the BLIP-2 model."""

    def __init__(self, model_name: str):
        """Initializes the BLIP-2 processor and model."""
    ...
    def process_image(self, image_path: str):
        ...
        inputs = self.processor(images=image, return_tensors="pt").to(self.device, torch.float16 if torch.cuda.is_available() else torch.float32)
        return inputs
    ...
    def generate_caption(self, image_paths: list) -> list:
        ...
        for path in image_paths:
            inputs = self.process_image(path)
