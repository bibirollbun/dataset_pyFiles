...
df_X = pd.read_csv("/kaggle/input/lish-moa/train_features.csv")
df_Y_fine = pd.read_csv("/kaggle/input/hierachicalmoa/train_targets_fine.csv")
df_Y_coarse = pd.read_csv("/kaggle/input/hierachicalmoa/train_targets_coarse.csv")
df_Y_medium = pd.read_csv("/kaggle/input/hierachicalmoa/train_targets_medium.csv")
df_Y = pd.read_csv("/kaggle/input/lish-moa/train_targets_scored.csv")


df_test_X = pd.read_csv("/kaggle/input/lish-moa/test_features.csv")


def preprocess_data_features(df):
    # Preprocess the data
    ...
    df['cp_type'] = df['cp_type'].map(cp_type_label_map)
    df['cp_dose'] = df['cp_dose'].map(dose_label_map)

    return df

def pca_reduce_train_features(df_X):
    gene_features = df_X.filter(regex='^g-')
    cell_features = df_X.filter(regex='^c-')

    # Apply PCA to gene features
    pca_gene = PCA(n_components=0.95)  # Adjust components as needed
    gene_reduced = pca_gene.fit_transform(gene_features)

    pca_cell = PCA(n_components=0.95)  # Adjust components as needed
    cell_reduced = pca_cell.fit_transform(cell_features)

    gene_reduced_df = pd.DataFrame(gene_reduced, columns=[f'PC_Gene_{i+1}' for i in range(gene_reduced.shape[1])])
    cell_reduced_df = pd.DataFrame(cell_reduced, columns=[f'PC_Cell_{i+1}' for i in range(cell_reduced.shape[1])])

    df_X = pd.concat([df_X['cp_type'], df_X['cp_dose'], gene_reduced_df, cell_reduced_df], axis=1)

    return df_X, pca_gene, pca_cell

def pca_reduce_test_features(df_test_X, pca_gene, pca_cell):
    gene_features = df_test_X.filter(regex='^g-')
    cell_features = df_test_X.filter(regex='^c-')

    # Apply PCA to gene features
    gene_reduced = pca_gene.transform(gene_features)

    cell_reduced = pca_cell.transform(cell_features)

    gene_reduced_df = pd.DataFrame(gene_reduced, columns=[f'PC_Gene_{i+1}' for i in range(gene_reduced.shape[1])])
    cell_reduced_df = pd.DataFrame(cell_reduced, columns=[f'PC_Cell_{i+1}' for i in range(cell_reduced.shape[1])])

    df_test_X = pd.concat([df_test_X['cp_type'], df_test_X['cp_dose'], gene_reduced_df, cell_reduced_df], axis=1)

    return df_test_X

df_X = preprocess_data_features(df_X)
df_test_X = preprocess_data_features(df_test_X)
df_X, pca_gene, pca_cell = pca_reduce_train_features(df_X)
df_test_X = pca_reduce_test_features(df_test_X, pca_gene, pca_cell)




clf_coarse = CatBoostClassifier().load_model("/kaggle/input/catboosthierachicalv2/other/default/1/catboost_coarse.cbm")
clf_medium = CatBoostClassifier().load_model("/kaggle/input/catboosthierachicalv2/other/default/1/catboost_medium.cbm")
clf_fine = CatBoostClassifier().load_model("/kaggle/input/catboosthierachicalv2/other/default/1/catboost_fine_augmented.cbm")

# print(clf_coarse.feature_names_)
# print(clf_medium.feature_names_)
# print(clf_fine.feature_names_)


coarse_preds_test = clf_coarse.predict_proba(df_test_X)
medium_preds_test = clf_medium.predict_proba(df_test_X)

coarse_pred_df_test = pd.DataFrame(np.vstack(coarse_preds_test),
                                    columns=[f"Coarse_{col}" for col in df_Y_coarse.columns])

medium_pred_df_test = pd.DataFrame(np.vstack(medium_preds_test),
                                    columns=[f"Medium_{col}" for col in df_Y_medium.columns])

X_test_augmented = pd.concat([df_test_X, coarse_pred_df_test, medium_pred_df_test], axis=1)

# Final fine-level prediction
final_preds = clf_fine.predict_proba(X_test_augmented)
...
sub = sub_1.copy()
sub[target_cols] = sub_1[target_cols] * 0.9 + final_preds * 0.1 #+ sub_2[target_cols] * 0.2 + sub_3[target_cols] * 0.15
# sub[target_cols] = sub_2[target_cols] * 0.4 + sub_3[target_cols] * 0.6

# sub[target_cols] = 0.5 * sub[target_cols] + 0.5 * final_preds

print(sub[target_cols])
# final submission
