...
for Jj in 'a,b,c,d'.split(','):

    df = pd.read_csv(p19+'0.70776.csv')
    
    def rxr(x,mean=df['diagnosed_diabetes'].mean()):
        if x < mean:
            if x > 0.15:
                return x / random.choice([1.000001, 1.000002, 1.000002, 1.000002])
            else:
                return x / random.choice([1.000000, 1.000001, 1.000001, 1.000002])
        else:
            if x < 0.85:
                return x * random.choice([1.000000, 1.000001, 1.000001, 1.000001])
            else:
                return x * random.choice([1.000000, 1.000000, 1.000000, 1.000001])
    
    df['diagnosed_diabetes'] = df['diagnosed_diabetes'].apply(lambda x: rxr(x))

    df.to_csv(f'{work_page}0.7077.{Jj}.csv',index=False)
    display(df.head(4))


pw = work_page

direct_blend([pw+'0.7077.a', p19+'0.70109'], weight=[0.99999, 0.00001], output_file='A.csv')
direct_blend([pw+'0.7077.b', p19+'0.70442'], weight=[0.99997, 0.00003], output_file='B.csv')
direct_blend([pw+'0.7077.c', p19+'0.70651'], weight=[0.99979, 0.00021], output_file='C.csv')
direct_blend([pw+'0.7077.d', p19+'0.70682'], weight=[0.99974, 0.00026], output_file='D.csv')

params = {
     'path'     : work_page,            
     'id_target': ['id',"diagnosed_diabetes"],          
     'type_sort': ['asc/desc',0.30,0.70 ],
     'subwts'   : [w/10_000 for w in [7,-1,-2,-4]],
     'subm'     : [
         {'name': f'A', 'weight':+0.03, 'color':'royalblue'},
         {'name': f'B', 'weight':+0.07, 'color':'orange'   },
         {'name': f'C', 'weight':+0.43, 'color':'green'    },
         {'name': f'D', 'weight':+0.47, 'color':'crimson'  },]
}
