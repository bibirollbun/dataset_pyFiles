...
class cfg(dict):
    ...
    train_img_size = 128  # 32, 64, 128, 224, 256, or try matching to pretrained model's native image size.
    batch_size = 32 
    
    item_tfms = RandomResizedCrop(train_img_size, min_scale=0.5)
    batch_tfms = aug_transforms(mult=3, flip_vert=True, max_rotate=20., max_zoom=1.3, max_warp=0.2, max_lighting=0.3)
...
# Create a learner
cfg.train_img_size = 128 #Train on small images
