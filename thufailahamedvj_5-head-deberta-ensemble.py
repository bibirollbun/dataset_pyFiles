...
DATA_DIR = "/kaggle/input/data-nlp/data"
SEED = 42
BASE_LR = 2e-5
WARMUP_RATIO = 0.06
...
def main():
    ...
    if len(all_test_preds) == len(MODEL_TYPES) and all(len(probs) > 0 for probs in all_test_preds.values()):
        ...
        sub_avg_df.to_csv(Path(OUTPUT_DIR) / "predictions.csv", index=False)
        ...
        sub_voting_df.to_csv(Path(OUTPUT_DIR) / "predictions_voting.csv", index=False)
        ...
        sub_optuna_df.to_csv(Path(OUTPUT_DIR) / "predictions_optuna.csv", index=False)
        
        logger.info("Saved submissions: predictions.csv, predictions_voting.csv, predictions_optuna.csv")
