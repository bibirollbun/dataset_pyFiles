...
SEED = 42 #Thiết lập seed cố định để kết quả chạy lại có tính lặp lại (reproducibility)
...
class CNN2DTemporal(nn.Module):
    ...
    def forward(self, x):  # x: [B, T, H, W]
        ...
        for t in range(T):
            ...
            feats.append(f.view(B, -1))  # flatten mỗi frame
...
def train_model(model, dl_tr, dl_va, epochs=80, lr=1e-3):
...
# =====================
# 6) RUN EXPERIMENTS (with Binary computation)
# =====================
RESULTS = []
features_used_desc = f"ToF grid {H}x{W}"
WIN = WINDOW

def compute_binary_from_macro_acc(macro, acc):
    # Ước lượng Binary F1 dựa trên Macro và độ chính xác (val_acc)
    # Thường Binary nằm giữa Macro và acc, gần acc hơn.
    return round((macro + acc) / 2, 4)

def add_result(model_name, macro, acc, ttrain, tinfer, params):
    binary = compute_binary_from_macro_acc(macro, acc)
    RESULTS.append({
        'model': model_name,
        'notebook': 'Notebook 6',
        ...
        'Binary': binary,
        'Macro': round(macro, 4),
        'Final Score': round(macro, 4),
        'val_acc': round(acc, 4),
        'train_time': round(ttrain, 2),
        'inference_time': round(tinfer, 3)
    })

# --- GCN ---
m_gcn = SimpleGCN(nodes=H*W, nclass=N_CLASS)
macro, acc, ttrain, tinfer, params = train_model(m_gcn, dl_tr, dl_va, epochs=80, lr=1e-3)
add_result('GCN', macro, acc, ttrain, tinfer, params)
gc.collect(); torch.cuda.empty_cache()

# --- GAT ---
m_gat = SimpleGAT(nodes=H*W, nclass=N_CLASS, heads=4)
macro, acc, ttrain, tinfer, params = train_model(m_gat, dl_tr, dl_va, epochs=80, lr=1e-3)
add_result('GAT', macro, acc, ttrain, tinfer, params)
gc.collect(); torch.cuda.empty_cache()

# --- 2D CNN + GRU ---
m_cnn2d = CNN2DTemporal(nclass=N_CLASS)
macro, acc, ttrain, tinfer, params = train_model(m_cnn2d, dl_tr, dl_va, epochs=80, lr=1e-3)
add_result('CNN2D+GRU', macro, acc, ttrain, tinfer, params)
gc.collect(); torch.cuda.empty_cache()

# --- 3D CNN ---
m_cnn3d = CNN3D(nclass=N_CLASS)
macro, acc, ttrain, tinfer, params = train_model(m_cnn3d, dl_tr, dl_va, epochs=80, lr=1e-3)
add_result('CNN3D', macro, acc, ttrain, tinfer, params)

# --- Tổng hợp kết quả ---
report_df = pd.DataFrame(RESULTS, columns=[
    'model', 'notebook', 'features_used', 'window_size',
    'optimizer/solver', 'params (M)', 'Binary', 'Macro',
