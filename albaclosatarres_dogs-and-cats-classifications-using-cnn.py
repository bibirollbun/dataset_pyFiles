...
# Aplicar el split asegurando balance en las clases
for train_idx, valid_idx in split.split(df, df['CATEGORIES']):  
    train_df = df.iloc[train_idx].reset_index(drop=True)
    valid_df = df.iloc[valid_idx].reset_index(drop=True)

# Verificar la distribuci贸n de clases en ambos conjuntos
...
train_datagen = ImageDataGenerator(
    ...
    rescale=1./255,
    shear_range=0.1,
    zoom_range=(0.8, 1.5),        # Acercar y alejar al gato
    width_shift_range=0.1,        # Desplazamiento horizontal
    height_shift_range=0.1,       # Desplazamiento vertical
    brightness_range=(0.5, 1.5),  # Ajuste de iluminaci贸n
