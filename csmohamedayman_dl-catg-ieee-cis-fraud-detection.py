...
dir_path = '/kaggle/input/ieee-fraud-detection'
train1 = pd.read_csv(f'{dir_path}/train_transaction.csv')
train2 = pd.read_csv(f'{dir_path}/train_identity.csv')
test1  = pd.read_csv(f'{dir_path}/test_transaction.csv')
test2  = pd.read_csv(f'{dir_path}/test_identity.csv')
...
test   = pd.merge(test1, test2, on="TransactionID", how="left")
test   = test.drop_duplicates(subset='TransactionID', keep='first')
test.head()
...
n_samples = min(int(1e5), train.shape[0])
...
train, _ = drop_id_feature(train, 'TransactionID')
test, test_id = drop_id_feature(test, 'TransactionID')
...
dropped_mode = set(mode_df[mode_df['mode %'] > 90]['column'])
train = train.drop(columns=list(dropped_mode - set([target_feature])))
test = test.drop(columns=list(dropped_mode - set([target_feature])))


selected_features = ['C2', 'C1', 'V84', 'V292', 'V93', 'ProductCD', 'V92', 'V34', 'C12', 'card3', 
                     'C4', 'V64', 'V32', 'C13', 'V73', 'V74', 'C6', 'V59', 'V69', 'V79', 'V16', 'V31', 
                     'V304', 'V81', 'V17', 'V80', 'V33', 'V15', 'V72', 'V303', 'V58', 'V291', 'V57', 
                     'C14', 'V71', 'V60', 'C8', 'V70', 'V94', 'V90', 'V302', 'V21', 'V22', 'V18', 
                     'V91', 'V63', 'C10', 'V85', 'C7', 'C11']
train = train[selected_features+[target_feature]]
...
selected_oversamplying_features = set(oversamplying_features) & (set(correlations_df.head(1)['Feature']) | set(correlations_df.tail(1)['Feature'])) & set(['isFraud'])
