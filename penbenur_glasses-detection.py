...
df['file']="/kaggle/input/applications-of-deep-learning-wustl-fall-2023/"+train_df['file']
...
label_counts = df['glasses'].value_counts()
...
def show_sample_images(df, label, num_images=5):
    sample_images = df[df['glasses'] == label]['file'].sample(num_images).values
...
show_sample_images(df, 0)
show_sample_images(df, 1)


X_train, X_test, y_train, y_test = train_test_split(df['file'], df['glasses'], test_size=0.2, random_state=42)
