...
%%capture
!pip install --no-index /kaggle/input/hf-libraries/bitsandbytes/bitsandbytes-0.45.3-py3-none-manylinux_2_24_x86_64.whl
...
def main(config: str = "/kaggle/working/biencoder.yaml") -> None:
    ...
    df["pred_ids"] = df["pred_ids"].apply(lambda x: ' '.join(x.split()[:100])) 
    # Clean up memory
    del model
    gc.collect()
    torch.cuda.empty_cache()
    ...
    # Save results
    df.to_parquet(Path(cfg.save_dir) / params.save_name, index=False)
...
def main(
    ...
    sampling_params = SamplingParams(temperature=0.0, max_tokens=1, logprobs=params.topk)
    ...
    for output in outputs:
        ...
        score_dict = {i: 0.0 for i in range(params.num_choice)}
...
input_dir: "/kaggle/input/eedi-mining-misconceptions-in-mathematics"
inference_listwise:
  model_name: "/kaggle/input/qwen2.5-14b-eedi/transformers/default/1"
...
def main(
    ...
    # Generate predictions
    sampling_params = SamplingParams(n=1,temperature=0.0, max_tokens=1, logprobs=params.topk)
    outputs = model.generate(tmp_df["prompt"].tolist(), sampling_params)
...
# df = pd.read_parquet("/kaggle/working/data_listwise_with_Qwen2.5_Math_7B_Instruct_stage_1.parquet")

# # Chỉ giữ lại các cột cần thiết và merge theo QuestionId_Answer
# merged = df[['QuestionId_Answer', 'pred_ids']]
# merged['pred_ids'] = merged['pred_ids'].apply(lambda x: ' '.join(x.split()[:25]))


#1+2
import pandas as pd

df = pd.read_parquet("/kaggle/working/data_listwise_with_Qwen2.5_Math_7B_Instruct_stage_2.parquet")
...
# df_stage_3 = pd.read_parquet("/kaggle/working/data_listwise_with_Qwen2.5_Math_7B_Instruct_stage_3.parquet")

# df_stage_2 =  df_stage_2[['QuestionId_Answer', 'pred_ids']].rename(columns={'pred_ids': 'pred_stage_2'})
# df_stage_3 =  df_stage_3[['QuestionId_Answer', 'pred_ids']].rename(columns={'pred_ids': 'pred_stage_3'})

# merged = pd.merge(df_stage_3, df_stage_2, on='QuestionId_Answer', how='inner')

# def combine_predictions(row):
#     stage3_ids = row['pred_stage_3'].split()[:9]
#     stage2_ids = row['pred_stage_2'].split()[9:25]
