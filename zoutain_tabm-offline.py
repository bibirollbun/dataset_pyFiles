...
train = pd.read_csv('E:\Python_Projects\CIBMTR\data\\train.csv')
test = pd.read_csv('E:\Python_Projects\CIBMTR\data\\test.csv')
...
uniques_dict = {c: combined[c].unique() for c in CATS}
joblib.dump(uniques_dict, 'tabm_uniques_dict.pkl')
...
joblib.dump({
    ...
    'arch_type': arch_type
}, 'tabm_preprocessing_params.pkl')

# Save scaler
joblib.dump(scaler, 'tabm_scaler.pkl')
...
for i, (train_index, val_index) in enumerate(kf.split(train[FEATURES])):
	...
	for epoch in range(100):
		...
		if val_cindex > best["val"]:
			...
			joblib.dump(bins, f'tabm_bins_fold_{i}.pkl')  # Instead of np.save()
...
sub = pd.read_csv("E:\Python_Projects\CIBMTR\data\sample_submission.csv")
