...
d=pd.read_csv("/kaggle/input/playground-series-s5e12/train.csv")
...
d['diagnosed_diabetes'].value_counts().plot(kind='bar')


d1=pd.read_csv("/kaggle/input/playground-series-s5e12/test.csv")
...
X =d.drop(['id', 'diagnosed_diabetes'], axis=1)
y =d['diagnosed_diabetes']
...
test_pred = l.predict_proba(X_val)[:, 1]
